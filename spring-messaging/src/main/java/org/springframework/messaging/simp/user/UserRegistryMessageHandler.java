...
/**
 * {@code MessageHandler} that handles user registry broadcasts from other application servers and
 * periodically broadcasts the content of the local user registry.
 *
...
public class UserRegistryMessageHandler
   ...
   */
  public UserRegistryMessageHandler(
  ...
  /**
   * Configure the amount of time (in milliseconds) before a remote user registry snapshot is
   * considered expired.
   *
    ...
    public void run() {
      try {
        ...
        brokerTemplate.convertAndSend(
            getBroadcastDestination(), payload, accessor.getMessageHeaders());
      } finally {
