...
/**
 * A base class for working with message headers in simple messaging protocols that support basic
 * messaging patterns. Provides uniform access to specific values common across protocols such as a
 * destination, message type (e.g. publish, subscribe, etc), session id, and others.
 *
 * <p>Use one of the static factory method in this class, then call getters and setters, and at the
 * end if necessary call {@link #toMap()} to obtain the updated headers.
 *
...
public class SimpMessageHeaderAccessor extends NativeMessageHeaderAccessor {
  ...
  /**
   * A header for internal use with "user" destinations where we need to restore the destination
   * prior to sending messages to clients.
   */
  public static final String ORIGINAL_DESTINATION = "simpOrigDestination";

  /**
   * A header that indicates to the broker that the sender will ignore errors. The header is simply
   * checked for presence or absence.
   */
  public static final String IGNORE_ERROR = "simpIgnoreError";

  @Nullable private Consumer<Principal> userCallback;

  /**
   * A constructor for creating new message headers. This constructor is protected. See factory
   * methods in this and sub-classes.
   */
  protected SimpMessageHeaderAccessor(
      SimpMessageType messageType, @Nullable Map<String, List<String>> externalSourceHeaders) {
  ...
  /**
   * A constructor for accessing and modifying existing message headers. This constructor is
   * protected. See factory methods in this and sub-classes.
   */
  ...
  /**
   * Provide a callback to be invoked if and when {@link #setUser(Principal)} is called. This is
   * used internally on the inbound channel to detect token-based authentications through an
   * interceptor.
   *
  ...
  /**
   * Create an instance with {@link org.springframework.messaging.simp.SimpMessageType} {@code
   * MESSAGE}.
   */
