...
/**
 * A {@code MessageHeaderAccessor} to use when creating a {@code Message} from a decoded STOMP
 * frame, or when encoding a {@code Message} to a STOMP frame.
 *
 * <p>When created from STOMP frame content, the actual STOMP headers are stored in the native
 * header sub-map managed by the parent class {@link
 * org.springframework.messaging.support.NativeMessageHeaderAccessor} while the parent class {@link
 * SimpMessageHeaderAccessor} manages common processing headers some of which are based on STOMP
 * headers (e.g. destination, content-type, etc).
 *
 * <p>An instance of this class can also be created by wrapping an existing {@code Message}. That
 * message may have been created with the more generic {@link
 * org.springframework.messaging.simp.SimpMessageHeaderAccessor} in which case STOMP headers are
 * created from common processing headers. In this case it is also necessary to invoke either {@link
 * #updateStompCommandAsClientMessage()} or {@link #updateStompCommandAsServerMessage()} if sending
 * a message and depending on whether a message is sent to a client or the message broker.
 *
...
public class StompHeaderAccessor extends SimpMessageHeaderAccessor {
  ...
  StompHeaderAccessor(
      StompCommand command, @Nullable Map<String, List<String>> externalSourceHeaders) {
  ...
  /**
   * A constructor for accessing and modifying existing message headers. Note that the message
   * headers may not have been created from a STOMP frame but may have rather originated from using
   * the more generic {@link org.springframework.messaging.simp.SimpMessageHeaderAccessor}.
   */
  ...
  private String appendPayload(Object payload) {
    if (payload.getClass() != byte[].class) {
      throw new IllegalStateException(
          "Expected byte array payload but got: "
              + ClassUtils.getQualifiedName(payload.getClass()));
    }
    ...
    return (bytes.length < 80)
        ...
        : contentType
            + " payload="
            + new String(Arrays.copyOf(bytes, 80), charset)
            + "...(truncated)";
  }
  ...
  public static StompHeaderAccessor create(
      StompCommand command, Map<String, List<String>> headers) {
    return new StompHeaderAccessor(command, headers);
  }

  /**
   * Create headers for a heartbeat. While a STOMP heartbeat frame does not have headers, a session
   * id is needed for processing purposes at a minimum.
   */
