...
/**
 * Submit messages to an {@link ExecutorSubscribableChannel}, one at a time. The channel must have
 * been configured with {@link #configureOutboundChannel}.
 *
...
class OrderedMessageSender implements MessageChannel {
  ...
  private void addCompletionCallback(Message<?> msg) {
    ...
    Assert.isTrue(
        accessor != null && accessor.isMutable(), "Expected mutable SimpMessageHeaderAccessor");
    accessor.setHeader(COMPLETION_TASK_HEADER, (Runnable) this::sendNextMessage);
  }

  /**
   * Install or remove an {@link ExecutorChannelInterceptor} that invokes a completion task once the
   * message is handled.
   *
   * @param channel the channel to configure
   * @param preservePublishOrder whether preserve order is on or off based on which an interceptor
  ...
  static void configureOutboundChannel(MessageChannel channel, boolean preservePublishOrder) {
    if (preservePublishOrder) {
      Assert.isInstanceOf(
          ExecutorSubscribableChannel.class,
          channel,
      ...
      ExecutorSubscribableChannel execChannel = (ExecutorSubscribableChannel) channel;
      execChannel.getInterceptors().stream()
          .filter(i -> i instanceof CallbackInterceptor)
          .findFirst()
