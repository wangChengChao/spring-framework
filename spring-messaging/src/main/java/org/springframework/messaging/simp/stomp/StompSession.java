...
/**
 * Represents a STOMP session with operations to send messages, create subscriptions and receive
 * messages on those subscriptions.
 *
...
public interface StompSession {
  ...
  /**
   * When enabled, a receipt header is automatically added to future {@code send} and {@code
   * subscribe} operations on this session, which causes the server to return a RECEIPT. An
   * application can then use the {@link StompSession.Receiptable Receiptable} returned from the
   * operation to track the receipt.
   *
   * <p>A receipt header can also be added manually through the overloaded methods that accept
   * {@code StompHeaders}.
   */
  void setAutoReceipt(boolean enabled);

  /**
   * Send a message to the specified destination, converting the payload to a {@code byte[]} with
   * the help of a {@link org.springframework.messaging.converter.MessageConverter
   * MessageConverter}.
   *
  ...
  /**
   * An overloaded version of {@link #send(String, Object)} with full {@link StompHeaders} instead
   * of just a destination. The headers must contain a destination and may also have other headers
   * such as "content-type" or custom headers for the broker to propagate to subscribers, or
   * broker-specific, non-standard headers.
   *
  ...
  /**
   * Subscribe to the given destination by sending a SUBSCRIBE frame and handle received messages
  ...
  /**
   * An overloaded version of {@link #subscribe(String, StompFrameHandler)} with full {@link
  ...
  /**
   * Send an acknowledgement whether a message was consumed or not resulting in an ACK or NACK frame
   * respectively.
   *
   * <p><strong>Note:</strong> to use this when subscribing you must set the {@link
   * StompHeaders#setAck(String) ack} header to "client" or "client-individual" in order ot use
   * this.
   *
  ...
  /**
   * An overloaded version of {@link #acknowledge(String, boolean)} with full {@link StompHeaders}
   * instead of just a {@code messageId}.
   *
  ...
  interface Receiptable {
    ...
    /**
     * Return the receipt id, or {@code null} if the STOMP frame for which the handle was returned
     * did not have a "receipt" header.
     */
    @Nullable
    String getReceiptId();
  ...
  interface Subscription extends Receiptable {
    ...
    /**
     * Alternative to {@link #unsubscribe()} with additional custom headers to send to the server.
     *
