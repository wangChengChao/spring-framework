...
/**
 * A "simple" message broker that recognizes the message types defined in {@link SimpMessageType},
 * keeps track of subscriptions with the help of a {@link SubscriptionRegistry} and sends messages
 * to subscribers.
 *
...
public class SimpleBrokerMessageHandler extends AbstractBrokerMessageHandler {
  ...
  @Nullable private String selectorHeaderName = "selector";

  @Nullable private TaskScheduler taskScheduler;

  @Nullable private long[] heartbeatValue;
  ...
  @Nullable private ScheduledFuture<?> heartbeatFuture;

  /**
   * Create a SimpleBrokerMessageHandler instance with the given message channels and destination
   * prefixes.
   *
   * @param clientInboundChannel the channel for receiving messages from clients (e.g. WebSocket
   *     clients)
   * @param clientOutboundChannel the channel for sending messages to clients (e.g. WebSocket
   *     clients)
   ...
   *
   * <p><strong>Note</strong> that when a custom PathMatcher is configured via {@link
   * #setPathMatcher}, if the custom registry is not an instance of {@link
   * DefaultSubscriptionRegistry}, the provided PathMatcher is not used and must be configured
   * directly on the custom registry.
   */
  ...
  /**
   * When configured, the given PathMatcher is passed down to the underlying SubscriptionRegistry to
   * use for matching destination to subscriptions.
   *
  ...
  private void initPathMatcherToUse() {
    if (this.pathMatcher != null
        && this.subscriptionRegistry instanceof DefaultSubscriptionRegistry) {
   ...
   *
   * <p>With a standard {@link DefaultSubscriptionRegistry}, the default cache limit is 1024.
   *
   * @since 4.3.2
   * @see #setSubscriptionRegistry
  ...
  /**
   * Configure the name of a header that a subscription message can have for the purpose of
   * filtering messages matched to the subscription. The header value is expected to be a Spring EL
   * boolean expression to be applied to the headers of messages matched to the subscription.
   *
   * <p>For example:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>By default this is set to "selector". You can set it to a different name, or to {@code null}
  ...
  private void initSelectorHeaderNameToUse() {
    if (this.subscriptionRegistry instanceof DefaultSubscriptionRegistry) {
      ((DefaultSubscriptionRegistry) this.subscriptionRegistry)
          .setSelectorHeaderName(this.selectorHeaderName);
    }
  }

  /**
   * Configure the {@link org.springframework.scheduling.TaskScheduler} to use for providing
   * heartbeat support. Setting this property also sets the {@link #setHeartbeatValue
  ...
  /**
   * Configure the value for the heart-beat settings. The first number represents how often the
   * server will write or send a heartbeat. The second is how often the client should write. 0 means
   * no heartbeats.
   *
   * <p>By default this is set to "0, 0" unless the {@link #setTaskScheduler taskScheduler} in which
   * case the default becomes "10000,10000" (in milliseconds).
   *
   * @since 4.2
   */
  ...
  /**
   * Configure a {@link MessageHeaderInitializer} to apply to the headers of all messages sent to
    ...
    if (this.taskScheduler != null) {
      ...
      if (interval > 0) {
        this.heartbeatFuture =
            this.taskScheduler.scheduleWithFixedDelay(new HeartbeatTask(), interval);
      }
    } else {
      Assert.isTrue(
          getHeartbeatValue() == null
  ...
  protected void handleMessageInternal(Message<?> message) {
    ...
    if (SimpMessageType.MESSAGE.equals(messageType)) {
      ...
      if (sessionId != null) {
        ...
        this.sessions.put(
            sessionId, new SessionInfo(sessionId, user, outChannel, heartbeatIn, heartbeatOut));
        SimpMessageHeaderAccessor connectAck =
  ...
  private void handleDisconnect(
      String sessionId, @Nullable Principal user, @Nullable Message<?> origMessage) {
  ...
  private static class SessionInfo {
    ...
    @Nullable private final Principal user;
    ...
    public SessionInfo(
        ...
        @Nullable long[] clientHeartbeat,
        @Nullable long[] serverHeartbeat) {
      ...
      if (clientHeartbeat != null && serverHeartbeat != null) {
        this.readInterval =
            (clientHeartbeat[0] > 0 && serverHeartbeat[1] > 0
                ? Math.max(clientHeartbeat[0], serverHeartbeat[1]) * HEARTBEAT_MULTIPLIER
                : 0);
        this.writeInterval =
            (clientHeartbeat[1] > 0 && serverHeartbeat[0] > 0
                ? Math.max(clientHeartbeat[1], serverHeartbeat[0])
                : 0);
      } else {
        this.readInterval = 0;
  ...
  private class HeartbeatTask implements Runnable {
    ...
    public void run() {
      ...
      for (SessionInfo info : sessions.values()) {
        ...
        if (info.getWriteInterval() > 0
            && (now - info.getLastWriteTime()) > info.getWriteInterval()) {
          SimpMessageHeaderAccessor accessor =
          ...
          info.getClientOutboundChannel()
              .send(MessageBuilder.createMessage(EMPTY_PAYLOAD, headers));
        }
      }
    }
  }
