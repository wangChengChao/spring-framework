...
/**
 * A {@link HandlerMethodReturnValueHandler} for sending to destinations specified in a {@link
 * SendTo} or {@link SendToUser} method-level annotations.
 *
 * <p>The value returned from the method is converted, and turned to a {@link Message} and sent
 * through the provided {@link MessageChannel}. The message is then enriched with the session id of
 * the input message as well as the destination from the annotation(s). If multiple destinations are
 * specified, a copy of the message is sent to each destination.
 *
...
public class SendToMethodReturnValueHandler implements HandlerMethodReturnValueHandler {
  ...
  public SendToMethodReturnValueHandler(
  ...
  /**
   * Configure a default prefix to add to message destinations in cases where a method is not
   * annotated with {@link SendTo @SendTo} or does not specify any destinations through the
  ...
  /**
   * Configure a default prefix to add to message destinations in cases where a method is annotated
   * with {@link SendToUser @SendToUser} but does not specify any destinations through the
   * annotation's value attribute.
   *
  ...
  public void handleReturnValue(
    ...
    if (sendToUser != null) {
      ...
      String[] destinations =
          getTargetDestinations(sendToUser, message, this.defaultUserDestinationPrefix);
    ...
    if (sendTo != null || sendToUser == null) {
      ...
      for (String destination : destinations) {
        ...
        this.messagingTemplate.convertAndSend(
            destination, returnValue, createHeaders(sessionId, returnType));
      }
    }
  }

  private DestinationHelper getDestinationHelper(
      MessageHeaders headers, MethodParameter returnType) {
    SendToUser m1 =
        AnnotatedElementUtils.findMergedAnnotation(returnType.getExecutable(), SendToUser.class);
    SendTo m2 =
        AnnotatedElementUtils.findMergedAnnotation(returnType.getExecutable(), SendTo.class);
    if ((m1 != null && !ObjectUtils.isEmpty(m1.value()))
        || (m2 != null && !ObjectUtils.isEmpty(m2.value()))) {
      return new DestinationHelper(headers, m1, m2);
    }

    SendToUser c1 =
        AnnotatedElementUtils.findMergedAnnotation(
            returnType.getDeclaringClass(), SendToUser.class);
    SendTo c2 =
        AnnotatedElementUtils.findMergedAnnotation(returnType.getDeclaringClass(), SendTo.class);
    if ((c1 != null && !ObjectUtils.isEmpty(c1.value()))
        || (c2 != null && !ObjectUtils.isEmpty(c2.value()))) {
      return new DestinationHelper(headers, c1, c2);
    }

    return (m1 != null || m2 != null
        ? new DestinationHelper(headers, m1, m2)
  ...
  protected String getUserName(Message<?> message, MessageHeaders headers) {
    ...
    if (principal != null) {
      return (principal instanceof DestinationUserNameProvider
          ? ((DestinationUserNameProvider) principal).getDestinationUserName()
          : principal.getName());
    }
    return null;
  }

  protected String[] getTargetDestinations(
      @Nullable Annotation annotation, Message<?> message, String defaultPrefix) {
    if (annotation != null) {
  ...
  private class DestinationHelper {
    ...
    @Nullable private final SendTo sendTo;

    @Nullable private final SendToUser sendToUser;

    public DestinationHelper(
        MessageHeaders headers, @Nullable SendToUser sendToUser, @Nullable SendTo sendTo) {
