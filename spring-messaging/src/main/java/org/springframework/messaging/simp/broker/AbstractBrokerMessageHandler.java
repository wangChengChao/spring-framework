...
public abstract class AbstractBrokerMessageHandler
  ...
  private final BrokerAvailabilityEvent notAvailableEvent =
      new BrokerAvailabilityEvent(false, this);
  ...
  private final ChannelInterceptor unsentDisconnectInterceptor =
      new UnsentDisconnectChannelInterceptor();
  ...
  public AbstractBrokerMessageHandler(
      ...
      MessageChannel outboundChannel,
      SubscribableChannel brokerChannel,
      @Nullable Collection<String> destinationPrefixes) {
    ...
    destinationPrefixes =
        (destinationPrefixes != null ? destinationPrefixes : Collections.emptyList());
  ...
  public void start() {
    synchronized (this.lifecycleMonitor) {
      ...
      if (this.clientInboundChannel instanceof InterceptableChannel) {
        ((InterceptableChannel) this.clientInboundChannel)
            .addInterceptor(0, this.unsentDisconnectInterceptor);
      }
      startInternal();
      this.running = true;
      logger.info("Started.");
  ...
  public void stop() {
    synchronized (this.lifecycleMonitor) {
      ...
      if (this.clientInboundChannel instanceof InterceptableChannel) {
        ((InterceptableChannel) this.clientInboundChannel)
            .removeInterceptor(this.unsentDisconnectInterceptor);
      }
      this.running = false;
      logger.info("Stopped.");
    }
   ...
   *
   * <p>Note that even when this message handler is running the {@link #isBrokerAvailable()} flag
   * may still independently alternate between being on and off depending on the concrete sub-class
   * implementation.
   */
  @Override
   ...
   *
   * <p>Note that this is in addition to the {@link #isRunning()} flag, which indicates whether this
   * message handler is running. In other words the message handler must first be running and then
   * the {@code #isBrokerAvailable()} flag may still independently alternate between being on and
   * off depending on the concrete sub-class implementation.
   *
   * <p>Application components may implement {@code
   * org.springframework.context.ApplicationListener&lt;BrokerAvailabilityEvent&gt;} to receive
   * notifications when broker becomes available and unavailable.
   */
  ...
  /**
   * Get the MessageChannel to use for sending messages to clients, possibly a per-session wrapper
  ...
  protected MessageChannel getClientOutboundChannelForSession(String sessionId) {
    return this.preservePublishOrder
        ? new OrderedMessageSender(getClientOutboundChannel(), logger)
