 ...
 *
 * <p>Also provides methods for sending messages to a user. See {@link
 * org.springframework.messaging.simp.user.UserDestinationResolver UserDestinationResolver} for more
 * on user destinations.
 *
...
public class SimpMessagingTemplate extends AbstractMessageSendingTemplate<String>
   ...
   * through the {@code SimpMessagingTemplate}.
   *
  ...
  /**
   * If the headers of the given message already contain a {@link
   * org.springframework.messaging.simp.SimpMessageHeaderAccessor#DESTINATION_HEADER
   * SimpMessageHeaderAccessor#DESTINATION_HEADER} then the message is sent without further changes.
   *
   * <p>If a destination header is not already present ,the message is sent to the configured {@link
   * #setDefaultDestination(Object) defaultDestination} or an exception an {@code
   * IllegalStateException} is raised if that isn't configured.
   *
  ...
  protected void doSend(String destination, Message<?> message) {
    ...
    if (simpAccessor != null) {
      if (simpAccessor.isMutable()) {
        ...
        simpAccessor =
            (SimpMessageHeaderAccessor) MessageHeaderAccessor.getMutableAccessor(message);
  ...
  public void convertAndSendToUser(
      String user, String destination, Object payload, @Nullable Map<String, Object> headers)
      throws MessagingException {
  ...
  @Override
  public void convertAndSendToUser(
      String user, String destination, Object payload, @Nullable MessagePostProcessor postProcessor)
      throws MessagingException {
  ...
  public void convertAndSendToUser(
    ...
    super.convertAndSend(
        this.destinationPrefix + user + destination, payload, headers, postProcessor);
  }

  /**
   * Creates a new map and puts the given headers under the key {@link
   * NativeMessageHeaderAccessor#NATIVE_HEADERS NATIVE_HEADERS NATIVE_HEADERS NATIVE_HEADERS}.
   * effectively treats the input header map as headers to be sent out to the destination.
   *
   * <p>However if the given headers already contain the key {@code NATIVE_HEADERS NATIVE_HEADERS}
   * then the same headers instance is returned without changes.
   *
   * <p>Also if the given headers were prepared and obtained with {@link
   * SimpMessageHeaderAccessor#getMessageHeaders()} then the same headers instance is also returned
   * without changes.
   */
  @Override
    ...
    if (headers instanceof MessageHeaders) {
      SimpMessageHeaderAccessor accessor =
          MessageHeaderAccessor.getAccessor(
              (MessageHeaders) headers, SimpMessageHeaderAccessor.class);
      if (accessor != null) {
        return headers;
      }
    }
