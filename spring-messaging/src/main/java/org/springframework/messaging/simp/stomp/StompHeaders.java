 ...
 *
 * <ul>
 *   <li>{@link #getFirst(String)} return the first value for a header name
 *   <li>{@link #add(String, String)} add to the list of values for a header name
 *   <li>{@link #set(String, String)} set a header name to a single string value
 * </ul>
 *
...
public class StompHeaders implements MultiValueMap<String, String>, Serializable {
  ...
  public void setContentType(@Nullable MimeType mimeType) {
    if (mimeType != null) {
      ...
      Assert.isTrue(
          !mimeType.isWildcardSubtype(), "'Content-Type' cannot contain wildcard subtype '*'");
  ...
  /**
   * Set the accept-version header. Must be one of "1.1", "1.2", or both. Applies to the CONNECT
   * frame.
   *
   * @since 5.0.7
   */
  ...
  public void setHeartbeat(@Nullable long[] heartbeat) {
    if (heartbeat == null || heartbeat.length != 2) {
      throw new IllegalArgumentException(
          "Heart-beat array must be of length 2, not "
  ...
  /**
   * Whether heartbeats are enabled. Returns {@code false} if {@link #setHeartbeat} is set to "0,0",
  ...
  /**
   * Set the ack header to one of "auto", "client", or "client-individual". Applies to the SUBSCRIBE
   * and MESSAGE frames.
   */
