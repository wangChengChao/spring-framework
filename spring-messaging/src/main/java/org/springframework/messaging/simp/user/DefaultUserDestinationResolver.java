...
/**
 * A default implementation of {@code UserDestinationResolver} that relies on a {@link
 * SimpUserRegistry} to find active sessions for a user.
 *
 * <p>When a user attempts to subscribe, e.g. to "/user/queue/position-updates", the "/user" prefix
 * is removed and a unique suffix added based on the session id, e.g.
 * "/queue/position-updates-useri9oqdfzo" to ensure different users can subscribe to the same
 * logical destination without colliding.
 *
 * <p>When sending to a user, e.g. "/user/{username}/queue/position-updates", the "/user/{username}"
 * prefix is removed and a suffix based on active session id's is added, e.g.
...
public class DefaultUserDestinationResolver implements UserDestinationResolver {
  ...
  /**
   * The prefix used to identify user destinations. Any destinations that do not start with the
   * given prefix are not be resolved.
   *
  ...
  /**
   * Use this property to indicate whether the leading slash from translated user destinations
   * should be removed or not. This depends on the destination prefixes the message broker is
   * configured with.
   *
   * <p>By default this is set to {@code false}, i.e. "do not change the target destination",
   * although {@link org.springframework.messaging.simp.config.AbstractMessageBrokerConfiguration
   * AbstractMessageBrokerConfiguration} may change that to {@code true} if the configured
   * destinations do not have a leading slash.
   *
  ...
  /**
   * Provide the {@code PathMatcher} in use for working with destinations which in turn helps to
   * determine whether the leading slash should be kept in actual destinations after removing the
   * {@link #setUserDestinationPrefix userDestinationPrefix}.
   *
   * <p>By default actual destinations have a leading slash, e.g. {@code /queue/position-updates}
   * which makes sense with brokers that support destinations with slash as separator. When a {@code
   * PathMatcher} is provided that supports an alternative separator, then resulting destinations
   * won't have a leading slash, e.g. {@code jms.queue.position-updates}.
   *
   * @param pathMatcher the PathMatcher used to work with destinations
   * @since 4.3
   * @deprecated as of 4.3.14 this property is no longer used and is replaced by {@link
   *     #setRemoveLeadingSlash(boolean)} that indicates more explicitly whether to keep the leading
   *     slash which may or may not be the case regardless of how the {@code PathMatcher} is
   *     configured.
   */
  @Deprecated
  ...
  public UserDestinationResult resolveDestination(Message<?> message) {
    ...
    for (String sessionId : parseResult.getSessionIds()) {
      ...
      String targetDestination =
          getTargetDestination(sourceDestination, actualDestination, sessionId, user);
  ...
  private ParseResult parseSubscriptionMessage(Message<?> message, String sourceDestination) {
    ...
    return new ParseResult(
        sourceDestination, actualDestination, sourceDestination, sessionIds, user);
  }
  ...
  /**
   * This method determines how to translate the source "user" destination to an actual target
   * destination for the given active user session.
   *
  ...
  protected String getTargetDestination(
      String sourceDestination, String actualDestination, String sessionId, @Nullable String user) {
