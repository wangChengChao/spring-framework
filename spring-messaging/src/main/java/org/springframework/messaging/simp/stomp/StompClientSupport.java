 ...
 *
 * <p>Subclasses can connect over WebSocket or TCP using any library. When creating a new
 * connection, a subclass can create an instance of @link DefaultStompSession} which extends {@link
 * org.springframework.messaging.tcp.TcpConnectionHandler} whose lifecycle methods the subclass must
 * then invoke.
 *
 * <p>In effect, {@code TcpConnectionHandler} and {@code TcpConnection} are the contracts that any
 * subclass must adapt to while using {@link StompEncoder} and {@link StompDecoder} to encode and
 * decode STOMP messages.
 *
...
public abstract class StompClientSupport {
  ...
  /**
   * Set the {@link MessageConverter} to use to convert the payload of incoming and outgoing
   * messages to and from {@code byte[]} based on object type and the "content-type" header.
   *
   ...
   *
   * <p><strong>Note:</strong> Some transports have built-in support to work with heartbeats and
   * therefore do not require a TaskScheduler. Receipts however, if needed, do require a
   * TaskScheduler to be configured.
   *
  ...
  /**
   * Configure the default value for the "heart-beat" header of the STOMP CONNECT frame. The first
   * number represents how often the client will write or send a heart-beat. The second is how often
   * the server should write. A value of 0 means no heart-beats.
   *
   * <p>By default this is set to "10000,10000" but subclasses may override that default and for
   * example set it to "0,0" if they require a TaskScheduler to be configured first.
   *
   ...
   *
   * <p>Returns {@code false} if {@link #setDefaultHeartbeat defaultHeartbeat} is set to "0,0", and
   * {@code true} otherwise.
   */
