...
/**
 * A {@link org.springframework.messaging.MessageHandler} that handles messages by forwarding them
 * to a STOMP broker.
 *
 * <p>For each new {@link SimpMessageType#CONNECT CONNECT} message, an independent TCP connection to
 * the broker is opened and used exclusively for all messages from the client that originated the
 * CONNECT message. Messages from the same client are identified through the session id message
 * header. Reversely, when the STOMP broker sends messages back on the TCP connection, those
 * messages are enriched with the session id of the client and sent back downstream through the
 * {@link MessageChannel} provided to the constructor.
 *
 * <p>This class also automatically opens a default "system" TCP connection to the message broker
 * that is used for sending messages that originate from the server application (as opposed to from
 * a client). Such messages are not associated with any client and therefore do not have a session
 * id header. The "system" connection is effectively shared and cannot be used to receive messages.
 * Several properties are provided to configure the "system" connection including:
 *
 * <ul>
 *   <li>{@link #setSystemLogin}
 *   <li>{@link #setSystemPasscode}
 *   <li>{@link #setSystemHeartbeatSendInterval}
 *   <li>{@link #setSystemHeartbeatReceiveInterval}
 * </ul>
 *
...
public class StompBrokerRelayMessageHandler extends AbstractBrokerMessageHandler {
  ...
  /**
   * Heartbeat starts once CONNECTED frame with heartbeat settings is received. If CONNECTED doesn't
   * arrive within a minute, we'll close the connection.
   */
  private static final int MAX_TIME_TO_CONNECTED_FRAME = 60 * 1000;

  private static final byte[] EMPTY_PAYLOAD = new byte[0];

  private static final ListenableFutureTask<Void> EMPTY_TASK =
      new ListenableFutureTask<>(new VoidCallable());
  ...
  static {
    ...
    HEARTBEAT_MESSAGE =
        MessageBuilder.createMessage(StompDecoder.HEARTBEAT_PAYLOAD, accessor.getMessageHeaders());
  }
  ...
  /**
   * Create a StompBrokerRelayMessageHandler instance with the given message channels and
   ...
   * @param destinationPrefixes the broker supported destination prefixes; destinations that do not
   *     match the given prefix are ignored.
   */
  public StompBrokerRelayMessageHandler(
  ...
  /**
   * Set the login to use when creating connections to the STOMP broker on behalf of connected
   * clients.
   *
  ...
  /**
   * Return the configured login to use for connections to the STOMP broker on behalf of connected
   * clients.
   *
   * @see #getSystemLogin()
   */
  ...
  /**
   * Set the client passcode to use to create connections to the STOMP broker on behalf of connected
   * clients.
   *
   * <p>By default this is set to "guest".
   *
  ...
  /**
   * Return the configured passcode to use for connections to the STOMP broker on behalf of
   * connected clients.
   *
  ...
  /**
   * Set the login for the shared "system" connection used to send messages to the STOMP broker from
   * within the application, i.e. messages not associated with a specific client session (e.g.
   * REST/HTTP request handling method).
   *
   * <p>By default this is set to "guest".
   */
  ...
  /**
   * Set the passcode for the shared "system" connection used to send messages to the STOMP broker
   * from within the application, i.e. messages not associated with a specific client session (e.g.
   * REST/HTTP request handling method).
   *
   * <p>By default this is set to "guest".
   */
  ...
  /**
   * Set the interval, in milliseconds, at which the "system" connection will, in the absence of any
   * other data being sent, send a heartbeat to the STOMP broker. A value of zero will prevent
   * heartbeats from being sent to the broker.
   *
  ...
  /**
   * Return the interval, in milliseconds, at which the "system" connection will send heartbeats to
   * the STOMP broker.
   */
  ...
  /**
   * Set the maximum interval, in milliseconds, at which the "system" connection expects, in the
   * absence of any other data, to receive a heartbeat from the STOMP broker. A value of zero will
   * configure the connection to expect not to receive heartbeats from the broker.
   *
   * <p>The default value is 10000.
   *
   * <p>See class-level documentation for more information on the "system" connection.
  ...
  /**
   * Return the interval, in milliseconds, at which the "system" connection expects to receive
   * heartbeats from the STOMP broker.
   */
  ...
  /**
   * Configure one more destinations to subscribe to on the shared "system" connection along with
   * MessageHandler's to handle received messages.
   *
   * <p>This is for internal use in a multi-application server scenario where servers forward
   * messages to each other (e.g. unresolved user destinations).
   *
  ...
  /**
   * Set the value of the "host" header to use in STOMP CONNECT frames. When this property is
   * configured, a "host" header will be added to every STOMP frame sent to the STOMP broker. This
   * may be useful for example in a cloud environment where the actual host to which the TCP
   * connection is established is different from the host providing the cloud-based STOMP service.
   *
   ...
   *
   * <p><strong>Note:</strong> when this property is used, any {@link #setRelayHost(String) host} or
   * {@link #setRelayPort(int) port} specified are effectively ignored.
   */
  ...
  /**
   * Get the configured TCP client (never {@code null} unless not configured invoked and this method
   * is invoked before the handler is started and hence a default implementation initialized).
   */
  @Nullable
  ...
  /**
   * Configure a {@link MessageHeaderInitializer} to apply to the headers of all messages created
   * through the {@code StompBrokerRelayMessageHandler} that are sent to the client outbound message
   * channel.
   *
   * <p>By default this property is not set.
   */
  ...
  /**
   * Return a String describing internal state and counters. Effectively {@code toString()} on
   * {@link #getStats() getStats()}.
   */
  ...
  private ReactorNettyTcpClient<byte[]> initTcpClient() {
    ...
    ReactorNettyTcpClient<byte[]> client =
        new ReactorNettyTcpClient<>(this.relayHost, this.relayPort, codec);
  ...
  protected void handleMessageInternal(Message<?> message) {
    ...
    if (!isBrokerAvailable()) {
      if (sessionId == null || SYSTEM_SESSION_ID.equals(sessionId)) {
        throw new MessageDeliveryException(
            "Message broker not active. Consider subscribing to "
    ...
    if (StompCommand.CONNECT.equals(command) || StompCommand.STOMP.equals(command)) {
      ...
      stompAccessor =
          (stompAccessor.isMutable() ? stompAccessor : StompHeaderAccessor.wrap(message));
  ...
  private String getTcpClientInfo() {
    return this.tcpClient != null
        ? this.tcpClient.toString()
        : this.relayHost + ":" + this.relayPort;
  }
  ...
  private class StompConnectionHandler implements TcpConnectionHandler<byte[]> {
    ...
    @Nullable private volatile TcpConnection<byte[]> tcpConnection;
    ...
    private StompConnectionHandler(
        String sessionId, StompHeaderAccessor connectHeaders, boolean isClientSession) {
          ...
          () -> {
            if (this.tcpConnection != null && !this.isStompConnected) {
              handleTcpConnectionFailure(
                  "No CONNECTED frame received in " + MAX_TIME_TO_CONNECTED_FRAME + " ms.", null);
            }
          },
          MAX_TIME_TO_CONNECTED_FRAME);
      connection.send(
          MessageBuilder.createMessage(EMPTY_PAYLOAD, this.connectHeaders.getMessageHeaders()));
    }
    ...
    /**
     * Invoked when any TCP connectivity issue is detected, i.e. failure to establish the TCP
     * connection, failure to send a message, missed heartbeat, etc.
     */
    ...
    /**
     * Invoked after the STOMP CONNECTED frame is received. At this point the connection is ready
     * for sending STOMP messages to the broker.
     */
    ...
    private void initHeartbeats(StompHeaderAccessor connectedHeaders) {
      ...
      if (clientReceiveInterval > 0 && serverSendInterval > 0) {
        final long interval =
            Math.max(clientReceiveInterval, serverSendInterval) * HEARTBEAT_MULTIPLIER;
     ...
     *
     * <p>The method checks whether we have an active TCP connection and have received the STOMP
     * CONNECTED frame. For client messages this should be false only if we lose the TCP connection
     * around the same time when a client message is being forwarded, so we simply log the ignored
     * message at debug level. For messages from within the application being sent on the "system"
     * connection an exception is raised so that components sending the message have a chance to
     * handle it -- by default the broker message channel is synchronous.
     *
     * <p>Note that if messages arrive concurrently around the same time a TCP connection is lost,
     * there is a brief period of time before the connection is reset when one or more messages may
     * sneak through and an attempt made to forward them. Rather than synchronizing to guard against
     * that, this method simply lets them try and fail. For client sessions that may result in an
     * additional STOMP ERROR frame(s) being sent downstream but code handling that downstream
     * should be idempotent in such cases.
     *
     * @param message the message to send (never {@code null})
     * @return a future to wait for the result
     */
    @SuppressWarnings("unchecked")
    public ListenableFuture<Void> forward(
      ...
      if (!this.isStompConnected || conn == null) {
        if (this.isRemoteClientSession) {
          if (logger.isDebugEnabled()) {
            logger.debug(
                "TCP connection closed already, ignoring "
      ...
      final Message<?> messageToSend =
          (accessor.isMutable() && accessor.isModified())
          ...
          new ListenableFutureCallback<Void>() {
            ...
            public void onFailure(Throwable ex) {
              if (tcpConnection != null) {
                handleTcpConnectionFailure(
                    "failed to forward " + accessor.getShortLogMessage(message.getPayload()), ex);
              } else if (logger.isErrorEnabled()) {
                logger.error(
    ...
    /**
     * After a DISCONNECT there should be no more client frames so we can close the connection
     * pro-actively. However, if the DISCONNECT has a receipt header we leave the connection open
     * and expect the server will respond with a RECEIPT and then close the connection.
     *
     * @see <a href="https://stomp.github.io/stomp-specification-1.2.html#DISCONNECT">STOMP
     *     Specification 1.2 DISCONNECT</a>
     */
    ...
    /**
     * Clean up state associated with the connection and close it. Any exception arising from
     * closing the connection are propagated.
     */
  ...
  private class SystemStompConnectionHandler extends StompConnectionHandler {
    ...
    private void sendSystemSubscriptions() {
      ...
      for (String destination : getSystemSubscriptions().keySet()) {
        ...
        if (conn != null) {
          MessageHeaders headers = accessor.getMessageHeaders();
          conn.send(MessageBuilder.createMessage(EMPTY_PAYLOAD, headers))
              .addCallback(
                  result -> {},
                  ex -> {
    ...
    protected void handleInboundMessage(Message<?> message) {
      ...
      if (accessor != null && StompCommand.MESSAGE.equals(accessor.getCommand())) {
        ...
        if (destination == null) {
          if (logger.isDebugEnabled()) {
            logger.debug(
                "Got message on \"system\" connection, with no destination: "
        ...
        if (!getSystemSubscriptions().containsKey(destination)) {
          if (logger.isDebugEnabled()) {
            logger.debug(
                "Got message on \"system\" connection with no handler: "
                    + accessor.getDetailedLogMessage(message.getPayload()));
          }
          return;
        }
