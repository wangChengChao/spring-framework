...
/**
 * Contains the result from parsing a "user" destination from a source message and translating it to
 * target destinations (one per active user session).
 *
...
public class UserDestinationResult {
   ...
   * "/user/queue/position-updates" when subscribing or "/user/{username}/queue/position-updates"
  ...
  /**
   * The target destinations that the source destination was translated to, one per active user
   * session, e.g. "/queue/position-updates-useri9oqdfzo".
   *
   * @return the target destinations, never {@code null} but possibly an empty set if there are no
   ...
   *
   * @return the user name or {@code null} if we have a session id only such as when the user is not
   *     authenticated; in such cases it is possible to use sessionId in place of a user name thus
   *     removing the need for a user-to-session lookup via {@link SimpUserRegistry}.
   */
  @Nullable
  ...
  public String toString() {
    return "UserDestinationResult [source="
