...
public class ReactorNettyTcpStompClient extends StompClientSupport {
  ...
  private static ReactorNettyTcpClient<byte[]> initTcpClient(String host, int port) {
    ReactorNettyTcpClient<byte[]> client =
        new ReactorNettyTcpClient<>(host, port, new StompReactorNettyCodec());
  ...
  /**
   * An overloaded version of {@link #connect(StompSessionHandler)} that accepts headers to use for
