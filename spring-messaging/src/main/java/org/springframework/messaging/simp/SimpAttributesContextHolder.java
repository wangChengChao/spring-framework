...
/**
 * Holder class to expose SiMP attributes associated with a session (e.g. WebSocket) in the form of
 * a thread-bound {@link SimpAttributes} object.
 *
...
public abstract class SimpAttributesContextHolder {
  ...
  /**
   * Extract the SiMP session attributes from the given message, wrap them in a {@link
  ...
  /**
   * Return the SimpAttributes currently bound to the thread or raise an {@link
   * java.lang.IllegalStateException} if none are bound.
   *
  ...
  public static SimpAttributes currentAttributes() throws IllegalStateException {
    ...
    if (attributes == null) {
      throw new IllegalStateException(
          "No thread-bound SimpAttributes found. "
