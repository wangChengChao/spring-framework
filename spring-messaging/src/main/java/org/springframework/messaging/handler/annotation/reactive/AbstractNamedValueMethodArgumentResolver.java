...
/**
 * Abstract base class to resolve method arguments from a named value, e.g. message headers or
 * destination variables. Named values could have one or more of a name, a required flag, and a
 * default value.
 *
 * <p>Subclasses only need to define specific steps such as how to obtain named value details from a
 * method parameter, how to resolve to argument values, or how to handle missing values.
 *
 * <p>A default value string can contain ${...} placeholders and Spring Expression Language {@code
 * #{...}} expressions which will be resolved if a {@link ConfigurableBeanFactory} is supplied to
 * the class constructor.
 *
 * <p>A {@link ConversionService} is used to convert a resolved String argument value to the
...
public abstract class AbstractNamedValueMethodArgumentResolver
   ...
   *
   * @param conversionService conversion service for converting String values to the target method
   *     parameter type
   * @param beanFactory a bean factory for resolving {@code ${...}} placeholders and {@code #{...}}
  ...
  public Object resolveArgumentValue(MethodParameter parameter, Message<?> message) {
    ...
    if (parameter != nestedParameter
        || !ClassUtils.isAssignableValue(parameter.getParameterType(), arg)) {
      arg =
          this.conversionService.convert(
              arg, TypeDescriptor.forObject(arg), new TypeDescriptor(parameter));
    }
  ...
  /**
   * Fall back on the parameter name from the class file if necessary and replace {@link
   * ValueConstants#DEFAULT_NONE} with null.
   */
  ...
  protected abstract Object resolveArgumentInternal(
      MethodParameter parameter, Message<?> message, String name);

  /**
   * Invoked when a value is required, but {@link #resolveArgumentInternal} returned {@code null}
   * and there is no default value. Sub-classes can throw an appropriate exception for this case.
   *
  ...
  /**
   * One last chance to handle a possible null value. Specifically for booleans method parameters,
   * use {@link Boolean#FALSE}. Also raise an ISE for primitive types.
   */
  @Nullable
