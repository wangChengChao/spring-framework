...
/**
 * {@link MessageCondition} to match the destination header of a Message against one or more
 * patterns through a {@link RouteMatcher}.
 *
...
public class DestinationPatternsMessageCondition
  ...
  /**
   * Constructor with patterns only. Creates and uses an instance of {@link AntPathMatcher} with
   * default settings.
   *
   ...
   *       in "other" using {@link org.springframework.util.PathMatcher#combine(String, String)}.
  ...
  /**
   * Check if any of the patterns match the given Message destination and return an instance that is
   * guaranteed to contain matching patterns, sorted via {@link
   * org.springframework.util.PathMatcher#getPatternComparator(String)}.
   *
   ...
   *     matching patterns; or {@code null} either if a destination can not be extracted or there is
   *     no match
   */
  @Override
  @Nullable
  ...
  private boolean matchPattern(String pattern, Object destination) {
    return destination instanceof RouteMatcher.Route
        ...
        : ((SimpleRouteMatcher) this.routeMatcher)
            .getPathMatcher()
            .match(pattern, (String) destination);
  }
  ...
  private Comparator<String> getPatternComparator(Object destination) {
    return destination instanceof RouteMatcher.Route
        ...
        : ((SimpleRouteMatcher) this.routeMatcher)
            .getPathMatcher()
            .getPatternComparator((String) destination);
  }

  /**
   * Compare the two conditions based on the destination patterns they contain. Patterns are
   * compared one at a time, from top to bottom via {@link
   * org.springframework.util.PathMatcher#getPatternComparator(String)}. If all compared patterns
   * match equally, but one instance has more patterns, it is considered a closer match.
   *
   * <p>It is assumed that both instances have been obtained via {@link
   * #getMatchingCondition(Message)} to ensure they contain only patterns that match the request and
   * are sorted with the best matches on top.
   */
  @Override
