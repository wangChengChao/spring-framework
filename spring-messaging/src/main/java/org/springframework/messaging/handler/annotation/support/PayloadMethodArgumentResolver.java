...
/**
 * A resolver to extract and convert the payload of a message using a {@link MessageConverter}. It
 * also validates the payload using a {@link Validator} if the argument is annotated with a
 * Validation annotation.
 *
 * <p>This {@link HandlerMethodArgumentResolver} should be ordered last as it supports all types and
 * does not require the {@link Payload} annotation.
 *
...
public class PayloadMethodArgumentResolver implements HandlerMethodArgumentResolver {
  ...
  /**
   * Create a new {@code PayloadArgumentResolver} with the given {@link MessageConverter} and {@link
   * Validator}.
   *
   * @param messageConverter the MessageConverter to use (required)
  ...
  public PayloadMethodArgumentResolver(
      MessageConverter messageConverter, @Nullable Validator validator) {
   ...
   * @param messageConverter the MessageConverter to use (required)
   * @param validator the Validator to use (optional)
   * @param useDefaultResolution if "true" (the default) this resolver supports all parameters; if
   *     "false" then only arguments with the {@code @Payload} annotation are supported.
   */
  public PayloadMethodArgumentResolver(
   ...
   *
   * <p>By default this is simply {@link MethodParameter#getParameterType()} but that can be
   * overridden to select a more specific target type after also taking into account the
   ...
   * Validated}, and custom annotations whose name starts with "Valid".
   *
