...
/**
 * Base class for a return value handler that encodes return values to {@code Flux<DataBuffer>}
 * through the configured {@link Encoder}s.
 *
 * <p>Subclasses must implement the abstract method {@link #handleEncodedContent} to handle the
 * resulting encoded content.
 *
 * <p>This handler should be ordered last since its {@link #supportsReturnType} returns {@code true}
...
public abstract class AbstractEncoderMethodReturnValueHandler
  ...
  private static final ResolvableType OBJECT_RESOLVABLE_TYPE =
  ...
  protected AbstractEncoderMethodReturnValueHandler(
        ...
        (DataBufferFactory)
            message
                ...
                .getOrDefault(
                    ...
                    this.defaultBufferFactory);

    MimeType mimeType = (MimeType) message.getHeaders().get(MessageHeaders.CONTENT_TYPE);
    Flux<DataBuffer> encodedContent =
        encodeContent(returnValue, returnType, bufferFactory, mimeType, Collections.emptyMap());

    return new ChannelSendOperator<>(
        encodedContent,
  ...
  private Flux<DataBuffer> encodeContent(
    ...
    if (adapter != null) {
      ...
      elementType =
          (returnValueType.toClass() == Object.class && content != null
  ...
  /**
   * Subclasses implement this method to handle encoded values in some way such as creating and
   * sending messages.
   *
   * @param encodedContent the encoded content; each {@code DataBuffer} represents the
   *     fully-aggregated, encoded content for one value (i.e. payload) returned from the
   *     HandlerMethod.
  ...
  /**
   * Invoked for a {@code null} return value, which could mean a void method or method returning an
   * async type parameterized by void.
   *
   * @param returnType return type of the handler method that produced the data
   * @param message the input message handled by the handler method
   * @return completion {@code Mono<Void>} for the handling
