...
/**
 * A sub-class of {@link AbstractExceptionHandlerMethodResolver} that looks for {@link
 * MessageExceptionHandler}-annotated methods in a given class. The actual exception types handled
 * are extracted either from the annotation, if present, or from the method signature as a fallback
 * option.
 *
...
public class AnnotationExceptionHandlerMethodResolver
  ...
  private static Map<Class<? extends Throwable>, Method> initExceptionMappings(
      Class<?> handlerType) {
    Map<Method, MessageExceptionHandler> methods =
        MethodIntrospector.selectMethods(
            handlerType,
            (MethodIntrospector.MetadataLookup<MessageExceptionHandler>)
                method ->
                    AnnotatedElementUtils.findMergedAnnotation(
                        method, MessageExceptionHandler.class));
