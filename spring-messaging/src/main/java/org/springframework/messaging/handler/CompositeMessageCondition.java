 ...
 *
 * <p>For {@link #combine} and {@link #compareTo} it is expected that the "other" composite contains
 * the same number, type, and order of message conditions.
 *
...
public class CompositeMessageCondition implements MessageCondition<CompositeMessageCondition> {
  ...
  @SuppressWarnings("unchecked")
  private <T extends MessageCondition<T>> T combine(
      MessageCondition<?> first, MessageCondition<?> second) {
  ...
  public String toString() {
    return this.messageConditions.stream()
        .map(Object::toString)
