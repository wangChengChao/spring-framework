...
/**
 * Base class for {@code MessageCondition's} that pre-declares abstract methods {@link
 * #getContent()} and {@link #getToStringInfix()} in order to provide implementations of {@link
...
public abstract class AbstractMessageCondition<T extends AbstractMessageCondition<T>>
  ...
  /**
   * Return the collection of objects the message condition is composed of (e.g. destination
   * patterns), never {@code null}.
   */
  protected abstract Collection<?> getContent();

  /**
   * The notation to use when printing discrete items of content. For example " || " for URL
   * patterns or " && " for param expressions.
   */
