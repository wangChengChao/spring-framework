 ...
 * ----------------------
 *
 * <p><strong>NOTE:</strong> This class was copied from {@code
 * org.springframework.http.server.reactive.ChannelSendOperator} and is identical to it. It's used
 * for the same purpose, i.e. the ability to switch to alternate handling via annotated exception
 * handler methods if the output publisher starts with an error.
 *
 * <p>----------------------<br>
 *
 * <p>Given a write function that accepts a source {@code Publisher<T>} to write with and returns
 * {@code Publisher<Void>} for the result, this operator helps to defer the invocation of the write
 * function, until we know if the source publisher will begin publishing without an error. If the
 * first emission is an error, the write function is bypassed, and the error is sent directly
...
class ChannelSendOperator<T> extends Mono<Void> implements Scannable {
  ...
  private enum State {
    ...
    /**
     * At least one signal of any kind has been received; we're ready to call the write function and
     * proceed with actual writing.
     */
    ...
    /**
     * The write subscriber has subscribed, and cached signals have been emitted to it; we're ready
     * to switch to a simple pass-through mode for all remaining signals.
     */
    READY_TO_WRITE
  }

  /**
   * A barrier inserted between the write source and the write subscriber (i.e. the HTTP server
   * adapter) that pre-fetches and waits for the first signal before deciding whether to hook in to
   * the write subscriber.
   *
   * <p>Acts as:
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>Also uses {@link WriteCompletionBarrier} to communicate completion and detect cancel signals
  ...
  private class WriteBarrier implements CoreSubscriber<T>, Subscription, Publisher<T> {
    ...
    @Nullable private Subscriber<? super T> writeSubscriber;
   ...
   *
   * <p>The completionSubscriber is subscribed initially to the WriteBarrier. Later after the first
   * signal is received, we need one more subscriber instance (per spec can only subscribe once) to
   * subscribe to the write function and switch to delegating completion signals from it.
   */
  private class WriteCompletionBarrier implements CoreSubscriber<Void>, Subscription {
    ...
    public WriteCompletionBarrier(
        CoreSubscriber<? super Void> subscriber, WriteBarrier writeBarrier) {
      this.completionSubscriber = subscriber;
      this.writeBarrier = writeBarrier;
    }

    /**
     * Connect the underlying completion subscriber to this barrier in order to track cancel signals
     * and pass them on to the write barrier.
     */
    public void connect() {
