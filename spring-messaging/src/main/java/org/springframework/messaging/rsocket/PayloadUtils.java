...
/**
 * Static utility methods to create {@link Payload} from {@link DataBuffer}s and vice versa.
 *
...
public abstract class PayloadUtils {
  ...
  /**
   * Use this method to slice, retain and wrap the data portion of the {@code Payload}, and also to
   * release the {@code Payload}. This assumes the Payload metadata has been read by now and ensures
   * downstream code need only be aware of {@code DataBuffer}s.
   *
  ...
  public static DataBuffer retainDataAndReleasePayload(
      Payload payload, DataBufferFactory bufferFactory) {
    try {
   ...
   *
   * <p>If at least one is {@link NettyDataBuffer} then {@link ByteBufPayload} is created with
   * either obtaining the underlying native {@link ByteBuf} or using {@link
   * Unpooled#wrappedBuffer(ByteBuffer...)} if necessary. Otherwise, if both are {@link
   * DefaultDataBuffer}, then {@link DefaultPayload} is created.
   *
  ...
  /**
   * Create a Payload with data only. The created payload is {@link ByteBufPayload} if the input is
   * {@link NettyDataBuffer} or otherwise it is {@link DefaultPayload}.
   *
   * @param data the data part for the payload
  ...
  public static Payload createPayload(DataBuffer data) {
    return data instanceof NettyDataBuffer
        ? ByteBufPayload.create(asByteBuf(data))
