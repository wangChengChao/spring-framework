...
final class DefaultRSocketRequester implements RSocketRequester {
  ...
  private class DefaultRequestSpec implements RequestSpec {
    ...
    private ResponseSpec toResponseSpec(Object input, ResolvableType elementType) {
      ...
      if (input instanceof Publisher) {
        ...
        Mono<Payload> payloadMono =
            Mono.fromCallable(() -> encodeData(input, ResolvableType.forInstance(input), null))
      ...
      Encoder<?> encoder =
          elementType != ResolvableType.NONE && !Object.class.equals(elementType.resolve())
              ? strategies.encoder(elementType, dataMimeType)
              : null;
    ...
    private <T> DataBuffer encodeData(
        T value, ResolvableType elementType, @Nullable Encoder<?> encoder) {
      if (encoder == null) {
        elementType = ResolvableType.forInstance(value);
        encoder = strategies.encoder(elementType, dataMimeType);
      }
      return ((Encoder<T>) encoder)
          .encodeValue(value, bufferFactory(), elementType, dataMimeType, EMPTY_HINTS);
    }
  ...
  private class DefaultResponseSpec implements ResponseSpec {
    ...
    @Nullable private final Mono<Payload> payloadMono;

    @Nullable private final Flux<Payload> payloadFlux;
    ...
    public Mono<Void> send() {
      Assert.state(
          this.payloadMono != null, "No RSocket interaction model for one-way send with Flux");
    ...
    private <T> Mono<T> retrieveMono(ResolvableType elementType) {
      Assert.notNull(
          this.payloadMono, "No RSocket interaction model for Flux request to Mono response.");
      ...
      return (Mono<T>)
          payloadMono
              ...
              .map(
                  dataBuffer -> decoder.decode(dataBuffer, elementType, dataMimeType, EMPTY_HINTS));
    }
    ...
    private <T> Flux<T> retrieveFlux(ResolvableType elementType) {
      ...
      return payloadFlux
          .map(this::retainDataAndReleasePayload)
          .map(
              dataBuffer -> (T) decoder.decode(dataBuffer, elementType, dataMimeType, EMPTY_HINTS));
    }
