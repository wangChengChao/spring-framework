...
/**
 * Responder {@link RSocket} that wraps the payload and metadata of incoming requests as a {@link
 * Message} and then delegates to the configured {@link RSocketMessageHandler} to handle it. The
 * response, if applicable, is obtained from the {@link
 * RSocketPayloadReturnValueHandler#RESPONSE_HEADER "rsocketResponse"} header.
 *
...
class MessagingRSocket extends AbstractRSocket {
  ...
  /**
   * Wrap the {@link ConnectionSetupPayload} with a {@link Message} and delegate to {@link
   * #handle(Payload, FrameType)} for handling.
   *
  ...
  private Flux<Payload> handleAndReply(
      Payload firstPayload, FrameType frameType, Flux<Payload> payloads) {
    ...
    Flux<DataBuffer> buffers =
        payloads.map(this::retainDataAndReleasePayload).doOnSubscribe(s -> read.set(true));
                    ...
                    replyMono.isTerminated()
                        ...
                        : Mono.error(
                            new IllegalStateException(
                                "Something went wrong: reply Mono not set"))));
  }
  ...
  private MessageHeaders createHeaders(
      Payload payload, FrameType frameType, @Nullable MonoProcessor<?> replyMono) {

    MessageHeaderAccessor headers = new MessageHeaderAccessor();
    headers.setLeaveMutable(true);

    Map<String, Object> metadataValues =
        this.metadataExtractor.extract(payload, this.metadataMimeType);
    ...
    headers.setHeader(
        RSocketRequesterMethodArgumentResolver.RSOCKET_REQUESTER_HEADER, this.requester);
    if (replyMono != null) {
