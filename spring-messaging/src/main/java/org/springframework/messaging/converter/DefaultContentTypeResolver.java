...
/**
 * A default {@link ContentTypeResolver} that checks the {@link MessageHeaders#CONTENT_TYPE} header
 * or falls back to a default value.
 *
 * <p>The header value is expected to be a {@link org.springframework.util.MimeType} or a {@code
 * String} that can be parsed into a {@code MimeType}.
 *
...
public class DefaultContentTypeResolver implements ContentTypeResolver {
  ...
  @Nullable private MimeType defaultMimeType;

  /**
   * Set the default MIME type to use when there is no {@link MessageHeaders#CONTENT_TYPE} header
   * present.
   *
  ...
  /**
   * Return the default MIME type to use if no {@link MessageHeaders#CONTENT_TYPE} header is
   * present.
   */
  @Nullable
