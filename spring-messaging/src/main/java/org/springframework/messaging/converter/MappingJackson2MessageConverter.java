...
public class MappingJackson2MessageConverter extends AbstractMessageConverter {
  ...
  @Nullable private Boolean prettyPrint;

  /**
   * Construct a {@code MappingJackson2MessageConverter} supporting the {@code application/json}
   * MIME type with {@code UTF-8} character set.
   */
  ...
  /**
   * Set the {@code ObjectMapper} for this converter. If not set, a default {@link
  ...
  /**
   * Whether to use the {@link DefaultPrettyPrinter} when writing JSON. This is a shortcut for
   * setting up an {@code ObjectMapper} as follows:
   *
   * <pre class="code">
   ...
   * @param cause the Jackson-thrown exception to evaluate (typically a {@link
  ...
  protected void logWarningIfNecessary(Type type, @Nullable Throwable cause) {
    ...
    boolean debugLevel =
        (cause instanceof JsonMappingException && cause.getMessage().startsWith("Cannot find"));

    if (debugLevel ? logger.isDebugEnabled() : logger.isWarnEnabled()) {
      String msg =
          "Failed to evaluate Jackson "
              + (type instanceof JavaType ? "de" : "")
              + "serialization for type ["
              + type
              + "]";
      if (debugLevel) {
  ...
  protected Object convertFromInternal(
    ...
    // Note: in the view case, calling withType instead of forType for compatibility with Jackson
    // <2.5
    try {
  ...
  protected Class<?> getSerializationView(@Nullable Object conversionHint) {
    if (conversionHint instanceof MethodParameter) {
      MethodParameter param = (MethodParameter) conversionHint;
      JsonView annotation =
          (param.getParameterIndex() >= 0
              ? param.getParameterAnnotation(JsonView.class)
