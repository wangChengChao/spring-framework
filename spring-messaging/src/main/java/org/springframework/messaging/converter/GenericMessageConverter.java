...
/**
 * An extension of the {@link SimpleMessageConverter} that uses a {@link ConversionService} to
 * convert the payload of the message to the requested type.
 *
 * <p>Return {@code null} if the conversion service cannot convert from the payload type to the
 * requested type.
 *
    ...
    if (this.conversionService.canConvert(payload.getClass(), targetClass)) {
      try {
        ...
        throw new MessageConversionException(
            message,
            "Failed to convert message payload '"
                + payload
                + "' to '"
                + targetClass.getName()
                + "'",
