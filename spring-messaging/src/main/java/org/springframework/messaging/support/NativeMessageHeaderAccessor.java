...
/**
 * An extension of {@link MessageHeaderAccessor} that also stores and provides read/write access to
 * message headers from an external source -- e.g. a Spring {@link Message} created to represent a
 * STOMP message received from a STOMP client or message broker. Native message headers are kept in
 * a {@code Map<String, List<String>>} under the key {@link #NATIVE_HEADERS}.
 *
 * <p>This class is not intended for direct use but is rather expected to be used indirectly through
 * protocol-specific sub-classes such as {@link
 * org.springframework.messaging.simp.stomp.StompHeaderAccessor StompHeaderAccessor}. Such
 * sub-classes may provide factory methods to translate message headers from an external messaging
 * source (e.g. STOMP) to Spring {@link Message} headers and reversely to translate Spring {@link
 * Message} headers to a message to send to an external source.
 *
