...
public class ExecutorSubscribableChannel extends AbstractSubscribableChannel {
  ...
  /**
   * Create a new {@link ExecutorSubscribableChannel} instance where messages will be sent in the
   * callers thread.
   */
  ...
  /**
   * Create a new {@link ExecutorSubscribableChannel} instance where messages will be sent via the
   * specified executor.
   *
   * @param executor the executor used to send the message, or {@code null} to execute in the
   *     callers thread.
   */
  ...
  private class SendTask implements MessageHandlingRunnable {
    ...
    public void run() {
      ...
      try {
        ...
        String description =
            "Failed to handle " + message + " to " + this + " in " + this.messageHandler;
    ...
    private Message<?> applyBeforeHandle(Message<?> message) {
      ...
      for (ExecutorChannelInterceptor interceptor : executorInterceptors) {
        messageToUse =
            interceptor.beforeHandle(
                messageToUse, ExecutorSubscribableChannel.this, this.messageHandler);
      ...
      for (int i = this.interceptorIndex; i >= 0; i--) {
        ...
        try {
          interceptor.afterMessageHandled(
              message, ExecutorSubscribableChannel.this, this.messageHandler, ex);
        } catch (Throwable ex2) {
