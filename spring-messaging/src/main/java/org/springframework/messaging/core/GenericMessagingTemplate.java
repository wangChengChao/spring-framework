...
/**
 * A messaging template that resolves destinations names to {@link MessageChannel}'s to send and
...
public class GenericMessagingTemplate
  ...
  /**
   * Configure the default timeout value to use for send operations. May be overridden for
   * individual messages.
   *
  ...
  /**
   * Configure the default timeout value to use for receive operations. May be overridden for
   * individual messages when using sendAndReceive operations.
   *
  ...
  /**
   * Set the name of the header used to determine the send timeout (if present). Default {@value
  ...
  /**
   * Set the name of the header used to determine the send timeout (if present). Default {@value
   * #DEFAULT_RECEIVE_TIMEOUT_HEADER}. The header is removed before sending the message to avoid
   * propagation.
   *
   * @since 5.0
   */
  ...
  /**
   * Whether the thread sending a reply should have an exception raised if the receiving thread
   * isn't going to receive the reply either because it timed out, or because it already received a
   * reply, or because it got an exception while sending the request message.
   *
   * <p>The default value is {@code false} in which case only a WARN message is logged. If set to
   * {@code true} a {@link MessageDeliveryException} is raised in addition to the log message.
   *
  ...
  protected final void doSend(MessageChannel channel, Message<?> message, long timeout) {
    ...
    boolean sent =
        (timeout >= 0 ? channel.send(messageToSend, timeout) : channel.send(messageToSend));
  ...
  protected final Message<?> doReceive(MessageChannel channel, long timeout) {
    Assert.notNull(channel, "MessageChannel is required");
    Assert.state(
        channel instanceof PollableChannel, "A PollableChannel is required to receive messages");

    Message<?> message =
        (timeout >= 0
            ? ((PollableChannel) channel).receive(timeout)
            : ((PollableChannel) channel).receive());
  ...
  protected final Message<?> doSendAndReceive(MessageChannel channel, Message<?> requestMessage) {
    ...
    TemporaryReplyChannel tempReplyChannel =
        new TemporaryReplyChannel(this.throwExceptionOnLateReply);
    requestMessage =
        MessageBuilder.fromMessage(requestMessage)
            .setReplyChannel(tempReplyChannel)
            .setHeader(this.sendTimeoutHeader, null)
            .setHeader(this.receiveTimeoutHeader, null)
            .setErrorChannel(tempReplyChannel)
            .build();
  ...
  private static final class TemporaryReplyChannel implements PollableChannel {
    ...
    @Nullable private volatile Message<?> replyMessage;
    ...
    public boolean send(Message<?> message, long timeout) {
      ...
      if (this.hasTimedOut) {
        ...
        errorDescription =
            "Reply message received but the receiving thread has exited due to "
