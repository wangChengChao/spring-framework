...
/**
 * {@link DestinationResolver} implementation that proxies a target DestinationResolver, caching its
 * {@link #resolveDestination} results. Such caching is particularly useful if the destination
 * resolving process is expensive (e.g. the destination has to be resolved through an external
 * system) and the resolution results are stable anyway.
 *
 * @author Agim Emruli
...
public class CachingDestinationResolverProxy<D>
    implements DestinationResolver<D>, InitializingBean {

  private final Map<String, D> resolvedDestinationCache = new ConcurrentHashMap<>();

  @Nullable private DestinationResolver<D> targetDestinationResolver;

  /**
   * Create a new CachingDestinationResolverProxy, setting the target DestinationResolver through
   * the {@link #setTargetDestinationResolver} bean property.
   */
  public CachingDestinationResolverProxy() {}

  /**
   * Create a new CachingDestinationResolverProxy using the given target DestinationResolver to
   * actually resolve destinations.
   *
  ...
  /**
   * Resolves and caches destinations if successfully resolved by the target DestinationResolver
   * implementation.
   *
   * @param name the destination name to be resolved
   * @return the currently resolved destination or an already cached destination
   * @throws DestinationResolutionException if the target DestinationResolver reports an error
