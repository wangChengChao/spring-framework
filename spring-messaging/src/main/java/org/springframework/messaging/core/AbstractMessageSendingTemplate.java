...
public abstract class AbstractMessageSendingTemplate<D> implements MessageSendingOperations<D> {
  ...
  /**
   * Name of the header that can be set to provide further information (e.g. a {@code
   * MethodParameter} instance) about the origin of the payload, to be taken into account as a
   * conversion hint.
   *
   * @since 4.2
   */
  ...
  @Nullable private D defaultDestination;

  private MessageConverter converter = new SimpleMessageConverter();

  /**
   * Configure the default destination to use in send methods that don't have a destination
  ...
  public void convertAndSend(
      D destination, Object payload, @Nullable MessagePostProcessor postProcessor)
      throws MessagingException {
  ...
  /**
   * Convert the given Object to serialized form, possibly using a {@link MessageConverter}, wrap it
   * as a message with the given headers and apply the given post processor.
   *
  ...
  protected Message<?> doConvert(
    ...
    Message<?> message =
        (converter instanceof SmartMessageConverter
    ...
    if (message == null) {
      ...
      Object contentType =
          (messageHeaders != null ? messageHeaders.get(MessageHeaders.CONTENT_TYPE) : null);
      throw new MessageConversionException(
          "Unable to convert payload with type='"
              + payloadType
              + "', contentType='"
              + contentType
              + "', converter=["
  ...
  /**
   * Provides access to the map of input headers before a send operation. Subclasses can modify the
   * headers and then return the same or a different map.
   *
