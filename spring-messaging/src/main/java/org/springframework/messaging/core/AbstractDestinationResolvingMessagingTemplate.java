...
/**
 * An extension of {@link AbstractMessagingTemplate} that adds operations for sending messages to a
 * resolvable destination name. Supports destination resolving as defined by the following
 * interfaces:
 *
 * <ul>
 *   <li>{@link DestinationResolvingMessageSendingOperations}
 *   <li>{@link DestinationResolvingMessageReceivingOperations}
 *   <li>{@link DestinationResolvingMessageRequestReplyOperations}
 * </ul>
 *
 * @author Mark Fisher
...
public abstract class AbstractDestinationResolvingMessagingTemplate<D>
  ...
  @Nullable private DestinationResolver<D> destinationResolver;

  /**
   * Configure the {@link DestinationResolver} to use to resolve String destination names into
   * actual destinations of type {@code <D>}.
   *
   * <p>This field does not have a default setting. If not configured, methods that require
   * resolving a destination name will raise an {@link IllegalArgumentException}.
   *
  ...
  public <T> void convertAndSend(
      String destinationName, T payload, @Nullable Map<String, Object> headers) {
    convertAndSend(destinationName, payload, headers, null);
  }

  @Override
  public <T> void convertAndSend(
      String destinationName, T payload, @Nullable MessagePostProcessor postProcessor) {
