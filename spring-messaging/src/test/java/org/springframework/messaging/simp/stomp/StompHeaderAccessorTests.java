...
public class StompHeaderAccessorTests {
  ...
  public void toNativeHeadersMessageFrame() {
    ...
    assertThat(actual.get(StompHeaderAccessor.STOMP_DESTINATION_HEADER).get(0)).isEqualTo("/d");
    assertThat(actual.get(StompHeaderAccessor.STOMP_CONTENT_TYPE_HEADER).get(0))
        .isEqualTo("application/json");
    assertThat(actual.get(StompHeaderAccessor.STOMP_MESSAGE_ID_HEADER).get(0))
        .as("message-id was not created")
        .isNotNull();
  }
  ...
  public void toNativeHeadersContentType() {
    ...
    Message<byte[]> message =
        MessageBuilder.createMessage(new byte[0], simpHeaderAccessor.getMessageHeaders());

    StompHeaderAccessor stompHeaderAccessor = StompHeaderAccessor.wrap(message);
    Map<String, List<String>> map = stompHeaderAccessor.toNativeHeaderMap();

    assertThat(map.get(StompHeaderAccessor.STOMP_CONTENT_TYPE_HEADER).get(0))
  ...
  public void encodeConnectWithLoginAndPasscode() throws UnsupportedEncodingException {
    ...
    StompHeaderAccessor headerAccessor =
        StompHeaderAccessor.create(StompCommand.CONNECT, extHeaders);
    Message<byte[]> message =
  ...
  public void messageIdAndTimestampEnabled() {
    IdTimestampMessageHeaderInitializer headerInitializer =
        new IdTimestampMessageHeaderInitializer();
  ...
  public void getAccessor() {
    ...
    assertThat(MessageHeaderAccessor.getAccessor(message, StompHeaderAccessor.class))
        .isSameAs(headerAccessor);
  }
  ...
  public void getShortLogMessage() {
    ...
    assertThat(actual)
        .isEqualTo(("SEND /foo session=123 application/json payload=" + sb + "...(truncated)"));
  }
}
