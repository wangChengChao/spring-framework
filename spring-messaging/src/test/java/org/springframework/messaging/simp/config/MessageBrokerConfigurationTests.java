...
public class MessageBrokerConfigurationTests {
  ...
  public void clientInboundChannelCustomized() {
    ...
    AbstractSubscribableChannel channel =
        context.getBean("clientInboundChannel", AbstractSubscribableChannel.class);
    assertThat(channel.getInterceptors().size()).isEqualTo(3);

    CustomThreadPoolTaskExecutor taskExecutor =
        context.getBean("clientInboundChannelExecutor", CustomThreadPoolTaskExecutor.class);
  ...
  public void clientOutboundChannelCustomized() {
    ...
    AbstractSubscribableChannel channel =
        context.getBean("clientOutboundChannel", AbstractSubscribableChannel.class);

    assertThat(channel.getInterceptors().size()).isEqualTo(4);

    ThreadPoolTaskExecutor taskExecutor =
        context.getBean("clientOutboundChannelExecutor", ThreadPoolTaskExecutor.class);
  ...
  public void brokerChannelCustomized() {
    ...
    AbstractSubscribableChannel channel =
        context.getBean("brokerChannel", AbstractSubscribableChannel.class);

    assertThat(channel.getInterceptors().size()).isEqualTo(4);

    ThreadPoolTaskExecutor taskExecutor =
        context.getBean("brokerChannelExecutor", ThreadPoolTaskExecutor.class);
  ...
  public void configureMessageConvertersCustom() {
    ...
    AbstractMessageBrokerConfiguration config =
        new BaseTestMessageBrokerConfig() {
          @Override
  ...
  public void customPathMatcher() {
    ...
    DefaultSubscriptionRegistry registry =
        (DefaultSubscriptionRegistry) broker.getSubscriptionRegistry();
  ...
  public void userBroadcasts() {
    ...
    assertThat(relay.getSystemSubscriptions().get("/topic/unresolved-user-destination"))
        .isSameAs(handler1);
  ...
  private void testDotSeparator(ApplicationContext context, boolean expectLeadingSlash) {
    ...
    template.convertAndSendToUser(
        "sess1", "queue.q1", "456".getBytes(), accessor.getMessageHeaders());
  ...
  static class CustomConfig extends BaseTestMessageBrokerConfig {
    ...
    protected void configureClientInboundChannel(ChannelRegistration registration) {
      ...
      registration
          .taskExecutor(new CustomThreadPoolTaskExecutor())
          .corePoolSize(11)
          .maxPoolSize(12)
          .keepAliveSeconds(13)
          .queueCapacity(14);
    ...
    protected void configureMessageBroker(MessageBrokerRegistry registry) {
      registry
          ...
          .interceptors(this.interceptor, this.interceptor, this.interceptor);
      registry
          .configureBrokerChannel()
          .taskExecutor()
          .corePoolSize(31)
