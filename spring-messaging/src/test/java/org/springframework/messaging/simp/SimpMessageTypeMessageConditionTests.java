...
public class SimpMessageTypeMessageConditionTests {
  ...
  public void combine() {
    ...
    SimpMessageType actual =
        condition(messageType).combine(condition(subscribeType)).getMessageType();
  ...
  public void compareTo() {
    ...
    assertThat(
            condition(SimpMessageType.MESSAGE)
                .compareTo(condition(SimpMessageType.MESSAGE), message))
        .isEqualTo(0);
    assertThat(
            condition(SimpMessageType.MESSAGE)
                .compareTo(condition(SimpMessageType.SUBSCRIBE), message))
        .isEqualTo(0);
  }
