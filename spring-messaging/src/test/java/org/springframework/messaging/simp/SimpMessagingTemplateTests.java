...
public class SimpMessagingTemplateTests {
  ...
  public void convertAndSendToUserWithEncoding() {
    this.messagingTemplate.convertAndSendToUser(
        "https://joe.openid.example.org/", "/queue/foo", "data");
    List<Message<byte[]>> messages = this.messageChannel.getMessages();
    ...
    assertThat(headerAccessor).isNotNull();
    assertThat(headerAccessor.getDestination())
        .isEqualTo("/user/https:%2F%2Fjoe.openid.example.org%2F/queue/foo");
  }
  ...
  public void convertAndSendWithCustomHeaderNonNative() {
    ...
    headers.put(
        NativeMessageHeaderAccessor.NATIVE_HEADERS, new LinkedMultiValueMap<String, String>());
    this.messagingTemplate.convertAndSend("/foo", "data", headers);
  ...
  public void processHeadersToSend() {
    ...
    assertThat(MessageHeaders.class.isAssignableFrom(map.getClass()))
        .as("Actual: " + map.getClass().toString())
        .isTrue();
  ...
  public void doSendWithStompHeaders() {
    ...
    MessageHeaderAccessor sentAccessor =
        MessageHeaderAccessor.getAccessor(sentMessage, MessageHeaderAccessor.class);
    assertThat(sentAccessor.getClass()).isEqualTo(StompHeaderAccessor.class);
    assertThat(((StompHeaderAccessor) sentAccessor).getDestination())
        .isEqualTo("/queue/foo-user123");
  }
}
