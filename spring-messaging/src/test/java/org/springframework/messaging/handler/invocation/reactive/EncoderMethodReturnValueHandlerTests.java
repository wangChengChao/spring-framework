...
public class EncoderMethodReturnValueHandlerTests {
  ...
  private final TestEncoderMethodReturnValueHandler handler =
      new TestEncoderMethodReturnValueHandler(
  ...
  public void fluxObjectReturnValue() {
    MethodParameter parameter =
        on(TestController.class).resolveReturnType(Flux.class, Object.class);
    this.handler.handleReturnValue(Flux.just("foo", "bar"), parameter, this.message).block();
    Flux<String> result = this.handler.getContentAsStrings();
  ...
  public void voidReturnValue() {
    ...
    testVoidReturnType(
        Mono.empty(), on(TestController.class).resolveReturnType(Mono.class, Void.class));
    testVoidReturnType(
        Completable.complete(), on(TestController.class).resolveReturnType(Completable.class));
  }
  ...
  public void noEncoder() {
    ...
    StepVerifier.create(this.handler.handleReturnValue(new Object(), parameter, this.message))
        .expectErrorMessage(
            "No encoder for java.lang.Object, current value type is class java.lang.Object")
        .verify();
  }
