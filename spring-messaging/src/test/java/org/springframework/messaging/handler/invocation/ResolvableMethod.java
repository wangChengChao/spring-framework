...
/**
 * NOTE: This class is a replica of the same class in spring-web so it can be used for tests in
 * spring-messaging.
 *
 ...
 * <h1>Background</h1>
 *
 * <p>When testing annotated methods we create test classes such as "TestController" with a diverse
 * range of method signatures representing supported annotations and argument types. It becomes
 * challenging to use naming strategies to keep track of methods and arguments especially in
 * combination with variables for reflection metadata.
 *
 * <p>The idea with {@link ResolvableMethod} is NOT to rely on naming techniques but to use hints to
 * zero in on method parameters. Such hints can be strongly typed and explicit about what is being
 * tested.
 *
 * <h2>1. Declared Return Type</h2>
 *
 * When testing return types it's likely to have many methods with a unique return type, possibly
 ...
 *
 * When testing method arguments it's more likely to have one or a small number of methods with a
 * wide array of argument types and parameter annotations.
 *
 * <pre>
...
public class ResolvableMethod {
  ...
  /**
   * Filter on method arguments with annotation. See {@link
   * org.springframework.web.method.MvcAnnotationPredicates}.
   */
  @SafeVarargs
  ...
  private String formatParameter(Parameter param) {
    ...
    return (anns.length > 0
        ? Arrays.stream(anns).map(this::formatAnnotation).collect(joining(",", "[", "]"))
            + " "
            + param
        : param.toString());
  }
  ...
  private static ResolvableType toResolvableType(
      Class<?> type, ResolvableType generic, ResolvableType... generics) {
  ...
  public static class Builder<T> {
    ...
    /**
     * Filter on annotated methods. See {@link
     * org.springframework.web.method.MvcAnnotationPredicates}.
     */
    @SafeVarargs
     ...
     *
     * @see #annot(Predicate[]) See {@link org.springframework.web.method.MvcAnnotationPredicates}.
     */
    @SafeVarargs
     ...
     */
    public Builder<T> returning(
        Class<?> returnType, ResolvableType generic, ResolvableType... generics) {
    ...
    /**
     * Build a {@code ResolvableMethod} from the provided filters which must resolve to a unique,
     * single method.
     *
     * <p>See additional resolveXxx shortcut methods going directly to {@link Method} or return type
     * parameter.
     *
    ...
    public ResolvableMethod build() {
      ...
      Assert.state(
          methods.size() == 1, () -> "Multiple matching methods: " + this + formatMethods(methods));
  ...
  public class ArgResolver {
    ...
    /**
     * Filter on method arguments with annotations. See {@link
     * org.springframework.web.method.MvcAnnotationPredicates}.
     */
    @SafeVarargs
    ...
    public final ArgResolver annotPresent(Class<? extends Annotation>... annotationTypes) {
      this.filters.add(
          param -> Arrays.stream(annotationTypes).allMatch(param::hasParameterAnnotation));
      return this;
    }
    ...
    public MethodParameter arg(ResolvableType type) {
      this.filters.add(
          p -> type.toString().equals(ResolvableType.forMethodParameter(p).toString()));
      return arg();
    }
      ...
      if (proxy == null) {
        try {
          ...
          throw new IllegalStateException(
              "Unable to instantiate proxy "
                  + "via both Objenesis and default constructor fails as well",
              ex);
        }
      }
