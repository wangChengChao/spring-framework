...
public class PayloadMethodArgumentResolverTests {
  ...
  public void supportsParameter() {
    ...
    assertThat(resolver.supportsParameter(this.testMethod.annotPresent(Payload.class).arg()))
        .isTrue();
    assertThat(
            resolver.supportsParameter(
                this.testMethod.annotNotPresent(Payload.class).arg(String.class)))
        .isTrue();
  ...
  public void emptyBodyWhenRequired() {
    MethodParameter param =
        this.testMethod.arg(ResolvableType.forClassWithGenerics(Mono.class, String.class));
    ...
    StepVerifier.create(mono)
        .consumeErrorWith(
            ex -> {
              ...
              assertThat(ex.getMessage().contains("Payload content is missing"))
                  .as(ex.getMessage())
                  .isTrue();
            })
        .verify();
  ...
  public void stringFlux() {
    ...
    Flux<Object> flux =
        resolveValue(
            param,
            Flux.fromIterable(body).delayElements(Duration.ofMillis(10)).map(this::toDataBuffer),
            null);
  ...
  @Nullable
  private <T> T resolveValue(
      MethodParameter param, Publisher<DataBuffer> content, Validator validator) {
    ...
    if (value != null) {
      ...
      assertThat(expectedType.isAssignableFrom(value.getClass()))
  ...
  private PayloadMethodArgumentResolver createResolver(
      @Nullable Validator validator, boolean useDefaultResolution) {
