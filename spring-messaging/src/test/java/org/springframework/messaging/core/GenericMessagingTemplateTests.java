...
public class GenericMessagingTemplateTests {
  ...
  public void sendWithTimeout() {
    ...
    willAnswer(
        ...
        .given(channel)
        .send(any(Message.class), eq(30_000L));
    Message<?> message =
    ...
    assertThat(
            sent.get()
                .getHeaders()
                .containsKey(GenericMessagingTemplate.DEFAULT_SEND_TIMEOUT_HEADER))
        .isFalse();
    assertThat(
            sent.get()
                .getHeaders()
                .containsKey(GenericMessagingTemplate.DEFAULT_RECEIVE_TIMEOUT_HEADER))
        .isFalse();
  }
  ...
  private MessageHandler createLateReplier(
      final CountDownLatch latch, final AtomicReference<Throwable> failure) {
