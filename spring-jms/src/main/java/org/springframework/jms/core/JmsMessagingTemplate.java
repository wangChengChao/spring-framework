...
public class JmsMessagingTemplate extends AbstractMessagingTemplate<Destination>
  ...
  @Nullable private JmsTemplate jmsTemplate;

  private MessageConverter jmsMessageConverter = new MessagingMessageConverter();

  private boolean converterSet;

  @Nullable private String defaultDestinationName;

  /**
   * Constructor for use with bean properties. Requires {@link #setConnectionFactory} or {@link
   * #setJmsTemplate} to be called.
   */
  public JmsMessagingTemplate() {}

  /**
   * Create a {@code JmsMessagingTemplate} instance with the JMS {@link ConnectionFactory} to use,
   * implicitly building a {@link JmsTemplate} based on it.
   *
   * @since 4.1.2
   */
  ...
  /**
   * Set the {@link MessageConverter} to use to convert a {@link Message} from the messaging to and
   * from a {@link javax.jms.Message}. By default, a {@link MessagingMessageConverter} is defined
   * using a {@link SimpleMessageConverter} to convert the payload of the message.
   *
   * <p>Consider configuring a {@link MessagingMessageConverter} with a different {@link
   * MessagingMessageConverter#setPayloadConverter(MessageConverter) payload converter} for more
   * advanced scenarios.
   *
  ...
  /**
   * Return the {@link MessageConverter} to use to convert a {@link Message} from the messaging to
   * and from a {@link javax.jms.Message}.
   */
  ...
  /**
   * Configure the default destination name to use in send methods that don't have a destination
   * argument. If a default destination is not configured, send methods without a destination
   * argument will raise an exception if invoked.
   *
  ...
  public void convertAndSend(
      String destinationName, Object payload, @Nullable Map<String, Object> headers)
      throws MessagingException {
  ...
  @Override
  public void convertAndSend(
      String destinationName, Object payload, @Nullable MessagePostProcessor postProcessor)
      throws MessagingException {
  ...
  public <T> T convertSendAndReceive(
    ...
    if (defaultDestination != null) {
      ...
      return convertSendAndReceive(
          getRequiredDefaultDestinationName(), request, targetClass, postProcessor);
    }
  }
  ...
  public <T> T convertSendAndReceive(
    ...
    return (replyMessage != null
        ? (T) getMessageConverter().fromMessage(replyMessage, targetClass)
        : null);
  }
  ...
  protected Message<?> doSendAndReceive(Destination destination, Message<?> requestMessage) {
    try {
      javax.jms.Message jmsMessage =
          obtainJmsTemplate().sendAndReceive(destination, createMessageCreator(requestMessage));
      return convertJmsMessage(jmsMessage);
    } catch (JmsException ex) {
      throw convertJmsException(ex);
    }
  ...
  protected Message<?> doSendAndReceive(String destinationName, Message<?> requestMessage) {
    try {
      javax.jms.Message jmsMessage =
          obtainJmsTemplate().sendAndReceive(destinationName, createMessageCreator(requestMessage));
      return convertJmsMessage(jmsMessage);
    } catch (JmsException ex) {
      throw convertJmsException(ex);
    }
