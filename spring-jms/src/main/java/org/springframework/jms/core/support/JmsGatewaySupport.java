 ...
 *
 * <p>Requires a ConnectionFactory or a JmsTemplate instance to be set. It will create its own
 * JmsTemplate if a ConnectionFactory is passed in. A custom JmsTemplate instance can be created for
 * a given ConnectionFactory through overriding the {@link #createJmsTemplate} method.
 *
 * @author Mark Pollack
 * @since 1.1.1
...
public abstract class JmsGatewaySupport implements InitializingBean {
  ...
  /**
   * Set the JMS connection factory to be used by the gateway. Will automatically create a
   * JmsTemplate for the given ConnectionFactory.
   *
   * @see #createJmsTemplate
  ...
  /**
   * Create a JmsTemplate for the given ConnectionFactory. Only invoked if populating the gateway
   * with a ConnectionFactory reference.
   *
   * <p>Can be overridden in subclasses to provide a JmsTemplate instance with a different
   * configuration.
   *
