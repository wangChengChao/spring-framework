 ...
 *
 * <p>If you want to use dynamic destination creation, you must specify the type of JMS destination
 * to create, using the "pubSubDomain" property. For other operations, this is not necessary.
 * Point-to-Point (Queues) is the default domain.
 *
 * <p>Default settings for JMS Sessions are "not transacted" and "auto-acknowledge". As defined by
 * the Java EE specification, the transaction and acknowledgement parameters are ignored when a JMS
 * Session is created inside an active transaction, no matter if a JTA transaction or a
 * Spring-managed transaction. To configure them for native JMS usage, specify appropriate values
 * for the "sessionTransacted" and "sessionAcknowledgeMode" bean properties.
 *
 * <p>This template uses a {@link
 * org.springframework.jms.support.destination.DynamicDestinationResolver} and a {@link
 * org.springframework.jms.support.converter.SimpleMessageConverter} as default strategies for
 * resolving a destination name or converting a message, respectively. These defaults can be
 * overridden through the "destinationResolver" and "messageConverter" bean properties.
 *
 * <p><b>NOTE: The {@code ConnectionFactory} used with this template should return pooled
 * Connections (or a single shared Connection) as well as pooled Sessions and MessageProducers.
 * Otherwise, performance of ad-hoc JMS operations is going to suffer.</b> The simplest option is to
 * use the Spring-provided {@link org.springframework.jms.connection.SingleConnectionFactory} as a
 * decorator for your target {@code ConnectionFactory}, reusing a single JMS Connection in a
 * thread-safe fashion; this is often good enough for the purpose of sending messages via this
 * template. In a Java EE environment, make sure that the {@code ConnectionFactory} is obtained from
 * the application's environment naming context via JNDI; application servers typically expose
 * pooled, transaction-aware factories there.
 *
 * @author Mark Pollack
...
public class JmsTemplate extends JmsDestinationAccessor implements JmsOperations {
  ...
  @Nullable private Object defaultDestination;
   ...
   *
   * <p>Note: The ConnectionFactory has to be set before using the instance. This constructor can be
   * used to prepare a JmsTemplate via a BeanFactory, typically setting the ConnectionFactory via
  ...
  /**
   * Set the destination to be used on send/receive operations that do not have a destination
   * parameter.
   *
   * <p>Alternatively, specify a "defaultDestinationName", to be dynamically resolved via the
   * DestinationResolver.
   *
  ...
  public Destination getDefaultDestination() {
    return (this.defaultDestination instanceof Destination
  ...
  /**
   * Return the destination name to be used on send/receive operations that do not have a
  ...
  /**
   * Set the message converter for this template. Used to resolve Object parameters to
   * convertAndSend methods and Object results from receiveAndConvert methods.
   *
   * <p>The default converter is a SimpleMessageConverter, which is able to handle BytesMessages,
  ...
  private MessageConverter getRequiredMessageConverter() throws IllegalStateException {
    ...
    if (converter == null) {
      throw new IllegalStateException(
          "No 'messageConverter' specified. Check configuration of JmsTemplate.");
    }
    return converter;
  }
   ...
   *
   * <p>This is only a hint to the JMS producer. See the JMS javadocs for details.
   *
   ...
   *
   * <p>The default is {@link #RECEIVE_TIMEOUT_INDEFINITE_WAIT}, which indicates a blocking receive
   * without timeout.
   *
   * <p>Specify {@link #RECEIVE_TIMEOUT_NO_WAIT} (or any other negative value) to indicate that a
   * receive operation should check if a message is immediately available without blocking.
   *
   ...
   *
   * <p>The default is -1 (no delivery delay passed on to the broker). Note that this feature
   * requires JMS 2.0.
   */
  ...
  /**
   * Set if the QOS values (deliveryMode, priority, timeToLive) should be used for sending a
   * message.
   *
   * @see #setDeliveryMode
   * @see #setPriority
  ...
  /**
   * If "true", then the values of deliveryMode, priority, and timeToLive will be used when sending
   * a message. Otherwise, the default values, that may be set administratively, will be used.
   *
   * @return true if overriding default values of QOS parameters (deliveryMode, priority, and
   *     timeToLive)
   * @see #setDeliveryMode
   * @see #setPriority
   * @see #setTimeToLive
  ...
  /**
   * Set whether message delivery should be persistent or non-persistent, specified as boolean value
   * ("true" or "false"). This will set the delivery mode accordingly, to either "PERSISTENT" (2) or
   * "NON_PERSISTENT" (1).
   *
  ...
  public void setDeliveryPersistent(boolean deliveryPersistent) {
    this.deliveryMode =
        (deliveryPersistent ? DeliveryMode.PERSISTENT : DeliveryMode.NON_PERSISTENT);
  }

  /**
   * Set the delivery mode to use when sending a message. Default is the JMS Message default:
   * "PERSISTENT".
   *
   * <p>Since a default value may be defined administratively, this is only used when
   * "isExplicitQosEnabled" equals "true".
   *
  ...
  /**
   * Execute the action specified by the given action object within a JMS Session. Generalized
   * version of {@code execute(SessionCallback)}, allowing the JMS Connection to be started on the
   * fly.
   *
   * <p>Use {@code execute(SessionCallback)} for the general case. Starting the JMS Connection is
   * just necessary for receiving messages, which is preferably achieved through the {@code receive}
   * methods.
   *
  ...
  @Override
  @Nullable
  public <T> T execute(final @Nullable Destination destination, final ProducerCallback<T> action)
      throws JmsException {
    Assert.notNull(action, "Callback object must not be null");
    return execute(
        session -> {
  ...
  @Override
  @Nullable
  public <T> T execute(final String destinationName, final ProducerCallback<T> action)
      throws JmsException {
    Assert.notNull(action, "Callback object must not be null");
    return execute(
        session -> {
  ...
  public Message receiveSelected(
      final Destination destination, @Nullable final String messageSelector) throws JmsException {
  ...
  @Nullable
  public Message receiveSelected(
      final String destinationName, @Nullable final String messageSelector) throws JmsException {
    return execute(
        session -> {
          Destination destination = resolveDestinationName(session, destinationName);
  ...
  protected Message doReceive(
      Session session, Destination destination, @Nullable String messageSelector)
      throws JMSException {
  ...
  /**
   * Send a request message to the given {@link Destination} and block until a reply has been
   * received on a temporary queue created on-the-fly.
   *
   ...
   */
  @Nullable
  protected Message doSendAndReceive(
      Session session, Destination destination, MessageCreator messageCreator) throws JMSException {
  ...
  /**
   * A variant of {@link #execute(SessionCallback, boolean)} that explicitly creates a
   * non-transactional {@link Session}. The given {@link SessionCallback} does not participate in an
   * existing transaction.
   */
  @Nullable
  private <T> T executeLocal(SessionCallback<T> action, boolean startConnection)
      throws JmsException {
    Assert.notNull(action, "Callback object must not be null");
    Connection con = null;
  ...
  public <T> T browseSelected(
      final Queue queue, @Nullable final String messageSelector, final BrowserCallback<T> action)
      throws JmsException {
  ...
  public <T> T browseSelected(
      ...
      final BrowserCallback<T> action)
      throws JmsException {
    ...
    return execute(
        session -> {
          Queue queue =
              (Queue) getDestinationResolver().resolveDestinationName(session, queueName, false);
          QueueBrowser browser = createBrowser(session, queue, messageSelector);
          try {
            return action.doInJms(session, browser);
          } finally {
   ...
   * @return an appropriate Connection fetched from the holder, or {@code null} if none found
   */
  @Nullable
   ...
   *
   * @param holder the JmsResourceHolder
   * @return an appropriate Session fetched from the holder, or {@code null} if none found
   */
  @Nullable
  ...
  /**
   * Create a JMS MessageProducer for the given Session and Destination, configuring it to disable
   * message ids and/or timestamps (if necessary).
   *
   * <p>Delegates to {@link #doCreateProducer} for creation of the raw JMS MessageProducer.
   *
   ...
   */
  protected MessageProducer createProducer(Session session, @Nullable Destination destination)
      throws JMSException {
   ...
   * @throws JMSException if thrown by JMS API methods
   */
  protected MessageProducer doCreateProducer(Session session, @Nullable Destination destination)
      throws JMSException {
  ...
  protected QueueBrowser createBrowser(
      Session session, Queue queue, @Nullable String messageSelector) throws JMSException {
