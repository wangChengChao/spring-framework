 ...
 *
 * <p>Implemented by {@link JmsTemplate}. Not often used but a useful option to enhance testability,
 * as it can easily be mocked or stubbed.
 *
 * <p>Provides {@code JmsTemplate's} {@code send(..)} and {@code receive(..)} methods that mirror
 * various JMS API methods. See the JMS specification and javadocs for details on those methods.
 *
 * <p>Provides also basic request reply operation using a temporary queue to collect the reply.
 *
 * @author Mark Pollack
...
public interface JmsOperations {
  ...
  /**
   * Send messages to the default JMS destination (or one specified for each send operation). The
   * callback gives access to the JMS Session and MessageProducer in order to perform complex send
   * operations.
   *
  ...
  /**
   * Send messages to a JMS destination. The callback gives access to the JMS Session and
   * MessageProducer in order to perform complex send operations.
   *
   ...
   * MessageProducer in order to perform complex send operations.
   *
   * @param destinationName the name of the destination to send messages to (to be resolved to an
   *     actual destination by a DestinationResolver)
   * @param action callback object that exposes the session/producer pair
   * @return the result object from working with the session
   * @throws JmsException checked JMSException converted to unchecked
   */
  ...
  /**
   * Send a message to the specified destination. The MessageCreator callback creates the message
   * given a Session.
   *
   ...
   * given a Session.
   *
   * @param destinationName the name of the destination to send this message to (to be resolved to
   *     an actual destination by a DestinationResolver)
   * @param messageCreator callback to create a message
   * @throws JmsException checked JMSException converted to unchecked
   */
  ...
  /**
   * Send the given object to the default destination, converting the object to a JMS message with a
  ...
  /**
   * Send the given object to the specified destination, converting the object to a JMS message with
  ...
  /**
   * Send the given object to the default destination, converting the object to a JMS message with a
   * configured MessageConverter. The MessagePostProcessor callback allows for modification of the
  ...
  /**
   * Send the given object to the specified destination, converting the object to a JMS message with
   * a configured MessageConverter. The MessagePostProcessor callback allows for modification of the
   * message after conversion.
   *
   * @param destination the destination to send this message to
   * @param message the object to convert to a message
  ...
  /**
   * Receive a message synchronously from the default destination, but only wait up to a specified
   * time for delivery.
   *
   * <p>This method should be used carefully, since it will block the thread until the message
   * becomes available or until the timeout value is exceeded.
   *
   * <p>This will only work with a default destination specified!
   *
  ...
  /**
   * Receive a message synchronously from the specified destination, but only wait up to a specified
   * time for delivery.
   *
   * <p>This method should be used carefully, since it will block the thread until the message
   * becomes available or until the timeout value is exceeded.
   ...
   * <p>This will only work with a default destination specified!
   *
   * @param messageSelector the JMS message selector expression (or {@code null} if none). See the
   *     JMS specification for a detailed definition of selector expressions.
   * @return the message received by the consumer, or {@code null} if the timeout expires
   * @throws JmsException checked JMSException converted to unchecked
   */
  @Nullable
  ...
  /**
   * Receive a message synchronously from the default destination, but only wait up to a specified
   * time for delivery. Convert the message into an object with a configured MessageConverter.
   *
   * <p>This method should be used carefully, since it will block the thread until the message
   * becomes available or until the timeout value is exceeded.
   *
  ...
  /**
   * Send a request message and receive the reply from a default destination. The {@link
   * MessageCreator} callback creates the message given a Session. A temporary queue is created as
   * part of this operation and is set in the {@code JMSReplyTO} header of the message.
   *
   * <p>This will only work with a default destination specified!
   *
  ...
  /**
   * Send a message and receive the reply from the specified destination. The {@link MessageCreator}
   * callback creates the message given a Session. A temporary queue is created as part of this
   * operation and is set in the {@code JMSReplyTO} header of the message.
   *
   * @param destination the destination to send this message to
   * @param messageCreator callback to create a message
   * @return the reply, possibly {@code null} if the message could not be received, for example due
  ...
  /**
   * Browse messages in the default JMS queue. The callback gives access to the JMS Session and
  ...
  /**
   * Browse messages in a JMS queue. The callback gives access to the JMS Session and QueueBrowser
   * in order to browse the queue and react to the contents.
   *
   ...
   * in order to browse the queue and react to the contents.
   *
   * @param queueName the name of the queue to browse (to be resolved to an actual destination by a
   *     DestinationResolver)
   * @param action callback object that exposes the session/browser pair
   * @return the result object from working with the session
   * @throws JmsException checked JMSException converted to unchecked
  ...
  /**
   * Browse selected messages in a JMS queue. The callback gives access to the JMS Session and
   * QueueBrowser in order to browse the queue and react to the contents.
   *
   * @param messageSelector the JMS message selector expression (or {@code null} if none). See the
   *     JMS specification for a detailed definition of selector expressions.
   ...
   */
  @Nullable
  <T> T browseSelected(String queueName, String messageSelector, BrowserCallback<T> action)
      throws JmsException;
}
