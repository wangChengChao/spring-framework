 ...
 *
 * <p>To be used with {@link JmsTemplate}'s callback methods that take a {@link ProducerCallback}
 * argument, often implemented as an anonymous inner class or as a lambda expression.
 *
 * <p>The typical implementation will perform multiple operations on the supplied JMS {@link
 * Session} and {@link MessageProducer}.
 *
 * @author Mark Pollack
 * @since 1.1
...
public interface ProducerCallback<T> {
   ...
   *
   * <p>The message producer is not associated with any destination unless when specified in the
   * JmsTemplate call.
   *
   * @param session the JMS {@code Session} object to use
   * @param producer the JMS {@code MessageProducer} object to use
   * @return a result object from working with the {@code Session}, if any (or {@code null} if none)
