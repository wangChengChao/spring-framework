 ...
 *
 * <p>Serializes remote invocation objects and deserializes remote invocation result objects. Uses
 * Java serialization just like RMI, but with the JMS provider as communication infrastructure.
 *
 * <p>To be configured with a {@link javax.jms.QueueConnectionFactory} and a target queue (either as
 * {@link javax.jms.Queue} reference or as queue name).
 *
 * <p>Thanks to James Strachan for the original prototype that this JMS invoker mechanism was
 * inspired by!
 *
...
public class JmsInvokerClientInterceptor implements MethodInterceptor, InitializingBean {
   ...
   *
   * <p>The specified name will be dynamically resolved via the {@link #setDestinationResolver
   * DestinationResolver}.
   */
   ...
   *
   * <p>The default resolver is a {@code DynamicDestinationResolver}. Specify a {@code
   ...
   */
  public void setRemoteInvocationFactory(
      @Nullable RemoteInvocationFactory remoteInvocationFactory) {
    this.remoteInvocationFactory =
        (remoteInvocationFactory != null
            ? remoteInvocationFactory
            : new DefaultRemoteInvocationFactory());
  }

  /**
   * Specify the {@link MessageConverter} to use for turning {@link
   * org.springframework.remoting.support.RemoteInvocation} objects into request messages, as well
   * as response messages into {@link org.springframework.remoting.support.RemoteInvocationResult}
   * objects.
   *
   * <p>Default is a {@link SimpleMessageConverter}, using a standard JMS {@link
   * javax.jms.ObjectMessage} for each invocation / invocation result object.
   *
   * <p>Custom implementations may generally adapt {@link java.io.Serializable} objects into special
   * kinds of messages, or might be specifically tailored for translating {@code
   * RemoteInvocation(Result)s} into specific kinds of messages.
   */
      ...
      if (result.hasInvocationTargetException()) {
        ...
        throw new RemoteInvocationFailureException(
            "Invocation of method ["
                ...
                + "] failed in JMS invoker remote service at queue ["
                + this.queue
                + "]",
            ex);
      }
   ...
   *
   * <p>Can be overridden in subclasses to provide custom {@code RemoteInvocation} subclasses,
   * containing additional invocation parameters like user credentials. Note that it is preferable
   * to use a custom {@code RemoteInvocationFactory} which is a reusable strategy.
   *
  ...
  /**
   * Execute the given remote invocation, sending an invoker request message to this accessor's
   * target queue and waiting for a corresponding response.
   *
   ...
   *
   * <p>The default implementation creates a JMS {@link javax.jms.ObjectMessage} for the given
   ...
   *
   * <p>The default implementation is based on standard JMS send/receive, using a {@link
   ...
   *
   * <p>The default implementation expects a JMS {@link javax.jms.ObjectMessage} carrying a {@link
   * RemoteInvocationResult} object. If an invalid response message is encountered, the {@code
  ...
  /**
   * Callback that is invoked by {@link #executeRequest} when the receive timeout has expired for
   * the specified {@link RemoteInvocation}.
   *
   * <p>By default, an {@link RemoteTimeoutException} is thrown. Sub-classes can choose to either
   * throw a more dedicated exception or even return a default {@link RemoteInvocationResult} as a
   * fallback.
   *
  ...
  /**
   * Callback that is invoked by {@link #extractInvocationResult} when it encounters an invalid
   * response message.
   *
   * <p>The default implementation throws a {@link MessageFormatException}.
   *
   * @param responseMessage the invalid response message
   * @return an alternative invocation result that should be returned to the caller (if desired)
   * @throws JMSException if the invalid response should lead to an infrastructure exception
  ...
  /**
   * Recreate the invocation result contained in the given {@link RemoteInvocationResult} object.
   *
   * <p>The default implementation calls the default {@code recreate()} method.
   *
   * <p>Can be overridden in subclasses to provide custom recreation, potentially processing the
  ...
  /**
   * Convert the given JMS invoker access exception to an appropriate Spring {@link
