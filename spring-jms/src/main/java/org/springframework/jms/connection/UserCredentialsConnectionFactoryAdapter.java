...
/**
 * An adapter for a target JMS {@link javax.jms.ConnectionFactory}, applying the given user
 * credentials to every standard {@code createConnection()} call, that is, implicitly invoking
 * {@code createConnection(username, password)} on the target. All other methods simply delegate to
 * the corresponding methods of the target ConnectionFactory.
 *
 * <p>Can be used to proxy a target JNDI ConnectionFactory that does not have user credentials
 * configured. Client code can work with the ConnectionFactory without passing in username and
 * password on every {@code createConnection()} call.
 *
 * <p>In the following example, client code can simply transparently work with the preconfigured
 * "myConnectionFactory", implicitly accessing "myTargetConnectionFactory" with the specified user
 * credentials.
 *
 * <pre class="code">
 ...
 * &lt;/bean></pre>
 *
 * <p>If the "username" is empty, this proxy will simply delegate to the standard {@code
 * createConnection()} method of the target ConnectionFactory. This can be used to keep a
 * UserCredentialsConnectionFactoryAdapter bean definition just for the <i>option</i> of implicitly
 * passing in user credentials if the particular target ConnectionFactory requires it.
 *
...
public class UserCredentialsConnectionFactoryAdapter
  ...
  /**
   * Set the username that this adapter should use for retrieving Connections. Default is no
   * specific user.
   */
  ...
  /**
   * Set the password that this adapter should use for retrieving Connections. Default is no
   * specific password.
   */
  ...
  /**
   * Set user credententials for this proxy and the current thread. The given username and password
   * will be applied to all subsequent {@code createConnection()} calls on this ConnectionFactory
   * proxy.
   *
   * <p>This will override any statically specified user credentials, that is, values of the
   * "username" and "password" bean properties.
   *
  ...
  /**
   * Remove any user credentials for this proxy from the current thread. Statically specified user
   * credentials apply again afterwards.
   *
  ...
  /**
   * Determine whether there are currently thread-bound credentials, using them if available,
   * falling back to the statically specified username and password (i.e. values of the bean
  ...
  /**
   * This implementation delegates to the {@code createConnection(username, password)} method of the
   * target ConnectionFactory, passing in the specified user credentials. If the specified username
   * is empty, it will simply delegate to the standard {@code createConnection()} method of the
  ...
  /**
   * This implementation delegates to the {@code createQueueConnection(username, password)} method
   * of the target QueueConnectionFactory, passing in the specified user credentials. If the
   * specified username is empty, it will simply delegate to the standard {@code
  ...
  /**
   * This implementation delegates to the {@code createTopicConnection(username, password)} method
   * of the target TopicConnectionFactory, passing in the specified user credentials. If the
   * specified username is empty, it will simply delegate to the standard {@code
  ...
  private static final class JmsUserCredentials {
    ...
    public String toString() {
      return "JmsUserCredentials[username='"
          + this.username
          + "',password='"
          + this.password
          + "']";
