...
/**
 * Resource holder wrapping a JMS {@link Connection} and a JMS {@link Session}. {@link
 * JmsTransactionManager} binds instances of this class to the thread, for a given JMS {@link
 * ConnectionFactory}.
 *
...
public class JmsResourceHolder extends ResourceHolderSupport {
   ...
   * Create a new JmsResourceHolder that is open for resources to be added.
   *
   * @param connectionFactory the JMS ConnectionFactory that this resource holder is associated with
   *     (may be {@code null})
   */
   ...
   */
  public JmsResourceHolder(
      @Nullable ConnectionFactory connectionFactory, Connection connection, Session session) {
    this.connectionFactory = connectionFactory;
    addConnection(connection);
    addSession(session, connection);
    this.frozen = true;
   ...
   * Connections and Sessions to it.
   *
   * @see #addConnection
   * @see #addSession
   */
  ...
  /** Return this resource holder's default Connection, or {@code null} if none. */
  @Nullable
   ...
   *
   * <p>In contrast to {@link #getSession()}, this must not lazily initialize a new Session, not
   * even in {@link JmsResourceHolder} subclasses.
   */
  @Nullable
  ...
  /** Return this resource holder's default Session, or {@code null} if none. */
  @Nullable
  ...
  /**
   * Return this resource holder's Session of the given type for the given connection, or {@code
   * null} if none.
   */
  @Nullable
    ...
    for (Session session : this.sessions) {
      try {
        ...
        if (this.connectionFactory != null) {
          try {
            Method getDataSourceMethod =
                this.connectionFactory.getClass().getMethod("getDataSource");
