...
/**
 * Proxy for a target JMS {@link javax.jms.ConnectionFactory}, adding awareness of Spring-managed
 * transactions. Similar to a transactional JNDI ConnectionFactory as provided by a Java EE
 * application server.
 *
 * <p>Messaging code which should remain unaware of Spring's JMS support can work with this proxy to
 * seamlessly participate in Spring-managed transactions. Note that the transaction manager, for
 * example {@link JmsTransactionManager}, still needs to work with the underlying ConnectionFactory,
 * <i>not</i> with this proxy.
 *
 * <p><b>Make sure that TransactionAwareConnectionFactoryProxy is the outermost ConnectionFactory of
 * a chain of ConnectionFactory proxies/adapters.</b> TransactionAwareConnectionFactoryProxy can
 * delegate either directly to the target factory or to some intermediary adapter like {@link
 * UserCredentialsConnectionFactoryAdapter}.
 *
 * <p>Delegates to {@link ConnectionFactoryUtils} for automatically participating in thread-bound
 * transactions, for example managed by {@link JmsTransactionManager}. {@code createSession} calls
 * and {@code close} calls on returned Sessions will behave properly within a transaction, that is,
 * always work on the transactional Session. If not within a transaction, normal ConnectionFactory
 * behavior applies.
 *
 * <p>Note that transactional JMS Sessions will be registered on a per-Connection basis. To share
 * the same JMS Session across a transaction, make sure that you operate on the same JMS Connection
 * handle - either through reusing the handle or through configuring a {@link
 * SingleConnectionFactory} underneath.
 *
 * <p>Returned transactional Session proxies will implement the {@link SessionProxy} interface to
 * allow for access to the underlying target Session. This is only intended for accessing
 * vendor-specific Session API or for testing purposes (e.g. to perform manual transaction control).
 * For typical application purposes, simply use the standard JMS Session interface.
 *
 * <p>As of Spring Framework 5, this class delegates JMS 2.0 {@code JMSContext} calls and therefore
...
public class TransactionAwareConnectionFactoryProxy
  ...
  /**
   * Set whether to allow for a local JMS transaction that is synchronized with a Spring-managed
   * transaction (where the main transaction might be a JDBC-based one for a specific DataSource,
   * for example), with the JMS transaction committing right after the main transaction. If not
   * allowed, the given ConnectionFactory needs to handle transaction enlistment underneath the
   * covers.
   *
   * <p>Default is "false": If not within a managed transaction that encompasses the underlying JMS
   * ConnectionFactory, standard Sessions will be returned. Turn this flag on to allow participation
   * in any Spring-managed transaction, with a local JMS transaction synchronized with the main
   * transaction.
   */
  ...
  /**
   * Return whether to allow for a local JMS transaction that is synchronized with a Spring-managed
   * transaction.
   */
  ...
  public QueueConnection createQueueConnection() throws JMSException {
    ...
    if (!(target instanceof QueueConnectionFactory)) {
      throw new javax.jms.IllegalStateException(
          "'targetConnectionFactory' is no QueueConnectionFactory");
    }
  ...
  public QueueConnection createQueueConnection(String username, String password)
    ...
    QueueConnection targetConnection =
        ((QueueConnectionFactory) target).createQueueConnection(username, password);
    return (QueueConnection) getTransactionAwareConnectionProxy(targetConnection);
  }
  ...
  public TopicConnection createTopicConnection() throws JMSException {
    ...
    if (!(target instanceof TopicConnectionFactory)) {
      throw new javax.jms.IllegalStateException(
          "'targetConnectionFactory' is no TopicConnectionFactory");
    }
  ...
  public TopicConnection createTopicConnection(String username, String password)
    ...
    TopicConnection targetConnection =
        ((TopicConnectionFactory) target).createTopicConnection(username, password);
    return (TopicConnection) getTransactionAwareConnectionProxy(targetConnection);
  }
  ...
  /**
   * Wrap the given Connection with a proxy that delegates every method call to it but handles
   * Session lookup in a transaction-aware fashion.
   *
      ...
      if (method.getName().equals("equals")) {
        ...
        QueueSession session =
            ConnectionFactoryUtils.getTransactionalQueueSession(
                (QueueConnectionFactory) getTargetConnectionFactory(),
                (QueueConnection) this.target,
        ...
        TopicSession session =
            ConnectionFactoryUtils.getTransactionalTopicSession(
                (TopicConnectionFactory) getTargetConnectionFactory(),
                (TopicConnection) this.target,
                isSynchedLocalTransactionAllowed());
        if (session != null) {
          return getCloseSuppressingSessionProxy(session);
        }
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      ...
      if (method.getName().equals("equals")) {
        ...
        throw new TransactionInProgressException(
            "Commit call not allowed within a managed transaction");
      } else if (method.getName().equals("rollback")) {
        throw new TransactionInProgressException(
            "Rollback call not allowed within a managed transaction");
