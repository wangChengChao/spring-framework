...
/**
 * A JMS ConnectionFactory adapter that returns the same Connection from all {@link
 * #createConnection()} calls, and ignores calls to {@link javax.jms.Connection#close()}. According
 * to the JMS Connection model, this is perfectly thread-safe (in contrast to e.g. JDBC). The shared
 * Connection can be automatically recovered in case of an Exception.
 *
 * <p>You can either pass in a specific JMS Connection directly or let this factory lazily create a
 * Connection via a given target ConnectionFactory. This factory generally works with JMS 1.1 as
 * well as the JMS 1.0.2 API.
 *
 * <p>Note that when using the JMS 1.0.2 API, this ConnectionFactory will switch into queue/topic
 * mode according to the JMS API methods used at runtime: {@code createQueueConnection} and {@code
 * createTopicConnection} will lead to queue/topic mode, respectively; generic {@code
 * createConnection} calls will lead to a JMS 1.1 connection which is able to serve both modes.
 *
 * <p>As of Spring Framework 5, this class supports JMS 2.0 {@code JMSContext} calls and therefore
 * requires the JMS 2.0 API to be present at runtime. It may nevertheless run against a JMS 1.1
 * driver (bound to the JMS 2.0 API) as long as no actual JMS 2.0 calls are triggered by the
 * application's setup.
 *
 * <p>Useful for testing and standalone environments in order to keep using the same Connection for
 * multiple {@link org.springframework.jms.core.JmsTemplate} calls, without having a pooling
 * ConnectionFactory underneath. This may span any number of transactions, even concurrently
 * executing transactions.
 *
 * <p>Note that Spring's message listener containers support the use of a shared Connection within
 * each listener container instance. Using SingleConnectionFactory in combination only really makes
 * sense for sharing a single JMS Connection <i>across multiple listener containers</i>.
 *
...
public class SingleConnectionFactory
  ...
  @Nullable private Boolean pubSubMode;

  /** An internal aggregator allowing for per-connection ExceptionListeners. */
  @Nullable private AggregatedExceptionListener aggregatedExceptionListener;
  ...
  /**
   * Create a new SingleConnectionFactory that always returns a single Connection that it will
   * lazily create via the given target ConnectionFactory.
   *
  ...
  /**
   * Return the target ConnectionFactory which will be used to lazily create a single Connection, if
   * any.
   */
  @Nullable
  ...
  /**
   * Specify a JMS client ID for the single Connection created and exposed by this
   * ConnectionFactory.
   *
   * <p>Note that client IDs need to be unique among all active Connections of the underlying JMS
   * provider. Furthermore, a client ID can only be assigned if the original ConnectionFactory
  ...
  /**
   * Return a JMS client ID for the single Connection created and exposed by this ConnectionFactory,
   * if any.
   */
  @Nullable
  ...
  /**
   * Specify an JMS ExceptionListener implementation that should be registered with the single
   * Connection created by this factory.
   *
  ...
  /**
   * Return the JMS ExceptionListener implementation that should be registered with the single
   * Connection created by this factory, if any.
   */
  @Nullable
  ...
  /**
   * Specify whether the single Connection should be reset (to be subsequently renewed) when a
   * JMSException is reported by the underlying Connection.
   *
   * <p>Default is "false". Switch this to "true" to automatically trigger recovery based on your
   * JMS provider's exception notifications.
   *
   * <p>Internally, this will lead to a special JMS ExceptionListener (this SingleConnectionFactory
   * itself) being registered with the underlying Connection. This can also be combined with a
  ...
  /**
   * Return whether the single Connection should be renewed when a JMSException is reported by the
   ...
   *
   * <p>Closes and reinitializes the Connection if an underlying Connection is present already.
   *
   * @throws javax.jms.JMSException if thrown by JMS API methods
  ...
  /**
   * Close the underlying shared connection. The provider of this ConnectionFactory needs to care
   * for proper shutdown.
   *
   * <p>As this bean implements DisposableBean, a bean factory will automatically invoke this on
   * destruction of its cached singletons.
   *
   * @see #resetConnection()
   */
  @Override
   ...
   *
   * <p>The default implementation applies ExceptionListener and client id. Can be overridden in
   * subclasses.
   *
   ...
   *
   * <p>The default implementation always returns {@code null}. Subclasses may override this for
   * exposing specific Session handles, possibly delegating to {@link #createSession} for the
   * creation of raw Session objects that will then get wrapped and returned from here.
   *
   * @param con the JMS Connection to operate on
   * @param mode the Session acknowledgement mode ({@code Session.TRANSACTED} or one of the common
   *     modes)
  ...
  /**
   * Create a default Session for this ConnectionFactory, adapting to JMS 1.0.2 style queue/topic
   * mode if necessary.
   *
   * @param con the JMS Connection to operate on
   * @param mode the Session acknowledgement mode ({@code Session.TRANSACTED} or one of the common
  ...
  /**
   * Wrap the given Connection with a proxy that delegates every method call to it but suppresses
   * close calls. This is useful for allowing application code to handle a special framework
   * Connection just like an ordinary Connection from a JMS ConnectionFactory.
   *
  ...
  private class SharedConnectionInvocationHandler implements InvocationHandler {
    ...
    @Nullable private ExceptionListener localExceptionListener;
    ...
    public Object invoke(Object proxy, Method method, @Nullable Object[] args) throws Throwable {
      if (method.getName().equals("equals") && args != null) {
        ...
        return null;
          ...
          || method.getName().equals("createQueueSession")
  ...
  /**
   * Internal aggregated ExceptionListener for handling the internal recovery listener in
   * combination with user-specified listeners.
   */
