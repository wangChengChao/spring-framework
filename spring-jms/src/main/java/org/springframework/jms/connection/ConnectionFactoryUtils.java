...
/**
 * Helper class for managing a JMS {@link javax.jms.ConnectionFactory}, in particular for obtaining
 * transactional JMS resources for a given ConnectionFactory.
 *
 * <p>Mainly for internal use within the framework. Used by {@link
 * org.springframework.jms.core.JmsTemplate} as well as {@link
...
public abstract class ConnectionFactoryUtils {
  ...
  public static void releaseConnection(
      @Nullable Connection con, @Nullable ConnectionFactory cf, boolean started) {
    if (con == null) {
      return;
    }
    if (started
  ...
  /**
   * Return the innermost target Session of the given Session. If the given Session is a proxy, it
   * will be unwrapped until a non-proxy Session is found. Otherwise, the passed-in Session will be
   * returned as-is.
   *
   ...
   */
  public static boolean isSessionTransactional(
      @Nullable Session session, @Nullable ConnectionFactory cf) {
   ...
   * @param existingCon the existing JMS Connection to obtain a Session for (may be {@code null})
   * @param synchedLocalTransactionAllowed whether to allow for a local JMS transaction that is
   *     synchronized with a Spring-managed transaction (where the main transaction might be a
   *     JDBC-based one for a specific DataSource, for example), with the JMS transaction committing
   *     right after the main transaction. If not allowed, the given ConnectionFactory needs to
   *     handle transaction enlistment underneath the covers.
  ...
  public static Session getTransactionalSession(
      ...
      @Nullable final Connection existingCon,
      final boolean synchedLocalTransactionAllowed)
      throws JMSException {

    return doGetTransactionalSession(
        cf,
        new ResourceFactory() {
          @Override
          @Nullable
  ...
  public static QueueSession getTransactionalQueueSession(
      final QueueConnectionFactory cf,
      @Nullable final QueueConnection existingCon,
      final boolean synchedLocalTransactionAllowed)
      throws JMSException {
    ...
    return (QueueSession)
        doGetTransactionalSession(
            ...
            new ResourceFactory() {
              ...
              @Nullable
              public Connection getConnection(JmsResourceHolder holder) {
                return (existingCon != null
                    ? existingCon
                    : holder.getConnection(QueueConnection.class));
              }
              ...
              public Session createSession(Connection con) throws JMSException {
                return ((QueueConnection) con)
                    .createQueueSession(synchedLocalTransactionAllowed, Session.AUTO_ACKNOWLEDGE);
              }
  ...
  public static TopicSession getTransactionalTopicSession(
      final TopicConnectionFactory cf,
      @Nullable final TopicConnection existingCon,
      final boolean synchedLocalTransactionAllowed)
      throws JMSException {
        ...
        doGetTransactionalSession(
            ...
            new ResourceFactory() {
              ...
              public Connection getConnection(JmsResourceHolder holder) {
                return (existingCon != null
                    ? existingCon
                    : holder.getConnection(TopicConnection.class));
              }
              ...
              public Session createSession(Connection con) throws JMSException {
                return ((TopicConnection) con)
                    .createTopicSession(synchedLocalTransactionAllowed, Session.AUTO_ACKNOWLEDGE);
              }
   ...
   * Obtain a JMS Session that is synchronized with the current transaction, if any.
   *
   * <p>This {@code doGetTransactionalSession} variant always starts the underlying JMS Connection,
   * assuming that the Session will be used for receiving messages.
   *
  ...
  /**
   * Callback interface for resource creation. Serving as argument for the {@code
  ...
  public interface ResourceFactory {
    ...
    /**
     * Return whether to allow for a local JMS transaction that is synchronized with a
     * Spring-managed transaction (where the main transaction might be a JDBC-based one for a
     * specific DataSource, for example), with the JMS transaction committing right after the main
     * transaction.
     *
  ...
  /**
   * Callback for resource cleanup at the end of a non-native JMS transaction (e.g. when
   * participating in a JtaTransactionManager transaction).
   *
  ...
  private static class JmsResourceSynchronization
    ...
    public JmsResourceSynchronization(
        JmsResourceHolder resourceHolder, Object resourceKey, boolean transacted) {
