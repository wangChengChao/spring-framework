 ...
 * JMS {@link javax.jms.ConnectionFactory}. Binds a JMS Connection/Session pair from the specified
 * ConnectionFactory to the thread, potentially allowing for one thread-bound Session per
 * ConnectionFactory.
 *
 * <p>This local strategy is an alternative to executing JMS operations within JTA transactions. Its
 * advantage is that it is able to work in any environment, for example a standalone application or
 * a test suite, with any message broker as target. However, this strategy is <i>not</i> able to
 * provide XA transactions, for example in order to share transactions between messaging and
 * database access. A full JTA/XA setup is required for XA transactions, typically using Spring's
 * {@link org.springframework.transaction.jta.JtaTransactionManager} as strategy.
 *
 * <p>Application code is required to retrieve the transactional JMS Session via {@link
 * ConnectionFactoryUtils#getTransactionalSession} instead of a standard Java EE-style {@link
 * ConnectionFactory#createConnection()} call with subsequent Session creation. Spring's {@link
 * org.springframework.jms.core.JmsTemplate} will autodetect a thread-bound Session and
 ...
 * pattern on a ConnectionFactory, for example for legacy code that is not aware of Spring at all.
 * In that case, define a {@link TransactionAwareConnectionFactoryProxy} for your target
 * ConnectionFactory, which will automatically participate in Spring-managed transactions.
 *
 * <p><b>The use of {@link CachingConnectionFactory} as a target for this transaction manager is
 * strongly recommended.</b> CachingConnectionFactory uses a single JMS Connection for all JMS
 * access in order to avoid the overhead of repeated Connection creation, as well as maintaining a
 * cache of Sessions. Each transaction will then share the same JMS Connection, while still using
 * its own individual JMS Session.
 *
 * <p>The use of a <i>raw</i> target ConnectionFactory would not only be inefficient because of the
 * lack of resource reuse. It might also lead to strange effects when your JMS driver doesn't accept
 * {@code MessageProducer.close()} calls and/or {@code MessageConsumer.close()} calls before {@code
 * Session.commit()}, with the latter supposed to commit all the messages that have been sent
 * through the producer handle and received through the consumer handle. As a safe general solution,
 * always pass in a {@link CachingConnectionFactory} into this transaction manager's {@link
 * #setConnectionFactory "connectionFactory"} property.
 *
 * <p>Transaction synchronization is turned off by default, as this manager might be used alongside
 * a datastore-based Spring transaction manager such as the JDBC {@link
 * org.springframework.jdbc.datasource.DataSourceTransactionManager}, which has stronger needs for
 * synchronization.
 *
...
public class JmsTransactionManager extends AbstractPlatformTransactionManager
   ...
   *
   * <p>Turns off transaction synchronization by default, as this manager might be used alongside a
   * datastore-based Spring transaction manager like DataSourceTransactionManager, which has
   * stronger needs for synchronization. Only one manager is allowed to drive synchronization at any
   * point of time.
   *
  ...
  public void setConnectionFactory(@Nullable ConnectionFactory cf) {
    if (cf instanceof TransactionAwareConnectionFactoryProxy) {
      ...
      this.connectionFactory =
          ((TransactionAwareConnectionFactoryProxy) cf).getTargetConnectionFactory();
    } else {
  ...
  /**
   * Specify whether this transaction manager should lazily retrieve a JMS Connection and Session on
   * access within a transaction ({@code true}). By default, it will eagerly create a JMS Connection
   * and Session at transaction begin ({@code false}).
   *
   * @since 5.1.6
  ...
  /**
   * Lazily initializing variant of {@link JmsResourceHolder}, initializing a JMS Connection and
   * Session on user access.
   */
  ...
  /**
   * JMS transaction object, representing a JmsResourceHolder. Used as transaction object by
  ...
  private static class JmsTransactionObject implements SmartTransactionObject {
    ...
    @Nullable private JmsResourceHolder resourceHolder;
