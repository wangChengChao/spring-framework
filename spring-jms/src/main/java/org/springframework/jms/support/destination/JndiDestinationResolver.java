...
/**
 * {@link DestinationResolver} implementation which interprets destination names as JNDI locations
 * (with a configurable fallback strategy).
 *
 * <p>Allows for customizing the JNDI environment if necessary, for example specifying appropriate
 * JNDI environment properties.
 *
 * <p>Dynamic queues and topics get cached by destination name. As a consequence, you need to use
 * unique destination names across both queues and topics. Caching can be turned off through the
 * {@link #setCache "cache"} flag.
 *
 * <p>Note that the fallback to resolution of dynamic destinations is turned <i>off</i> by default.
 * Switch the {@link #setFallbackToDynamicDestination "fallbackToDynamicDestination"} flag on to
 ...
 */
public class JndiDestinationResolver extends JndiLocatorSupport
   ...
   *
   * <p>This flag can be turned off to re-lookup a destination for each operation, which allows for
   * hot restarting of destinations. This is mainly useful during development.
   *
   * <p>Note that dynamic queues and topics get cached by destination name. As a consequence, you
   * need to use unique destination names across both queues and topics.
   */
  ...
  /**
   * Set whether this resolver is supposed to create dynamic destinations if the destination name is
   * not found in JNDI. Default is "false".
   *
  ...
  /**
   * Set the {@link DestinationResolver} to use when falling back to dynamic destinations.
   *
  ...
  public Destination resolveDestinationName(
      @Nullable Session session, String destinationName, boolean pubSubDomain) throws JMSException {
    ...
    if (dest != null) {
      ...
      try {
        ...
        if (this.fallbackToDynamicDestination) {
          dest =
              this.dynamicDestinationResolver.resolveDestinationName(
   ...
   * @param pubSubDomain {@code true} if a Topic is expected, {@code false} in case of a Queue
   */
  protected void validateDestination(
      Destination destination, String destinationName, boolean pubSubDomain) {
