...
/**
 * Convert a {@link Message} from the messaging abstraction to and from a {@link javax.jms.Message}
 * using an underlying {@link MessageConverter} for the payload and a {@link
 * org.springframework.jms.support.JmsHeaderMapper} to map the JMS headers to and from standard
 * message headers.
 *
...
public class MessagingMessageConverter implements MessageConverter, InitializingBean {
  ...
  public MessagingMessageConverter(
      MessageConverter payloadConverter, JmsHeaderMapper headerMapper) {
  ...
  public Object fromMessage(javax.jms.Message message)
    ...
    MessageBuilder<Object> builder =
        (convertedObject instanceof org.springframework.messaging.Message
            ? MessageBuilder.fromMessage(
                (org.springframework.messaging.Message<Object>) convertedObject)
  ...
  /**
   * Create a JMS message for the specified payload and conversionHint. The conversion hint is an
   * extra object passed to the {@link MessageConverter}, e.g. the associated {@code
   * MethodParameter} (may be {@code null}}.
   *
   * @since 4.3
