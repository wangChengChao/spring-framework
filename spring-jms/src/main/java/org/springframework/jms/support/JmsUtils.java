...
/**
 * Generic utility methods for working with JMS. Mainly for internal use within the framework, but
 * also useful for custom JMS access code.
 *
...
public abstract class JmsUtils {
   ...
   * {@code finally} blocks in manual JMS code.
   *
  ...
  /**
   * Close the given JMS Session and ignore any thrown exception. This is useful for typical {@code
  ...
  /**
   * Build a descriptive exception message for the given JMSException, incorporating a linked
   * exception's message if appropriate.
   *
  ...
  public static JmsException convertJmsAccessException(JMSException ex) {
    ...
    if (ex instanceof javax.jms.IllegalStateException) {
      return new org.springframework.jms.IllegalStateException(
          (javax.jms.IllegalStateException) ex);
    }
