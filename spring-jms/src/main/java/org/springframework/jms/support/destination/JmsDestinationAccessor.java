 ...
 * helpers, adding destination-related properties to {@link JmsAccessor JmsAccessor's} common
 * properties.
 *
...
public abstract class JmsDestinationAccessor extends JmsAccessor {
  ...
  /**
   * Timeout value indicating that a receive operation should check if a message is immediately
  ...
  /**
   * Set the {@link DestinationResolver} that is to be used to resolve {@link javax.jms.Destination}
  ...
  /**
   * Resolve the given destination name into a JMS {@link Destination}, via this accessor's {@link
   * DestinationResolver}.
   *
  ...
  protected Destination resolveDestinationName(Session session, String destinationName)
    ...
    return getDestinationResolver()
        .resolveDestinationName(session, destinationName, isPubSubDomain());
  }
   ...
   * @param timeout the receive timeout (a negative value indicates a no-wait receive; 0 indicates
   *     an indefinite wait attempt)
