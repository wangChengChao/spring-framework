 ...
 * Marshals an object to a {@link BytesMessage}, or to a {@link TextMessage} if the {@link
 * #setTargetType targetType} is set to {@link MessageType#TEXT}. Unmarshals from a {@link
 * TextMessage} or {@link BytesMessage} to an object.
 *
...
public class MarshallingMessageConverter implements MessageConverter, InitializingBean {
  ...
  /**
   * Construct a new {@code MarshallingMessageConverter} with no {@link Marshaller} or {@link
   * Unmarshaller} set. The marshaller must be set after construction by invoking {@link
   * #setMarshaller(Marshaller)} and {@link #setUnmarshaller(Unmarshaller)} .
   */
   ...
   *
   * <p>If the given {@link Marshaller} also implements the {@link Unmarshaller} interface, it is
   * used for both marshalling and unmarshalling. Otherwise, an exception is thrown.
   *
   * <p>Note that all {@link Marshaller} implementations in Spring also implement the {@link
   * Unmarshaller} interface, so that you can safely use this constructor.
   *
   * @param marshaller object used as marshaller and unmarshaller
   * @throws IllegalArgumentException when {@code marshaller} does not implement the {@link
  ...
  /**
   * This implementation marshals the given object to a {@link javax.jms.TextMessage} or {@link
   * javax.jms.BytesMessage}. The desired message type can be defined by setting the {@link
   * #setTargetType "marshalTo"} property.
   *
   ...
   * @see Marshaller#marshal(Object, Result)
   */
  protected BytesMessage marshalToBytesMessage(
      Object object, Session session, Marshaller marshaller)
      throws JMSException, IOException, XmlMappingException {
  ...
  /**
   * Template method that allows for custom message marshalling. Invoked when {@link #setTargetType}
   * is not {@link MessageType#TEXT} or {@link MessageType#BYTES}.
   *
   ...
   * @throws XmlMappingException in case of OXM mapping errors
   */
  protected Message marshalToMessage(
      Object object, Session session, Marshaller marshaller, MessageType targetType)
      throws JMSException, IOException, XmlMappingException {
   ...
   * #fromMessage(Message)} is invoked with a message that is not a {@link TextMessage} or {@link
   * BytesMessage}.
   *
   * <p>The default implementation throws an {@link IllegalArgumentException}.
   *
