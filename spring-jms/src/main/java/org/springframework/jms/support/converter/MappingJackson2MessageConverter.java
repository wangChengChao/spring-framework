...
/**
 * Message converter that uses Jackson 2.x to convert messages to and from JSON. Maps an object to a
 * {@link BytesMessage}, or to a {@link TextMessage} if the {@link #setTargetType targetType} is set
 * to {@link MessageType#TEXT}. Converts from a {@link TextMessage} or {@link BytesMessage} to an
 * object.
 *
...
public class MappingJackson2MessageConverter
    implements SmartMessageConverter, BeanClassLoaderAware {
  ...
  @Nullable private String encodingPropertyName;

  @Nullable private String typeIdPropertyName;
  ...
  /**
   * Specify whether {@link #toMessage(Object, Session)} should marshal to a {@link BytesMessage} or
   * a {@link TextMessage}.
   *
   * <p>The default is {@link MessageType#BYTES}, i.e. this converter marshals to a {@link
   * BytesMessage}. Note that the default version of this converter supports {@link
   * MessageType#BYTES} and {@link MessageType#TEXT} only.
   *
  ...
  /**
   * Specify the encoding to use when converting to and from text-based message body content. The
   * default encoding will be "UTF-8".
   *
   * <p>When reading from a a text-based message, an encoding may have been suggested through a
   * special JMS property which will then be preferred over the encoding set on this
  ...
  /**
   * Specify the name of the JMS message property that carries the encoding from bytes to String and
   * back is BytesMessage is used during the conversion process.
   *
   * <p>Default is none. Setting this property is optional; if not set, UTF-8 will be used for
   * decoding any incoming bytes message.
   *
   * @see #setEncoding
   */
   ...
   *
   * <p>Default is none. <b>NOTE: This property needs to be set in order to allow for converting
   * from an incoming message to a Java object.</b>
   *
  ...
  /**
   * Specify mappings from type ids to Java classes, if desired. This allows for synthetic ids in
   * the type id message property, instead of transferring Java class names.
   *
  ...
  /**
   * Convert a Java object to a JMS Message using the specified json view and the supplied session
   ...
   * @see Session#createBytesMessage
   */
  protected BytesMessage mapToBytesMessage(
      Object object, Session session, ObjectWriter objectWriter) throws JMSException, IOException {
    ...
    if (this.encodingPropertyName != null) {
      message.setStringProperty(
          this.encodingPropertyName, (this.encoding != null ? this.encoding : DEFAULT_ENCODING));
    }
    return message;
  }
   ...
   * @throws IOException in case of I/O errors
   */
  protected Message mapToMessage(
      Object object, Session session, ObjectWriter objectWriter, MessageType targetType)
      throws JMSException, IOException {
   ...
   *
   * <p>The default implementation consults the configured type id mapping and sets the resulting
   * value (either a mapped id or the raw Java class name) into the configured type id message
   * property.
   *
   ...
   * property.
   *
   * <p>The default implementation parses the configured type id property name and consults the
   * configured type id mapping. This can be overridden with a different strategy, e.g. doing some
   * heuristics based on message origin.
   *
   * @param message the JMS Message to set the type id on
   * @throws JMSException if thrown by JMS methods
  ...
  protected JavaType getJavaTypeForMessage(Message message) throws JMSException {
    ...
    if (typeId == null) {
      throw new MessageConversionException(
          "Could not find type id property ["
              + this.typeIdPropertyName
              + "] on message ["
   ...
   *
   * @param conversionHint the conversion hint Object as passed into the converter for the current
   *     conversion attempt
  ...
  private Class<?> extractViewClass(JsonView annotation, Object conversionHint) {
    ...
    if (classes.length != 1) {
      throw new IllegalArgumentException(
          "@JsonView only supported for handler methods with exactly 1 class argument: "
              + conversionHint);
    }
    return classes[0];
  }
