...
/**
 * A {@link javax.jms.MessageListener} adapter that invokes a configurable {@link
 * InvocableHandlerMethod}.
 *
 * <p>Wraps the incoming {@link javax.jms.Message} to Spring's {@link Message} abstraction, copying
 * the JMS standard headers using a configurable {@link JmsHeaderMapper}.
 *
 * <p>The original {@link javax.jms.Message} and the {@link javax.jms.Session} are provided as
 * additional arguments so that these can be injected as method arguments if necessary.
 *
...
public class MessagingMessageListenerAdapter extends AbstractAdaptableMessageListener {
  ...
  @Nullable private InvocableHandlerMethod handlerMethod;
  ...
  @Override
  public void onMessage(javax.jms.Message jmsMessage, @Nullable Session session)
      throws JMSException {
  ...
  private Object invokeHandler(
      javax.jms.Message jmsMessage, @Nullable Session session, Message<?> message) {
    InvocableHandlerMethod handlerMethod = getHandlerMethod();
    try {
      return handlerMethod.invoke(message, jmsMessage, session);
    } catch (MessagingException ex) {
      throw new ListenerExecutionFailedException(
          createMessagingErrorMessage("Listener method could not be invoked with incoming message"),
          ex);
    } catch (Exception ex) {
      throw new ListenerExecutionFailedException(
          "Listener method '" + handlerMethod.getMethod().toGenericString() + "' threw exception",
          ex);
    }
  }
