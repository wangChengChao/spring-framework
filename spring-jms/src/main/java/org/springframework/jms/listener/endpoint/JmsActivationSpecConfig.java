...
/**
 * Common configuration object for activating a JMS message endpoint. Gets converted into a
 * provider-specific JCA 1.5 ActivationSpec object for activating the endpoint.
 *
 * <p>Typically used in combination with {@link JmsMessageEndpointManager}, but not tied to it.
 *
...
public class JmsActivationSpecConfig {
  ...
  @Nullable private String destinationName;
  ...
  @Nullable private String messageSelector;
   ...
   *
   * <p>Note that JCA resource adapters generally only support auto and dups-ok (see Spring's {@link
   * StandardJmsActivationSpecFactory}). ActiveMQ also supports "SESSION_TRANSACTED" in the form of
   * RA-managed transactions (automatically translated by Spring's {@link
   ...
   * String, e.g. "10".
   *
   * <p>JCA listener containers will always scale from zero to the given upper limit. A specified
   * lower limit will effectively be ignored.
   *
   * <p>This property is primarily supported for configuration compatibility with {@link
   * org.springframework.jms.listener.DefaultMessageListenerContainer}. For this activation config,
   * generally use {@link #setMaxConcurrency} instead.
   */
  ...
  /**
   * Specify the maximum number of consumers/sessions to use, effectively controlling the number of
   * concurrent invocations on the target listener.
   */
