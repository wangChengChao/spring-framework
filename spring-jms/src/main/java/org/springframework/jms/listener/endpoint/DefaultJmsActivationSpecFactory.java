...
/**
 * Default implementation of the {@link JmsActivationSpecFactory} interface. Supports the standard
 * JMS properties as defined by the JCA 1.5 specification, as well as Spring's extended
 * "maxConcurrency" and "prefetchSize" settings through autodetection of well-known vendor-specific
 * provider properties.
 *
 * <p>An ActivationSpec factory is effectively dependent on the concrete JMS provider, e.g. on
 * ActiveMQ. This default implementation simply guesses the ActivationSpec class name from the
 * provider's class name ("ActiveMQResourceAdapter" -> "ActiveMQActivationSpec" in the same package,
 * or "ActivationSpecImpl" in the same package as the ResourceAdapter class), and populates the
 ...
 *
 * <p>Note: ActiveMQ, JORAM and WebSphere are supported in terms of extended settings (through the
 * detection of their bean property naming conventions). The default ActivationSpec class detection
 * rules may apply to other JMS providers as well.
 *
 * <p>Thanks to Agim Emruli and Laurie Chan for pointing out WebSphere MQ settings and contributing
 * corresponding tests!
 *
...
public class DefaultJmsActivationSpecFactory extends StandardJmsActivationSpecFactory {
   ...
   * "ActiveMQResourceAdapter" -> "ActiveMQActivationSpec" in the same package, or a class named
  ...
  protected Class<?> determineActivationSpecClass(ResourceAdapter adapter) {
    ...
    if (adapterClassName.endsWith(RESOURCE_ADAPTER_SUFFIX)) {
      // e.g. ActiveMQ
      String providerName =
          adapterClassName.substring(
              0, adapterClassName.length() - RESOURCE_ADAPTER_SUFFIX.length());
      ...
      String providerName =
          adapterClassName.substring(
              0, adapterClassName.length() - RESOURCE_ADAPTER_IMPL_SUFFIX.length());
    ...
    throw new IllegalStateException(
        "No ActivationSpec class defined - "
            + "specify the 'activationSpecClass' property or override the 'determineActivationSpecClass' method");
  }

  /**
   * This implementation supports Spring's extended "maxConcurrency" and "prefetchSize" settings
   * through detecting corresponding ActivationSpec properties: "maxSessions"/"maxNumberOfWorks" and
   * "maxMessagesPerSessions"/"maxMessages", respectively (following ActiveMQ's and JORAM's naming
   * conventions).
   */
  @Override
  ...
  /**
   * This implementation maps {@code SESSION_TRANSACTED} onto an ActivationSpec property named
   * "useRAManagedTransaction", if available (following ActiveMQ's naming conventions).
   */
  @Override
