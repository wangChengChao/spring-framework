...
/**
 * Standard implementation of the {@link JmsActivationSpecFactory} interface. Supports the standard
 * JMS properties as defined by the JMS 1.5 specification (Appendix B); ignores Spring's
 * "maxConcurrency" and "prefetchSize" settings.
 *
 * <p>The 'activationSpecClass' property is required, explicitly defining the fully-qualified class
 * name of the provider's ActivationSpec class (e.g.
 * "org.apache.activemq.ra.ActiveMQActivationSpec").
 *
 * <p>Check out {@link DefaultJmsActivationSpecFactory} for an extended variant of this class,
 * supporting some further default conventions beyond the plain JMS 1.5 specification.
 *
...
public class StandardJmsActivationSpecFactory implements JmsActivationSpecFactory {
  ...
  @Nullable private Class<?> activationSpecClass;

  @Nullable private Map<String, String> defaultProperties;
   ...
   *
   * <p>Applied to each ActivationSpec object before it gets populated with listener-specific
   * settings. Allows for configuring vendor-specific properties beyond the Spring-defined settings
   * in {@link JmsActivationSpecConfig}.
   */
   ...
   *
   * <p>Note that a DestinationResolver for use with this factory must be able to work
   * <i>without</i> an active JMS Session: e.g. {@link
   * org.springframework.jms.support.destination.JndiDestinationResolver} or {@link
   * org.springframework.jms.support.destination.BeanFactoryDestinationResolver} but not {@link
  ...
  /**
   * Determine the ActivationSpec class for the given ResourceAdapter, if possible. Called if no
   * 'activationSpecClass' has been set explicitly
   *
   * @param adapter the ResourceAdapter to check
   * @return the corresponding ActivationSpec class, or {@code null} if not determinable
   * @see #setActivationSpecClass
   */
  @Nullable
  ...
  /**
   * Populate the given ApplicationSpec object with the settings defined in the given configuration
   * object.
   *
   * <p>This implementation applies all standard JMS settings, but ignores "maxConcurrency" and
   * "prefetchSize" - not supported in standard JCA 1.5.
   *
  ...
  protected void populateActivationSpecProperties(BeanWrapper bw, JmsActivationSpecConfig config) {
    ...
    if (destinationName != null) {
      ...
      if (this.destinationResolver != null) {
        try {
          destination =
              this.destinationResolver.resolveDestinationName(null, destinationName, pubSubDomain);
      ...
      bw.setPropertyValue(
          "destinationType", pubSubDomain ? Topic.class.getName() : Queue.class.getName());
    }

    if (bw.isWritableProperty("subscriptionDurability")) {
      bw.setPropertyValue(
          "subscriptionDurability", config.isSubscriptionDurable() ? "Durable" : "NonDurable");
    } else if (config.isSubscriptionDurable()) {
      // Standard JCA 1.5 "subscriptionDurability" apparently not supported...
      throw new IllegalArgumentException(
          "Durable subscriptions not supported by underlying provider");
    }
    if (config.isSubscriptionShared()) {
      throw new IllegalArgumentException(
          "Shared subscriptions not supported for JCA-driven endpoints");
    }
   ...
   *
   * <p>This implementation applies the standard JCA 1.5 acknowledge modes "Auto-acknowledge" and
   * "Dups-ok-acknowledge". It throws an exception in case of {@code CLIENT_ACKNOWLEDGE} or {@code
   * SESSION_TRANSACTED} having been requested.
   *
   * @param bw the BeanWrapper wrapping the ActivationSpec object
   * @param ackMode the configured acknowledge mode (according to the constants in {@link
   *     javax.jms.Session}
  ...
  protected void applyAcknowledgeMode(BeanWrapper bw, int ackMode) {
    if (ackMode == Session.SESSION_TRANSACTED) {
      throw new IllegalArgumentException(
          "No support for SESSION_TRANSACTED: Only \"Auto-acknowledge\" "
              + "and \"Dups-ok-acknowledge\" supported in standard JCA 1.5");
    } else if (ackMode == Session.CLIENT_ACKNOWLEDGE) {
      throw new IllegalArgumentException(
          "No support for CLIENT_ACKNOWLEDGE: Only \"Auto-acknowledge\" "
              + "and \"Dups-ok-acknowledge\" supported in standard JCA 1.5");
      ...
      throw new IllegalArgumentException(
          "Dups-ok-acknowledge not supported by underlying provider");
    }
  }
}
