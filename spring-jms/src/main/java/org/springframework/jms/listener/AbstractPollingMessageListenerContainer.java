 ...
 * for listener handling based on {@link javax.jms.MessageConsumer}, optionally participating in
 ...
 * {@link #receiveAndExecute} method. The MessageConsumer used may be reobtained fo reach attempt or
 * cached in between attempts; this is up to the concrete implementation. The receive timeout for
 * each attempt can be configured through the {@link #setReceiveTimeout "receiveTimeout"} property.
 *
 * <p>The underlying mechanism is based on standard JMS MessageConsumer handling, which is perfectly
 * compatible with both native JMS and JMS in a Java EE environment. Neither the JMS {@code
 * MessageConsumer.setMessageListener} facility nor the JMS ServerSessionPool facility is required.
 * A further advantage of this approach is full control over the listening process, allowing for
 * custom scaling and throttling and of concurrent message processing (which is up to concrete
 * subclasses).
 *
 * <p>Message reception and listener execution can automatically be wrapped in transactions through
 ...
 * combination with a JTA-aware JMS ConnectionFactory obtained from JNDI (check your application
 * server's documentation).
 *
 * <p>This base class does not assume any specific mechanism for asynchronous execution of polling
 * invokers. Check out {@link DefaultMessageListenerContainer} for a concrete implementation which
 * is based on Spring's {@link org.springframework.core.task.TaskExecutor} abstraction, including
 * dynamic scaling of concurrent consumers and automatic self recovery.
 *
...
public abstract class AbstractPollingMessageListenerContainer
   ...
   * for transactional wrapping of message reception plus listener execution.
   *
   * <p>Default is none, not performing any transactional wrapping. If specified, this will usually
   * be a Spring {@link org.springframework.transaction.jta.JtaTransactionManager} or one of its
   * subclasses, in combination with a JTA-aware ConnectionFactory that this message listener
   * container obtains its Connections from.
   *
   * <p><b>Note: Consider the use of local JMS transactions instead.</b> Simply switch the {@link
   * #setSessionTransacted "sessionTransacted"} flag to "true" in order to use a locally transacted
   * JMS Session for the entire receive processing, including any Session operations performed by a
   * {@link SessionAwareMessageListener} (e.g. sending a response message). This allows for fully
   * synchronized Spring transactions based on local JMS transactions, similar to what {@link
   * org.springframework.jms.connection.JmsTransactionManager} provides. Check {@link
   * AbstractMessageListenerContainer}'s javadoc for a discussion of transaction choices and message
  ...
  /**
   * Set the timeout to use for receive calls, in <b>milliseconds</b>. The default is 1000 ms, that
   * is, 1 second.
   *
   * <p><b>NOTE:</b> This value needs to be smaller than the transaction timeout used by the
   * transaction manager (in the appropriate unit, of course). 0 indicates no timeout at all;
   * however, this is only feasible if not running within a transaction manager and generally
   * discouraged since such a listener container cannot cleanly shut down. A negative value such as
   * -1 indicates a no-wait receive operation.
   *
  ...
  /**
   * Actually execute the listener for a message received from the given consumer, fetching all
   * requires resources and invoking the listener.
   *
   * @param session the JMS Session to work on
   * @param consumer the MessageConsumer to work on
  ...
  /**
   * This implementation checks whether the Session is externally synchronized. In this case, the
   * Session is not locally transacted, despite the listener container's "sessionTransacted" flag
   * being set to "true".
   *
  ...
  protected boolean isSessionLocallyTransacted(Session session) {
    ...
    JmsResourceHolder resourceHolder =
        (JmsResourceHolder)
            TransactionSynchronizationManager.getResource(obtainConnectionFactory());
    return (resourceHolder == null
        || resourceHolder instanceof LocallyExposedJmsResourceHolder
        || !resourceHolder.containsSession(session));
  }

  /**
   * Determine whether to trigger a commit after no message has been received. This is a good idea
   * on any modern-day JMS provider.
   *
  ...
  private void rollbackOnException(
      PlatformTransactionManager manager, TransactionStatus status, Throwable ex) {
  ...
  /**
   * Template method that gets called right when a new message has been received, before attempting
   * to process it. Allows subclasses to react to the event of an actual incoming message, for
   * example adapting their consumer count.
   *
  ...
  /**
   * Template method that gets called when <i>no</i> message has been received, before returning to
   * the receive loop again. Allows subclasses to react to the event of no incoming message, for
   * example marking the invoker as idle.
   *
   * @param invoker the invoker object (passed through)
   * @param session the receiving JMS Session
   */
  ...
  /**
   * ResourceFactory implementation that delegates to this listener container's protected callback
   * methods.
   */
