...
/**
 * An abstract JMS {@link MessageListener} adapter providing the necessary infrastructure to extract
 * the payload of a JMS {@link Message}.
 *
...
public abstract class AbstractAdaptableMessageListener
  ...
  @Nullable private Object defaultResponseDestination;

  private DestinationResolver destinationResolver = new DynamicDestinationResolver();

  @Nullable private MessageConverter messageConverter = new SimpleMessageConverter();

  private final MessagingMessageConverterAdapter messagingMessageConverter =
      new MessagingMessageConverterAdapter();

  @Nullable private QosSettings responseQosSettings;

  /**
   * Set the default destination to send response messages to. This will be applied in case of a
   * request message that does not carry a "JMSReplyTo" field.
   *
   * <p>Response destinations are only relevant for listener methods that return result objects,
   * which will be wrapped in a response message and sent to a response destination.
   *
   * <p>Alternatively, specify a "defaultResponseQueueName" or "defaultResponseTopicName", to be
  ...
  /**
   * Set the name of the default response queue to send response messages to. This will be applied
  ...
  /**
   * Set the name of the default response topic to send response messages to. This will be applied
   * in case of a request message that does not carry a "JMSReplyTo" field.
   *
   * <p>Alternatively, specify a JMS Destination object as "defaultResponseDestination".
   *
  ...
  /**
   * Set the DestinationResolver that should be used to resolve response destination names for this
   * adapter.
   *
   * <p>The default resolver is a DynamicDestinationResolver. Specify a JndiDestinationResolver for
   * resolving destination names as JNDI locations.
   *
  ...
  /**
   * Set the converter that will convert incoming JMS messages to listener method arguments, and
   * objects returned from listener methods back to JMS messages.
   *
   * <p>The default converter is a {@link SimpleMessageConverter}, which is able to handle {@link
   * javax.jms.BytesMessage BytesMessages}, {@link javax.jms.TextMessage TextMessages} and {@link
  ...
  /**
   * Return the converter that will convert incoming JMS messages to listener method arguments, and
   * objects returned from listener methods back to JMS messages.
   */
  @Nullable
  ...
  /**
   * Set the {@link JmsHeaderMapper} implementation to use to map the standard JMS headers. By
   * default, a {@link SimpleJmsHeaderMapper} is used.
   *
  ...
  /**
   * Set the {@link QosSettings} to use when sending a response. Can be set to {@code null} to
   * indicate that the broker's defaults should be used.
   *
   * @param responseQosSettings the QoS settings to use when sending a response or {@code null} to
  ...
  /**
   * Return the {@link QosSettings} to use when sending a response, or {@code null} if the defaults
   * should be used.
   *
   * @since 5.0
   */
   ...
   *
   * <p>Delegates the message to the target listener method, with appropriate conversion of the
   * message argument. In case of an exception, the {@link #handleListenerException(Throwable)}
   * method will be invoked.
   *
   * <p><b>Note:</b> Does not support sending response messages based on result objects returned
   * from listener methods. Use the {@link SessionAwareMessageListener} entry point (typically
   * through a Spring message listener container) for handling result objects as well.
   *
  ...
  /**
   * Handle the given exception that arose during listener execution. The default implementation
   * logs the exception at error level.
   *
   * <p>This method only applies when used as standard JMS {@link MessageListener}. In case of the
   * Spring {@link SessionAwareMessageListener} mechanism, exceptions get handled by the caller
   * instead.
   *
   ...
   * @return the content of the message, to be passed into the listener method as an argument
  ...
  /**
   * Handle the given result object returned from the listener method, sending a response message
   * back.
   *
  ...
  protected void handleResult(Object result, Message request, @Nullable Session session) {
    if (session != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Listener method returned result ["
                + result
   ...
   * @return the payload response to handle, either the {@code result} argument or any other object
   *     (for instance wrapping the result).
   * @since 4.3
   */
   ...
   *
   * <p>The default implementation sets the response's correlation id to the request message's
   * correlation id, if any; otherwise to the request message id.
   *
  ...
  private Destination getResponseDestination(
      Message request, Message response, Session session, Object result) throws JMSException {
   ...
   *
   * <p>The default implementation first checks the JMS Reply-To {@link Destination} of the supplied
   * request; if that is not {@code null} it is returned; if it is {@code null}, then the configured
   * {@link #resolveDefaultResponseDestination default response destination} is returned; if this
   * too is {@code null}, then an {@link javax.jms.InvalidDestinationException} is thrown.
   *
   * @param request the original incoming JMS message
   * @param response the outgoing JMS message about to be sent
   * @param session the JMS Session to operate on
    ...
    if (replyTo == null) {
      ...
      if (replyTo == null) {
        throw new InvalidDestinationException(
            "Cannot determine response destination: "
  ...
  /**
   * Resolve the default response destination into a JMS {@link Destination}, using this accessor's
   * {@link DestinationResolver} in case of a destination name.
   *
  ...
  protected Destination resolveDefaultResponseDestination(Session session) throws JMSException {
    ...
    if (this.defaultResponseDestination instanceof DestinationNameHolder) {
      ...
      return getDestinationResolver()
          .resolveDestinationName(session, nameHolder.name, nameHolder.isTopic);
    }
    return null;
  }
  ...
  protected void sendResponse(Session session, Destination destination, Message response)
    ...
    try {
      ...
      if (settings != null) {
        producer.send(
            response, settings.getDeliveryMode(), settings.getPriority(), settings.getTimeToLive());
      } else {
  ...
  /**
   * A {@link MessagingMessageConverter} that lazily invoke payload extraction and delegate it to
   * {@link #extractMessage(javax.jms.Message)} in order to enforce backward compatibility.
   */
  private class MessagingMessageConverterAdapter extends MessagingMessageConverter {
    ...
    @Override
    protected Message createMessageForPayload(
    ...
    protected class LazyResolutionMessage implements org.springframework.messaging.Message<Object> {
      ...
      @Nullable private MessageHeaders headers;
      ...
      /**
       * Extract the payload of the current message. Since we deferred the resolution of the
       * payload, a custom converter may still return a full message for it. In this case, its
       * payload is returned.
       *
