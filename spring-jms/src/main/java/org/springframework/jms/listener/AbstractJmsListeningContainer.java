...
/**
 * Common base class for all containers which need to implement listening based on a JMS Connection
 * (either shared or freshly obtained for each attempt). Inherits basic Connection and Session
 * configuration handling from the {@link org.springframework.jms.support.JmsAccessor} base class.
 *
 * <p>This class provides basic lifecycle management, in particular management of a shared JMS
 * Connection. Subclasses are supposed to plug into this lifecycle, implementing the {@link
 * #sharedConnectionEnabled()} as well as the {@link #doInitialize()} and {@link #doShutdown()}
 * template methods.
 *
 * <p>This base class does not assume any specific listener programming model or listener invoker
 * mechanism. It just provides the general runtime lifecycle management needed for any kind of
 * JMS-based listening mechanism that operates on a JMS Connection/Session.
 *
 * <p>For a concrete listener programming model, check out the {@link
 * AbstractMessageListenerContainer} subclass. For a concrete listener invoker mechanism, check out
 * the {@link DefaultMessageListenerContainer} class.
 *
...
public abstract class AbstractJmsListeningContainer extends JmsDestinationAccessor
  ...
  @Nullable private Connection sharedConnection;
   ...
   *
   * <p>Default is "true"; set this to "false" to allow for manual startup through the {@link
   * #start()} method.
   */
  ...
  /**
   * Specify the phase in which this container should be started and stopped. The startup order
   * proceeds from lowest to highest, and the shutdown order is the reverse of that. By default this
   * value is Integer.MAX_VALUE meaning that this container starts as late as possible and stops as
   * soon as possible.
   */
   ...
   *
   * <p>Creates a JMS Connection, starts the {@link javax.jms.Connection} (if {@link
   * #setAutoStartup(boolean) "autoStartup"} hasn't been turned off), and calls {@link
   * #doInitialize()}.
   *
    ...
    try {
      ...
      synchronized (this.sharedConnectionMonitor) {
        ConnectionFactoryUtils.releaseConnection(
            this.sharedConnection, getConnectionFactory(), this.autoStartup);
  ...
  public void shutdown() throws JmsException {
    ...
    try {
      ...
      if (sharedConnectionEnabled()) {
        synchronized (this.sharedConnectionMonitor) {
          ConnectionFactoryUtils.releaseConnection(
              this.sharedConnection, getConnectionFactory(), false);
          this.sharedConnection = null;
        }
      }
    }
  ...
  /**
   * Return whether this container is currently active, that is, whether it has been set up but not
   * shut down yet.
   */
  ...
  /**
   * Determine whether this container is currently running, that is, whether it has been started and
   * not stopped yet.
   *
   * @see #start()
   * @see #stop()
   ...
   *
   * <p>This implementation always returns {@code true}; the default 'running' state is purely
   * determined by {@link #start()} / {@link #stop()}.
   *
   * <p>Subclasses may override this method to check against temporary conditions that prevent
   * listeners from actually running. In other words, they may apply further restrictions to the
   * 'running' state, returning {@code false} if such a restriction prevents listeners from running.
   */
   ...
   *
   * <p>The default implementation delegates to {@link #createSharedConnection()}, which does one
   * immediate attempt and throws an exception if it fails. Can be overridden to have a recovery
   * process in place, retrying until a Connection can be successfully established.
   *
   * @throws JMSException if thrown by JMS API methods
   */
   ...
   *
   * <p>Called on startup and also after an infrastructure exception that occurred during invoker
   ...
   *
   * <p>The default implementation creates a standard Connection and prepares it through {@link
  ...
  /**
   * Prepare the given Connection, which is about to be registered as shared Connection for this
   * container.
   *
   * <p>The default implementation sets the specified client id, if any. Subclasses can override
   * this to apply further settings.
   *
   ...
   * @throws IllegalStateException if this container does not maintain a shared Connection, or if
   *     the Connection hasn't been initialized yet
   * @see #sharedConnectionEnabled()
   */
  ...
  /**
   * Take the given task object and reschedule it, either immediately if this container is currently
   * running, or later once this container has been restarted.
   *
  ...
  /**
   * Try to resume all paused tasks. Tasks for which rescheduling failed simply remain in paused
   * mode.
   */
   ...
   *
   * <p>To be implemented by subclasses if they ever call {@code rescheduleTaskIfNecessary}. This
   ...
   *
   * <p>The default implementation simply logs a corresponding message at debug level.
   *
   ...
   *
   * <p>A shared JMS Connection, if any, will already have been started at this point.
   *
   ...
   * <p>Subclasses need to implement this method for their specific invoker management process.
   *
   * <p>A shared JMS Connection, if any, will automatically be closed <i>afterwards</i>.
   *
   * @throws JMSException if shutdown failed
   * @see #shutdown()
   */
  ...
  /**
   * Exception that indicates that the initial setup of this container's shared JMS Connection
   * failed. This is indicating to invokers that they need to establish the shared Connection
   * themselves on first access.
   */
