...
/**
 * Abstract parser for JMS listener container elements, providing support for common properties that
 * are identical for all listener container variants.
 *
...
abstract class AbstractListenerContainerParser implements BeanDefinitionParser {
  ...
  public BeanDefinition parse(Element element, ParserContext parserContext) {
    ...
    MutablePropertyValues specificProperties =
        parseSpecificContainerProperties(element, parserContext);
    ...
    for (int i = 0; i < childNodes.getLength(); i++) {
      ...
      if (child.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (LISTENER_ELEMENT.equals(localName)) {
          parseListener(
              element, (Element) child, parserContext, commonProperties, specificProperties);
        }
      }
    }
  ...
  private void parseListener(
      ...
      MutablePropertyValues commonContainerProperties,
    ...
    if (!StringUtils.hasText(ref)) {
      parserContext
          .getReaderContext()
          .error("Listener 'ref' attribute contains empty value.", listenerEle);
    } else {
    ...
    if (listenerEle.hasAttribute(RESPONSE_DESTINATION_ATTRIBUTE)) {
      ...
      listenerDef
          .getPropertyValues()
          .add(
              pubSubDomain ? "defaultResponseTopicName" : "defaultResponseQueueName",
              responseDestination);
      PropertyValue destinationResolver =
          containerDef.getPropertyValues().getPropertyValue("destinationResolver");
    ...
    parserContext.registerBeanComponent(
        new BeanComponentDefinition(containerDef, containerBeanName));
  }

  protected void parseListenerConfiguration(
      Element ele, ParserContext parserContext, MutablePropertyValues configValues) {
    String destination = ele.getAttribute(DESTINATION_ATTRIBUTE);
    if (!StringUtils.hasText(destination)) {
      parserContext
          .getReaderContext()
          .error("Listener 'destination' attribute contains empty value.", ele);
    }
    ...
    if (ele.hasAttribute(SUBSCRIPTION_ATTRIBUTE)) {
      ...
      if (!StringUtils.hasText(subscription)) {
        parserContext
            .getReaderContext()
            .error("Listener 'subscription' attribute contains empty value.", ele);
      }
    ...
    if (ele.hasAttribute(SELECTOR_ATTRIBUTE)) {
      ...
      if (!StringUtils.hasText(selector)) {
        parserContext
            .getReaderContext()
            .error("Listener 'selector' attribute contains empty value.", ele);
      }
    ...
    if (ele.hasAttribute(CONCURRENCY_ATTRIBUTE)) {
      ...
      if (!StringUtils.hasText(concurrency)) {
        parserContext
            .getReaderContext()
            .error("Listener 'concurrency' attribute contains empty value.", ele);
      }
  ...
  protected MutablePropertyValues parseCommonContainerProperties(
    ...
    if (!StringUtils.hasText(replyDestinationType)) {
      ...
      parserContext
          .getReaderContext()
          .error(
              "Invalid listener container 'response-destination-type': only "
                  + "\"queue\", \"topic\" supported.",
              containerEle);
    }
    ...
    if (containerEle.hasAttribute(CLIENT_ID_ATTRIBUTE)) {
      ...
      if (!StringUtils.hasText(clientId)) {
        parserContext
            .getReaderContext()
            .error("Listener 'client-id' attribute contains empty value.", containerEle);
      }
    ...
    if (containerEle.hasAttribute(MESSAGE_CONVERTER_ATTRIBUTE)) {
      ...
      if (!StringUtils.hasText(messageConverter)) {
        parserContext
            .getReaderContext()
            .error(
                "listener container 'message-converter' attribute contains empty value.",
                containerEle);
      } else {
  ...
  /**
   * Parse the common properties for all listeners as defined by the specified container {@link
   * Element}.
   */
  protected abstract MutablePropertyValues parseSpecificContainerProperties(
      Element containerEle, ParserContext parserContext);

  /**
   * Create the {@link BeanDefinition} for the container factory using the specified shared property
   * values.
   */
  @Nullable
  protected abstract RootBeanDefinition createContainerFactory(
      String factoryId,
      Element containerEle,
      ParserContext parserContext,
  ...
  protected abstract RootBeanDefinition createContainer(
      Element containerEle,
      Element listenerEle,
      ParserContext parserContext,
      PropertyValues commonContainerProperties,
