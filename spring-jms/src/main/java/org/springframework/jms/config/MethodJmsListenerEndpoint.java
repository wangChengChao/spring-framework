...
/**
 * A {@link JmsListenerEndpoint} providing the method to invoke to process an incoming message for
 * this endpoint.
 *
...
public class MethodJmsListenerEndpoint extends AbstractJmsListenerEndpoint
  ...
  @Nullable private Object bean;

  @Nullable private Method method;

  @Nullable private Method mostSpecificMethod;
   ...
   *
   * <p>In case of a proxy, this will be the method on the target class (if annotated itself, that
   * is, if not just annotated in an interface).
   *
   * @since 4.2.3
   */
  ...
  protected MessagingMessageListenerAdapter createMessageListener(
    ...
    Assert.state(
        this.messageHandlerMethodFactory != null,
    ...
    if (ann != null) {
      ...
      if (destinations.length != 1) {
        throw new IllegalStateException(
            "Invalid @"
                + SendTo.class.getSimpleName()
                + " annotation on '"
                + specificMethod
                + "' one destination must be set (got "
    ...
    if (ann == null) {
      ann =
          AnnotatedElementUtils.findMergedAnnotation(
              specificMethod.getDeclaringClass(), SendTo.class);
    }
    return ann;
  }
