...
public abstract class AbstractJmsListenerEndpoint implements JmsListenerEndpoint {
  ...
  @Nullable private String concurrency;
   ...
   *
   * <p>The concurrency limits can be a "lower-upper" String, e.g. "5-10", or a simple upper limit
   * String, e.g. "10" (the lower limit will be 1 in this case).
   *
   * <p>The underlying container may or may not support all features. For instance, it may not be
   * able to scale: in that case only the upper value is used.
   */
  ...
  /**
   * Create a {@link MessageListener} that is able to serve this endpoint for the specified
   * container.
   */
    ...
    public void configureEndpoint(Object listenerContainer) {
      if (listenerContainer instanceof JmsMessageEndpointManager) {
        ...
        throw new IllegalArgumentException(
            "Could not configure endpoint with the specified container '"
                + listenerContainer
                + "' Only JMS ("
                + AbstractMessageListenerContainer.class.getName()
                + " subclass) or JCA ("
