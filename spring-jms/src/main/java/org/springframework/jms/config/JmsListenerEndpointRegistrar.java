...
/**
 * Helper bean for registering {@link JmsListenerEndpoint} with a {@link
...
public class JmsListenerEndpointRegistrar implements BeanFactoryAware, InitializingBean {
  ...
  @Nullable private JmsListenerContainerFactory<?> containerFactory;

  @Nullable private String containerFactoryBeanName;
  ...
  /**
   * Return the {@link JmsListenerEndpointRegistry} instance for this registrar, may be {@code
   * null}.
   */
  @Nullable
  ...
  public void setMessageHandlerMethodFactory(
      @Nullable MessageHandlerMethodFactory messageHandlerMethodFactory) {
  ...
  /**
   * Set the {@link JmsListenerContainerFactory} to use in case a {@link JmsListenerEndpoint} is
   * registered with a {@code null} container factory.
   *
   * <p>Alternatively, the bean name of the {@link JmsListenerContainerFactory} to use can be
   * specified for a lazy lookup, see {@link #setContainerFactoryBeanName}.
   */
  ...
  /**
   * Set the bean name of the {@link JmsListenerContainerFactory} to use in case a {@link
   * JmsListenerEndpoint} is registered with a {@code null} container factory. Alternatively, the
  ...
  /**
   * A {@link BeanFactory} only needs to be available in conjunction with {@link
  ...
  private JmsListenerContainerFactory<?> resolveContainerFactory(
      JmsListenerEndpointDescriptor descriptor) {
  ...
  /**
   * Register a new {@link JmsListenerEndpoint} alongside the {@link JmsListenerContainerFactory} to
   * use to create the underlying container.
   *
   * <p>The {@code factory} may be {@code null} if the default factory has to be used for that
   * endpoint.
   */
  public void registerEndpoint(
      JmsListenerEndpoint endpoint, @Nullable JmsListenerContainerFactory<?> factory) {
    ...
    synchronized (this.mutex) {
      if (this.startImmediately) { // register and start immediately
        Assert.state(this.endpointRegistry != null, "No JmsListenerEndpointRegistry set");
        this.endpointRegistry.registerListenerContainer(
            descriptor.endpoint, resolveContainerFactory(descriptor), true);
      } else {
  ...
  /**
   * Register a new {@link JmsListenerEndpoint} using the default {@link
  ...
  private static class JmsListenerEndpointDescriptor {
    ...
    @Nullable public final JmsListenerContainerFactory<?> containerFactory;

    public JmsListenerEndpointDescriptor(
        JmsListenerEndpoint endpoint, @Nullable JmsListenerContainerFactory<?> containerFactory) {
