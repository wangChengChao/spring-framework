...
abstract class AbstractJmsAnnotationDrivenTests {
  ...
  /**
   * Test for {@link SampleBean} discovery. If a factory with the default name is set, an endpoint
   ...
   * provide a default registry. This shows that the default factory is only retrieved if it needs
   * to be.
   */
  protected void testFullConfiguration(ApplicationContext context) {
    ...
    MethodJmsListenerEndpoint endpoint =
        (MethodJmsListenerEndpoint) simpleFactory.getListenerContainers().get(0).getEndpoint();
  ...
  /**
   * Test for {@link CustomBean} and an manually endpoint registered with "myCustomEndpointId". The
   * custom endpoint does not provide any factory so it's registered with the default one
   */
  protected void testCustomConfiguration(ApplicationContext context) {
    ...
    assertThat(endpoint.getClass())
        .as("Wrong endpoint type")
        .isEqualTo(SimpleJmsListenerEndpoint.class);
    assertThat(((SimpleJmsListenerEndpoint) endpoint).getMessageListener())
        .as("Wrong listener set in custom endpoint")
        .isEqualTo(context.getBean("simpleMessageListener"));

    JmsListenerEndpointRegistry customRegistry =
        context.getBean("customRegistry", JmsListenerEndpointRegistry.class);
    assertThat(customRegistry.getListenerContainerIds().size())
        .as("Wrong number of containers in the registry")
        .isEqualTo(2);
    assertThat(customRegistry.getListenerContainers().size())
        .as("Wrong number of containers in the registry")
        .isEqualTo(2);
    assertThat(customRegistry.getListenerContainer("listenerId"))
        .as("Container with custom id on the annotation should be found")
        .isNotNull();
    assertThat(customRegistry.getListenerContainer("myCustomEndpointId"))
        .as("Container created with custom id should be found")
        .isNotNull();
  }

  /**
   * Test for {@link DefaultBean} that does not define the container factory to use as a default is
   * registered with an explicit default.
   */
  ...
  /**
   * Test for {@link DefaultBean} that does not define the container factory to use as a default is
   * registered with the default name.
   */
  ...
  /**
   * Test for {@link ValidationBean} with a validator ({@link TestValidator}) specified in a custom
   * {@link org.springframework.messaging.handler.annotation.support.MessageHandlerMethodFactory}.
   *
   * <p>The test should throw a {@link
   ...
   * {@code @JmsListener} annotation is repeatable and generate one specific container per
   * annotation.
   */
  protected void testJmsListenerRepeatable(ApplicationContext context) {
    ...
    MethodJmsListenerEndpoint first =
        (MethodJmsListenerEndpoint) simpleFactory.getListenerContainer("first").getEndpoint();
    ...
    MethodJmsListenerEndpoint second =
        (MethodJmsListenerEndpoint) simpleFactory.getListenerContainer("second").getEndpoint();
  ...
  static class FullConfigurableBean {
    ...
    @JmsListener(
        id = "${jms.listener.id}",
        containerFactory = "${jms.listener.containerFactory}",
        destination = "${jms.listener.destination}",
        selector = "${jms.listener.selector}",
        subscription = "${jms.listener.subscription}",
