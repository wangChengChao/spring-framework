...
/**
 * A Spring-provided extension of the standard Hibernate {@link Configuration} class, adding {@link
 * SpringSessionContext} as a default and providing convenient ways to specify a JDBC {@link
 * DataSource} and an application class loader.
 *
 * <p>This is designed for programmatic use, e.g. in {@code @Bean} factory methods; consider using
 * {@link LocalSessionFactoryBean} for XML bean definition files. Typically combined with {@link
 * HibernateTransactionManager} for declarative transactions against the {@code SessionFactory} and
 * its JDBC {@code DataSource}.
 *
 * <p>Compatible with Hibernate 5.0/5.1 as well as 5.2/5.3, as of Spring 5.1. Set up with Hibernate
 * 5.2/5.3, this builder is also a convenient way to set up a JPA {@code EntityManagerFactory} since
 * the Hibernate {@code SessionFactory} natively exposes the JPA {@code EntityManagerFactory}
 * interface as well now.
 *
 * <p>This builder supports Hibernate 5.3 {@code BeanContainer} integration, {@link MetadataSources}
 * from custom {@link BootstrapServiceRegistryBuilder} setup, as well as other advanced Hibernate
 * configuration options beyond the standard JPA bootstrap contract.
 *
...
public class LocalSessionFactoryBuilder extends Configuration {
  ...
  @Nullable private TypeFilter[] entityTypeFilters = DEFAULT_ENTITY_TYPE_FILTERS;
  ...
  public LocalSessionFactoryBuilder(
      @Nullable DataSource dataSource, ResourceLoader resourceLoader) {
    this(
        dataSource,
        resourceLoader,
        new MetadataSources(
   ...
   *     using (may be {@code null})
   * @param resourceLoader the ResourceLoader to load application classes from
   * @param metadataSources the Hibernate MetadataSources service to use (e.g. reusing an existing
   *     one)
   * @since 4.3
   */
  public LocalSessionFactoryBuilder(
    ...
    getProperties()
        .put(AvailableSettings.CURRENT_SESSION_CONTEXT_CLASS, SpringSessionContext.class.getName());
    if (dataSource != null) {
    ...
    getProperties()
        .put(
            AvailableSettings.CLASSLOADERS, Collections.singleton(resourceLoader.getClassLoader()));
    this.resourcePatternResolver = ResourcePatternUtils.getResourcePatternResolver(resourceLoader);
  }

  /**
   * Set the Spring {@link JtaTransactionManager} or the JTA {@link TransactionManager} to be used
   * with Hibernate, if any. Allows for using a Spring-managed transaction manager for Hibernate 5's
  ...
  /**
   * Set a Hibernate {@link org.hibernate.resource.beans.container.spi.BeanContainer} for the given
   * Spring {@link ConfigurableListableBeanFactory}.
   *
   * <p>Note: Bean container integration requires Hibernate 5.3 or higher. It enables autowiring of
  ...
  public LocalSessionFactoryBuilder setMultiTenantConnectionProvider(
    ...
    getProperties()
        .put(AvailableSettings.MULTI_TENANT_CONNECTION_PROVIDER, multiTenantConnectionProvider);
    return this;
  }
  ...
  public void setCurrentTenantIdentifierResolver(
    ...
    getProperties()
        .put(AvailableSettings.MULTI_TENANT_IDENTIFIER_RESOLVER, currentTenantIdentifierResolver);
    ...
    try {
      ...
      for (String className : converterClassNames) {
        addAttributeConverter(
            (Class<? extends AttributeConverter<?, ?>>) ClassUtils.forName(className, cl));
      }
  ...
  /**
   * Build the Hibernate {@code SessionFactory} through background bootstrapping, using the given
   * executor for a parallel initialization phase (e.g. a {@link
  ...
  /**
   * Proxy invocation handler for background bootstrapping, only enforcing a fully initialized
   * target {@code SessionFactory} when actually needed.
   *
   * @since 4.3
   */
    ...
    private SessionFactory getSessionFactory() {
      try {
        ...
        throw new IllegalStateException(
            "Interrupted during initialization of Hibernate SessionFactory", ex);
        ...
        throw new IllegalStateException(
            "Failed to asynchronously initialize Hibernate SessionFactory: " + ex.getMessage(),
            cause);
      }
    }
  }
