 ...
 * Hibernate {@link SessionFactory}. Binds a Hibernate Session from the specified factory to the
 * thread, potentially allowing for one thread-bound Session per factory. {@code
 * SessionFactory.getCurrentSession()} is required for Hibernate access code that needs to support
 * this transaction handling mechanism, with the SessionFactory being configured with {@link
 * SpringSessionContext}.
 *
 * <p>Supports custom isolation levels, and timeouts that get applied as Hibernate transaction
 * timeouts.
 *
 * <p>This transaction manager is appropriate for applications that use a single Hibernate
 * SessionFactory for transactional data access, but it also supports direct DataSource access
 * within a transaction (i.e. plain JDBC code working with the same DataSource). This allows for
 * mixing services which access Hibernate and services which use plain JDBC (without being aware of
 * Hibernate)! Application code needs to stick to the same simple Connection lookup pattern as with
 * {@link org.springframework.jdbc.datasource.DataSourceTransactionManager} (i.e. {@link
 * org.springframework.jdbc.datasource.DataSourceUtils#getConnection} or going through a {@link
 * org.springframework.jdbc.datasource.TransactionAwareDataSourceProxy}).
 *
 * <p>Note: To be able to register a DataSource's Connection for plain JDBC code, this instance
 * needs to be aware of the DataSource ({@link #setDataSource}). The given DataSource should
 * obviously match the one used by the given SessionFactory.
 *
 * <p>JTA (usually through {@link org.springframework.transaction.jta.JtaTransactionManager}) is
 * necessary for accessing multiple transactional resources within the same transaction. The
 * DataSource that Hibernate uses needs to be JTA-enabled in such a scenario (see container setup).
 *
 * <p>This transaction manager supports nested transactions via JDBC 3.0 Savepoints. The {@link
 * #setNestedTransactionAllowed} "nestedTransactionAllowed"} flag defaults to "false", though, as
 * nested transactions will just apply to the JDBC Connection, not to the Hibernate Session and its
 * cached entity objects and related context. You can manually set the flag to "true" if you want to
 * use nested transactions for JDBC access code which participates in Hibernate transactions
 * (provided that your JDBC driver supports Savepoints). <i>Note that Hibernate itself does not
 * support nested transactions! Hence, do not expect Hibernate access code to semantically
...
public class HibernateTransactionManager extends AbstractPlatformTransactionManager
  ...
  @Nullable private Object entityInterceptor;
  ...
  /**
   * Create a new HibernateTransactionManager instance. A SessionFactory has to be set to be able to
   * use it.
   *
   * @see #setSessionFactory
   */
   ...
   *
   * <p>If the SessionFactory was configured with LocalDataSourceConnectionProvider, i.e. by
   * Spring's LocalSessionFactoryBean with a specified "dataSource", the DataSource will be
   * auto-detected: You can still explicitly specify the DataSource, but you don't need to in this
   * case.
   *
   * <p>A transactional JDBC Connection for this DataSource will be provided to application code
   ...
   *
   * <p>The DataSource specified here should be the target DataSource to manage transactions for,
   * not a TransactionAwareDataSourceProxy. Only data access code may work with
   ...
   *
   * <p><b>NOTE: For scenarios with many transactions that just read data from Hibernate's cache
   * (and do not actually access the database), consider using a {@link
   * org.springframework.jdbc.datasource.LazyConnectionDataSourceProxy} for the actual target
   * DataSource. Alternatively, consider switching {@link #setPrepareConnection "prepareConnection"}
   * to {@code false}.</b> In both cases, this transaction manager will not eagerly acquire a JDBC
   * Connection for each Hibernate Session anymore (as of Spring 5.1).
   *
   ...
   *
   * <p>Can be turned off to deliberately ignore an available DataSource, in order to not expose
   ...
   *
   * <p>Default is "true". If you turn this flag off, the transaction manager will not support
   * per-transaction isolation levels anymore. It will not call {@code Connection.setReadOnly(true)}
   * for read-only transactions anymore either. If this flag is turned off, no cleanup of a JDBC
   * Connection is required after a transaction, since no Connection settings will get modified.
   *
   ...
   *
   * <p>Default is "false". Turning this flag on enforces over-commit holdability on the underlying
   * JDBC Connection (if {@link #prepareConnection "prepareConnection"} is on) and skips the
   * disconnect-on-completion step.
   *
  ...
  /**
   * Set whether to operate on a Hibernate-managed Session instead of a Spring-managed Session, that
   * is, whether to obtain the Session through Hibernate's {@link
   * SessionFactory#getCurrentSession()} instead of {@link SessionFactory#openSession()} (with a
   * Spring {@link TransactionSynchronizationManager} check preceding it).
   *
   * <p>Default is "false", i.e. using a Spring-managed Session: taking the current thread-bound
   * Session if available (e.g. in an Open-Session-in-View scenario), creating a new Session for the
   * current transaction otherwise.
   *
   * <p>Switch this flag to "true" in order to enforce use of a Hibernate-managed Session. Note that
   * this requires {@link SessionFactory#getCurrentSession()} to always return a proper Session when
   * called for a Spring-managed transaction; transaction begin will fail if the {@code
   * getCurrentSession()} call fails.
   *
   * <p>This mode will typically be used in combination with a custom Hibernate {@link
   * org.hibernate.context.spi.CurrentSessionContext} implementation that stores Sessions in a place
   * other than Spring's TransactionSynchronizationManager. It may also be used in combination with
   * Spring's Open-Session-in-View support (using Spring's default {@link SpringSessionContext}), in
   * which case it subtly differs from the Spring-managed Session mode: The pre-bound Session will
   * <i>not</i> receive a {@code clear()} call (on rollback) or a {@code disconnect()} call (on
   * transaction completion) in such a scenario; this is rather left up to a custom
   * CurrentSessionContext implementation (if desired).
   */
  ...
  /**
   * Set the bean name of a Hibernate entity interceptor that allows to inspect and change property
   * values before writing to and reading from the database. Will get applied to any new Session
   * created by this transaction manager.
   *
   * <p>Requires the bean factory to be known, to be able to resolve the bean name to an interceptor
   * instance on session creation. Typically used for prototype interceptors, i.e. a new interceptor
   * instance per session.
   *
   * <p>Can also be used for shared interceptor instances, but it is recommended to set the
   * interceptor reference directly in such a scenario.
   *
   * @param entityInterceptorBeanName the name of the entity interceptor in the bean factory
   * @see #setBeanFactory
    ...
    try {
      ...
      if (txObject.isNewSessionHolder()) {
        TransactionSynchronizationManager.bindResource(
            obtainSessionFactory(), txObject.getSessionHolder());
      }
  ...
  protected void doResume(@Nullable Object transaction, Object suspendedResources) {
    ...
    TransactionSynchronizationManager.bindResource(
        sessionFactory, resourcesHolder.getSessionHolder());
  ...
  protected void doSetRollbackOnly(DefaultTransactionStatus status) {
    ...
    if (status.isDebug()) {
      logger.debug(
          "Setting Hibernate transaction on Session ["
              + txObject.getSessionHolder().getSession()
              + "] rollback-only");
    }
  ...
  /**
   * Disconnect a pre-existing Hibernate Session on transaction completion, returning its database
   * connection but preserving its entity state.
   *
   * <p>The default implementation simply calls {@link Session#disconnect()}. Subclasses may
   * override this with a no-op or with fine-tuned disconnection logic.
   *
   ...
   *
   * <p>The default implementation checks the Session's connection release mode to be "on_close".
   *
  ...
  /**
   * Determine whether the given Session is (still) physically connected to the database, that is,
   * holds an active JDBC Connection internally.
   *
   * @param session the Hibernate Session to check
  ...
  /**
   * Convert the given HibernateException to an appropriate exception from the {@code
   * org.springframework.dao} hierarchy.
   *
   * <p>Will automatically apply a specified SQLExceptionTranslator to a Hibernate JDBCException,
  ...
  private class HibernateTransactionObject extends JdbcTransactionObjectSupport {
    ...
    @Nullable private Integer previousHoldability;
    ...
    public boolean hasHibernateManagedTransaction() {
      return (this.sessionHolder != null
          && this.sessionHolder.getSession().getTransaction().getStatus()
  ...
  private static final class SuspendedResourcesHolder {
    ...
    @Nullable private final ConnectionHolder connectionHolder;

    private SuspendedResourcesHolder(
        SessionHolder sessionHolder, @Nullable ConnectionHolder conHolder) {
      this.sessionHolder = sessionHolder;
