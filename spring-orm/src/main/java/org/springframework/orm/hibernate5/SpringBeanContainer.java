...
/**
 * Spring's implementation of Hibernate 5.3's {@link BeanContainer} SPI, delegating to a Spring
 * {@link ConfigurableListableBeanFactory}.
 *
 * <p>Auto-configured by {@link LocalSessionFactoryBean#setBeanFactory}, programmatically supported
 * via {@link LocalSessionFactoryBuilder#setBeanContainer}, and manually configurable through a
 * "hibernate.resource.beans.container" entry in JPA properties, e.g.:
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * Please note that Spring's {@link LocalSessionFactoryBean} is an immediate alternative to {@link
 ...
 * will be registered out of the box.
 *
...
public final class SpringBeanContainer implements BeanContainer {
      ...
      if (lifecycleOptions.useJpaCompliantCreation()) {
        return new SpringContainedBean<>(
            this.beanFactory.createBean(
                beanType, AutowireCapableBeanFactory.AUTOWIRE_CONSTRUCTOR, false),
  ...
  private SpringContainedBean<?> createBean(
    ...
    try {
      if (lifecycleOptions.useJpaCompliantCreation()) {
        ...
        return new SpringContainedBean<>(
            bean, beanInstance -> this.beanFactory.destroyBean(name, beanInstance));
      } else {
      ...
      try {
        ...
        if (ex instanceof BeanCreationException) {
          if (logger.isDebugEnabled()) {
            logger.debug(
                "Fallback producer failed for " + beanType + " with name '" + name + "': " + ex2);
          }
          // Rethrow original Spring exception from first attempt.
          throw ex;
        } else {
  ...
  private static final class SpringContainedBean<B> implements ContainedBean<B> {
    ...
    @Nullable private Consumer<B> destructionCallback;
