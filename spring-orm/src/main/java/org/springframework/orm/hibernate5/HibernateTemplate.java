...
/**
 * Helper class that simplifies Hibernate data access code. Automatically converts
 * HibernateExceptions into DataAccessExceptions, following the {@code org.springframework.dao}
 * exception hierarchy.
 *
 * <p>The central method is {@code execute}, supporting Hibernate access code implementing the
 * {@link HibernateCallback} interface. It provides Hibernate Session handling such that neither the
 * HibernateCallback implementation nor the calling code needs to explicitly care about
 * retrieving/closing Hibernate Sessions, or handling Session lifecycle exceptions. For typical
 * single step actions, there are various convenience methods (find, load, saveOrUpdate, delete).
 *
 * <p>Can be used within a service implementation via direct instantiation with a SessionFactory
 * reference, or get prepared in an application context and given to services as bean reference.
 * Note: The SessionFactory should always be configured as bean in the application context, in the
 * first case given to the service directly, in the second case to the prepared template.
 *
 * <p><b>NOTE: Hibernate access code can also be coded against the native Hibernate {@link Session}.
 * Hence, for newly started projects, consider adopting the standard Hibernate style of coding
 * against {@link SessionFactory#getCurrentSession()}. Alternatively, use {@link
 * #execute(HibernateCallback)} with Java 8 lambda code blocks against the callback-provided {@code
 * Session} which results in elegant code as well, decoupled from the Hibernate Session lifecycle.
 * The remaining operations on this HibernateTemplate are deprecated in the meantime and primarily
 * exist as a migration helper for older Hibernate 3.x/4.x data access code in existing
 * applications.</b>
 *
...
public class HibernateTemplate implements HibernateOperations, InitializingBean {
  ...
  @Nullable private String[] filterNames;
  ...
  @Nullable private String queryCacheRegion;
  ...
  /**
   * Set one or more names of Hibernate filters to be activated for all Sessions that this accessor
   * works with.
   *
   * <p>Each of those filters will be enabled at the beginning of each operation and correspondingly
   * disabled at the end of the operation. This will work for newly opened Sessions as well as for
   * existing Sessions (for example, within a transaction).
   *
   ...
   *
   * <p>Default is "false": a Session proxy will be returned, suppressing {@code close} calls and
   * automatically applying query cache settings and transaction timeouts.
   *
   * @see HibernateCallback
   * @see Session
   * @see #setCacheQueries
  ...
  /**
   * Return whether to expose the native Hibernate Session to HibernateCallback code, or rather a
   * Session proxy.
   */
  ...
  /**
   * Set whether to check that the Hibernate Session is not in read-only mode in case of write
   * operations (save/update/delete).
   *
   * <p>Default is "true", for fail-fast behavior when attempting write operations within a
   * read-only transaction. Turn this off to allow save/update/delete on a Session with flush mode
   * MANUAL.
   *
  ...
  /**
   * Return whether to check that the Hibernate Session is not in read-only mode in case of write
   * operations (save/update/delete).
   */
   ...
   *
   * <p>If this is "true", all Query and Criteria objects created by this template will be marked as
   * cacheable (including all queries through find methods).
   *
   * <p>To specify the query region to be used for queries cached by this template, set the
   * "queryCacheRegion" property.
   *
   * @see #setQueryCacheRegion
   ...
   *
   * <p>If this is specified, it will be applied to all Query and Criteria objects created by this
   * template (including all queries through find methods).
   *
   * <p>The cache region will not take effect unless queries created by this template are configured
   * to be cached via the "cacheQueries" property.
   *
   * @see #setCacheQueries
  ...
  /**
   * Set the fetch size for this HibernateTemplate. This is important for processing large result
   * sets: Setting this higher than the default value will increase processing speed at the cost of
   * memory consumption; setting this lower can avoid transferring row data that will never be read
   * by the application.
   *
  ...
  /**
   * Set the maximum number of rows for this HibernateTemplate. This is important for processing
   * subsets of large result sets, avoiding to read and hold the entire result set in the database
   * or in the JDBC driver if we're never interested in the entire result in the first place (for
   * example, when performing searches that might return a large number of matches).
   *
   * <p>Default is 0, indicating to use the JDBC driver's default.
   */
  ...
  /**
   * Execute the action specified by the given action object within a native {@link Session}.
   *
   * <p>This execute variant overrides the template-wide {@link #isExposeNativeSession()
  ...
  @Nullable
  public <T> T get(
      final Class<T> entityClass, final Serializable id, @Nullable final LockMode lockMode)
      throws DataAccessException {
  ...
  @Nullable
  public Object get(
      final String entityName, final Serializable id, @Nullable final LockMode lockMode)
      throws DataAccessException {
  ...
  @Override
  public void refresh(final Object entity, @Nullable final LockMode lockMode)
      throws DataAccessException {
    executeWithNativeSession(
        session -> {
          if (lockMode != null) {
  ...
  @Override
  public void update(final Object entity, @Nullable final LockMode lockMode)
      throws DataAccessException {
    executeWithNativeSession(
        session -> {
          checkWriteOperationAllowed(session);
  ...
  @Override
  public void update(
      final String entityName, final Object entity, @Nullable final LockMode lockMode)
      throws DataAccessException {
  ...
  public void replicate(
      final String entityName, final Object entity, final ReplicationMode replicationMode)
      throws DataAccessException {
  ...
  @Override
  public void delete(final Object entity, @Nullable final LockMode lockMode)
      throws DataAccessException {
    executeWithNativeSession(
        session -> {
          checkWriteOperationAllowed(session);
  ...
  public List<?> findByCriteria(
      final DetachedCriteria criteria, final int firstResult, final int maxResults)
      throws DataAccessException {
  ...
  public List<?> find(final String queryString, @Nullable final Object... values)
      throws DataAccessException {
    return nonNull(
        executeWithNativeSession(
            (HibernateCallback<List<?>>)
                      ...
                      (org.hibernate.Query)
                          nonNull(
                              ReflectionUtils.invokeMethod(
                                  getNamedQueryMethod, session, queryName));
                  prepareQuery(queryObject);
                  if (values != null) {
                    for (int i = 0; i < values.length; i++) {
                      applyNamedParameterToQuery(queryObject, paramNames[i], values[i]);
  ...
  @Override
  @SuppressWarnings({"rawtypes", "deprecation"})
  public int bulkUpdate(final String queryString, @Nullable final Object... values)
      throws DataAccessException {
    Integer result =
        executeWithNativeSession(
            session -> {
   ...
   *
   * <p>Default implementation throws an InvalidDataAccessApiUsageException in case of {@code
  ...
  protected void checkWriteOperationAllowed(Session session)
    ...
    if (isCheckWriteOperations()
        && SessionFactoryUtils.getFlushMode(session).lessThan(FlushMode.COMMIT)) {
  ...
  protected void applyNamedParameterToQuery(
      org.hibernate.Query queryObject, String paramName, Object value) throws HibernateException {
