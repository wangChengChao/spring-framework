...
/**
 * Spring web request interceptor that binds a Hibernate {@code Session} to the thread for the
 * entire processing of the request.
 *
 * <p>This class is a concrete expression of the "Open Session in View" pattern, which is a pattern
 * that allows for the lazy loading of associations in web views despite the original transactions
 * already being completed.
 *
 * <p>This interceptor makes Hibernate Sessions available via the current thread, which will be
 ...
 *
 * <p>In contrast to {@link OpenSessionInViewFilter}, this interceptor is configured in a Spring
 * application context and can thus take advantage of bean wiring.
 *
 * <p><b>WARNING:</b> Applying this interceptor to existing logic can cause issues that have not
 * appeared before, through the use of a single Hibernate {@code Session} for the processing of an
 * entire request. In particular, the reassociation of persistent objects with a Hibernate {@code
 * Session} has to occur at the very beginning of request processing, to avoid clashes with already
...
public class OpenSessionInViewInterceptor implements AsyncWebRequestInterceptor {
  ...
  /**
   * Suffix that gets appended to the {@code SessionFactory} {@code toString()} representation for
   * the "participate in existing session handling" request attribute.
   *
  ...
  /**
   * Open a new Hibernate {@code Session} according and bind it to the thread via the {@link
   ...
   *
   * <p>The default implementation takes the {@code toString()} representation of the {@code
   * SessionFactory} instance and appends {@link #PARTICIPATE_SUFFIX}.
   */
