...
/**
 * {@link org.springframework.orm.jpa.JpaDialect} implementation for Hibernate EntityManager.
 * Developed against Hibernate 5.1/5.2/5.3.
 *
...
public class HibernateJpaDialect extends DefaultJpaDialect {
    ...
    try {
      ...
      try {
        ...
        throw new IllegalStateException(
            "No compatible Hibernate getFlushMode signature found", ex2);
      }
    }
    // Check that it is the Hibernate FlushMode type, not JPA's...
    Assert.state(
        FlushMode.class == getFlushMode.getReturnType(),
        "Could not find Hibernate getFlushMode method");
  }

  boolean prepareConnection = true;

  @Nullable private SQLExceptionTranslator jdbcExceptionTranslator;
   ...
   *
   * <p>Default is "true". If you turn this flag off, JPA transaction management will not support
   ...
   *
   * <p><b>NOTE:</b> The default behavior in terms of read-only handling changed in Spring 4.1,
   * propagating the read-only status to the JDBC Connection now, analogous to other Spring
   * transaction managers. This may have the effect that you're running into read-only enforcement
   * now where previously write access has accidentally been tolerated: Please revise your
   * transaction declarations accordingly, removing invalid read-only markers if necessary.
   *
   * @since 4.1
   ...
   *
   * <p>Applied to any detected {@link java.sql.SQLException} root cause of a Hibernate {@link
   * JDBCException}, overriding Hibernate's own {@code SQLException} translation (which is based on
   * a Hibernate Dialect for a specific target database).
   *
   * @since 5.1
  ...
  public Object beginTransaction(EntityManager entityManager, TransactionDefinition definition)
    ...
    return new SessionTransactionData(
        session, previousFlushMode, preparedCon, previousIsolationLevel);
  }

  @Override
  public Object prepareTransaction(
      EntityManager entityManager, boolean readOnly, @Nullable String name)
  ...
  protected DataAccessException convertHibernateAccessException(HibernateException ex) {
    if (this.jdbcExceptionTranslator != null && ex instanceof JDBCException) {
      ...
      DataAccessException dae =
          this.jdbcExceptionTranslator.translate(
              "Hibernate operation: " + jdbcEx.getMessage(),
              jdbcEx.getSQL(),
    ...
    if (ex instanceof UnresolvableObjectException) {
      ...
      return new ObjectRetrievalFailureException(
          hibEx.getEntityName(), hibEx.getIdentifier(), ex.getMessage(), ex);
    }
    ...
    if (ex instanceof StaleObjectStateException) {
      ...
      return new ObjectOptimisticLockingFailureException(
          hibEx.getEntityName(), hibEx.getIdentifier(), ex);
    }
  ...
  private static class SessionTransactionData {
    ...
    @Nullable private final FlushMode previousFlushMode;

    @Nullable private final Connection preparedCon;

    @Nullable private final Integer previousIsolationLevel;

    public SessionTransactionData(
        Session session,
        @Nullable FlushMode previousFlushMode,
        @Nullable Connection preparedCon,
        @Nullable Integer previousIsolationLevel) {
    ...
    public void resetSessionState() {
      ...
      if (this.preparedCon != null && this.session.isConnected()) {
        ...
        if (conToReset != this.preparedCon) {
          LogFactory.getLog(HibernateJpaDialect.class)
              .warn(
  ...
  private static class HibernateConnectionHandle implements ConnectionHandle {
    ...
    @Nullable private static volatile Method connectionMethodToUse;
