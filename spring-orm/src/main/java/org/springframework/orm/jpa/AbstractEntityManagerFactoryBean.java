...
/**
 * Abstract {@link org.springframework.beans.factory.FactoryBean} that creates a local JPA {@link
 ...
 *
 * <p>Implements support for standard JPA configuration conventions as well as Spring's customizable
 * {@link JpaVendorAdapter} mechanism, and controls the EntityManagerFactory's lifecycle.
 *
 * <p>This class also implements the {@link
 * org.springframework.dao.support.PersistenceExceptionTranslator} interface, as autodetected by
 * Spring's {@link org.springframework.dao.annotation.PersistenceExceptionTranslationPostProcessor},
 * for AOP-based translation of native exceptions to Spring DataAccessExceptions. Hence, the
 * presence of e.g. LocalEntityManagerFactoryBean automatically enables a
 * PersistenceExceptionTranslationPostProcessor to translate JPA exceptions.
 *
...
public abstract class AbstractEntityManagerFactoryBean
  ...
  @Nullable private PersistenceProvider persistenceProvider;

  @Nullable private String persistenceUnitName;

  private final Map<String, Object> jpaPropertyMap = new HashMap<>();

  @Nullable private Class<? extends EntityManagerFactory> entityManagerFactoryInterface;

  @Nullable private Class<? extends EntityManager> entityManagerInterface;

  @Nullable private JpaDialect jpaDialect;

  @Nullable private JpaVendorAdapter jpaVendorAdapter;
   ...
   * If not specified, the persistence provider will be taken from the JpaVendorAdapter (if any) or
   * retrieved through scanning (as far as possible).
   *
   ...
   */
  public void setPersistenceProviderClass(
  ...
  /**
   * Set the PersistenceProvider instance to use for creating the EntityManagerFactory. If not
   * specified, the persistence provider will be taken from the JpaVendorAdapter (if any) or
   * determined by the persistence unit deployment descriptor (as far as possible).
   *
   * @see JpaVendorAdapter#getPersistenceProvider()
   * @see javax.persistence.spi.PersistenceProvider
   * @see javax.persistence.Persistence
   ...
   *
   * <p>Default is none, indicating the default EntityManagerFactory configuration. The persistence
   * provider will throw an exception if ambiguous EntityManager configurations are found.
   *
  ...
  /**
   * Specify JPA properties, to be passed into {@code Persistence.createEntityManagerFactory} (if
   * any).
   *
   ...
   * @see
   *     javax.persistence.spi.PersistenceProvider#createContainerEntityManagerFactory(javax.persistence.spi.PersistenceUnitInfo,
   *     java.util.Map)
   */
  ...
  /**
   * Specify the (potentially vendor-specific) EntityManagerFactory interface that this
   * EntityManagerFactory proxy is supposed to implement.
   *
   * <p>The default will be taken from the specific JpaVendorAdapter, if any, or set to the standard
   * {@code javax.persistence.EntityManagerFactory} interface else.
   *
  ...
  /**
   * Specify the (potentially vendor-specific) EntityManager interface that this factory's
   * EntityManagers are supposed to implement.
   *
   * <p>The default will be taken from the specific JpaVendorAdapter, if any, or set to the standard
   * {@code javax.persistence.EntityManager} interface else.
   *
  ...
  /**
   * Specify the vendor-specific JpaDialect implementation to associate with this
   * EntityManagerFactory. This will be exposed through the EntityManagerFactoryInfo interface, to
   * be picked up as default dialect by accessors that intend to use JpaDialect functionality.
   *
  ...
  /**
   * Specify the JpaVendorAdapter implementation for the desired JPA provider, if any. This will
   * initialize appropriate defaults for the given provider, such as persistence provider class and
  ...
  /**
   * Return the JpaVendorAdapter implementation for this EntityManagerFactory, or {@code null} if
   * not known.
   */
  @Nullable
   ...
   *
   * <p>{@code EntityManagerFactory} initialization will then switch into background bootstrap mode,
   * with a {@code EntityManagerFactory} proxy immediately returned for injection purposes instead
   * of waiting for the JPA provider's bootstrapping to complete. However, note that the first
   * actual call to a {@code EntityManagerFactory} method will then block until the JPA provider's
   * bootstrapping completed, if not ready by then. For maximum benefit, make sure to avoid early
   * {@code EntityManagerFactory} calls in init methods of related beans, even for metadata
  ...
  /**
   * Create a proxy for the given {@link EntityManagerFactory}. We do this to be able to return a
   * transaction-aware proxy for an application-managed {@link EntityManager}.
   *
   * @param emf the EntityManagerFactory as returned by the persistence provider, if initialized
   *     already
   * @return the EntityManagerFactory proxy
   */
  protected EntityManagerFactory createEntityManagerFactoryProxy(
      @Nullable EntityManagerFactory emf) {
    ...
    try {
      ...
      if (entityManagerFactoryInterface != null) {
        ...
        throw new IllegalStateException(
            "Conflicting EntityManagerFactory interfaces - "
                + "consider specifying the 'jpaVendorAdapter' or 'entityManagerFactoryInterface' property "
                + "to select a specific EntityManagerFactory interface to proceed with",
            ex);
      }
    }
  }
  ...
  /**
   * Delegate an incoming invocation from the proxy, dispatching to EntityManagerFactoryInfo or the
   * native EntityManagerFactory accordingly.
   */
  Object invokeProxyMethod(Method method, @Nullable Object[] args) throws Throwable {
    if (method.getDeclaringClass().isAssignableFrom(EntityManagerFactoryInfo.class)) {
      ...
      // Redirect to plain createEntityManager and add synchronization semantics through Spring
      // proxy
    ...
    if (retVal instanceof EntityManager) {
      ...
      retVal =
          ExtendedEntityManagerCreator.createApplicationManagedEntityManager(
              rawEntityManager, this, false);
    }
    return retVal;
  }

  /**
   * Subclasses must implement this method to create the EntityManagerFactory that will be returned
   * by the {@code getObject()} method.
   *
   ...
   *
   * <p>Uses the dialect's conversion if possible; otherwise falls back to standard JPA exception
   * conversion.
   *
  ...
  public EntityManagerFactory getNativeEntityManagerFactory() {
    if (this.nativeEntityManagerFactory != null) {
      ...
      Assert.state(
          this.nativeEntityManagerFactoryFuture != null,
          "No native EntityManagerFactory available");
      try {
        return this.nativeEntityManagerFactoryFuture.get();
      } catch (InterruptedException ex) {
        Thread.currentThread().interrupt();
        throw new IllegalStateException(
            "Interrupted during initialization of native EntityManagerFactory", ex);
        ...
        throw new IllegalStateException(
            "Failed to asynchronously initialize native EntityManagerFactory: " + ex.getMessage(),
            cause);
      }
    }
  }
  ...
  private void readObject(ObjectInputStream ois) throws IOException, ClassNotFoundException {
    throw new NotSerializableException(
        "An EntityManagerFactoryBean itself is not deserializable - "
  ...
  protected Object writeReplace() throws ObjectStreamException {
    if (this.beanFactory != null && this.beanName != null) {
      ...
      throw new NotSerializableException(
          "EntityManagerFactoryBean does not run within a BeanFactory");
    }
  }

  /**
   * Minimal bean reference to the surrounding AbstractEntityManagerFactoryBean. Resolved to the
   * actual AbstractEntityManagerFactoryBean instance on deserialization.
   */
  @SuppressWarnings("serial")
  ...
  /**
   * Dynamic proxy invocation handler for an {@link EntityManagerFactory}, returning a proxy {@link
   * EntityManager} (if necessary) from {@code createEntityManager} methods.
   */
  @SuppressWarnings("serial")
