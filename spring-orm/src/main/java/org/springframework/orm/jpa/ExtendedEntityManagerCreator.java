...
/**
 * Delegate for creating a variety of {@link javax.persistence.EntityManager} proxies that follow
 * the JPA spec's semantics for "extended" EntityManagers.
 *
 * <p>Supports several different variants of "extended" EntityManagers: in particular, an
 * "application-managed extended EntityManager", as defined by {@link
 * javax.persistence.EntityManagerFactory#createEntityManager()}, as well as a "container-managed
 * extended EntityManager", as defined by {@link javax.persistence.PersistenceContextType#EXTENDED}.
 *
 * <p>The original difference between "application-managed" and "container-managed" was the need for
 * explicit joining of an externally managed transaction through the {@link
 * EntityManager#joinTransaction()} method in the "application" case versus the automatic joining on
 * each user-level EntityManager operation in the "container" case. As of JPA 2.1, both join modes
 * are available with both kinds of EntityManagers, so the difference between "application-" and
 * "container-managed" is now primarily in the join mode default and in the restricted lifecycle of
 * a container-managed EntityManager (i.e. tied to the object that it is injected into).
 *
...
public abstract class ExtendedEntityManagerCreator {
  ...
  private static final Map<Class<?>, Class<?>[]> cachedEntityManagerInterfaces =
   ...
   * @param emfInfo the EntityManagerFactoryInfo to obtain the JpaDialect and PersistenceUnitInfo
   *     from
   * @return an application-managed EntityManager that can join transactions but does not
   *     participate in them automatically
   */
   ...
   * @param emfInfo the EntityManagerFactoryInfo to obtain the JpaDialect and PersistenceUnitInfo
   *     from
   * @param synchronizedWithTransaction whether to automatically join ongoing transactions
   *     (according to the JPA 2.1 SynchronizationType rules)
   * @return an application-managed EntityManager that can join transactions but does not
   *     participate in them automatically
   * @since 4.0
   */
   ...
   * @param emfInfo the EntityManagerFactoryInfo to obtain the JpaDialect and PersistenceUnitInfo
   *     from
   * @return a container-managed EntityManager that will automatically participate in any managed
   *     transaction
   */
   ...
   * Create a container-managed extended EntityManager proxy.
   *
   * @param emf the EntityManagerFactory to create the EntityManager with. If this implements the
   *     EntityManagerFactoryInfo interface, the corresponding JpaDialect and PersistenceUnitInfo
   ...
   *     EntityManagerFactoryInfo interface, the corresponding JpaDialect and PersistenceUnitInfo
   *     will be detected accordingly.
   * @param properties the properties to be passed into the {@code createEntityManager} call (may be
   *     {@code null})
   * @return a container-managed EntityManager that will automatically participate in any managed
   *     transaction
   ...
   */
  public static EntityManager createContainerManagedEntityManager(
      EntityManagerFactory emf, @Nullable Map<?, ?> properties) {
   ...
   * @param synchronizedWithTransaction whether to automatically join ongoing transactions
   *     (according to the JPA 2.1 SynchronizationType rules)
   * @return a container-managed EntityManager that expects container-driven lifecycle management
   *     but may opt out of automatic transaction synchronization
   * @since 4.0
   * @see javax.persistence.EntityManagerFactory#createEntityManager(java.util.Map)
   */
  public static EntityManager createContainerManagedEntityManager(
    ...
    if (emf instanceof EntityManagerFactoryInfo) {
      ...
      return createProxy(
          rawEntityManager, null, null, null, null, true, synchronizedWithTransaction);
    }
  }
   ...
   * @param emfInfo the EntityManagerFactoryInfo to obtain the JpaDialect and PersistenceUnitInfo
   *     from
   * @param containerManaged whether to follow container-managed EntityManager or
   *     application-managed EntityManager semantics
   * @param synchronizedWithTransaction whether to automatically join ongoing transactions
   *     (according to the JPA 2.1 SynchronizationType rules)
  ...
  private static EntityManager createProxy(
    ...
    Boolean jta =
        (pui != null ? pui.getTransactionType() == PersistenceUnitTransactionType.JTA : null);
    return createProxy(
        rawEntityManager,
        emfInfo.getEntityManagerInterface(),
        emfInfo.getBeanClassLoader(),
        jpaDialect,
        jta,
        containerManaged,
   ...
   * @param emIfc the (potentially vendor-specific) EntityManager interface to proxy, or {@code
   *     null} for default detection of all interfaces
   ...
   * @param jta whether to create a JTA-aware EntityManager (or {@code null} if not known in
   *     advance)
   * @param containerManaged whether to follow container-managed EntityManager or
   *     application-managed EntityManager semantics
   * @param synchronizedWithTransaction whether to automatically join ongoing transactions
  ...
  private static EntityManager createProxy(
    ...
    if (emIfc != null) {
      interfaces =
          cachedEntityManagerInterfaces.computeIfAbsent(
              emIfc,
              key -> {
      ...
      interfaces =
          cachedEntityManagerInterfaces.computeIfAbsent(
              ...
              key -> {
                Set<Class<?>> ifcs = new LinkedHashSet<>();
                ifcs.addAll(ClassUtils.getAllInterfacesForClassAsSet(key, cl));
                ifcs.add(EntityManagerProxy.class);
                return ClassUtils.toClassArray(ifcs);
              });
    }
  ...
  private static final class ExtendedEntityManagerInvocationHandler
    ...
    private static final Log logger =
        LogFactory.getLog(ExtendedEntityManagerInvocationHandler.class);
     ...
     *
     * @param enforce whether to enforce the transaction (i.e. whether failure to join is considered
     *     fatal)
     */
    ...
    private void enlistInCurrentTransaction() {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Starting resource-local transaction on application-managed "
                + "EntityManager ["
                + this.target
                + "]");
      }
      ...
      TransactionSynchronizationManager.bindResource(
          this.target, extendedEntityManagerSynchronization);
  ...
  /**
   * TransactionSynchronization enlisting an extended EntityManager with a current Spring
   * transaction.
   */
  private static class ExtendedEntityManagerSynchronization
      extends ResourceHolderSynchronization<EntityManagerHolder, EntityManager> implements Ordered {
