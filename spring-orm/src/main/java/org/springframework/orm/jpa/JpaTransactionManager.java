 ...
 * JPA {@link javax.persistence.EntityManagerFactory}. Binds a JPA EntityManager from the specified
 * factory to the thread, potentially allowing for one thread-bound EntityManager per factory.
 * {@link SharedEntityManagerCreator} and {@code @PersistenceContext} are aware of thread-bound
 * entity managers and participate in such transactions automatically. Using either is required for
 * JPA access code supporting this transaction management mechanism.
 *
 * <p>This transaction manager is appropriate for applications that use a single JPA
 * EntityManagerFactory for transactional data access. JTA (usually through {@link
 * org.springframework.transaction.jta.JtaTransactionManager}) is necessary for accessing multiple
 * transactional resources within the same transaction. Note that you need to configure your JPA
 * provider accordingly in order to make it participate in JTA transactions.
 *
 * <p>This transaction manager also supports direct DataSource access within a transaction (i.e.
 * plain JDBC code working with the same DataSource). This allows for mixing services which access
 * JPA and services which use plain JDBC (without being aware of JPA)! Application code needs to
 * stick to the same simple Connection lookup pattern as with {@link
 * org.springframework.jdbc.datasource.DataSourceTransactionManager} (i.e. {@link
 * org.springframework.jdbc.datasource.DataSourceUtils#getConnection} or going through a {@link
 * org.springframework.jdbc.datasource.TransactionAwareDataSourceProxy}). Note that this requires a
 * vendor-specific {@link JpaDialect} to be configured.
 *
 ...
 * obviously match the one used by the given EntityManagerFactory. This transaction manager will
 * autodetect the DataSource used as the connection factory of the EntityManagerFactory, so you
 ...
 * #setNestedTransactionAllowed "nestedTransactionAllowed"} flag defaults to {@code false} though,
 * since nested transactions will just apply to the JDBC Connection, not to the JPA EntityManager
 * and its cached entity objects and related context. You can manually set the flag to {@code true}
 * if you want to use nested transactions for JDBC access code which participates in JPA
 * transactions (provided that your JDBC driver supports Savepoints). <i>Note that JPA itself does
 * not support nested transactions! Hence, do not expect JPA access code to semantically participate
 * in a nested transaction.</i>
 *
...
public class JpaTransactionManager extends AbstractPlatformTransactionManager
  ...
  @Nullable private String persistenceUnitName;
   ...
   *
   * <p>Alternatively, specify the persistence unit name of the target EntityManagerFactory. By
   * default, a default EntityManagerFactory will be retrieved by finding a single unique bean of
   * type EntityManagerFactory in the containing BeanFactory.
   *
   ...
   *
   * <p>This is an alternative to specifying the EntityManagerFactory by direct reference, resolving
   * it by its persistence unit name instead. If no EntityManagerFactory and no persistence unit
   * name have been specified, a default EntityManagerFactory will be retrieved by finding a single
   * unique bean of type EntityManagerFactory.
   *
   * @see #setEntityManagerFactory
   */
  ...
  /**
   * Specify JPA properties, to be passed into {@code EntityManagerFactory.createEntityManager(Map)}
   * (if any).
   *
  ...
  /**
   * Specify JPA properties as a Map, to be passed into {@code
  ...
  /**
   * Allow Map access to the JPA properties to be passed to the persistence provider, with the
   * option to add or override specific entries.
   *
   ...
   * should match the one used by the JPA EntityManagerFactory: for example, you could specify the
   * same JNDI DataSource for both.
   *
   * <p>If the EntityManagerFactory uses a known DataSource as its connection factory, the
   * DataSource will be autodetected: You can still explicitly specify the DataSource, but you don't
   ...
   * taken from the JPA EntityManager.
   *
   * <p>Note that you need to use a JPA dialect for a specific JPA implementation to allow for
   * exposing JPA transactions as JDBC transactions.
   *
  ...
  /**
   * Set the JPA dialect to use for this transaction manager. Used for vendor-specific transaction
   * management and JDBC connection exposure.
   *
   * <p>If the EntityManagerFactory uses a known JpaDialect, it will be autodetected: You can still
   * explicitly specify the DataSource, but you don't need to in this case.
   *
   * <p>The dialect object can be used to retrieve the underlying JDBC connection and thus allows
   * for exposing JPA transactions as JDBC transactions.
   *
  ...
  /**
   * Retrieves an EntityManagerFactory by persistence unit name, if none set explicitly. Falls back
   * to a default EntityManagerFactory bean if no persistence unit specified.
   *
   * @see #setPersistenceUnitName
   */
  @Override
    ...
    if (getEntityManagerFactory() == null) {
      if (!(beanFactory instanceof ListableBeanFactory)) {
        throw new IllegalStateException(
            "Cannot retrieve EntityManagerFactory by persistence unit name "
                + "in a non-listable BeanFactory: "
                + beanFactory);
      }
    ...
    try {
      ...
      Object transactionData =
          getJpaDialect()
              .beginTransaction(
                  em,
                  new JpaTransactionDefinition(
                      definition, timeoutToUse, txObject.isNewEntityManagerHolder()));
   ...
   *
   * <p>The default implementation checks whether the EntityManagerFactory is a Spring proxy and
   * unwraps it first.
   *
  ...
  /**
   * This implementation returns "true": a JPA commit will properly handle transactions that have
        ...
        try {
          getJpaDialect()
              .releaseJdbcConnection(
                  conHandle, txObject.getEntityManagerHolder().getEntityManager());
        } catch (Exception ex) {
  ...
  /**
   * JPA transaction object, representing a EntityManagerHolder. Used as transaction object by
   * JpaTransactionManager.
   */
  private class JpaTransactionObject extends JdbcTransactionObjectSupport {

    @Nullable private EntityManagerHolder entityManagerHolder;

    private boolean newEntityManagerHolder;

    @Nullable private Object transactionData;
  ...
  private static class JpaTransactionDefinition extends DelegatingTransactionDefinition
    ...
    public JpaTransactionDefinition(
        TransactionDefinition targetDefinition, int timeout, boolean localResource) {
  ...
  private static final class SuspendedResourcesHolder {
    ...
    private SuspendedResourcesHolder(
        EntityManagerHolder emHolder, @Nullable ConnectionHolder conHolder) {
