...
/**
 * Spring web request interceptor that binds a JPA EntityManager to the thread for the entire
 * processing of the request. Intended for the "Open EntityManager in View" pattern, i.e. to allow
 * for lazy loading in web views despite the original transactions already being completed.
 *
 * <p>This interceptor makes JPA EntityManagers available via the current thread, which will be
 ...
 * read-only execution.
 *
 * <p>In contrast to {@link OpenEntityManagerInViewFilter}, this interceptor is set up in a Spring
...
public class OpenEntityManagerInViewInterceptor extends EntityManagerFactoryAccessor
  ...
  /**
   * Suffix that gets appended to the EntityManagerFactory toString representation for the
   * "participate in existing entity manager handling" request attribute.
   *
   ...
   * Default implementation takes the toString representation of the EntityManagerFactory instance
