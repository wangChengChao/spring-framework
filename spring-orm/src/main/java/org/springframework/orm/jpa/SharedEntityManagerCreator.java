...
/**
 * Delegate for creating a shareable JPA {@link javax.persistence.EntityManager} reference for a
 * given {@link javax.persistence.EntityManagerFactory}.
 *
 * <p>A shared EntityManager will behave just like an EntityManager fetched from an application
 * server's JNDI environment, as defined by the JPA specification. It will delegate all calls to the
 * current transactional EntityManager, if any; otherwise it will fall back to a newly created
 * EntityManager per operation.
 *
 * <p>For a behavioral definition of such a shared transactional EntityManager, see {@link
 * javax.persistence.PersistenceContextType#TRANSACTION} and its discussion in the JPA spec
 * document. This is also the default being used for the annotation-based {@link
...
public abstract class SharedEntityManagerCreator {
  ...
  private static final Map<Class<?>, Class<?>[]> cachedQueryInterfaces =
  ...
  public static EntityManager createSharedEntityManager(
    ...
    Class<?> emIfc =
        (emf instanceof EntityManagerFactoryInfo
            ...
            : EntityManager.class);
    return createSharedEntityManager(
        emf,
        properties,
   ...
   * @param properties the properties to be passed into the {@code createEntityManager} call (may be
   *     {@code null})
   * @param entityManagerInterfaces the interfaces to be implemented by the EntityManager. Allows
   *     the addition or specification of proprietary interfaces.
  ...
  /**
   * Invocation handler that delegates all calls to the current transactional EntityManager, if any;
   * else, it will fall back to a newly created EntityManager per operation.
   */
  ...
  private static class SharedEntityManagerInvocationHandler
    ...
    @Nullable private final Map<?, ?> properties;

    private final boolean synchronizedWithTransaction;

    @Nullable private transient volatile ClassLoader proxyClassLoader;

    public SharedEntityManagerInvocationHandler(
        EntityManagerFactory target,
        @Nullable Map<?, ?> properties,
        boolean synchronizedWithTransaction) {
    ...
    private void initProxyClassLoader() {
      if (this.targetFactory instanceof EntityManagerFactoryInfo) {
        this.proxyClassLoader =
            ((EntityManagerFactoryInfo) this.targetFactory).getBeanClassLoader();
      } else {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      ...
      if (method.getName().equals("equals")) {
        ...
        // JPA 2.0: return EntityManagerFactory's CriteriaBuilder/Metamodel (avoid creation of
        // EntityManager)
        try {
        ...
        if (target == null
          ...
          throw new TransactionRequiredException(
              "No EntityManager with actual transaction available "
                  + "for current thread - cannot reliably process '"
                  + method.getName()
                  + "' call");
        }
      }
      ...
      if (target == null) {
        ...
        target =
            (!CollectionUtils.isEmpty(this.properties)
      ...
      try {
        ...
        if (result instanceof Query) {
          ...
          if (isNewEm) {
            Class<?>[] ifcs =
                cachedQueryInterfaces.computeIfAbsent(
                    query.getClass(),
                    key -> ClassUtils.getAllInterfacesForClass(key, this.proxyClassLoader));
            result =
                Proxy.newProxyInstance(
  ...
  /**
   * Invocation handler that handles deferred Query objects created by non-transactional createQuery
   * invocations on a shared EntityManager.
   *
   * <p>Includes deferred output parameter access for JPA 2.1 StoredProcedureQuery, retrieving the
   * corresponding values for all registered parameters on query termination and returning the
   * locally cached values for subsequent access.
   */
  private static class DeferredQueryInvocationHandler implements InvocationHandler {
    ...
    @Nullable private Map<Object, Object> outputParameters;
