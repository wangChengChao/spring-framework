...
/**
 * Helper class featuring methods for JPA EntityManager handling, allowing for reuse of
 * EntityManager instances within transactions. Also provides support for exception translation.
 *
...
public abstract class EntityManagerFactoryUtils {
  ...
  /**
   * Order value for TransactionSynchronization objects that clean up JPA EntityManagers. Return
   * DataSourceUtils.CONNECTION_SYNCHRONIZATION_ORDER - 100 to execute EntityManager cleanup before
   * JDBC Connection cleanup, if any.
   *
  ...
  /**
   * Find an EntityManagerFactory with the given name in the given Spring application context
   * (represented as ListableBeanFactory).
   *
   * <p>The specified unit name will be matched against the configured persistence unit, provided
   * that a discovered EntityManagerFactory implements the {@link EntityManagerFactoryInfo}
   * interface. If not, the persistence unit name will be matched against the Spring bean name,
   * assuming that the EntityManagerFactory bean names follow that convention.
   *
   * <p>If no unit name has been given, this method will search for a default EntityManagerFactory
   * through {@link ListableBeanFactory#getBean(Class)}.
   *
   * @param beanFactory the ListableBeanFactory to search
   * @param unitName the name of the persistence unit (may be {@code null} or empty, in which case a
   *     single bean of type EntityManagerFactory will be searched for)
  ...
  public static EntityManagerFactory findEntityManagerFactory(
      ListableBeanFactory beanFactory, @Nullable String unitName)
   ...
   * bound to the current thread, e.g. when using JpaTransactionManager.
   *
        ...
        if (!emHolder.isSynchronizedWithTransaction()) {
          ...
          if (TransactionSynchronizationManager.isSynchronizationActive()) {
            ...
            TransactionSynchronizationManager.registerSynchronization(
                new TransactionalEntityManagerSynchronization(
                    emHolder, emf, transactionData, false));
  ...
  private static void cleanupTransaction(
      @Nullable Object transactionData, EntityManagerFactory emf) {
    if (emf instanceof EntityManagerFactoryInfo) {
      EntityManagerFactoryInfo emfInfo = (EntityManagerFactoryInfo) emf;
      JpaDialect jpaDialect = emfInfo.getJpaDialect();
      if (jpaDialect != null) {
   ...
   * org.springframework.dao} hierarchy. Return null if no translation is appropriate: any other
   * exception may have resulted from user code, and should not be translated.
   *
   * <p>The most important cases like object not found or optimistic locking failure are covered
   * here. For more fine-granular conversion, JpaTransactionManager etc support sophisticated
   * translation of exceptions via a JpaDialect.
   *
   * @param ex runtime exception that occurred
   * @return the corresponding DataAccessException instance, or {@code null} if the exception should
   *     not be translated
   */
  @Nullable
  ...
  /**
   * Callback for resource cleanup at the end of a non-JPA transaction (e.g. when participating in a
   * JtaTransactionManager transaction), fully synchronized with the ongoing transaction.
   *
  ...
  private static class TransactionalEntityManagerSynchronization
    ...
    @Nullable private final Object transactionData;

    @Nullable private final JpaDialect jpaDialect;
    ...
    public TransactionalEntityManagerSynchronization(
      ...
      this.jpaDialect =
          (emf instanceof EntityManagerFactoryInfo
              ? ((EntityManagerFactoryInfo) emf).getJpaDialect()
              : null);
    ...
    protected void releaseResource(
        EntityManagerHolder resourceHolder, EntityManagerFactory resourceKey) {
  ...
  private static class TransactionScopedEntityManagerSynchronization
    ...
    public TransactionScopedEntityManagerSynchronization(
        EntityManagerHolder emHolder, EntityManagerFactory emf) {
      super(emHolder, emf);
    }
