...
/**
 * {@link FactoryBean} that exposes a shared JPA {@link javax.persistence.EntityManager} reference
 * for a given EntityManagerFactory. Typically used for an EntityManagerFactory created by {@link
 * org.springframework.orm.jpa.LocalContainerEntityManagerFactoryBean}, as direct alternative to a
 * JNDI lookup for a Java EE server's EntityManager reference.
 *
 * <p>The shared EntityManager will behave just like an EntityManager fetched from an application
 ...
 * EntityManagerFactory. Note that Spring's {@link
 * org.springframework.orm.jpa.JpaTransactionManager} always needs an EntityManagerFactory in order
 * to create new transactional EntityManager instances.
 *
...
public class SharedEntityManagerBean extends EntityManagerFactoryAccessor
  ...
  @Nullable private EntityManager shared;

  /**
   * Specify the EntityManager interface to expose.
   *
   * <p>Default is the EntityManager interface as defined by the EntityManagerFactoryInfo, if
   * available. Else, the standard {@code javax.persistence.EntityManager} interface will be used.
   *
  ...
  public Class<? extends EntityManager> getObjectType() {
    return (this.entityManagerInterface != null
        ? this.entityManagerInterface
        : EntityManager.class);
  }
