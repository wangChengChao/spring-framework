...
public class SimpleTaskWorkManager implements WorkManager {
  ...
  /**
   * Specify the TaskExecutor to use for <i>asynchronous</i> work execution (i.e. {@link #startWork}
   * and {@link #scheduleWork} calls).
   *
   ...
   * @return the time elapsed from Work acceptance until start of execution (or -1 if not applicable
   *     or not known)
  ...
  protected long executeWork(
    ...
    try {
      ...
      WorkException wex =
          new WorkRejectedException("TaskExecutor rejected Work because of timeout: " + work, ex);
    ...
    public void run() {
      ...
      try {
        ...
        this.workListener.workCompleted(
            new WorkEvent(
                this, WorkEvent.WORK_COMPLETED, this.work, new WorkCompletedException(ex)));
        throw ex;
      }
      this.workListener.workCompleted(
