...
/**
 * {@link org.springframework.core.task.TaskExecutor} implementation that delegates to a JCA 1.7
 * WorkManager, implementing the {@link javax.resource.spi.work.WorkManager} interface.
 *
 * <p>This is mainly intended for use within a JCA ResourceAdapter implementation, but may also be
 * used in a standalone environment, delegating to a locally embedded WorkManager implementation
 * (such as Geronimo's).
 *
 * <p>Also implements the JCA 1.7 WorkManager interface itself, delegating all calls to the target
 * WorkManager. Hence, a caller can choose whether it wants to talk to this executor through the
 * Spring TaskExecutor interface or the WorkManager interface.
 *
 * <p>This adapter is also capable of obtaining a JCA WorkManager from JNDI. This is for example
 * appropriate on the Geronimo application server, where WorkManager GBeans (e.g. Geronimo's default
 * "DefaultWorkManager" GBean) can be linked into the Java EE environment through "gbean-ref"
 * entries in the {@code geronimo-web.xml} deployment descriptor.
 *
...
public class WorkManagerTaskExecutor extends JndiLocatorSupport
  ...
  @Nullable private String workManagerName;

  private boolean blockUntilStarted = false;

  private boolean blockUntilCompleted = false;

  @Nullable private WorkListener workListener;
   ...
   *
   * <p>This can either be a fully qualified JNDI name, or the JNDI name relative to the current
   * environment naming context if "resourceRef" is set to "true".
   *
   * @see #setWorkManager
   * @see #setResourceRef
   */
   ...
   *
   * <p>Uses the JCA {@code startWork} operation underneath, instead of the default {@code
   * scheduleWork}.
   *
   ...
   *
   * <p>Uses the JCA {@code doWork} operation underneath, instead of the default {@code
   * scheduleWork}.
   *
   ...
   *
   * <p>This shared WorkListener instance will be passed on to the WorkManager by all {@link
   * #execute} calls on this TaskExecutor.
   */
  ...
  /**
   * Obtain a default WorkManager to delegate to. Called if no explicit WorkManager or WorkManager
   * JNDI name has been specified.
   *
   * <p>The default implementation returns a {@link SimpleTaskWorkManager}. Can be overridden in
   * subclasses.
   */
  ...
  public void execute(Runnable task, long startTimeout) {
    Work work =
        new DelegatingWork(this.taskDecorator != null ? this.taskDecorator.decorate(task) : task);
    try {
  ...
  @Override
  public void doWork(
      Work work, long delay, ExecutionContext executionContext, WorkListener workListener)
