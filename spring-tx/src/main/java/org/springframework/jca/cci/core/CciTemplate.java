...
/**
 * <b>This is the central class in the CCI core package.</b> It simplifies the use of CCI and helps
 * to avoid common errors. It executes core CCI workflow, leaving application code to provide
 * parameters to CCI and extract results. This class executes EIS queries or updates, catching
 * ResourceExceptions and translating them to the generic exception hierarchy defined in the {@code
 * org.springframework.dao} package.
 *
 * <p>Code using this class can pass in and receive {@link javax.resource.cci.Record} instances, or
 * alternatively implement callback interfaces for creating input Records and extracting result
 * objects from output Records (or CCI ResultSets).
 *
 * <p>Can be used within a service implementation via direct instantiation with a ConnectionFactory
 * reference, or get prepared in an application context and given to services as bean reference.
 * Note: The ConnectionFactory should always be configured as a bean in the application context, in
...
public class CciTemplate implements CciOperations {
  ...
  @Nullable private ConnectionSpec connectionSpec;

  @Nullable private RecordCreator outputRecordCreator;
  ...
  /**
   * Construct a new CciTemplate, given a ConnectionFactory to obtain Connections from. Note: This
   * will trigger eager initialization of the exception translator.
   *
   ...
   *
   * @param connectionFactory the JCA ConnectionFactory to obtain Connections from
   * @param connectionSpec the CCI ConnectionSpec to obtain Connections for (may be {@code null})
   */
   ...
   *
   * <p>Default is none: When no explicit output Record gets passed into an {@code execute} method,
   * CCI's {@code Interaction.execute} variant that returns an output Record will be called.
   *
   * <p>Specify a RecordCreator here if you always need to call CCI's {@code Interaction.execute}
   * variant with a passed-in output Record. Unless there is an explicitly specified output Record,
   * CciTemplate will then invoke this RecordCreator to create a default output Record instance.
   *
  ...
  /**
   * Create a template derived from this template instance, inheriting the ConnectionFactory and
   * other settings but overriding the ConnectionSpec used for obtaining Connections.
   *
   * @param connectionSpec the CCI ConnectionSpec that the derived template instance is supposed to
   *     obtain Connections for
  ...
  @Override
  public <T> T execute(
      InteractionSpec spec, RecordCreator inputCreator, RecordExtractor<T> outputExtractor)
      throws DataAccessException {
  ...
  /**
   * Execute the specified interaction on an EIS with CCI. All other interaction execution methods
   * go through this.
   *
   * @param spec the CCI InteractionSpec instance that defines the interaction (connector-specific)
  ...
  protected <T> T doExecute(
      ...
      @Nullable final Record outputRecord,
      @Nullable final RecordExtractor<T> outputExtractor)
      throws DataAccessException {

    return execute(
        (InteractionCallback<T>)
            (interaction, connectionFactory) -> {
              ...
              try {
                ...
                return (outputExtractor != null
                    ? outputExtractor.extractData(outputRecordToUse)
                    : null);
              } finally {
   ...
   *
   * <p>Default implementation returns the connector's RecordFactory if available, falling back to a
   * NotSupportedRecordFactory placeholder. This allows to invoke a RecordCreator callback with a
   * non-null RecordFactory reference in any case.
   *
  ...
  /**
   * Close the given CCI Interaction and ignore any thrown exception. This is useful for typical
   * finally blocks in manual CCI code.
   *
  ...
  /**
   * Close the given CCI ResultSet and ignore any thrown exception. This is useful for typical
   * finally blocks in manual CCI code.
   *
