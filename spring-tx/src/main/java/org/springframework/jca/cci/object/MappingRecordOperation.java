...
/**
 * EIS operation object that expects mapped input and output objects, converting to and from CCI
 * Records.
 *
 * <p>Concrete subclasses must implement the abstract {@code createInputRecord(RecordFactory,
 * Object)} and {@code extractOutputData(Record)} methods, to create an input Record from an object
 * and to convert an output Record into an object, respectively.
 *
...
public abstract class MappingRecordOperation extends EisOperation {
   ...
   */
  public MappingRecordOperation(
      ConnectionFactory connectionFactory, InteractionSpec interactionSpec) {
   ...
   *
   * <p>Default is none: CCI's {@code Interaction.execute} variant that returns an output Record
   * will be called.
   *
   * <p>Specify a RecordCreator here if you always need to call CCI's {@code Interaction.execute}
   * variant with a passed-in output Record. This RecordCreator will then be invoked to create a
   * default output Record instance.
   *
   ...
   *
   * @param inputObject the input data, to be converted to a Record by the {@code createInputRecord}
   *     method
  ...
  public Object execute(Object inputObject) throws DataAccessException {
    ...
    return getCciTemplate()
        .execute(interactionSpec, new RecordCreatorImpl(inputObject), new RecordExtractorImpl());
  }

  /**
   * Subclasses must implement this method to generate an input Record from an input object passed
   * into the {@code execute} method.
   *
  ...
  /**
   * Subclasses must implement this method to convert the Record returned by CCI execution into a
   * result object for the {@code execute} method.
   *
  ...
  /**
   * Implementation of RecordCreator that calls the enclosing class's {@code createInputRecord}
   * method.
   */
  ...
  /**
   * Implementation of RecordExtractor that calls the enclosing class's {@code extractOutputData}
   * method.
   */
