...
/**
 * A CCI ConnectionFactory adapter that returns the same Connection on all {@code getConnection}
 * calls, and ignores calls to {@code Connection.close()}.
 *
 * <p>Useful for testing and standalone environments, to keep using the same Connection for multiple
 * CciTemplate calls, without having a pooling ConnectionFactory, also spanning any number of
 * transactions.
 *
 * <p>You can either pass in a CCI Connection directly, or let this factory lazily create a
 * Connection via a given target ConnectionFactory.
 *
...
public class SingleConnectionFactory extends DelegatingConnectionFactory implements DisposableBean {
  ...
  /**
   * Create a new SingleConnectionFactory that always returns a single Connection which it will
  ...
  /**
   * Close the underlying Connection. The provider of this ConnectionFactory needs to care for
   * proper shutdown.
   *
   ...
   * Connection just like an ordinary Connection from a CCI ConnectionFactory.
   *
