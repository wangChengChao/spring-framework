...
/**
 * Helper class that provides static methods for obtaining CCI Connections from a {@link
 * javax.resource.cci.ConnectionFactory}. Includes special support for Spring-managed transactional
 * Connections, e.g. managed by {@link CciLocalTransactionManager} or {@link
 * org.springframework.transaction.jta.JtaTransactionManager}.
 *
 * <p>Used internally by {@link org.springframework.jca.cci.core.CciTemplate}, Spring's CCI
 * operation objects and the {@link CciLocalTransactionManager}. Can also be used directly in
 * application code.
 *
...
public abstract class ConnectionFactoryUtils {
  ...
  /**
   * Obtain a Connection from the given ConnectionFactory. Translates ResourceExceptions into the
   * Spring hierarchy of unchecked generic data access exceptions, simplifying calling code and
   ...
   * {@link CciLocalTransactionManager}. Will bind a Connection to the thread if transaction
   * synchronization is active (e.g. if in a JTA transaction).
   *
   * @param cf the ConnectionFactory to obtain Connection from
   * @return a CCI Connection from the given ConnectionFactory
   * @throws org.springframework.jca.cci.CannotGetCciConnectionException if the attempt to get a
   *     Connection failed
   * @see #releaseConnection
   */
   ...
   *
   * @param cf the ConnectionFactory to obtain Connection from
   * @param spec the ConnectionSpec for the desired Connection (may be {@code null}). Note: If this
   *     is specified, a new Connection will be obtained for every call, without participating in a
  ...
  /**
   * Actually obtain a CCI Connection from the given ConnectionFactory. Same as {@link
   * #getConnection}, but throwing the original ResourceException.
   *
   * <p>Is aware of a corresponding Connection bound to the current thread, for example when using
   * {@link CciLocalTransactionManager}. Will bind a Connection to the thread if transaction
   * synchronization is active (e.g. if in a JTA transaction).
  ...
  public static Connection doGetConnection(ConnectionFactory cf) throws ResourceException {
    ...
    ConnectionHolder conHolder =
        (ConnectionHolder) TransactionSynchronizationManager.getResource(cf);
    if (conHolder != null) {
    ...
    if (TransactionSynchronizationManager.isSynchronizationActive()) {
      ...
      TransactionSynchronizationManager.registerSynchronization(
          new ConnectionSynchronization(conHolder, cf));
  ...
  /**
   * Determine whether the given JCA CCI Connection is transactional, that is, bound to the current
  ...
  /**
   * Close the given Connection, obtained from the given ConnectionFactory, if it is not managed
   * externally (that is, not bound to the thread).
   *
   * @param con the Connection to close if necessary (if this is {@code null}, the call will be
   *     ignored)
   * @param cf the ConnectionFactory that the Connection was obtained from (can be {@code null})
   * @see #getConnection
   */
  ...
  /**
   * Actually close the given Connection, obtained from the given ConnectionFactory. Same as {@link
   * #releaseConnection}, but throwing the original ResourceException.
   *
   * <p>Directly accessed by {@link TransactionAwareConnectionFactoryProxy}.
   *
   * @param con the Connection to close if necessary (if this is {@code null}, the call will be
  ...
  /**
   * Callback for resource cleanup at the end of a non-native CCI transaction (e.g. when
