 ...
 *
 * <p>Requires a {@link javax.resource.cci.ConnectionFactory} to be set, providing a {@link
 * org.springframework.jca.cci.core.CciTemplate} based on it to subclasses through the {@link
 * #getCciTemplate()} method.
 *
 * <p>This base class is mainly intended for CciTemplate usage but can also be used when working
 * with a Connection directly or when using {@code org.springframework.jca.cci.object} classes.
 *
...
public abstract class CciDaoSupport extends DaoSupport {
  ...
  @Nullable private CciTemplate cciTemplate;
  ...
  /**
   * Create a CciTemplate for the given ConnectionFactory. Only invoked if populating the DAO with a
   * ConnectionFactory reference!
   *
   * <p>Can be overridden in subclasses to provide a CciTemplate instance with different
   * configuration, or a custom CciTemplate subclass.
   *
  ...
  /**
   * Set the CciTemplate for this DAO explicitly, as an alternative to specifying a
   * ConnectionFactory.
   */
  ...
  /**
   * Return the CciTemplate for this DAO, pre-initialized with the ConnectionFactory or set
   * explicitly.
   */
  @Nullable
  ...
  /**
   * Obtain a CciTemplate derived from the main template instance, inheriting the ConnectionFactory
   * and other settings but overriding the ConnectionSpec used for obtaining Connections.
   *
   * @param connectionSpec the CCI ConnectionSpec that the returned template instance is supposed to
  ...
  /**
   * Close the given CCI Connection, created via this bean's ConnectionFactory, if it isn't bound to
   * the thread.
   *
