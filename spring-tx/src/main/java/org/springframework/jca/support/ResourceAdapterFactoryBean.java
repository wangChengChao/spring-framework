...
/**
 * {@link org.springframework.beans.factory.FactoryBean} that bootstraps the specified JCA 1.7
 * {@link javax.resource.spi.ResourceAdapter}, starting it with a local {@link
 * javax.resource.spi.BootstrapContext} and exposing it for bean references. It will also stop the
 * ResourceAdapter on context shutdown. This corresponds to 'non-managed' bootstrap in a local
 * environment, according to the JCA 1.7 specification.
 *
 * <p>This is essentially an adapter for bean-style bootstrapping of a JCA ResourceAdapter, allowing
 * the BootstrapContext or its elements (such as the JCA WorkManager) to be specified through bean
 * properties.
 *
...
public class ResourceAdapterFactoryBean
  ...
  /**
   * Specify the target JCA ResourceAdapter as class, to be instantiated with its default
   * configuration.
   *
   * <p>Alternatively, specify a pre-configured ResourceAdapter instance through the
  ...
  /**
   * Specify the target JCA ResourceAdapter, passed in as configured instance which hasn't been
   * started yet. This will typically happen as an inner bean definition, configuring the
   * ResourceAdapter instance through its vendor-specific bean properties.
   */
   ...
   *
   * <p>Alternatively, you can specify the individual parts (such as the JCA WorkManager) as
