 ...
 *
 * <p>Holds all status information that {@link AbstractPlatformTransactionManager} needs internally,
 * including a generic transaction object determined by the concrete transaction manager
 * implementation.
 *
 * <p>Supports delegating savepoint-related methods to a transaction object that implements the
 * {@link SavepointManager} interface.
 *
...
public class DefaultTransactionStatus extends AbstractTransactionStatus {
  ...
  @Nullable private final Object transaction;
   ...
   * @param newTransaction if the transaction is new, otherwise participating in an existing
   *     transaction
   ...
   *     in here can prevent repeated calls to ask the logging system whether debug logging should
   *     be enabled.
  ...
  /**
   * Return whether the progress of this transaction is debugged. This is used by {@link
   * AbstractPlatformTransactionManager} as an optimization, to prevent repeated calls to {@code
   * logger.isDebugEnabled()}. Not really intended for client code.
   */
  ...
  /**
   * Determine the rollback-only flag via checking the transaction object, provided that the latter
   * implements the {@link SmartTransactionObject} interface.
   *
   * <p>Will return {@code true} if the global transaction itself has been marked rollback-only by
   * the transaction coordinator, for example in case of a timeout.
   *
  ...
  /**
   * This implementation exposes the {@link SavepointManager} interface of the underlying
  ...
  /**
   * Return whether the underlying transaction implements the {@link SavepointManager} interface and
   * therefore supports savepoints.
   *
   * @see #getTransaction()
