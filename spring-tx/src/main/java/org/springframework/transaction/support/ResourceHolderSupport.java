 ...
 *
 * <p>Features rollback-only support for participating transactions. Can expire after a certain
...
public abstract class ResourceHolderSupport implements ResourceHolder {
  ...
  /**
   * Return the time to live for this object in seconds. Rounds up eagerly, e.g. 9.00001 still to
   * 10.
   *
  ...
  /**
   * Set the transaction rollback-only if the deadline has been reached, and throw a
  ...
  private void checkTransactionTimeout(boolean deadlineReached)
    ...
    if (deadlineReached) {
      ...
      throw new TransactionTimedOutException(
          "Transaction timed out: deadline was " + this.deadline);
    }
  }

  /**
   * Increase the reference count by one because the holder has been requested (i.e. someone
   * requested the resource held by it).
   */
  ...
  /**
   * Decrease the reference count by one because the holder has been released (i.e. someone released
   * the resource held by it).
   */
