...
/**
 * WebSphere-specific PlatformTransactionManager implementation that delegates to a {@link
 * com.ibm.wsspi.uow.UOWManager} instance, obtained from WebSphere's JNDI environment. This allows
 * Spring to leverage the full power of the WebSphere transaction coordinator, including transaction
 * suspension, in a manner that is perfectly compliant with officially supported WebSphere API.
 *
 * <p>The {@link CallbackPreferringPlatformTransactionManager} interface implemented by this class
 * indicates that callers should preferably pass in a {@link TransactionCallback} through the {@link
 * #execute} method, which will be handled through the callback-based WebSphere UOWManager API
 * instead of through standard JTA API (UserTransaction / TransactionManager). This avoids the use
 * of the non-public {@code javax.transaction.TransactionManager} API on WebSphere, staying within
 * supported WebSphere API boundaries.
 *
 * <p>This transaction manager implementation derives from Spring's standard {@link
 * JtaTransactionManager}, inheriting the capability to support programmatic transaction demarcation
 * via {@code getTransaction} / {@code commit} / {@code rollback} calls through a JTA
 * UserTransaction handle, for callers that do not use the TransactionCallback-based {@link
 * #execute} method. However, transaction suspension is <i>not</i> supported in this {@code
 * getTransaction} style (unless you explicitly specify a {@link #setTransactionManager} reference,
 * despite the official WebSphere recommendations). Use the {@link #execute} style for any code that
 * might require transaction suspension.
 *
 * <p>This transaction manager is compatible with WebSphere 6.1.0.9 and above. The default JNDI
 * location for the UOWManager is "java:comp/websphere/UOWManager". If the location happens to
 * differ according to your WebSphere documentation, simply specify the actual location through this
 * transaction manager's "uowManagerName" bean property.
 *
 * <p><b>NOTE: This JtaTransactionManager is intended to refine specific transaction demarcation
 * behavior on Spring's side. It will happily co-exist with independently configured WebSphere
 * transaction strategies in your persistence provider, with no need to specifically connect those
 * setups in any way.</b>
 *
...
public class WebSphereUowTransactionManager extends JtaTransactionManager
  ...
  @Nullable private UOWManager uowManager;

  @Nullable private String uowManagerName;
   ...
   *
   * <p>Typically just used for test setups; in a Java EE environment, the UOWManager will always be
   * fetched from JNDI.
   *
  ...
  /**
   * Set the JNDI name of the WebSphere UOWManager. The default "java:comp/websphere/UOWManager" is
   * used if not set.
   *
  ...
  protected UOWManager lookupDefaultUowManager() throws TransactionSystemException {
    try {
      logger.debug(
          "Retrieving WebSphere UOWManager from default JNDI location ["
              + DEFAULT_UOW_MANAGER_NAME
              + "]");
      return getJndiTemplate().lookup(DEFAULT_UOW_MANAGER_NAME, UOWManager.class);
    } catch (NamingException ex) {
      logger.debug(
          "WebSphere UOWManager is not available at default JNDI location ["
              + DEFAULT_UOW_MANAGER_NAME
              + "] - falling back to UOWManagerFactory lookup");
  ...
  protected void doRegisterAfterCompletionWithJtaTransaction(
    ...
    obtainUOWManager()
        .registerInterposedSynchronization(new JtaAfterCompletionSynchronization(synchronizations));
  }

  /**
   * Returns {@code true} since WebSphere ResourceAdapters (as exposed in JNDI) implicitly perform
   * transaction enlistment if the MessageEndpointFactory's {@code isDeliveryTransacted} method
   * returns {@code true}. In that case we'll simply skip the {@link #createTransaction} call.
   *
  ...
  public <T> T execute(@Nullable TransactionDefinition definition, TransactionCallback<T> callback)
    ...
    boolean existingTx =
        (uowManager.getUOWStatus() != UOWSynchronizationRegistry.UOW_STATUS_NONE
    ...
    public void run() {
      ...
      DefaultTransactionStatus status =
          prepareTransactionStatus(
              this.definition,
              (this.actualTransaction ? this : null),
              this.newTransaction,
              this.newSynchronization,
              this.debug,
              null);
      try {
        ...
        if (status.isNewSynchronization()) {
          ...
          if (!synchronizations.isEmpty()) {
            uowManager.registerInterposedSynchronization(
