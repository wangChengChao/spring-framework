...
/**
 * {@link org.springframework.transaction.PlatformTransactionManager} implementation for JTA,
 * delegating to a backend JTA provider. This is typically used to delegate to a Java EE server's
 * transaction coordinator, but may also be configured with a local JTA provider which is embedded
 * within the application.
 *
 * <p>This transaction manager is appropriate for handling distributed transactions, i.e.
 * transactions that span multiple resources, and for controlling transactions on application server
 * resources (e.g. JDBC DataSources available in JNDI) in general. For a single JDBC DataSource,
 * DataSourceTransactionManager is perfectly sufficient, and for accessing a single resource with
 * Hibernate (including transactional cache), HibernateTransactionManager is appropriate, for
 * example.
 *
 * <p><b>For typical JTA transactions (REQUIRED, SUPPORTS, MANDATORY, NEVER), a plain
 * JtaTransactionManager definition is all you need, portable across all Java EE servers.</b> This
 * corresponds to the functionality of the JTA UserTransaction, for which Java EE specifies a
 * standard JNDI name ("java:comp/UserTransaction"). There is no need to configure a server-specific
 * TransactionManager lookup for this kind of JTA usage.
 *
 * <p><b>Transaction suspension (REQUIRES_NEW, NOT_SUPPORTED) is just available with a JTA
 * TransactionManager being registered.</b> Common TransactionManager locations are autodetected by
 * JtaTransactionManager, provided that the "autodetectTransactionManager" flag is set to "true"
 * (which it is by default).
 *
 * <p>Note: Support for the JTA TransactionManager interface is not required by Java EE. Almost all
 * Java EE servers expose it, but do so as extension to EE. There might be some issues with
 * compatibility, despite the TransactionManager interface being part of JTA. As a consequence,
 * Spring provides various vendor-specific PlatformTransactionManagers, which are recommended to be
 * used if appropriate: {@link WebLogicJtaTransactionManager} and {@link
 * WebSphereUowTransactionManager}. For all other Java EE servers, the standard
 * JtaTransactionManager is sufficient.
 *
 * <p>This pure JtaTransactionManager class supports timeouts but not per-transaction isolation
 * levels. Custom subclasses may override the {@link #doJtaBegin} method for specific JTA extensions
 * in order to provide this functionality; Spring includes a corresponding {@link
 * WebLogicJtaTransactionManager} class for WebLogic Server. Such adapters for specific Java EE
 * transaction coordinators may also expose transaction names for monitoring; with standard JTA,
 * transaction names will simply be ignored.
 *
 * <p><b>Consider using Spring's {@code tx:jta-transaction-manager} configuration element for
 * automatically picking the appropriate JTA platform transaction manager (automatically detecting
 * WebLogic and WebSphere).</b>
 *
 * <p>JTA 1.1 adds the TransactionSynchronizationRegistry facility, as public Java EE 5 API in
 * addition to the standard JTA UserTransaction handle. As of Spring 2.5, this JtaTransactionManager
 * autodetects the TransactionSynchronizationRegistry and uses it for registering Spring-managed
 * synchronizations when participating in an existing JTA transaction (e.g. controlled by EJB CMT).
 * If no TransactionSynchronizationRegistry is available, then such synchronizations will be
...
public class JtaTransactionManager extends AbstractPlatformTransactionManager
  ...
  /**
   * Default JNDI location for the JTA UserTransaction. Many Java EE servers also provide support
   * for the JTA TransactionManager interface there.
   *
   * @see #setUserTransactionName
  ...
  /**
   * Fallback JNDI locations for the JTA TransactionManager. Applied if the JTA UserTransaction does
   * not implement the JTA TransactionManager interface, provided that the
   * "autodetectTransactionManager" flag is "true".
   *
   * @see #setTransactionManagerName
   * @see #setAutodetectTransactionManager
   */
  ...
  /**
   * Standard Java EE 5 JNDI location for the JTA TransactionSynchronizationRegistry. Autodetected
   * when available.
   */
  ...
  @Nullable private transient UserTransaction userTransaction;

  @Nullable private String userTransactionName;
  ...
  @Nullable private transient TransactionManager transactionManager;

  @Nullable private String transactionManagerName;

  private boolean autodetectTransactionManager = true;

  @Nullable private transient TransactionSynchronizationRegistry transactionSynchronizationRegistry;

  @Nullable private String transactionSynchronizationRegistryName;
  ...
  /**
   * Create a new JtaTransactionManager instance, to be configured as bean. Invoke {@code
  ...
  /**
   * Set the JNDI environment to use for JNDI lookups. Creates a JndiTemplate with the given
   ...
   *
   * <p>Typically just used for local JTA setups; in a Java EE environment, the UserTransaction will
   * always be fetched from JNDI.
   *
   * @see #setUserTransactionName
   ...
   *
   * <p>Note that the UserTransaction will be autodetected at the Java EE default location
   * "java:comp/UserTransaction" if not specified explicitly.
   *
  ...
  /**
   * Set whether to autodetect the JTA UserTransaction at its default JNDI location
   * "java:comp/UserTransaction", as specified by Java EE. Will proceed without UserTransaction if
   * none found.
   *
   * <p>Default is "true", autodetecting the UserTransaction unless it has been specified
   * explicitly. Turn this flag off to allow for JtaTransactionManager operating against the
   * TransactionManager only, despite a default UserTransaction being available.
   *
   * @see #DEFAULT_USER_TRANSACTION_NAME
   */
   ...
   *
   * <p>Default is "true": UserTransaction lookup will only happen at startup, reusing the same
   * UserTransaction handle for all transactions of all threads. This is the most efficient choice
   * for all application servers that provide a shared UserTransaction object (the typical case).
   *
   * <p>Turn this flag off to enforce a fresh lookup of the UserTransaction for every transaction.
   * This is only necessary for application servers that return a new UserTransaction for every
   * transaction, keeping state tied to the UserTransaction object itself rather than the current
   * thread.
   *
   * @see #setUserTransactionName
   */
   ...
   *
   * <p>A TransactionManager is necessary for suspending and resuming transactions, as this not
   * supported by the UserTransaction interface.
   *
   * <p>Note that the TransactionManager will be autodetected if the JTA UserTransaction object
   * implements the JTA TransactionManager interface too, as well as autodetected at various
   * well-known fallback JNDI locations.
   *
   * @see #setTransactionManagerName
   * @see #setAutodetectTransactionManager
   */
  ...
  /**
   * Set whether to autodetect a JTA UserTransaction object that implements the JTA
   * TransactionManager interface too (i.e. the JNDI location for the TransactionManager is
   * "java:comp/UserTransaction", same as for the UserTransaction). Also checks the fallback JNDI
   * locations "java:comp/TransactionManager" and "java:/TransactionManager". Will proceed without
   * TransactionManager if none found.
   *
   * <p>Default is "true", autodetecting the TransactionManager unless it has been specified
   * explicitly. Can be turned off to deliberately ignore an available TransactionManager, for
   * example when there are known issues with suspend/resume and any attempt to use REQUIRES_NEW or
   * NOT_SUPPORTED should fail fast.
   *
   ...
   *
   * <p>A TransactionSynchronizationRegistry allows for interposed registration of transaction
   * synchronizations, as an alternative to the regular registration methods on the JTA
   * TransactionManager API. Also, it is an official part of the Java EE 5 platform, in contrast to
   * the JTA TransactionManager itself.
   *
   * <p>Note that the TransactionSynchronizationRegistry will be autodetected in JNDI and also from
   * the UserTransaction/TransactionManager object if implemented there as well.
   *
  ...
  /**
   * Return the JTA 1.1 TransactionSynchronizationRegistry that this transaction manager uses, if
   * any.
   */
  @Nullable
   ...
   *
   * <p>Note that the TransactionSynchronizationRegistry will be autodetected at the Java EE 5
   * default location "java:comp/TransactionSynchronizationRegistry" if not specified explicitly.
   *
  ...
  /**
   * Set whether to autodetect a JTA 1.1 TransactionSynchronizationRegistry object at its default
   * JDNI location ("java:comp/TransactionSynchronizationRegistry") if the UserTransaction has also
   * been obtained from JNDI, and also whether to fall back to checking whether the JTA
   * UserTransaction/TransactionManager object implements the JTA TransactionSynchronizationRegistry
   * interface too.
   *
   * <p>Default is "true", autodetecting the TransactionSynchronizationRegistry unless it has been
   * specified explicitly. Can be turned off to delegate synchronization registration to the regular
   * JTA TransactionManager API.
   */
   ...
   *
   * <p>Default is "false", throwing an exception if a non-default isolation level is specified for
   * a transaction. Turn this flag on if affected resource adapters check the thread-bound
   * transaction context and apply the specified isolation levels individually (e.g. through an
  ...
  /**
   * Check the UserTransaction as well as the TransactionManager handle, assuming standard JTA
   * requirements.
   *
  ...
  protected void checkUserTransactionAndTransactionManager() throws IllegalStateException {
    ...
    if (this.userTransaction != null) {
      ...
      throw new IllegalStateException(
          "No JTA UserTransaction available - specify either "
   ...
   *
   * <p>To be called after {@link #initUserTransactionAndTransactionManager()}, since it may check
   * the UserTransaction and TransactionManager handles.
   *
   * @throws TransactionSystemException if initialization failed
   */
  protected void initTransactionSynchronizationRegistry() {
    ...
    if (this.transactionSynchronizationRegistry != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Using JTA TransactionSynchronizationRegistry: "
                + this.transactionSynchronizationRegistry);
      }
    }
  }
   ...
   *
   * <p>Called by {@code afterPropertiesSet} if no direct UserTransaction reference was set. Can be
   * overridden in subclasses to provide a different UserTransaction object.
   *
  ...
  protected UserTransaction lookupUserTransaction(String userTransactionName)
    ...
    try {
      ...
      throw new TransactionSystemException(
          "JTA UserTransaction is not available at JNDI location [" + userTransactionName + "]",
          ex);
    }
  }
   ...
   *
   * <p>Called by {@code afterPropertiesSet} if no direct TransactionManager reference was set. Can
   * be overridden in subclasses to provide a different TransactionManager object.
   *
   ...
   *
   * @param registryName the JNDI name of the TransactionSynchronizationRegistry
   * @return the TransactionSynchronizationRegistry object
   * @throws TransactionSystemException if the JNDI lookup failed
   * @see #setJndiTemplate
   * @see #setTransactionSynchronizationRegistryName
   */
  protected TransactionSynchronizationRegistry lookupTransactionSynchronizationRegistry(
    ...
    try {
      ...
      throw new TransactionSystemException(
          "JTA TransactionSynchronizationRegistry is not available at JNDI location ["
              + registryName
              + "]",
          ex);
    }
  ...
  /**
   * Allows subclasses to retrieve the JTA UserTransaction in a vendor-specific manner. Only called
   * if no "userTransaction" or "userTransactionName" specified.
   *
  ...
  /**
   * Allows subclasses to retrieve the JTA TransactionManager in a vendor-specific manner. Only
   * called if no "transactionManager" or "transactionManagerName" specified.
   *
   * <p>The default implementation simply returns {@code null}.
   *
  ...
  /**
   * Allows subclasses to retrieve the JTA 1.1 TransactionSynchronizationRegistry in a
   * vendor-specific manner.
   *
   * <p>The default implementation simply returns {@code null}.
   *
   * @return the JTA TransactionSynchronizationRegistry handle to use, or {@code null} if none found
   * @throws TransactionSystemException in case of errors
   */
  @Nullable
  ...
  /**
   * Find the JTA TransactionManager through autodetection: checking whether the UserTransaction
   * object implements the TransactionManager, and checking the fallback JNDI locations.
   *
  ...
  /**
   * Find the JTA 1.1 TransactionSynchronizationRegistry through autodetection: checking whether the
   * UserTransaction object or TransactionManager object implements it, and checking Java EE 5's
   * standard JNDI location.
   *
   * <p>The default implementation simply returns {@code null}.
   *
   * @param ut the JTA UserTransaction object
  ...
  protected TransactionSynchronizationRegistry findTransactionSynchronizationRegistry(
      @Nullable UserTransaction ut, @Nullable TransactionManager tm)
      throws TransactionSystemException {
    ...
    if (this.userTransactionObtainedFromJndi) {
      ...
      try {
        TransactionSynchronizationRegistry tsr =
            getJndiTemplate().lookup(jndiName, TransactionSynchronizationRegistry.class);
        if (logger.isDebugEnabled()) {
          logger.debug(
   ...
   *
   * <p>The UserTransaction object will either be looked up freshly for the current transaction, or
   * the cached one looked up at startup will be used. The latter is the default: Most application
   * servers use a shared singleton UserTransaction that can be cached. Turn off the
   * "cacheUserTransaction" flag to enforce a fresh lookup for every transaction.
   *
   ...
   *
   * <p>Subclasses can override this to provide a JtaTransactionObject subclass, for example holding
   * some additional JTA handle needed.
   *
  ...
  /**
   * This implementation returns false to cause a further invocation of doBegin despite an already
   * existing transaction.
   *
   * <p>JTA implementations might support nested transactions via further {@code
   ...
   *
   * <p>This implementation only supports standard JTA functionality: that is, no per-transaction
   * isolation levels and no transaction names. Can be overridden in subclasses, for specific JTA
   * implementations.
   *
   * <p>Calls {@code applyIsolationLevel} and {@code applyTimeout} before invoking the
   * UserTransaction's {@code begin} method.
   *
   * @param txObject the JtaTransactionObject containing the UserTransaction
   * @param definition the TransactionDefinition instance, describing propagation behavior,
  ...
  /**
   * Apply the given transaction isolation level. The default implementation will throw an exception
   * for any level other than ISOLATION_DEFAULT.
   *
   * <p>To be overridden in subclasses for specific JTA implementations, as alternative to
   * overriding the full {@link #doJtaBegin} method.
   *
   * @param txObject the JtaTransactionObject containing the UserTransaction
  ...
  protected void applyIsolationLevel(JtaTransactionObject txObject, int isolationLevel)
    ...
    if (!this.allowCustomIsolationLevels
  ...
  /**
   * Apply the given transaction timeout. The default implementation will call {@code
  ...
  /**
   * This implementation returns "true": a JTA commit will properly handle transactions that have
   * been marked rollback-only at a global level.
   */
  @Override
    ...
    try {
      ...
      if (jtaStatus == Status.STATUS_NO_TRANSACTION) {
        ...
        throw new UnexpectedRollbackException(
            "JTA transaction already completed - probably rolled back");
      }
      if (jtaStatus == Status.STATUS_ROLLEDBACK) {
        ...
        throw new UnexpectedRollbackException(
            "JTA transaction already rolled back (probably due to a timeout)");
      }
  ...
  protected void registerAfterCompletionWithExistingTransaction(
    ...
    try {
      ...
      if (ex instanceof RollbackException || ex.getCause() instanceof RollbackException) {
        logger.debug(
            "Participating in existing JTA transaction that has been marked for rollback: "
        ...
        logger.debug(
            "Participating in existing JTA transaction, but unexpected internal transaction "
  ...
  /**
   * Register a JTA synchronization on the JTA TransactionManager, for calling {@code
   * afterCompletion} on the given Spring TransactionSynchronizations.
   *
   * <p>The default implementation registers the synchronizations on the JTA 1.1
   * TransactionSynchronizationRegistry, if available, or on the JTA TransactionManager's current
   * Transaction - again, if available. If none of the two is available, a warning will be logged.
   *
   * <p>Can be overridden in subclasses, for specific JTA implementations.
   *
  ...
  protected void doRegisterAfterCompletionWithJtaTransaction(
    ...
    if (this.transactionSynchronizationRegistry != null) {
      ...
      logger.warn(
          "Participating in existing JTA transaction, but no JTA TransactionManager available: "
              + "cannot register Spring after-completion callbacks with outer JTA transaction - "
