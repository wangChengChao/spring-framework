...
/**
 * Adapter that implements the JTA {@link javax.transaction.Synchronization} interface delegating to
 * an underlying Spring {@link org.springframework.transaction.support.TransactionSynchronization}.
 *
 * <p>Useful for synchronizing Spring resource management code with plain JTA / EJB CMT
 * transactions, despite the original code being built for Spring transaction synchronization.
 *
...
public class SpringJtaSynchronizationAdapter implements Synchronization {
  ...
  @Nullable private UserTransaction jtaTransaction;

  private boolean beforeCompletionCalled = false;

  /**
   * Create a new SpringJtaSynchronizationAdapter for the given Spring TransactionSynchronization
   * and JTA TransactionManager.
   *
   ...
   * and JTA TransactionManager.
   *
   * <p>Note that this adapter will never perform a rollback-only call on WebLogic, since WebLogic
   * Server is known to automatically mark the transaction as rollback-only in case of a {@code
   * beforeCompletion} exception. Hence, on WLS, this constructor is equivalent to the single-arg
   * constructor.
   *
   * @param springSynchronization the Spring TransactionSynchronization to delegate to
   * @param jtaUserTransaction the JTA UserTransaction to use for rollback-only setting in case of
   *     an exception thrown in {@code beforeCompletion} (can be omitted if the JTA provider itself
   *     marks the transaction rollback-only in such a scenario, which is required by the JTA
   *     specification as of JTA 1.1).
   */
  public SpringJtaSynchronizationAdapter(
      TransactionSynchronization springSynchronization,
      @Nullable UserTransaction jtaUserTransaction) {

    this(springSynchronization);
    if (jtaUserTransaction != null
        && !jtaUserTransaction.getClass().getName().startsWith("weblogic.")) {
   ...
   *
   * @param springSynchronization the Spring TransactionSynchronization to delegate to
   * @param jtaTransactionManager the JTA TransactionManager to use for rollback-only setting in
   *     case of an exception thrown in {@code beforeCompletion} (can be omitted if the JTA provider
   *     itself marks the transaction rollback-only in such a scenario, which is required by the JTA
   *     specification as of JTA 1.1)
   */
  public SpringJtaSynchronizationAdapter(
      TransactionSynchronization springSynchronization,
    ...
    if (jtaTransactionManager != null
        && !jtaTransactionManager.getClass().getName().startsWith("weblogic.")) {
  ...
  private void setRollbackOnlyIfPossible() {
    if (this.jtaTransaction != null) {
      try {
        ...
        logger.debug(
            "JTA transaction handle does not support setRollbackOnly method - "
      ...
      logger.debug(
          "No JTA transaction handle available and/or running on WebLogic - "
              + "relying on JTA provider to mark the transaction as rollback-only based on "
              + "the exception thrown from beforeCompletion");
    }
  }
   ...
   *
   * <p>Needs to invoke the Spring synchronization's {@code beforeCompletion} at this late stage in
   * case of a rollback, since there is no corresponding callback with JTA.
   *
