...
/**
 * Special {@link JtaTransactionManager} variant for BEA WebLogic (9.0 and higher). Supports the
 * full power of Spring's transaction definitions on WebLogic's transaction coordinator, <i>beyond
 * standard JTA</i>: transaction names, per-transaction isolation levels, and proper resuming of
 * transactions in all cases.
 *
 * <p>Uses WebLogic's special {@code begin(name)} method to start a JTA transaction, in order to
 * make <b>Spring-driven transactions visible in WebLogic's transaction monitor</b>. In case of
 * Spring's declarative transactions, the exposed name will (by default) be the fully-qualified
 * class name + "." + method name.
 *
 * <p>Supports a <b>per-transaction isolation level</b> through WebLogic's corresponding JTA
 * transaction property "ISOLATION LEVEL". This will apply the specified isolation level (e.g.
 * ISOLATION_SERIALIZABLE) to all JDBC Connections that participate in the given transaction.
 *
 * <p>Invokes WebLogic's special {@code forceResume} method if standard JTA resume failed, to
 * <b>also resume if the target transaction was marked rollback-only</b>. If you're not relying on
 * this feature of transaction suspension in the first place, Spring's standard
 * JtaTransactionManager will behave properly too.
 *
 * <p>By default, the JTA UserTransaction and TransactionManager handles are fetched directly from
 * WebLogic's {@code TransactionHelper}. This can be overridden by specifying
 ...
 * behavior on Spring's side. It will happily co-exist with independently configured WebLogic
...
public class WebLogicJtaTransactionManager extends JtaTransactionManager {
  ...
  private static final String CLIENT_TRANSACTION_MANAGER_CLASS_NAME =
  ...
  private static final String TRANSACTION_HELPER_CLASS_NAME =
  ...
  @Nullable private Method beginWithNameMethod;

  @Nullable private Method beginWithNameAndTimeoutMethod;

  private boolean weblogicTransactionManagerAvailable;

  @Nullable private Method forceResumeMethod;

  @Nullable private Method setPropertyMethod;

  @Nullable private Object transactionHelper;
  ...
  protected UserTransaction retrieveUserTransaction() throws TransactionSystemException {
    ...
    try {
      ...
      throw new TransactionSystemException(
          "WebLogic's TransactionHelper.getUserTransaction() method failed",
  ...
  protected TransactionManager retrieveTransactionManager() throws TransactionSystemException {
    ...
    try {
      ...
      throw new TransactionSystemException(
          "WebLogic's TransactionHelper.getTransactionManager() method failed",
          ex.getTargetException());
    } catch (Exception ex) {
      throw new TransactionSystemException(
  ...
  private Object loadWebLogicTransactionHelper() throws TransactionSystemException {
    ...
    if (helper == null) {
      try {
        ...
        throw new TransactionSystemException(
            "WebLogic's TransactionHelper.getTransactionHelper() method failed",
            ex.getTargetException());
      } catch (Exception ex) {
        throw new TransactionSystemException(
    ...
    try {
      ...
      if (this.weblogicTransactionManagerAvailable) {
        ...
        this.forceResumeMethod =
            transactionManagerClass.getMethod("forceResume", Transaction.class);
        this.setPropertyMethod =
            transactionClass.getMethod("setProperty", String.class, Serializable.class);
    ...
    if (this.weblogicUserTransactionAvailable && definition.getName() != null) {
      try {
        if (timeout > TransactionDefinition.TIMEOUT_DEFAULT) {
          ...
          */
          Assert.state(
              this.beginWithNameAndTimeoutMethod != null, "WebLogic JTA API not initialized");
  ...
  protected void doJtaResume(@Nullable JtaTransactionObject txObject, Object suspendedTransaction)
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Standard JTA resume threw InvalidTransactionException: "
                + ex.getMessage()
      ...
      */
      try {
        ...
        throw new TransactionSystemException(
            "WebLogic's TransactionManager.forceResume(Transaction) method failed",
            ex2.getTargetException());
      } catch (Exception ex2) {
        throw new TransactionSystemException(
      ...
      try {
        ...
        if (ex.getTargetException() instanceof NotSupportedException) {
          ...
          throw new SystemException(
              "WebLogic's begin() method failed with an unexpected error: "
                  + ex.getTargetException());
        }
      } catch (Exception ex) {
        throw new SystemException(
