 ...
 *
 * <p>The tokens can be in <strong>any</strong> order. Propagation and isolation codes must use the
 * names of the constants in the TransactionDefinition class. Timeout values are in seconds. If no
 * timeout is specified, the transaction manager will apply a default timeout specific to the
 * particular transaction manager.
 *
 * <p>A "+" before an exception name substring indicates that transactions should commit even if
 * this exception is thrown; a "-" that they should roll back.
 *
 * @author Rod Johnson
...
public class TransactionAttributeEditor extends PropertyEditorSupport {
  ...
  /**
   * Format is PROPAGATION_NAME,ISOLATION_NAME,readOnly,timeout_NNNN,+Exception1,-Exception2. Null
   * or the empty string means that the method is non transactional.
   */
  @Override
    ...
    if (StringUtils.hasLength(text)) {
      ...
      for (String token : tokens) {
        ...
        if (trimmedToken.startsWith(RuleBasedTransactionAttribute.PREFIX_PROPAGATION)) {
          ...
          String value =
              trimmedToken.substring(DefaultTransactionAttribute.PREFIX_TIMEOUT.length());
