 ...
 * aspect.
 *
 * <p>This enables the underlying Spring transaction infrastructure to be used easily to implement
 * an aspect for any aspect system.
 *
 * <p>Subclasses are responsible for calling methods in this class in the correct order.
 *
 * <p>If no transaction name has been specified in the {@link TransactionAttribute}, the exposed
 * name will be the {@code fully-qualified class name + "." + method name} (by default).
 *
 * <p>Uses the <b>Strategy</b> design pattern. A {@link PlatformTransactionManager} or {@link
 * ReactiveTransactionManager} implementation will perform the actual transaction management, and a
 * {@link TransactionAttributeSource} (e.g. annotation-based) is used for determining transaction
 * definitions for a particular class or method.
 *
 * <p>A transaction aspect is serializable if its {@code PlatformTransactionManager} and {@code
...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
  ...
  private static final boolean vavrPresent =
      ClassUtils.isPresent("io.vavr.control.Try", TransactionAspectSupport.class.getClassLoader());
   ...
   *
   * <p>A TransactionInfo will be returned even if no transaction was created. The {@code
  ...
  @Nullable private final ReactiveAdapterRegistry reactiveAdapterRegistry;

  @Nullable private String transactionManagerBeanName;

  @Nullable private TransactionManager transactionManager;

  @Nullable private TransactionAttributeSource transactionAttributeSource;

  @Nullable private BeanFactory beanFactory;

  private final ConcurrentMap<Object, Object> transactionManagerCache =
  ...
  /**
   * Specify the <em>default</em> transaction manager to use to drive transactions. This can either
   * be a traditional {@link PlatformTransactionManager} or a {@link ReactiveTransactionManager} for
   * reactive transaction management.
   *
   * <p>The default transaction manager will be used if a <em>qualifier</em> has not been declared
   * for a given transaction or if an explicit name for the default transaction manager bean has not
   * been specified.
   *
   ...
   *
   * <p>Note: Method names are always applied to the target class, no matter if defined in an
   * interface or the class itself.
   *
   * <p>Internally, a NameMatchTransactionAttributeSource will be created from the given properties.
   *
   * @see #setTransactionAttributeSource
                ...
                .execute(
                    ...
                    status -> {
                      ...
                      try {
                        ...
                        if (vavrPresent && VavrDelegate.isVavrTry(retVal)) {
                          // Set rollback-only in case of Vavr failure matching our rollback
                          // rules...
                          retVal = VavrDelegate.evaluateTryFailure(retVal, txAttr, status);
                        }
                        return retVal;
  ...
  protected PlatformTransactionManager determineTransactionManager(
    ...
    if (StringUtils.hasText(qualifier)) {
      ...
      PlatformTransactionManager defaultTransactionManager =
          asPlatformTransactionManager(getTransactionManager());
  ...
  private PlatformTransactionManager determineQualifiedTransactionManager(
      BeanFactory beanFactory, String qualifier) {
    PlatformTransactionManager txManager =
        asPlatformTransactionManager(this.transactionManagerCache.get(qualifier));
    if (txManager == null) {
      txManager =
   ...
   *
   * <p>The default implementation returns {@code null}, indicating the use of {@link
   * DefaultTransactionAttribute#getDescriptor()} instead, ending up as {@link
   ...
   *
   * <p>Allows callers to perform custom TransactionAttribute lookups through the
  ...
  protected TransactionInfo createTransactionIfNecessary(
    ...
    if (txAttr != null && txAttr.getName() == null) {
      txAttr =
          new DelegatingTransactionAttribute(txAttr) {
            @Override
  ...
  protected static final class TransactionInfo {
    ...
    @Nullable private final PlatformTransactionManager transactionManager;

    @Nullable private final TransactionAttribute transactionAttribute;

    private final String joinpointIdentification;

    @Nullable private TransactionStatus transactionStatus;

    @Nullable private TransactionInfo oldTransactionInfo;
  ...
  private static class VavrDelegate {
    ...
    public static Object evaluateTryFailure(
        Object retVal, TransactionAttribute txAttr, TransactionStatus status) {
      return ((Try<?>) retVal)
          .onFailure(
              ex -> {
                if (txAttr.rollbackOn(ex)) {
  ...
  private class ReactiveTransactionSupport {
    ...
    public Object invokeWithinTransaction(
        Method method, @Nullable Class<?> targetClass, InvocationCallback invocation) {
      // If the transaction attribute is null, the method is non-transactional.
      TransactionAttributeSource tas = getTransactionAttributeSource();
                ...
                context ->
                    createTransactionIfNecessary(tm, txAttr, joinpointIdentification)
                        .flatMap(
                            it -> {
                              try {
                                // Need re-wrapping until we get hold of the exception through
                                // usingWhen.
      ...
      return this.adapter.fromPublisher(
    ...
    @Nullable
    private ReactiveTransactionManager determineTransactionManager(
        @Nullable TransactionAttribute txAttr) {
      // Do not attempt to lookup tx manager if no tx attributes are set
      ...
      if (StringUtils.hasText(qualifier)) {
        ...
        ReactiveTransactionManager defaultTransactionManager =
            asReactiveTransactionManager(getTransactionManager());
        if (defaultTransactionManager == null) {
          defaultTransactionManager =
    ...
    private ReactiveTransactionManager determineQualifiedTransactionManager(
        BeanFactory beanFactory, String qualifier) {
      ReactiveTransactionManager txManager =
          asReactiveTransactionManager(transactionManagerCache.get(qualifier));
      if (txManager == null) {
        txManager =
            BeanFactoryAnnotationUtils.qualifiedBeanOfType(
    ...
    @Nullable
    private ReactiveTransactionManager asReactiveTransactionManager(
        @Nullable Object transactionManager) {
      if (transactionManager == null || transactionManager instanceof ReactiveTransactionManager) {
        ...
        throw new IllegalStateException(
            "Specified transaction manager is not a ReactiveTransactionManager: "
                + transactionManager);
      }
    }
      ...
      return tx.map(it -> prepareTransactionInfo(tm, attrToUse, joinpointIdentification, it))
          .switchIfEmpty(
              Mono.defer(
                  () ->
                      Mono.just(
                          prepareTransactionInfo(tm, attrToUse, joinpointIdentification, null))));
    }
    ...
    private ReactiveTransactionInfo prepareTransactionInfo(
      ...
      ReactiveTransactionInfo txInfo =
          new ReactiveTransactionInfo(tm, txAttr, joinpointIdentification);
      if (txAttr != null) {
        // We need a transaction for this method...
        if (logger.isTraceEnabled()) {
          logger.trace("Getting transaction for [" + txInfo.getJoinpointIdentification() + "]");
      ...
      if (txInfo != null && txInfo.getReactiveTransaction() != null) {
        ...
        if (txInfo.transactionAttribute != null && txInfo.transactionAttribute.rollbackOn(ex)) {
          return txInfo
              ...
              .rollback(txInfo.getReactiveTransaction())
              .onErrorMap(
                  ex2 -> {
                    logger.error("Application exception overridden by rollback exception", ex);
          ...
          return txInfo
              .getTransactionManager()
              .commit(txInfo.getReactiveTransaction())
              .onErrorMap(
                  ex2 -> {
                    logger.error("Application exception overridden by commit exception", ex);
                    if (ex2 instanceof TransactionSystemException) {
  ...
  private static final class ReactiveTransactionInfo {
    ...
    @Nullable private final ReactiveTransactionManager transactionManager;

    @Nullable private final TransactionAttribute transactionAttribute;

    private final String joinpointIdentification;

    @Nullable private ReactiveTransaction reactiveTransaction;
