...
/**
 * Simple {@link TransactionAttributeSource} implementation that allows attributes to be stored per
 * method in a {@link Map}.
 *
 * @author Rod Johnson
...
public class MethodMapTransactionAttributeSource
  ...
  @Nullable private Map<String, TransactionAttribute> methodMap;
   ...
   * "com.mycompany.mycode.MyClass.myMethod") and {@link TransactionAttribute} instances (or Strings
   * to be converted to {@code TransactionAttribute} instances).
   *
   * <p>Intended for configuration via setter injection, typically within a Spring bean factory.
   * Relies on {@link #afterPropertiesSet()} being called afterwards.
   *
  ...
  /**
   * Add an attribute for a transactional method. Method names can end or start with "*" for
    ...
    for (Method method : matchingMethods) {
      ...
      if (regMethodName == null
        ...
        if (logger.isDebugEnabled() && regMethodName != null) {
          logger.debug(
              "Replacing attribute for transactional method ["
                  + method
                  + "]: current name '"
                  + name
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Keeping attribute for transactional method ["
                  + method
                  + "]: current name '"
                  + name
