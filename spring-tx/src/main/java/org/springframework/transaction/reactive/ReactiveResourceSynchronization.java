...
/**
 * {@link TransactionSynchronization} implementation that manages a resource object bound through
...
public abstract class ReactiveResourceSynchronization<O, K> implements TransactionSynchronization {
    ...
    return Mono.defer(
        () -> {
          ...
          if (shouldUnbindAtCompletion()) {
            ...
            sync =
                cleanupResource(
                    this.resourceObject, this.resourceKey, (status == STATUS_COMMITTED));
          }
          return sync;
        });
  }
  ...
  /**
   * Return whether this holder should be unbound at completion (or should rather be left bound to
   * the thread after the transaction).
   *
  ...
  /**
   * Return whether this holder's resource should be released before transaction completion ({@code
   * true}) or rather after transaction completion ({@code false}).
   *
   * <p>Note that resources will only be released when they are unbound from the thread ({@link
  ...
  /**
   * Return whether this holder's resource should be released after transaction completion ({@code
   * true}).
   *
   * <p>The default implementation returns {@code !shouldReleaseBeforeCompletion()}, releasing after
   * completion if no attempt was made before completion.
   *
   * @see #releaseResource
   */
  ...
  /**
   * After-commit callback for the given resource holder. Only called when the resource hasn't been
   * released yet ({@link #shouldReleaseBeforeCompletion()}).
   *
   ...
   * @param resourceHolder the resource holder to process
   * @param resourceKey the key that the resource object was bound for
   * @param committed whether the transaction has committed ({@code true}) or rolled back ({@code
   *     false})
   */
