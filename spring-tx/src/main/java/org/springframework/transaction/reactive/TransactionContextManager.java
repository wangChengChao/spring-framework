 ...
 *
 * <p>Typically used by components that intercept or orchestrate transactional flows such as AOP
 * interceptors or transactional operators.
 *
 * @author Mark Paluch
 * @since 5.2
...
public abstract class TransactionContextManager {
  ...
  /**
   * Obtain the current {@link TransactionContext} from the subscriber context or the transactional
   * context holder. Context retrieval fails with NoTransactionException if no context or context
   * holder is registered.
   *
   * @return the current {@link TransactionContext}
   * @throws NoTransactionException if no TransactionContext was found in the subscriber context or
   *     no context found in a holder
   */
  ...
  /**
   * Return a {@link Function} to create or associate a new {@link TransactionContext}. Interaction
   * with transactional resources through {@link TransactionSynchronizationManager} requires a
   * TransactionContext to be registered in the subscriber context.
   *
   * @return functional context registration.
   */
  ...
  /**
   * Return a {@link Function} to create or associate a new {@link TransactionContextHolder}.
   * Creation and release of transactions within a reactive flow requires a mutable holder that
   * follows a top to down execution scheme. Reactor's subscriber context follows a down to top
  ...
  public static Function<Context, Context> getOrCreateContextHolder() {
    return context -> {
      if (!context.hasKey(TransactionContextHolder.class)) {
        return context.put(
            TransactionContextHolder.class, new TransactionContextHolder(new ArrayDeque<>()));
      }
      return context;
    };
  }
