...
/**
 * Operator class that simplifies programmatic transaction demarcation and transaction exception
 * handling.
 *
 * <p>The central method is {@link #transactional}, supporting transactional wrapping of functional
 * sequences code that. This operator handles the transaction lifecycle and possible exceptions such
 * that neither the ReactiveTransactionCallback implementation nor the calling code needs to
 ...
 * database connections but are not transaction-aware themselves. Instead, they can implicitly
...
public interface TransactionalOperator {
  ...
  /**
   * Create a new {@link TransactionalOperator} using {@link ReactiveTransactionManager}, using a
   * default transaction.
   *
  ...
  /**
   * Create a new {@link TransactionalOperator} using {@link ReactiveTransactionManager} and {@link
