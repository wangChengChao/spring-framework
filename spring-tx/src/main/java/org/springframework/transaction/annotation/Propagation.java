...
public enum Propagation {
   ...
   *
   * <p>Note: For transaction managers with transaction synchronization, {@code SUPPORTS} is
   * slightly different from no transaction at all, as it defines a transaction scope that
   * synchronization will apply for. As a consequence, the same resources (JDBC Connection,
   * Hibernate Session, etc) will be shared for the entire specified scope. Note that this depends
  ...
  /**
   * Support a current transaction, throw an exception if none exists. Analogous to EJB transaction
   * attribute of the same name.
   */
   ...
   *
   * <p>Note: Actual creation of a nested transaction will only work on specific transaction
   * managers. Out of the box, this only applies to the JDBC DataSourceTransactionManager. Some JTA
