 ...
 * autodetecting WebLogic and WebSphere servers and exposing the corresponding {@link
...
public class JtaTransactionManagerFactoryBean
  ...
  public JtaTransactionManagerFactoryBean() {
    ...
    try {
      Class<? extends JtaTransactionManager> clazz =
          (Class<? extends JtaTransactionManager>)
              ClassUtils.forName(
                  className, JtaTransactionManagerFactoryBean.class.getClassLoader());
      this.transactionManager = BeanUtils.instantiateClass(clazz);
    } catch (ClassNotFoundException ex) {
      throw new IllegalStateException(
          "Failed to load JtaTransactionManager class: " + className, ex);
    }
  }
