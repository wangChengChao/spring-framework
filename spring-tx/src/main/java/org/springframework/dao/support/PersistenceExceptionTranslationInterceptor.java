...
/**
 * AOP Alliance MethodInterceptor that provides persistence exception translation based on a given
 * PersistenceExceptionTranslator.
 *
 * <p>Delegates to the given {@link PersistenceExceptionTranslator} to translate a RuntimeException
 * thrown into Spring's DataAccessException hierarchy (if appropriate). If the RuntimeException in
 * question is declared on the target method, it is always propagated as-is (with no translation
 * applied).
 *
 * @author Rod Johnson
...
public class PersistenceExceptionTranslationInterceptor
  ...
  @Nullable private volatile PersistenceExceptionTranslator persistenceExceptionTranslator;
  ...
  /**
   * Create a new PersistenceExceptionTranslationInterceptor. Needs to be configured with a
   * PersistenceExceptionTranslator afterwards.
   *
   ...
   *
   * @param beanFactory the ListableBeanFactory to obtaining all PersistenceExceptionTranslators
   *     from
   */
   ...
   *
   * <p>Default is to autodetect all PersistenceExceptionTranslators in the containing BeanFactory,
   * using them in a chain.
   *
  ...
  /**
   * Specify whether to always translate the exception ("true"), or whether throw the raw exception
   * when declared, i.e. when the originating method signature's exception declarations allow for
   * the raw exception to be thrown ("false").
   *
   * <p>Default is "false". Switch this flag to "true" in order to always translate applicable
   * exceptions, independent from the originating method signature.
   *
   * <p>Note that the originating method does not have to declare the specific exception. Any base
   * class will do as well, even {@code throws Exception}: As long as the originating method does
   * explicitly declare compatible exceptions, the raw exception will be rethrown. If you would like
   * to avoid throwing raw exceptions in any case, switch this flag to "true".
   */
   ...
   * @param beanFactory the ListableBeanFactory to obtaining all PersistenceExceptionTranslators
   *     from
   * @return a chained PersistenceExceptionTranslator, combining all PersistenceExceptionTranslators
  ...
  protected PersistenceExceptionTranslator detectPersistenceExceptionTranslators(
    ...
    Map<String, PersistenceExceptionTranslator> pets =
