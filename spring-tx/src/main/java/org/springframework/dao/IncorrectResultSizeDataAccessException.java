...
/**
 * Data access exception thrown when a result was not of the expected size, for example when
 * expecting a single row but getting 0 or more than 1 rows.
 *
...
public class IncorrectResultSizeDataAccessException extends DataRetrievalFailureException {
  ...
  public IncorrectResultSizeDataAccessException(
      String msg, int expectedSize, int actualSize, Throwable ex) {
    super(msg, ex);
    this.expectedSize = expectedSize;
    this.actualSize = actualSize;
  }
