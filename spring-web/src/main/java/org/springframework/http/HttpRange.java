...
public abstract class HttpRange {
  ...
  /**
   * Turn a {@code Resource} into a {@link ResourceRegion} using the range information contained in
   * the current {@code HttpRange}.
   *
  ...
  public ResourceRegion toResourceRegion(Resource resource) {
    ...
    Assert.isTrue(
        resource.getClass() != InputStreamResource.class,
   ...
   * @throws IllegalArgumentException if the string cannot be parsed or if the number of ranges is
   *     greater than 100
   */
  ...
  /**
   * Convert each {@code HttpRange} into a {@code ResourceRegion}, selecting the appropriate segment
   * of the given {@code Resource} using HTTP Range information.
   *
    ...
    private void assertPositions(long firstBytePos, @Nullable Long lastBytePos) {
      ...
      if (lastBytePos != null && lastBytePos < firstBytePos) {
        throw new IllegalArgumentException(
            "firstBytePosition="
                + firstBytePos
                + " should be less then or equal to lastBytePosition="
                + lastBytePos);
      }
    }
