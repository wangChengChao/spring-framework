...
/**
 * An alternative to {@link AbstractListenerWriteProcessor} but instead writing a {@code
 * Publisher<Publisher<T>>} with flush boundaries enforces after the completion of each nested
 * Publisher.
 *
...
public abstract class AbstractListenerWriteFlushProcessor<T>
  ...
  /**
   * Invoked when flushing is possible, either in the same thread after a check via {@link
  ...
  /**
   * Flush the output if ready, or otherwise {@link #isFlushPending()} should return true after.
   *
   * <p>This is primarily for the Servlet non-blocking I/O API where flush cannot be called without
   * a readyToWrite check.
   */
  ...
  /**
   * Invoked when an error happens while flushing. Sub-classes may choose to ignore this if they
  ...
  private enum State {
    UNSUBSCRIBED {
      ...
      public <T> void onSubscribe(
          AbstractListenerWriteFlushProcessor<T> processor, Subscription subscription) {
    ...
    FLUSHING {
      ...
      @Override
      public <T> void onNext(
          AbstractListenerWriteFlushProcessor<T> proc, Publisher<? extends T> pub) {
        // ignore
      }
    ...
    public <T> void onSubscribe(
        AbstractListenerWriteFlushProcessor<T> proc, Subscription subscription) {
      subscription.cancel();
    }
    ...
    /**
     * Subscriber to receive and delegate completion notifications for from the current Publisher,
     * i.e. for the current flush boundary.
     */
      ...
      public void onComplete() {
        if (rsWriteFlushLogger.isTraceEnabled()) {
          rsWriteFlushLogger.trace(
              this.processor.getLogPrefix() + this.processor.state + " writeComplete");
        }
