...
/**
 * Abstract base class for {@code Publisher} implementations that bridge between event-listener read
 * APIs and Reactive Streams.
 *
 * <p>Specifically a base class for reading from the HTTP request body with Servlet 3.1 non-blocking
 * I/O and Undertow XNIO as well as handling incoming WebSocket messages with standard Java
 * WebSocket (JSR-356), Jetty, and Undertow.
 *
...
public abstract class AbstractListenerReadPublisher<T> implements Publisher<T> {
  ...
  protected static Log rsReadLogger =
      LogDelegateFactory.getHiddenLog(AbstractListenerReadPublisher.class);
  ...
  @Nullable private volatile Subscriber<? super T> subscriber;

  private volatile boolean completionBeforeDemand;

  @Nullable private volatile Throwable errorBeforeDemand;
  ...
  /**
   * Invoked when reading is possible, either in the same thread after a check via {@link
   * #checkOnDataAvailable()}, or as a callback from the underlying container.
   */
  ...
  /**
   * Sub-classes can call this method to delegate a contain notification when all data has been
   * read.
   */
  ...
  /**
   * Check if data is available and either call {@link #onDataAvailable()} immediately or schedule a
   * notification.
   */
   ...
   *
   * <p><strong>Note:</strong> This method is guaranteed not to compete with {@link
   * #checkOnDataAvailable()} so it can be used to safely suspend reading, if the underlying API
   * supports it, i.e. without competing with an implicit call to resume via {@code
  ...
  /**
   * Invoked after an I/O read error from the underlying server or after a cancellation signal from
   * the downstream consumer to allow sub-classes to discard any current cached data they might
   * have.
   *
   * @since 5.0.11
   */
  ...
  /**
   * Read and publish data one at a time until there is no more data, no more demand, or perhaps we
   * completed in the mean time.
   *
   * @return {@code true} if there is more demand; {@code false} if there is no more demand or we
   *     have completed.
   */
  ...
  private enum State {
    UNSUBSCRIBED {
      @Override
      <T> void subscribe(
          AbstractListenerReadPublisher<T> publisher, Subscriber<? super T> subscriber) {
    ...
    /**
     * Very brief state where we know we have a Subscriber but must not send onComplete and onError
     * until we after onSubscribe.
     */
    SUBSCRIBING {
      @Override
