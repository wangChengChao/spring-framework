...
/**
 * Abstract base class for {@code Processor} implementations that bridge between event-listener
 * write APIs and Reactive Streams.
 *
 * <p>Specifically a base class for writing to the HTTP response body with Servlet 3.1 non-blocking
 * I/O and Undertow XNIO as well for writing WebSocket messages through the Java WebSocket API
 * (JSR-356), Jetty, and Undertow.
 *
...
public abstract class AbstractListenerWriteProcessor<T> implements Processor<T, Void> {
  ...
  protected static final Log rsWriteLogger =
      LogDelegateFactory.getHiddenLog(AbstractListenerWriteProcessor.class);

  private final AtomicReference<State> state = new AtomicReference<>(State.UNSUBSCRIBED);

  @Nullable private Subscription subscription;

  @Nullable private volatile T currentData;
  ...
  /**
   * Error signal from the upstream, write Publisher. This is also used by sub-classes to delegate
   * error notifications from the container.
   */
  @Override
  ...
  /**
   * Completion signal from the upstream, write Publisher. This is also used by sub-classes to
   * delegate completion notifications from the container.
   */
  @Override
  ...
  /**
   * Invoked when writing is possible, either in the same thread after a check via {@link
   * #isWritePossible()}, or as a callback from the underlying container.
   */
  ...
  /**
   * Invoked during an error or completion callback from the underlying container to cancel the
  ...
  /**
   * Template method invoked after a data item to write is received via {@link
   * Subscriber#onNext(Object)}. The default implementation saves the data item for writing once
   * that is possible.
   */
   ...
   *
   * <p><strong>Note:</strong> Sub-classes are responsible for releasing any data buffer associated
   * with the item, once fully written, if pooled buffers apply to the underlying container.
   *
   * @param data the item to write
   * @return whether the current data item was written and another one requested ({@code true}), or
   *     or otherwise if more writes are required.
   */
  protected abstract boolean write(T data) throws IOException;

  /**
   * Invoked after the current data has been written and before requesting the next item from the
   * upstream, write Publisher.
   *
   * <p>The default implementation is a no-op.
   *
   * @deprecated originally introduced for Undertow to stop write notifications when no data is
   *     available, but deprecated as of as of 5.0.6 since constant switching on every requested
   *     item causes a significant slowdown.
   */
  @Deprecated
  ...
  /**
   * Invoked when an I/O error occurs during a write. Sub-classes may choose to ignore this if they
   * know the underlying API will provide an error notification in a container thread.
   *
   * <p>Defaults to no-op.
   */
  ...
  /**
   * Invoked after any error (either from the upstream write Publisher, or from I/O operations to
   * the underlying server) and cancellation to discard in-flight data that was in the process of
   * being written when the error took place.
   *
  ...
  private enum State {
    UNSUBSCRIBED {
      @Override
      public <T> void onSubscribe(
          AbstractListenerWriteProcessor<T> processor, Subscription subscription) {
