...
/**
 * Structured representation of a URI path parsed via {@link #parsePath(String)} into a sequence of
 * {@link Separator} and {@link PathSegment} elements.
 *
 * <p>Each {@link PathSegment} exposes its content in decoded form and with path parameters removed.
 * This makes it safe to match one path segment at a time without the risk of decoded reserved
 * characters altering the structure of the path.
 *
...
public interface PathContainer {
  ...
  /**
   * Extract a sub-path from the given start offset into the element list (inclusive) and to the end
   * offset (exclusive).
   *
  ...
  /**
   * Parse the path value into a sequence of {@code "/"} {@link Separator Separator} and {@link
   * PathSegment PathSegment} elements.
   *
  ...
  /**
   * Parse the path value into a sequence of {@link Separator Separator} and {@link PathSegment
  ...
  class Options {
     ...
     *
     * @param separator the separator for parsing the path into segments; currently this must be
     *     slash or dot.
     * @param decodeAndParseSegments whether to URL decode path segment values and parse path
     *     parameters. If set to false, only escape sequences for the separator char are decoded.
     */
