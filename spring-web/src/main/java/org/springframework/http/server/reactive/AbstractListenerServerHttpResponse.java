...
public abstract class AbstractListenerServerHttpResponse extends AbstractServerHttpResponse {
  ...
  public AbstractListenerServerHttpResponse(
  ...
  protected final Mono<Void> writeAndFlushWithInternal(
    ...
    if (this.writeCalled.compareAndSet(false, true)) {
      Processor<? super Publisher<? extends DataBuffer>, Void> processor =
    ...
    return Mono.error(
        new IllegalStateException("writeWith() or writeAndFlushWith() has already been called"));
  }

  /**
   * Abstract template method to create a {@code Processor<Publisher<DataBuffer>, Void>} that will
   * write the response body with flushes to the underlying output. Called from {@link
