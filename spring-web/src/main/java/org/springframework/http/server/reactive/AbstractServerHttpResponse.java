...
public abstract class AbstractServerHttpResponse implements ServerHttpResponse {
  ...
  /**
   * COMMITTING -> COMMITTED is the period after doCommit is called but before the response status
   * and headers have been applied to the underlying response during which time pre-commit actions
   * can still make changes to the response status and headers.
   */
  private enum State {
    NEW,
    COMMITTING,
  ...
  @Override
  public final Mono<Void> writeAndFlushWith(
      Publisher<? extends Publisher<? extends DataBuffer>> body) {
    return new ChannelSendOperator<>(
            body, inner -> doCommit(() -> writeAndFlushWithInternal(inner)))
        .doOnError(t -> removeContentLength());
  }
  ...
  /**
   * Apply {@link #beforeCommit(Supplier) beforeCommit} actions, apply the response status and
   * headers/cookies, and write the response body.
   *
  ...
  /**
   * Apply header changes from {@link #getHeaders()} to the underlying response. This method is
   * called once only.
   */
  protected abstract void applyHeaders();

  /**
   * Add cookies from {@link #getHeaders()} to the underlying response. This method is called once
   * only.
   */
