...
/**
 * A subclass of {@link MimeType} that adds support for quality parameters as defined in the HTTP
 * specification.
 *
 ...
 * @author Sam Brannen
 * @since 3.0
 * @see <a href="https://tools.ietf.org/html/rfc7231#section-3.1.1.1">HTTP 1.1: Semantics and
 *     Content, section 3.1.1.1</a>
 */
...
public class MediaType extends MimeType implements Serializable {
   ...
   *
   * @deprecated Deprecated as of Spring Framework 5.2 in favor of {@link #APPLICATION_JSON} since
   *     major browsers like Chrome <a
   *     href="https://bugs.chromium.org/p/chromium/issues/detail?id=438464">now comply with the
   *     specification</a> and interpret correctly UTF-8 special characters without requiring a
   ...
   *     since major browsers like Chrome <a
   *     href="https://bugs.chromium.org/p/chromium/issues/detail?id=438464">now comply with the
   *     specification</a> and interpret correctly UTF-8 special characters without requiring a
   *     {@code charset=UTF-8} parameter.
   */
   ...
   *
   * @since 5.0
   * @deprecated Deprecated as of Spring Framework 5.2 in favor of {@link
   *     #APPLICATION_PROBLEM_JSON_VALUE} since major browsers like Chrome <a
   *     href="https://bugs.chromium.org/p/chromium/issues/detail?id=438464">now comply with the
   *     specification</a> and interpret correctly UTF-8 special characters without requiring a
   *     {@code charset=UTF-8} parameter.
   */
  ...
  public MediaType(String type, String subtype, double qualityValue) {
    this(
        type,
        subtype,
        Collections.singletonMap(PARAM_QUALITY_FACTOR, Double.toString(qualityValue)));
  }
   ...
   * symmetric.
   *
   * <p>Simply calls {@link MimeType#includes(MimeType)} but declared with a {@code MediaType}
   ...
   * symmetric.
   *
   * <p>Simply calls {@link MimeType#isCompatibleWith(MimeType)} but declared with a {@code
   * MediaType} parameter for binary backwards compatibility.
   *
   * @param other the reference media type with which to compare
   * @return {@code true} if this media type is compatible with the given media type; {@code false}
   *     otherwise
   */
  ...
  /**
   * Parse the given String value into a {@code MediaType} object, with this method name following
  ...
  /**
   * Parse the given list of (potentially) comma-separated strings into a list of {@code MediaType}
   * objects.
   *
   * <p>This method can be used to parse an Accept or Content-Type header.
   *
   ...
   *       type without the wildcard is ordered before the other.
   ...
   *       media type without the wildcard is sorted before the other.
   ...
   *       the media type with the highest quality value is ordered before the other.
   *   <li>if the two media types have a different amount of {@linkplain #getParameter(String)
   *       parameters}, then the media type with the most parameters is ordered before the other.
   * </ol>
   *
   * <p>For example:
   *
  ...
  public static final Comparator<MediaType> SPECIFICITY_COMPARATOR =
      new SpecificityComparator<MediaType>() {
