...
/**
 * A data structure representing HTTP request or response headers, mapping String header names to a
 * list of String values, also offering accessors for common application-level data types.
 *
 * <p>In addition to the regular methods defined by {@link Map}, this class offers many common
 * convenience methods, for example:
 *
 * <ul>
 *   <li>{@link #getFirst(String)} returns the first value associated with a given header name
 *   <li>{@link #add(String, String)} adds a header value to the list of values for a header name
 *   <li>{@link #set(String, String)} sets the header value to a single string value
 * </ul>
 *
...
public class HttpHeaders implements MultiValueMap<String, String>, Serializable {
   ...
   *
   * @see <a href="https://tools.ietf.org/html/rfc7231#section-3.1.2.2">Section 3.1.2.2 of RFC
   *     7231</a>
   */
   ...
   *
   * @see <a href="https://tools.ietf.org/html/rfc7231#section-3.1.3.2">Section 3.1.3.2 of RFC
   *     7231</a>
   */
   ...
   *
   * @see <a href="https://tools.ietf.org/html/rfc7231#section-3.1.4.2">Section 3.1.4.2 of RFC
   *     7231</a>
   */
   ...
   *
   * @see <a href="https://tools.ietf.org/html/rfc7231#section-3.1.1.5">Section 3.1.1.5 of RFC
   *     7231</a>
   */
   ...
   *
   * @see <a href="https://tools.ietf.org/html/rfc7231#section-7.1.1.2">Section 7.1.1.2 of RFC
   *     7231</a>
   */
   ...
   *
   * <p>This constructor is available as an optimization for adapting to existing headers map
   * structures, primarily for internal use within the framework.
   *
  ...
  /**
   * Return the list of acceptable {@linkplain MediaType media types}, as specified by the {@code
   * Accept} header.
   *
   ...
   *
   * <p>If you only need sorted, preferred locales only use {@link #getAcceptLanguageAsLocales()} or
   * if you need to filter based on a list of supported locales you can pass the returned list to
  ...
  /**
   * A variant of {@link #getAcceptLanguage()} that converts each {@link
   * java.util.Locale.LanguageRange} to a {@link Locale}.
   *
  ...
  public void setAccessControlRequestMethod(@Nullable HttpMethod requestMethod) {
    setOrRemove(
        ACCESS_CONTROL_REQUEST_METHOD, (requestMethod != null ? requestMethod.name() : null));
  }
  ...
  /**
   * Return the list of acceptable {@linkplain Charset charsets}, as specified by the {@code
   * Accept-Charset} header.
   */
  ...
  /**
   * Set the value of the {@linkplain #AUTHORIZATION Authorization} header to Basic Authentication
   * based on the given username and password.
   *
   * <p>Note that this method only supports characters in the {@link StandardCharsets#ISO_8859_1
   * ISO-8859-1} character set.
   *
   * @param username the username
   * @param password the password
   * @throws IllegalArgumentException if either {@code user} or {@code password} contain characters
   *     that cannot be encoded to ISO-8859-1
   * @since 5.1
   ...
   * @throws IllegalArgumentException if {@code username} or {@code password} contains characters
   *     that cannot be encoded to the given charset
   * @since 5.1
   * @see #setBasicAuth(String)
  ...
  /**
   * Set the value of the {@linkplain #AUTHORIZATION Authorization} header to Basic Authentication
   * based on the given {@linkplain #encodeBasicAuth encoded credentials}.
   *
   * <p>Favor this method over {@link #setBasicAuth(String, String)} and {@link
   * #setBasicAuth(String, String, Charset)} if you wish to cache the encoded credentials.
   *
  ...
  /**
   * Set the value of the {@linkplain #AUTHORIZATION Authorization} header to the given Bearer
   * token.
   *
  ...
  /**
   * Set a configured {@link CacheControl} instance as the new value of the {@code Cache-Control}
   * header.
   *
   * @since 5.0.5
   */
  ...
  /**
   * Set the {@code Content-Disposition} header when creating a {@code "multipart/form-data"}
   * request.
   *
   * <p>Applications typically would not set this header directly but rather prepare a {@code
   * MultiValueMap<String, Object>}, containing an Object or a {@link
   * org.springframework.core.io.Resource} for each part, and then pass that to the {@code
   * RestTemplate} or {@code WebClient}.
   *
   ...
   *
   * <p>This could be used on a response to indicate if the content is expected to be displayed
   * inline in the browser or as an attachment to be saved locally.
   *
   * <p>It can also be used for a {@code "multipart/form-data"} request. For more details see notes
   * on {@link #setContentDispositionFormData}.
   *
   * @since 5.0
   * @see #getContentDisposition()
   */
  ...
  /**
   * Set the {@link Locale} of the content language, as specified by the {@literal Content-Language}
   * header.
   *
   * <p>Use {@code put(CONTENT_LANGUAGE, list)} if you need to set multiple content languages.
   *
   * @since 5.0
   */
  ...
  /**
   * Return the first {@link Locale} of the content languages, as specified by the {@literal
   * Content-Language} header.
   *
   * <p>Returns {@code null} when the content language is unknown.
   *
   * <p>Use {@code getValuesAsList(CONTENT_LANGUAGE)} if you need to get multiple content languages.
   *
   * @since 5.0
   */
  @Nullable
  public Locale getContentLanguage() {
    return getValuesAsList(CONTENT_LANGUAGE).stream()
        .findFirst()
  ...
  /**
   * Set the {@linkplain MediaType media type} of the body, as specified by the {@code Content-Type}
   * header.
   */
  public void setContentType(@Nullable MediaType mediaType) {
    if (mediaType != null) {
      ...
      Assert.isTrue(
          !mediaType.isWildcardSubtype(), "Content-Type cannot contain wildcard subtype '*'");
  ...
  /**
   * Return the {@linkplain MediaType media type} of the body, as specified by the {@code
   * Content-Type} header.
   *
  ...
  /**
   * Return the date and time at which the message was created, as specified by the {@code Date}
   * header.
   *
   * <p>The date is returned as the number of milliseconds since January 1, 1970 GMT. Returns -1
  ...
  public void setETag(@Nullable String etag) {
    if (etag != null) {
      Assert.isTrue(
          etag.startsWith("\"") || etag.startsWith("W/"),
  ...
  /**
   * Set the duration after which the message is no longer valid, as specified by the {@code
   * Expires} header.
   *
   * @since 5.0.5
   */
  ...
  /**
   * Return the date and time at which the message is no longer valid, as specified by the {@code
   * Expires} header.
   *
   * <p>The date is returned as the number of milliseconds since January 1, 1970 GMT. Returns -1
   * when the date is unknown.
   ...
   * will only contain the {@linkplain InetSocketAddress#getHostString() host name}.
   *
   * @since 5.0
   */
   ...
   *
   * <p>If the header value does not contain a port, the {@linkplain InetSocketAddress#getPort()
   * port} in the returned address will be {@code 0}.
   *
   * @since 5.0
   */
  @Nullable
  public InetSocketAddress getHost() {
    ...
    int port = 0;
    int separator =
        (value.startsWith("[") ? value.indexOf(':', value.indexOf(']')) : value.lastIndexOf(':'));
    if (separator != -1) {
  ...
  /**
   * Return the time the resource was last changed, as specified by the {@code Last-Modified}
   * header.
   *
   * <p>The date is returned as the number of milliseconds since January 1, 1970 GMT. Returns -1
   * when the date is unknown.
  ...
  /**
   * Set the request header names (e.g. "Accept-Language") for which the response is subject to
   * content negotiation and variances based on the value of those request headers.
   *
   ...
   * RFC-1123 date-time formatter. The equivalent of {@link #set(String, String)} but for date
   * headers.
   *
   * @since 5.0
   */
  ...
  /**
   * Parse the first header value for the given header name as a date, return -1 if there is no
   * value, or raise {@link IllegalArgumentException} if the value cannot be parsed as a date.
   *
   * @param headerName the header name
  ...
  /**
   * Parse the first header value for the given header name as a date, return -1 if there is no
   * value or also in case of an invalid value (if {@code rejectInvalid=false}), or raise {@link
   * IllegalArgumentException} if the value cannot be parsed as a date.
   *
   * @param headerName the header name
   * @param rejectInvalid whether to reject invalid values with an {@link IllegalArgumentException}
   *     ({@code true}) or rather return -1 in that case ({@code false})
  ...
  /**
   * Parse the first header value for the given header name as a date, return {@code null} if there
   * is no value, or raise {@link IllegalArgumentException} if the value cannot be parsed as a date.
   *
   * @param headerName the header name
  ...
  /**
   * Parse the first header value for the given header name as a date, return {@code null} if there
   * is no value or also in case of an invalid value (if {@code rejectInvalid=false}), or raise
   * {@link IllegalArgumentException} if the value cannot be parsed as a date.
   *
   * @param headerName the header name
   * @param rejectInvalid whether to reject invalid values with an {@link IllegalArgumentException}
   *     ({@code true}) or rather return {@code null} in that case ({@code false})
   ...
   * comma-separated values, can become confusing with regular {@link Map} formatting that also uses
   ...
   *
   * <p>The encoded credentials returned by this method can be supplied to {@link
   ...
   * @see <a href="https://tools.ietf.org/html/rfc7617">RFC 7617</a>
   */
  public static String encodeBasicAuth(
