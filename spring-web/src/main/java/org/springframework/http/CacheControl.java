 ...
 *
 * <ul>
 *   <li>Caching HTTP responses with {@code CacheControl cc = CacheControl.maxAge(1,
 *       TimeUnit.HOURS)} will result in {@code Cache-Control: "max-age=3600"}
 *   <li>Preventing cache with {@code CacheControl cc = CacheControl.noStore()} will result in
 *       {@code Cache-Control: "no-store"}
 *   <li>Advanced cases like {@code CacheControl cc = CacheControl.maxAge(1,
 *       TimeUnit.HOURS).noTransform().cachePublic()} will result in {@code Cache-Control:
 *       "max-age=3600, no-transform, public"}
 * </ul>
 *
 * <p>Note that to be efficient, Cache-Control headers should be written along HTTP validators such
 * as "Last-Modified" or "ETag" headers.
 *
 * @author Brian Clozel
...
public class CacheControl {
   ...
   *
   * <p>This is well suited for using other optional directives without "max-age", "no-cache" or
   * "no-store".
   *
   ...
   *
   * <p>This directive is well suited for publicly caching resources, knowing that they won't change
   * within the configured amount of time. Additional directives can be also used, in case resources
   * shouldn't be cached ({@link #cachePrivate()}) or transformed ({@link #noTransform()}) by shared
   * caches.
   *
   * <p>In order to prevent caches to reuse the cached response even when it has become stale (i.e.
   * the "max-age" delay is passed), the "must-revalidate" directive should be set ({@link
   * #mustRevalidate()}
   *
   ...
   *
   * <p>This directive is well suited for telling caches that the response can be reused only if the
   * client revalidates it with the server. This directive won't disable cache altogether and may
   * result with clients sending conditional requests (with "ETag", "If-Modified-Since" headers) and
   * the server responding with "304 - Not Modified" status.
   *
   * <p>In order to disable caching and minimize requests/responses exchanges, the {@link
   * #noStore()} directive should be used instead of {@code #noCache()}.
   *
   * @return {@code this}, to facilitate method chaining
   ...
   *
   * <p>This directive is well suited for preventing caches (browsers and proxies) to cache the
   ...
   *
   * <p>This directive indicates that once it has become stale, a cache MUST NOT use the response to
   * satisfy subsequent requests without successful validation on the origin server.
   *
   * @return {@code this}, to facilitate method chaining
   ...
   *
   * <p>This directive indicates that intermediaries (caches and others) should not transform the
   * response content. This can be useful to force caches and CDNs not to automatically gzip or
   ...
   *
   * <p>This directive indicates that any cache MAY store the response, even if the response would
   ...
   *
   * <p>This directive indicates that the response message is intended for a single user and MUST
   ...
   *
   * <p>This directive has the same meaning as the "must-revalidate" directive, except that it does
   * not apply to private caches (i.e. browsers, HTTP clients).
   *
   * @return {@code this}, to facilitate method chaining
   ...
   *
   * <p>This directive indicates that caches MAY serve the response in which it appears after it
   * becomes stale, up to the indicated number of seconds. If a cached response is served stale due
   * to the presence of this extension, the cache SHOULD attempt to revalidate it while still
   * serving stale responses (i.e. without blocking).
   *
   * @param staleWhileRevalidate the maximum time the response should be used while being
   *     revalidated
   ...
   *
   * <p>This directive indicates that when an error is encountered, a cached stale response MAY be
   * used to satisfy the request, regardless of other freshness information.
   *
  ...
  private String toHeaderValue() {
    ...
    if (this.staleWhileRevalidate != null) {
      appendDirective(
          headerValue, "stale-while-revalidate=" + this.staleWhileRevalidate.getSeconds());
    }
