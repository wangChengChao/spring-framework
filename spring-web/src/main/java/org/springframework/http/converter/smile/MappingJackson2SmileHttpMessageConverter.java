 ...
 * HttpMessageConverter} that can read and write Smile data format ("binary JSON") using <a
 * href="https://github.com/FasterXML/jackson-dataformats-binary/tree/master/smile">the dedicated
 * Jackson 2.x extension</a>.
 *
 * <p>By default, this converter supports {@code "application/x-jackson-smile"} media type. This can
 * be overridden by setting the {@link #setSupportedMediaTypes supportedMediaTypes} property.
 *
...
public class MappingJackson2SmileHttpMessageConverter extends AbstractJackson2HttpMessageConverter {
  ...
  /**
   * Construct a new {@code MappingJackson2SmileHttpMessageConverter} with a custom {@link
   * ObjectMapper} (must be configured with a {@code SmileFactory} instance). You can use {@link
   * Jackson2ObjectMapperBuilder} to build it easily.
   *
