 ...
 *
 * <p>This converter can read {@linkplain Collection collections} that contain classes annotated
 * with {@link XmlRootElement} and {@link XmlType}. Note that this converter does not support
 * writing.
 *
...
public class Jaxb2CollectionHttpMessageConverter<T extends Collection>
  ...
  /**
   * Always returns {@code false} since Jaxb2CollectionHttpMessageConverter required generic type
   * information in order to read a Collection.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>Jaxb2CollectionHttpMessageConverter can read a generic {@link Collection} where the generic
   * type is a JAXB type annotated with {@link XmlRootElement} or {@link XmlType}.
   */
  @Override
  public boolean canRead(
    ...
    return (typeArgumentClass.isAnnotationPresent(XmlRootElement.class)
            || typeArgumentClass.isAnnotationPresent(XmlType.class))
        && canRead(mediaType);
  }

  /**
   * Always returns {@code false} since Jaxb2CollectionHttpMessageConverter does not convert
   * collections to XML.
   */
  @Override
  ...
  public boolean canWrite(
      @Nullable Type type, @Nullable Class<?> clazz, @Nullable MediaType mediaType) {
    return false;
  }
  ...
  /**
   * Create an {@code XMLInputFactory} that this converter will use to create {@link
