 ...
 * HttpMessageConverter} that can read and write XML using Spring's {@link Marshaller} and {@link
 * Unmarshaller} abstractions.
 *
 * <p>This converter requires a {@code Marshaller} and {@code Unmarshaller} before it can be used.
 * These can be injected by the {@linkplain #MarshallingHttpMessageConverter(Marshaller)
 ...
 * overridden by setting the {@link #setSupportedMediaTypes(java.util.List) supportedMediaTypes}
 * property.
 *
...
public class MarshallingHttpMessageConverter extends AbstractXmlHttpMessageConverter<Object> {
  ...
  /**
   * Construct a new {@code MarshallingHttpMessageConverter} with no {@link Marshaller} or {@link
   * Unmarshaller} set. The Marshaller and Unmarshaller must be set after construction by invoking
   ...
   *
   * <p>Note that all {@code Marshaller} implementations in Spring also implement the {@code
  ...
  /**
   * Construct a new {@code MarshallingMessageConverter} with the given {@code Marshaller} and
   * {@code Unmarshaller}.
   *
