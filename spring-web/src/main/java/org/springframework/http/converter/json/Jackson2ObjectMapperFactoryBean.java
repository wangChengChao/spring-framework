...
/**
 * A {@link FactoryBean} for creating a Jackson 2.x {@link ObjectMapper} (default) or {@link
 * XmlMapper} ({@code createXmlMapper} property set to true) with setters to enable or disable
 * Jackson features from within XML configuration.
 *
 ...
 * </ul>
 *
 * <p>Example usage with {@link MappingJackson2HttpMessageConverter}:
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * <p>In case there are no specific setters provided (for some rarely used options), you can still
 * use the more general methods {@link #setFeaturesToEnable} and {@link #setFeaturesToDisable}.
 *
 * <pre class="code">
 * &lt;bean class="org.springframework.http.converter.json.Jackson2ObjectMapperFactoryBean">
 ...
 *       support for Java 7 types like {@link java.nio.file.Path}
 ...
 * </ul>
 *
 * <p>In case you want to configure Jackson's {@link ObjectMapper} with a custom {@link Module}, you
 * can register one or more such Modules by class name via {@link #setModulesToInstall}:
 *
 * <pre class="code">
 * &lt;bean class="org.springframework.http.converter.json.Jackson2ObjectMapperFactoryBean">
...
public class Jackson2ObjectMapperFactoryBean
  ...
  /**
   * Set the {@link ObjectMapper} instance to use. If not set, the {@link ObjectMapper} will be
  ...
  /**
   * If set to true and no custom {@link ObjectMapper} has been set, a {@link XmlMapper} will be
   * created using its default constructor.
   *
   * @since 4.1
   */
   ...
   * Spring either (see {@link #setFindModulesViaServiceLoader}). As a consequence, specifying an
   * empty list here will suppress any kind of module detection.
   *
  ...
  /**
   * Specify one or more modules by class (or class name in XML) to be registered with the {@link
   * ObjectMapper}.
   *
   * <p>Modules specified here will be registered after Spring's autodetection of JSR-310 and
   * Joda-Time, or Jackson's finding of modules (see {@link #setFindModulesViaServiceLoader}),
   ...
   * metadata in the classpath. Requires Jackson 2.2 or higher.
   *
   * <p>If this mode is not set, Spring's Jackson2ObjectMapperFactoryBean itself will try to find
   * the JSR-310 and Joda-Time support modules on the classpath - provided that Java 8 and Joda-Time
  ...
  /**
   * Set the builder {@link ApplicationContext} in order to autowire Jackson handlers ({@link
