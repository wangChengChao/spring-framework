...
public class GsonFactoryBean implements FactoryBean<Gson>, InitializingBean {
   ...
   *
   * <p>When set to {@code true}, a custom {@link com.google.gson.TypeAdapter} will be registered
   * via {@link GsonBuilder#registerTypeHierarchyAdapter(Class, Object)} which serializes a {@code
   * byte[]} property to and from a Base64-encoded String instead of a JSON array.
   *
  ...
  /**
   * Whether to use the {@link GsonBuilder#disableHtmlEscaping()} when writing JSON. Set to {@code
  ...
  /**
   * Define the date/time format with a {@link SimpleDateFormat}-style pattern. This is a shortcut
   * for setting up a {@code Gson} as follows:
   *
   * <pre class="code">
