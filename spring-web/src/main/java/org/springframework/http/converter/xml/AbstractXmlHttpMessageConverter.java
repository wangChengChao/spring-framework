 ...
 *
 * <p>By default, subclasses of this converter support {@code text/xml}, {@code application/xml},
 * and {@code application/*-xml}. This can be overridden by setting the {@link
...
public abstract class AbstractXmlHttpMessageConverter<T> extends AbstractHttpMessageConverter<T> {
   ...
   * supportedMediaTypes} to {@code text/xml} and {@code application/xml}, and {@code
   * application/*-xml}.
   */
  ...
  public final T readInternal(Class<? extends T> clazz, HttpInputMessage inputMessage)
    ...
    try {
      return readFromSource(
          clazz, inputMessage.getHeaders(), new StreamSource(inputMessage.getBody()));
    } catch (IOException | HttpMessageConversionException ex) {
      throw ex;
    } catch (Exception ex) {
      throw new HttpMessageNotReadableException(
          "Could not unmarshal to [" + clazz + "]: " + ex.getMessage(), ex, inputMessage);
    }
  }
  ...
  protected final void writeInternal(T t, HttpOutputMessage outputMessage)
    ...
    try {
      ...
      throw new HttpMessageNotWritableException(
          "Could not marshal [" + t + "]: " + ex.getMessage(), ex);
    }
  }
