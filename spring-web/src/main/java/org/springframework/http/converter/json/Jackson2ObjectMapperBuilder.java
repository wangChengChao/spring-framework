 ...
 * </ul>
 *
 * <p>It also automatically registers the following well-known modules if they are detected on the
 * classpath:
 *
 * <ul>
...
public class Jackson2ObjectMapperBuilder {
  ...
  private final Map<PropertyAccessor, JsonAutoDetect.Visibility> visibilities =
      new LinkedHashMap<>();
  ...
  @Nullable private JsonFactory factory;

  @Nullable private DateFormat dateFormat;

  @Nullable private Locale locale;

  @Nullable private TimeZone timeZone;

  @Nullable private AnnotationIntrospector annotationIntrospector;

  @Nullable private PropertyNamingStrategy propertyNamingStrategy;

  @Nullable private TypeResolverBuilder<?> defaultTyping;

  @Nullable private JsonInclude.Include serializationInclusion;

  @Nullable private FilterProvider filters;

  @Nullable private List<Module> modules;

  @Nullable private Class<? extends Module>[] moduleClasses;
  ...
  @Nullable private HandlerInstantiator handlerInstantiator;
  ...
  /**
   * If set to {@code true}, an {@link XmlMapper} will be created using its default constructor.
   * This is only applicable to {@link #build()} calls, not to {@link #configure} calls.
   */
  ...
  /**
   * Define the {@link JsonFactory} to be used to create the {@link ObjectMapper} instance.
   *
   * @since 5.0
   */
   ...
   *
   * <p>Note: Setting this property makes the exposed {@link ObjectMapper} non-thread-safe,
   * according to Jackson's thread safety rules.
   *
  ...
  /**
   * Override the default {@link TimeZone} to use for formatting. Default value used is UTC (NOT
   * local timezone).
   *
   * @since 4.1.5
   */
  ...
  /**
   * Specify a {@link com.fasterxml.jackson.databind.PropertyNamingStrategy} to configure the {@link
   * ObjectMapper} with.
   */
  public Jackson2ObjectMapperBuilder propertyNamingStrategy(
   ...
   */
  public Jackson2ObjectMapperBuilder serializationInclusion(
  ...
  /**
   * Set the global filters to use in order to support {@link JsonFilter @JsonFilter} annotated
   * POJO.
   *
   * @since 4.2
   ...
   * Add mix-in annotations to use for augmenting specified class or interface.
   *
   * @param mixIns a Map of entries with target classes (or interface) whose annotations to
   *     effectively override as key and mix-in classes (or interface) whose annotations are to be
   *     "added" to target's annotations as value.
   * @since 4.1.2
   * @see com.fasterxml.jackson.databind.ObjectMapper#addMixIn(Class, Class)
   */
   ...
   * @see #serializers(JsonSerializer...)
   */
  public Jackson2ObjectMapperBuilder serializersByType(
  ...
  /**
   * Configure custom deserializers. Each deserializer is registered for the type returned by {@link
   * JsonDeserializer#handledType()}, which must not be {@code null}.
   *
   * @since 4.3
  ...
  public Jackson2ObjectMapperBuilder deserializers(JsonDeserializer<?>... deserializers) {
    for (JsonDeserializer<?> deserializer : deserializers) {
      ...
      if (handledType == null || handledType == Object.class) {
        throw new IllegalArgumentException(
            "Unknown handled type in " + deserializer.getClass().getName());
      }
   ...
   * @since 4.1.2
   */
  public Jackson2ObjectMapperBuilder deserializerByType(
      Class<?> type, JsonDeserializer<?> deserializer) {
    this.deserializers.put(type, deserializer);
    return this;
  }

  /** Configure custom deserializers for the given types. */
  public Jackson2ObjectMapperBuilder deserializersByType(
  ...
  /**
   * Shortcut for {@link MapperFeature#AUTO_DETECT_SETTERS}/ {@link
   * MapperFeature#AUTO_DETECT_GETTERS}/{@link MapperFeature#AUTO_DETECT_IS_GETTERS} options.
   */
  ...
  /**
   * Define if a wrapper will be used for indexed (List, array) properties or not by default (only
   * applies to {@link XmlMapper}).
   *
   * @since 4.3
   */
   ...
   */
  public Jackson2ObjectMapperBuilder visibility(
      PropertyAccessor accessor, JsonAutoDetect.Visibility visibility) {
  ...
  /**
   * Specify one or more modules to be registered with the {@link ObjectMapper}. Multiple
   * invocations are not additive, the last one defines the modules to register.
   *
   * <p>Note: If this is set, no finding of modules is going to happen - not by Jackson, and not by
   * Spring either (see {@link #findModulesViaServiceLoader}). As a consequence, specifying an empty
   * list here will suppress any kind of module detection.
   *
  ...
  /**
   * Set a complete list of modules to be registered with the {@link ObjectMapper}. Multiple
   * invocations are not additive, the last one defines the modules to register.
   *
   * <p>Note: If this is set, no finding of modules is going to happen - not by Jackson, and not by
   * Spring either (see {@link #findModulesViaServiceLoader}). As a consequence, specifying an empty
   ...
   * invocations are not additive, the last one defines the modules to register.
   *
   * <p>Modules specified here will be registered after Spring's autodetection of JSR-310 and
   * Joda-Time, or Jackson's finding of modules (see {@link #findModulesViaServiceLoader}), allowing
   * to eventually override their configuration.
   *
  ...
  /**
   * Specify one or more modules by class to be registered with the {@link ObjectMapper}. Multiple
   * invocations are not additive, the last one defines the modules to register.
   *
   * <p>Modules specified here will be registered after Spring's autodetection of JSR-310 and
   * Joda-Time, or Jackson's finding of modules (see {@link #findModulesViaServiceLoader}), allowing
  ...
  /**
   * Set whether to let Jackson find available modules via the JDK ServiceLoader, based on META-INF
   * metadata in the classpath.
   *
   * <p>If this mode is not set, Spring's Jackson2ObjectMapperBuilder itself will try to find the
   * JSR-310 and Joda-Time support modules on the classpath - provided that Java 8 and Joda-Time
   * themselves are available, respectively.
   *
  ...
  /**
   * Customize the construction of Jackson handlers ({@link JsonSerializer}, {@link
   * JsonDeserializer}, {@link KeyDeserializer}, {@code TypeResolverBuilder} and {@code
   * TypeIdResolver}).
   *
   * @since 4.1.3
  ...
  /**
   * Set the Spring {@link ApplicationContext} in order to autowire Jackson handlers ({@link
   * JsonSerializer}, {@link JsonDeserializer}, {@link KeyDeserializer}, {@code TypeResolverBuilder}
   * and {@code TypeIdResolver}).
   *
   * @since 4.1.3
   ...
   *
   * <p>Each build operation produces an independent {@link ObjectMapper} instance. The builder's
   * settings can get modified, with a subsequent build operation then producing a new {@link
   * ObjectMapper} based on the most recent settings.
   *
  ...
  private void registerWellKnownModulesIfAvailable(
      MultiValueMap<Object, Module> modulesToRegister) {
    try {
      Class<? extends Module> jdk8ModuleClass =
    ...
    try {
      Class<? extends Module> javaTimeModuleClass =
          (Class<? extends Module>)
              ClassUtils.forName(
    ...
    if (ClassUtils.isPresent("org.joda.time.LocalDate", this.moduleClassLoader)) {
      try {
        Class<? extends Module> jodaModuleClass =
            (Class<? extends Module>)
                ClassUtils.forName(
                    "com.fasterxml.jackson.datatype.joda.JodaModule", this.moduleClassLoader);
    ...
    if (KotlinDetector.isKotlinPresent()) {
      try {
        Class<? extends Module> kotlinModuleClass =
            (Class<? extends Module>)
                ClassUtils.forName(
                    "com.fasterxml.jackson.module.kotlin.KotlinModule", this.moduleClassLoader);
  ...
  /**
   * Obtain a {@link Jackson2ObjectMapperBuilder} instance in order to build a regular JSON {@link
  ...
  /**
   * Obtain a {@link Jackson2ObjectMapperBuilder} instance in order to build an {@link XmlMapper}
   * instance.
   */
  ...
  /**
   * Obtain a {@link Jackson2ObjectMapperBuilder} instance in order to build a Smile data format
  ...
  /**
   * Obtain a {@link Jackson2ObjectMapperBuilder} instance in order to build a CBOR data format
   * {@link ObjectMapper} instance.
   *
   * @since 5.0
   */
