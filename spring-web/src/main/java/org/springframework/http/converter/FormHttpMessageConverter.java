...
/**
 * Implementation of {@link HttpMessageConverter} to read and write 'normal' HTML forms and also to
 * write (but not read) multipart data (e.g. file uploads).
 *
 * <p>In other words, this converter can read and write the {@code
 * "application/x-www-form-urlencoded"} media type as {@link MultiValueMap MultiValueMap&lt;String,
 * String&gt;}, and it can also write (but not read) the {@code "multipart/form-data"} and {@code
 * "multipart/mixed"} media types as {@link MultiValueMap MultiValueMap&lt;String, Object&gt;}.
 *
 * <h3>Multipart Data</h3>
 *
 * <p>By default, {@code "multipart/form-data"} is used as the content type when {@linkplain #write
 * writing} multipart data. As of Spring Framework 5.2 it is also possible to write multipart data
 * using other multipart subtypes such as {@code "multipart/mixed"} and {@code "multipart/related"},
 * as long as the multipart subtype is registered as a {@linkplain #getSupportedMediaTypes supported
 * media type} <em>and</em> the desired multipart subtype is specified as the content type when
 * {@linkplain #write writing} the multipart data. Note that {@code "multipart/mixed"} is registered
 * as a supported media type by default.
 *
 * <p>When writing multipart data, this converter uses other {@link HttpMessageConverter
 * HttpMessageConverters} to write the respective MIME parts. By default, basic converters are
 * registered for byte array, {@code String}, and {@code Resource}. These can be overridden via
 * {@link #setPartConverters} or augmented via {@link #addPartConverter}.
 *
 * <h3>Examples</h3>
 *
 * <p>The following snippet shows how to submit an HTML form using the {@code "multipart/form-data"}
 * content type.
 *
 * <pre class="code">
 ...
 *
 * <p>The following snippet shows how to do a file upload using the {@code "multipart/form-data"}
 * content type.
 *
 * <pre class="code">
 ...
 *
 * <p>The following snippet shows how to do a file upload using the {@code "multipart/mixed"}
 * content type.
 *
 * <pre class="code">
 * MultiValueMap&lt;String, Object&gt; parts = new LinkedMultiValueMap&lt;&gt;();
 ...
 *
 * <p>The following snippet shows how to do a file upload using the {@code "multipart/related"}
 * content type.
 *
 * <pre class="code">
 ...
 * <h3>Miscellaneous</h3>
 *
 * <p>Some methods in this class were inspired by {@code
...
public class FormHttpMessageConverter implements HttpMessageConverter<MultiValueMap<String, ?>> {
   ...
   *
   * <p>The supplied {@code MediaType} objects will be appended to the list of {@linkplain
   * #getSupportedMediaTypes() supported MediaType objects}.
   *
  ...
  public void addSupportedMediaTypes(MediaType... supportedMediaTypes) {
    ...
    Assert.noNullElements(
        supportedMediaTypes, "'supportedMediaTypes' must not contain null elements");
   ...
   * response {@code Content-Type} header does not explicitly specify it.
   *
   * <p>As of 4.3, this is also used as the default charset for the conversion of text bodies in a
   * multipart request.
   *
   * <p>As of 5.0, this is also used for part headers including {@code Content-Disposition} (and its
   * filename parameter) unless (the mutually exclusive) {@link #setMultipartCharset
   * multipartCharset} is also set, in which case part headers are encoded as ASCII and
   * <i>filename</i> is encoded with the {@code encoded-word} syntax from RFC 2047.
   *
   ...
   * based on the {@code encoded-word} syntax defined in RFC 2047 and relies on {@code MimeUtility}
   * from {@code javax.mail}.
   *
   * <p>As of 5.0 by default part headers, including {@code Content-Disposition} (and its filename
   * parameter) will be encoded based on the setting of {@link #setCharset(Charset)} or {@code
   * UTF-8} by default.
   *
   * @since 4.1.1
  ...
  public MultiValueMap<String, String> read(
      @Nullable Class<? extends MultiValueMap<String, ?>> clazz, HttpInputMessage inputMessage)
  ...
  /**
   * When {@link #setMultipartCharset(Charset)} is configured (i.e. RFC 2047, {@code encoded-word}
   * syntax) we need to use ASCII for part headers, or otherwise we encode directly using the
   * configured {@link #setCharset(Charset)}.
   */
    ...
    for (HttpMessageConverter<?> messageConverter : this.partConverters) {
      if (messageConverter.canWrite(partType, partContentType)) {
        ...
        ((HttpMessageConverter<Object>) messageConverter)
            .write(partBody, partContentType, multipartMessage);
        return;
      }
    }
  ...
  /**
   * Return the filename of the given multipart part. This value will be used for the {@code
   * Content-Disposition} header.
   *
   * <p>The default implementation returns {@link Resource#getFilename()} if the part is a {@code
   * Resource}, and {@code null} in other cases. Can be overridden in subclasses.
   *
