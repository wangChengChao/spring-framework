...
/**
 * Implementation of {@link HttpMessageConverter} that can read and write {@link BufferedImage
 * BufferedImages}.
 *
 * <p>By default, this converter can read all media types that are supported by the {@linkplain
 * ImageIO#getReaderMIMETypes() registered image readers}, and writes using the media type of the
 * first available {@linkplain javax.imageio.ImageIO#getWriterMIMETypes() registered image writer}.
 * The latter can be overridden by setting the {@link #setDefaultContentType defaultContentType}
 * property.
 *
 * <p>If the {@link #setCacheDir cacheDir} property is set, this converter will cache image data.
 *
 * <p>The {@link #process(ImageReadParam)} and {@link #process(ImageWriteParam)} template methods
 * allow subclasses to override Image I/O parameters.
 *
...
public class BufferedImageHttpMessageConverter implements HttpMessageConverter<BufferedImage> {
  ...
  @Nullable private File cacheDir;
   ...
   *
   * @throws IllegalArgumentException if the given content type is not supported by the Java Image
   *     I/O API
   */
  public void setDefaultContentType(@Nullable MediaType defaultContentType) {
    if (defaultContentType != null) {
      Iterator<ImageWriter> imageWriters =
          ImageIO.getImageWritersByMIMEType(defaultContentType.toString());
  ...
  /**
   * Returns the default {@code Content-Type} to be used for writing. Called when {@link #write} is
   * invoked without a specified content type parameter.
   */
  @Nullable
  ...
  /**
   * Sets the cache directory. If this property is set to an existing directory, this converter will
   * cache image data.
   */
  ...
  public BufferedImage read(
      @Nullable Class<? extends BufferedImage> clazz, HttpInputMessage inputMessage)
  ...
  public void write(
    ...
    if (outputMessage instanceof StreamingHttpOutputMessage) {
      ...
      streamingOutputMessage.setBody(
          outputStream -> writeInternal(image, selectedContentType, outputStream));
    } else {
  ...
  /**
   * Template method that allows for manipulating the {@link ImageReadParam} before it is used to
   * read an image.
   *
  ...
  /**
   * Template method that allows for manipulating the {@link ImageWriteParam} before it is used to
   * write an image.
   *
   * <p>The default implementation is empty.
   */
