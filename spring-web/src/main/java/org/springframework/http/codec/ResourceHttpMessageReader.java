...
/**
 * {@code HttpMessageReader} that wraps and delegates to a {@link ResourceDecoder} that extracts the
 * filename from the {@code "Content-Disposition"} header, if available, and passes it as the {@link
...
public class ResourceHttpMessageReader extends DecoderHttpMessageReader<Resource> {
  ...
  protected Map<String, Object> getReadHints(
    ...
    return StringUtils.hasText(name)
        ? Hints.from(ResourceDecoder.FILENAME_HINT, name)
        : Hints.none();
  }
}
