...
/**
 * Specialization of {@link Part} that represents an uploaded file received in a multipart request.
 *
...
public interface FilePart extends Part {
   ...
   * If the destination file already exists, it will be truncated first.
   *
   * <p>The default implementation delegates to {@link #transferTo(Path)}.
   *
   * @param dest the target file
   * @return completion {@code Mono} with the result of the file transfer, possibly {@link
   *     IllegalStateException} if the part isn't a file
   * @see #transferTo(Path)
   */
