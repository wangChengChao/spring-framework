...
/**
 * {@link HttpMessageWriter} for writing a {@code MultiValueMap<String, String>} as HTML form data,
 * i.e. {@code "application/x-www-form-urlencoded"}, to the body of a request.
 *
 * <p>Note that unless the media type is explicitly set to {@link
 * MediaType#APPLICATION_FORM_URLENCODED}, the {@link #canWrite} method will need generic type
 * information to confirm the target map has String values. This is because a MultiValueMap with
 * non-String values can be used to write multipart requests.
 *
 * <p>To support both form data and multipart requests, consider using {@link
 * org.springframework.http.codec.multipart.MultipartHttpMessageWriter} configured with this writer
 * as the fallback for writing plain form data.
 *
...
public class FormHttpMessageWriter extends LoggingCodecSupport
  ...
  /**
   * Set the default character set to use for writing form data when the response Content-Type
   * header does not explicitly specify it.
   *
  ...
  public Mono<Void> write(
      Publisher<? extends MultiValueMap<String, String>> inputStream,
    ...
    return Mono.from(inputStream)
        .flatMap(
            form -> {
              ...
              DataBuffer buffer =
                  message.bufferFactory().wrap(byteBuffer); // wrapping only, no allocation
