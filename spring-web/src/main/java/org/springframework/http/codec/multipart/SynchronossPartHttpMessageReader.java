...
/**
 * {@code HttpMessageReader} for parsing {@code "multipart/form-data"} requests to a stream of
 * {@link Part}'s using the Synchronoss NIO Multipart library.
 *
 * <p>This reader can be provided to {@link MultipartHttpMessageReader} in order to aggregate all
 * parts into a Map.
 *
 ...
 */
public class SynchronossPartHttpMessageReader extends LoggingCodecSupport
  ...
  public Flux<Part> read(
    ...
    return Flux.create(
            new SynchronossPartGenerator(message, this.bufferFactory, this.streamStorageFactory))
        .doOnNext(
            part -> {
  ...
  public Mono<Part> readMono(
      ResolvableType elementType, ReactiveHttpInputMessage message, Map<String, Object> hints) {
    return Mono.error(
        new UnsupportedOperationException("Cannot read multipart request body into single Part"));
  }

  /**
   * Consume and feed input to the Synchronoss parser, then listen for parser output events and
   * adapt to {@code Flux<Sink<Part>>}.
   */
  private static class SynchronossPartGenerator implements Consumer<FluxSink<Part>> {
    ...
    public void accept(FluxSink<Part> emitter) {
      ...
      NioMultipartParserListener listener =
          new FluxSinkAdapterListener(emitter, this.bufferFactory, context);
      NioMultipartParser parser =
          Multipart.multipart(context)
  ...
  private static class FluxSinkAdapterListener implements NioMultipartParserListener {
    ...
    FluxSinkAdapterListener(
        FluxSink<Part> sink, DataBufferFactory factory, MultipartContext context) {
      this.sink = sink;
  ...
  private static class SynchronossPart extends AbstractSynchronossPart {
    ...
    public Flux<DataBuffer> content() {
      return DataBufferUtils.readInputStream(
          getStorage()::getInputStream, getBufferFactory(), 4096);
    }
  ...
  private static class SynchronossFilePart extends SynchronossPart implements FilePart {
    ...
    SynchronossFilePart(
        HttpHeaders headers, String filename, StreamStorage storage, DataBufferFactory factory) {
  ...
  private static class SynchronossFormFieldPart extends AbstractSynchronossPart
