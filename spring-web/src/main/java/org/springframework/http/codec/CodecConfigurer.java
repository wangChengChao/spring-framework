...
/**
 * Defines a common interface for configuring either client or server HTTP message readers and
 * writers. This is used as follows:
 *
 * <ul>
 *   <li>Use {@link ClientCodecConfigurer#create()} or {@link ServerCodecConfigurer#create()} to
 *       create an instance.
 *   <li>Use {@link #defaultCodecs()} to customize HTTP message readers or writers registered by
 *       default.
 *   <li>Use {@link #customCodecs()} to add custom HTTP message readers or writers.
 *   <li>Use {@link #getReaders()} and {@link #getWriters()} to obtain the list of configured HTTP
 *       message readers and writers.
 * </ul>
 *
 * <p>HTTP message readers and writers are divided into 3 categories that are ordered as follows:
 *
 * <ol>
...
public interface CodecConfigurer {
  ...
  /**
   * Provides a way to completely turn off registration of default HTTP message readers and writers,
   * and instead rely only on the ones provided via {@link #customCodecs()}.
   *
   * <p>By default this is set to {@code "true"} in which case default registrations are made;
   * setting this to {@code false} disables default registrations.
   */
  ...
  /**
   * Customize or replace the HTTP message readers and writers registered by default. The options
   * are further extended by {@link ClientCodecConfigurer.ClientDefaultCodecs ClientDefaultCodecs}
   * and {@link ServerCodecConfigurer.ServerDefaultCodecs ServerDefaultCodecs}.
   */
  interface DefaultCodecs {
    ...
    /**
     * Whether to log form data at DEBUG level, and headers at TRACE level. Both may contain
  ...
  interface CustomCodecs {
    ...
    /**
     * Add a custom {@code Decoder} internally wrapped with {@link DecoderHttpMessageReader}).
     *
    ...
    /**
     * Add a custom {@code Encoder}, internally wrapped with {@link EncoderHttpMessageWriter}.
     *
     ...
     * consider using the shortcut {@link #decoder(Decoder)} instead.
     *
    ...
    /**
     * Add a custom {@link HttpMessageWriter}. For writers of type {@link EncoderHttpMessageWriter}
     * consider using the shortcut {@link #encoder(Encoder)} instead.
     *
