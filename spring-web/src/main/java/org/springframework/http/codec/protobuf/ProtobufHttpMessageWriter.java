...
/**
 * {@code HttpMessageWriter} that can write a protobuf {@link Message} and adds {@code
 * X-Protobuf-Schema}, {@code X-Protobuf-Message} headers and a {@code delimited=true} parameter is
 * added to the content type if a flux is serialized.
 *
 * <p>For {@code HttpMessageReader}, just use {@code new DecoderHttpMessageReader(new
 * ProtobufDecoder())}.
 *
...
public class ProtobufHttpMessageWriter extends EncoderHttpMessageWriter<Message> {
  ...
  public Mono<Void> write(
    ...
    try {
      ...
      if (inputStream instanceof Flux) {
        if (mediaType == null) {
          message
              .getHeaders()
              .setContentType(
                  ((HttpMessageEncoder<?>) getEncoder()).getStreamingMediaTypes().get(0));
        } else if (!ProtobufEncoder.DELIMITED_VALUE.equals(
            mediaType.getParameters().get(ProtobufEncoder.DELIMITED_KEY))) {
      ...
      return Mono.error(
          new DecodingException("Could not read Protobuf message: " + ex.getMessage(), ex));
    }
  }
