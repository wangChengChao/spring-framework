...
public class ReactorClientHttpConnector implements ClientHttpConnector {
   ...
   * Reactor Netty global resources. By default the client participates in the Reactor Netty global
   * resources held in {@link reactor.netty.http.HttpResources}, which is recommended since fixed,
   * shared resources are favored for event loop concurrency. However, consider declaring a {@link
   * ReactorResourceFactory} bean with {@code globalResources=true} in order to ensure the Reactor
  ...
  public Mono<ClientHttpResponse> connect(
    ...
    return this.httpClient
        ...
        .responseConnection(
            (res, con) -> Mono.just(adaptResponse(res, con.inbound(), con.outbound().alloc())))
        .next();
  }
  ...
  private ClientHttpResponse adaptResponse(
      HttpClientResponse response, NettyInbound nettyInbound, ByteBufAllocator allocator) {
