 ...
 * href="https://netty.io/">Netty 4</a> to create requests.
 *
 * <p>Allows to use a pre-configured {@link EventLoopGroup} instance: useful for sharing across
 * multiple clients.
 *
 * <p>Note that this implementation consistently closes the HTTP connection on each request.
 *
...
public class Netty4ClientHttpRequestFactory
  ...
  @Nullable private volatile Bootstrap bootstrap;
  ...
  /**
   * Create a new {@code Netty4ClientHttpRequestFactory} with the given {@link EventLoopGroup}.
   *
   * <p><b>NOTE:</b> the given group will <strong>not</strong> be {@linkplain
   * EventLoopGroup#shutdownGracefully() shutdown} by this factory; doing so becomes the
  ...
  /**
   * Set the SSL context. When configured it is used to create and insert an {@link
   * io.netty.handler.ssl.SslHandler} in the channel pipeline.
   *
  ...
  /**
   * Set the underlying connect timeout (in milliseconds). A timeout value of 0 specifies an
   * infinite timeout.
   *
            ...
            new ChannelInitializer<SocketChannel>() {
              ...
              protected void initChannel(SocketChannel channel) throws Exception {
                ...
                if (isSecure) {
                  ...
                  pipeline.addLast(
                      sslContext.newHandler(channel.alloc(), uri.getHost(), uri.getPort()));
                }
