...
/**
 * Builder for the body of a multipart request, producing {@code MultiValueMap<String, HttpEntity>},
 * which can be provided to the {@code WebClient} through the {@code body} method.
 *
 * <p>Examples:
 *
 * <pre class="code">
...
public final class MultipartBodyBuilder {
   ...
   *   <li>{@link HttpEntity} -- part content and headers although generally it's easier to add
   *       headers through the returned builder
  ...
  public PartBuilder part(String name, Object part, @Nullable MediaType contentType) {
    ...
    if (part instanceof PublisherEntity<?, ?>) {
      PublisherPartBuilder<?, ?> builder =
          new PublisherPartBuilder<>(name, (PublisherEntity<?, ?>) part);
      if (contentType != null) {
        builder.contentType(contentType);
      }
  ...
  public <T, P extends Publisher<T>> PartBuilder asyncPart(
    ...
    PublisherPartBuilder<T, P> builder =
        new PublisherPartBuilder<>(name, null, publisher, elementClass);
    this.parts.add(name, builder);
    return builder;
  }

  /**
   * Variant of {@link #asyncPart(String, Publisher, Class)} with a {@link
  ...
  public <T, P extends Publisher<T>> PartBuilder asyncPart(
    ...
    PublisherPartBuilder<T, P> builder =
        new PublisherPartBuilder<>(name, null, publisher, typeReference);
    this.parts.add(name, builder);
    return builder;
  }
  ...
  public interface PartBuilder {
    ...
    /**
     * Set the filename parameter for a file part. This should not be necessary with {@link
     * org.springframework.core.io.Resource Resource} based parts that expose a filename but may be
     * useful for {@link Publisher} parts.
     *
  ...
  private static class DefaultPartBuilder implements PartBuilder {
    ...
    @Nullable protected final Object body;
  ...
  private static class PublisherPartBuilder<S, P extends Publisher<S>> extends DefaultPartBuilder {
    ...
    public PublisherPartBuilder(
        String name, @Nullable HttpHeaders headers, P body, Class<S> elementClass) {
  ...
  /**
   * Specialization of {@link HttpEntity} for use with a {@link Publisher}-based body, for which we
   * also need to keep track of the element type.
   *
