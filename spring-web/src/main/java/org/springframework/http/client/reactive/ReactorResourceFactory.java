...
/**
 * Factory to manage Reactor Netty resources, i.e. {@link LoopResources} for event loop threads, and
 * {@link ConnectionProvider} for the connection pool, within the lifecycle of a Spring {@code
 * ApplicationContext}.
 *
 * <p>This factory implements {@link InitializingBean} and {@link DisposableBean} and is expected
 * typically to be declared as a Spring-managed bean.
 *
...
public class ReactorResourceFactory implements InitializingBean, DisposableBean {
  ...
  @Nullable private Consumer<HttpResources> globalResourcesConsumer;
   ...
   *
   * <p>Default is "true" in which case this factory initializes and stops the global Reactor Netty
   * resources within Spring's {@code ApplicationContext} lifecycle. If set to "false" the factory
   * manages its resources independent of the global ones.
   *
  ...
  /**
   * Whether this factory exposes the global {@link reactor.netty.http.HttpResources HttpResources}
   * holder.
   */
  ...
  /**
   * Add a Consumer for configuring the global Reactor Netty resources on startup. When this option
   * is used, {@link #setUseGlobalResources} is also enabled.
   *
  ...
  public void addGlobalResourcesConsumer(Consumer<HttpResources> consumer) {
    ...
    this.globalResourcesConsumer =
        this.globalResourcesConsumer != null
            ? this.globalResourcesConsumer.andThen(consumer)
            : consumer;
  }

  /**
   * Use this option when you don't want to participate in global resources and you want to
   * customize the creation of the managed {@code ConnectionProvider}.
   *
   * <p>By default, {@code ConnectionProvider.elastic("http")} is used.
   *
   * <p>Note that this option is ignored if {@code userGlobalResources=false} or {@link
   * #setConnectionProvider(ConnectionProvider)} is set.
   *
   ...
   *
   * <p>Note that this option is ignored if {@code userGlobalResources=false} or {@link
   * #setLoopResources(LoopResources)} is set.
   *
   * @param supplier the supplier to use
   */
  ...
  /**
   * Use this option when you want to provide an externally managed {@link ConnectionProvider}
   * instance.
   *
  ...
  /**
   * Use this option when you want to provide an externally managed {@link LoopResources} instance.
   *
  ...
  public void afterPropertiesSet() {
    if (this.useGlobalResources) {
      Assert.isTrue(
          this.loopResources == null && this.connectionProvider == null,
