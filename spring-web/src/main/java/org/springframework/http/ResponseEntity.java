...
/**
 * Extension of {@link HttpEntity} that adds a {@link HttpStatus} status code. Used in {@code
 * RestTemplate} as well {@code @Controller} methods.
 *
 * <p>In {@code RestTemplate}, this class is returned by {@link
 * org.springframework.web.client.RestTemplate#getForEntity getForEntity()} and {@link
 * org.springframework.web.client.RestTemplate#exchange exchange()}:
 *
 * <pre class="code">
...
public class ResponseEntity<T> extends HttpEntity<T> {
  ...
  public ResponseEntity(
      @Nullable T body, @Nullable MultiValueMap<String, String> headers, HttpStatus status) {
    super(body, headers);
  ...
  /**
   * Create a new {@code HttpEntity} with the given body, headers, and status code. Just used behind
   * the nested builder API.
   *
   * @param body the entity body
   * @param headers the entity headers
  ...
  private ResponseEntity(
      @Nullable T body, @Nullable MultiValueMap<String, String> headers, Object status) {
    super(body, headers);
    Assert.notNull(status, "HttpStatus must not be null");
    this.status = status;
  }
  ...
  /**
   * A shortcut for creating a {@code ResponseEntity} with the given body and the {@linkplain
   * HttpStatus#OK OK} status, or an empty body and a {@linkplain HttpStatus#NOT_FOUND NOT FOUND}
   * status in case of a {@linkplain Optional#empty()} parameter.
   *
  ...
  /**
   * A shortcut for creating a {@code ResponseEntity} with the given body and the status set to
  ...
  /**
   * Create a new builder with a {@linkplain HttpStatus#CREATED CREATED} status and a location
  ...
  /**
   * Create a builder with an {@linkplain HttpStatus#UNPROCESSABLE_ENTITY UNPROCESSABLE_ENTITY}
   * status.
   *
   * @return the created builder
   * @since 4.1.3
  ...
  public interface HeadersBuilder<B extends HeadersBuilder<B>> {
    ...
    /**
     * Manipulate this entity's headers with the given consumer. The headers provided to the
