...
/**
 * {@link org.aopalliance.intercept.MethodInterceptor} for accessing a Hessian service. Supports
 * authentication via username and password. The service URL must be an HTTP URL exposing a Hessian
 * service.
 *
 * <p>Hessian is a slim, binary RPC protocol. For information on Hessian, see the <a
 * href="http://hessian.caucho.com">Hessian website</a> <b>Note: As of Spring 4.0, this client
 * requires Hessian 4.0 or above.</b>
 *
 * <p>Note: There is no requirement for services accessed with this proxy factory to have been
 * exported using Spring's {@link HessianServiceExporter}, as there is no special handling involved.
 * As a consequence, you can also access services that have been exported using Caucho's {@link
...
public class HessianClientInterceptor extends UrlBasedRemoteAccessor implements MethodInterceptor {
  ...
  @Nullable private Object hessianProxy;

  /**
   * Set the HessianProxyFactory instance to use. If not specified, a default HessianProxyFactory
   * will be created.
   *
   * <p>Allows to use an externally configured factory instance, in particular a custom
   ...
   *
   * <p>This will typically be passed in as an inner bean definition of type {@code
    ...
    try {
      ...
      if (targetEx instanceof HessianConnectionException) {
        throw convertHessianAccessException(targetEx);
          ...
          || targetEx instanceof HessianRuntimeException) {
