 ...
 * {@link javax.jws.WebService} annotation) and exporting them through the HTTP server included in
 * Sun's JDK 1.6. The full address for each service will consist of the server's base address with
 * the service name appended (e.g. "http://localhost:8080/OrderService").
 *
 * <p>Note that this exporter will only work on Sun's JDK 1.6 or higher, as well as on JDKs that
 * ship Sun's entire class library as included in the Sun JDK. For a portable JAX-WS exporter, have
 * a look at {@link SimpleJaxWsServiceExporter}.
 *
...
public class SimpleHttpServerJaxWsServiceExporter extends AbstractJaxWsServiceExporter {
  ...
  @Nullable private List<Filter> filters;
  ...
  /**
   * Specify an existing HTTP server to register the web service contexts with. This will typically
   * be a server managed by the general Spring {@link
   * org.springframework.remoting.support.SimpleHttpServerFactoryBean}.
   *
   * <p>Alternatively, configure a local HTTP server through the {@link #setPort "port"}, {@link
   * #setHostname "hostname"} and {@link #setBacklog "backlog"} properties (or rely on the defaults
   * there).
   */
   ...
   *
   * <p>Only applicable for a locally configured HTTP server. Ignored when the {@link #setServer
  ...
  /**
   * Specify the HTTP server's hostname to bind to. Default is localhost; can be overridden with a
   * specific network address to bind to.
   *
   * <p>Only applicable for a locally configured HTTP server. Ignored when the {@link #setServer
   * "server"} property has been specified.
   */
  ...
  /**
   * Specify the number of seconds to wait until HTTP exchanges have completed when shutting down
   * the HTTP server. Default is 0.
   *
   * <p>Only applicable for a locally configured HTTP server. Ignored when the {@link #setServer
   * "server"} property has been specified.
   */
   ...
   *
   * <p>For each context publication path, the service name will be appended to this base address.
   * E.g. service name "OrderService" -> "/OrderService".
   *
   * @see javax.xml.ws.Endpoint#publish(Object)
  ...
  /**
   * Register common {@link com.sun.net.httpserver.Filter Filters} to be applied to all detected
   * {@link javax.jws.WebService} annotated beans.
   */
  ...
  /**
   * Register a common {@link com.sun.net.httpserver.Authenticator} to be applied to all detected
   * {@link javax.jws.WebService} annotated beans.
   */
