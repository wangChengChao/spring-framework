 ...
 * {@link javax.jws.WebService} annotation) and exporting them with a configured base address (by
 * default "http://localhost:8080/") using the JAX-WS provider's built-in publication support. The
 * full address for each service will consist of the base address with the service name appended
 * (e.g. "http://localhost:8080/OrderService").
 *
 * <p>Note that this exporter will only work if the JAX-WS runtime actually supports publishing with
...
public class SimpleJaxWsServiceExporter extends AbstractJaxWsServiceExporter {
   ...
   *
   * <p>For each actual publication address, the service name will be appended to this base address.
   * E.g. service name "OrderService" -> "http://localhost:8080/OrderService".
   *
   * @see javax.xml.ws.Endpoint#publish(String)
