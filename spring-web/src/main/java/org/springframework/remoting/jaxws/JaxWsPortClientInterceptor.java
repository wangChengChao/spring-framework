...
/**
 * {@link org.aopalliance.intercept.MethodInterceptor} for accessing a specific port of a JAX-WS
 * service.
 *
 * <p>Uses either {@link LocalJaxWsServiceFactory}'s facilities underneath, or takes an explicit
 * reference to an existing JAX-WS Service instance (e.g. obtained via {@link
...
public class JaxWsPortClientInterceptor extends LocalJaxWsServiceFactory
  ...
  @Nullable private Service jaxWsService;

  @Nullable private String portName;

  @Nullable private String username;

  @Nullable private String password;

  @Nullable private String endpointAddress;

  private boolean maintainSession;

  private boolean useSoapAction;

  @Nullable private String soapActionUri;

  @Nullable private Map<String, Object> customProperties;

  @Nullable private WebServiceFeature[] portFeatures;

  @Nullable private Class<?> serviceInterface;

  private boolean lookupServiceOnStartup = true;

  @Nullable private ClassLoader beanClassLoader = ClassUtils.getDefaultClassLoader();

  @Nullable private QName portQName;

  @Nullable private Object portStub;

  private final Object preparationMonitor = new Object();

  /**
   * Set a reference to an existing JAX-WS Service instance, for example obtained via {@link
   * org.springframework.jndi.JndiObjectFactoryBean}. If not set, {@link LocalJaxWsServiceFactory}'s
  ...
  /**
   * Allow Map access to the custom properties to be set on the stub, with the option to add or
   * override specific entries.
   *
   * <p>Useful for specifying entries directly, for example via "customProperties[myKey]". This is
  ...
  /**
   * Specify WebServiceFeature objects (e.g. as inner bean definitions) to apply to JAX-WS port stub
   * creation.
   *
   * @since 4.0
   ...
   *
   * <p>Default is "true". Turn this flag off to allow for late start of the target server. In this
   * case, the JAX-WS service will be lazily fetched on first access.
   */
  ...
  /**
   * Set the bean ClassLoader to use for this interceptor: primarily for building a client proxy in
   * the {@link JaxWsPortProxyFactoryBean} subclass.
   */
  @Override
  ...
  /**
   * Initialize this client interceptor's properties from the given WebService annotation, if
   * necessary and possible (i.e. if "wsdlDocumentUrl", "namespaceUri", "serviceName" and "portName"
   * haven't been set but corresponding values are declared at the annotation level of the specified
   * service interface).
   *
  ...
  protected Object getPortStub(Service service, @Nullable QName portQName) {
    if (this.portFeatures != null) {
      return (portQName != null
          ? service.getPort(portQName, getServiceInterface(), this.portFeatures)
          : service.getPort(getServiceInterface(), this.portFeatures));
    } else {
      return (portQName != null
          ? service.getPort(portQName, getServiceInterface())
  ...
  /**
   * Return the underlying JAX-WS port stub that this interceptor delegates to for each method
   ...
   */
  @Nullable
  protected Object doInvoke(MethodInvocation invocation, @Nullable Object portStub)
      throws Throwable {
