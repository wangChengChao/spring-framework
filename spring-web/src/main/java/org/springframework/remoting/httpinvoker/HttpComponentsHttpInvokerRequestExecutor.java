...
/**
 * {@link org.springframework.remoting.httpinvoker.HttpInvokerRequestExecutor} implementation that
 * uses <a href="https://hc.apache.org/httpcomponents-client-ga/httpclient/">Apache HttpComponents
 * HttpClient</a> to execute POST requests.
 *
 * <p>Allows to use a pre-configured {@link org.apache.http.client.HttpClient} instance, potentially
 * with authentication, HTTP connection pooling, etc. Also designed for easy subclassing, providing
...
public class HttpComponentsHttpInvokerRequestExecutor extends AbstractHttpInvokerRequestExecutor {
  ...
  /**
   * Create a new instance of the HttpComponentsClientHttpRequestFactory with the given {@link
   * HttpClient} instance.
   *
  ...
  private HttpComponentsHttpInvokerRequestExecutor(
      HttpClient httpClient, @Nullable RequestConfig requestConfig) {
  ...
  /**
   * Set the connection timeout for the underlying HttpClient. A timeout value of 0 specifies an
   * infinite timeout.
   *
   ...
   * using the underlying HttpClient. A timeout value of 0 specifies an infinite timeout.
   *
   * <p>Additional properties can be configured by specifying a {@link RequestConfig} instance on a
   * custom {@link HttpClient}.
   *
  ...
  public void setConnectionRequestTimeout(int connectionRequestTimeout) {
    this.requestConfig =
        cloneRequestConfig().setConnectionRequestTimeout(connectionRequestTimeout).build();
  }

  /**
   * Set the socket read timeout for the underlying HttpClient. A timeout value of 0 specifies an
   * infinite timeout.
   *
   * <p>Additional properties can be configured by specifying a {@link RequestConfig} instance on a
   * custom {@link HttpClient}.
   ...
   *
   * <p>This method implements the basic processing workflow: The actual work happens in this
   * class's template methods.
   *
   * @see #createHttpPost
   * @see #setRequestBody
   * @see #executeHttpPost
  ...
  /**
   * Create a {@link RequestConfig} for the given configuration. Can return {@code null} to indicate
   * that no custom request config should be set and the defaults of the {@link HttpClient} should
   * be used.
   *
   ...
   *
   * <p>Default implementation rejects any HTTP status code beyond 2xx, to avoid parsing the
   * response body and trying to deserialize from a corrupted stream.
   *
   * @param config the HTTP invoker configuration that specifies the target service
   ...
   *
   * <p>The default implementation simply fetches the HttpPost's response body stream. If the
   * response is recognized as GZIP response, the InputStream will get wrapped in a GZIPInputStream.
   *
   * @param config the HTTP invoker configuration that specifies the target service
  ...
  protected InputStream getResponseBody(
      HttpInvokerClientConfiguration config, HttpResponse httpResponse) throws IOException {
   ...
   *
   * <p>The default implementation checks whether the HTTP "Content-Encoding" header contains "gzip"
   * (in any casing).
   *
