 ...
 * and a {@linkplain #getBean() bean}. Provides convenient access to method parameters, the method
 * return value, method annotations, etc.
 *
 * <p>The class may be created with a bean instance or with a bean name (e.g. lazy-init bean,
 * prototype bean). Use {@link #createWithResolvedBean()} to obtain a {@code HandlerMethod} instance
...
public class HandlerMethod {
  ...
  @Nullable private HttpStatus responseStatus;
  ...
  /**
   * Create an instance from a bean name, a method, and a {@code BeanFactory}. The method {@link
   * #createWithResolvedBean()} may be used later to re-create the {@code HandlerMethod} with an
   * initialized bean.
   */
   ...
   *
   * <p>Note that if the bean type is a CGLIB-generated class, the original user-defined class is
   * returned.
   */
  ...
  /**
   * Return a single annotation on the underlying method traversing its super methods if no
   * annotation can be found on the given method itself.
   *
   * <p>Also supports <em>merged</em> composed annotations with attribute overrides as of Spring
   * Framework 4.2.2.
   *
  ...
  /**
   * Return the HandlerMethod from which this HandlerMethod instance was resolved via {@link
  ...
  /**
   * If the provided instance contains a bean name rather than an object instance, the bean name is
   * resolved before a {@link HandlerMethod} is created and returned.
   */
  ...
  protected static String formatArgumentError(MethodParameter param, String message) {
    return "Could not resolve parameter ["
   ...
   * declared. In some cases the actual controller instance at request- processing time may be a JDK
   * dynamic proxy (lazy initialization, prototype beans, and others). {@code @Controller}'s that
   * require proxying should prefer class-based proxy mechanisms.
   */
  protected void assertTargetBean(Method method, Object targetBean, Object[] args) {
    ...
    if (!methodDeclaringClass.isAssignableFrom(targetBeanClass)) {
      String text =
          "The mapped handler method class '"
