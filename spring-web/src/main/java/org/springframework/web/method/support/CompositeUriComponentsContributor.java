...
/**
 * A {@link UriComponentsContributor} containing a list of other contributors to delegate and also
 * encapsulating a specific {@link ConversionService} to use for formatting method argument values
 * to Strings.
 *
...
public class CompositeUriComponentsContributor implements UriComponentsContributor {
   ...
   * HandlerMethodArgumentResolvers}. Since both of these tend to be implemented by the same class,
   * the most convenient option is to obtain the configured {@code HandlerMethodArgumentResolvers}
   * in {@code RequestMappingHandlerAdapter} and provide that to this constructor.
   *
   * @param contributors a collection of {@link UriComponentsContributor} or {@link
   ...
   * HandlerMethodArgumentResolvers}. Since both of these tend to be implemented by the same class,
   * the most convenient option is to obtain the configured {@code HandlerMethodArgumentResolvers}
   * in the {@code RequestMappingHandlerAdapter} and provide that to this constructor.
   *
   * <p>If the {@link ConversionService} argument is {@code null}, {@link
   * org.springframework.format.support.DefaultFormattingConversionService} will be used by default.
   *
   * @param contributors a collection of {@link UriComponentsContributor} or {@link
   *     HandlerMethodArgumentResolver HandlerMethodArgumentResolvers}.
   * @param cs a ConversionService to use when method argument values need to be formatted as
   *     Strings before being added to the URI
   */
  public CompositeUriComponentsContributor(
      @Nullable Collection<?> contributors, @Nullable ConversionService cs) {
