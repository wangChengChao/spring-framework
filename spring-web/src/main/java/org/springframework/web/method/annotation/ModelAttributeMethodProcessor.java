 ...
 * added to the model). Once created the attribute is populated via data binding to Servlet request
 * parameters. Validation may be applied if the argument is annotated with
 * {@code @javax.validation.Valid}. or Spring's own
 * {@code @org.springframework.validation.annotation.Validated}.
 *
 * <p>When this handler is created with {@code annotationNotRequired=true} any non-simple type
 * argument and return value is regarded as a model attribute with or without the presence of an
 * {@code @ModelAttribute}.
 *
...
public class ModelAttributeMethodProcessor
  ...
  /**
   * Returns {@code true} if the parameter is annotated with {@link ModelAttribute} or, if in
   * default resolution mode, for any method parameter that is not a simple type.
   */
  @Override
  ...
  public final Object resolveArgument(
    ...
    Assert.state(
        mavContainer != null, "ModelAttributeMethodProcessor requires ModelAndViewContainer");
    Assert.state(
        binderFactory != null, "ModelAttributeMethodProcessor requires WebDataBinderFactory");
   ...
   *
   * <p>The default implementation typically uses the unique public no-arg constructor if available
   * but also handles a "primary constructor" approach for data classes: It understands the
   * JavaBeans {@link ConstructorProperties} annotation as well as runtime-retained parameter names
   * in the bytecode, associating request parameters with constructor arguments by name. If no such
   * constructor is found, the default constructor will be used (even if not public), assuming
   ...
   *
   * <p>Called from {@link #createAttribute(String, MethodParameter, WebDataBinderFactory,
   * NativeWebRequest)} after constructor resolution.
   *
  ...
  protected Object constructAttribute(
    ...
    Assert.state(
        paramNames != null, () -> "Cannot resolve parameter names for constructor " + ctor);
   ...
   *
   * @since 5.0
   * @deprecated as of 5.1, in favor of {@link #constructAttribute(Constructor, String,
   *     MethodParameter, WebDataBinderFactory, NativeWebRequest)}
   */
  @Deprecated
  @Nullable
  protected Object constructAttribute(
   ...
   *
   * <p>The default implementation checks for {@code @javax.validation.Valid}, Spring's {@link
   * org.springframework.validation.annotation.Validated}, and custom annotations whose name starts
   * with "Valid".
   *
   ...
   */
  protected void validateValueIfApplicable(
          ...
          if (validator instanceof SmartValidator) {
            try {
              ((SmartValidator) validator)
                  .validateValue(
                      targetType, fieldName, value, binder.getBindingResult(), validationHints);
  ...
  /**
   * Return {@code true} if there is a method-level {@code @ModelAttribute} or, in default
   * resolution mode, for any return value type that is not a simple type.
   */
  @Override
  public boolean supportsReturnType(MethodParameter returnType) {
    return (returnType.hasMethodAnnotation(ModelAttribute.class)
        || (this.annotationNotRequired
            && !BeanUtils.isSimpleProperty(returnType.getParameterType())));
  }
  ...
  private static class FieldAwareConstructorParameter extends MethodParameter {
    ...
    public FieldAwareConstructorParameter(
        Constructor<?> constructor, int parameterIndex, String parameterName) {
