...
/**
 * Encapsulates information about an {@link ControllerAdvice @ControllerAdvice} Spring-managed bean
 * without necessarily requiring it to be instantiated.
 *
 * <p>The {@link #findAnnotatedBeans(ApplicationContext)} method can be used to discover such beans.
 * However, a {@code ControllerAdviceBean} may be created from any object, including ones without an
 * {@code @ControllerAdvice} annotation.
 *
...
public class ControllerAdviceBean implements Ordered {
  ...
  /** Reference to the actual bean instance or a {@code String} representing the bean name. */
  private final Object beanOrName;

  /**
   * Reference to the resolved bean instance, potentially lazily retrieved via the {@code
   * BeanFactory}.
   */
  ...
  /**
   * Create a {@code ControllerAdviceBean} using the given bean name and {@code BeanFactory}.
   *
   * @param beanName the name of the bean
   * @param beanFactory a {@code BeanFactory} to retrieve the bean type initially and later to
   *     resolve the actual bean
   */
  ...
  /**
   * Create a {@code ControllerAdviceBean} using the given bean name, {@code BeanFactory}, and
   * {@link ControllerAdvice @ControllerAdvice} annotation.
   *
   * @param beanName the name of the bean
   * @param beanFactory a {@code BeanFactory} to retrieve the bean type initially and later to
   *     resolve the actual bean
   * @param controllerAdvice the {@code @ControllerAdvice} annotation for the bean, or {@code null}
  ...
  public ControllerAdviceBean(
    ...
    Assert.isTrue(
        ...
        () ->
            "BeanFactory ["
                + beanFactory
                + "] does not contain specified controller advice bean '"
                + beanName
                + "'");
   ...
   *
   * <p>As of Spring Framework 5.2, the order value is lazily retrieved using the following
   * algorithm and cached.
   *
   * <ul>
   *   <li>If the {@linkplain #resolveBean resolved bean} implements {@link Ordered}, use the value
   *       returned by {@link Ordered#getOrder()}.
   *   <li>Otherwise use the value returned by {@link OrderUtils#getOrder(Class, int)} with {@link
   *       Ordered#LOWEST_PRECEDENCE} used as the default order value.
   * </ul>
   *
   * @see #resolveBean()
   */
   ...
   *
   * <p>If the bean type is a CGLIB-generated class, the original user-defined class is returned.
   */
  @Nullable
  ...
  /**
   * Get the bean instance for this {@code ControllerAdviceBean}, if necessary resolving the bean
   * name through the {@link BeanFactory}.
   *
   * <p>As of Spring Framework 5.2, once the bean instance has been resolved it will be cached,
   * thereby avoiding repeated lookups in the {@code BeanFactory}.
   */
  ...
  /**
   * Check whether the given bean type should be advised by this {@code ControllerAdviceBean}.
   *
  ...
  public boolean equals(@Nullable Object other) {
    ...
    return (this.beanOrName.equals(otherAdvice.beanOrName)
        && this.beanFactory == otherAdvice.beanFactory);
  }
  ...
  /**
   * Find beans annotated with {@link ControllerAdvice @ControllerAdvice} in the given {@link
   * ApplicationContext} and wrap them as {@code ControllerAdviceBean} instances.
   *
   * <p>As of Spring Framework 5.2, the {@code ControllerAdviceBean} instances in the returned list
  ...
  public static List<ControllerAdviceBean> findAnnotatedBeans(ApplicationContext context) {
    ...
    for (String name : BeanFactoryUtils.beanNamesForTypeIncludingAncestors(context, Object.class)) {
      ControllerAdvice controllerAdvice =
          context.findAnnotationOnBean(name, ControllerAdvice.class);
