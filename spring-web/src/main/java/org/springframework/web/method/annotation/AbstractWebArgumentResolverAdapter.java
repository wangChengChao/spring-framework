 ...
 * recommended to re-write a {@code WebArgumentResolver} as {@code HandlerMethodArgumentResolver}.
 * Since {@link #supportsParameter} can only be implemented by actually resolving the value and then
 * checking the result is not {@code WebArgumentResolver#UNRESOLVED} any exceptions raised must be
 * absorbed and ignored since it's not clear whether the adapter doesn't support the parameter or
 * whether it failed for an internal reason. The {@code HandlerMethodArgumentResolver} contract also
 * provides access to model attributes and to {@code WebDataBinderFactory} (for type conversion).
 *
...
public abstract class AbstractWebArgumentResolverAdapter implements HandlerMethodArgumentResolver {
  ...
  /**
   * Actually resolve the value and check the resolved value is not {@link
  ...
  public boolean supportsParameter(MethodParameter parameter) {
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Error in checking support for parameter [" + parameter + "]: " + ex.getMessage());
      }
      return false;
    }
  }
   ...
   *
   * @throws IllegalStateException if the resolved value is not assignable to the method parameter.
   */
  @Override
  @Nullable
  public Object resolveArgument(
    ...
    Object result = this.adaptee.resolveArgument(parameter, webRequest);
    if (result == WebArgumentResolver.UNRESOLVED
        || !ClassUtils.isAssignableValue(paramType, result)) {
