...
public class CookieWebSessionIdResolver implements WebSessionIdResolver {
  ...
  @Nullable private Consumer<ResponseCookie.ResponseCookieBuilder> cookieInitializer = null;
  ...
  /**
   * Set the value for the "Max-Age" attribute of the cookie that holds the session id. For the
   * range of values see {@link ResponseCookie#getMaxAge()}.
   *
  ...
  /**
   * Add a {@link Consumer} for a {@code ResponseCookieBuilder} that will be invoked for each cookie
   * being built, just before the call to {@code build()}.
   *
  ...
  public void addCookieInitializer(Consumer<ResponseCookie.ResponseCookieBuilder> initializer) {
    this.cookieInitializer =
        this.cookieInitializer != null ? this.cookieInitializer.andThen(initializer) : initializer;
  }
  ...
  private ResponseCookie initSessionCookie(ServerWebExchange exchange, String id, Duration maxAge) {
    ...
    ResponseCookie.ResponseCookieBuilder cookieBuilder =
        ResponseCookie.from(this.cookieName, id)
