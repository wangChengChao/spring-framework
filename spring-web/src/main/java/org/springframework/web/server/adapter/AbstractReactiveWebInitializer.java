...
/**
 * Base class for a {@link org.springframework.web.WebApplicationInitializer} that installs a Spring
 * Reactive Web Application on a Servlet container.
 *
 * <p>Spring configuration is loaded and given to {@link WebHttpHandlerBuilder#applicationContext
 * WebHttpHandlerBuilder} which scans the context looking for specific beans and creates a reactive
 * {@link HttpHandler}. The resulting handler is installed as a Servlet through the {@link
...
public abstract class AbstractReactiveWebInitializer implements WebApplicationInitializer {
   ...
   * {@link org.springframework.stereotype.Component @Component} classes that make up the
   * application configuration. The config classes are given to {@linkplain
  ...
  /**
   * Register a {@link ServletContextListener} that closes the given application context when the
   * servlet context is destroyed.
   *
   * @param servletContext the servlet context to listen to
   * @param applicationContext the application context that is to be closed when {@code
  ...
  /**
   * Return the Servlet mapping to use. Only the default Servlet mapping '/' and path-based Servlet
   * mappings such as '/api/*' are supported.
   *
