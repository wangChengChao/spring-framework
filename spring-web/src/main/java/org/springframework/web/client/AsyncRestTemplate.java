...
/**
 * <strong>Spring's central class for asynchronous client-side HTTP access.</strong> Exposes similar
 * methods as {@link RestTemplate}, but returns {@link ListenableFuture} wrappers as opposed to
 * concrete results.
 *
 * <p>The {@code AsyncRestTemplate} exposes a synchronous {@link RestTemplate} via the {@link
 * #getRestOperations()} method and shares its {@linkplain #setErrorHandler error handler} and
 * {@linkplain #setMessageConverters message converters} with that {@code RestTemplate}.
 *
 * <p><strong>Note:</strong> by default {@code AsyncRestTemplate} relies on standard JDK facilities
 * to establish HTTP connections. You can switch to use a different HTTP library such as Apache
 * HttpComponents, Netty, and OkHttp by using a constructor accepting an {@link
...
public class AsyncRestTemplate
    extends org.springframework.http.client.support.InterceptingAsyncHttpAccessor
   ...
   *
   * <p>This constructor uses a {@link SimpleClientHttpRequestFactory} in combination with a {@link
   * SimpleAsyncTaskExecutor} for asynchronous execution.
   */
   ...
   * org.springframework.http.client.AsyncClientHttpRequestFactory}.
   *
   * <p>This constructor will cast the given asynchronous {@code AsyncClientHttpRequestFactory} to a
   * {@link ClientHttpRequestFactory}. Since all implementations of {@code ClientHttpRequestFactory}
   * provided in Spring also implement {@code AsyncClientHttpRequestFactory}, this should not result
   * in a {@code ClassCastException}.
   */
  public AsyncRestTemplate(
  ...
  /**
   * Creates a new instance of the {@code AsyncRestTemplate} using the given asynchronous and
   * synchronous request factories.
   *
  ...
  /**
   * Create a new instance of the {@code AsyncRestTemplate} using the given {@link
   * org.springframework.http.client.AsyncClientHttpRequestFactory} and synchronous {@link
   * RestTemplate}.
   *
   ...
   */
  public AsyncRestTemplate(
      org.springframework.http.client.AsyncClientHttpRequestFactory requestFactory,
   ...
   *
   * <p>By default, AsyncRestTemplate uses a {@link
  ...
  /**
   * This property has the same purpose as the corresponding property on the {@code RestTemplate}.
   * For more details see {@link RestTemplate#setUriTemplateHandler}.
   *
  ...
  @Override
  public <T> ListenableFuture<ResponseEntity<T>> getForEntity(
      String url, Class<T> responseType, Object... uriVariables) throws RestClientException {
  ...
  @Override
  public <T> ListenableFuture<ResponseEntity<T>> getForEntity(
      String url, Class<T> responseType, Map<String, ?> uriVariables) throws RestClientException {
  ...
  @Override
  public ListenableFuture<URI> postForLocation(
      String url, @Nullable HttpEntity<?> request, Object... uriVars) throws RestClientException {

    AsyncRequestCallback callback = httpEntityCallback(request);
    ResponseExtractor<HttpHeaders> extractor = headersExtractor();
    ListenableFuture<HttpHeaders> future =
        execute(url, HttpMethod.POST, callback, extractor, uriVars);
    return adaptToLocationHeader(future);
  }

  @Override
  public ListenableFuture<URI> postForLocation(
      String url, @Nullable HttpEntity<?> request, Map<String, ?> uriVars)
      throws RestClientException {
  ...
  @Override
  public <T> ListenableFuture<ResponseEntity<T>> postForEntity(
      String url, @Nullable HttpEntity<?> request, Class<T> responseType, Object... uriVariables)
      throws RestClientException {
  ...
  @Override
  public <T> ListenableFuture<ResponseEntity<T>> postForEntity(
      URI url, @Nullable HttpEntity<?> request, Class<T> responseType) throws RestClientException {
  ...
  @Override
  public ListenableFuture<?> put(URI url, @Nullable HttpEntity<?> request)
      throws RestClientException {
    AsyncRequestCallback requestCallback = httpEntityCallback(request);
  ...
  public ListenableFuture<Set<HttpMethod>> optionsForAllow(String url, Object... uriVars)
    ...
    ResponseExtractor<HttpHeaders> extractor = headersExtractor();
    ListenableFuture<HttpHeaders> future =
        execute(url, HttpMethod.OPTIONS, null, extractor, uriVars);
  ...
  private static ListenableFuture<Set<HttpMethod>> adaptToAllowHeader(
      ListenableFuture<HttpHeaders> future) {
  ...
  @Override
  public <T> ListenableFuture<ResponseEntity<T>> exchange(
      String url,
      HttpMethod method,
  ...
  /**
   * Execute the given method on the provided URI. The {@link
   * org.springframework.http.client.ClientHttpRequest} is processed using the {@link
   * RequestCallback}; the response with the {@link ResponseExtractor}.
   *
   * @param url the fully-expanded URL to connect to
   * @param method the HTTP method to execute (GET, POST, etc.)
   * @param requestCallback object that prepares the request (can be {@code null})
   * @param responseExtractor object that extracts the return value from the response (can be {@code
   *     null})
   * @return an arbitrary object, as returned by the {@link ResponseExtractor}
   */
  protected <T> ListenableFuture<T> doExecute(
      URI url,
      HttpMethod method,
      @Nullable AsyncRequestCallback requestCallback,
      @Nullable ResponseExtractor<T> responseExtractor)
  ...
  /**
   * Returns a request callback implementation that prepares the request {@code Accept} headers
   * based on the given response type and configured {@linkplain #getMessageConverters() message
   * converters}.
   */
   ...
   * Returns a request callback implementation that writes the given object to the request stream.
   */
  protected <T> AsyncRequestCallback httpEntityCallback(
      @Nullable HttpEntity<T> request, Type responseType) {
    return new AsyncRequestCallbackAdapter(
  ...
  /**
   * Future returned from {@link #doExecute(URI, HttpMethod, AsyncRequestCallback,
   * ResponseExtractor)}.
   */
  private class ResponseExtractorFuture<T> extends ListenableFutureAdapter<T, ClientHttpResponse> {
    ...
    @Nullable private final ResponseExtractor<T> responseExtractor;

    public ResponseExtractorFuture(
        HttpMethod method,
        URI url,
