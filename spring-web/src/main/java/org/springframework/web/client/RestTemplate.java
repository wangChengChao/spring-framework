 ...
 * underlying HTTP client libraries such as the JDK {@code HttpURLConnection}, Apache
 * HttpComponents, and others.
 *
 * <p>The RestTemplate offers templates for common scenarios by HTTP method, in addition to the
 * generalized {@code exchange} and {@code execute} methods that support of less frequent cases.
 *
 * <p><strong>NOTE:</strong> As of 5.0, the non-blocking, reactive {@code
 * org.springframework.web.reactive.client.WebClient} offers a modern alternative to the {@code
 * RestTemplate} with efficient support for both sync and async, as well as streaming scenarios. The
 * {@code RestTemplate} will be deprecated in a future version and will not have major new features
 * added going forward. See the WebClient section of the Spring Framework reference documentation
...
public class RestTemplate extends InterceptingHttpAccessor implements RestOperations {
  ...
  /**
   * Create a new instance of the {@link RestTemplate} using default settings. Default {@link
   * HttpMessageConverter HttpMessageConverters} are initialized.
   */
  public RestTemplate() {
  ...
  /**
   * Create a new instance of the {@link RestTemplate} based on the given {@link
  ...
  /**
   * Create a new instance of the {@link RestTemplate} using the given list of {@link
  ...
  private void validateConverters(List<HttpMessageConverter<?>> messageConverters) {
    ...
    Assert.noNullElements(
        messageConverters, "The HttpMessageConverter list must not contain null elements");
  }
  ...
  public void setDefaultUriVariables(Map<String, ?> uriVars) {
    ...
    } else if (this.uriTemplateHandler
   ...
   *
   * <p>By default, {@link DefaultUriBuilderFactory} is used and for backwards compatibility, the
   * encoding mode is set to {@link EncodingMode#URI_COMPONENT URI_COMPONENT}. As of 5.0.8, prefer
   * using {@link EncodingMode#TEMPLATE_AND_VALUES TEMPLATE_AND_VALUES}.
   *
   * <p><strong>Note:</strong> in 5.0 the switch from {@link
   * org.springframework.web.util.DefaultUriTemplateHandler DefaultUriTemplateHandler} (deprecated
   * in 4.3), as the default to use, to {@link DefaultUriBuilderFactory} brings in a different
   * default for the {@code parsePath} property (switching from false to true).
   *
  ...
  @Override
  public <T> ResponseEntity<T> postForEntity(
      URI url, @Nullable Object request, Class<T> responseType) throws RestClientException {
  ...
  public <T> ResponseEntity<T> exchange(RequestEntity<?> requestEntity, Class<T> responseType)
    ...
    return nonNull(
        doExecute(
            requestEntity.getUrl(), requestEntity.getMethod(), requestCallback, responseExtractor));
  }
   ...
   * {@inheritDoc}
   *
   * <p>To provide a {@code RequestCallback} or {@code ResponseExtractor} only, but not both,
   * consider using:
   *
   * <ul>
   ...
   *
   * <p>The {@link ClientHttpRequest} is processed using the {@link RequestCallback}; the response
   * with the {@link ResponseExtractor}.
   *
  ...
  protected <T> T doExecute(
    ...
    try {
      ...
      throw new ResourceAccessException(
          "I/O error on " + method.name() + " request for \"" + resource + "\": " + ex.getMessage(),
          ex);
    } finally {
      if (response != null) {
        response.close();
  ...
  /**
   * Handle the given response, performing appropriate logging and invoking the {@link
  ...
  /**
   * Return a {@code RequestCallback} that sets the request {@code Accept} header based on the given
   * response type, cross-checked against the configured message converters.
   */
  ...
  /**
   * Return a {@code RequestCallback} implementation that writes the given object to the request
   * stream.
   */
   ...
   *
   * <ol>
   *   <li>Sets the request {@code Accept} header based on the given response type, cross-checked
   *       against the configured message converters.
  ...
  private class AcceptHeaderRequestCallback implements RequestCallback {
    ...
    @Nullable private final Type responseType;
  ...
  private class HttpEntityRequestCallback extends AcceptHeaderRequestCallback {
        ...
        for (HttpMessageConverter<?> messageConverter : getMessageConverters()) {
          if (messageConverter instanceof GenericHttpMessageConverter) {
            ...
            ((HttpMessageConverter<Object>) messageConverter)
                .write(requestBody, requestContentType, httpRequest);
            return;
          }
        }
    ...
    private void logBody(
        Object body, @Nullable MediaType mediaType, HttpMessageConverter<?> converter) {
      if (logger.isDebugEnabled()) {
        if (mediaType != null) {
  ...
  private class ResponseEntityResponseExtractor<T> implements ResponseExtractor<ResponseEntity<T>> {
    ...
    @Nullable private final HttpMessageConverterExtractor<T> delegate;
