...
/**
 * Implementation of {@link ClientHttpResponse} that can not only check if the response has a
 * message body, but also if its length is 0 (i.e. empty) by actually reading the input stream.
 *
 * @author Brian Clozel
 * @since 4.1.5
...
class MessageBodyClientHttpResponseWrapper implements ClientHttpResponse {
  ...
  @Nullable private PushbackInputStream pushbackInputStream;
   ...
   *
   * <ul>
   *   <li>a response status of {@code 1XX}, {@code 204} or {@code 304}
   *   <li>a {@code Content-Length} header of {@code 0}
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>if no bytes are available, the message body is empty
   *   <li>otherwise it is not empty and the stream is reset to its start for further reading
   * </ul>
   *
