...
/**
 * Base class for {@code Filter}s that perform logging operations before and after a request is
 * processed.
 *
 * <p>Subclasses should override the {@code beforeRequest(HttpServletRequest, String)} and {@code
 * afterRequest(HttpServletRequest, String)} methods to perform the actual logging around the
 * request.
 *
 * <p>Subclasses are passed the message to write to the log in the {@code beforeRequest} and {@code
 * afterRequest} methods. By default, only the URI of the request is logged. However, setting the
 * {@code includeQueryString} property to {@code true} will cause the query string of the request to
 * be included also; this can be further extended through {@code includeClientInfo} and {@code
 * includeHeaders}. The payload (body content) of the request can be logged via the {@code
 * includePayload} flag: Note that this will only log the part of the payload which has actually
 * been read, not necessarily the entire body of the request.
 *
 * <p>Prefixes and suffixes for the before and after messages can be configured using the {@code
 * beforeMessagePrefix}, {@code afterMessagePrefix}, {@code beforeMessageSuffix} and {@code
...
public abstract class AbstractRequestLoggingFilter extends OncePerRequestFilter {
   ...
   *
   * <p>Should be configured using an {@code <init-param>} for parameter name "includeQueryString"
   * in the filter definition in {@code web.xml}.
   */
   ...
   *
   * <p>Should be configured using an {@code <init-param>} for parameter name "includeClientInfo" in
   * the filter definition in {@code web.xml}.
   */
   ...
   *
   * <p>Should be configured using an {@code <init-param>} for parameter name "includeHeaders" in
   * the filter definition in {@code web.xml}.
   *
   * @since 4.3
   */
   ...
   *
   * <p>Should be configured using an {@code <init-param>} for parameter name "includePayload" in
   * the filter definition in {@code web.xml}.
   *
   * @since 3.0
   */
  ...
  /**
   * Configure a predicate for selecting which headers should be logged if {@link
   * #setIncludeHeaders(boolean)} is set to {@code true}.
   *
  ...
  /**
   * Set the value that should be prepended to the log message written <i>before</i> a request is
   * processed.
   */
  ...
  /**
   * Set the value that should be appended to the log message written <i>before</i> a request is
   * processed.
   */
  ...
  /**
   * Set the value that should be prepended to the log message written <i>after</i> a request is
   * processed.
   */
  ...
  /**
   * Set the value that should be appended to the log message written <i>after</i> a request is
   * processed.
   */
   ...
   *
   * <p>If {@code includeQueryString} is {@code true}, then the inner part of the log message will
   * take the form {@code request_uri?query_string}; otherwise the message will simply be of the
   * form {@code request_uri}.
   *
   * <p>The final message is composed of the inner part as described and the supplied prefix and
   * suffix.
   */
  ...
  /**
   * Extracts the message payload portion of the message created by {@link
   * #createMessage(HttpServletRequest, String, String)} when {@link #isIncludePayload()} returns
   * true.
   *
   * @since 5.0.3
   */
   ...
   * building).
   *
   * <p>The default implementation always returns {@code true}. Subclasses may override this with a
   * log level check.
   *
  ...
  /**
   * Concrete subclasses should implement this method to write a log message <i>before</i> the
  ...
  /**
   * Concrete subclasses should implement this method to write a log message <i>after</i> the
   * request is processed.
   *
   * @param request current HTTP request
   * @param message the message to log
