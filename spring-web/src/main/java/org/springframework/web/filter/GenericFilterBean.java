...
/**
 * Simple base implementation of {@link javax.servlet.Filter} which treats its config parameters
 * ({@code init-param} entries within the {@code filter} tag in {@code web.xml}) as bean properties.
 *
 * <p>A handy superclass for any type of filter. Type conversion of config parameters is automatic,
 * with the corresponding setter method getting invoked with the converted value. It is also
 * possible for subclasses to specify required properties. Parameters without matching bean property
 * setter will simply be ignored.
 *
 * <p>This filter leaves actual filtering to subclasses, which have to implement the {@link
 * javax.servlet.Filter#doFilter} method.
 *
 * <p>This generic filter base class has no dependency on the Spring {@link
 * org.springframework.context.ApplicationContext} concept. Filters usually don't load their own
 * context but rather access service beans from the Spring root application context, accessible via
 * the filter's {@link #getServletContext() ServletContext} (see {@link
...
public abstract class GenericFilterBean
   ...
   *
   * <p>Only relevant in case of initialization as bean, to have a name as fallback to the filter
   ...
   * default.
   *
   * <p>This {@code Environment} object is used only for resolving placeholders in resource paths
   * passed into init-parameters for this filter. If no init-params are used, this {@code
   * Environment} can be essentially ignored.
   */
  @Override
   ...
   *
   * <p>Only relevant in case of initialization as bean, to have a ServletContext as fallback to the
   ...
   *
   * <p>Note: This method will be called from standard filter destruction as well as filter bean
   ...
   *
   * <p>This method is only relevant in case of traditional initialization driven by a FilterConfig
   * instance.
   *
   ...
   * filter will have been set before this method is invoked.
   *
   * <p>Note: This method will be called from standard filter initialization as well as filter bean
  ...
  /**
   * Make the FilterConfig of this filter available, if any. Analogous to GenericServlet's {@code
   * getServletConfig()}.
   *
   * <p>Public to resemble the {@code getFilterConfig()} method of the Servlet Filter version that
  ...
  /**
   * Make the name of this filter available to subclasses. Analogous to GenericServlet's {@code
   * getServletName()}.
   *
   * <p>Takes the FilterConfig's filter name by default. If initialized as bean in a Spring
  ...
  /**
   * Make the ServletContext of this filter available to subclasses. Analogous to GenericServlet's
   * {@code getServletContext()}.
   *
   * <p>Takes the FilterConfig's ServletContext by default. If initialized as bean in a Spring
  ...
  private static class FilterConfigPropertyValues extends MutablePropertyValues {
    ...
    public FilterConfigPropertyValues(FilterConfig config, Set<String> requiredProperties)
      ...
      Set<String> missingProps =
          (!CollectionUtils.isEmpty(requiredProperties) ? new HashSet<>(requiredProperties) : null);
