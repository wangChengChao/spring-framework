...
/**
 * Extract values from "Forwarded" and "X-Forwarded-*" headers, wrap the request and response, and
 * make they reflect the client-originated protocol and address in the following methods:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>This filter can also be used in a {@link #setRemoveOnly removeOnly} mode where "Forwarded" and
 * "X-Forwarded-*" headers are eliminated, and not used.
 *
...
public class ForwardedHeaderFilter extends OncePerRequestFilter {
  ...
  /**
   * Enables mode in which any "Forwarded" or "X-Forwarded-*" headers are removed only and the
  ...
  /**
   * Use this property to enable relative redirects as explained in {@link RelativeRedirectFilter},
   * and also using the same response wrapper as that filter does, or if both are configured, only
   * one will wrap.
   *
   * <p>By default, if this property is set to false, in which case calls to {@link
   * HttpServletResponse#sendRedirect(String)} are overridden in order to turn relative into
   * absolute URLs, also taking into account forwarded headers.
   *
  ...
  private static class ForwardedHeaderExtractingRequest extends ForwardedHeaderRemovingRequest {
    ...
    ForwardedHeaderExtractingRequest(HttpServletRequest request, UrlPathHelper pathHelper) {
      ...
      this.forwardedPrefixExtractor =
          new ForwardedPrefixExtractor(delegateRequest, pathHelper, baseUrl);
    }
  ...
  /**
   * Responsible for the contextPath, requestURI, and requestURL with forwarded headers in mind, and
   * also taking into account changes to the path of the underlying delegate request (e.g. on a
   * Servlet FORWARD).
   */
  private static class ForwardedPrefixExtractor {
     ...
     *
     * @param delegateRequest supplier for the current {@link HttpServletRequestWrapper#getRequest()
     *     delegate request} which may change during a forward (e.g. Tomcat.
  ...
  private static class ForwardedHeaderExtractingResponse extends HttpServletResponseWrapper {
    ...
    public void sendRedirect(String location) throws IOException {
      ...
      String result =
          UriComponentsBuilder.fromHttpRequest(new ServletServerHttpRequest(this.request))
              .replacePath(path)
