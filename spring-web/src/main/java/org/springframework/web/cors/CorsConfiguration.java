...
/**
 * A container for CORS configuration along with methods to check against the actual origin, HTTP
 * methods, and headers of a given request.
 *
 * <p>By default a newly created {@code CorsConfiguration} does not permit any cross-origin requests
 * and must be configured explicitly to indicate what should be allowed. Use {@link
 * #applyPermitDefaultValues()} to flip the initialization model to start with open defaults that
...
public class CorsConfiguration {
  ...
  @Nullable private List<HttpMethod> resolvedMethods = DEFAULT_METHODS;
  ...
  /**
   * Construct a new {@code CorsConfiguration} instance with no cross-origin requests allowed for
  ...
  /**
   * Construct a new {@code CorsConfiguration} instance by copying all values from the supplied
   ...
   * <p>By default this is not set.
   *
   * <p><strong>Note:</strong> CORS checks use values from "Forwarded" (<a
   * href="https://tools.ietf.org/html/rfc7239">RFC 7239</a>), "X-Forwarded-Host",
  ...
  /**
   * Return the allowed HTTP methods, or {@code null} in which case only {@code "GET"} and {@code
   * "HEAD"} allowed.
   *
  ...
  /**
   * Configure how long, as a duration, the response from a pre-flight request can be cached by
   * clients.
   *
   * @since 5.2
   * @see #setMaxAge(Long)
  ...
  /**
   * Configure how long, in seconds, the response from a pre-flight request can be cached by
   * clients.
   *
   * <p>By default this is not set.
   */
  ...
  /**
   * By default a newly created {@code CorsConfiguration} does not permit any cross-origin requests
   * and must be configured explicitly to indicate what should be allowed.
   *
   * <p>Use this method to flip the initialization model to start with open defaults that permit all
   * cross-origin requests for GET, HEAD, and POST requests. Note however that this method will not
   * override any existing values already set.
   *
  ...
  public CorsConfiguration applyPermitDefaultValues() {
    ...
    if (this.allowedMethods == null) {
      ...
      this.resolvedMethods =
          DEFAULT_PERMIT_METHODS.stream().map(HttpMethod::resolve).collect(Collectors.toList());
    }
    if (this.allowedHeaders == null) {
   ...
   *
   * <p>When combining single values like {@code allowCredentials} or {@code maxAge}, {@code this}
   * properties are overridden by non-null {@code other} properties if any.
   *
   * <p>Combining lists like {@code allowedOrigins}, {@code allowedMethods}, {@code allowedHeaders}
   * or {@code exposedHeaders} is done in an additive way. For example, combining {@code ["GET",
   * "POST"]} with {@code ["PATCH"]} results in {@code ["GET", "POST", "PATCH"]}, but keep in mind
   * that combining {@code ["GET", "POST"]} with {@code ["*"]} results in {@code ["*"]}.
   *
   * <p>Notice that default permit values set by {@link
   * CorsConfiguration#applyPermitDefaultValues()} are overridden by any value explicitly defined.
   *
   * @return the combined {@code CorsConfiguration}, or {@code this} configuration if the supplied
   ...
   * @return the origin to use for the response, or {@code null} which means the request origin is
   *     not allowed
   */
  @Nullable
  ...
  /**
   * Check the HTTP request method (or the method from the {@code Access-Control-Request-Method}
   * header on a pre-flight request) against the configured allowed methods.
   *
   * @param requestMethod the HTTP request method to check
   * @return the list of HTTP methods to list in the response of a pre-flight request, or {@code
   *     null} if the supplied {@code requestMethod} is not allowed
   */
  @Nullable
  ...
  /**
   * Check the supplied request headers (or the headers listed in the {@code
   * Access-Control-Request-Headers} of a pre-flight request) against the configured allowed
   * headers.
   *
   * @param requestHeaders the request headers to check
   * @return the list of allowed headers to list in the response of a pre-flight request, or {@code
   *     null} if none of the supplied request headers is allowed
   */
  @Nullable
