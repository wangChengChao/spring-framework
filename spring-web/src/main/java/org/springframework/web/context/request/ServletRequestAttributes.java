 ...
 *
 * <p>Accesses objects from servlet request and HTTP session scope, with no distinction between
...
public class ServletRequestAttributes extends AbstractRequestAttributes {
  ...
  /**
   * Constant identifying the {@link String} prefixed to the name of a destruction callback when it
   * is stored in a {@link HttpSession}.
   */
  ...
  @Nullable private volatile HttpSession session;
  ...
  /**
   * Update all accessed session attributes through {@code session.setAttribute} calls, explicitly
   * indicating to the container that they might have been modified.
   */
  @Override
  ...
  /**
   * Determine whether the given value is to be considered as an immutable session attribute, that
   * is, doesn't have to be re-set via {@code session.setAttribute} since its value cannot
   * meaningfully change internally.
   *
   * <p>The default implementation returns {@code true} for {@code String}, {@code Character},
   * {@code Boolean} and standard {@code Number} values.
   *
   * @param name the name of the attribute
   * @param value the corresponding value to check
   * @return {@code true} if the value is to be considered as immutable for the purposes of session
   *     attribute management; {@code false} otherwise
   ...
   *
   * <p>Note: The callback object should be serializable in order to survive web app restarts.
   *
  ...
  protected void registerSessionDestructionCallback(String name, Runnable callback) {
    ...
    session.setAttribute(
        DESTRUCTION_CALLBACK_NAME_PREFIX + name, new DestructionCallbackBindingListener(callback));
  }
