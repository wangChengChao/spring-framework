...
/**
 * Convenience methods for retrieving the root {@link WebApplicationContext} for a given {@link
 * ServletContext}. This is useful for programmatically accessing a Spring application context from
 * within custom web views or MVC actions.
 *
 * <p>Note that there are more convenient ways of accessing the root context for many web
 * frameworks, either part of Spring or available as an external library. This helper class is just
 * the most generic way to access the root context.
 *
...
public abstract class WebApplicationContextUtils {
  ...
  /**
   * Find the root {@code WebApplicationContext} for this web app, typically loaded via {@link
   * org.springframework.web.context.ContextLoaderListener}.
   *
   * <p>Will rethrow an exception that happened on root context startup, to differentiate between a
   * failed context startup and no context at all.
   *
  ...
  /**
   * Find a unique {@code WebApplicationContext} for this web app: either the root web app context
   * (preferred) or a unique {@code WebApplicationContext} among the registered {@code
   * ServletContext} attributes (typically coming from a single {@code DispatcherServlet} in the
   * current web application).
   *
   * <p>Note that {@code DispatcherServlet}'s exposure of its context can be controlled through its
   * {@code publishContext} property, which is {@code true} by default but can be selectively
   * switched to only publish a single context despite multiple {@code DispatcherServlet}
  ...
  /**
   * Register web-specific scopes ("request", "session", "globalSession", "application") with the
  ...
  public static void registerWebApplicationScopes(
      ConfigurableListableBeanFactory beanFactory, @Nullable ServletContext sc) {
  ...
  public static void registerEnvironmentBeans(
      ConfigurableListableBeanFactory bf, @Nullable ServletContext sc) {
  ...
  public static void registerEnvironmentBeans(
    ...
    if (servletConfig != null
      ...
      bf.registerSingleton(
          ConfigurableWebApplicationContext.SERVLET_CONFIG_BEAN_NAME, servletConfig);
    }
   ...
   * ServletContext, ServletConfig)} that always provides {@code null} for the {@link ServletConfig}
   * parameter.
   *
  ...
  /**
   * Replace {@code Servlet}-based {@link StubPropertySource stub property sources} with actual
   * instances populated with the given {@code servletContext} and {@code servletConfig} objects.
   *
   * <p>This method is idempotent with respect to the fact it may be called any number of times but
   * will perform replacement of stub property sources with their corresponding actual property
   * sources once and only once.
   *
   * @param sources the {@link MutablePropertySources} to initialize (must not be {@code null})
   * @param servletContext the current {@link ServletContext} (ignored if {@code null} or if the
   *     {@link StandardServletEnvironment#SERVLET_CONTEXT_PROPERTY_SOURCE_NAME servlet context
   *     property source} has already been initialized)
   * @param servletConfig the current {@link ServletConfig} (ignored if {@code null} or if the
   *     {@link StandardServletEnvironment#SERVLET_CONFIG_PROPERTY_SOURCE_NAME servlet config
   *     property source} has already been initialized)
  ...
  private static class ResponseObjectFactory
    ...
    public ServletResponse getObject() {
      ...
      if (response == null) {
        throw new IllegalStateException(
            "Current servlet response not available - "
  ...
  private static class FacesDependencyRegistrar {
    ...
    public static void registerFacesDependencies(ConfigurableListableBeanFactory beanFactory) {
      beanFactory.registerResolvableDependency(
          FacesContext.class,
          new ObjectFactory<FacesContext>() {
            @Override
      ...
      beanFactory.registerResolvableDependency(
          ...
          new ObjectFactory<ExternalContext>() {
            @Override
