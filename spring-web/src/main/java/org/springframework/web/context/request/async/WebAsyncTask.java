...
public class WebAsyncTask<V> implements BeanFactoryAware {
   ...
   *
   * <p>This factory reference will automatically be set when {@code WebAsyncTask} is used within a
  ...
  public AsyncTaskExecutor getExecutor() {
    if (this.executor != null) {
      ...
      Assert.state(
          this.beanFactory != null, "BeanFactory is required to look up an executor bean by name");
   ...
   * {@code Callable} has completed. The callback is executed in the same thread and therefore
   * should return without blocking. It may return an alternative value to use, including an {@link
   * Exception} or return {@link CallableProcessingInterceptor#RESULT_NONE RESULT_NONE}.
   */
   ...
   *
   * <p>This method is called from a container thread when an error occurred while processing an
   * async request before the {@code Callable} has completed. The callback is executed in the same
   * thread and therefore should return without blocking. It may return an alternative value to use,
   * including an {@link Exception} or return {@link CallableProcessingInterceptor#RESULT_NONE
   * RESULT_NONE}.
   *
   * @since 5.0
   */
   ...
   *
   * <p>This method is called from a container thread when an async request completed for any
