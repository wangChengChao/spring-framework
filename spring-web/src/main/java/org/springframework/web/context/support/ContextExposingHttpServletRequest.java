...
/**
 * HttpServletRequest decorator that makes all Spring beans in a given WebApplicationContext
 * accessible as request attributes, through lazy checking once an attribute gets accessed.
 *
...
public class ContextExposingHttpServletRequest extends HttpServletRequestWrapper {
   ...
   * @param originalRequest the original HttpServletRequest
   * @param context the WebApplicationContext that this request runs in
   * @param exposedContextBeanNames the names of beans in the context which are supposed to be
   *     exposed (if this is non-null, only the beans in this Set are eligible for exposure as
   *     attributes)
   */
  public ContextExposingHttpServletRequest(
