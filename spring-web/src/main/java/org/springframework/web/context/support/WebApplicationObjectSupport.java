 ...
 * Provides {@code getWebApplicationContext()}, {@code getServletContext()}, and {@code
 * getTempDir()} accessors.
 *
 * <p>Note: It is generally recommended to use individual callback interfaces for the actual
 * callbacks needed. This broad base class is primarily intended for use within the framework, in
 * case of {@link ServletContext} access etc typically being needed.
 *
...
public abstract class WebApplicationObjectSupport extends ApplicationObjectSupport
  ...
  /**
   * Overrides the base class behavior to enforce running in an ApplicationContext. All accessors
   * will throw IllegalStateException if not running in a context.
   *
  ...
  /**
   * Calls {@link #initServletContext(javax.servlet.ServletContext)} if the given ApplicationContext
   * is a {@link WebApplicationContext}.
   */
  @Override
  ...
  /**
   * Subclasses may override this for custom initialization based on the ServletContext that this
   * application object runs in.
   *
   * <p>The default implementation is empty. Called by {@link
   * #initApplicationContext(org.springframework.context.ApplicationContext)} as well as {@link
   * #setServletContext(javax.servlet.ServletContext)}.
   *
   * @param servletContext the ServletContext that this application object runs in (never {@code
   *     null})
   */
   ...
   *
   * <p><b>NOTE:</b> Only use this if you actually need to access WebApplicationContext-specific
   * functionality. Preferably use {@code getApplicationContext()} or {@code getServletContext()}
