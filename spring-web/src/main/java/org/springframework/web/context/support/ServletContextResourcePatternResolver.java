...
/**
 * ServletContext-aware subclass of {@link PathMatchingResourcePatternResolver}, able to find
 * matching resources below the web application root directory via {@link
 * ServletContext#getResourcePaths}. Falls back to the superclass' file system checking for other
 * resources.
 *
...
public class ServletContextResourcePatternResolver extends PathMatchingResourcePatternResolver {
  ...
  /**
   * Overridden version which checks for ServletContextResource and uses {@code
   * ServletContext.getResourcePaths} to find matching resources below the web application root
   * directory. In case of other resources, delegates to the superclass version.
   *
  ...
  /**
   * Recursively retrieve ServletContextResources that match the given pattern, adding them to the
   * given result set.
   *
  ...
  protected void doRetrieveMatchingServletContextResources(
    ...
    if (candidates != null) {
      ...
      if (jarFileSep > 0
          && jarFileSep + ResourceUtils.JAR_URL_SEPARATOR.length() < fullPattern.length()) {
        jarFilePath = fullPattern.substring(0, jarFileSep);
        pathInJarFile =
            fullPattern.substring(jarFileSep + ResourceUtils.JAR_URL_SEPARATOR.length());
      }
      for (String currPath : candidates) {
        ...
        if (currPath.endsWith("/")
            && (dirDepthNotFixed
                || StringUtils.countOccurrencesOf(currPath, "/")
  ...
  private void doRetrieveMatchingJarEntries(
      String jarFilePath, String entryPattern, Set<Resource> result) {
    ...
    try {
      ...
      if (logger.isWarnEnabled()) {
        logger.warn(
            "Cannot search for matching resources in jar file ["
                + jarFilePath
