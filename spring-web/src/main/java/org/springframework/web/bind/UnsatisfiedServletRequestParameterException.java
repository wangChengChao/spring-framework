...
public class UnsatisfiedServletRequestParameterException extends ServletRequestBindingException {
  ...
  public UnsatisfiedServletRequestParameterException(
      String[] paramConditions, Map<String, String[]> actualParams) {
    super("");
  ...
  /**
   * Return the parameter conditions that have been violated or the first group in case of multiple
   * groups.
   *
   * @see org.springframework.web.bind.annotation.RequestMapping#params()
   */
  ...
  private static String requestParameterMapToString(Map<String, String[]> actualParams) {
    ...
    for (Iterator<Map.Entry<String, String[]>> it = actualParams.entrySet().iterator();
        it.hasNext(); ) {
      Map.Entry<String, String[]> entry = it.next();
      result
          .append(entry.getKey())
          .append('=')
          .append(ObjectUtils.nullSafeToString(entry.getValue()));
      if (it.hasNext()) {
        result.append(", ");
      }
    }
