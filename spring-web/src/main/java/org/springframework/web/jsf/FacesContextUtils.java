...
/**
 * Convenience methods to retrieve Spring's root {@link WebApplicationContext} for a given JSF
 * {@link FacesContext}. This is useful for accessing a Spring application context from custom
 * JSF-based code.
 *
...
public abstract class FacesContextUtils {
   ...
   * HttpSessionMutexListener needs to be defined in {@code web.xml}. Falls back to the Session
   * reference itself if no mutex attribute found.
   *
   ...
   *
   * <p>In many cases, the Session reference itself is a safe mutex as well, since it will always be
   * the same object reference for the same active logical session. However, this is not guaranteed
