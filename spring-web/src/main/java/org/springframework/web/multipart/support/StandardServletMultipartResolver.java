 ...
 * DispatcherServlet context, without any extra configuration at the bean level (see below).
 *
 * <p><b>Note:</b> In order to use Servlet 3.0 based multipart parsing, you need to mark the
 * affected servlet with a "multipart-config" section in {@code web.xml}, or with a {@link
 * javax.servlet.MultipartConfigElement} in programmatic servlet registration, or (in case of a
 * custom servlet class) possibly with a {@link javax.servlet.annotation.MultipartConfig} annotation
 * on your servlet class. Configuration settings such as maximum sizes or storage locations need to
 * be applied at that servlet registration level; Servlet 3.0 does not allow for them to be set at
 * the MultipartResolver level.
 *
 * <pre class="code">
