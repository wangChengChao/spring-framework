...
/**
 * Helper for decoding HTML Strings by replacing character entity references with the referred
 * character.
 *
...
class HtmlCharacterEntityDecoder {
  ...
  public HtmlCharacterEntityDecoder(
      HtmlCharacterEntityReferences characterEntityReferences, String original) {
  ...
  private void findNextPotentialReference(int startPosition) {
    this.nextPotentialReferencePosition =
        Math.max(startPosition, this.nextSemicolonPosition - MAX_REFERENCE_SIZE);
    ...
    do {
      ...
      if (this.nextSemicolonPosition != -1
        ...
        this.nextSemicolonPosition =
            this.originalMessage.indexOf(';', this.nextPotentialReferencePosition + 1);
      }
  ...
  private void processPossibleReference() {
    if (this.nextPotentialReferencePosition != -1) {
      ...
      boolean wasProcessable =
          isNumberedReference ? processNumberedReference() : processNamedReference();
      if (wasProcessable) {
