 ...
 *
 * <ul>
 *   <li>{@code "encodeXyz"} -- these encode a specific URI component (e.g. path, query) by percent
 *       encoding illegal characters, which includes non-US-ASCII characters, and also characters
 *       that are otherwise illegal within the given URI component type, as defined in RFC 3986. The
 *       effect of this method, with regards to encoding, is comparable to using the multi-argument
 *       constructor of {@link URI}.
 *   <li>{@code "encode"} and {@code "encodeUriVariables"} -- these can be used to encode URI
 *       variable values by percent encoding all characters that are either illegal, or have any
 *       reserved meaning, anywhere within a URI.
 * </ul>
 *
...
public abstract class UriUtils {
  ...
  /**
   * Encode all characters that are either illegal, or have any reserved meaning, anywhere within a
   * URI, as defined in <a href="https://tools.ietf.org/html/rfc3986">RFC 3986</a>. This is useful
   * to ensure that the given String will be preserved as-is and will not have any o impact on the
  ...
  private static String encode(
      String scheme, String encoding, HierarchicalUriComponents.Type type) {
    return HierarchicalUriComponents.encodeUriComponent(scheme, encoding, type);
  }

  private static String encode(
      String scheme, Charset charset, HierarchicalUriComponents.Type type) {
