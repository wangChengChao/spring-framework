...
/**
 * Parser for URI template patterns. It breaks the path pattern into a number of {@link PathElement
 * PathElements} in a linked list. Instances are reusable but are not thread-safe.
 *
 * @author Andy Clement
 * @since 5.0
 */
class InternalPathPatternParser {
  ...
  @Nullable private List<String> capturedVariableNames;
          ...
          if ((this.variableCaptureStart + 1 + (this.isCaptureTheRestVariable ? 1 : 0)) == this.pos
            ...
            throw new PatternParseException(
                ...
                Character.toString(ch));
                  ...
                  > (this.variableCaptureStart + 1 + (this.isCaptureTheRestVariable ? 1 : 0))
  ...
  /**
   * Just hit a ':' and want to jump over the regex specification for this variable. pos will be
   * pointing at the ':', we want to skip until the }.
   *
   * <p>Nested {...} pairs don't have to be escaped: <tt>/abc/{var:x{1,2}}/def</tt>
   *
  ...
  private void skipCaptureRegex() {
    ...
    while (this.pos < this.pathPatternLength) {
      ...
      if (ch == this.parser.getPathOptions().separator() && !previousBackslash) {
        throw new PatternParseException(
            this.pos, this.pathPatternData, PatternMessage.MISSING_CLOSE_CAPTURE);
      }
      this.pos++;
    ...
    throw new PatternParseException(
        this.pos - 1, this.pathPatternData, PatternMessage.MISSING_CLOSE_CAPTURE);
  }

  /**
   * After processing a separator, a quick peek whether it is followed by (and only before the end
   * of the pattern or the next separator).
   */
  ...
  /**
   * Used the knowledge built up whilst processing since the last path element to determine what
   * kind of path element to create.
   *
  ...
  private PathElement createPathElement() {
    ...
    if (this.variableCaptureCount > 0) {
      if (this.variableCaptureCount == 1
          && this.pathElementStart == this.variableCaptureStart
          && this.pathPatternData[this.pos - 1] == '}') {
        if (this.isCaptureTheRestVariable) {
          // It is {*....}
          newPE =
              new CaptureTheRestPathElement(this.pathElementStart, getPathElementText(), separator);
        } else {
          // It is a full capture of this element (possibly with constraint), for example:
          // /foo/{abc}/
          try {
            newPE =
                new CaptureVariablePathElement(
                    this.pathElementStart,
                    getPathElementText(),
                    this.parser.isCaseSensitive(),
                    separator);
          ...
          recordCapturedVariable(
              this.pathElementStart, ((CaptureVariablePathElement) newPE).getVariableName());
        }
      } else {
        if (this.isCaptureTheRestVariable) {
          throw new PatternParseException(
      ...
      if (this.wildcard) {
        ...
        newPE =
            new SingleCharWildcardedPathElement(
                this.pathElementStart,
                getPathElementText(),
                this.singleCharWildcardCount,
                this.parser.isCaseSensitive(),
  ...
  /**
   * For a path element representing a captured variable, locate the constraint pattern. Assumes
   * there is a constraint pattern.
   *
   * @param data a complete path expression, e.g. /aaa/bbb/{ccc:...}
   * @param offset the start of the capture pattern of interest
   * @return the index of the character after the ':' within the pattern expression relative to the
  ...
  private void recordCapturedVariable(int pos, String variableName) {
    ...
    if (this.capturedVariableNames.contains(variableName)) {
      throw new PatternParseException(
          pos, this.pathPatternData, PatternMessage.ILLEGAL_DOUBLE_CAPTURE, variableName);
    }
