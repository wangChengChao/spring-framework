...
/**
 * {@code RouteMatcher} built on {@link PathPatternParser} that uses {@link PathContainer} and
 * {@link PathPattern} as parsed representations of routes and patterns.
 *
...
public class PathPatternRouteMatcher implements RouteMatcher {
  ...
  /**
   * Default constructor with {@link PathPatternParser} customized for {@link
   * org.springframework.http.server.PathContainer.Options#MESSAGE_ROUTE MESSAGE_ROUTE} and without
  ...
  public Route parseRoute(String routeValue) {
    return new PathContainerRoute(
        PathContainer.parsePath(routeValue, this.parser.getPathOptions()));
  }
  ...
  public Map<String, String> matchAndExtract(String pattern, Route route) {
    PathPattern.PathMatchInfo info =
        getPathPattern(pattern).matchAndExtract(getPathContainer(route));
