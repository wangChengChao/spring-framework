...
/**
 * {@code UriBuilderFactory} that relies on {@link UriComponentsBuilder} for the actual building of
 * the URI.
 *
 * <p>Provides options to create {@link UriBuilder} instances with a common base URI, alternative
 * encoding mode strategies, among others.
 *
...
public class DefaultUriBuilderFactory implements UriBuilderFactory {
  ...
  public enum EncodingMode {
     ...
     *
     * <ul>
     *   <li>For the URI template replace <em>only</em> non-ASCII and illegal (within a given URI
     *       component type) characters with escaped octets.
     *   <li>For URI variables do the same and also replace characters with reserved meaning.
     * </ul>
     *
     * <p>For most cases, this mode is most likely to give the expected result because in treats URI
     * variables as opaque data to be fully encoded, while {@link #URI_COMPONENT} by comparison is
     * useful only if intentionally expanding URI variables with reserved characters.
     *
     * @since 5.0.8
  ...
  @Nullable private final UriComponentsBuilder baseUri;
   ...
   *
   * <p>The given URI template is parsed via {@link UriComponentsBuilder#fromUriString} and then
   * applied as a base URI to every UriBuilder via {@link UriComponentsBuilder#uriComponents} unless
   * the UriBuilder itself was created with a URI template that already has a target address.
   *
   ...
   *
   * <p><strong>Note:</strong> In 5.1 the default was changed from {@link EncodingMode#URI_COMPONENT
   * EncodingMode.URI_COMPONENT}. Consequently the {@code WebClient}, which relies on the built-in
   * default has also been switched to the new default. The {@code RestTemplate} however sets this
   * explicitly to {@link EncodingMode#URI_COMPONENT EncodingMode.URI_COMPONENT} explicitly for
   * historic and backwards compatibility reasons.
   *
  ...
  /**
   * Provide default URI variable values to use when expanding URI templates with a Map of
   * variables.
   *
  ...
  /**
   * Whether to parse the input path into path segments if the encoding mode is set to {@link
   * EncodingMode#URI_COMPONENT EncodingMode.URI_COMPONENT}, which ensures that URI variables in the
   * path are encoded according to path segment rules and for example a '/' is encoded.
   *
  ...
  /**
   * Whether to parse the path into path segments if the encoding mode is set to {@link
   * EncodingMode#URI_COMPONENT EncodingMode.URI_COMPONENT}.
   */
