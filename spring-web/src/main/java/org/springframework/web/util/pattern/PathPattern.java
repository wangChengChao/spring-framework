...
/**
 * Representation of a parsed path pattern. Includes a chain of path elements for fast matching and
 * accumulates computed state for quick comparison of patterns.
 *
 * <p>{@code PathPattern} matches URL paths using the following rules:<br>
 *
 * <ul>
 *   <li>{@code ?} matches one character
 *   <li>{@code *} matches zero or more characters within a path segment
 *   <li>{@code **} matches zero or more <em>path segments</em> until the end of the path
 *   <li><code>{spring}</code> matches a <em>path segment</em> and captures it as a variable named
 *       "spring"
 *   <li><code>{spring:[a-z]+}</code> matches the regexp {@code [a-z]+} as a path variable named
 *       "spring"
 *   <li><code>{*spring}</code> matches zero or more <em>path segments</em> until the end of the
 *       path and captures it as a variable named "spring"
 * </ul>
 *
 * <h3>Examples</h3>
 *
 * <ul>
 *   <li>{@code /pages/t?st.html} &mdash; matches {@code /pages/test.html} as well as {@code
 *       /pages/tXst.html} but not {@code /pages/toast.html}
 *   <li>{@code /resources/*.png} &mdash; matches all {@code .png} files in the {@code resources}
 *       directory
 *   <li><code>/resources/&#42;&#42;</code> &mdash; matches all files underneath the {@code
 *       /resources/} path, including {@code /resources/image.png} and {@code
 *       /resources/css/spring.css}
 *   <li><code>/resources/{&#42;path}</code> &mdash; matches all files underneath the {@code
 *       /resources/} path and captures their relative path in a variable named "path"; {@code
 *       /resources/image.png} will match with "spring" &rarr; "/image.png", and {@code
 *       /resources/css/spring.css} will match with "spring" &rarr; "/css/spring.css"
 *   <li><code>/resources/{filename:\\w+}.dat</code> will match {@code /resources/spring.dat} and
 *       assign the value {@code "spring"} to the {@code filename} variable
 * </ul>
 *
 * @author Andy Clement
...
public class PathPattern implements Comparable<PathPattern> {
  ...
  /**
   * If this pattern has no trailing slash, allow candidates to include one and still match
   * successfully.
   */
  ...
  /**
   * The normalized length is trying to measure the 'active' part of the pattern. It is computed by
   * assuming all captured variables have a normalized length of 1. Effectively this means changing
  ...
  PathPattern(String patternText, PathPatternParser parser, @Nullable PathElement head) {
    ...
    while (elem != null) {
      ...
      if (elem instanceof SeparatorPathElement
          && elem.next != null
          && elem.next instanceof WildcardPathElement
          && elem.next.next == null) {
  ...
  /**
   * Whether the pattern string contains pattern syntax that would require use of {@link
   * #matches(PathContainer)}, or if it is a regular String that could be compared directly to
   * others.
   *
   * @since 5.2
   */
  ...
  public boolean matches(PathContainer pathContainer) {
    if (this.head == null) {
      ...
      if (this.head instanceof WildcardTheRestPathElement
          || this.head instanceof CaptureTheRestPathElement) {
  ...
  /**
   * Match this pattern to the given URI path and return extracted URI template variables as well as
   * path parameters (matrix variables).
   *
   * @param pathContainer the candidate path to attempt to match against
  ...
  public PathMatchInfo matchAndExtract(PathContainer pathContainer) {
    if (this.head == null) {
      return hasLength(pathContainer)
              && !(this.matchOptionalTrailingSeparator
                  && pathContainerIsJustSeparator(pathContainer))
          ? null
          : PathMatchInfo.EMPTY;
    } else if (!hasLength(pathContainer)) {
      if (this.head instanceof WildcardTheRestPathElement
  ...
  /**
   * Match the beginning of the given path and return the remaining portion not covered by this
   * pattern. This is useful for matching nested routes where the path is matched incrementally at
   * each level.
   *
   * @param pathContainer the candidate path to attempt to match against
  ...
  public PathRemainingMatchInfo matchStartOfPath(PathContainer pathContainer) {
    ...
    if (!matches) {
      ...
      if (matchingContext.remainingPathIndex == pathContainer.elements().size()) {
        ...
        info =
            new PathRemainingMatchInfo(
                pathContainer.subPath(matchingContext.remainingPathIndex),
   ...
   *
   * <ul>
   *   <li>'{@code /docs/cvs/commit.html}' and '{@code /docs/cvs/commit.html} &rarr; ''
   *   <li>'{@code /docs/*}' and '{@code /docs/cvs/commit}' &rarr; '{@code cvs/commit}'
   *   <li>'{@code /docs/cvs/*.html}' and '{@code /docs/cvs/commit.html} &rarr; '{@code
   *       commit.html}'
   *   <li>'{@code /docs/**}' and '{@code /docs/cvs/commit} &rarr; '{@code cvs/commit}'
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>Assumes that {@link #matches} returns {@code true} for the same path but does
   ...
   * @return the subset of the path that is matched by pattern or "" if none of it is matched by
   *     pattern elements
   */
  public PathContainer extractPathWithinPattern(PathContainer path) {
    ...
    int startIndex = 0;
    // Find first path element that is not a separator or a literal (i.e. the first pattern based
    // element)
    ...
    for (int i = startIndex; i < (endIndex - 1); i++) {
      if ((pathElements.get(i) instanceof Separator)
          && (pathElements.get(i + 1) instanceof Separator)) {
  ...
  /**
   * Compare this pattern with a supplied pattern: return -1,0,+1 if this pattern is more specific,
   * the same or less specific than the supplied pattern. The aim is to sort more specific patterns
   * first.
   */
  @Override
  public int compareTo(@Nullable PathPattern otherPattern) {
    ...
    return (result == 0 && otherPattern != null
        ? this.patternString.compareTo(otherPattern.patternString)
        : result);
  }
  ...
  public PathPattern combine(PathPattern pattern2string) {
    ...
    // /hotels + booking => /hotels/booking
    int starDotPos1 =
        this.patternString.indexOf("*."); // Are there any file prefix/suffix things to consider?
    ...
    String firstExtension =
        this.patternString.substring(starDotPos1 + 1); // looking for the first extension
  ...
  /**
   * The normalized length is trying to measure the 'active' part of the pattern. It is computed by
   * assuming all capture variables have a normalized length of 1. Effectively this means changing
   * your variable name lengths isn't going to change the length of the active part of the pattern.
   * Useful when comparing two patterns.
   */
  ...
  /**
   * Join two paths together including a separator if necessary. Extraneous separators are removed
   * (if the first path ends with one and the second path starts with one).
   *
  ...
  /**
   * Holder for URI variables and path parameters (matrix variables) extracted based on the pattern
   * for a given matched path.
   */
  public static class PathMatchInfo {
    ...
    PathMatchInfo(
      ...
      this.matrixVariables =
          matrixVars != null ? Collections.unmodifiableMap(matrixVars) : Collections.emptyMap();
    }
    ...
    @Override
    public String toString() {
      return "PathMatchInfo[uriVariables="
          + this.uriVariables
          + ", "
          + "matrixVariables="
          + this.matrixVariables
  ...
  /**
   * Holder for the result of a match on the start of a pattern. Provides access to the remaining
   * path not matched to the pattern as well as any variables bound in that first part that was
   * matched.
   */
  public static class PathRemainingMatchInfo {
    ...
    /**
     * Return variables that were bound in the part of the path that was successfully matched or an
     * empty map.
     */
    public Map<String, String> getUriVariables() {
  ...
  /**
   * Encapsulates context when attempting a match. Includes some fixed state like the candidate
   * currently being considered for a match but also some accumulators for extracted variables.
   */
  class MatchingContext {
    ...
    @Nullable private Map<String, MultiValueMap<String, String>> extractedMatrixVariables;
    ...
    public void set(String key, String value, MultiValueMap<String, String> parameters) {
      ...
      if (!parameters.isEmpty()) {
        ...
        this.extractedMatrixVariables.put(
            key, CollectionUtils.unmodifiableMultiValueMap(parameters));
      }
    }
