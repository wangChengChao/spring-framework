 ...
 *
 * <p>Support {@link #setBaseUrl} and {@link #setDefaultUriVariables} properties that should be
 * relevant regardless of the URI template expand and encode mechanism used in sub-classes.
 *
...
public abstract class AbstractUriTemplateHandler implements UriTemplateHandler {
  ...
  /**
   * Configure default URI variable values to use with every expanded URI template. These default
   * values apply only when expanding with a Map, and not with an array, where the Map supplied to
   * {@link #expand(String, Map)} can override the default values.
   *
