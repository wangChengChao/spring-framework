...
public class PatternParseException extends IllegalArgumentException {
  ...
  PatternParseException(
      Throwable cause, int pos, char[] pattern, PatternMessage messageType, Object... inserts) {
  ...
  /**
   * Return a detailed message that includes the original pattern text with a pointer to the error
   * position, as well as the error message.
   */
  ...
  /**
   * The messages that can be included in a {@link PatternParseException} when there is a parse
   * failure.
   */
  public enum PatternMessage {
    ...
    ILLEGAL_CHARACTER_AT_START_OF_CAPTURE_DESCRIPTOR(
        "Char ''{0}'' not allowed at start of captured variable name"),
    ILLEGAL_CHARACTER_IN_CAPTURE_DESCRIPTOR(
        "Char ''{0}'' is not allowed in a captured variable name"),
    ...
    BADLY_FORMED_CAPTURE_THE_REST(
        "Expected form when capturing the rest of the path is simply '{*...}'"),
