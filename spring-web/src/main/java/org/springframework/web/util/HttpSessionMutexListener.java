 ...
 * created. To be registered as a listener in {@code web.xml}.
 *
 ...
 *
 * <p>In many cases, the HttpSession reference itself is a safe mutex as well, since it will always
 * be the same object reference for the same active logical session. However, this is not guaranteed
...
public class HttpSessionMutexListener implements HttpSessionListener {
  ...
  /**
   * The mutex to be registered. Doesn't need to be anything but a plain Object to synchronize on.
