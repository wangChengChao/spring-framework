...
/**
 * A regex path element. Used to represent any complicated element of the path. For example in
 * '<tt>/foo/&ast;_&ast;/&ast;_{foobar}</tt>' both <tt>*_*</tt> and <tt>*_{foobar}</tt> are {@link
 * RegexPathElement} path elements. Derived from the general {@link
 * org.springframework.util.AntPathMatcher} approach.
 *
 * @author Andy Clement
 * @since 5.0
 */
class RegexPathElement extends PathElement {
  ...
  RegexPathElement(
      int pos, char[] regex, boolean caseSensitive, char[] completePattern, char separator) {
    super(pos, separator);
    this.regex = regex;
        ...
        if (colonIdx == -1) {
          ...
          if (this.variableNames.contains(variableName)) {
            throw new PatternParseException(
                this.pos,
                completePattern,
                PatternParseException.PatternMessage.ILLEGAL_DOUBLE_CAPTURE,
                variableName);
          }
