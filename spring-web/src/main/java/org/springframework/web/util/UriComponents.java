 ...
 * Contains convenience getters for all components. Effectively similar to {@link java.net.URI}, but
 * with more powerful encoding options and support for URI template variables.
 *
...
public abstract class UriComponents implements Serializable {
  ...
  /**
   * Invoke this <em>after</em> expanding URI variables to encode the resulting URI component
   * values.
   *
   * <p>In comparison to {@link UriComponentsBuilder#encode()}, this method <em>only</em> replaces
   * non-ASCII and illegal (within a given URI component type) characters, but not characters with
   * reserved meaning. For most cases, {@link UriComponentsBuilder#encode()} is more likely to give
   * the expected result.
   *
   ...
   *
   * <p>The given map keys represent variable names; the corresponding values represent variable
   * values. The order of variables is not significant.
   *
  ...
  /**
   * Replace all URI template variables with the values from the given {@link UriTemplateVariables}.
   *
  ...
  /**
   * Normalize the path removing sequences like "path/..". Note that normalization is applied to the
   * full path, and not to individual path segments.
   *
   ...
   *
   * <p>This method does nothing more than a simple concatenation based on current values. That
   * means it could produce different results if invoked before vs after methods that can change
   * individual values such as {@code encode}, {@code expand}, or {@code normalize}.
   */
   ...
   *
   * <p>If the current instance is {@link #encode() encoded}, form the full URI String via {@link
   * #toUriString()}, and then pass it to the single argument {@link URI} constructor which
   * preserves percent encoding.
   *
   * <p>If not yet encoded, pass individual URI component values to the multi-argument {@link URI}
   * constructor which quotes illegal characters that cannot appear in their respective URI
   * component.
   */
  ...
  @Nullable
  static String expandUriComponent(
  ...
  public interface UriTemplateVariables {
     ...
     * This is useful for partial expanding of some but not all URI variables.
     */
    Object SKIP_VALUE = UriTemplateVariables.class;

    /**
     * Get the value for the given URI variable name. If the value is {@code null}, an empty String
     * is expanded. If the value is {@link #SKIP_VALUE}, the URI variable is not expanded.
     *
