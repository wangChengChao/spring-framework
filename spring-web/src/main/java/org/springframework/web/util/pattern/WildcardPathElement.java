...
/**
 * A wildcard path element. In the pattern '/foo/&ast;/goo' the * is represented by a
 * WildcardPathElement. Within a path it matches at least one character but at the end of a path it
...
class WildcardPathElement extends PathElement {
  ...
  /**
   * Matching on a WildcardPathElement is quite straight forward. Scan the candidate from the
   * candidateIndex onwards for the next separator or the end of the candidate.
   */
  @Override
      ...
      if (matchingContext.determineRemainingPath) {
        ...
        if (pathIndex == matchingContext.pathLength) {
          ...
          return (matchingContext.isMatchOptionalTrailingSeparator()
              && // if optional slash is on...
              segmentData != null
              && segmentData.length() > 0
              && // and there is at least one character to match the *...
              (pathIndex + 1) == matchingContext.pathLength
              && // and the next path element is the end of the candidate...
