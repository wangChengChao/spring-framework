 ...
 * '/foo/{bar}/goo' the {bar} is represented as a {@link CaptureVariablePathElement}. There must be
 * at least one character to bind to the variable.
 *
 * @author Andy Clement
 * @since 5.0
 */
class CaptureVariablePathElement extends PathElement {
  ...
  @Nullable private Pattern constraintPattern;
  ...
  CaptureVariablePathElement(
      int pos, char[] captureDescriptor, boolean caseSensitive, char separator) {
    super(pos, separator);
    int colon = -1;
    ...
    if (this.constraintPattern != null) {
      ...
      if (matcher.groupCount() != 0) {
        throw new IllegalArgumentException(
            "No capture groups allowed in the constraint regex: "
