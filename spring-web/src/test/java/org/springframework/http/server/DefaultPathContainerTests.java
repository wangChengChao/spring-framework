...
public class DefaultPathContainerTests {
  ...
  private void testPathSegment(
      String rawValue, String valueToMatch, MultiValueMap<String, String> params) {
    ...
    assertThat(segment.valueToMatch())
        .as("valueToMatch: '" + rawValue + "'")
        .isEqualTo(valueToMatch);
  ...
  private void testPath(
      String input, PathContainer.Options options, String value, List<String> expectedElements) {
    PathContainer path = PathContainer.parsePath(input, options);

    assertThat(path.value()).as("value: '" + input + "'").isEqualTo(value);
    assertThat(
            path.elements().stream().map(PathContainer.Element::value).collect(Collectors.toList()))
        .as("elements: " + input)
