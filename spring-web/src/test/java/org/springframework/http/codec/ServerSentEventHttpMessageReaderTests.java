...
public class ServerSentEventHttpMessageReaderTests extends AbstractLeakCheckingTestCase {
  ...
  public void cantRead() {
    assertThat(
            messageReader.canRead(
                ResolvableType.forClass(Object.class), new MediaType("foo", "bar")))
        .isFalse();
  ...
  public void canRead() {
    assertThat(
            messageReader.canRead(
                ResolvableType.forClass(Object.class), new MediaType("text", "event-stream")))
        .isTrue();
    assertThat(
            messageReader.canRead(
                ResolvableType.forClass(ServerSentEvent.class), new MediaType("foo", "bar")))
        .isTrue();
  }
  ...
  public void readPojo() {
    ...
    Flux<Pojo> data =
        messageReader
            .read(ResolvableType.forClass(Pojo.class), request, Collections.emptyMap())
            .cast(Pojo.class);
  ...
  public void decodeFullContentAsString() {
    ...
    MockServerHttpRequest request =
        MockServerHttpRequest.post("/").body(Mono.just(stringBuffer(body)));
