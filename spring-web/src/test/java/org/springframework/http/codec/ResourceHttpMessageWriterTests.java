...
public class ResourceHttpMessageWriterTests {
  ...
  private final Mono<Resource> input =
      Mono.just(
  ...
  public void writeSingleRegion() throws Exception {
    ...
    assertThat(this.response.getHeaders().getContentType()).isEqualTo(TEXT_PLAIN);
    assertThat(this.response.getHeaders().getFirst(HttpHeaders.CONTENT_RANGE))
        .isEqualTo("bytes 0-5/39");
  ...
  public void writeMultipleRegions() throws Exception {
    ...
    StepVerifier.create(this.response.getBodyAsString())
        .consumeNextWith(
            content -> {
              String[] actualRanges =
                  StringUtils.tokenizeToStringArray(content, "\r\n", false, true);
              String[] expected =
                  new String[] {
                    "--" + boundary,
  ...
  private void testWrite(MockServerHttpRequest request) {
    Mono<Void> mono =
        this.writer.write(this.input, null, null, TEXT_PLAIN, request, this.response, HINTS);
