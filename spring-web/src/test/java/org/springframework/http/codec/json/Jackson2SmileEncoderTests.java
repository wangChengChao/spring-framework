...
public class Jackson2SmileEncoderTests extends AbstractEncoderTestCase<Jackson2SmileEncoder> {
  ...
  private static final MimeType STREAM_SMILE_MIME_TYPE =
      new MimeType("application", "stream+x-jackson-smile");
  ...
  private <T> Consumer<DataBuffer> expect(T expected, Class<T> expectedType) {
    return dataBuffer -> {
      try {
        Object actual =
            this.mapper.reader().forType(expectedType).readValue(dataBuffer.asInputStream());
        assertThat(actual).isEqualTo(expected);
      } catch (IOException e) {
