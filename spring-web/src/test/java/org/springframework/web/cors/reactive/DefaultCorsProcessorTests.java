...
public class DefaultCorsProcessorTests {
  ...
  public void requestWithoutOriginHeader() throws Exception {
    MockServerHttpRequest request =
        MockServerHttpRequest.method(HttpMethod.GET, "http://domain1.example/test.html").build();
    ...
    assertThat(response.getHeaders().get(VARY))
        .contains(ORIGIN, ACCESS_CONTROL_REQUEST_METHOD, ACCESS_CONTROL_REQUEST_HEADERS);
  ...
  public void sameOriginRequest() throws Exception {
    MockServerHttpRequest request =
        MockServerHttpRequest.method(HttpMethod.GET, "http://domain1.example/test.html")
  ...
  public void actualRequestWithOriginHeaderAndAllowedOrigin() throws Exception {
    ...
    assertThat(response.getHeaders().containsKey(HttpHeaders.ACCESS_CONTROL_EXPOSE_HEADERS))
        .isFalse();
    assertThat(response.getHeaders().get(VARY))
        .contains(ORIGIN, ACCESS_CONTROL_REQUEST_METHOD, ACCESS_CONTROL_REQUEST_HEADERS);
    assertThat((Object) response.getStatusCode()).isNull();
  ...
  public void actualRequestCredentials() throws Exception {
    ...
    ServerHttpResponse response = exchange.getResponse();
    assertThat(response.getHeaders().containsKey(ACCESS_CONTROL_ALLOW_ORIGIN)).isTrue();
    assertThat(response.getHeaders().getFirst(ACCESS_CONTROL_ALLOW_ORIGIN))
        .isEqualTo("https://domain2.com");
    assertThat(response.getHeaders().containsKey(HttpHeaders.ACCESS_CONTROL_ALLOW_CREDENTIALS))
        .isTrue();
  ...
  public void preflightRequestAllOriginsAllowed() throws Exception {
    ServerWebExchange exchange =
        MockServerWebExchange.from(preFlightRequest().header(ACCESS_CONTROL_REQUEST_METHOD, "GET"));
    this.conf.addAllowedOrigin("*");
    this.processor.process(this.conf, exchange);
  ...
  public void preflightRequestValidRequestAndConfig() throws Exception {
    ...
    assertThat(response.getHeaders().containsKey(ACCESS_CONTROL_ALLOW_ORIGIN)).isTrue();
    assertThat(response.getHeaders().getFirst(ACCESS_CONTROL_ALLOW_ORIGIN)).isEqualTo("*");
    assertThat(response.getHeaders().containsKey(HttpHeaders.ACCESS_CONTROL_ALLOW_METHODS))
        .isTrue();
    assertThat(response.getHeaders().getFirst(HttpHeaders.ACCESS_CONTROL_ALLOW_METHODS))
        .isEqualTo("GET,PUT");
    assertThat(response.getHeaders().containsKey(HttpHeaders.ACCESS_CONTROL_MAX_AGE)).isFalse();
  ...
  public void preflightRequestAllowedHeaders() throws Exception {
    ...
    assertThat(response.getHeaders().getFirst(ACCESS_CONTROL_ALLOW_HEADERS).contains("Header2"))
        .isTrue();
    assertThat(response.getHeaders().getFirst(ACCESS_CONTROL_ALLOW_HEADERS).contains("Header3"))
        .isFalse();
    assertThat(response.getHeaders().get(VARY))
        .contains(ORIGIN, ACCESS_CONTROL_REQUEST_METHOD, ACCESS_CONTROL_REQUEST_HEADERS);
    assertThat((Object) response.getStatusCode()).isNull();
  ...
  public void preflightRequestAllowsAllHeaders() throws Exception {
    ...
    assertThat(response.getHeaders().getFirst(ACCESS_CONTROL_ALLOW_HEADERS).contains("Header2"))
        .isTrue();
    assertThat(response.getHeaders().getFirst(ACCESS_CONTROL_ALLOW_HEADERS).contains("*"))
        .isFalse();
    assertThat(response.getHeaders().get(VARY))
        .contains(ORIGIN, ACCESS_CONTROL_REQUEST_METHOD, ACCESS_CONTROL_REQUEST_HEADERS);
    assertThat((Object) response.getStatusCode()).isNull();
  ...
  private MockServerHttpRequest.BaseBuilder<?> corsRequest(HttpMethod method) {
    return MockServerHttpRequest.method(method, "http://localhost/test.html")
