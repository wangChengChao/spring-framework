...
public class CorsUtilsTests {
  ...
  public void isSameOriginWithDifferentSchemes() {
    MockServerHttpRequest request =
        MockServerHttpRequest.get("http://mydomain1.example")
  ...
  private void testWithXForwardedHeaders(
    ...
    MockServerHttpRequest.BaseBuilder<?> builder =
        get(url).header(HttpHeaders.ORIGIN, originHeader);
  ...
  private void testWithForwardedHeader(
      String serverName, int port, String forwardedHeader, String originHeader) {
    ...
    MockServerHttpRequest.BaseBuilder<?> builder =
        get(url).header("Forwarded", forwardedHeader).header(HttpHeaders.ORIGIN, originHeader);
  ...
  private ServerHttpRequest adaptFromForwardedHeaders(
    ...
    new org.springframework.web.filter.reactive.ForwardedHeaderFilter()
        .filter(
            exchange,
            exchange2 -> {
