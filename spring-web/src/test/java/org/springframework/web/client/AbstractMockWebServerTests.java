...
abstract class AbstractMockWebServerTests {
  ...
  private MockResponse getRequest(RecordedRequest request, byte[] body, String contentType) {
    if (request.getMethod().equals("OPTIONS")) {
      return new MockResponse()
          .setResponseCode(200)
          .setHeader("Allow", "GET, OPTIONS, HEAD, TRACE");
    }
    Buffer buf = new Buffer();
    buf.write(body);
    MockResponse response =
        new MockResponse().setHeader(CONTENT_LENGTH, body.length).setBody(buf).setResponseCode(200);
    if (contentType != null) {
  ...
  private MockResponse postRequest(
    ...
    assertThat(Integer.parseInt(request.getHeader(CONTENT_LENGTH)))
        .as("Invalid request content-length")
        .isGreaterThan(0);
    ...
    assertThat(request.getBody().readString(charset))
        .as("Invalid request body")
        .isEqualTo(expectedRequestContent);
    Buffer buf = new Buffer();
  ...
  private MockResponse jsonPostRequest(
      RecordedRequest request, String location, String contentType) {
  ...
  private MockResponse multipartFormDataRequest(RecordedRequest request) {
    ...
    assertThat(mediaType.isCompatibleWith(MULTIPART_FORM_DATA))
        .as(MULTIPART_FORM_DATA.toString())
        .isTrue();
  ...
  private MockResponse multipartRelatedRequest(RecordedRequest request) {
    MediaType mediaType = MediaType.parseMediaType(request.getHeader(CONTENT_TYPE));
    assertThat(mediaType.isCompatibleWith(MULTIPART_RELATED))
        .as(MULTIPART_RELATED.toString())
        .isTrue();
    assertMultipart(request, mediaType);
    return new MockResponse().setResponseCode(200);
  }
  ...
  private void assertMultipart(RecordedRequest request, MediaType mediaType) {
    assertThat(mediaType.isCompatibleWith(new MediaType("multipart", "*")))
        .as("multipart/*")
        .isTrue();
  ...
  private MockResponse formRequest(RecordedRequest request) {
    assertThat(request.getHeader(CONTENT_TYPE))
        .isEqualTo("application/x-www-form-urlencoded;charset=UTF-8");
    assertThat(request.getBody().readUtf8())
        .contains("name+1=value+1", "name+2=value+2%2B1", "name+2=value+2%2B2");
    return new MockResponse().setResponseCode(200);
  }

  private MockResponse patchRequest(
      RecordedRequest request,
      String expectedRequestContent,
      String contentType,
      byte[] responseBody) {
    ...
    public MockResponse dispatch(RecordedRequest request) throws InterruptedException {
      try {
        ...
        if (request.getPath().equals("/get")) {
          ...
          return postRequest(
              request, helloWorld, "/post/1", textContentType.toString(), helloWorldBytes);
