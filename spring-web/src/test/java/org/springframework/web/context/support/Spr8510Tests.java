...
/**
 * Tests the interaction between a WebApplicationContext and ContextLoaderListener with regard to
 * config location precedence, overriding and defaulting in programmatic configuration use cases,
 * e.g. with Spring 3.1's WebApplicationInitializer.
 *
 * @author Chris Beams
 * @since 3.1
...
public class Spr8510Tests {
  ...
  public void abstractRefreshableWAC_respectsProgrammaticConfigLocations() {
    ...
    assertThatExceptionOfType(Throwable.class)
        .isThrownBy(() -> cll.contextInitialized(new ServletContextEvent(sc)))
        .withMessageEndingWith("Could not open ServletContext resource [/programmatic.xml]");
  }

  /**
   * If a contextConfigLocation init-param has been specified for the ContextLoaderListener, then it
   * should take precedence. This is generally not a recommended practice, but when it does happen,
   * the init-param should be considered more specific than the programmatic configuration, given
   * that it still quite possibly externalized in hybrid web.xml + WebApplicationInitializer cases.
   */
  @Test
  ...
  /**
   * If setConfigLocation has not been called explicitly against the application context, then fall
   * back to the ContextLoaderListener init-param if present.
   */
  @Test
  ...
  /**
   * If context config locations have been specified neither against the application context nor the
   * context loader listener, then fall back to default values.
   */
  @Test
