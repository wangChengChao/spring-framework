 ...
 *
 * <p>Compatible with Servlet 3.1 but can be configured to expose a specific version through {@link
 * #setMajorVersion}/{@link #setMinorVersion}; default is 3.1. Note that Servlet 3.1 support is
 * limited: servlet, filter and listener registration methods are not supported; neither is JSP
 * configuration. We generally do not recommend to unit test your ServletContainerInitializers and
 * WebApplicationInitializers which is where those registration methods would be used.
 *
 * <p>For setting up a full {@code WebApplicationContext} in a test environment, you can use {@code
 * AnnotationConfigWebApplicationContext}, {@code XmlWebApplicationContext}, or {@code
 * GenericWebApplicationContext}, passing in a corresponding {@code MockServletContext} instance.
 * Consider configuring your {@code MockServletContext} with a {@code FileSystemResourceLoader} in
 * order to interpret resource paths as relative filesystem locations.
 *
 * @author Rod Johnson
...
public class MockServletContext implements ServletContext {
  ...
  @Nullable private Set<SessionTrackingMode> sessionTrackingModes;
  ...
  /**
   * Create a new {@code MockServletContext}, using no base path and a {@link DefaultResourceLoader}
  ...
  /**
   * Create a new {@code MockServletContext}, using the specified {@link ResourceLoader} and no base
   * path.
   *
  ...
  /**
   * Create a new {@code MockServletContext} using the supplied resource base path and resource
   * loader.
   *
   * <p>Registers a {@link MockRequestDispatcher} for the Servlet named {@literal 'default'}.
   *
   * @param resourceBasePath the root directory of the WAR (should not end with a slash)
   * @param resourceLoader the ResourceLoader to use (or null for the default)
  ...
  /**
   * Register a {@link RequestDispatcher} (typically a {@link MockRequestDispatcher}) that acts as a
   ...
   *
   * <p>Also {@link #unregisterNamedDispatcher unregisters} the current default {@link
   * RequestDispatcher} and {@link #registerNamedDispatcher replaces} it with a {@link
   * MockRequestDispatcher} for the provided {@code defaultServletName}.
   *
   * @param defaultServletName the name of the <em>default</em> {@code Servlet}; never {@code null}
   *     or empty
  ...
  public Set<SessionTrackingMode> getEffectiveSessionTrackingModes() {
    return (this.sessionTrackingModes != null
        ? Collections.unmodifiableSet(this.sessionTrackingModes)
