 ...
 *
 * <p>One can override the default Java source compatibility version with a dedicated property on
 * the CLI: {@code "./gradlew test -PjavaSourceVersion=11"}.
 *
 * @author Brian Clozel
 */
public class CompilerConventionsPlugin implements Plugin<Project> {
  ...
  /**
   * The project property that can be used to switch the Java source compatibility version for
   * building source and test classes.
   */
  ...
  static {
    List<String> commonCompilerArgs =
        Arrays.asList(
            "-Xlint:serial",
            "-Xlint:cast",
            "-Xlint:classfile",
  ...
  public void apply(Project project) {
    project
        .getPlugins()
        .withType(JavaPlugin.class, javaPlugin -> applyJavaCompileConventions(project));
  }
  ...
  private void applyJavaCompileConventions(Project project) {
    ...
    if (project.hasProperty(JAVA_SOURCE_VERSION_PROPERTY)) {
      JavaVersion javaSourceVersion =
          JavaVersion.toVersion(project.property(JAVA_SOURCE_VERSION_PROPERTY));
    ...
    project
        ...
        .withType(JavaCompile.class)
        .matching(
            javaCompile -> javaCompile.getName().equals(JavaPlugin.COMPILE_TEST_JAVA_TASK_NAME))
        .forEach(
            compileTask -> {
