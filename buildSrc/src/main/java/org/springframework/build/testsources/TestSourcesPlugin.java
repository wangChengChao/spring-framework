...
/**
 * {@link Plugin} that automatically updates testCompile dependencies to include the test source
 * sets of {@code project()} dependencies.
 *
 * <p>This plugin is used in the Spring Framework build to share test utilities and fixtures between
 * projects.
 *
 * @author Phillip Webb
 * @author Brian Clozel
...
public class TestSourcesPlugin implements Plugin<Project> {
  ...
  private void collectProjectDependencies(
      Set<ProjectDependency> projectDependencies, Project project) {
                ...
                dependency -> {
                  if (dependency instanceof ProjectDependency) {
                    ...
                    collectProjectDependencies(
                        projectDependencies, projectDependency.getDependencyProject());
                  }
                });
      }
    }
  ...
  private void addTestSourcesFromDependency(
      final Project currentProject, ProjectDependency dependency) {
        ...
        .withType(
            ...
            plugin -> {
              final JavaPluginConvention javaPlugin =
                  dependencyProject.getConvention().getPlugin(JavaPluginConvention.class);
              SourceSetOutput test =
                  javaPlugin.getSourceSets().findByName(SourceSet.TEST_SOURCE_SET_NAME).getOutput();
              currentProject
                  .getDependencies()
                  .add(JavaPlugin.TEST_COMPILE_CONFIGURATION_NAME, test);
            });
  }
}
