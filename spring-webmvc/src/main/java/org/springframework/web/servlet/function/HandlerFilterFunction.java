...
public interface HandlerFilterFunction<T extends ServerResponse, R extends ServerResponse> {
  ...
  /**
   * Apply this filter to the given handler function. The given {@linkplain HandlerFunction handler
   * function} represents the next entity in the chain, and can be {@linkplain
   * HandlerFunction#handle(ServerRequest) invoked} in order to proceed to this entity, or not
   * invoked to block the chain.
   *
  ...
  /**
   * Return a composed filter function that first applies this filter, and then applies the {@code
   * after} filter.
   *
   * @param after the filter to apply after this filter is applied
   * @return a composed filter that first applies this function and then applies the {@code after}
   *     function
   */
   ...
   * {@code ServerResponse} on a given exception.
   *
   ...
   */
  static <T extends ServerResponse> HandlerFilterFunction<T, T> ofErrorHandler(
      Predicate<Throwable> predicate, BiFunction<Throwable, ServerRequest, T> errorHandler) {
