...
/**
 * The {@code <url>} tag creates URLs. Modeled after the JSTL {@code c:url} tag with backwards
 * compatibility in mind.
 *
 * <p>Enhancements to the JSTL functionality include:
 *
 * <ul>
 *   <li>URL encoded template URI variables
 *   <li>HTML/XML escaping of URLs
 *   <li>JavaScript escaping of URLs
 * </ul>
 *
 * <p>Template URI variables are indicated in the {@link #setValue(String) 'value'} attribute and
 * marked by braces '{variableName}'. The braces and attribute name are replaced by the URL encoded
 * value of a parameter defined with the spring:param tag in the body of the url tag. If no
 * parameter is available the literal value is passed through. Params matched to template variables
 * will not be added to the query string.
 *
 * <p>Use of the spring:param tag for URI template variables is strongly recommended over direct EL
 * substitution as the values are URL encoded. Failure to properly encode URL can leave an
 * application vulnerable to XSS and other injection attacks.
 *
 * <p>URLs can be HTML/XML escaped by setting the {@link #setHtmlEscape(boolean) 'htmlEscape'}
 * attribute to 'true'. Detects an HTML escaping setting, either on this tag instance, the page
 * level, or the {@code web.xml} level. The default is 'false'. When setting the URL value into a
 * variable, escaping is not recommended.
 *
 * <p>Example usage:
 *
 ...
 * &lt;/spring:url&gt;</pre>
 *
 * <p>The above results in: {@code /currentApplicationContext/url/path/more%20than%20JSTL%20c%3Aurl}
 *
 * <table>
...
public class UrlTag extends HtmlEscapingAwareTag implements ParamAware {
  ...
  /**
   * Set the variable name to expose the URL under. Defaults to rendering the URL to the current
   * JspWriter
   */
  ...
  /**
   * Set the scope to export the URL variable to. This attribute has no meaning unless var is also
   * defined.
   */
  ...
  /**
   * Build the query string from available parameters that have not already been applied as template
   * params.
   *
  ...
  /**
   * Replace template markers in the URL matching available parameters. The name of matched
   * parameters are added to the used parameters set.
   *
  ...
  protected String replaceUriTemplateParams(String uri, List<Param> params, Set<String> usedParams)
    ...
    for (Param param : params) {
      String template =
          URL_TEMPLATE_DELIMITER_PREFIX + param.getName() + URL_TEMPLATE_DELIMITER_SUFFIX;
      if (uri.contains(template)) {
        ...
        try {
          uri =
              StringUtils.replace(
                  uri, template, (value != null ? UriUtils.encodePath(value, encoding) : ""));
        } catch (UnsupportedCharsetException ex) {
          throw new JspException(ex);
        }
      } else {
