...
/**
 * The {@code <eval>} tag evaluates a Spring expression (SpEL) and either prints the result or
 * assigns it to a variable. Supports the standard JSP evaluation context consisting of implicit
...
public class EvalTag extends HtmlEscapingAwareTag {
  ...
  /**
   * {@link javax.servlet.jsp.PageContext} attribute for the page-level {@link EvaluationContext}
   * instance.
   */
  ...
  /**
   * Set the variable name to expose the evaluation result under. Defaults to rendering the result
  ...
  /**
   * Set the scope to export the evaluation result to. This attribute has no meaning unless var is
   * also defined.
   */
  ...
  public int doEndTag() throws JspException {
    ...
    if (this.var != null) {
      Object result =
          (this.expression != null ? this.expression.getValue(evaluationContext) : null);
      this.pageContext.setAttribute(this.var, result, this.scope);
    } else {
      try {
        String result =
            (this.expression != null
                ? this.expression.getValue(evaluationContext, String.class)
                : null);
  ...
  private EvaluationContext createEvaluationContext(PageContext pageContext) {
    ...
    context.setBeanResolver(
        new BeanFactoryResolver(getRequestContext().getWebApplicationContext()));
  ...
  private ConversionService getConversionService(PageContext pageContext) {
    return (ConversionService)
        pageContext.getRequest().getAttribute(ConversionService.class.getName());
  }
  ...
  private static class JspPropertyAccessor implements PropertyAccessor {
    ...
    @Nullable private final javax.servlet.jsp.el.VariableResolver variableResolver;
    ...
    public boolean canRead(EvaluationContext context, @Nullable Object target, String name)
      ...
      return (target == null
          && (resolveImplicitVariable(name) != null
