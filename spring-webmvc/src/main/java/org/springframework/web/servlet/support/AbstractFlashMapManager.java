...
public abstract class AbstractFlashMapManager implements FlashMapManager {
  ...
  /**
   * Set the amount of time in seconds after a {@link FlashMap} is saved (at request completion) and
   * before it expires.
   *
  ...
  @Override
  @Nullable
  public final FlashMap retrieveAndUpdate(
  ...
  /**
   * Whether the given FlashMap matches the current request. Uses the expected request path and
   * query parameters saved in the FlashMap.
   */
  ...
  @Override
  public final void saveOutputFlashMap(
  ...
  /**
   * Obtain a mutex for modifying the FlashMap List as handled by {@link #retrieveFlashMaps} and
   * {@link #updateFlashMaps},
   *
   * <p>The default implementation returns a shared static mutex. Subclasses are encouraged to
   * return a more specific mutex, or {@code null} to indicate that no synchronization is necessary.
   *
   * @param request the current request
