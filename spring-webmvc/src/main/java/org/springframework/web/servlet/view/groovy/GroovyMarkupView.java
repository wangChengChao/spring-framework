...
public class GroovyMarkupView extends AbstractTemplateView {
  ...
  @Nullable private MarkupTemplateEngine engine;

  /**
   * Set the MarkupTemplateEngine to use in this view.
   *
   * <p>If not set, the engine is auto-detected by looking up a single {@link GroovyMarkupConfig}
   * bean in the web application context and using it to obtain the configured {@code
   * MarkupTemplateEngine} instance.
   *
  ...
  /**
   * Invoked at startup. If no {@link #setTemplateEngine(MarkupTemplateEngine) templateEngine} has
   * been manually set, this method looks up a {@link GroovyMarkupConfig} bean by type and uses it
   * to obtain the Groovy Markup template engine.
   *
   * @see GroovyMarkupConfig
  ...
  /**
   * Autodetect a MarkupTemplateEngine via the ApplicationContext. Called if a MarkupTemplateEngine
  ...
  protected MarkupTemplateEngine autodetectMarkupTemplateEngine() throws BeansException {
    try {
      return BeanFactoryUtils.beanOfTypeIncludingAncestors(
              obtainApplicationContext(), GroovyMarkupConfig.class, true, false)
          .getTemplateEngine();
    } catch (NoSuchBeanDefinitionException ex) {
      throw new ApplicationContextException(
          "Expected a single GroovyMarkupConfig bean in the current "
  ...
  /**
   * Return a template compiled by the configured Groovy Markup template engine for the given view
   * URL.
   */
