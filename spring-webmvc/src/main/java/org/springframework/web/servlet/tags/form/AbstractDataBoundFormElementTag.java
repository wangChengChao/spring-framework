 ...
 *
 * <p>Provides the common {@link #setPath path} and {@link #setId id} properties. Provides
 * sub-classes with utility methods for accessing the {@link BindStatus} of their bound value and
 * also for {@link #writeOptionalAttribute interacting} with the {@link TagWriter}.
 *
 * @author Rob Harrop
...
public abstract class AbstractDataBoundFormElementTag extends AbstractFormTag
  ...
  /**
   * Name of the exposed path variable within the scope of this tag: "nestedPath". Same value as
   * {@link org.springframework.web.servlet.tags.NestedPathTag#NESTED_PATH_VARIABLE_NAME}.
   */
  ...
  @Nullable private BindStatus bindStatus;

  /**
   * Set the property path from the {@link FormTag#setModelAttribute form object}. May be a runtime
   * expression.
   */
  ...
  /**
   * Get the {@link #evaluate resolved} property path for the {@link FormTag#setModelAttribute form
   * object}.
   */
   ...
   *
   * <p>May be a runtime expression; defaults to the value of {@link #getName()}. Note that the
   * default value may not be valid for certain tags.
   */
  @Override
  ...
  /**
   * Writes the default set of attributes to the supplied {@link TagWriter}. Further abstract
   * sub-classes should override this method to add in any additional default attributes but
   * <strong>must</strong> remember to call the {@code super} method.
   *
   ...
   *
   * <p>The default implementation simply delegates to {@link #getPropertyPath()} to use the
   * property path as the name. For the most part this is desirable as it links with the server-side
   * expectation for data binding. However, some subclasses may wish to change the value of the
   * '{@code name}' attribute without changing the bind path.
   *
  ...
  /**
   * Build the property path for this tag, including the nested path but <i>not</i> prefixed with
  ...
  /**
   * Get a display String for the given value, converted by a PropertyEditor that the BindStatus may
   * have registered for the value's Class.
   */
  ...
  /**
   * Process the given form field through a {@link RequestDataValueProcessor} instance if one is
   * configured or otherwise returns the same value.
   */
