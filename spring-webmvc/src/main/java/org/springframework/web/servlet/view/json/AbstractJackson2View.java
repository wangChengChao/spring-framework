...
public abstract class AbstractJackson2View extends AbstractView {
   ...
   * ObjectMapper#ObjectMapper() ObjectMapper} will be used.
   *
   * <p>Setting a custom-configured {@code ObjectMapper} is one way to take further control of the
   * JSON serialization process. The other option is to use Jackson's provided annotations on the
  ...
  /**
   * Set the {@code JsonEncoding} for this view. By default, {@linkplain JsonEncoding#UTF8 UTF-8} is
   * used.
   */
  ...
  /**
   * Whether to use the default pretty printer when writing the output. This is a shortcut for
  ...
  /**
   * Whether to update the 'Content-Length' header of the response. When set to {@code true}, the
   * response is buffered in order to determine the content length and set the 'Content-Length'
  ...
  /**
   * Set the attribute in the model that should be rendered by this view. When set, all other model
