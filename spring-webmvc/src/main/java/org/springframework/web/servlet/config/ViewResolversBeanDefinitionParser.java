...
/**
 * Parse the {@code view-resolvers} MVC namespace element and register {@link
 * org.springframework.web.servlet.ViewResolver} bean definitions.
 *
 * <p>All registered resolvers are wrapped in a single (composite) ViewResolver with its order
 * property set to 0 so that other external resolvers may be ordered before or after it.
 *
 * <p>When content negotiation is enabled the order property is set to highest priority instead with
 * the ContentNegotiatingViewResolver encapsulating all other registered view resolver instances.
 * That way the resolvers registered through the MVC namespace form self-encapsulated resolver
 * chain.
 *
...
public class ViewResolversBeanDefinitionParser implements BeanDefinitionParser {
  ...
  public BeanDefinition parse(Element element, ParserContext context) {
    ...
    RootBeanDefinition compositeResolverBeanDef =
        new RootBeanDefinition(ViewResolverComposite.class);
    ...
    context
        .getReaderContext()
        .getRegistry()
        .registerBeanDefinition(beanName, compositeResolverBeanDef);
    context.registerComponent(new BeanComponentDefinition(compositeResolverBeanDef, beanName));
    context.popAndRegisterContainingComponent();
    return null;
  }

  private void addUrlBasedViewResolverProperties(
      Element element, RootBeanDefinition beanDefinition) {
  ...
  private BeanDefinition createContentNegotiatingViewResolver(
      Element resolverElement, ParserContext context) {
