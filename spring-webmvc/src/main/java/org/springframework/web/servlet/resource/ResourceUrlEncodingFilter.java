...
/**
 * A filter that wraps the {@link HttpServletResponse} and overrides its {@link
 * HttpServletResponse#encodeURL(String) encodeURL} method in order to translate internal resource
 * request URLs into public URL paths for external use.
 *
...
public class ResourceUrlEncodingFilter extends GenericFilterBean {
  ...
  private static class ResourceUrlEncodingRequestWrapper extends HttpServletRequestWrapper {
    ...
    public String resolveUrlPath(String url) {
      if (this.resourceUrlProvider == null) {
        logger.trace(
            "ResourceUrlProvider not available via request attribute "
  ...
  private static class ResourceUrlEncodingResponseWrapper extends HttpServletResponseWrapper {
    ...
    ResourceUrlEncodingResponseWrapper(
        ResourceUrlEncodingRequestWrapper request, HttpServletResponse wrapped) {
      super(wrapped);
  ...
  /**
   * Runtime exception to get far enough (to ResourceUrlProviderExposingInterceptor) where it can be
   * re-thrown as ServletRequestBindingException to result in a 400 response.
   */
