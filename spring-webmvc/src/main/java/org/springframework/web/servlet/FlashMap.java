...
/**
 * A FlashMap provides a way for one request to store attributes intended for use in another. This
 * is most commonly needed when redirecting from one URL to another -- e.g. the Post/Redirect/Get
 * pattern. A FlashMap is saved before the redirect (typically in the session) and is made available
 * after the redirect and removed immediately.
 *
 * <p>A FlashMap can be set up with a request path and request parameters to help identify the
 * target request. Without this information, a FlashMap is made available to the next request, which
 * may or may not be the intended recipient. On a redirect, the target URL is known and a FlashMap
 * can be updated with that information. This is done automatically when the {@code
 * org.springframework.web.servlet.view.RedirectView} is used.
 *
 * <p>Note: annotated controllers will usually not use FlashMap directly. See {@code
 * org.springframework.web.servlet.mvc.support.RedirectAttributes} for an overview of using flash
 * attributes in annotated controllers.
 *
...
public final class FlashMap extends HashMap<String, Object> implements Comparable<FlashMap> {
   ...
   *
   * <p>The path may be absolute (e.g. "/application/resource") or relative to the current request
   * (e.g. "../resource").
   */
  ...
  /**
   * Set the expiration time for the FlashMap. This is provided for serialization purposes but can
   * also be used instead {@link #startExpirationPeriod(int)}.
   *
   * @since 4.2
   */
  ...
  /**
   * Return whether this instance has expired depending on the amount of elapsed time since the call
   * to {@link #startExpirationPeriod}.
   */
  ...
  /**
   * Compare two FlashMaps and prefer the one that specifies a target URL path or has more target
   * URL parameters. Before comparing FlashMap instances ensure that they match a given request.
   */
  @Override
  ...
  public String toString() {
    return "FlashMap [attributes="
        + super.toString()
        + ", targetRequestPath="
        + this.targetRequestPath
        + ", targetRequestParams="
