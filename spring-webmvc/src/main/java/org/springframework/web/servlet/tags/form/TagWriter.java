...
public class TagWriter {
  ...
  /**
   * Create a new instance of the {@link TagWriter} class that writes to the supplied {@link
   * PageContext}.
   *
  ...
  /**
   * Start a new tag with the supplied name. Leaves the tag open so that attributes, inner text or
   * nested tags can be written into it.
   *
   * @see #endTag()
   */
   ...
   *
   * <p>Be sure to write all attributes <strong>before</strong> writing any inner text or nested
   * tags.
   *
   ...
   *
   * <p>Useful when you plan to write additional content in the body outside the context of the
   * current {@link TagWriter}.
   */
   ...
   *
   * <p>Correctly writes an empty tag if no inner text or nested tags have been written.
   */
   ...
   * <p>Correctly writes an empty tag if no inner text or nested tags have been written.
   *
   * @param enforceClosingTag whether a full closing tag should be rendered in any case, even in
   *     case of a non-block tag
   */
  ...
  private static final class SafeWriter {
    ...
    @Nullable private PageContext pageContext;
