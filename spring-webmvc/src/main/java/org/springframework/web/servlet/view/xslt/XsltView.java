...
/**
 * XSLT-driven View that allows for response context to be rendered as the result of an XSLT
 * transformation.
 *
 * <p>The XSLT Source object is supplied as a parameter in the model and then {@link #locateSource
 * detected} during response rendering. Users can either specify a specific entry in the model via
 * the {@link #setSourceKey sourceKey} property or have Spring locate the Source object. This class
 * also provides basic conversion of objects into Source implementations. See {@link
 * #getSourceTypes() here} for more details.
 *
 * <p>All model parameters are passed to the XSLT Transformer as parameters. In addition the user
 * can configure {@link #setOutputProperties output properties} to be passed to the Transformer.
 *
 * @author Rob Harrop
...
public class XsltView extends AbstractUrlBasedView {
   ...
   *
   * <p>The default constructor of the specified class will be called to build the
   * TransformerFactory for this view.
   */
  ...
  /**
   * Set the name of the model attribute that represents the XSLT Source. If not specified, the
   * model map will be searched for a matching value type.
   *
   * <p>The following source types are supported out of the box: {@link Source}, {@link Document},
   * {@link Node}, {@link Reader}, {@link InputStream} and {@link Resource}.
   *
   * @see #getSourceTypes
   * @see #convertSource
   */
  ...
  /**
   * Set an implementation of the {@link javax.xml.transform.ErrorListener} interface for custom
   * handling of transformation errors and warnings.
   *
   * <p>If not set, a default {@link org.springframework.util.xml.SimpleTransformErrorListener} is
   * used that simply logs warnings using the logger instance of the view class, and rethrows errors
  ...
  public void setErrorListener(@Nullable ErrorListener errorListener) {
    this.errorListener =
        (errorListener != null ? errorListener : new SimpleTransformErrorListener(logger));
  }

  /**
   * Set whether the XSLT transformer may add additional whitespace when outputting the result tree.
   *
   * <p>Default is {@code true} (on); set this to {@code false} (off) to not specify an "indent"
   * key, leaving the choice up to the stylesheet.
   *
   ...
   *
   * <p>Any values specified here will override defaults that this view sets programmatically.
   *
   ...
   *
   * <p>The default value is "true". Only set this to "false" in development, where caching does not
   ...
   *
   * <p>The default implementation simply calls {@link
   * javax.xml.transform.TransformerFactory#newInstance()}. If a {@link #setTransformerFactoryClass
   * "transformerFactoryClass"} has been specified explicitly, the default constructor of the
    ...
    if (transformerFactoryClass != null) {
      try {
        ...
        throw new TransformerFactoryConfigurationError(
            ex, "Could not instantiate TransformerFactory");
      }
    } else {
  ...
  /**
   * Locate the {@link Source} object in the supplied model, converting objects as required. The
   * default implementation first attempts to look under the configured {@link #setSourceKey source
   * key}, if any, before attempting to locate an object of {@link #getSourceTypes() supported
   * type}.
   *
  ...
  /**
   * Return the array of {@link Class Classes} that are supported when converting to an XSLT {@link
   * Source}.
   *
   * <p>Currently supports {@link Source}, {@link Document}, {@link Node}, {@link Reader}, {@link
   * InputStream} and {@link Resource}.
   *
  ...
  /**
   * Convert the supplied {@link Object} into an XSLT {@link Source} if the {@link Object} type is
   * {@link #getSourceTypes() supported}.
   *
  ...
  protected Source convertSource(Object source) throws Exception {
    if (source instanceof Source) {
      ...
      throw new IllegalArgumentException(
          "Value '" + source + "' cannot be converted to XSLT Source");
    }
  }
   ...
   *
   * <p>The default implementation copies parameters from the model into the Transformer's {@link
   * Transformer#setParameter parameter set}. This implementation also copies the {@link
   * #setOutputProperties output properties} into the {@link Transformer} {@link
   * Transformer#setOutputProperty output properties}. Indentation properties are set as well.
   *
  ...
  protected void configureTransformer(
      Map<String, Object> model, HttpServletResponse response, Transformer transformer) {
  ...
  /**
   * Copy the configured output {@link Properties}, if any, into the {@link
   * Transformer#setOutputProperty output property set} of the supplied {@link Transformer}.
   *
   * @param transformer the target transformer
   */
  ...
  /**
   * Copy all entries from the supplied Map into the {@link Transformer#setParameter(String, Object)
   * parameter set} of the supplied {@link Transformer}.
   *
   * @param model merged output Map (never {@code null})
   * @param transformer the target transformer
   */
   ...
   *
   * <p>The default implementation of this method sets the {@link HttpServletResponse#setContentType
   * content type} and {@link HttpServletResponse#setCharacterEncoding encoding} from the
   * "media-type" and "encoding" output properties specified in the {@link Transformer}.
   *
   * @param model merged output Map (never {@code null})
   * @param response current HTTP response
   * @param transformer the target transformer
  ...
  protected void configureResponse(
    ...
    if (StringUtils.hasText(encoding)) {
      // Only apply encoding if content type is specified but does not contain charset clause
      // already.
      if (contentType != null
   ...
   *
   * <p>The default implementation simply calls {@link Templates#newTransformer()}, and configures
   * the {@link Transformer} with the custom {@link URIResolver} if specified.
   *
