...
/**
 * Utility class for testing whether a candidate value matches a {@link BindStatus#getValue data
 * bound value}. Eagerly attempts to prove a comparison through a number of avenues to deal with
 * issues such as instance inequality, logical (String-representation-based) equality and {@link
 * PropertyEditor}-based comparison.
 *
 * <p>Full support is provided for comparing arrays, {@link Collection Collections} and {@link Map
 * Maps}.
 *
 * <p>
 *
 ...
 *
 * For single-valued objects equality is first tested using standard {@link Object#equals Java
 * equality}. As such, user code should endeavour to implement {@link Object#equals} to speed up the
 * comparison process. If {@link Object#equals} returns {@code false} then an attempt is made at an
 * {@link #exhaustiveCompare exhaustive comparison} with the aim being to <strong>prove</strong>
 * equality rather than disprove it.
 *
 * <p>Next, an attempt is made to compare the {@code String} representations of both the candidate
 * and bound values. This may result in {@code true} in a number of cases due to the fact both
 * values will be represented as {@code Strings} when shown to the user.
 *
 * <p>Next, if the candidate value is a {@code String}, an attempt is made to compare the bound
 * value to result of applying the corresponding {@link PropertyEditor} to the candidate. This
 * comparison may be executed twice, once against the direct {@code String} instances, and then
 * against the {@code String} representations if the first comparison results in {@code false}.
 *
 * @author Rob Harrop
...
abstract class SelectedValueComparator {
  ...
  /**
   * Returns {@code true} if the supplied candidate value is equal to the value bound to the
   * supplied {@link BindStatus}. Equality in this case differs from standard Java equality and is
   * described in more detail <a href="#equality-contract">here</a>.
   */
  public static boolean isSelected(BindStatus bindStatus, @Nullable Object candidateValue) {
    ...
    if (candidateValue != null) {
      if (boundValue.getClass().isArray()) {
        selected =
            collectionCompare(CollectionUtils.arrayToList(boundValue), candidateValue, bindStatus);
  ...
  private static boolean mapCompare(
      Map<?, ?> boundMap, Object candidateValue, BindStatus bindStatus) {
    try {
  ...
  private static boolean exhaustiveCompare(
      @Nullable Object boundValue,
      @Nullable Object candidate,
      @Nullable PropertyEditor editor,
      @Nullable Map<PropertyEditor, Object> convertedValueCache) {
