 ...
 *
 * <p>Error views are analogous to error page JSPs, but can be used with any kind of exception
...
public class SimpleMappingExceptionResolver extends AbstractHandlerExceptionResolver {
  ...
  @Nullable private String exceptionAttribute = DEFAULT_EXCEPTION_ATTRIBUTE;

  /**
   * Set the mappings between exception class names and error view names. The exception class name
   * can be a substring, with no wildcard support at present. A value of "ServletException" would
   * match {@code javax.servlet.ServletException} and subclasses, for example.
   *
   ...
   * will probably hide other rules. "java.lang.Exception" would be correct if "Exception" was meant
   * to define a rule for all checked exceptions. With more unusual exception names such as
   * "BaseBusinessException" there's no need to use a FQN.
   *
   * @param mappings exception patterns (can also be fully qualified class names) as keys, and error
  ...
  /**
   * Set one or more exceptions to be excluded from the exception mappings. Excluded exceptions are
   * checked first and if one of them equals the actual exception, the exception will remain
   * unresolved.
   *
  ...
  /**
   * Set the HTTP status code that this exception resolver will apply for a given resolved error
   * view. Keys are view names; values are status codes.
   *
   * <p>Note that this error code will only get applied in case of a top-level request. It will not
   * be set for an include request, since the HTTP status cannot be modified from within an include.
   *
  ...
  public void setStatusCodes(Properties statusCodes) {
    for (Enumeration<?> enumeration = statusCodes.propertyNames();
  ...
  /**
   * Returns the HTTP status codes provided via {@link #setStatusCodes(Properties)}. Keys are view
   * names; values are status codes.
   */
  ...
  /**
   * Set the default HTTP status code that this exception resolver will apply if it resolves an
   * error view and if there is no status code mapping defined.
   *
   * <p>Note that this error code will only get applied in case of a top-level request. It will not
   * be set for an include request, since the HTTP status cannot be modified from within an include.
   *
   * <p>If not specified, no status code will be applied, either leaving this to the controller or
   * view, or keeping the servlet engine's default of 200 (OK).
   *
   * @param defaultStatusCode the HTTP status code value, for example 500 ({@link
   *     HttpServletResponse#SC_INTERNAL_SERVER_ERROR}) or 404 ({@link
  ...
  /**
   * Set the name of the model attribute as which the exception should be exposed. Default is
   * "exception".
   *
   * <p>This can be either set to a different attribute name or to {@code null} for not exposing an
   * exception attribute at all.
   *
   ...
   *     processing in the resolution chain
   */
  @Override
  @Nullable
   ...
   *
   * <p>0 means ex matches exactly. Returns -1 if there's no match. Otherwise, returns depth. Lowest
   * depth wins.
   */
   ...
   *
   * <p>The default implementation returns the status code for the given view name (specified
   * through the {@link #setStatusCodes(Properties) statusCodes} property), or falls back to the
   ...
   * @return the HTTP status code to use, or {@code null} for the servlet container's default (200
   *     in case of a standard error view)
  ...
  protected ModelAndView getModelAndView(
      String viewName, Exception ex, HttpServletRequest request) {
   ...
   *
   * <p>The default implementation adds the specified exception attribute. Can be overridden in
   * subclasses.
   *
   * @param viewName the name of the error view
   * @param ex the exception that got thrown during handler execution
