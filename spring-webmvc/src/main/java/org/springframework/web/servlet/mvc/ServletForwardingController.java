...
/**
 * Spring Controller implementation that forwards to a named servlet, i.e. the "servlet-name" in
 * web.xml rather than a URL path mapping. A target servlet doesn't even need a "servlet-mapping" in
 * web.xml in the first place: A "servlet" declaration is sufficient.
 *
 * <p>Useful to invoke an existing servlet via Spring's dispatching infrastructure, for example to
 * apply Spring HandlerInterceptors to its requests. This will work even in a minimal Servlet
 * container that does not support Servlet filters.
 *
 * <p><b>Example:</b> web.xml, mapping all "/myservlet" requests to a Spring dispatcher. Also
 ...
 * &lt;/servlet-mapping&gt;</pre>
 *
 * <b>Example:</b> myDispatcher-servlet.xml, in turn forwarding "/myservlet" to your servlet
 * (identified by servlet name). All such requests will go through the configured HandlerInterceptor
 * chain (e.g. an OpenSessionInViewInterceptor). From the servlet point of view, everything will
 * work as usual.
 *
 * <pre class="code">
...
public class ServletForwardingController extends AbstractController implements BeanNameAware {
  ...
  /**
   * Set the name of the servlet to forward to, i.e. the "servlet-name" of the target servlet in
   * web.xml.
   *
