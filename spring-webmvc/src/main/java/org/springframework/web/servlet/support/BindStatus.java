...
/**
 * Simple adapter to expose the bind status of a field or object. Set as a variable both by the JSP
 * bind tag and FreeMarker macros.
 *
 * <p>Obviously, object status representations (i.e. errors at the object level rather than the
 * field level) do not have an expression and a value but only error codes and messages. For
 * simplicity's sake and to be able to use the same tags and macros, the same status class is used
 * for both scenarios.
 *
 * @author Rod Johnson
...
public class BindStatus {
  ...
  @Nullable private final Errors errors;

  private final String[] errorCodes;

  @Nullable private String[] errorMessages;

  @Nullable private List<? extends ObjectError> objectErrors;

  @Nullable private Object value;

  @Nullable private Class<?> valueType;

  @Nullable private Object actualValue;

  @Nullable private PropertyEditor editor;

  @Nullable private BindingResult bindingResult;
  ...
  /**
   * Return a bind expression that can be used in HTML forms as input name for the respective field,
   * or {@code null} if not field-specific.
   *
   * <p>Returns a bind path appropriate for resubmission, e.g. "address.street". Note that the
   * complete bind path as required by the bind tag is "customer.address.street", if bound to a
   * "customer" bean.
   */
  @Nullable
  ...
  /**
   * Return the current value of the field, i.e. either the property value or a rejected update, or
   * {@code null} if not field-specific.
   *
   * <p>This value will be an HTML-escaped String if the original value already was a String.
   */
  @Nullable
  ...
  /**
   * Get the '{@code Class}' type of the field. Favor this instead of '{@code
   * getValue().getClass()}' since '{@code getValue()}' may return '{@code null}'.
   */
  @Nullable
  ...
  /**
   * Return the actual value of the field, i.e. the raw property value, or {@code null} if not
   * available.
   */
  @Nullable
  ...
  /**
   * Return a suitable display value for the field, i.e. the stringified value if not null, and an
   * empty string in case of a null value.
   *
   * <p>This value will be an HTML-escaped String if the original value was non-null: the {@code
   * toString} result of the original value will get HTML-escaped.
   */
  public String getDisplayValue() {
    ...
    if (this.value != null) {
      return (this.htmlEscape
          ? HtmlUtils.htmlEscape(this.value.toString())
          : this.value.toString());
    }
    return "";
  }
  ...
  /**
   * Return the error codes for the field or object, if any. Returns an empty array instead of null
   * if none.
   */
  ...
  /**
   * Return the resolved error messages for the field or object, if any. Returns an empty array
   * instead of null if none.
   */
  ...
  /**
   * Return the Errors instance (typically a BindingResult) that this bind status is currently
   * associated with.
   *
  ...
  /**
   * Find a PropertyEditor for the given value class, associated with the property that this bound
   * status is currently bound to.
   *
  ...
  public PropertyEditor findEditor(Class<?> valueClass) {
    return (this.bindingResult != null
        ? this.bindingResult.findEditor(this.expression, valueClass)
        : null);
  }
