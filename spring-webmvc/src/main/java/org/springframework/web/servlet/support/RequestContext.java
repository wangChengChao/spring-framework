 ...
 * Errors instances.
 *
 * <p>Suitable for exposition to views, and usage within JSP's "useBean" tag, JSP scriptlets, JSTL
 * EL, etc. Necessary for views that do not have access to the servlet request, like FreeMarker
 * templates.
 *
 * <p>Can be instantiated manually, or automatically exposed to views as model attribute via
...
public class RequestContext {
  ...
  /**
   * Default theme name used if the RequestContext cannot find a ThemeResolver. Only applies to
   * non-DispatcherServlet requests.
   *
   * <p>Same as AbstractThemeResolver's default, but not linked in here to avoid package
   * interdependencies.
   *
  ...
  /**
   * Request attribute to hold the current web application context for RequestContext usage. By
   * default, the DispatcherServlet's context (or the root context as fallback) is exposed.
   */
  public static final String WEB_APPLICATION_CONTEXT_ATTRIBUTE =
      RequestContext.class.getName() + ".CONTEXT";
  ...
  @Nullable private RequestDataValueProcessor requestDataValueProcessor;

  @Nullable private Map<String, Errors> errorsMap;

  /**
   * Create a new RequestContext for the given request, using the request attributes for Errors
   * retrieval.
   *
   * <p>This only works with InternalResourceViews, as Errors instances are part of the model and
   * not normally exposed as request attributes. It will typically be used within JSPs or custom
   * tags.
   *
   * <p><b>Will only work within a DispatcherServlet request.</b> Pass in a ServletContext to be
   * able to fallback to the root WebApplicationContext.
   *
   ...
   * @see org.springframework.web.servlet.DispatcherServlet
   * @see #RequestContext(javax.servlet.http.HttpServletRequest,
   *     javax.servlet.http.HttpServletResponse, javax.servlet.ServletContext, Map)
   */
  ...
  /**
   * Create a new RequestContext for the given request, using the given model attributes for Errors
   * retrieval.
   *
    ...
    if (wac == null) {
      ...
      if (wac == null) {
        throw new IllegalStateException(
            "No WebApplicationContext found: not in a DispatcherServlet "
  ...
  /**
   * Return the underlying HttpServletRequest. Only intended for cooperating classes in this
   * package.
   */
   ...
   *
   * <p>Typically coming from a DispatcherServlet's {@link LocaleResolver}. Also includes a fallback
   * check for JSTL's Locale attribute.
   *
   ...
   *
   * <p>Typically coming from a DispatcherServlet's {@link LocaleContextResolver}. Also includes a
   * fallback check for JSTL's TimeZone attribute.
   *
   ...
   * RequestContext.
   *
   * <p>The default is the application-wide setting (the "defaultHtmlEscape" context-param in
   * web.xml).
   *
  ...
  /**
   * Is default HTML escaping active? Falls back to {@code false} in case of no explicit default
   * given.
   */
  ...
  /**
   * Return the default HTML escape setting, differentiating between no default specified and an
   * explicit value.
   *
  ...
  /**
   * Is HTML escaping using the response encoding by default? If enabled, only XML markup
   * significant characters will be escaped with UTF-* encodings.
   *
  ...
  public boolean isResponseEncodedHtmlEscape() {
    return (this.responseEncodedHtmlEscape == null
        || this.responseEncodedHtmlEscape.booleanValue());
  }
   ...
   *
   * @return whether default use of response encoding HTML escaping is enabled (null = no explicit
   *     default)
   * @since 4.1.2
   */
  @Nullable
  ...
  /**
   * Set the UrlPathHelper to use for context path and request URI decoding. Can be used to pass a
   * shared UrlPathHelper instance in.
   *
  ...
  /**
   * Return the UrlPathHelper used for context path and request URI decoding. Can be used to
   * configure the current UrlPathHelper.
   *
   * <p>A default UrlPathHelper is always available.
   */
  ...
  /**
   * Return the RequestDataValueProcessor instance to use obtained from the WebApplicationContext
   * under the name {@code "requestDataValueProcessor"}. Or {@code null} if no matching bean was
   * found.
   */
  @Nullable
  ...
  /**
   * Return the context path of the original request, that is, the path that indicates the current
   * web application. This is useful for building links to other resources within the application.
   *
  ...
  /**
   * Return the path to URL mappings within the current servlet including the context path and the
   * servlet path of the original request. This is useful for building links to other resources
   * within the application where a servlet mapping of the style {@code "/main/*"} is used.
   *
   ...
   * This is particularly useful as HTML form action target, possibly in combination with the
  ...
  /**
   * Return the query string of the current request, that is, the part after the request path. This
   * is particularly useful for building an HTML form action target in combination with the original
   * request URI.
   *
   * <p>Delegates to the UrlPathHelper for decoding.
   *
  ...
  public String getMessage(String code, @Nullable List<?> args, String defaultMessage) {
    return getMessage(
        code, (args != null ? args.toArray() : null), defaultMessage, isDefaultHtmlEscape());
  }
  ...
  public String getMessage(
      String code, @Nullable Object[] args, String defaultMessage, boolean htmlEscape) {
  ...
  /**
   * Retrieve the given MessageSourceResolvable (e.g. an ObjectError instance), using the
   ...
   *
   * <p>Note that theme messages are never HTML-escaped, as they typically denote theme-specific
   * resource paths and not client-visible messages.
   *
   * @param code code of the message
   * @param defaultMessage the String to return if the lookup fails
   * @return the message
  ...
  public String getThemeMessage(String code, @Nullable List<?> args, String defaultMessage) {
    String msg =
        getTheme()
            .getMessageSource()
            .getMessage(code, (args != null ? args.toArray() : null), defaultMessage, getLocale());
    return (msg != null ? msg : "");
  }
   ...
   * @throws org.springframework.context.NoSuchMessageException if not found
   */
  public String getThemeMessage(String code, @Nullable Object[] args)
      throws NoSuchMessageException {
   ...
   *
   * @param path the bean and property path for which values and errors will be resolved (e.g.
   *     "person.age")
  ...
  /**
   * Inner class that isolates the JSTL dependency. Just called to resolve the fallback locale if
   * the JSTL API is present.
   */
  private static class JstlLocaleResolver {

    @Nullable
    public static Locale getJstlLocale(
