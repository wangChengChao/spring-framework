...
/**
 * A {@link org.springframework.web.servlet.resource.ResourceResolver} that resolves resources from
 * a {@link org.springframework.cache.Cache} or otherwise delegates to the resolver chain and saves
...
public class CachingResourceResolver extends AbstractResourceResolver {
  ...
  private final List<String> contentCodings =
      new ArrayList<>(EncodedResourceResolver.DEFAULT_CODINGS);
  ...
  /**
   * Configure the supported content codings from the {@literal "Accept-Encoding"} header for which
   * to cache resource variations.
   *
   * <p>The codings configured here are generally expected to match those configured on {@link
   * EncodedResourceResolver#setContentCodings(List)}.
   *
   * <p>By default this property is set to {@literal ["br", "gzip"]} based on the value of {@link
