...
/**
 * {@link LocaleResolver} implementation that uses a cookie sent back to the user in case of a
 * custom setting, with a fallback to the specified default locale or the request's accept-header
 * locale.
 *
 * <p>This is particularly useful for stateless applications without user sessions. The cookie may
 * optionally contain an associated time zone value as well; alternatively, you may specify a
 * default time zone.
 *
 * <p>Custom controllers can override the user's locale and time zone by calling {@code
 * #setLocale(Context)} on the resolver, e.g. responding to a locale change request. As a more
 * convenient alternative, consider using {@link
 * org.springframework.web.servlet.support.RequestContext#changeLocale}.
 *
...
public class CookieLocaleResolver extends CookieGenerator implements LocaleContextResolver {
   ...
   *
   * <p>Only used for overriding a cookie value if the locale has been changed in the course of the
   * current request!
   *
   * <p>Use {@code RequestContext(Utils).getLocale()} to retrieve the current locale in controllers
   * or views.
   *
   ...
   * current request!
   *
   * <p>Use {@code RequestContext(Utils).getTimeZone()} to retrieve the current time zone in
  ...
  public static final String TIME_ZONE_REQUEST_ATTRIBUTE_NAME =
      CookieLocaleResolver.class.getName() + ".TIME_ZONE";
  ...
  /**
   * Create a new instance of the {@link CookieLocaleResolver} class using the {@link
   * #DEFAULT_COOKIE_NAME default cookie name}.
   */
  ...
  /**
   * Specify whether this resolver's cookies should be compliant with BCP 47 language tags instead
   * of Java's legacy locale specification format.
   *
   * <p>The default is {@code true}, as of 5.1. Switch this to {@code false} for rendering Java's
   * legacy locale specification format. For parsing, this resolver leniently accepts the legacy
   * {@link Locale#toString} format as well as BCP 47 language tags in any case.
   *
   * @since 4.3
  ...
  /**
   * Return whether this resolver's cookies should be compliant with BCP 47 language tags instead of
   * Java's legacy locale specification format.
   *
   * @since 4.3
   */
   ...
   *
   * <p>The default is {@code true}. Turn this off for lenient handling of parse failures, falling
   * back to the default locale and time zone in such a case.
   *
   * @since 5.1.7
   * @see #setDefaultLocale
   * @see #setDefaultTimeZone
   ...
   *
   * <p>The default implementation calls {@link StringUtils#parseLocale(String)}, accepting the
   ...
   *
   * <p>The default implementation calls {@link Locale#toString()} or JDK 7's {@link
   * Locale#toLanguageTag()}, depending on the {@link #setLanguageTagCompliant
  ...
  /**
   * Determine the default time zone for the given request, Called if no time zone cookie has been
   * found.
   *
   * <p>The default implementation returns the specified default time zone, if any, or {@code null}
   * otherwise.
   *
