...
public class AsyncSupportConfigurer {
  ...
  @Nullable private AsyncTaskExecutor taskExecutor;
   ...
   *   <li>Perform blocking writes when streaming to the response through a reactive (e.g. Reactor,
   *       RxJava) controller method return value.
   * </ol>
   *
   * <p>By default only a {@link SimpleAsyncTaskExecutor} is used. However when using the above two
   * use cases, it's recommended to configure an executor backed by a thread pool such as {@link
  ...
  /**
   * Specify the amount of time, in milliseconds, before asynchronous request handling times out. In
  ...
  /**
   * Configure lifecycle interceptors with callbacks around concurrent request execution that starts
   * when a controller returns a {@link java.util.concurrent.Callable}.
   *
  ...
  /**
   * Configure lifecycle interceptors with callbacks around concurrent request execution that starts
   * when a controller returns a {@link DeferredResult}.
   *
   * @param interceptors the interceptors to register
   */
