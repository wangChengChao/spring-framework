...
/**
 * Tiles {@link org.apache.tiles.preparer.PreparerFactory} implementation that expects preparer
 * class names and builds preparer instances for those, creating them through the Spring
 * ApplicationContext in order to apply Spring container callbacks and configured Spring
 * BeanPostProcessors.
 *
          ...
          try {
            ...
            if (!ViewPreparer.class.isAssignableFrom(beanClass)) {
              throw new PreparerException(
                  "Invalid preparer class ["
                      + name
                      + "]: does not implement ViewPreparer interface");
            }
