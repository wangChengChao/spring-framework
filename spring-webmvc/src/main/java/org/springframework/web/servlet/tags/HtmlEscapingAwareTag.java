 ...
 *
 * <p>Provides a "htmlEscape" property for explicitly specifying whether to apply HTML escaping. If
 * not set, a page-level default (e.g. from the HtmlEscapeTag) or an application-wide default (the
 * "defaultHtmlEscape" context-param in {@code web.xml}) is used.
 *
...
public abstract class HtmlEscapingAwareTag extends RequestContextAwareTag {
  ...
  @Nullable private Boolean htmlEscape;

  /**
   * Set HTML escaping for this tag, as boolean value. Overrides the default HTML escaping setting
   ...
   *
   * <p>The default implementation checks the RequestContext's setting, falling back to {@code
   ...
   *
   * <p>The response encoding will be taken into account if the "responseEncodedHtmlEscape" setting
   * is enabled as well.
   *
