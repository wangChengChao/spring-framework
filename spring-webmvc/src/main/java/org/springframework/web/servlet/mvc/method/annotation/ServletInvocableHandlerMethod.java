...
/**
 * Extends {@link InvocableHandlerMethod} with the ability to handle return values through a
 * registered {@link HandlerMethodReturnValueHandler} and also supports setting the response status
 * based on a method-level {@code @ResponseStatus} annotation.
 *
 * <p>A {@code null} return value (including void) may be interpreted as the end of request
 * processing in combination with a {@code @ResponseStatus} annotation, a not-modified check
 * condition (see {@link ServletWebRequest#checkNotModified(long)}), or a method argument that
...
public class ServletInvocableHandlerMethod extends InvocableHandlerMethod {
  ...
  private String formatErrorForReturnValue(@Nullable Object returnValue) {
    return "Error handling return value=["
        + returnValue
        + "]"
  ...
  /**
   * Create a nested ServletInvocableHandlerMethod subclass that returns the the given value (or
   * raises an Exception if the value is one) rather than actually invoking the controller method.
   * This is useful when processing async return values (e.g. Callable, DeferredResult,
   * ListenableFuture).
   */
  ...
  /**
   * A nested subclass of {@code ServletInvocableHandlerMethod} that uses a simple {@link Callable}
   * instead of the original controller as the handler in order to return the fixed (concurrent)
   * result value given to it. Effectively "resumes" processing with the asynchronously produced
   * return value.
   */
  private class ConcurrentResultHandlerMethod extends ServletInvocableHandlerMethod {
    ...
    public ConcurrentResultHandlerMethod(
        final Object result, ConcurrentResultMethodParameter returnType) {
      super(
          (Callable<Object>)
              () -> {
    ...
    /**
     * Bridge to actual return value or generic type within the declared async return type, e.g. Foo
     * instead of {@code DeferredResult<Foo>}.
     */
    @Override
  ...
  /**
   * MethodParameter subclass based on the actual return value type or if that's null falling back
   * on the generic type within the declared async return type, e.g. Foo instead of {@code
   * DeferredResult<Foo>}.
   */
  private class ConcurrentResultMethodParameter extends HandlerMethodParameter {
    ...
    public ConcurrentResultMethodParameter(Object returnValue) {
      ...
      this.returnType =
          (returnValue instanceof ReactiveTypeHandler.CollectedValuesList
