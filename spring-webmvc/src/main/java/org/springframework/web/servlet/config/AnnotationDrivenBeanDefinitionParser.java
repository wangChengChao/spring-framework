...
/**
 * A {@link BeanDefinitionParser} that provides the configuration for the {@code
 * <annotation-driven/>} MVC namespace element.
 *
 ...
 * </ul>
 *
 * <p><strong>Note:</strong> Additional HandlerMappings may be registered as a result of using the
 * {@code <view-controller>} or the {@code <resources>} MVC namespace elements.
 *
 ...
 * </ul>
 *
 * <p>This class registers an {@link org.springframework.util.AntPathMatcher} and a {@link
 * org.springframework.web.util.UrlPathHelper} to be used by:
 *
 * <ul>
 ...
 * ExceptionHandlerExceptionResolver} are configured with instances of the following by default:
 *
 * <ul>
 *   <li>A {@link ContentNegotiationManager}
 *   <li>A {@link DefaultFormattingConversionService}
 *   <li>A {@link org.springframework.validation.beanvalidation.LocalValidatorFactoryBean} if a
...
class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
  ...
  public BeanDefinition parse(Element element, ParserContext context) {
    ...
    ManagedList<?> deferredResultInterceptors =
        getDeferredResultInterceptors(element, source, context);
    ...
    context.registerComponent(
        new BeanComponentDefinition(mappedInterceptorDef, mappedInterceptorName));
    context.registerComponent(
  ...
  protected void addRequestBodyAdvice(RootBeanDefinition beanDef) {
    if (jackson2Present) {
      beanDef
          .getPropertyValues()
          .add("requestBodyAdvice", new RootBeanDefinition(JsonViewRequestBodyAdvice.class));
    }
  }
  ...
  private ManagedList<?> getCallableInterceptors(
    ...
    if (asyncElement != null) {
      Element interceptorsElement =
          DomUtils.getChildElementByTagName(asyncElement, "callable-interceptors");
      if (interceptorsElement != null) {
        interceptors.setSource(source);
        for (Element converter : DomUtils.getChildElementsByTagName(interceptorsElement, "bean")) {
          BeanDefinitionHolder beanDef =
              context.getDelegate().parseBeanDefinitionElement(converter);
          if (beanDef != null) {
  ...
  private ManagedList<?> getMessageConverters(
    ...
    if (convertersElement == null
      ...
      messageConverters.add(
          createConverterDefinition(ResourceRegionHttpMessageConverter.class, source));
  ...
  private RootBeanDefinition createConverterDefinition(
      Class<?> converterClass, @Nullable Object source) {
   ...
   * HandlerMethodArgumentResolver's configured in RequestMappingHandlerAdapter after it is fully
   * initialized.
   */
  static class CompositeUriComponentsContributorFactoryBean
    ...
    @Nullable private RequestMappingHandlerAdapter handlerAdapter;

    @Nullable private ConversionService conversionService;

    @Nullable private CompositeUriComponentsContributor uriComponentsContributor;
