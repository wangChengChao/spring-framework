 ...
 * {@code @RequestMapping} methods on Spring MVC controllers.
 *
 * <p>There are several groups of methods:
 *
 * <ul>
 *   <li>Static {@code fromXxx(...)} methods to prepare links using information from the current
 *       request as determined by a call to {@link
 *       org.springframework.web.servlet.support.ServletUriComponentsBuilder#fromCurrentServletMapping()}.
 *   <li>Static {@code fromXxx(UriComponentsBuilder,...)} methods can be given a baseUrl when
 *       operating outside the context of a request.
 *   <li>Instance-based {@code withXxx(...)} methods where an instance of MvcUriComponentsBuilder is
 *       created with a baseUrl via {@link
 *       #relativeTo(org.springframework.web.util.UriComponentsBuilder)}.
 * </ul>
 *
 * <p><strong>Note:</strong> This class uses values from "Forwarded" (<a
 * href="https://tools.ietf.org/html/rfc7239">RFC 7239</a>), "X-Forwarded-Host", "X-Forwarded-Port",
 * and "X-Forwarded-Proto" headers, if present, in order to reflect the client-originated protocol
 * and address. Consider using the {@code ForwardedHeaderFilter} in order to choose from a central
 * place whether to extract and use, or to discard such headers. See the Spring Framework reference
...
public class MvcUriComponentsBuilder {
  ...
  /**
   * Create an instance of this class with a base URL. After that calls to one of the instance based
   * {@code withXxx(...}} methods will create URLs relative to the given base URL.
   */
   ...
   *
   * <p><strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*"
   ...
   * context of processing a request or to apply a custom baseUrl not matching the current request.
   *
   * <p><strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*"
   * headers if found. See class-level docs.
   *
   ...
   * @return a UriComponentsBuilder instance (never {@code null})
   */
  public static UriComponentsBuilder fromController(
      @Nullable UriComponentsBuilder builder, Class<?> controllerType) {
   ...
   * method argument values. This method delegates to {@link #fromMethod(Class, Method, Object...)}.
   *
   * <p><strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*"
   * headers if found. See class-level docs.
   *
   ...
   * @return a UriComponentsBuilder instance, never {@code null}
   * @throws IllegalArgumentException if there is no matching or if there is more than one matching
   *     method
   */
  ...
  /**
   * An alternative to {@link #fromMethodName(Class, String, Object...)} that accepts a {@code
   * UriComponentsBuilder} representing the base URL. This is useful when using
   ...
   */
  public static UriComponentsBuilder fromMethodName(
      UriComponentsBuilder builder, Class<?> controllerType, String methodName, Object... args) {
  ...
  /**
   * Create a {@link UriComponentsBuilder} from the mapping of a controller method and an array of
   * method argument values. The array of values must match the signature of the controller method.
   * Values for {@code @RequestParam} and {@code @PathVariable} are used for building the URI (via
   * implementations of {@link org.springframework.web.method.support.UriComponentsContributor
   * UriComponentsContributor}) while remaining argument values are ignored and can be {@code null}.
   *
   * <p><strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*"
   * headers if found. See class-level docs.
   *
   ...
   * @since 4.2
   */
  public static UriComponentsBuilder fromMethod(
      Class<?> controllerType, Method method, Object... args) {
    return fromMethodInternal(null, controllerType, method, args);
  }

  /**
   * An alternative to {@link #fromMethod(Class, Method, Object...)} that accepts a {@code
   * UriComponentsBuilder} representing the base URL. This is useful when using
   * MvcUriComponentsBuilder outside the context of processing a request or to apply a custom
   * baseUrl not matching the current request.
   *
  ...
  public static UriComponentsBuilder fromMethod(
    ...
    return fromMethodInternal(
        baseUrl,
        (controllerType != null ? controllerType : method.getDeclaringClass()),
        method,
        args);
  }
   ...
   * method and the supplied argument values are then used to delegate to {@link #fromMethod(Class,
   * Method, Object...)}.
   *
   ...
   *
   * <p>Note that this is a shorthand version of {@link #controller(Class)} intended for inline use
   ...
   *
   * <p>This is a longer version of {@link #on(Class)}. It is needed with controller methods
   ...
   *
   * <p>The configured {@link
   * org.springframework.web.servlet.handler.HandlerMethodMappingNamingStrategy
   * HandlerMethodMappingNamingStrategy} determines the names of controller method request mappings
   * at startup. By default all mappings are assigned a name based on the capital letters of the
   * class name, followed by "#" as separator, and then the method name. For example "PC#getPerson"
   * for a class named PersonController with method getPerson. In case the naming convention does
   * not produce unique results, an explicit name may be assigned through the name attribute of the
   * {@code @RequestMapping} annotation.
   *
   * <p>This is aimed primarily for use in view rendering technologies and EL expressions. The
   * Spring URL tag library registers this method as a function called "mvcUrl".
   *
   * <p>For example, given this controller:
   *
   * <pre class="code">
   ...
   * </pre>
   *
   * <p>Note that it's not necessary to specify all arguments. Only the ones required to prepare the
   ...
   * @since 4.2
   */
  public static MethodArgumentBuilder fromMappingName(
      @Nullable UriComponentsBuilder builder, String name) {
    WebApplicationContext wac = getWebApplicationContext();
    Assert.notNull(wac, "No WebApplicationContext. ");
    Map<String, RequestMappingInfoHandlerMapping> map =
        wac.getBeansOfType(RequestMappingInfoHandlerMapping.class);
    ...
    if (handlerMethods == null) {
      ...
      throw new IllegalArgumentException(
          "No unique match for mapping " + name + ": " + handlerMethods);
    } else {
  ...
  /**
   * An alternative to {@link #fromController(Class)} for use with an instance of this class created
   * via a call to {@link #relativeTo}.
   *
   * <p><strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*"
   * headers if found. See class-level docs.
   *
   ...
   * @since 4.2
   */
  public UriComponentsBuilder withMethodName(
      Class<?> controllerType, String methodName, Object... args) {
  ...
  private static UriComponentsBuilder fromMethodInternal(
      @Nullable UriComponentsBuilder builder,
  ...
  private static String getPathPrefix(Class<?> controllerType) {
    WebApplicationContext wac = getWebApplicationContext();
    if (wac != null) {
      Map<String, RequestMappingHandlerMapping> map =
          wac.getBeansOfType(RequestMappingHandlerMapping.class);
  ...
  private static Method getMethod(
      Class<?> controllerType, final String methodName, final Object... args) {
    MethodFilter selector =
        method -> {
  ...
  private static CompositeUriComponentsContributor getUriComponentsContributor() {
    ...
    if (wac != null) {
      try {
        return wac.getBean(
            MVC_URI_COMPONENTS_CONTRIBUTOR_BEAN_NAME, CompositeUriComponentsContributor.class);
  ...
  private static class ControllerMethodInvocationInterceptor
    ...
    @Nullable private Method controllerMethod;

    @Nullable private Object[] argumentValues;
    ...
    private static <T> T initProxy(
      ...
      interceptor =
          interceptor != null
              ? interceptor
              : new ControllerMethodInvocationInterceptor(controllerType);
  ...
  public static class MethodArgumentBuilder {
     ...
     * @since 4.2
     */
    public MethodArgumentBuilder(
        @Nullable UriComponentsBuilder baseUrl, Class<?> controllerType, Method method) {
