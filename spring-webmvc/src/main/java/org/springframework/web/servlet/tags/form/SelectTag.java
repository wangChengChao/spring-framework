...
/**
 * The {@code <select>} tag renders an HTML 'select' element. Supports data binding to the selected
 * option.
 *
 * <p>Inner '{@code option}' tags can be rendered using one of the approaches supported by the
 * OptionWriter class.
 *
 * <p>Also supports the use of nested {@link OptionTag OptionTags} or (typically one) nested {@link
 * OptionsTag}.
 *
 * <p>
 *
...
public class SelectTag extends AbstractHtmlInputElementTag {
  ...
  /**
   * Marker object for items that have been specified but resolve to null. Allows to differentiate
   * between 'set but null' and 'not set at all'.
   */
  private static final Object EMPTY = new Object();

  /**
   * The {@link Collection}, {@link Map} or array of objects used to generate the inner '{@code
   * option}' tags.
   */
  ...
  @Nullable private Object multiple;
  ...
  /**
   * Set the {@link Collection}, {@link Map} or array of objects used to generate the inner '{@code
   * option}' tags.
   *
   * <p>Required when wishing to render '{@code option}' tags from an array, {@link Collection} or
   * {@link Map}.
   *
   ...
   * tag.
   *
   * <p>Required when wishing to render '{@code option}' tags from an array or {@link Collection}.
   *
   * <p>May be a runtime expression.
   */
   ...
   *
   * <p>Renders nested '{@code option}' tags if the {@link #setItems items} property is set,
   * otherwise exposes the bound value for the nested {@link OptionTag OptionTags}.
   */
  @Override
      ...
      if (items != EMPTY) {
        ...
        if (itemsObject != null) {
          ...
          OptionWriter optionWriter =
              new OptionWriter(
                  itemsObject, getBindStatus(), valueProperty, labelProperty, isHtmlEscape()) {
                @Override
  ...
  /**
   * Returns '{@code true}' if the bound value requires the resultant '{@code select}' tag to be
   * multi-select.
   */
