 ...
 * Caches {@link org.springframework.web.servlet.View} objects once resolved: This means that view
 * resolution won't be a performance problem, no matter how costly initial view retrieval is.
 *
 * <p>Subclasses need to implement the {@link #loadView} template method, building the View object
 * for a specific view name and locale.
 *
 * @author Rod Johnson
...
public abstract class AbstractCachingViewResolver extends WebApplicationObjectSupport
  ...
  /**
   * Whether a view name once resolved to {@code null} should be cached and automatically resolved
   * to {@code null} subsequently.
   *
   * <p>Default is "true": unresolved view names are being cached, as of Spring 3.1. Note that this
   * flag only applies if the general {@link #setCache "cache"} flag is kept at its default of
   * "true" as well.
   *
   * <p>Of specific interest is the ability for some AbstractUrlBasedView implementations
   * (FreeMarker, Tiles) to check if an underlying resource exists via {@link
   * AbstractUrlBasedView#checkResource(Locale)}. With this flag set to "false", an underlying
   * resource that re-appears is noticed and used. With the flag set to "true", one check is made
   * only.
   */
  ...
  /**
   * Sets the filter that determines if view should be cached. Default behaviour is to cache all
   * views.
   *
   * @since 5.2
   */
   ...
   *
   * <p>Default is a String consisting of view name and locale suffix. Can be overridden in
   * subclasses.
   *
   * <p>Needs to respect the locale in general, as a different locale can lead to a different view
   * resource.
   */
   ...
   *
   * <p>This can be handy in case developer are able to modify views (e.g. FreeMarker templates) at
   * runtime after which you'd need to clear the cache for the specified view.
   *
   * @param viewName the view name for which the cached view object (if any) needs to be removed
  ...
  /**
   * Clear the entire view cache, removing all cached view objects. Subsequent resolve calls will
   * lead to recreation of demanded view objects.
   */
  ...
  public interface CacheFilter {
    ...
    /**
     * Indicates whether the given view should be cached. The name and locale used to resolve the
