...
/**
 * Creates a {@code ContentNegotiationManager} and configures it with one or more {@link
 * ContentNegotiationStrategy} instances.
 *
 * <p>As of 5.0 you can set the exact strategies to use via {@link #strategies(List)}.
 *
 * <p>As an alternative you can also rely on the set of defaults described below which can be turned
 * on or off or customized through the methods of this builder:
 *
 * <table>
 * <tr>
 ...
 * </table>
 *
 * <p>The order in which strategies are configured is fixed. You can only turn them on or off.
 *
 * <p><strong>Note:</strong> if you must use URL-based content type resolution, the use of a query
 * parameter is simpler and preferable to the use of a path extension since the latter can cause
 * issues with URI variables, path parameters, and URI decoding. Consider setting {@link
 * #favorPathExtension} to {@literal false} or otherwise set the strategies to use explicitly via
...
public class ContentNegotiationConfigurer {
   ...
   *
   * <p><strong>Note:</strong> use of this method is mutually exclusive with use of all other
   * setters in this class which customize a default, fixed set of strategies. See class level doc
   * for more details.
   *
   ...
   * type.
   *
   * <p>By default this is set to {@code true} in which case a request for {@code /hotels.pdf} will
   * be interpreted as a request for {@code "application/pdf"} regardless of the 'Accept' header.
   */
   ...
   * This is required in order for the parameter strategy to work. Any extensions explicitly
   * registered here are also whitelisted for the purpose of Reflected File Download attack
   * detection (see Spring Framework reference documentation for more details on RFD attack
   * protection).
   *
   * <p>The path extension strategy will also try to use {@link ServletContext#getMimeType} and
   * {@link MediaTypeFactory} to resolve path extensions. To change this behavior see the {@link
   * #useRegisteredExtensionsOnly} property.
   *
   ...
   * Setting this to {@code false} will result in an {@code HttpMediaTypeNotAcceptableException} if
   * there is no match.
   *
  ...
  /**
   * When {@link #favorPathExtension} is set, this property determines whether to allow use of JAF
   * (Java Activation Framework) to resolve a path extension to a specific MediaType.
   *
   * @deprecated as of 5.0, in favor of {@link #useRegisteredExtensionsOnly(boolean)} which has
   *     reverse behavior
   */
  @Deprecated
  ...
  /**
   * When {@link #favorPathExtension favorPathExtension} is set, this property determines whether to
   * use only registered {@code MediaType} mappings to resolve a path extension to a specific
   * MediaType.
   *
   * <p>By default this is not set in which case {@code PathExtensionContentNegotiationStrategy}
   * will use defaults if available.
   */
  public ContentNegotiationConfigurer useRegisteredExtensionsOnly(
   ...
   * media type. For this option to work you must register {@link #mediaType(String, MediaType)
   * media type mappings}.
   *
   ...
   *
   * <p>If destinations are present that do not support any of the given media types, consider
   * appending {@link MediaType#ALL} at the end.
   *
  ...
  /**
   * Set a custom {@link ContentNegotiationStrategy} to use to determine the content type to use
   * when no content type is requested.
   *
   * <p>By default this is not set.
   *
   * @since 4.1.2
