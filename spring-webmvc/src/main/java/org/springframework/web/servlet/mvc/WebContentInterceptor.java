...
/**
 * Handler interceptor that checks the request and prepares the response. Checks for supported
 * methods and a required session, and applies the specified {@link
 * org.springframework.http.CacheControl} builder. See superclass bean properties for configuration
 * options.
 *
 * <p>All the settings supported by this interceptor can also be set on {@link AbstractController}.
 * This interceptor is mainly intended for applying checks and preparations to a set of controllers
 * mapped by a HandlerMapping.
 *
...
public class WebContentInterceptor extends WebContentGenerator implements HandlerInterceptor {
   ...
   *
   * <p>Overrides the default cache seconds setting of this interceptor. Can specify "-1" to exclude
   * a URL path from default caching.
   *
   * <p>Supports direct matches, e.g. a registered "/test" matches "/test", and a various Ant-style
   * pattern matches, e.g. a registered "/t*" matches both "/test" and "/team". For details, see the
   * AntPathMatcher javadoc.
   *
   * <p><b>NOTE:</b> Path patterns are not supposed to overlap. If a request matches several
   * mappings, it is effectively undefined which one will apply (due to the lack of key ordering in
   * {@code java.util.Properties}).
   *
   * @param cacheMappings a mapping between URL paths (as keys) and cache seconds (as values, need
   *     to be integer-parsable)
   * @see #setCacheSeconds
   ...
   *
   * <p>Overrides the default cache seconds setting of this interceptor. Can specify a empty {@link
   * org.springframework.http.CacheControl} instance to exclude a URL path from default caching.
   *
   * <p>Supports direct matches, e.g. a registered "/test" matches "/test", and a various Ant-style
   * pattern matches, e.g. a registered "/t*" matches both "/test" and "/team". For details, see the
   * AntPathMatcher javadoc.
   ...
   * <p><b>NOTE:</b> Path patterns are not supposed to overlap. If a request matches several
   * mappings, it is effectively undefined which one will apply (due to the lack of key ordering in
   * the underlying {@code java.util.HashMap}).
   *
   ...
   * patterns, for determining cache mappings. Default is AntPathMatcher.
   *
   * @see #addCacheMapping
   * @see #setCacheMappings
   * @see org.springframework.util.AntPathMatcher
