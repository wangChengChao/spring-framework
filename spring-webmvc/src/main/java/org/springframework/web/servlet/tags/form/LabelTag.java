 ...
 *
 * <p>See the "formTags" showcase application that ships with the full Spring distribution for an
...
public class LabelTag extends AbstractHtmlElementTag {
  ...
  /**
   * Writes the opening '{@code label}' tag and forces a block tag so that body content is written
   * correctly.
   *
  ...
  /**
   * Overrides {@code #getName()} to always return {@code null}, because the '{@code name}'
   * attribute is not supported by the '{@code label}' tag.
   *
   ...
   *
   * <p>The default implementation delegates to {@link #getPropertyPath()}, deleting invalid
