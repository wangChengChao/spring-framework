...
/**
 * A strategy interface for retrieving and saving FlashMap instances. See {@link FlashMap} for a
...
public interface FlashMapManager {
  ...
  /**
   * Find a FlashMap saved by a previous request that matches to the current request, remove it from
   * underlying storage, and also remove other expired FlashMap instances.
   *
   * <p>This method is invoked in the beginning of every request in contrast to {@link
   * #saveOutputFlashMap}, which is invoked only when there are flash attributes to be saved - i.e.
   * before a redirect.
   *
