...
/**
 * Private helper class to assist with handling "reactive" return values types that can be adapted
 * to a Reactive Streams {@link Publisher} through the {@link ReactiveAdapterRegistry}.
 *
 * <p>Such return values may be bridged to a {@link ResponseBodyEmitter} for streaming purposes at
 * the presence of a streaming media type or based on the generic type.
 *
 * <p>For all other cases {@code Publisher} output is collected and bridged to {@link
...
class ReactiveTypeHandler {
  ...
  /**
   * Process the given reactive return value and decide whether to adapt it to a {@link
   * ResponseBodyEmitter} or a {@link DeferredResult}.
   *
   * @return an emitter for streaming, or {@code null} if handled internally with a {@link
   *     DeferredResult}
   */
  @Nullable
  ...
  private Collection<MediaType> getMediaTypes(NativeWebRequest request)
    ...
    return CollectionUtils.isEmpty(mediaTypes)
        ? this.contentNegotiationManager.resolveMediaTypes(request)
        : mediaTypes;
  }
  ...
  private static class DeferredResultSubscriber implements Subscriber<Object> {
    ...
    DeferredResultSubscriber(
        DeferredResult<Object> result, ReactiveAdapter adapter, ResolvableType elementType) {
      this.result = result;
