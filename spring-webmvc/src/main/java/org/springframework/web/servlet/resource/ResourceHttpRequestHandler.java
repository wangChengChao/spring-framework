...
/**
 * {@code HttpRequestHandler} that serves static resources in an optimized way according to the
 * guidelines of Page Speed, YSlow, etc.
 *
 * <p>The {@linkplain #setLocations "locations"} property takes a list of Spring {@link Resource}
 * locations from which static resources are allowed to be served by this handler. Resources could
 * be served from a classpath location, e.g. "classpath:/META-INF/public-web-resources/", allowing
 * convenient packaging and serving of resources such as .js, .css, and others in jar files.
 *
 * <p>This request handler may also be configured with a {@link #setResourceResolvers(List)
 * resourcesResolver} and {@link #setResourceTransformers(List) resourceTransformer} chains to
 * support arbitrary resolution and transformation of resources being served. By default a {@link
 * PathResourceResolver} simply finds resources based on the configured "locations". An application
 * can configure additional resolvers and transformers such as the {@link VersionResourceResolver}
 * which can resolve and prepare URLs for resources with a version in the URL.
 *
 * <p>This handler also properly evaluates the {@code Last-Modified} header (if present) so that a
 * {@code 304} status code will be returned as appropriate, avoiding unnecessary overhead for
 * resources that are already cached by the client.
 *
 * @author Keith Donald
...
public class ResourceHttpRequestHandler extends WebContentGenerator
  ...
  @Nullable private ResourceResolverChain resolverChain;
  ...
  @Nullable private PathExtensionContentNegotiationStrategy contentNegotiationStrategy;

  @Nullable private CorsConfiguration corsConfiguration;

  @Nullable private UrlPathHelper urlPathHelper;
  ...
  /**
   * An alternative to {@link #setLocations(List)} that accepts a list of String-based location
   * values, with support for {@link UrlResource}'s (e.g. files or HTTP URLs) with a special prefix
   * to indicate the charset to use when appending relative paths. For example {@code
  ...
  /**
   * Set the {@code List} of {@code Resource} locations to use as sources for serving static
   * resources.
   *
   ...
   *
   * <p>Note that if {@link #setLocationValues(List) locationValues} are provided, instead of loaded
   * Resource-based locations, this method will return empty until after initialization via {@link
   * #afterPropertiesSet()}.
   *
   ...
   *
   * <p>By default {@link PathResourceResolver} is configured. If using this property, it is
   * recommended to add {@link PathResourceResolver} as the last resolver.
   */
   ...
   * @since 4.3
   */
  public void setResourceHttpMessageConverter(
      @Nullable ResourceHttpMessageConverter messageConverter) {
  ...
  /**
   * Configure a {@code ContentNegotiationManager} to help determine the media types for resources
   * being served. If the manager contains a path extension strategy it will be checked for
  ...
  /**
   * Provide a reference to the {@link UrlPathHelper} used to map requests to static resources. This
   * helps to derive information about the lookup path such as whether it is decoded or not.
   *
   * @since 4.3.13
   */
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    this.transformerChain =
        new DefaultResourceTransformerChain(this.resolverChain, this.resourceTransformers);
  ...
  /**
   * Look for a {@code PathResourceResolver} among the configured resource resolvers and set its
   * {@code allowedLocations} property (if empty) to match the {@link #setLocations locations}
   ...
   *
   * <p>Checks for the existence of the requested resource in the configured list of locations. If
   * the resource does not exist, a {@code 404} response will be returned to the client. If the
   * resource exists, the request will be checked for the presence of the {@code Last-Modified}
   * header, and its value will be compared against the last-modified timestamp of the given
   * resource, returning a {@code 304} status code if the {@code Last-Modified} value is greater. If
   * the resource is newer than the {@code Last-Modified} value, or the header is not present, the
   * content resource of the resource will be written to the response with caching headers set to
   * expire one year in the future.
   */
  @Override
   ...
   *   <li>Any combination of leading slash and control characters (00-1F and 7F) with a single "/"
   *       or "". For example {@code " / // foo/bar"} becomes {@code "/foo/bar"}.
   * </ul>
   *
   * @since 3.2.12
   */
   ...
   *   <li>Paths that contain "../" after a call to {@link
   *       org.springframework.util.StringUtils#cleanPath}.
   *   <li>Paths that represent a {@link org.springframework.util.ResourceUtils#isUrl valid URL} or
   *       would represent one after the leading slash is removed.
   * </ul>
   *
   * <p><strong>Note:</strong> this method assumes that leading, duplicate '/' or control characters
   * (e.g. white space) have been trimmed so that the path starts predictably with a single '/' or
   * does not have one.
   *
   * @param path the path to validate
   * @return {@code true} if the path is invalid, {@code false} otherwise
  ...
  /**
   * Determine the media type for the given request and the resource matched to it. This
   * implementation tries to determine the MediaType based on the file extension of the Resource via
  ...
  protected void setHeaders(
      HttpServletResponse response, Resource resource, @Nullable MediaType mediaType)
      throws IOException {
