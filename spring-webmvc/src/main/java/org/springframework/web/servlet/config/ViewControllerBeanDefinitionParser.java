...
/**
 * {@link org.springframework.beans.factory.xml.BeanDefinitionParser} that parses the following MVC
 * namespace elements:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>All elements result in the registration of a {@link
 * org.springframework.web.servlet.mvc.ParameterizableViewController ParameterizableViewController}
 * with all controllers mapped using in a single {@link
 * org.springframework.web.servlet.handler.SimpleUrlHandlerMapping SimpleUrlHandlerMapping}.
 *
 * @author Keith Donald
...
class ViewControllerBeanDefinitionParser implements BeanDefinitionParser {
  ...
  private BeanDefinition registerHandlerMapping(ParserContext context, @Nullable Object source) {
    ...
    beanDef
        .getPropertyValues()
        .add("pathMatcher", MvcNamespaceUtils.registerPathMatcher(null, context, source));
    beanDef
        .getPropertyValues()
        .add("urlPathHelper", MvcNamespaceUtils.registerUrlPathHelper(null, context, source));
  ...
  private RootBeanDefinition getRedirectView(
      Element element, @Nullable HttpStatus status, @Nullable Object source) {
    RootBeanDefinition redirectView = new RootBeanDefinition(RedirectView.class);
    redirectView.setSource(source);
    redirectView
        .getConstructorArgumentValues()
        .addIndexedArgumentValue(0, element.getAttribute("redirect-url"));
    ...
    if (element.hasAttribute("context-relative")) {
      redirectView
          .getPropertyValues()
          .add("contextRelative", element.getAttribute("context-relative"));
    } else {
    ...
    if (element.hasAttribute("keep-query-params")) {
      redirectView
          .getPropertyValues()
          .add("propagateQueryParams", element.getAttribute("keep-query-params"));
    }
