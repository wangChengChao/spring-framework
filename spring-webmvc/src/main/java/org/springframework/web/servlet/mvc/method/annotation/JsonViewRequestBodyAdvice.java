...
/**
 * A {@link RequestBodyAdvice} implementation that adds support for Jackson's {@code @JsonView}
 * annotation declared on a Spring MVC {@code @HttpEntity} or {@code @RequestBody} method parameter.
 *
 * <p>The deserialization view specified in the annotation will be passed in to the {@link
 * org.springframework.http.converter.json.MappingJackson2HttpMessageConverter} which will then use
 * it to deserialize the request body with.
 *
 * <p>Note that despite {@code @JsonView} allowing for more than one class to be specified, the use
 * for a request body advice is only supported with exactly one class argument. Consider the use of
...
public class JsonViewRequestBodyAdvice extends RequestBodyAdviceAdapter {
  ...
  public HttpInputMessage beforeBodyRead(
    ...
    return new MappingJacksonInputMessage(
        inputMessage.getBody(), inputMessage.getHeaders(), classes[0]);
  }
}
