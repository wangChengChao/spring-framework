 ...
 * Supports ordering, a default handler, handler interceptors, including handler interceptors mapped
 * by path patterns.
 *
...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
  ...
  /**
   * Set the default handler for this handler mapping. This handler will be returned if no specific
   * mapping was found.
   *
   ...
   * MappedInterceptor. Mapped interceptors apply only to request URLs that match its path patterns.
  ...
  /**
   * Configure a custom {@link CorsProcessor} to use to apply the matched {@link CorsConfiguration}
   * for a request.
   *
  ...
  /**
   * Extension hook that subclasses can override to register additional interceptors, given the
   ...
   *
   * <p>This is called in addition to any {@link MappedInterceptor MappedInterceptors} that may have
   * been provided via {@link #setInterceptors}, by default adding all beans of type {@link
   * MappedInterceptor} from the current context and its ancestors. Subclasses can override and
   * refine this policy.
   *
  ...
  /**
   * Initialize the specified interceptors, checking for {@link MappedInterceptor
   * MappedInterceptors} and adapting {@link HandlerInterceptor}s and {@link WebRequestInterceptor
   * HandlerInterceptor}s and {@link WebRequestInterceptor}s if necessary.
   *
   * @see #setInterceptors
   * @see #adaptInterceptor
   */
   ...
   * WebRequestInterceptor}. Each given {@link WebRequestInterceptor} will be wrapped in a {@link
   * WebRequestHandlerInterceptorAdapter}. Can be overridden in subclasses.
   *
  ...
  /**
   * Look up a handler for the given request, falling back to the default handler if no specific one
   * is found.
   *
   ...
   *
   * <p>On CORS pre-flight requests this method should return a match not for the pre-flight request
   * but for the expected actual request based on the URL path, the HTTP methods from the
   * "Access-Control-Request-Method" header, and the headers from the
   * "Access-Control-Request-Headers" header thus allowing the CORS configuration to be obtained via
   * {@link #getCorsConfiguration(Object, HttpServletRequest)},
   *
   * <p>Note: This method may also return a pre-built {@link HandlerExecutionChain}, combining a
   * handler object with dynamically determined interceptors. Statically specified interceptors will
   * get merged into such an existing chain.
   *
   * @param request current HTTP request
   ...
   * handler, the handler mapping's common interceptors, and any {@link MappedInterceptor
   ...
   * HandlerExecutionChain}. This method should handle those two cases explicitly, either building a
   * new {@link HandlerExecutionChain} or extending the existing chain.
   *
