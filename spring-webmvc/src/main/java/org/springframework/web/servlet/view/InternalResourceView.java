 ...
 * request attributes and forwards the request to the specified resource URL using a {@link
 * javax.servlet.RequestDispatcher}.
 *
 * <p>A URL for this view is supposed to specify a resource within the web application, suitable for
 * RequestDispatcher's {@code forward} or {@code include} method.
 *
 * <p>If operating within an already included request or within a response that has already been
 * committed, this view will fall back to an include instead of a forward. This can be enforced by
 * calling {@code response.flushBuffer()} (which will commit the response) before rendering the
 * view.
 *
 * <p>Typical usage with {@link InternalResourceViewResolver} looks as follows, from the perspective
 * of the DispatcherServlet context definition:
 *
 * <pre class="code">
 ...
 * "myView" -> "/WEB-INF/jsp/myView.jsp"), using this view class by default.
 *
 * @author Rod Johnson
...
public class InternalResourceView extends AbstractUrlBasedView {
   ...
   *
   * <p>Default is "false". Switch this to "true" for convention-based views where a dispatch back
   * to the current handler path is a definitive error.
   */
   ...
   * rendering operations can't overwrite each other's contexts etc.
   *
   * <p>Called by {@link #renderMergedOutputModel(Map, HttpServletRequest, HttpServletResponse)}.
   * The default implementation is empty. This method can be overridden to add custom helpers as
   * request attributes.
   *
   ...
   *
   * <p>This implementation simply returns the configured URL. Subclasses can override this to
   * determine a resource to render, typically interpreting the URL in a different manner.
   *
   * @param request current HTTP request
   ...
   * HttpServletRequest#getRequestDispatcher(String)}. Can be overridden in subclasses.
   *
   * @param request current HTTP request
