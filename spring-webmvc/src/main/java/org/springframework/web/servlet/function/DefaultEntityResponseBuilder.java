...
final class DefaultEntityResponseBuilder<T> implements EntityResponse.Builder<T> {
  ...
  public EntityResponse<T> build() {
    if (this.entity instanceof CompletionStage) {
      ...
      return new CompletionStageEntityResponse(
          this.status, this.headers, this.cookies, completionStage, this.entityType);
    } else if (reactiveStreamsPresent && PublisherEntityResponse.isPublisher(this.entity)) {
      Publisher publisher = (Publisher) this.entity;
      return new PublisherEntityResponse(
          this.status, this.headers, this.cookies, publisher, this.entityType);
    } else {
      return new DefaultEntityResponse<>(
          this.status, this.headers, this.cookies, this.entity, this.entityType);
    }
  }
  ...
  public static <T> EntityResponse.Builder<T> fromObject(
      T t, ParameterizedTypeReference<?> bodyType) {
  ...
  private static class DefaultEntityResponse<T>
      extends DefaultServerResponseBuilder.AbstractServerResponse implements EntityResponse<T> {
    ...
    protected ModelAndView writeToInternal(
        HttpServletRequest servletRequest, HttpServletResponse servletResponse, Context context)
    ...
    protected void writeEntityWithMessageConverters(
        Object entity,
      ...
      List<MediaType> producibleMediaTypes =
          producibleMediaTypes(context.messageConverters(), entityClass);
    ...
    protected void tryWriteEntityWithMessageConverters(
        Object entity,
        HttpServletRequest request,
        HttpServletResponse response,
        ServerResponse.Context context) {
  ...
  private static class CompletionStageEntityResponse<T>
    ...
    public CompletionStageEntityResponse(
        int statusCode,
        HttpHeaders headers,
        MultiValueMap<String, Cookie> cookies,
        CompletionStage<T> entity,
        Type entityType) {
  ...
  private static class PublisherEntityResponse<T> extends DefaultEntityResponse<Publisher<T>> {
    ...
    public PublisherEntityResponse(
        int statusCode,
        HttpHeaders headers,
        MultiValueMap<String, Cookie> cookies,
        Publisher<T> entity,
    ...
    protected ModelAndView writeToInternal(
      ...
      AsyncContext asyncContext =
          servletRequest.startAsync(
              servletRequest, new NoContentLengthResponseWrapper(servletResponse));
