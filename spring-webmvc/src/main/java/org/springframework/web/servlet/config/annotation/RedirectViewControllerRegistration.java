...
public class RedirectViewControllerRegistration {
   ...
   *
   * <p>If not set, {@link org.springframework.web.servlet.view.RedirectView} will select {@code
   * HttpStatus.MOVED_TEMPORARILY (302)} by default.
   */
  ...
  /**
   * Whether to interpret a given redirect URL that starts with a slash ("/") as relative to the
   * current ServletContext, i.e. as relative to the web application root.
   *
  ...
  /**
   * Whether to propagate the query parameters of the current request through to the target redirect
   * URL.
   *
