 ...
 * entry is used. Users can either specify a specific entry in the model via the {@link
 * #setModelKey(String) sourceKey} property.
 *
...
public class MappingJackson2XmlView extends AbstractJackson2View {
  ...
  /**
   * Construct a new {@code MappingJackson2XmlView} using default configuration provided by {@link
   * Jackson2ObjectMapperBuilder} and setting the content type to {@code application/xml}.
   */
  ...
  /**
   * Construct a new {@code MappingJackson2XmlView} using the provided {@link XmlMapper} and setting
    ...
    if (this.modelKey != null) {
      ...
      for (Map.Entry<String, Object> entry : model.entrySet()) {
        if (!(entry.getValue() instanceof BindingResult)
          ...
          if (value != null) {
            throw new IllegalStateException(
                "Model contains more than one object to render, only one is supported");
          }
