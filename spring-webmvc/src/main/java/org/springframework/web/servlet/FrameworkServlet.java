 ...
 *       WebApplicationContext} instance per servlet. The servlet's configuration is determined by
 *       beans in the servlet's namespace.
 *   <li>Publishes events on request processing, whether or not a request is successfully handled.
 * </ul>
 *
 * <p>Subclasses must implement {@link #doService} to handle requests. Because this extends {@link
 * HttpServletBean} rather than HttpServlet directly, bean properties are automatically mapped onto
 * it. Subclasses can override {@link #initFrameworkServlet()} for custom initialization.
 *
 * <p>Detects a "contextClass" parameter at the servlet init-param level, falling back to the
 * default context class, {@link org.springframework.web.context.support.XmlWebApplicationContext
 * XmlWebApplicationContext}, if not found. Note that, with the default {@code FrameworkServlet}, a
 * custom context class needs to implement the {@link
 * org.springframework.web.context.ConfigurableWebApplicationContext
 * ConfigurableWebApplicationContext} SPI.
 *
 * <p>Accepts an optional "contextInitializerClasses" servlet init-param that specifies one or more
 * {@link org.springframework.context.ApplicationContextInitializer ApplicationContextInitializer}
 * classes. The managed web application context will be delegated to these initializers, allowing
 * for additional programmatic configuration, e.g. adding property sources or activating profiles
 * against the {@linkplain
 * org.springframework.context.ConfigurableApplicationContext#getEnvironment() context's
 * environment}. See also {@link org.springframework.web.context.ContextLoader} which supports a
 * "contextInitializerClasses" context-param with identical semantics for the "root" web application
 * context.
 *
 * <p>Passes a "contextConfigLocation" servlet init-param to the context instance, parsing it into
 * potentially multiple file paths which can be separated by any number of commas and spaces, like
 * "test-servlet.xml, myServlet.xml". If not explicitly specified, the context implementation is
 * supposed to build a default location from the namespace of the servlet.
 *
 * <p>Note: In case of multiple config locations, later bean definitions will override ones defined
 * in earlier loaded files, at least when using Spring's default ApplicationContext implementation.
 * This can be leveraged to deliberately override certain bean definitions via an extra XML file.
 *
 * <p>The default namespace is "'servlet-name'-servlet", e.g. "test-servlet" for a servlet-name
 * "test" (leading to a "/WEB-INF/test-servlet.xml" default location with XmlWebApplicationContext).
 * The namespace can also be set explicitly via the "namespace" servlet init-param.
 *
 * <p>As of Spring 3.1, {@code FrameworkServlet} may now be injected with a web application context,
 * rather than creating its own internally. This is useful in Servlet 3.0+ environments, which
 * support programmatic registration of servlet instances. See {@link
...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
  ...
  private final List<ApplicationContextInitializer<ConfigurableApplicationContext>>
   ...
   *       through the "contextInitializerClasses" init-param or through the {@link
  ...
  /**
   * Set a custom context class. This class must be of type {@link
   * org.springframework.web.context.WebApplicationContext}.
   *
   * <p>When using the default FrameworkServlet implementation, the context class must also
   * implement the {@link org.springframework.web.context.ConfigurableWebApplicationContext}
   * interface.
   *
   ...
   *
   * <p>This is especially handy during testing, although it is debatable whether it's good practice
   ...
   *
   * <p>Default is "false", to avoid side effects on spawned background threads. Switch this to
   * "true" to enable inheritance for custom child threads which are spawned during request
   * processing and only used for this request (that is, ending after their initial task, without
   * reuse of the thread).
   *
   * <p><b>WARNING:</b> Do not use inheritance for child threads if you are accessing a thread pool
   * which is configured to potentially add new threads on demand (e.g. a JDK {@link
   * java.util.concurrent.ThreadPoolExecutor}), since this will expose the inherited context to such
   * a pooled thread.
   */
   ...
   * method.
   *
   * <p>Default in the {@code FrameworkServlet} is "false", applying {@link
   * javax.servlet.http.HttpServlet}'s default behavior (i.e.enumerating all standard HTTP request
   * methods as a response to the OPTIONS request). Note however that as of 4.3 the {@code
   * DispatcherServlet} sets this property to "true" by default due to its built-in support for
   * OPTIONS.
   *
   * <p>Turn this flag on if you prefer OPTIONS requests to go through the regular dispatching
   * chain, just like other HTTP requests. This usually means that your controllers will receive
   * those requests; make sure that those endpoints are actually able to handle an OPTIONS request.
   *
   * <p>Note that HttpServlet's default OPTIONS processing will be applied in any case if your
   * controllers happen to not set the 'Allow' header (as required for an OPTIONS response).
   */
   ...
   * method.
   *
   * <p>Default is "false", applying {@link javax.servlet.http.HttpServlet}'s default behavior (i.e.
   * reflecting the message received back to the client).
   *
   * <p>Turn this flag on if you prefer TRACE requests to go through the regular dispatching chain,
   * just like other HTTP requests. This usually means that your controllers will receive those
   * requests; make sure that those endpoints are actually able to handle a TRACE request.
   *
   * <p>Note that HttpServlet's default TRACE processing will be applied in any case if your
   * controllers happen to not generate a response of content type 'message/http' (as required for a
   * TRACE response).
   */
  ...
  /**
   * Whether to log request params at DEBUG level, and headers at TRACE level. Both may contain
  ...
  protected final void initServletBean() throws ServletException {
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
          "Completed initialization in " + (System.currentTimeMillis() - startTime) + " ms");
    }
  }
   ...
   *
   * <p>Subclasses may override this method to provide a different {@code WebApplicationContext}
   * retrieval strategy.
   *
   ...
   * for this servlet.
   *
   * <p>The default implementation is empty. {@code refresh()} will be called automatically after
   * this method returns.
   *
   * <p>Note that this method is designed to allow subclasses to modify the application context,
   * while {@link #initWebApplicationContext} is designed to allow end-users to modify the context
   * through the use of {@link ApplicationContextInitializer ApplicationContextInitializers}.
   *
   ...
   *
   * <p>The default implementation returns {@code SERVLET_CONTEXT_PREFIX + servlet name}.
   *
   ...
   *
   * <p>The default implementation calls {@link #onRefresh}, triggering a refresh of this servlet's
   ...
   *
   * <p>Will also be invoked by HttpServlet's default implementation of {@code doHead}, with a
   * {@code NoBodyResponse} that just captures the content length.
   *
   * @see #doService
   * @see #doHead
   */
   ...
   *
   * <p>Applies HttpServlet's standard OPTIONS processing otherwise, and also if there is still no
   * 'Allow' header set after dispatching.
   *
   * @see #doService
   */
  @Override
   ...
   *
   * <p>The default implementation takes the name of the UserPrincipal, if any. Can be overridden in
   * subclasses.
   *
   * @param request current HTTP request
   ...
   *
   * <p>The contract is essentially the same as that for the commonly overridden {@code doGet} or
   * {@code doPost} methods of HttpServlet.
   *
   * <p>This class intercepts calls to ensure that exception handling and event publication takes
   * place.
   *
   * @param request current HTTP request
   * @param response current HTTP response
  ...
  private class RequestBindingInterceptor implements CallableProcessingInterceptor {
    ...
    public <T> void preProcess(NativeWebRequest webRequest, Callable<T> task) {
      ...
      if (request != null) {
        ...
        initContextHolders(
            request, buildLocaleContext(request), buildRequestAttributes(request, response, null));
      }
    }

    @Override
    public <T> void postProcess(
        NativeWebRequest webRequest, Callable<T> task, Object concurrentResult) {
      HttpServletRequest request = webRequest.getNativeRequest(HttpServletRequest.class);
      if (request != null) {
