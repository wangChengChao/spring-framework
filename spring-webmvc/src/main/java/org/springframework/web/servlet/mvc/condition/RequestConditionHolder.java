...
/**
 * A holder for a {@link RequestCondition} useful when the type of the request condition is not
 * known ahead of time, e.g. custom condition. Since this class is also an implementation of {@code
 * RequestCondition}, effectively it decorates the held request condition and allows it to be
 * combined and compared with other request conditions in a type and null safe way.
 *
 * <p>When two {@code RequestConditionHolder} instances are combined or compared with each other, it
 * is expected the conditions they hold are of the same type. If they are not, a {@link
...
public final class RequestConditionHolder extends AbstractRequestCondition<RequestConditionHolder> {
  ...
  @Nullable private final RequestCondition<Object> condition;
  ...
  protected Collection<?> getContent() {
    return (this.condition != null
        ? Collections.singleton(this.condition)
  ...
  /**
   * Combine the request conditions held by the two RequestConditionHolder instances after making
   * sure the conditions are of the same type. Or if one holder is empty, the other holder is
   * returned.
   */
  @Override
  ...
  private void assertEqualConditionTypes(
      RequestCondition<?> thisCondition, RequestCondition<?> otherCondition) {
    Class<?> clazz = thisCondition.getClass();
    Class<?> otherClazz = otherCondition.getClass();
    if (!clazz.equals(otherClazz)) {
      throw new ClassCastException(
          "Incompatible request conditions: " + clazz + " and " + otherClazz);
    }
  }

  /**
   * Get the matching condition for the held request condition wrap it in a new
   * RequestConditionHolder instance. Or otherwise if this is an empty holder, return the same
   * holder instance.
   */
  @Override
  @Nullable
  ...
  /**
   * Compare the request conditions held by the two RequestConditionHolder instances after making
   * sure the conditions are of the same type. Or if one holder is empty, the other holder is
   * preferred.
   */
  @Override
