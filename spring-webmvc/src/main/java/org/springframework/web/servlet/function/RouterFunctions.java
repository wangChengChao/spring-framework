...
public abstract class RouterFunctions {
  ...
  /**
   * Name of the request attribute that contains the URI templates map, mapping variable names to
   * values.
   */
  ...
  /**
   * Name of the request attribute that contains the matching pattern, as a {@link
   ...
   */
  public static Function<ServerRequest, Optional<Resource>> resourceLookupFunction(
   ...
   * function (of the same response type {@code T}) if this route had {@linkplain Optional#empty()
   * no result}.
   *
   ...
   * (of a different response type) if this route had {@linkplain Optional#empty() no result}.
   */
    ...
    public Optional<HandlerFunction<T>> route(ServerRequest request) {
      if (this.predicate.test(request)) {
        if (logger.isTraceEnabled()) {
          logger.trace(
              String.format("Predicate \"%s\" matches against \"%s\"", this.predicate, request));
        }
