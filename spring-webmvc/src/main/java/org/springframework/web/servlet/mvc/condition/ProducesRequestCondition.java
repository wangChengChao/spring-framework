...
/**
 * A logical disjunction (' || ') request condition to match a request's 'Accept' header to a list
 * of media type expressions. Two kinds of media type expressions are supported, which are described
 * in {@link RequestMapping#produces()} and {@link RequestMapping#headers()} where the header name
 * is 'Accept'. Regardless of which syntax is used, the semantics are the same.
 *
...
public final class ProducesRequestCondition
  ...
  private static final String MEDIA_TYPES_ATTRIBUTE =
      ProducesRequestCondition.class.getName() + ".MEDIA_TYPES";
  ...
  /**
   * Creates a new instance from "produces" expressions. If 0 expressions are provided in total,
   * this condition will match to any request.
   *
  ...
  /**
   * Creates a new instance with "produces" and "header" expressions. "Header" expressions where the
   * header name is not 'Accept' or have no header value defined are ignored. If 0 expressions are
  ...
  /**
   * Same as {@link #ProducesRequestCondition(String[], String[])} but also accepting a {@link
  ...
  public ProducesRequestCondition(
      String[] produces, @Nullable String[] headers, @Nullable ContentNegotiationManager manager) {

    this.expressions = new ArrayList<>(parseExpressions(produces, headers));
    Collections.sort(this.expressions);
    this.contentNegotiationManager =
        manager != null ? manager : DEFAULT_CONTENT_NEGOTIATION_MANAGER;
  }

  /**
   * Private constructor for internal use to create matching conditions. Note the expressions List
   * is neither sorted nor deep copied.
   */
  private ProducesRequestCondition(
      List<ProduceMediaTypeExpression> expressions, ProducesRequestCondition other) {
    this.expressions = expressions;
    this.contentNegotiationManager = other.contentNegotiationManager;
  }

  private Set<ProduceMediaTypeExpression> parseExpressions(
      String[] produces, @Nullable String[] headers) {
   ...
   * Practically that means a method-level "produces" overrides a type-level "produces" condition.
   */
  @Override
  ...
  /**
   * Checks if any of the contained media type expressions match the given request 'Content-Type'
   * header and returns an instance that is guaranteed to contain matching expressions only. The
   * match is performed via {@link MediaType#isCompatibleWith(MediaType)}.
   *
   * @param request the current request
   * @return the same instance if there are no expressions; or a new condition with matching
   *     expressions; or {@code null} if no expressions match.
   */
  @Override
  @Nullable
  ...
  @Nullable
  private List<ProduceMediaTypeExpression> getMatchingExpressions(
      List<MediaType> acceptedMediaTypes) {
  ...
  public int compareTo(ProducesRequestCondition other, HttpServletRequest request) {
    try {
      ...
      throw new IllegalStateException(
          "Cannot compare without having any requested media types", ex);
    }
  }
  ...
  /**
   * Return the contained "produces" expressions or if that's empty, a list with a {@value
   * MediaType#ALL_VALUE} expression.
   */
  ...
  /**
   * Use this to clear {@link #MEDIA_TYPES_ATTRIBUTE} that contains the parsed, requested media
   * types.
   *
   * @param request the current request
   * @since 5.2
    ...
    private boolean matchMediaType(List<MediaType> acceptedMediaTypes) {
      for (MediaType acceptedMediaType : acceptedMediaTypes) {
        if (getMediaType().isCompatibleWith(acceptedMediaType)
            && matchParameters(acceptedMediaType)) {
          return true;
        }
      }
      return false;
