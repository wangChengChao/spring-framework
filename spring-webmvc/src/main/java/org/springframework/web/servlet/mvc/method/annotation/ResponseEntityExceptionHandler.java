...
/**
 * A convenient base class for {@link ControllerAdvice @ControllerAdvice} classes that wish to
 * provide centralized exception handling across all {@code @RequestMapping} methods through
 * {@code @ExceptionHandler} methods.
 *
 * <p>This base class provides an {@code @ExceptionHandler} method for handling internal Spring MVC
 * exceptions. This method returns a {@code ResponseEntity} for writing to the response with a
 * {@link HttpMessageConverter message converter}, in contrast to {@link
 * org.springframework.web.servlet.mvc.support.DefaultHandlerExceptionResolver
 * DefaultHandlerExceptionResolver} which returns a {@link
 * org.springframework.web.servlet.ModelAndView ModelAndView}.
 *
 * <p>If there is no need to write error content to the response body, or when using view resolution
 * (e.g., via {@code ContentNegotiatingViewResolver}), then {@code DefaultHandlerExceptionResolver}
 * is good enough.
 *
 * <p>Note that in order for an {@code @ControllerAdvice} subclass to be detected, {@link
...
public abstract class ResponseEntityExceptionHandler {
  ...
  public final ResponseEntity<Object> handleException(Exception ex, WebRequest request)
    ...
    if (ex instanceof HttpRequestMethodNotSupportedException) {
      ...
      return handleConversionNotSupported(
          (ConversionNotSupportedException) ex, headers, status, request);
      ...
      return handleHttpMessageNotReadable(
          (HttpMessageNotReadableException) ex, headers, status, request);
    } else if (ex instanceof HttpMessageNotWritableException) {
      HttpStatus status = HttpStatus.INTERNAL_SERVER_ERROR;
      return handleHttpMessageNotWritable(
          (HttpMessageNotWritableException) ex, headers, status, request);
   ...
   *
   * <p>This method logs a warning, sets the "Allow" header, and delegates to {@link
   ...
   *
   * <p>This method sets the "Accept" header and delegates to {@link #handleExceptionInternal}.
   *
   * @param ex the exception
   * @param headers the headers to be written to the response
   * @param status the selected response status
