 ...
 * configured ahead of handlers that support any return value type annotated with
...
public class HttpEntityMethodProcessor extends AbstractMessageConverterMethodProcessor {
  ...
  /**
   * Basic constructor with converters only. Suitable for resolving {@code HttpEntity}. For handling
   * {@code ResponseEntity} consider also providing a {@code ContentNegotiationManager}.
   */
   ...
   * {@code HttpEntity} and handling {@code ResponseEntity} without {@code Request~} or {@code
   * ResponseBodyAdvice}.
   */
  ...
  /**
   * Complete constructor for resolving {@code HttpEntity} method arguments. For handling {@code
   * ResponseEntity} consider also providing a {@code ContentNegotiationManager}.
   *
   * @since 4.2
   */
  public HttpEntityMethodProcessor(
  ...
  public Object resolveArgument(
    ...
    if (RequestEntity.class == parameter.getParameterType()) {
      return new RequestEntity<>(
          body, inputMessage.getHeaders(), inputMessage.getMethod(), inputMessage.getURI());
    } else {
  ...
  private Type getHttpEntityType(MethodParameter parameter) {
    ...
    if (parameterType instanceof ParameterizedType) {
      ...
      if (type.getActualTypeArguments().length != 1) {
        throw new IllegalArgumentException(
            "Expected single generic parameter on '"
                + parameter.getParameterName()
                + "' in method "
                + parameter.getMethod());
      }
  ...
  private List<String> getVaryRequestHeadersToAdd(
      HttpHeaders responseHeaders, HttpHeaders entityHeaders) {
  ...
  private boolean isResourceNotModified(
  ...
  private void saveFlashAttributes(
      ModelAndViewContainer mav, NativeWebRequest request, String location) {
