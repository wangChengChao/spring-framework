...
/**
 * The {@code <form>} tag renders an HTML 'form' tag and exposes a binding path to inner tags for
 * binding.
 *
 * <p>Users should place the form object into the {@link
 * org.springframework.web.servlet.ModelAndView ModelAndView} when populating the data for their
 * view. The name of this form object can be configured using the {@link #setModelAttribute
...
public class FormTag extends AbstractHtmlElementTag {
  ...
  /**
   * The name of the {@link javax.servlet.jsp.PageContext} attribute under which the form object
   * name is exposed.
   */
  ...
  @Nullable private String servletRelativeAction;

  private String method = DEFAULT_METHOD;

  @Nullable private String target;

  @Nullable private String enctype;

  @Nullable private String acceptCharset;

  @Nullable private String onsubmit;

  @Nullable private String onreset;

  @Nullable private String autocomplete;

  private String methodParam = DEFAULT_METHOD_PARAM;

  /** Caching a previous nested path, so that it may be reset. */
  @Nullable private String previousNestedPath;
   ...
   * <p>May be a runtime expression.
   *
   * <p>Name is not a valid attribute for form on XHTML 1.0. However, it is sometimes needed for
  ...
  /**
   * Set the value of the '{@code action}' attribute through a value that is to be appended to the
   * current servlet path.
   *
   * <p>May be a runtime expression.
   *
   * @since 3.2.3
  ...
  /**
   * Writes the opening part of the block '{@code form}' tag and exposes the form object name in the
  ...
  protected int writeTagContent(TagWriter tagWriter) throws JspException {
    ...
    if (!isMethodBrowserSupported(getMethod())) {
      ...
      writeOptionalAttribute(
          tagWriter, VALUE_ATTRIBUTE, processFieldValue(inputName, getMethod(), inputType));
      tagWriter.endTag();
    }
    ...
    this.pageContext.setAttribute(
        MODEL_ATTRIBUTE_VARIABLE_NAME, modelAttribute, PageContext.REQUEST_SCOPE);

    // Save previous nestedPath value, build and expose current nestedPath value.
    // Use request scope to expose nestedPath to included pages too.
    this.previousNestedPath =
        (String)
            this.pageContext.getAttribute(NESTED_PATH_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
    this.pageContext.setAttribute(
   ...
   *
   * <p>If the user configured an '{@code action}' value then the result of evaluating this value is
   * used. If the user configured an '{@code servletRelativeAction}' value then the value is
   * prepended with the context and servlet paths, and the result is used. Otherwise, the {@link
   * org.springframework.web.servlet.support.RequestContext#getRequestUri() originating URI} is
   * used.
   *
  ...
  /**
   * Process the action through a {@link RequestDataValueProcessor} instance if one is configured or
   * otherwise returns the action unmodified.
   */
  ...
  /**
   * Closes the '{@code form}' block tag and removes the form object name from the {@link
  ...
  public void doFinally() {
    ...
    if (this.previousNestedPath != null) {
      ...
      this.pageContext.setAttribute(
          NESTED_PATH_VARIABLE_NAME, this.previousNestedPath, PageContext.REQUEST_SCOPE);
    } else {
