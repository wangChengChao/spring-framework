...
public final class RequestMethodsRequestCondition
   ...
   *
   * @param requestMethods 0 or more HTTP request methods; if, 0 the condition will match to every
   *     request
   */
  ...
  /**
   * Returns a new instance with a union of the HTTP request methods from "this" and the "other"
   * instance.
   */
  @Override
  ...
  /**
   * Check if any of the HTTP request methods match the given request and return an instance that
   * contains the matching HTTP request method only.
   *
   * @param request the current request
   * @return the same instance if the condition is empty (unless the request method is HTTP
   *     OPTIONS), a new condition with the matched request method, or {@code null} if there is no
   *     match or the condition is empty and the request method is OPTIONS.
   */
  @Override
  @Nullable
  ...
  /**
   * On a pre-flight request match to the would-be, actual request. Hence empty conditions is a
   * match, otherwise try to match to the HTTP method in the "Access-Control-Request-Method" header.
   */
  @Nullable
   ...
   * #getMatchingCondition(HttpServletRequest)} and therefore each instance contains the matching
   * HTTP request method only or is otherwise empty.
   */
  @Override
  public int compareTo(RequestMethodsRequestCondition other, HttpServletRequest request) {
    if (other.methods.size() != this.methods.size()) {
      ...
      if (this.methods.contains(RequestMethod.HEAD) && other.methods.contains(RequestMethod.GET)) {
        return -1;
      } else if (this.methods.contains(RequestMethod.GET)
          && other.methods.contains(RequestMethod.HEAD)) {
        return 1;
      }
    }
    return 0;
