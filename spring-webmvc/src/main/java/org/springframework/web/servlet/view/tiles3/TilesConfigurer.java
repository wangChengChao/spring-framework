...
/**
 * Helper class to configure Tiles 3.x for the Spring Framework. See <a
 * href="https://tiles.apache.org">https://tiles.apache.org</a> for more information about Tiles,
 * which basically is a templating mechanism for web applications using JSPs and other template
 * engines.
 *
 * <p>The TilesConfigurer simply configures a TilesContainer using a set of files containing
 * definitions, to be accessed by {@link TilesView} instances. This is a Spring-based alternative
 * (for usage in Spring configuration) to the Tiles-provided {@code ServletContextListener} (e.g.
 * {@link org.apache.tiles.extras.complete.CompleteAutoloadTilesListener} for usage in {@code
 * web.xml}.
 *
 * <p>TilesViews can be managed by any {@link org.springframework.web.servlet.ViewResolver}. For
 * simple convention-based view resolution, consider using {@link TilesViewResolver}.
 *
 ...
 * </pre>
 *
 * The values in the list are the actual Tiles XML files containing the definitions. If the list is
 * not specified, the default is {@code "/WEB-INF/tiles.xml"}.
 *
 * <p>Note that in Tiles 3 an underscore in the name of a file containing Tiles definitions is used
 * to indicate locale information, for example:
 *
 * <pre class="code">
 * &lt;bean id="tilesConfigurer" class="org.springframework.web.servlet.view.tiles3.TilesConfigurer">
 *   &lt;property name="definitions">
...
public class TilesConfigurer implements ServletContextAware, InitializingBean, DisposableBean {
  ...
  private static final boolean tilesElPresent =
      ClassUtils.isPresent(
          "org.apache.tiles.el.ELAttributeEvaluator", TilesConfigurer.class.getClassLoader());

  protected final Log logger = LogFactory.getLog(getClass());

  @Nullable private TilesInitializer tilesInitializer;

  @Nullable private String[] definitions;

  private boolean checkRefresh = false;

  private boolean validateDefinitions = true;

  @Nullable private Class<? extends DefinitionsFactory> definitionsFactoryClass;

  @Nullable private Class<? extends PreparerFactory> preparerFactoryClass;
   ...
   *
   * <p>Default is a variant of {@link org.apache.tiles.startup.DefaultTilesInitializer}, respecting
   * the "definitions", "preparerFactoryClass" etc properties on this configurer.
   *
   * <p><b>NOTE: Specifying a custom TilesInitializer effectively disables all other bean properties
   * on this configurer.</b> The entire initialization procedure is then left to the
   * TilesInitializer as specified.
   */
   ...
   *
   * <p>See {@link org.apache.tiles.extras.complete.CompleteAutoloadTilesContainerFactory} for
   * details on the complete-autoload mode.
   *
   * <p><b>NOTE: Specifying the complete-autoload mode effectively disables all other bean
   * properties on this configurer.</b> The entire initialization procedure is then left to {@link
  ...
  /**
   * Set the Tiles definitions, i.e. the list of files containing the definitions. Default is
   * "/WEB-INF/tiles.xml".
   */
  ...
  /**
   * Set the {@link org.apache.tiles.definition.DefinitionsFactory} implementation to use. Default
   * is {@link org.apache.tiles.definition.UnresolvingLocaleDefinitionsFactory}, operating on
   * definition resource URLs.
   *
   * <p>Specify a custom DefinitionsFactory, e.g. a UrlDefinitionsFactory subclass, to customize the
   * creation of Tiles Definition objects. Note that such a DefinitionsFactory has to be able to
   * handle {@link java.net.URL} source objects, unless you configure a different
   * TilesContainerFactory.
   */
  public void setDefinitionsFactoryClass(
   ...
   * Default is {@link org.apache.tiles.preparer.factory.BasicPreparerFactory}, creating shared
   * instances for specified preparer classes.
   *
   * <p>Specify {@link SimpleSpringPreparerFactory} to autowire {@link
   * org.apache.tiles.preparer.ViewPreparer} instances based on specified preparer classes, applying
   * Spring's container callbacks as well as applying configured Spring BeanPostProcessors. If
   * Spring's context-wide annotation-config has been activated, annotations in ViewPreparer classes
   * will be automatically detected and applied.
   *
   * <p>Specify {@link SpringBeanPreparerFactory} to operate on specified preparer <i>names</i>
   ...
   * application context in this case, allowing for the use of scoped beans etc. Note that you need
   * to define one Spring bean definition per preparer name (as used in your Tiles definitions).
   *
  ...
  /**
   * Set whether to use a MutableTilesContainer (typically the CachingTilesContainer implementation)
   * for this application. Default is "false".
   *
  ...
  /**
   * Creates and exposes a TilesContainer for this web application, delegating to the
   * TilesInitializer.
   *
  ...
  private class SpringTilesContainerFactory extends BasicTilesContainerFactory {
    ...
    protected TilesContainer createDecoratedContainer(
        TilesContainer originalContainer, ApplicationContext context) {
      return (useMutableTilesContainer
          ? new CachingTilesContainer(originalContainer)
          : originalContainer);
    }
    ...
    protected BaseLocaleUrlDefinitionDAO instantiateLocaleDefinitionDao(
      ...
      BaseLocaleUrlDefinitionDAO dao =
    ...
    protected DefinitionsReader createDefinitionsReader(ApplicationContext context) {
      DigesterDefinitionsReader reader =
          (DigesterDefinitionsReader) super.createDefinitionsReader(context);
    ...
    protected DefinitionsFactory createDefinitionsFactory(
      ...
      if (definitionsFactoryClass != null) {
        ...
        if (bw.isWritableProperty("definitionDAO")) {
          bw.setPropertyValue(
              "definitionDAO", createLocaleDefinitionDao(applicationContext, resolver));
        }
        return factory;
      } else {
    ...
    protected AttributeEvaluatorFactory createAttributeEvaluatorFactory(
        ApplicationContext context, LocaleResolver resolver) {
