 ...
 * in a {@link ResourceBundle}, specified by the bundle basename.
 *
 * <p>The bundle is typically defined in a properties file, located in the classpath. The default
 * bundle basename is "views".
 *
 * <p>This {@code ViewResolver} supports localized view definitions, using the default support of
 * {@link java.util.PropertyResourceBundle}. For example, the basename "views" will be resolved as
 * class path resources "views_de_AT.properties", "views_de.properties", "views.properties" - for a
 * given Locale "de_AT".
 *
...
public class ResourceBundleViewResolver extends AbstractCachingViewResolver
  ...
  @Nullable private String defaultParentView;

  @Nullable private Locale[] localesToInitialize;
  ...
  /**
   * Set a single basename, following {@link java.util.ResourceBundle} conventions. The default is
   * "views".
   *
   * <p>{@code ResourceBundle} supports different locale suffixes. For example, a base name of
   * "views" might map to {@code ResourceBundle} files "views", "views_en_au" and "views_de".
   *
   * <p>Note that ResourceBundle names are effectively classpath locations: As a consequence, the
   * JDK's standard ResourceBundle treats dots as package separators. This means that "test.theme"
   * is effectively equivalent to "test/theme", just like it is for programmatic {@code
  ...
  /**
   * Set an array of basenames, each following {@link java.util.ResourceBundle} conventions. The
   * default is a single basename "views".
   *
   * <p>{@code ResourceBundle} supports different locale suffixes. For example, a base name of
   * "views" might map to {@code ResourceBundle} files "views", "views_en_au" and "views_de".
   *
  ...
  /**
   * Set the {@link ClassLoader} to load resource bundles with. Default is the thread context {@code
   * ClassLoader}.
   */
   ...
   *
   * <p>Default is the specified bundle {@code ClassLoader}, usually the thread context {@code
   * ClassLoader}.
   */
   ...
   *
   * <p>This avoids repeated "yyy1.(parent)=xxx", "yyy2.(parent)=xxx" definitions in the bundle,
   * especially if all defined views share the same parent.
   *
   * <p>The parent will typically define the view class and common attributes. Concrete views might
   * simply consist of a URL definition then: a la "yyy1.url=/my.jsp", "yyy2.url=/your.jsp".
   *
   * <p>View definitions that define their own parent or carry their own class can still override
   * this. Strictly speaking, the rule that a default parent setting does not apply to a bean
   * definition that carries a class is there for backwards compatibility reasons. It still matches
   ...
   *
   * <p>Allows for pre-initialization of common Locales, eagerly checking the view configuration for
   * those Locales.
   */
  ...
  /**
   * Initialize the View {@link BeanFactory} from the {@code ResourceBundle}, for the given {@link
   * Locale locale}.
   *
