...
public class DefaultEntityResponseBuilderTests {
  ...
  public void fromObjectTypeReference() {
    String body = "foo";
    EntityResponse<String> response =
        EntityResponse.fromObject(body, new ParameterizedTypeReference<String>() {}).build();
  ...
  public void headers() {
    ...
    headers.set("foo", "bar");
    EntityResponse<String> result =
        EntityResponse.fromObject(body).headers(h -> h.addAll(headers)).build();
  ...
  public void notModifiedLastModified() throws ServletException, IOException {
    ...
    EntityResponse<String> entityResponse =
        EntityResponse.fromObject("bar").lastModified(oneMinuteBeforeNow).build();

    MockHttpServletRequest mockRequest = new MockHttpServletRequest("GET", "https://example.com");
    mockRequest.addHeader(
        HttpHeaders.IF_MODIFIED_SINCE, DateTimeFormatter.RFC_1123_DATE_TIME.format(now));
