...
public class RequestResponseBodyAdviceChainTests {
  ...
  public void setup() {
    ...
    this.paramType =
        new MethodParameter(ClassUtils.getMethod(this.getClass(), "handle", String.class), 0);
    this.returnType =
        new MethodParameter(ClassUtils.getMethod(this.getClass(), "handle", String.class), -1);
  ...
  public void requestBodyAdvice() throws IOException {
    ...
    given(
            requestAdvice.beforeBodyRead(
                eq(this.request), eq(this.paramType), eq(String.class), eq(this.converterType)))
        .willReturn(wrapped);
