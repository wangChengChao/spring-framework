...
/**
 * Test various scenarios for detecting method-level and method parameter annotations depending on
 * where they are located -- on interfaces, parent classes, in parameterized methods, or in
...
class HandlerMethodAnnotationDetectionTests {
  ...
  private void setUp(Class<?> controllerType, boolean useAutoProxy) {
    ...
    context.registerBeanDefinition(
        "handlerMapping", new RootBeanDefinition(RequestMappingHandlerMapping.class));
    context.registerBeanDefinition(
        "handlerAdapter", new RootBeanDefinition(RequestMappingHandlerAdapter.class));
    context.registerBeanDefinition(
        "exceptionResolver", new RootBeanDefinition(ExceptionHandlerExceptionResolver.class));
    if (useAutoProxy) {
      ...
      context.registerBeanDefinition(
          "controllerAdvice", new RootBeanDefinition(ControllerAdvisor.class));
    }
    context.refresh();
  ...
  void testRequestMappingMethod(Class<?> controllerType, boolean useAutoProxy) throws Exception {
    ...
    ModelAndView mav =
        handlerAdapter.handle(request, new MockHttpServletResponse(), chain.getHandler());
    ...
    exceptionResolver.resolveException(
        request, response, chain.getHandler(), new Exception("failure"));
  ...
  static class InterfaceController implements MappingInterface {
    ...
    public void initBinder(
        WebDataBinder dataBinder, @RequestParam("datePattern") String thePattern) {
  ...
  static class ParameterizedAbstractClassController
      extends MappingGenericAbstractClass<String, Date, Date> {
  ...
  static
  class ParameterizedSubclassDoesNotOverrideConcreteImplementationsFromGenericAbstractSuperclass
      extends MappedGenericAbstractClassWithConcreteImplementations<String, Date, Date> {
   ...
   *
   * <p>Cannot be used as JDK dynamic proxy since parameterized interface does not contain type
   * information.
   */
  static class ParameterizedInterfaceController
      implements MappingGenericInterface<String, Date, Date> {
