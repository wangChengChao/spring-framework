...
/**
 * A test fixture with a sub-class of {@link WebMvcConfigurationSupport} that also implements the
 * various {@link WebMvcConfigurer} extension points.
 *
 * <p>The former doesn't implement the latter but the two must have compatible callback method
 * signatures to support moving from simple to advanced configuration -- i.e. dropping @EnableWebMvc
 * + WebMvcConfigurer and extending directly from WebMvcConfigurationSupport.
 *
...
public class WebMvcConfigurationSupportExtensionTests {
  ...
  public void handlerMappings() throws Exception {
    RequestMappingHandlerMapping rmHandlerMapping =
        this.config.requestMappingHandlerMapping(
            this.config.mvcContentNegotiationManager(),
            this.config.mvcConversionService(),
    ...
    assertThat(chain.getInterceptors()[2].getClass())
        .isEqualTo(ConversionServiceExposingInterceptor.class);
    assertThat(chain.getInterceptors()[3].getClass())
        .isEqualTo(ResourceUrlProviderExposingInterceptor.class);
    ...
    assertThat(info.getPatternsCondition().getPatterns())
        .isEqualTo(Collections.singleton("/api/user/{id}"));

    AbstractHandlerMapping handlerMapping =
        (AbstractHandlerMapping)
            this.config.viewControllerHandlerMapping(
    ...
    handlerMapping =
        (AbstractHandlerMapping)
            this.config.resourceHandlerMapping(
                this.config.mvcUrlPathHelper(),
                this.config.mvcPathMatcher(),
                this.config.mvcContentNegotiationManager(),
                this.config.mvcConversionService(),
                this.config.mvcResourceUrlProvider());
    handlerMapping.setApplicationContext(this.context);
    ...
    assertThat(chain).isNotNull();
    assertThat(chain.getHandler()).isNotNull();
    assertThat(chain.getInterceptors().length)
        .as(Arrays.toString(chain.getInterceptors()))
        .isEqualTo(5);
    assertThat(chain.getInterceptors()[0].getClass().getSimpleName()).isEqualTo("CorsInterceptor");
    ...
    assertThat(chain.getInterceptors()[3].getClass())
        .isEqualTo(ConversionServiceExposingInterceptor.class);
    assertThat(chain.getInterceptors()[4].getClass())
        .isEqualTo(ResourceUrlProviderExposingInterceptor.class);
  ...
  public void requestMappingHandlerAdapter() throws Exception {
    RequestMappingHandlerAdapter adapter =
        this.config.requestMappingHandlerAdapter(
            this.config.mvcContentNegotiationManager(),
            this.config.mvcConversionService(),
    ...
    ObjectMapper objectMapper =
        ((MappingJackson2HttpMessageConverter) converters.get(1)).getObjectMapper();
    ...
    assertThat(objectMapper.getDeserializationConfig().isEnabled(FAIL_ON_UNKNOWN_PROPERTIES))
        .isFalse();
    ...
    List<HandlerMethodArgumentResolver> argResolvers =
        (List<HandlerMethodArgumentResolver>)
            fieldAccessor.getPropertyValue("customArgumentResolvers");
    assertThat(argResolvers.size()).isEqualTo(1);

    List<HandlerMethodReturnValueHandler> handlers =
        (List<HandlerMethodReturnValueHandler>)
            fieldAccessor.getPropertyValue("customReturnValueHandlers");
  ...
  public void contentNegotiation() throws Exception {
    ...
    assertThat(manager.resolveMediaTypes(webRequest))
        .isEqualTo(Collections.singletonList(APPLICATION_JSON));

    request.setRequestURI("/foo.xml");
    assertThat(manager.resolveMediaTypes(webRequest))
        .isEqualTo(Collections.singletonList(APPLICATION_XML));
    ...
    assertThat(manager.resolveMediaTypes(webRequest))
        .isEqualTo(Collections.singletonList(APPLICATION_ATOM_XML));
  ...
  public void exceptionResolvers() throws Exception {
    List<HandlerExceptionResolver> resolvers =
        ((HandlerExceptionResolverComposite) this.config.handlerExceptionResolver(null))
  ...
  public void viewResolvers() throws Exception {
    ViewResolverComposite viewResolver =
        (ViewResolverComposite)
            this.config.mvcViewResolver(this.config.mvcContentNegotiationManager());
    ...
    assertThat((boolean) (Boolean) accessor.getPropertyValue("useNotAcceptableStatusCode"))
        .isFalse();
   ...
   * class is also to ensure the two are in sync with each other. Effectively that ensures that
   * application config classes that use the combo {@code @EnableWebMvc} plus WebMvcConfigurer can
   * switch to extending WebMvcConfigurationSupport directly for more advanced configuration needs.
   */
  private class TestWebMvcConfigurationSupport extends WebMvcConfigurationSupport
    ...
    public void addViewControllers(ViewControllerRegistry registry) {
      ...
      registry
          .addRedirectViewController("/old", "/new")
