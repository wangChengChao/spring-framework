...
public class RequestMappingHandlerAdapterTests {
  ...
  public void setAlwaysUseRedirectAttributes() throws Exception {
    HandlerMethodArgumentResolver redirectAttributesResolver =
    ...
    this.handlerAdapter.setArgumentResolvers(
        Arrays.asList(redirectAttributesResolver, modelResolver));
    ...
    HandlerMethod handlerMethod =
        handlerMethod(new RedirectAttributeController(), "handle", Model.class);
    ModelAndView mav = this.handlerAdapter.handle(request, response, handlerMethod);

    assertThat(mav.getModel().isEmpty())
        .as("Without RedirectAttributes arg, model should be empty")
        .isTrue();
  }
  ...
  public void responseBodyAdvice() throws Exception {
    ...
    assertThat(this.response.getContentAsString())
        .isEqualTo("{\"status\":400,\"message\":\"body\"}");
  }

  private HandlerMethod handlerMethod(Object handler, String methodName, Class<?>... paramTypes)
      throws Exception {
    Method method = handler.getClass().getDeclaredMethod(methodName, paramTypes);
    return new InvocableHandlerMethod(handler, method);
  }

  private void assertMethodProcessorCount(
      int resolverCount, int initBinderResolverCount, int handlerCount) {
    assertThat(this.handlerAdapter.getArgumentResolvers().size()).isEqualTo(resolverCount);
    assertThat(this.handlerAdapter.getInitBinderArgumentResolvers().size())
        .isEqualTo(initBinderResolverCount);
  ...
  /**
   * This class additionally implements {@link RequestBodyAdvice} solely for the purpose of
   * verifying that controller advice implementing both {@link ResponseBodyAdvice} and {@link
   * RequestBodyAdvice} does not get registered twice.
   *
