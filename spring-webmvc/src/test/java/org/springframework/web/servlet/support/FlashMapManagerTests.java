...
public class FlashMapManagerTests {
  ...
  public void retrieveAndUpdateMatchByOriginatingPath() {
    ...
    assertThat(this.flashMapManager.getFlashMaps().size())
        .as("Input FlashMap should have been removed")
        .isEqualTo(0);
  }
  ...
  public void retrieveAndUpdateMatchByParams() {
    ...
    assertThat(this.flashMapManager.getFlashMaps().size())
        .as("FlashMap should not have been removed")
        .isEqualTo(1);
  ...
  public void saveOutputFlashMap() {
    ...
    this.flashMapManager.setFlashMapTimeout(
        -1); // expire immediately so we can check expiration started
    this.flashMapManager.saveOutputFlashMap(flashMap, this.request, this.response);
    List<FlashMap> allMaps = this.flashMapManager.getFlashMaps();
  ...
  public void saveOutputFlashMapDecodeParameters() throws Exception {
    ...
    flashMap =
        this.flashMapManager.retrieveAndUpdate(requestAfterRedirect, new MockHttpServletResponse());
  ...
  private static class TestFlashMapManager extends AbstractFlashMapManager {
    ...
    @Override
    protected void updateFlashMaps(
        List<FlashMap> maps, HttpServletRequest request, HttpServletResponse response) {
      this.flashMaps = maps;
    }
  }
}
