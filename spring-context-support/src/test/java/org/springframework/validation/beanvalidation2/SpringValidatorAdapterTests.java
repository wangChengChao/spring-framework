...
public class SpringValidatorAdapterTests {
  ...
  private final SpringValidatorAdapter validatorAdapter =
      new SpringValidatorAdapter(nativeValidator);
  ...
  public void testNoStringArgumentValue() throws Exception {
    ...
    assertThat(error.unwrap(ConstraintViolation.class).getPropertyPath().toString())
        .isEqualTo("password");
    assertThat(SerializationTestUtils.serializeAndDeserialize(error.toString()))
  ...
  @Test // SPR-13406
  public void
      testApplyMessageSourceResolvableToStringArgumentValueWithUnresolvedLogicalFieldName() {
    TestBean testBean = new TestBean();
    ...
    assertThat(error1.unwrap(ConstraintViolation.class).getPropertyPath().toString())
        .isEqualTo("email");
    assertThat(error2.contains(ConstraintViolation.class)).isTrue();
    assertThat(error2.unwrap(ConstraintViolation.class).getPropertyPath().toString())
        .isEqualTo("confirmEmail");
  }
  ...
  public static class Parent {
    ...
    @Valid private Set<Child> childSet = new LinkedHashSet<>();

    @Valid private List<Child> childList = new LinkedList<>();
  ...
  public class BeanWithListElementConstraint {
    ...
    @Valid private List<@NotNull String> property;
