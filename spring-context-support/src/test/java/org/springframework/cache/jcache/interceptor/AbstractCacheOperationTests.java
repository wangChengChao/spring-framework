...
public abstract class AbstractCacheOperationTests<O extends JCacheOperation<?>>
  ...
  public void simple() {
    ...
    assertThat(operation.getAnnotations().size())
        .as("Unexpected number of annotation on " + operation.getMethod())
        .isEqualTo(1);
    assertThat(operation.getAnnotations().iterator().next())
        .as("Wrong method annotation")
        .isEqualTo(operation.getCacheAnnotation());
  ...
  protected void assertCacheInvocationParameter(
    ...
    assertThat(actual.getParameterPosition())
        .as("wrong parameter position for " + actual)
        .isEqualTo(position);
  }

  protected <A extends Annotation> CacheMethodDetails<A> create(
      Class<A> annotationType, Class<?> targetType, String methodName, Class<?>... parameterTypes) {
