...
/**
 * FreeMarker {@link TemplateLoader} adapter that loads via a Spring {@link ResourceLoader}. Used by
 * {@link FreeMarkerConfigurationFactory} for any resource loader path that cannot be resolved to a
 * {@link java.io.File}.
 *
...
public class SpringTemplateLoader implements TemplateLoader {
  ...
  public SpringTemplateLoader(ResourceLoader resourceLoader, String templateLoaderPath) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "SpringTemplateLoader for FreeMarker: using resource loader ["
              + this.resourceLoader
              + "] and template loader path ["
