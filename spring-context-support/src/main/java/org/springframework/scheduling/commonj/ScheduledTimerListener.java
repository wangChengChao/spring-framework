...
/**
 * JavaBean that describes a scheduled TimerListener, consisting of the TimerListener itself (or a
 * Runnable to create a TimerListener for) and a delay plus period. Period needs to be specified;
 * there is no point in a default for it.
 *
 * <p>The CommonJ TimerManager does not offer more sophisticated scheduling options such as cron
 * expressions. Consider using Quartz for such advanced needs.
 *
 * <p>Note that the TimerManager uses a TimerListener instance that is shared between repeated
 * executions, in contrast to Quartz which instantiates a new Job for each execution.
 *
...
public class ScheduledTimerListener {
  ...
  @Nullable private TimerListener timerListener;
   ...
   */
  public ScheduledTimerListener(
      TimerListener timerListener, long delay, long period, boolean fixedRate) {
    this.timerListener = timerListener;
    this.delay = delay;
    this.period = period;
   ...
   *
   * <p>If the "firstTime" property is specified, this property will be ignored. Specify one or the
   * other, not both.
   */
   ...
   *
   * <p>Default is -1, leading to one-time execution. In case of zero or a positive value, the task
   * will be executed repeatedly, with the given interval in-between executions.
   *
   * <p>Note that the semantics of the period value vary between fixed-rate and fixed-delay
   * execution.
   *
   * <p><b>Note:</b> A period of 0 (for example as fixed delay) <i>is</i> supported, because the
   * CommonJ specification defines this as a legal value. Hence a value of 0 will result in
   * immediate re-execution after a job has finished (not in one-time execution like with {@code
   * java.util.Timer}).
   *
   * @see #setFixedRate
   * @see #isOneTimeTask()
