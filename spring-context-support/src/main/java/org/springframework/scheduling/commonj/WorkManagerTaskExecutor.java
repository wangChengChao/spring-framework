...
/**
 * TaskExecutor implementation that delegates to a CommonJ WorkManager, implementing the {@link
 * commonj.work.WorkManager} interface, which either needs to be specified as reference or through
 * the JNDI name.
 *
 * <p><b>This is the central convenience class for setting up a CommonJ WorkManager in a Spring
 * context.</b>
 *
 * <p>Also implements the CommonJ WorkManager interface itself, delegating all calls to the target
 * WorkManager. Hence, a caller can choose whether it wants to talk to this executor through the
 * Spring TaskExecutor interface or the CommonJ WorkManager interface.
 *
 * <p>The CommonJ WorkManager will usually be retrieved from the application server's JNDI
 * environment, as defined in the server's management console.
 *
 * <p>Note: On the upcoming EE 7 compliant versions of WebLogic and WebSphere, a {@link
 * org.springframework.scheduling.concurrent.DefaultManagedTaskExecutor} should be preferred,
 * following JSR-236 support in Java EE 7.
 *
