...
/**
 * {@link org.springframework.beans.factory.FactoryBean} that retrieves a CommonJ {@link
 * commonj.timers.TimerManager} and exposes it for bean references.
 *
 * <p><b>This is the central convenience class for setting up a CommonJ TimerManager in a Spring
 * context.</b>
 *
 * <p>Allows for registration of ScheduledTimerListeners. This is the main purpose of this class;
 * the TimerManager itself could also be fetched from JNDI via {@link
 * org.springframework.jndi.JndiObjectFactoryBean}. In scenarios that just require static
 * registration of tasks at startup, there is no need to access the TimerManager itself in
 * application code.
 *
...
public class TimerManagerFactoryBean extends TimerManagerAccessor
  ...
  @Nullable private ScheduledTimerListener[] scheduledTimerListeners;

  private final List<Timer> timers = new LinkedList<>();

  /**
   * Register a list of ScheduledTimerListener objects with the TimerManager that this FactoryBean
   * creates. Depending on each ScheduledTimerListener's settings, it will be registered via one of
   * TimerManager's schedule methods.
   *
  ...
  /**
   * Cancels all statically registered Timers on shutdown, and stops the underlying TimerManager (if
   * not shared).
   *
