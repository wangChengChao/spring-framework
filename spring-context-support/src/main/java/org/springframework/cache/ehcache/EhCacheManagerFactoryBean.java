...
/**
 * {@link FactoryBean} that exposes an EhCache {@link net.sf.ehcache.CacheManager} instance
 * (independent or shared), configured from a specified config location.
 *
 * <p>If no config location is specified, a CacheManager will be configured from "ehcache.xml" in
 * the root of the class path (that is, default EhCache initialization - as defined in the EhCache
 * docs - will apply).
 *
 * <p>Setting up a separate EhCacheManagerFactoryBean is also advisable when using
 * EhCacheFactoryBean, as it provides a (by default) independent CacheManager instance and cares for
 * proper shutdown of the CacheManager. EhCacheManagerFactoryBean is also necessary for loading
...
public class EhCacheManagerFactoryBean
  ...
  @Nullable private String cacheManagerName;
  ...
  /**
   * Set whether an existing EhCache CacheManager of the same name will be accepted for this
   * EhCacheManagerFactoryBean setup. Default is "false".
   *
   * <p>Typically used in combination with {@link #setCacheManagerName "cacheManagerName"} but will
   * simply work with the default CacheManager name if none specified. All references to the same
   * CacheManager name (or the same default) in the same ClassLoader space will share the specified
   * CacheManager then.
   *
   ...
   * or independent (typically local within the application). Default is "false", creating an
   * independent local instance.
   *
   * <p><b>NOTE:</b> This feature allows for sharing this EhCacheManagerFactoryBean's CacheManager
   * with any code calling <code>CacheManager.create()</code> in the same ClassLoader space, with no
   * need to agree on a specific CacheManager name. However, it only supports a single
   * EhCacheManagerFactoryBean involved which will control the lifecycle of the underlying
   * CacheManager (in particular, its shutdown).
   *
   * <p>This flag overrides {@link #setAcceptExisting "acceptExisting"} if both are set, since it
   * indicates the 'stronger' mode of sharing.
   *
