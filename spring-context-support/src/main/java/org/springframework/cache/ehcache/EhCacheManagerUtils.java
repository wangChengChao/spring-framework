 ...
 * programmatic bootstrapping from a Spring-provided resource. This is primarily intended for use
 * within {@code @Bean} methods in a Spring configuration class.
 *
 * <p>These methods are a simple alternative to custom {@link CacheManager} setup code. For any
 * advanced purposes, consider using {@link #parseConfiguration}, customizing the configuration
 * object, and then calling the {@link CacheManager#CacheManager(Configuration)} constructor.
 *
...
public abstract class EhCacheManagerUtils {
   ...
   *
   * <p>The CacheManager will be configured from "ehcache.xml" in the root of the class path (that
   * is, default EhCache initialization - as defined in the EhCache docs - will apply). If no
   * configuration file can be found, a fail-safe fallback configuration will be used.
   *
  ...
  /**
   * Parse EhCache configuration from the given resource, for further use with custom {@link
