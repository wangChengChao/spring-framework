...
class CacheResultInterceptor
  ...
  protected void cacheException(
      @Nullable Cache exceptionCache, ExceptionTypeFilter filter, Object cacheKey, Throwable ex) {
    if (exceptionCache == null) {
      return;
    }
  ...
  /**
   * Rewrite the call stack of the specified {@code exception} so that it matches the current call
   * stack up to (included) the specified method invocation.
   *
   * <p>Clone the specified exception. If the exception is not {@code serializable}, the original
   * exception is returned. If no common ancestor can be found, returns the original exception.
   *
   ...
   * @return a clone exception with a rewritten call stack composed of the current call stack up to
   *     (included) the common ancestor specified by the {@code className} and {@code methodName}
   *     arguments, followed by stack trace elements of the specified {@code exception} after the
   *     common ancestor.
   */
  ...
  private static int findCommonAncestorIndex(
      StackTraceElement[] callStack, String className, String methodName) {
