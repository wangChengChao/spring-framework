...
abstract class AbstractJCacheKeyOperation<A extends Annotation> extends AbstractJCacheOperation<A> {
   ...
   */
  protected AbstractJCacheKeyOperation(
      CacheMethodDetails<A> methodDetails, CacheResolver cacheResolver, KeyGenerator keyGenerator) {
  ...
  /**
   * Return the {@link CacheInvocationParameter} for the parameters that are to be used to compute
   * the key.
   *
   * <p>Per the spec, if some method parameters are annotated with {@link
   * javax.cache.annotation.CacheKey}, only those parameters should be part of the key. If none are
   * annotated, all parameters except the parameter annotated with {@link
   * javax.cache.annotation.CacheValue} should be part of the key.
   *
   ...
   * @return the {@link CacheInvocationParameter} instances for the parameters to be used to compute
   *     the key
   */
    ...
    for (CacheParameterDetail keyParameterDetail : this.keyParameterDetails) {
      ...
      if (parameterPosition >= values.length) {
        throw new IllegalStateException(
            "Values mismatch, key parameter at position "
                + parameterPosition
