...
/**
 * Spring's {@link KeyGenerator} implementation that either delegates to a standard JSR-107 {@link
 * javax.cache.annotation.CacheKeyGenerator}, or wrap a standard {@link KeyGenerator} so that only
 * relevant parameters are handled.
 *
...
class KeyGeneratorAdapter implements KeyGenerator {
  ...
  @Nullable private KeyGenerator keyGenerator;

  @Nullable private CacheKeyGenerator cacheKeyGenerator;

  /**
   * Create an instance with the given {@link KeyGenerator} so that {@link
   * javax.cache.annotation.CacheKey} and {@link javax.cache.annotation.CacheValue} are handled
  ...
  /**
   * Return the target key generator to use in the form of either a {@link KeyGenerator} or a {@link
   * CacheKeyGenerator}.
   */
  ...
  public Object generate(Object target, Method method, Object... params) {
    JCacheOperation<?> operation =
        this.cacheOperationSource.getCacheOperation(method, target.getClass());
    ...
    CacheKeyInvocationContext<?> invocationContext =
        createCacheKeyInvocationContext(target, operation, params);
  ...
  private CacheKeyInvocationContext<?> createCacheKeyInvocationContext(
    ...
    AbstractJCacheKeyOperation<Annotation> keyCacheOperation =
        (AbstractJCacheKeyOperation<Annotation>) operation;
