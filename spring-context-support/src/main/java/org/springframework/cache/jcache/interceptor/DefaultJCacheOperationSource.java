...
/**
 * The default {@link JCacheOperationSource} implementation delegating default operations to
 * configurable services with sensible defaults when not present.
 *
...
public class DefaultJCacheOperationSource extends AnnotationJCacheOperationSource
  ...
  @Nullable private SingletonSupplier<CacheManager> cacheManager;

  @Nullable private SingletonSupplier<CacheResolver> cacheResolver;

  @Nullable private SingletonSupplier<CacheResolver> exceptionCacheResolver;
  ...
  /**
   * Construct a new {@code DefaultJCacheOperationSource} with the given cache manager, cache
   * resolver and key generator suppliers, applying the corresponding default if a supplier is not
   * resolvable.
   *
   * @since 5.1
   */
  public DefaultJCacheOperationSource(
      ...
      @Nullable Supplier<CacheResolver> exceptionCacheResolver,
  ...
  /**
   * Set the default {@link CacheManager} to use to lookup cache by name. Only mandatory if the
   * {@linkplain CacheResolver cache resolver} has not been set.
   */
  ...
  /**
   * Set the default {@link KeyGenerator}. If none is set, a {@link SimpleKeyGenerator} honoring the
   * JSR-107 {@link javax.cache.annotation.CacheKey} and {@link javax.cache.annotation.CacheValue}
   * will be used.
   */
  ...
  protected <T> T getBean(Class<T> type) {
    Assert.state(
        this.beanFactory != null, () -> "BeanFactory required for resolution of [" + type + "]");
    try {
  ...
  protected CacheManager getDefaultCacheManager() {
    if (getCacheManager() == null) {
      Assert.state(
          this.beanFactory != null, "BeanFactory required for default CacheManager resolution");
      try {
   ...
   *
   * <p>A non-JSR-107 setup requires either a {@link CacheManager} or a {@link CacheResolver}. If
   * only the latter is specified, it is not possible to extract a default exception {@code
   * CacheResolver} from a custom {@code CacheResolver} implementation so we have to fall back on
   * the {@code CacheManager}.
   *
   * <p>This gives this weird situation of a perfectly valid configuration that breaks all the
   * sudden because the JCache support is enabled. To avoid this we resolve the default exception
   * {@code CacheResolver} as late as possible to avoid such hard requirement in other cases.
   */
