 ...
 *
 * <p>Mirrors the simple setters of {@link org.springframework.mail.SimpleMailMessage}, directly
 * applying the values to the underlying MimeMessage. Allows for defining a character encoding for
 * the entire message, automatically applied by all methods of this helper class.
 *
 * <p>Offers support for HTML text content, inline elements such as images, and typical mail
 * attachments. Also supports personal names that accompany mail addresses. Note that advanced
 * settings can still be applied directly to the underlying MimeMessage object!
 *
 * <p>Typically used in {@link MimeMessagePreparator} implementations or {@link JavaMailSender}
 * client code: simply instantiating it as a MimeMessage wrapper, invoking setters on the wrapper,
 * using the underlying MimeMessage for mail sending. Also used internally by {@link
 * JavaMailSenderImpl}.
 *
 ...
 * });</pre>
 *
 * Consider using {@link MimeMailMessage} (which implements the common {@link
 * org.springframework.mail.MailMessage} interface, just like {@link
 * org.springframework.mail.SimpleMailMessage}) on top of this helper, in order to let message
 * population code interact with a simple message or a MIME message through a common interface.
 *
 * <p><b>Warning regarding multipart mails:</b> Simple MIME messages that just contain HTML text but
 * no inline elements or attachments will work on more or less any email client that is capable of
 * HTML rendering. However, inline elements and attachments are still a major compatibility issue
 * between email clients: It's virtually impossible to get inline elements and attachments working
 * across Microsoft Outlook, Lotus Notes and Mac Mail. Consider choosing a specific multipart mode
 * for your needs: The javadoc on the MULTIPART_MODE constants contains more detailed information.
 *
...
public class MimeMessageHelper {
   ...
   * Texts, inline elements and attachements will all get added to that root element.
   *
   * <p>This was Spring 1.0's default behavior. It is known to work properly on Outlook. However,
   * other mail clients tend to misinterpret inline elements as attachments and/or show attachments
   * inline as well.
   */
   ...
   * Texts, inline elements and attachements will all get added to that root element.
   *
   * <p>This was the default behavior from Spring 1.1 up to 1.2 final. This is the "Microsoft
   * multipart mode", as natively sent by Outlook. It is known to work properly on Outlook, Outlook
   * Express, Yahoo Mail, and to a large degree also on Mac Mail (with an additional attachment
   * listed for an inline element, despite the inline element also shown inline). Does not work
   * properly on Lotus Notes (attachments won't be shown there).
   */
  public static final int MULTIPART_MODE_RELATED = 2;

  /**
   * Constant indicating a multipart message with a root multipart element "mixed" plus a nested
   * multipart element of type "related". Texts and inline elements will get added to the nested
   * "related" element, while attachments will get added to the "mixed" root element.
   *
   * <p>This is the default since Spring 1.2.1. This is arguably the most correct MIME structure,
   * according to the MIME spec: It is known to work properly on Outlook, Outlook Express, Yahoo
   * Mail, and Lotus Notes. Does not work properly on Mac Mail. If you target Mac Mail or experience
   * issues with specific mails on Outlook, consider using MULTIPART_MODE_RELATED instead.
   */
  ...
  @Nullable private MimeMultipart rootMimeMultipart;
  ...
  /**
   * Create a new MimeMessageHelper for the given MimeMessage, assuming a simple text message (no
   * multipart content, i.e. no alternative texts and no inline elements or attachments).
   *
   * <p>The character encoding for the message will be taken from the passed-in MimeMessage object,
   * if carried there. Else, JavaMail's default encoding will be used.
   *
   ...
   * alternative texts, inline elements and attachments) if requested.
   *
   * <p>Consider using the MimeMessageHelper constructor that takes a multipartMode argument to
   * choose a specific multipart mode other than MULTIPART_MODE_MIXED_RELATED.
   *
   * <p>The character encoding for the message will be taken from the passed-in MimeMessage object,
   * if carried there. Else, JavaMail's default encoding will be used.
   *
   * @param mimeMessage the mime message to work on
   * @param multipart whether to create a multipart message that supports alternative texts, inline
   ...
   *
   * @param mimeMessage the mime message to work on
   * @param multipartMode which kind of multipart message to create (MIXED, RELATED, MIXED_RELATED,
   *     or NO)
   * @throws MessagingException if multipart creation failed
   ...
   *
   * <p>Texts and inline elements can either be stored in the root element itself
   * (MULTIPART_MODE_MIXED, MULTIPART_MODE_RELATED) or in a nested element rather than the root
   * element directly (MULTIPART_MODE_MIXED_RELATED).
   *
   * <p>By default, the root MimeMultipart element will be of type "mixed" (MULTIPART_MODE_MIXED) or
   * "related" (MULTIPART_MODE_RELATED). The main multipart element will either be added as nested
   * element of type "related" (MULTIPART_MODE_MIXED_RELATED) or be identical to the root element
   * itself (MULTIPART_MODE_MIXED, MULTIPART_MODE_RELATED).
   *
   * @param mimeMessage the MimeMessage object to add the root MimeMultipart object to
   * @param multipartMode the multipart mode, as passed into the constructor (MIXED, RELATED,
   *     MIXED_RELATED, or NO)
   * @throws MessagingException if multipart creation failed
   ...
   *
   * @param root the root MimeMultipart object, which attachments will be added to; or {@code null}
   *     to indicate no multipart at all
   * @param main the main MimeMultipart object, which text(s) and inline elements will be added to
   *     (can be the same as the root multipart object, or an element nested underneath the root
   *     multipart element)
   */
  protected final void setMimeMultiparts(
      @Nullable MimeMultipart root, @Nullable MimeMultipart main) {
   ...
   *
   * <p>This will be the direct content of the MimeMessage, in case of a multipart mail.
   *
  ...
  /**
   * Return the underlying MIME "multipart/related" object, if any. Can be used to manually add body
   * parts, inline elements, etc.
   *
   * <p>This will be nested within the root MimeMultipart, in case of a multipart mail.
   *
   * @throws IllegalStateException if this helper is not in multipart mode
   * @see #isMultipart
   ...
   * @return the default encoding associated with the MimeMessage, or {@code null} if none found
   */
  @Nullable
   ...
   * default {@code validateAddress method} for validation on older JavaMail versions (or for custom
   * validation).
   *
   * @see #validateAddress
   */
   ...
   *
   * <p>Note that this method will just work on JavaMail >= 1.3. You can override it for validation
  ...
  private InternetAddress parseAddress(String address) throws MessagingException {
    ...
    try {
      return (getEncoding() != null
          ? new InternetAddress(raw.getAddress(), raw.getPersonal(), getEncoding())
          : raw);
  ...
  /**
   * Set the given text directly as content in non-multipart mode or as default body part in
   * multipart mode. Always applies the default content type "text/plain".
   *
   * <p><b>NOTE:</b> Invoke {@link #addInline} <i>after</i> {@code setText}; else, mail readers
   * might not be able to resolve inline references correctly.
   *
  ...
  /**
   * Set the given text directly as content in non-multipart mode or as default body part in
   * multipart mode. The "html" flag determines the content type to apply.
   *
   * <p><b>NOTE:</b> Invoke {@link #addInline} <i>after</i> {@code setText}; else, mail readers
   * might not be able to resolve inline references correctly.
   *
   * @param text the text for the message
   * @param html whether to apply content type "text/html" for an HTML mail, using default content
   *     type ("text/plain") else
   * @throws MessagingException in case of errors
   */
  ...
  /**
   * Set the given plain text and HTML text as alternatives, offering both options to the email
   * client. Requires multipart mode.
   *
   * <p><b>NOTE:</b> Invoke {@link #addInline} <i>after</i> {@code setText}; else, mail readers
   * might not be able to resolve inline references correctly.
   *
  ...
  /**
   * Add an inline element to the MimeMessage, taking the content from a {@code
   * javax.activation.DataSource}.
   *
   * <p>Note that the InputStream returned by the DataSource implementation needs to be a <i>fresh
   * one on each call</i>, as JavaMail will invoke {@code getInputStream()} multiple times.
   *
   * <p><b>NOTE:</b> Invoke {@code addInline} <i>after</i> {@link #setText}; else, mail readers
   * might not be able to resolve inline references correctly.
   *
   * @param contentId the content ID to use. Will end up as "Content-ID" header in the body part,
   *     surrounded by angle brackets: e.g. "myId" -> "&lt;myId&gt;". Can be referenced in HTML
   *     source via src="cid:myId" expressions.
   * @param dataSource the {@code javax.activation.DataSource} to take the content from, determining
   *     the InputStream and the content type
   * @throws MessagingException in case of errors
  ...
  /**
   * Add an inline element to the MimeMessage, taking the content from a {@code java.io.File}.
   *
   * <p>The content type will be determined by the name of the given content file. Do not use this
   * for temporary files with arbitrary filenames (possibly ending in ".tmp" or the like)!
   *
   * <p><b>NOTE:</b> Invoke {@code addInline} <i>after</i> {@link #setText}; else, mail readers
   * might not be able to resolve inline references correctly.
   *
   ...
   * for temporary files with arbitrary filenames (possibly ending in ".tmp" or the like)!
   *
   * <p>Note that the InputStream returned by the Resource implementation needs to be a <i>fresh one
   * on each call</i>, as JavaMail will invoke {@code getInputStream()} multiple times.
   *
   * <p><b>NOTE:</b> Invoke {@code addInline} <i>after</i> {@link #setText}; else, mail readers
   * might not be able to resolve inline references correctly.
   *
  ...
  /**
   * Add an inline element to the MimeMessage, taking the content from an {@code
   * org.springframework.core.InputStreamResource}, and specifying the content type explicitly.
   *
   * <p>You can determine the content type for any given filename via a Java Activation Framework's
   * FileTypeMap, for example the one held by this helper.
   *
   * <p>Note that the InputStream returned by the InputStreamSource implementation needs to be a
   * <i>fresh one on each call</i>, as JavaMail will invoke {@code getInputStream()} multiple times.
   *
   * <p><b>NOTE:</b> Invoke {@code addInline} <i>after</i> {@code setText}; else, mail readers might
   * not be able to resolve inline references correctly.
   *
   * @param contentId the content ID to use. Will end up as "Content-ID" header in the body part,
   *     surrounded by angle brackets: e.g. "myId" -> "&lt;myId&gt;". Can be referenced in HTML
   *     source via src="cid:myId" expressions.
  ...
  /**
   * Add an attachment to the MimeMessage, taking the content from a {@code
   * javax.activation.DataSource}.
   *
   * <p>Note that the InputStream returned by the DataSource implementation needs to be a <i>fresh
   * one on each call</i>, as JavaMail will invoke {@code getInputStream()} multiple times.
   *
   * @param attachmentFilename the name of the attachment as it will appear in the mail (the content
   *     type will be determined by this)
   * @param dataSource the {@code javax.activation.DataSource} to take the content from, determining
   *     the InputStream and the content type
   * @throws MessagingException in case of errors
   * @see #addAttachment(String, org.springframework.core.io.InputStreamSource)
  ...
  /**
   * Add an attachment to the MimeMessage, taking the content from a {@code java.io.File}.
   *
   * <p>The content type will be determined by the name of the given content file. Do not use this
   * for temporary files with arbitrary filenames (possibly ending in ".tmp" or the like)!
   *
  ...
  /**
   * Add an attachment to the MimeMessage, taking the content from an {@code
   * org.springframework.core.io.InputStreamResource}.
   *
   * <p>The content type will be determined by the given filename for the attachment. Thus, any
   * content source will be fine, including temporary files with arbitrary filenames.
   *
   * <p>Note that the InputStream returned by the InputStreamSource implementation needs to be a
   * <i>fresh one on each call</i>, as JavaMail will invoke {@code getInputStream()} multiple times.
   *
   * @param attachmentFilename the name of the attachment as it will appear in the mail
   * @param inputStreamSource the resource to take the content from (all of Spring's Resource
   *     implementations can be passed in here)
   * @throws MessagingException in case of errors
   * @see #addAttachment(String, java.io.File)
   * @see #addAttachment(String, javax.activation.DataSource)
