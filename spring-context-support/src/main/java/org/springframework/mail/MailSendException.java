...
/**
 * Exception thrown when a mail sending error is encountered. Can register failed messages with
 * their exceptions.
 *
...
public class MailSendException extends MailException {
  ...
  @Nullable private final Exception[] messageExceptions;
  ...
  /**
   * Constructor for registration of failed messages, with the messages that failed as keys, and the
   * thrown exceptions as values.
   *
   * <p>The messages should be the same that were originally passed to the invoked send method.
   *
   * @param msg the detail message
   * @param cause the root cause from the mail API in use
   * @param failedMessages a Map of failed messages as keys and thrown exceptions as values
   */
  public MailSendException(
      @Nullable String msg, @Nullable Throwable cause, Map<Object, Exception> failedMessages) {
    super(msg, cause);
   ...
   *
   * <p>Note that a general mail server connection failure will not result in failed messages being
   * returned here: A message will only be contained here if actually sending it was attempted but
   * failed.
   *
   * <p>The messages will be the same that were originally passed to the invoked send method, that
   * is, SimpleMailMessages in case of using the generic MailSender interface.
   *
   * <p>In case of sending MimeMessage instances via JavaMailSender, the messages will be of type
   * MimeMessage.
   *
   * <p><b>NOTE:</b> This Map will not be available after serialization. Use {@link
   * #getMessageExceptions()} in such a scenario, which will be available after serialization as
   * well.
   *
