...
/**
 * Integration tests for auto proxy creation by advisor recognition working in conjunction with
...
class AdvisorAutoProxyCreatorIntegrationTests {
  ...
  void testDefaultExclusionPrefix() throws Exception {
    DefaultAdvisorAutoProxyCreator aapc =
        (DefaultAdvisorAutoProxyCreator) getBeanFactory().getBean(ADVISOR_APC_BEAN_NAME);
    assertThat(aapc.getAdvisorBeanNamePrefix())
        .isEqualTo((ADVISOR_APC_BEAN_NAME + DefaultAdvisorAutoProxyCreator.SEPARATOR));
  ...
  void testTransactionAttributeOnMethod() throws Exception {
    ...
    CallCountingTransactionManager txMan =
        (CallCountingTransactionManager) bf.getBean(TXMANAGER_BEAN_NAME);
...
class NeverMatchAdvisor extends StaticMethodMatcherPointcutAdvisor {
  ...
  /**
   * This method is solely to allow us to create a mixture of dependencies in the bean definitions.
   * The dependencies don't have any meaning, and don't <b>do</b> anything.
   */
