...
class AopNamespaceHandlerScopeIntegrationTests {
  ...
  @Autowired ITestBean singletonScoped;

  @Autowired ITestBean requestScoped;

  @Autowired ITestBean sessionScoped;

  @Autowired ITestBean sessionScopedAlias;

  @Autowired ITestBean testBean;
  ...
  void testSingletonScoping() throws Exception {
    ...
    ITestBean deserialized =
        (ITestBean) SerializationTestUtils.serializeAndDeserialize(singletonScoped);
  ...
  void testRequestScoping() throws Exception {
    ...
    assertThat(((Advised) requestScoped).getAdvisors().length > 0)
        .as("Should have advisors")
        .isTrue();
  }
  ...
  void testSessionScoping() throws Exception {
    ...
    assertThat(((Advised) sessionScoped).getAdvisors().length > 0)
        .as("Should have advisors")
        .isTrue();
  }
}
