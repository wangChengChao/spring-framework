...
public abstract class Literal extends SpelNodeImpl {
  ...
  @Nullable private final String originalValue;
  ...
  /**
   * Process the string form of a number, using the specified base if supplied and return an
   * appropriate literal to hold it. Any suffix to indicate a long will be taken into account
   * (either 'l' or 'L' is supported).
   *
  ...
  public static Literal getLongLiteral(String numberToken, int startPos, int endPos, int radix) {
    try {
      ...
      throw new InternalParseException(
          new SpelParseException(startPos, ex, SpelMessage.NOT_A_LONG, numberToken));
    }
  }

  public static Literal getRealLiteral(
    ...
    try {
      ...
      throw new InternalParseException(
          new SpelParseException(startPos, ex, SpelMessage.NOT_A_REAL, numberToken));
    }
  }
}
