...
/**
 * Contains all the messages that can be produced by the Spring Expression Language. Each message
 * has a kind (info, warn, error) and a code number. Tests can be written to expect particular code
 * numbers rather than particular text, enabling the message text to more easily be modified and the
 * tests to run successfully in different locales.
 *
 * <p>When a message is formatted, it will have this kind of form, capturing the prefix and the
 * error kind:
 *
...
public enum SpelMessage {
  ...
  METHOD_NOT_FOUND(Kind.ERROR, 1004, "Method call: Method {0} cannot be found on type {1}"),

  TYPE_NOT_FOUND(Kind.ERROR, 1005, "Type cannot be found ''{0}''"),

  FUNCTION_NOT_DEFINED(Kind.ERROR, 1006, "Function ''{0}'' could not be found"),
