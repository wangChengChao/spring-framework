...
/**
 * A {@code SpelExpression} represents a parsed (valid) expression that is ready to be evaluated in
 * a specified context. An expression can be evaluated standalone or in a specified context. During
 * expression evaluation the context may be asked to resolve references to types, beans, properties,
 * and methods.
 *
 * @author Andy Clement
...
public class SpelExpression implements Expression {
  ...
  @Nullable private EvaluationContext evaluationContext;

  // Holds the compiled form of the expression (if it has been compiled)
  @Nullable private CompiledExpression compiledAst;
  ...
  public SpelExpression(
      String expression, SpelNodeImpl ast, SpelParserConfiguration configuration) {
  ...
  /**
   * Return the default evaluation context that will be used if none is supplied on an evaluation
   * call.
   *
  ...
  public Object getValue() throws EvaluationException {
    ...
    ExpressionState expressionState =
        new ExpressionState(getEvaluationContext(), this.configuration);
  ...
  @Nullable
  public <T> T getValue(
      EvaluationContext context, @Nullable Object rootObject, @Nullable Class<T> expectedResultType)
      throws EvaluationException {
  ...
  @Override
  @Nullable
  public TypeDescriptor getValueTypeDescriptor(@Nullable Object rootObject)
      throws EvaluationException {
    ExpressionState expressionState =
        new ExpressionState(getEvaluationContext(), toTypedValue(rootObject), this.configuration);
  ...
  public TypeDescriptor getValueTypeDescriptor(
      EvaluationContext context, @Nullable Object rootObject) throws EvaluationException {
  ...
  public void setValue(
    ...
    Assert.notNull(context, "EvaluationContext is required");
    this.ast.setValue(
        new ExpressionState(context, toTypedValue(rootObject), this.configuration), value);
  }

  /**
   * Compile the expression if it has been evaluated more than the threshold number of times to
   * trigger compilation.
   *
    ...
    if (this.compiledAst == null) {
      synchronized (this.expression) {
        ...
        SpelCompiler compiler =
            SpelCompiler.getCompiler(this.configuration.getCompilerClassLoader());
  ...
  /**
   * Cause an expression to revert to being interpreted if it has been using a compiled form. It
   * also resets the compilation attempt failure count (an expression is normally no longer
   * considered compilable if it cannot be compiled after 100 attempts).
   */
  ...
  /**
   * Produce a string representation of the Abstract Syntax Tree for the expression. This should
   * ideally look like the input expression, but properly formatted since any unnecessary whitespace
   * will have been discarded during the parse of the expression.
   *
