...
/**
 * A basic implementation of {@link EvaluationContext} that focuses on a subset of essential SpEL
 * features and customization options, targeting simple condition evaluation and in particular data
 * binding scenarios.
 *
 * <p>In many cases, the full extent of the SpEL language is not required and should be meaningfully
 * restricted. Examples include but are not limited to data binding expressions, property-based
 * filters, and others. To that effect, {@code SimpleEvaluationContext} is tailored to support only
 * a subset of the SpEL language syntax, e.g. excluding references to Java types, constructors, and
 * bean references.
 *
 * <p>When creating a {@code SimpleEvaluationContext} you need to choose the level of support that
 * you need for property access in SpEL expressions:
 *
 * <ul>
 *   <li>A custom {@code PropertyAccessor} (typically not reflection-based), potentially combined
 *       with a {@link DataBindingPropertyAccessor}
 *   <li>Data binding properties for read-only access
 *   <li>Data binding properties for read and write
 * </ul>
 *
 * <p>Conveniently, {@link SimpleEvaluationContext#forReadOnlyDataBinding()} enables read access to
 * properties via {@link DataBindingPropertyAccessor}; same for {@link
 * SimpleEvaluationContext#forReadWriteDataBinding()} when write access is needed as well.
 * Alternatively, configure custom accessors via {@link
 * SimpleEvaluationContext#forPropertyAccessors}, and potentially activate method resolution and/or
 * a type converter through the builder.
 *
 * <p>Note that {@code SimpleEvaluationContext} is typically not configured with a default root
 * object. Instead it is meant to be created once and used repeatedly through {@code getValue} calls
 * on a pre-compiled {@link org.springframework.expression.Expression} with both an {@code
 * EvaluationContext} and a root object as arguments: {@link
 * org.springframework.expression.Expression#getValue(EvaluationContext, Object)}.
 *
 * <p>For more power and flexibility, in particular for internal configuration scenarios, consider
 * using {@link StandardEvaluationContext} instead.
 *
...
public final class SimpleEvaluationContext implements EvaluationContext {
  ...
  private SimpleEvaluationContext(
      List<PropertyAccessor> accessors,
      List<MethodResolver> resolvers,
      @Nullable TypeConverter converter,
      @Nullable TypedValue rootObject) {
   ...
   *
   * @return {@code TypeLocator} implementation that raises a {@link SpelEvaluationException} with
   *     {@link SpelMessage#TYPE_NOT_FOUND}.
   */
  @Override
  ...
  /**
   * Create a {@code SimpleEvaluationContext} for the specified {@link PropertyAccessor} delegates:
   * typically a custom {@code PropertyAccessor} specific to a use case (e.g. attribute resolution
   * in a custom data structure), potentially combined with a {@link DataBindingPropertyAccessor} if
   * property dereferences are needed as well.
   *
    ...
    for (PropertyAccessor accessor : accessors) {
      if (accessor.getClass() == ReflectivePropertyAccessor.class) {
        throw new IllegalArgumentException(
            "SimpleEvaluationContext is not designed for use with a plain "
  ...
  /**
   * Create a {@code SimpleEvaluationContext} for read-only access to public properties via {@link
  ...
  /**
   * Create a {@code SimpleEvaluationContext} for read-write access to public properties via {@link
   * DataBindingPropertyAccessor}.
   *
   * @see DataBindingPropertyAccessor#forReadWriteAccess()
   * @see #forPropertyAccessors
  ...
  public static class Builder {
    ...
    /**
     * Register the specified {@link MethodResolver} delegates for a combination of property access
    ...
    /**
     * Register a {@link DataBindingMethodResolver} for instance method invocation purposes (i.e.
     * not supporting static methods) in addition to the specified property accessors, typically in
     * combination with a {@link DataBindingPropertyAccessor}.
     *
    ...
    public Builder withInstanceMethods() {
      this.resolvers =
          Collections.singletonList(DataBindingMethodResolver.forInstanceMethodInvocation());
      return this;
    }
     ...
     *
     * <p>By default a {@link StandardTypeConverter} backed by a {@link
     * org.springframework.core.convert.support.DefaultConversionService} is used.
     *
    ...
    public SimpleEvaluationContext build() {
      return new SimpleEvaluationContext(
          this.accessors, this.resolvers, this.typeConverter, this.rootObject);
    }
  }
}
