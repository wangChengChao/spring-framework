...
/**
 * A SpelCompiler will take a regular parsed expression and create (and load) a class containing
 * byte code that does the same thing as that expression. The compiled form of an expression will
 * evaluate far faster than the interpreted form.
 *
 * <p>The SpelCompiler is not currently handling all expression types but covers many of the common
 * cases. The framework is extensible to cover more cases in the future. For absolute maximum speed
 * there is *no checking* in the compiled code. The compiled version of the expression uses
 * information learned during interpreted runs of the expression when it generates the byte code.
 * For example if it knows that a particular property dereference always seems to return a Map then
 * it will generate byte code that expects the result of the property dereference to be a Map. This
 * ensures maximal performance but should the dereference result in something other than a map, the
 * compiled expression will fail - like a ClassCastException would occur if passing data of an
 * unexpected type in a regular Java program.
 *
 * <p>Due to the lack of checking there are likely some expressions that should never be compiled,
 * for example if an expression is continuously dealing with different types of data. Due to these
 * cases the compiler is something that must be selectively turned on for an associated
 * SpelExpressionParser (through the {@link SpelParserConfiguration} object), it is not on by
 * default.
 *
...
public final class SpelCompiler implements Opcodes {
   ...
   * @return an instance of the class implementing the compiled expression, or {@code null} if
  ...
  /**
   * Generate the class that encapsulates the compiled expression and define it. The generated class
   * will be a subtype of CompiledExpression.
   *
   * @param expressionToCompile the expression to be compiled
   * @return the expression call, or {@code null} if the decision was to opt out of compilation
   *     during code generation
   */
  @Nullable
  private Class<? extends CompiledExpression> createExpressionClass(
      SpelNodeImpl expressionToCompile) {
    // Create class outline 'spel/ExNNN extends
    ...
    mv =
        cw.visitMethod(
            ACC_PUBLIC,
            "getValue",
            "(Ljava/lang/Object;Lorg/springframework/expression/EvaluationContext;)Ljava/lang/Object;",
            null,
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            expressionToCompile.getClass().getSimpleName()
                + ".generateCode opted out of compilation: "
                + ex.getMessage());
      }
      return null;
    }
  ...
  /**
   * Load a compiled expression class. Makes sure the classloaders aren't used too much because they
   * anchor compiled classes in memory and prevent GC. If you have expressions continually
   * recompiling over time then by replacing the classloader periodically at least some of the older
   * variants can be garbage collected.
   *
  ...
  /**
   * Factory method for compiler instances. The returned SpelCompiler will attach a class loader as
   * the child of the given class loader and this child will be used to load compiled expressions.
   *
   ...
   * for compilation. Used for testing.
   *
