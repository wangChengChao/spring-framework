...
/**
 * Increment operator. Can be used in a prefix or postfix form. This will throw appropriate
 * exceptions if the operand in question does not support increment.
 *
 * @author Andy Clement
  ...
  public TypedValue getValueInternal(ExpressionState state) throws EvaluationException {
    ...
    if (value instanceof Number) {
      ...
      if (op1 instanceof BigDecimal) {
        newValue =
            new TypedValue(((BigDecimal) op1).add(BigDecimal.ONE), typedValue.getTypeDescriptor());
        ...
        newValue =
            new TypedValue(((BigInteger) op1).add(BigInteger.ONE), typedValue.getTypeDescriptor());
    ...
    try {
      ...
      if (see.getMessageCode() == SpelMessage.SETVALUE_NOT_SUPPORTED) {
        throw new SpelEvaluationException(
            operand.getStartPosition(), SpelMessage.OPERAND_NOT_INCREMENTABLE);
      } else {
        throw see;
      }
    }
