...
public abstract class SpelNodeImpl implements SpelNode, Opcodes {
  ...
  /**
   * Indicates the type descriptor for the result of this expression node. This is set as soon as it
   * is known. For a literal node it is known immediately. For a property access or method
   * invocation it is known after one evaluation of that node.
   *
   * <p>The descriptor is like the bytecode form but is slightly easier to work with. It does not
   * include the trailing semicolon (for non array reference types). Some examples:
  ...
  @Override
  public void setValue(ExpressionState expressionState, @Nullable Object newValue)
      throws EvaluationException {
    throw new SpelEvaluationException(
        getStartPosition(), SpelMessage.SETVALUE_NOT_SUPPORTED, getClass());
  }
  ...
  /**
   * Check whether a node can be compiled to bytecode. The reasoning in each node may be different
   * but will typically involve checking whether the exit type descriptor of the node is known and
   * any relevant child nodes are compilable.
   *
  ...
  /**
   * Generate the bytecode for this node into the supplied visitor. Context info about the current
   * expression being compiled is available in the codeflow object, e.g. including information about
   * the type of the object currently on the stack.
   *
  ...
  @Nullable
  protected final <T> T getValue(ExpressionState state, Class<T> desiredReturnType)
      throws EvaluationException {
    return ExpressionUtils.convertTypedValue(
        state.getEvaluationContext(), getValueInternal(state), desiredReturnType);
  }

  protected ValueRef getValueRef(ExpressionState state) throws EvaluationException {
    throw new SpelEvaluationException(
        getStartPosition(), SpelMessage.NOT_ASSIGNABLE, toStringAST());
  }
  ...
  /**
   * Generate code that handles building the argument values for the specified method. This method
   * will take account of whether the invoked method is a varargs method and if it is then the
   * argument values will be appropriately packaged into an array.
   *
   ...
   * boxing/unboxing/checkcasting to ensure it matches the expected parameter descriptor.
   */
  protected static void generateCodeForArgument(
      MethodVisitor mv, CodeFlow cf, SpelNodeImpl argument, String paramDesc) {
    ...
    if (primitiveOnStack && paramDesc.charAt(0) == 'L') {
      ...
      // This would be unnecessary in the case of subtyping (e.g. method takes Number but Integer
      // passed in)
