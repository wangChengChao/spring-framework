...
/**
 * An Indexer can index into some proceeding structure to access a particular piece of it. Supported
 * structures are: strings / collections (lists/sets) / arrays.
 *
 * @author Andy Clement
 * @author Phillip Webb
...
public class Indexer extends SpelNodeImpl {
  ...
  @Nullable private String cachedReadName;

  @Nullable private Class<?> cachedReadTargetType;

  @Nullable private PropertyAccessor cachedReadAccessor;
  ...
  @Nullable private String cachedWriteName;

  @Nullable private Class<?> cachedWriteTargetType;

  @Nullable private PropertyAccessor cachedWriteAccessor;

  @Nullable private IndexedType indexedType;
  ...
  protected ValueRef getValueRef(ExpressionState state) throws EvaluationException {
    ...
    if (target == null) {
      throw new SpelEvaluationException(
          getStartPosition(), SpelMessage.CANNOT_INDEX_INTO_NULL_VALUE);
    }
    ...
    if (target instanceof Map) {
      ...
      return new MapIndexingValueRef(
          state.getTypeConverter(), (Map<?, ?>) target, key, targetDescriptor);
    }
  ...
  public boolean isCompilable() {
    if (this.indexedType == IndexedType.ARRAY) {
      ...
      return (this.children[0] instanceof PropertyOrFieldReference
          || this.children[0].isCompilable());
    } else if (this.indexedType == IndexedType.OBJECT) {
      // If the string name is changing the accessor is clearly going to change (so no compilation
      // possible)
    ...
    if (this.indexedType == IndexedType.ARRAY) {
      ...
      if (member instanceof Method) {
        mv.visitMethodInsn(
            ...
            classDesc,
            member.getName(),
            CodeFlow.createSignatureDescriptor((Method) member),
            false);
      } else {
        mv.visitFieldInsn(
  ...
  private void checkAccess(int arrayLength, int index) throws SpelEvaluationException {
    if (index > arrayLength) {
      throw new SpelEvaluationException(
          getStartPosition(), SpelMessage.ARRAY_INDEX_OUT_OF_BOUNDS, arrayLength, index);
    }
  }
  ...
  private class ArrayIndexingValueRef implements ValueRef {
    ...
    ArrayIndexingValueRef(
        TypeConverter typeConverter, Object array, int index, TypeDescriptor typeDescriptor) {
  ...
  private class MapIndexingValueRef implements ValueRef {
    ...
    public void setValue(@Nullable Object newValue) {
      if (this.mapEntryDescriptor.getMapValueTypeDescriptor() != null) {
        newValue =
            this.typeConverter.convertValue(
                newValue,
  ...
  private class PropertyIndexingValueRef implements ValueRef {
    ...
    public PropertyIndexingValueRef(
        Object targetObject,
        String value,
    ...
    public TypedValue getValue() {
      ...
      try {
        if (Indexer.this.cachedReadName != null
            && Indexer.this.cachedReadName.equals(this.name)
        ...
        for (PropertyAccessor accessor : accessorsToTry) {
          if (accessor.canRead(this.evaluationContext, this.targetObject, this.name)) {
            if (accessor instanceof ReflectivePropertyAccessor) {
              accessor =
                  ((ReflectivePropertyAccessor) accessor)
                      .createOptimalAccessor(this.evaluationContext, this.targetObject, this.name);
            }
            ...
            if (accessor instanceof ReflectivePropertyAccessor.OptimalPropertyAccessor) {
              ...
              Indexer.this.exitTypeDescriptor =
    ...
    public void setValue(@Nullable Object newValue) {
      ...
      try {
        if (Indexer.this.cachedWriteName != null
            && Indexer.this.cachedWriteName.equals(this.name)
  ...
  private class CollectionIndexingValueRef implements ValueRef {
    ...
    public CollectionIndexingValueRef(
    ...
    public TypedValue getValue() {
      ...
      throw new IllegalStateException(
          "Failed to find indexed element " + this.index + ": " + this.collection);
    }
    ...
    private void growCollectionIfNecessary() {
      if (this.index >= this.collection.size()) {
        ...
        if (this.index >= this.maximumSize) {
          throw new SpelEvaluationException(
              getStartPosition(), SpelMessage.UNABLE_TO_GROW_COLLECTION);
        }
        if (this.collectionEntryDescriptor.getElementTypeDescriptor() == null) {
          throw new SpelEvaluationException(
        ...
        try {
          ...
          throw new SpelEvaluationException(
              getStartPosition(), ex, SpelMessage.UNABLE_TO_GROW_COLLECTION);
        }
      }
    }
