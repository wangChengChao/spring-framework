...
public class ReflectivePropertyAccessor implements PropertyAccessor {
  ...
  public boolean canRead(EvaluationContext context, @Nullable Object target, String name)
    ...
    if (method != null) {
      ...
      // The readerCache will only contain gettable properties (let's not worry about setters for
      // now).
  ...
  public TypedValue read(EvaluationContext context, @Nullable Object target, String name)
    ...
    if (invoker == null || invoker.member instanceof Method) {
      ...
      if (method == null) {
        ...
        if (method != null) {
          // Treat it like a property...
          // The readerCache will only contain gettable properties (let's not worry about setters
          // for now).
          Property property = new Property(type, method, null);
          TypeDescriptor typeDescriptor = new TypeDescriptor(property);
          method = ClassUtils.getInterfaceMethodIfPossible(method);
  ...
  protected Method findGetterForProperty(
    ...
    Method method =
        findMethodForProperty(
            getPropertyMethodSuffixes(propertyName), "get", clazz, mustBeStatic, 0, ANY_TYPES);
    if (method == null) {
      method =
          findMethodForProperty(
              getPropertyMethodSuffixes(propertyName), "is", clazz, mustBeStatic, 0, BOOLEAN_TYPES);
    }
    return method;
  }
  ...
  @Nullable
  protected Method findSetterForProperty(
      String propertyName, Class<?> clazz, boolean mustBeStatic) {
  ...
  private Method[] getSortedMethods(Class<?> clazz) {
    return this.sortedMethodsCache.computeIfAbsent(
        clazz,
        key -> {
          Method[] methods = key.getMethods();
          Arrays.sort(
              methods, (o1, o2) -> (o1.isBridge() == o2.isBridge() ? 0 : (o1.isBridge() ? 1 : -1)));
          return methods;
        });
  }

  /**
   * Determine whether the given {@code Method} is a candidate for property access on an instance of
   * the given target class.
   *
   * <p>The default implementation considers any method as a candidate, even for non-user-declared
   * properties on the {@link Object} base class.
   *
  ...
  /**
   * Attempt to create an optimized property accessor tailored for a property of a particular name
   * on a particular class. The general ReflectivePropertyAccessor will always work but is not
   * optimal due to the need to lookup which reflective member (method/field) to use each time
   * read() is called. This method will just return the ReflectivePropertyAccessor instance if it is
   * unable to build a more optimal accessor.
   *
   * <p>Note: An optimal accessor is currently only usable for read attempts. Do not call this
   * method if you need a read-write accessor.
   *
   * @see OptimalPropertyAccessor
   */
  public PropertyAccessor createOptimalAccessor(
      EvaluationContext context, @Nullable Object target, String name) {
  ...
  /**
   * Captures the member (method/field) to call reflectively to access a property value and the type
   * descriptor for the value returned by the reflective call.
   */
  ...
  private static final class PropertyCacheKey implements Comparable<PropertyCacheKey> {
    ...
    public boolean equals(@Nullable Object other) {
      ...
      return (this.clazz == otherKey.clazz
  ...
  /**
   * An optimized form of a PropertyAccessor that will use reflection but only knows how to access a
   * particular property on a particular class. This is unlike the general
   * ReflectivePropertyResolver which manages a cache of methods/fields that may be invoked to
   * access different properties on different classes. This optimal accessor exists because looking
