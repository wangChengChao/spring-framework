...
public class PropertyOrFieldReference extends SpelNodeImpl {
  ...
  @Nullable private String originalPrimitiveExitTypeDescriptor;
  ...
  public PropertyOrFieldReference(
      boolean nullSafe, String propertyOrFieldName, int startPos, int endPos) {
  ...
  public ValueRef getValueRef(ExpressionState state) throws EvaluationException {
    return new AccessorLValue(
        this,
        state.getActiveContextObject(),
        state.getEvaluationContext(),
  ...
  private TypedValue getValueInternal(
      TypedValue contextObject, EvaluationContext evalContext, boolean isAutoGrowNullReferences)
      ...
      if (List.class == resultDescriptor.getType()) {
        ...
        try {
          ...
          throw new SpelEvaluationException(
              ...
              result.getTypeDescriptor().getType());
        } catch (Throwable ex) {
          throw new SpelEvaluationException(
              getStartPosition(),
              ex,
  ...
  @Override
  public void setValue(ExpressionState state, @Nullable Object newValue)
      throws EvaluationException {
    writeProperty(
        state.getActiveContextObject(), state.getEvaluationContext(), this.name, newValue);
  }

  @Override
  public boolean isWritable(ExpressionState state) throws EvaluationException {
    return isWritableProperty(
        this.name, state.getActiveContextObject(), state.getEvaluationContext());
  }
   ...
   */
  private TypedValue readProperty(
      TypedValue contextObject, EvaluationContext evalContext, String name)
      throws EvaluationException {
    ...
    // Go through the accessors that may be able to resolve it. If they are a cacheable accessor
    // then
    ...
    try {
      for (PropertyAccessor accessor : accessorsToTry) {
        if (accessor.canRead(evalContext, contextObject.getValue(), name)) {
          if (accessor instanceof ReflectivePropertyAccessor) {
            accessor =
                ((ReflectivePropertyAccessor) accessor)
                    .createOptimalAccessor(evalContext, contextObject.getValue(), name);
          }
      ...
      throw new SpelEvaluationException(
          ex, SpelMessage.EXCEPTION_DURING_PROPERTY_READ, name, ex.getMessage());
    }
  ...
  /**
   * Determines the set of property resolvers that should be used to try and access a property on
   * the specified target type. The resolvers are considered to be in an ordered list, however in
   * the returned list any that are exact matches for the input target type (as opposed to 'general'
   * resolvers that could work for any type) are placed at the start of the list. In addition, there
   * are specific resolvers that exactly name the class in question and resolvers that name a
   * specific class but it is a supertype of the class we have. These are put at the end of the
   * specific resolvers set and will be tried after exactly matching accessors but before generic
   * accessors.
   *
  ...
  private static class AccessorLValue implements ValueRef {
    ...
    public TypedValue getValue() {
      TypedValue value =
          this.ref.getValueInternal(
              this.contextObject, this.evalContext, this.autoGrowNullReferences);
      PropertyAccessor accessorToUse = this.ref.cachedReadAccessor;
      if (accessorToUse instanceof CompilablePropertyAccessor) {
        this.ref.setExitTypeDescriptor(
