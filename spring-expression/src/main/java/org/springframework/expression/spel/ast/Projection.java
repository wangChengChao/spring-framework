...
/**
 * Represents projection, where a given operation is performed on all elements in some input
 * sequence, returning a new sequence of the same size. For example:
  ...
  protected ValueRef getValueRef(ExpressionState state) throws EvaluationException {
    ...
    if (operand instanceof Map) {
      ...
      return new ValueRef.TypedValueHolderValueRef(
          new TypedValue(result), this); // TODO unable to build correct type descriptor
    }
    ...
    if (operand == null) {
      ...
      throw new SpelEvaluationException(
          getStartPosition(), SpelMessage.PROJECTION_NOT_SUPPORTED_ON_TYPE, "null");
    }
