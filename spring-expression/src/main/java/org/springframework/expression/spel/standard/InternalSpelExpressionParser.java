...
class InternalSpelExpressionParser extends TemplateAwareExpressionParser {
  ...
  private SpelNodeImpl eatDottedNode() {
    ...
    if (maybeEatMethodOrProperty(nullSafeNavigation)
    ...
    if (peekToken() == null) {
      ...
      throw internalException(
          t.startPos, SpelMessage.UNEXPECTED_DATA_AFTER_DOT, toString(peekToken()));
    }
  }
    ...
    if (args == null) {
      push(
          new VariableReference(
              functionOrVariableName.stringValue(), t.startPos, functionOrVariableName.endPos));
      return true;
    }

    push(
        new FunctionReference(
            functionOrVariableName.stringValue(), t.startPos, functionOrVariableName.endPos, args));
    return true;
  }
  ...
  private boolean maybeEatBeanReference() {
    if (peekToken(TokenKind.BEAN_REF) || peekToken(TokenKind.FACTORY_BEAN_REF)) {
      ...
      if (beanRefToken.getKind() == TokenKind.FACTORY_BEAN_REF) {
        ...
        beanReference =
            new BeanReference(beanRefToken.startPos, beanNameToken.endPos, beanNameString);
      } else {
  ...
  private boolean maybeEatInlineListOrMap() {
    ...
    if (peekToken(TokenKind.RCURLY, true)) {
      ...
      if (peekToken(TokenKind.RCURLY)) { // list with one item in it
        ...
        closingCurly = eatToken(TokenKind.RCURLY);
        expr =
            new InlineList(t.startPos, closingCurly.endPos, elements.toArray(new SpelNodeImpl[0]));
        ...
        closingCurly = eatToken(TokenKind.RCURLY);
        expr =
            new InlineMap(t.startPos, closingCurly.endPos, elements.toArray(new SpelNodeImpl[0]));
      } else {
        throw internalException(t.startPos, SpelMessage.OOD);
      }
    }
  ...
  private boolean maybeEatSelection(boolean nullSafeNavigation) {
    ...
    if (t.kind == TokenKind.SELECT_FIRST) {
      this.constructedNodes.push(
          new Selection(nullSafeNavigation, Selection.FIRST, t.startPos, t.endPos, expr));
    } else if (t.kind == TokenKind.SELECT_LAST) {
      this.constructedNodes.push(
          new Selection(nullSafeNavigation, Selection.LAST, t.startPos, t.endPos, expr));
    } else {
      this.constructedNodes.push(
          new Selection(nullSafeNavigation, Selection.ALL, t.startPos, t.endPos, expr));
    }
    return true;
  }
  ...
  private SpelNodeImpl eatPossiblyQualifiedId() {
    ...
    return new QualifiedIdentifier(
        ...
        qualifiedIdPieces.toArray(new SpelNodeImpl[0]));
  }
      ...
      if (args == null) {
        ...
        push(
            new PropertyOrFieldReference(
                nullSafeNavigation,
                methodOrPropertyName.stringValue(),
                methodOrPropertyName.startPos,
  ...
  // constructor
  // :	('new' qualifiedId LPAREN) => 'new' qualifiedId ctorArgs -> ^(CONSTRUCTOR qualifiedId
  // ctorArgs)
  private boolean maybeEatConstructorReference() {
    if (peekIdentifierToken("new")) {
      ...
      if (peekToken(TokenKind.RSQUARE)) {
        ...
        push(
            new PropertyOrFieldReference(
                false, newToken.stringValue(), newToken.startPos, newToken.endPos));
        return true;
      }
      ...
      if (peekToken(TokenKind.LSQUARE)) {
        ...
        push(
            new ConstructorReference(
                newToken.startPos,
                newToken.endPos,
                dimensions.toArray(new SpelNodeImpl[0]),
                nodes.toArray(new SpelNodeImpl[0])));
      } else {
        ...
        push(
            new ConstructorReference(
                newToken.startPos, newToken.endPos, nodes.toArray(new SpelNodeImpl[0])));
      }
      return true;
    }
    return false;
  ...
  private boolean peekToken(TokenKind desiredTokenKind, boolean consumeIfMatched) {
    ...
    if (desiredTokenKind == TokenKind.IDENTIFIER) {
      ...
      if (t.kind.ordinal() >= TokenKind.DIV.ordinal()
  ...
  private boolean peekSelectToken() {
    ...
    return (t.kind == TokenKind.SELECT
        || t.kind == TokenKind.SELECT_FIRST
  ...
  private void checkOperands(
      Token token, @Nullable SpelNodeImpl left, @Nullable SpelNodeImpl right) {
  ...
  private InternalParseException internalException(
      int startPos, SpelMessage message, Object... inserts) {
    return new InternalParseException(
        new SpelParseException(this.expressionString, startPos, message, inserts));
  }
}
