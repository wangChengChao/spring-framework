...
public abstract class ReflectionHelper {
  ...
  /**
   * Compare argument arrays and return information about whether they match. A supplied type
   * converter and conversionAllowed flag allow for matches to take into account that a type may be
   * transformed into a different type by the converter.
   *
   * @param expectedArgTypes the types the method/constructor is expecting
   * @param suppliedArgTypes the types that are being supplied at the point of invocation
   * @param typeConverter a registered type converter
   * @return a MatchInfo object indicating what kind of match it was, or {@code null} if it was not
   *     a match
   */
  @Nullable
  static ArgumentsMatchInfo compareArguments(
      List<TypeDescriptor> expectedArgTypes,
      List<TypeDescriptor> suppliedArgTypes,
      TypeConverter typeConverter) {

    Assert.isTrue(
        expectedArgTypes.size() == suppliedArgTypes.size(),
  ...
  /**
   * Based on {@link MethodInvoker#getTypeDifferenceWeight(Class[], Object[])} but operates on
   * TypeDescriptors.
   */
  public static int getTypeDifferenceWeight(
      List<TypeDescriptor> paramTypes, List<TypeDescriptor> argTypes) {
    int result = 0;
   ...
   * Compare argument arrays and return information about whether they match. A supplied type
   * converter and conversionAllowed flag allow for matches to take into account that a type may be
   * transformed into a different type by the converter. This variant of compareArguments also
   * allows for a varargs match.
   *
   * @param expectedArgTypes the types the method/constructor is expecting
   * @param suppliedArgTypes the types that are being supplied at the point of invocation
   * @param typeConverter a registered type converter
  ...
  static ArgumentsMatchInfo compareArgumentsVarargs(
    ...
    Assert.isTrue(
        !CollectionUtils.isEmpty(expectedArgTypes),
        "Expected arguments must at least include one array (the varargs parameter)");
    Assert.isTrue(
        expectedArgTypes.get(expectedArgTypes.size() - 1).isArray(),
    ...
    // Deal with the arguments up to 'expected number' - 1 (that is everything but the varargs
    // argument)
    ...
    if (suppliedArgTypes.size() == expectedArgTypes.size()
      ...
      // expected argument - that is a match, the caller has already built the array. Proceed with
      // it.
    } else {
      ...
      for (int i = expectedArgTypes.size() - 1; i < suppliedArgTypes.size(); i++) {
        ...
        if (suppliedArg == null) {
          ...
          if (varargsParamType != suppliedArg.getType()) {
            ...
            } else if (typeConverter.canConvert(
                suppliedArg, TypeDescriptor.valueOf(varargsParamType))) {
              match = ArgumentsMatchKind.REQUIRES_CONVERSION;
            } else {
              match = null;
            }
   ...
   * to a varargs method then the final entry in the parameterTypes array is going to be an array
   * itself whose component type should be used as the conversion target for extraneous arguments.
   * (For example, if the parameterTypes are {Integer, String[]} and the input arguments are
   * {Integer, boolean, float} then both the boolean and float must be converted to strings). This
   * method does *not* repackage the arguments into a form suitable for the varargs invocation - a
   * subsequent call to setupArgumentsForVarargsInvocation handles that.
   *
  ...
  public static boolean convertAllArguments(
      TypeConverter converter, Object[] arguments, Method method) throws SpelEvaluationException {
  ...
  /**
   * Takes an input set of argument values and converts them to the types specified as the required
   * parameter types. The arguments are converted 'in-place' in the input array.
   *
   * @param converter the type converter to use for attempting conversions
   * @param arguments the actual arguments that need conversion
   * @param executable the target Method or Constructor
   * @param varargsPosition the known position of the varargs argument, if any ({@code null} if not
   *     varargs)
   * @return {@code true} if some kind of conversion occurred on an argument
   * @throws EvaluationException if a problem occurs during conversion
   */
  static boolean convertArguments(
      TypeConverter converter,
      Object[] arguments,
      Executable executable,
      @Nullable Integer varargsPosition)
    ...
    if (varargsPosition == null) {
      for (int i = 0; i < arguments.length; i++) {
        TypeDescriptor targetType =
            new TypeDescriptor(MethodParameter.forExecutable(executable, i));
        Object argument = arguments[i];
        arguments[i] =
            converter.convertValue(argument, TypeDescriptor.forObject(argument), targetType);
      ...
      if (varargsPosition == arguments.length - 1) {
        ...
        // 1) the input argument was already compatible (ie. array of valid type) and nothing was
        // done
  ...
  /**
   * Check if the supplied value is the first entry in the array represented by the possibleArray
   * value.
   *
  ...
  private static boolean isFirstEntryInArray(Object value, @Nullable Object possibleArray) {
    ...
    if (!type.isArray()
        || Array.getLength(possibleArray) == 0
  ...
  /**
   * Package up the arguments so that they correctly match what is expected in parameterTypes. For
   * example, if parameterTypes is {@code (int, String[])} because the second parameter was declared
   * {@code String...}, then if arguments is {@code [1,"a","b"]} then it must be repackaged as
   ...
   */
  public static Object[] setupArgumentsForVarargsInvocation(
      Class<?>[] requiredParameterTypes, Object... args) {
  ...
  enum ArgumentsMatchKind {
    ...
    /**
     * An exact match is where the parameter types exactly match what the method/constructor is
     * expecting.
     */
    EXACT,
    ...
    /**
     * A conversion match is where the type converter must be used to transform some of the
     * parameter types.
     */
    REQUIRES_CONVERSION
  }
  ...
  /**
   * An instance of ArgumentsMatchInfo describes what kind of match was achieved between two sets of
   * arguments - the set that a method/constructor is expecting and the set that are being supplied
   * at the point of invocation. If the kind indicates that conversion is required for some of the
   * arguments then the arguments that require conversion are listed in the argsRequiringConversion
   * array.
   */
