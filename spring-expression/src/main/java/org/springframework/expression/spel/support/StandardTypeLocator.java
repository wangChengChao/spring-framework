...
/**
 * A simple implementation of {@link TypeLocator} that uses the context ClassLoader (or any
 * ClassLoader set upon it). It supports 'well-known' packages: So if a type cannot be found, it
 * will try the registered imports to locate it.
 *
 * @author Andy Clement
...
public class StandardTypeLocator implements TypeLocator {
  ...
  /**
   * Create a StandardTypeLocator for the default ClassLoader (typically, the thread context
   * ClassLoader).
   */
  ...
  /**
   * Register a new import prefix that will be used when searching for unqualified types. Expected
   * format is something like "java.lang".
   *
  ...
  /**
   * Find a (possibly unqualified) type reference - first using the type name as-is, then trying any
   * registered prefixes if the type name cannot be found.
   *
