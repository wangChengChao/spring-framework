...
public interface SpelNode {
  ...
  /**
   * Evaluate the expression node in the context of the supplied expression state and return the
   * value.
   *
   ...
   * @throws EvaluationException if something went wrong trying to determine if the node supports
   *     writing
   */
  boolean isWritable(ExpressionState expressionState) throws EvaluationException;

  /**
   * Evaluate the expression to a node and then set the new value on that node. For example, if the
   * expression evaluates to a property reference, then the property will be set to the new value.
   *
   * @param expressionState the current expression state (includes the context)
   * @param newValue the new value
   * @throws EvaluationException if any problem occurs evaluating the expression or setting the new
   *     value
   */
  void setValue(ExpressionState expressionState, @Nullable Object newValue)
   ...
   * @return the class of the object if it is not already a class object, or {@code null} if the
   *     object is {@code null}
   */
  @Nullable
