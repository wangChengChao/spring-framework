...
/**
 * An ExpressionState is for maintaining per-expression-evaluation state, any changes to it are not
 * seen by other expressions but it gives a place to hold local variables and for component
 * expressions in a compound expression to communicate state. This is in contrast to the
 * EvaluationContext, which is shared amongst expression evaluations, and any changes to it will be
 * seen by other expressions or any code that chooses to ask questions of the context.
 *
 * <p>It also acts as a place for to define common utility routines that the various AST nodes might
 * need.
 *
 * @author Andy Clement
...
public class ExpressionState {
  ...
  @Nullable private Deque<TypedValue> contextObjects;

  @Nullable private Deque<VariableScope> variableScopes;
  ...
  // element from list1
  @Nullable private ArrayDeque<TypedValue> scopeRootObjects;
  ...
  /**
   * The active context object is what unqualified references to properties/etc are resolved
   * against.
   */
  ...
  public Object convertValue(Object value, TypeDescriptor targetTypeDescriptor)
    ...
    Object result =
        this.relatedContext
            .getTypeConverter()
            .convertValue(value, TypeDescriptor.forObject(value), targetTypeDescriptor);
    if (result == null) {
  ...
  public TypedValue operate(Operation op, @Nullable Object left, @Nullable Object right)
      throws EvaluationException {
  ...
  /**
   * A new scope is entered when a function is called and it is used to hold the parameters to the
   * function call. If the names of the parameters clash with those in a higher level scope, those
   * in the higher level scope will not be accessible whilst the function is executing. When the
   * function returns, the scope is exited.
   */
