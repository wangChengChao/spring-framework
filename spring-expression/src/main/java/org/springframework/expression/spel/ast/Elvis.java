...
/**
 * Represents the elvis operator ?:. For an expression "a?:b" if a is not null, the value of the
 * expression is "a", if a is null then the value of the expression is "b".
 *
 * @author Andy Clement
...
public class Elvis extends SpelNodeImpl {
   ...
   * @throws EvaluationException if the condition does not evaluate correctly to a boolean or there
   *     is a problem executing the chosen alternative
   */
  @Override
  ...
  public boolean isCompilable() {
    ...
    return (condition.isCompilable()
        && ifNullValue.isCompilable()
        && condition.exitTypeDescriptor != null
        && ifNullValue.exitTypeDescriptor != null);
  }
  ...
  private void computeExitTypeDescriptor() {
    if (this.exitTypeDescriptor == null
        && this.children[0].exitTypeDescriptor != null
