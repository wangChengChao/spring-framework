 ...
 * </ul>
 *
 * <p>It can be used as a unary operator for numbers. The standard promotions are performed when the
 * operand types vary (double+int=double). For other options it defers to the registered overloader.
 *
 * @author Andy Clement
...
public class OpPlus extends Operator {
  ...
  public TypedValue getValueInternal(ExpressionState state) throws EvaluationException {
    ...
    if (rightOperand instanceof String) {
      return new TypedValue(
          (leftOperand == null ? "null" : convertTypedValueToString(operandOneValue, state))
              + rightOperand);
    }
  ...
  private static String convertTypedValueToString(TypedValue value, ExpressionState state) {
    ...
    if (typeConverter.canConvert(value.getTypeDescriptor(), typeDescriptor)) {
      return String.valueOf(
          typeConverter.convertValue(value.getValue(), value.getTypeDescriptor(), typeDescriptor));
    }
  ...
  /**
   * Walk through a possible tree of nodes that combine strings and append them all to the same (on
   * stack) StringBuilder.
   */
