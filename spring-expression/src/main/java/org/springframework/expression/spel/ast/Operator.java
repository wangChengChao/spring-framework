...
/**
 * Common supertype for operators that operate on either one or two operands. In the case of
 * multiply or divide there would be two operands, but for unary plus or minus, there is only one.
 *
 * @author Andy Clement
...
public abstract class Operator extends SpelNodeImpl {
  ...
  protected boolean isCompilableOperatorUsingNumerics() {
    ...
    DescriptorComparison dc =
        DescriptorComparison.checkNumericCompatibility(
            leftDesc, rightDesc, this.leftActualDescriptor, this.rightActualDescriptor);
    return (dc.areNumbers && dc.areCompatible);
  }

  /**
   * Numeric comparison operators share very similar generated code, only differing in two
   * comparison instructions.
   */
  protected void generateComparisonCode(
      MethodVisitor mv, CodeFlow cf, int compInstruction1, int compInstruction2) {
    SpelNodeImpl left = getLeftOperand();
    SpelNodeImpl right = getRightOperand();
    String leftDesc = left.exitTypeDescriptor;
    String rightDesc = right.exitTypeDescriptor;
   ...
   *
   * <p>This method is not just used for reflective comparisons in subclasses but also from compiled
   * expression code, which is why it needs to be declared as {@code public static} here.
   *
   * @param context the current evaluation context
   * @param left the left-hand operand value
   * @param right the right-hand operand value
   */
  public static boolean equalityCheck(
      EvaluationContext context, @Nullable Object left, @Nullable Object right) {
    if (left instanceof Number && right instanceof Number) {
      ...
      if (leftNumber instanceof BigDecimal || rightNumber instanceof BigDecimal) {
        ...
        BigInteger leftBigInteger =
            NumberUtils.convertNumberToTargetClass(leftNumber, BigInteger.class);
        BigInteger rightBigInteger =
            NumberUtils.convertNumberToTargetClass(rightNumber, BigInteger.class);
  ...
  protected static final class DescriptorComparison {
    ...
    static final DescriptorComparison INCOMPATIBLE_NUMBERS =
        new DescriptorComparison(true, false, ' ');
     ...
     *
     * <p>A declared descriptor is what could statically be determined (e.g. from looking at the
     * return value of a property accessor method) whilst an actual descriptor is the type of an
     * actual object that was returned, which may differ.
     *
     * <p>For generic types with unbound type variables, the declared descriptor discovered may be
     * 'Object' but from the actual descriptor it is possible to observe that the objects are really
     * numeric values (e.g. ints).
     *
    ...
    public static DescriptorComparison checkNumericCompatibility(
        @Nullable String leftDeclaredDescriptor,
        @Nullable String rightDeclaredDescriptor,
        @Nullable String leftActualDescriptor,
