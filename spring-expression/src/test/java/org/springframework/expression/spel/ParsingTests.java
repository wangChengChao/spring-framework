...
/**
 * Parse some expressions and check we get the AST we expect. Rather than inspecting each node in
 * the AST, we ask it to write itself to a string form and check that is as expected.
 *
 * @author Andy Clement
 */
public class ParsingTests {
  ...
  public void testRelOperatorsMatches01() {
    parseCheck(
        "'5.0067' matches '^-?\\d+(\\.\\d{2})?$'", "('5.0067' matches '^-?\\d+(\\.\\d{2})?$')");
  } // false
  ...
  // parseCheck("(#max = {|x,y| $x > $y ? $x : $y }; #max(5,25))", "(#max={|x,y| ($x > $y) ? $x : $y
  // };#max(5,25))");
  // }
  //
  ...
  /**
   * Parse the supplied expression and then create a string representation of the resultant AST, it
   * should be the same as the original expression.
   *
   * @param expression the expression to parse *and* the expected value of the string form of the
   *     resultant AST
   */
