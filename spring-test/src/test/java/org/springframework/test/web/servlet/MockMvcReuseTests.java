...
/**
 * Integration tests that verify that {@link MockMvc} can be reused multiple times within the same
 * test method without side effects between independent requests.
 *
...
public class MockMvcReuseTests {
  ...
  public void sessionAttributesAreClearedBetweenInvocations() throws Exception {
    ...
    this.mvc
        .perform(get("/").sessionAttr(FOO, BAR))
        .andExpect(content().string(HELLO))
