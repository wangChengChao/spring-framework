...
/**
 * Unit tests for building a {@link MockHttpServletRequest} with {@link
...
public class MockHttpServletRequestBuilderTests {
  ...
  public void requestUriWithDoubleSlashes() throws URISyntaxException {
    this.builder =
        new MockHttpServletRequestBuilder(HttpMethod.GET, new URI("/test//currentlyValid/0"));
    MockHttpServletRequest request = this.builder.buildRequest(this.servletContext);
  ...
  public void contextPathServletPathInvalid() {
    testContextPathServletPathInvalid(
        "/Foo", "", "Request URI [/foo/bar] does not start with context path [/Foo]");
    ...
    testContextPathServletPathInvalid(
        "/foo", "/Bar", "Invalid servlet path [/Bar] for request URI [/foo/bar]");
  ...
  private void testContextPathServletPathInvalid(
      String contextPath, String servletPath, String message) {
    try {
  ...
  public void requestParameterFromRequestBodyFormData() throws Exception {
    ...
    MockHttpServletRequest request =
        new MockHttpServletRequestBuilder(HttpMethod.POST, "/foo")
            .contentType(contentType)
            .content(body.getBytes(StandardCharsets.UTF_8))
            .buildRequest(this.servletContext);

    assertThat(request.getParameterMap().get("name 1")).isEqualTo(new String[] {"value 1"});
    assertThat(request.getParameterMap().get("name 2"))
        .isEqualTo(new String[] {"value A", "value B"});
