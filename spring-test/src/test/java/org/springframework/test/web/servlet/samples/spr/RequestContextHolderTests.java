...
public class RequestContextHolderTests {
  ...
  public void requestScopedController() throws Exception {
    assertThat(AopUtils.isCglibProxy(this.requestScopedController))
        .as("request-scoped controller must be a CGLIB proxy")
        .isTrue();
    this.mockMvc.perform(
        get("/requestScopedController").requestAttr(FROM_MVC_TEST_MOCK, FROM_MVC_TEST_MOCK));
  }

  @Test
  public void requestScopedService() throws Exception {
    assertThat(AopUtils.isCglibProxy(this.requestScopedService))
        .as("request-scoped service must be a CGLIB proxy")
        .isTrue();
    this.mockMvc.perform(
  ...
  public void sessionScopedService() throws Exception {
    assertThat(AopUtils.isCglibProxy(this.sessionScopedService))
        .as("session-scoped service must be a CGLIB proxy")
        .isTrue();
    this.mockMvc.perform(
  ...
  private static void assertRequestAttributes(ServletRequest request, boolean withinMockMvc) {
    if (withinMockMvc) {
      ...
      assertThat(request.getAttribute(FROM_REQUEST_ATTRIBUTES_FILTER))
          .isEqualTo(FROM_REQUEST_ATTRIBUTES_FILTER);
    } else {
