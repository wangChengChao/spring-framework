...
/**
 * Examples to demonstrate writing client-side REST tests with Spring MVC Test. While the tests in
 * this class invoke the RestTemplate directly, in actual tests the RestTemplate may likely be
 * invoked indirectly, i.e. through client code.
 *
...
public class SampleAsyncTests {
  ...
  private final org.springframework.web.client.AsyncRestTemplate restTemplate =
  ...
  public void performGetManyTimes() throws Exception {
    ...
    this.mockServer
        .expect(manyTimes(), requestTo("/composers/42"))
        .andExpect(method(HttpMethod.GET))
        .andRespond(withSuccess(responseBody, MediaType.APPLICATION_JSON));
  ...
  public void verify() {
    ...
    @SuppressWarnings("unused")
    ListenableFuture<ResponseEntity<String>> result =
        this.restTemplate.getForEntity("/number", String.class);
    // result == "1"
    ...
    try {
      ...
      assertThat(error.getMessage().contains("2 unsatisfied expectation(s)"))
          .as(error.getMessage())
          .isTrue();
    }
  }
