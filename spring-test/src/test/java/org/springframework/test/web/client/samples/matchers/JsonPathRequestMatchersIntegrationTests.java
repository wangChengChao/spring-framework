...
/**
 * Examples of defining expectations on JSON request content with <a
  ...
  public void hamcrestMatchers() throws Exception {
    this.mockServer
        ...
        .andExpect(
            jsonPath(
                "$.composers[1].name",
                is(in(Arrays.asList("Johann Sebastian Bach", "Johannes Brahms")))))
    ...
    this.mockServer
        ...
        .andExpect(
            jsonPath(composerName, 1)
                .value(is(in(Arrays.asList("Johann Sebastian Bach", "Johannes Brahms")))))
        .andRespond(withSuccess());
