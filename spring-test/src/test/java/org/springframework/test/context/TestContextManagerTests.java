...
/**
 * JUnit 4 based unit test for {@link TestContextManager}, which verifies proper <em>execution
 * order</em> of registered {@link TestExecutionListener TestExecutionListeners}.
 *
 * @author Sam Brannen
 * @since 2.5
 */
class TestContextManagerTests {
  ...
  void listenerExecutionOrder() throws Exception {
    // @formatter:off
    assertThat(this.testContextManager.getTestExecutionListeners().size())
        .as("Registered TestExecutionListeners")
        .isEqualTo(3);

    this.testContextManager.beforeTestMethod(this, this.testMethod);
    assertExecutionOrder(
        "beforeTestMethod", "beforeTestMethod-1", "beforeTestMethod-2", "beforeTestMethod-3");
  ...
  private static void assertExecutionOrder(
      String usageContext, String... expectedBeforeTestMethodCalls) {
    assertThat(executionOrder)
        .as("execution order (" + usageContext + ") ==>")
