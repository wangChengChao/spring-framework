...
class EventPublishingTestExecutionListenerTests {
  ...
  private final EventPublishingTestExecutionListener listener =
      new EventPublishingTestExecutionListener();

  @Mock private TestContext testContext;

  @Mock private ApplicationContext applicationContext;

  @Captor private ArgumentCaptor<Function<TestContext, ? extends ApplicationEvent>> eventFactory;
  ...
  void configureMock(TestInfo testInfo) {
    ...
    given(testContext.hasApplicationContext())
        .willReturn(testInfo.getTestMethod().get().getName().startsWith("publish"));
  }
  ...
  private void assertEvent(
      Class<? extends TestContextEvent> eventClass, Consumer<TestContext> callback) {
