...
/**
 * Concrete implementation of {@link AbstractTransactionalAnnotatedConfigClassTests} that does
 * <b>not</b> use a true {@link Configuration @Configuration class} but rather a <em>lite mode</em>
 * configuration class (see the Javadoc for {@link Bean @Bean} for details).
 *
 * @author Sam Brannen
 * @since 3.2
 * @see Bean
...
@ContextConfiguration(
    classes =
        TransactionalAnnotatedConfigClassesWithoutAtConfigurationTests.AnnotatedFactoryBeans.class)
public class TransactionalAnnotatedConfigClassesWithoutAtConfigurationTests
    extends AbstractTransactionalAnnotatedConfigClassTests {

  /**
   * This is intentionally <b>not</b> annotated with {@code @Configuration}.
   *
   * <p>Consequently, this class contains <i>annotated factory bean methods</i> instead of standard
   * singleton bean methods.
   */
  // @Configuration
  static class AnnotatedFactoryBeans {
    ...
    /**
     * Since this method does not reside in a true {@code @Configuration class}, it acts as a
     * factory method when invoked directly (e.g., from {@link #transactionManager()}) and as a
     * singleton bean when retrieved through the application context (e.g., when injected into the
     * test instance). The result is that this method will be called twice:
     *
     * <ol>
     *   <li>once <em>indirectly</em> by the {@link TransactionalTestExecutionListener} when it
     *       retrieves the {@link PlatformTransactionManager} from the application context
     *   <li>and again when the {@link DataSource} is injected into the test instance in {@link
     *       AbstractTransactionalAnnotatedConfigClassTests#setDataSource(DataSource)}.
     * </ol>
     *
     * Consequently, the {@link JdbcTemplate} used by this test instance and the {@link
     * PlatformTransactionManager} used by the Spring TestContext Framework will operate on two
     * different {@code DataSource} instances, which is almost certainly not the desired or intended
     * behavior.
     */
    @Bean
