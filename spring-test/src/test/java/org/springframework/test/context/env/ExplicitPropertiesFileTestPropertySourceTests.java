...
/**
 * Collection of integration tests that verify proper support for {@link
 * TestPropertySource @TestPropertySource} with explicit properties files declared with absolute
 * paths, relative paths, and placeholders in the classpath and in the file system.
 *
 * @author Sam Brannen
 * @since 5.2
 */
...
class ExplicitPropertiesFileTestPropertySourceTests {
  ...
  static void setSystemProperty() {
    String path =
        ClassUtils.classPackageAsResourcePath(ExplicitPropertiesFileTestPropertySourceTests.class);
  ...
  class FileSystemTests {
    ...
    @TestPropertySource(
        "file:src/test/resources/org/springframework/test/context/env/explicit.properties")
    ...
    @DisplayName("with relative path")
    @TestPropertySource(
        "file:../spring-test/src/test/resources/org/springframework/test/context/env/explicit.properties")
    class RelativePathTests extends AbstractExplicitPropertiesFileTests {}
    ...
    @TestPropertySource(
        "file:${user.dir}/src/test/resources/org/springframework/test/context/env/explicit.properties")
    ...
    @TestPropertySource(
        "file:${user.dir}/src/test/resources/${current.test.package}/explicit.properties")
    ...
    @TestPropertySource(
        "file:${user.dir}/../spring-test/src/test/resources/${current.test.package}/../env/explicit.properties")
