...
/**
 * Unit test which verifies proper {@link ContextLoader#processLocations(Class, String...)
 * processing} of {@code resource locations} by a {@link GenericXmlContextLoader} configured via
 * {@link ContextConfiguration @ContextConfiguration}. Specifically, this test addresses the issues
 * raised in <a href="https://opensource.atlassian.com/projects/spring/browse/SPR-3949"
 * target="_blank">SPR-3949</a>: <em>ContextConfiguration annotation should accept not only
 * classpath resources</em>.
 *
 * @author Sam Brannen
 * @since 2.5
 */
class GenericXmlContextLoaderResourceLocationsTests {
  ...
  private static final Log logger =
      LogFactory.getLog(GenericXmlContextLoaderResourceLocationsTests.class);

  @ParameterizedTest(name = "[{index}] {0}")
  @MethodSource("contextConfigurationLocationsData")
  void assertContextConfigurationLocations(String testClassName, String[] expectedLocations)
      throws Exception {
    Class<?> testClass =
        ClassUtils.forName(getClass().getName() + "$" + testClassName, getClass().getClassLoader());
    ...
    final String[] processedLocations =
        contextLoader.processLocations(testClass, configuredLocations);
    ...
    assertThat(processedLocations)
        .as("Verifying locations for test [" + testClass + "].")
  ...
  static Stream<Arguments> contextConfigurationLocationsData() {
    return Stream.of(
        ...
        arguments(
            ...
            array(
                "classpath:/org/springframework/test/context/support/context1.xml",
                "classpath:/org/springframework/test/context/support/context2.xml")),
        arguments(
            ExplicitClasspathLocationsTestCase.class.getSimpleName(),
            array("classpath:context.xml")),
        arguments(
            ExplicitFileLocationsTestCase.class.getSimpleName(),
            array("file:/testing/directory/context.xml")),
        arguments(
            ExplicitUrlLocationsTestCase.class.getSimpleName(),
            array("https://example.com/context.xml")),
        arguments(
            ...
            array(
                ...
                "file:/testing/directory/context.xml",
