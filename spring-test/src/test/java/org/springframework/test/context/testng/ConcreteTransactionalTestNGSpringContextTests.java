...
/**
 * Combined integration test for {@link AbstractTestNGSpringContextTests} and {@link
...
public class ConcreteTransactionalTestNGSpringContextTests
  ...
  void afterClass() {
    ...
    assertThat(numSetUpCallsInTransaction)
        .as("number of calls to setUp() within a transaction.")
        .isEqualTo(NUM_TX_TESTS);
    assertThat(numTearDownCalls).as("number of calls to tearDown().").isEqualTo(NUM_TESTS);
    assertThat(numTearDownCallsInTransaction)
        .as("number of calls to tearDown() within a transaction.")
        .isEqualTo(NUM_TX_TESTS);
  }
  ...
  void verifyBeanNameSet() {
    ...
    assertThat(this.beanName)
        .as(
            "The bean name of this test instance should have been set to the fully qualified class name due to BeanNameAware semantics.")
  ...
  void verifyResourceAnnotationInjectedMethods() {
    assertThatTransaction().isNotActive();
    assertThat(bar)
        .as("The setBar() method should have been injected via @Resource.")
        .isEqualTo("Bar");
  }
