 ...
 * Jupiter by registering the {@link SpringExtension} via a static field. Note, however, that this
 * is not the recommended way to register the {@code SpringExtension}.
 *
...
class RegisterExtensionSpringExtensionTests {
  ...
  @RegisterExtension static final SpringExtension springExtension = new SpringExtension();
  ...
  void genericApplicationContextInjectedIntoMethod(GenericApplicationContext applicationContext) {
    assertThat(applicationContext)
        .as("GenericApplicationContext should have been injected by Spring")
        .isNotNull();
    assertThat(applicationContext.getBean("dilbert", Person.class)).isEqualTo(this.dilbert);
  }
  ...
  void autowiredFields() {
    ...
    assertThat(this.cat)
        .as("Catbert should have been @Autowired by Spring as the @Primary cat")
        .isNotNull();
  ...
  /**
   * NOTE: Test code must be compiled with "-g" (debug symbols) or "-parameters" in order for the
   * parameter name to be used as the qualifier; otherwise, use {@code @Qualifier("wally")}.
   */
  @Test
  ...
  void autowiredParameterThatDoesNotExistButIsNotRequired(
    ...
    assertThat(number)
        .as("Non-required number should have been @Autowired as 'null' by Spring")
        .isNull();
  }
  ...
  void valueParameterFromDefaultValueForPropertyPlaceholder(
    ...
    assertThat(defaultValue)
        .as("Default value should have been injected via @Value by Spring")
        .isNotNull();
  ...
  void valueParameterFromSpelExpression(@Value("#{@dilbert.name}") String name) {
    assertThat(name)
        .as("Dilbert's name should have been injected via SpEL expression in @Value by Spring")
        .isNotNull();
  ...
  void valueParameterFromSpelExpressionWithNestedPropertyPlaceholder(
    ...
    assertThat(hello)
        .as("hello should have been injected via SpEL expression in @Value by Spring")
        .isNotNull();
    assertThat(hello).as("hello from SpEL expression").isEqualTo("Hello 42");
  }

  @Test
  void junitAndSpringMethodInjectionCombined(
      @Autowired Cat kittyCat,
      TestInfo testInfo,
