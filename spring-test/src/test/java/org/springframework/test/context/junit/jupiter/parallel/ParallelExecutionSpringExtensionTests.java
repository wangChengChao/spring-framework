...
/**
 * Integration tests which verify that {@code @BeforeEach} and {@code @AfterEach} methods that
 * accept {@code @Autowired} arguments can be executed in parallel without issues regarding
 * concurrent access to the {@linkplain Parameter parameters} of such methods.
 *
 * @author Sam Brannen
 * @since 5.1.3
 */
class ParallelExecutionSpringExtensionTests {
  ...
  void runTestsInParallel() {
    ...
    assertThat(listener.getSummary().getTestsSucceededCount())
        .as("number of tests executed successfully")
        .isEqualTo(NUM_TESTS);
  }
