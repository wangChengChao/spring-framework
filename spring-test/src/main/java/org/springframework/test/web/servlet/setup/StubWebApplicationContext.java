 ...
 *
 * <p>As registered object instances are instantiated and initialized externally, there is no
 * wiring, bean initialization, lifecycle events, as well as no pre-processing and post-processing
 * hooks typically associated with beans managed by an {@link ApplicationContext}. Just a simple
 * lookup into a {@link StaticListableBeanFactory}.
 *
...
class StubWebApplicationContext implements WebApplicationContext {
  ...
  /**
   * An extension of StaticListableBeanFactory that implements AutowireCapableBeanFactory in order
   * to allow bean initialization of {@link ApplicationContextAware} singletons.
   */
  private class StubBeanFactory extends StaticListableBeanFactory
      implements AutowireCapableBeanFactory {

    @Override
    public Object initializeBean(Object existingBean, String beanName) throws BeansException {
      if (existingBean instanceof ApplicationContextAware) {
        ((ApplicationContextAware) existingBean)
            .setApplicationContext(StubWebApplicationContext.this);
      }
      return existingBean;
    }
