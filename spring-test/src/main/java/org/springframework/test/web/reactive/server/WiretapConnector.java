...
/**
 * Decorate another {@link ClientHttpConnector} with the purpose of intercepting, capturing, and
 * exposing actual request and response data transmitted to and received from the server.
 *
...
class WiretapConnector implements ClientHttpConnector {
  ...
  class Info {
    ...
    public ExchangeResult createExchangeResult(Duration timeout, @Nullable String uriTemplate) {
      return new ExchangeResult(
          this.request,
          this.response,
          this.request.getRecorder().getContent(),
          this.response.getRecorder().getContent(),
          timeout,
          uriTemplate);
    }
  }
  ...
  static final class WiretapRecorder {
    ...
    @Nullable private final Flux<? extends DataBuffer> publisher;

    @Nullable private final Flux<? extends Publisher<? extends DataBuffer>> publisherNested;
    ...
    public WiretapRecorder(
        @Nullable Publisher<? extends DataBuffer> publisher,
      ...
      this.publisherNested =
          publisherNested != null
              ? Flux.from(publisherNested)
                  .doOnSubscribe(s -> this.hasContentConsumer = true)
                  .map(
                      p -> Flux.from(p).doOnNext(this.buffer::write).doOnError(this::handleOnError))
                  .doOnError(this::handleOnError)
                  .doOnCancel(this::handleOnComplete)
                  .doOnComplete(this::handleOnComplete)
              : null;
                      ...
                      ex ->
                          new IllegalStateException(
                              "Content has not been consumed, and "
                                  + "an error was raised while attempting to produce it.",
                              ex))
                  .subscribe();
            }
            return this.content;
  ...
  private static class WiretapClientHttpRequest extends ClientHttpRequestDecorator {
    ...
    @Nullable private WiretapRecorder recorder;
