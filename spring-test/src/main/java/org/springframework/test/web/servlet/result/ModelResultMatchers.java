 ...
 *
 * <p>An instance of this class is typically accessed via {@link MockMvcResultMatchers#model}.
 *
...
public class ModelResultMatchers {
  ...
  /** Protected constructor. Use {@link MockMvcResultMatchers#model()}. */
  ...
  public ResultMatcher attributeErrorCount(String name, int expectedCount) {
    return result -> {
      ...
      assertEquals(
          "Binding/validation error count for attribute '" + name + "', ",
          expectedCount,
  ...
  public ResultMatcher attributeHasFieldErrors(String name, String... fieldNames) {
    return mvcResult -> {
      ...
      for (String fieldName : fieldNames) {
        ...
        assertTrue(
            "No errors for field '" + fieldName + "' of attribute '" + name + "'", hasFieldErrors);
      }
    };
  }
  ...
  public ResultMatcher attributeHasFieldErrorCode(String name, String fieldName, String error) {
    return mvcResult -> {
      ...
      assertNotNull(
          "No errors for field '" + fieldName + "' of attribute '" + name + "'", fieldError);
   ...
   * @since 4.1
   */
  public <T> ResultMatcher attributeHasFieldErrorCode(
      String name, String fieldName, Matcher<? super String> matcher) {
    ...
    return result -> {
      ...
      for (Object value : mav.getModel().values()) {
        if (value instanceof Errors) {
          assertTrue(
              "Unexpected binding/validation errors: " + value, !((Errors) value).hasErrors());
        }
      }
    };
  }
  ...
  private BindingResult getBindingResult(ModelAndView mav, String name) {
    BindingResult result =
        (BindingResult) mav.getModel().get(BindingResult.MODEL_KEY_PREFIX + name);
