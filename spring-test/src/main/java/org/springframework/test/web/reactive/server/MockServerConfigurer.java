 ...
 * {@link WebTestClient.MockServerSpec} and expose that as a shortcut.
 *
 * <p>An implementation of this interface can be plugged in via {@link
 * WebTestClient.MockServerSpec#apply} where instances are likely obtained via static methods, e.g.:
 *
 * <pre class="code">
...
public interface MockServerConfigurer {
  ...
  /**
   * Invoked just before the mock server is built. Use this hook to inspect and/or modify
   * application-declared filters and exception handlers.
   *
   * @param builder the builder for the {@code HttpHandler} that will handle requests (i.e. the mock
   *     server)
   */
