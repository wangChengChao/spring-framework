...
/**
 * {@code ExchangeResult} variant with the response body decoded as {@code Flux<T>} but not yet
 * consumed.
 *
...
public class FluxExchangeResult<T> extends ExchangeResult {
   ...
   *
   * <p>The response body stream can then be consumed further with the "reactor-test" {@code
   * StepVerifier} and cancelled when enough elements have been consumed from the (possibly
   * infinite) stream:
   *
   * <pre>
  ...
  /**
   * Invoke the given consumer within {@link #assertWithDiagnostics(Runnable)} passing {@code
   * "this"} instance to it. This method allows the following, without leaving the {@code
   * WebTestClient} chain of calls:
   *
   * <pre class="code">
   * client.get()
   * 		.uri("/persons")
