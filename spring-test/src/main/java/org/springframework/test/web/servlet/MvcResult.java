...
public interface MvcResult {
  ...
  /**
   * Return any exception raised by a handler and successfully resolved through a {@link
   ...
   *
   * <p>This method will wait for the async result to be set within the timeout value associated
   * with the async request, see {@link org.springframework.mock.web.MockAsyncContext#setTimeout
   * MockAsyncContext#setTimeout}. Alternatively, use {@link #getAsyncResult(long)} to specify the
   ...
   *
   * @param timeToWait how long to wait for the async result to be set, in milliseconds; if -1, then
   *     fall back on the timeout value associated with the async request, see {@link
   *     org.springframework.mock.web.MockAsyncContext#setTimeout MockAsyncContext#setTimeout} for
   *     more details.
   * @throws IllegalStateException if the async result was not set
   */
