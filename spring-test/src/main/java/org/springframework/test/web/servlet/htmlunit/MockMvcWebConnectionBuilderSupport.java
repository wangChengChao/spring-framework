...
/**
 * Support class that simplifies the creation of a {@link WebConnection} that uses {@link MockMvc}
 * and optionally delegates to a real {@link WebConnection} for specific requests.
 *
 * <p>The default is to use {@link MockMvc} for requests to {@code localhost} and otherwise use a
 * real {@link WebConnection}.
 *
 * @author Rob Winch
 * @author Sam Brannen
 * @since 4.2
...
public abstract class MockMvcWebConnectionBuilderSupport<
    T extends MockMvcWebConnectionBuilderSupport<T>> {
   ...
   *
   * @param context the {@code WebApplicationContext} to create a {@code MockMvc} instance from;
   *     never {@code null}
   */
  ...
  /**
   * Create a new instance using the supplied {@link WebApplicationContext} and {@link
   * MockMvcConfigurer}.
   *
   * @param context the {@code WebApplicationContext} to create a {@code MockMvc} instance from;
   *     never {@code null}
   ...
   */
  protected MockMvcWebConnectionBuilderSupport(
   ...
   *
   * <p>If the supplied value is {@code null} or empty, the first path segment of the request URL is
   * assumed to be the context path.
   *
   * <p>Default is {@code ""}.
   *
  ...
  /**
   * Specify that {@link MockMvc} should always be used regardless of what the request looks like.
   *
   * @return this builder for further customization
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * Add additional {@link WebRequestMatcher} instances that will ensure that {@link MockMvc} is
   * used to process the request, if such a matcher matches against the web request.
   *
  ...
  /**
   * Add additional {@link WebRequestMatcher} instances that return {@code true} if a supplied host
   * matches &mdash; for example, {@code "example.com"} or {@code "example.com:8080"}.
   *
  ...
  /**
   * Create a new {@link WebConnection} that will use a {@link MockMvc} instance if one of the
   * specified {@link WebRequestMatcher} instances matches.
   *
   * @param webClient the WebClient to use if none of the specified {@code WebRequestMatcher}
   *     instances matches (never {@code null})
   * @return a new {@code WebConnection} that will use a {@code MockMvc} instance if one of the
   *     specified {@code WebRequestMatcher} matches
   * @since 4.3
