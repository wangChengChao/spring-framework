 ...
 *
 * <p>An instance of this class is available via {@link MockMvcResultMatchers#header}.
 *
...
public class HeaderResultMatchers {
  ...
  /** Protected constructor. See {@link MockMvcResultMatchers#header()}. */
  ...
  public ResultMatcher doesNotExist(String name) {
    return result ->
        assertTrue(
            "Response should not contain header '" + name + "'",
            !result.getResponse().containsHeader(name));
  }
   ...
   *
   * <p>The {@link ResultMatcher} returned by this method throws an {@link AssertionError} if the
   * response does not contain the specified header, or if the supplied {@code value} does not match
   * the primary value.
   */
  ...
  /**
   * Assert the primary value of the named response header parsed into a date using the preferred
   * date format described in RFC 7231.
   *
   * <p>The {@link ResultMatcher} returned by this method throws an {@link AssertionError} if the
   * response does not contain the specified header, or if the supplied {@code value} does not match
   * the primary value.
    ...
    return result -> {
      ...
      assertEquals(
          "Response header '"
              ...
              + "'",
          headers.getFirstDate("expected"),
          headers.getFirstDate("actual"));
    };
  }
}
