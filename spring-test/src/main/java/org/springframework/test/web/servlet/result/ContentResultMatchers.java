 ...
 *
 * <p>An instance of this class is typically accessed via {@link MockMvcResultMatchers#content}.
 *
...
public class ContentResultMatchers {
  ...
  /**
   * Assert the ServletResponse content type. The given content type must fully match including
   * type, sub-type, and parameters. For checking only the type and sub-type see {@link
  ...
  /**
   * Assert the ServletResponse content type after parsing it as a MediaType. The given content type
   * must fully match including type, sub-type, and parameters. For checking only the type and
   * sub-type see {@link #contentTypeCompatibleWith(MediaType)}.
   */
  ...
  public ResultMatcher contentTypeCompatibleWith(MediaType contentType) {
    return result -> {
      ...
      assertTrue(
          "Content type [" + actual + "] is not compatible with [" + contentType + "]",
  ...
  public ResultMatcher string(String expectedContent) {
    return result ->
        assertEquals(
            "Response content", expectedContent, result.getResponse().getContentAsString());
  }
  ...
  public ResultMatcher bytes(byte[] expectedContent) {
    return result ->
        assertEquals(
            "Response content", expectedContent, result.getResponse().getContentAsByteArray());
  }
   ...
   *
   * <p>Use of this matcher requires the <a href="http://xmlunit.sourceforge.net/">XMLUnit</a>
   * library.
   *
  ...
  /**
   * Parse the expected and actual strings as JSON and assert the two are "similar" - i.e. they
   * contain the same attribute-value pairs regardless of formatting with a lenient checking
   ...
   * they contain the same attribute-value pairs regardless of formatting.
   *
   * <p>Can compare in two modes, depending on {@code strict} parameter value:
   *
   * <ul>
   *   <li>{@code true}: strict checking. Not extensible, and strict array ordering.
   *   <li>{@code false}: lenient checking. Extensible, and non-strict array ordering.
   * </ul>
   *
