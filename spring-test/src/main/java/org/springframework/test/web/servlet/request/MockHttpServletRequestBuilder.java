...
/**
 * Default builder for {@link MockHttpServletRequest} required as input to perform requests in
 * {@link MockMvc}.
 *
 * <p>Application tests will typically access this builder through the static factory methods in
 * {@link MockMvcRequestBuilders}.
 *
 * <p>This class is not open for extension. To apply custom initialization to the created {@code
 * MockHttpServletRequest}, please use the {@link #with(RequestPostProcessor)} extension point.
 *
...
public class MockHttpServletRequestBuilder
  ...
  /**
   * Package private constructor. To get an instance, use static factory methods in {@link
   * MockMvcRequestBuilders}.
   *
   * <p>Although this class cannot be extended, additional ways to initialize the {@code
   * MockHttpServletRequest} can be plugged in via {@link #with(RequestPostProcessor)}.
   *
  ...
  MockHttpServletRequestBuilder(HttpMethod httpMethod, String url, Object... vars) {
    this(
        httpMethod.name(),
        UriComponentsBuilder.fromUriString(url).buildAndExpand(vars).encode().toUri());
  }

  /**
   * Alternative to {@link #MockHttpServletRequestBuilder(HttpMethod, String, Object...)} with a
   * pre-built URI.
   *
   * @param httpMethod the HTTP method (GET, POST, etc)
   * @param url the URL
  ...
  /**
   * Specify the portion of the requestURI that represents the context path. The context path, if
   * specified, must match to the start of the request URI.
   *
   * <p>In most cases, tests can be written by omitting the context path from the requestURI. This
   * is because most applications don't actually depend on the name under which they're deployed. If
   * specified here, the context path must start with a "/" and must not end with a "/".
   *
   ...
   * This is typically a portion of the requestURI after the context path.
   *
   * <p>In most cases, tests can be written by omitting the servlet path from the requestURI. This
   * is because most applications don't actually depend on the prefix to which a servlet is mapped.
   * For example if a Servlet is mapped to {@code "/main/*"}, tests can be written with the
   * requestURI {@code "/accounts/1"} as opposed to {@code "/main/accounts/1"}. If specified here,
   * the servletPath must start with a "/" and must not end with a "/".
   *
   ...
   *
   * <p>If left unspecified (recommended), the pathInfo will be automatically derived by removing
   * the contextPath and the servletPath from the requestURI and using any remaining part. If
   * specified here, the pathInfo must start with a "/".
   *
  ...
  /**
   * Set the secure property of the {@link ServletRequest} indicating use of a secure channel, such
   * as HTTPS.
   *
  ...
  /**
   * Add a map of request parameters to the {@link MockHttpServletRequest}, for example when testing
   * a form submission.
   *
   * <p>If called more than once, new values get added to existing ones.
   *
   ...
   *
   * <p>Individual attributes provided via {@link #sessionAttr(String, Object)} override the content
   * of the session provided here.
   *
  ...
  /**
   * An extension point for further initialization of {@link MockHttpServletRequest} in ways not
   * built directly into the {@code MockHttpServletRequestBuilder}. Implementation of this interface
   * can have builder-style methods themselves and be made accessible through static factory
   * methods.
   *
  ...
  /**
   * Merges the properties of the "parent" RequestBuilder accepting values only if not already set
   * in "this" instance.
   *
  ...
  public final MockHttpServletRequest buildRequest(ServletContext servletContext) {
    ...
    this.sessionAttributes.forEach(
        (name, attribute) -> {
  ...
  /**
   * Create a new {@link MockHttpServletRequest} based on the supplied {@code ServletContext}.
   *
  ...
  private void updatePathRequestProperties(MockHttpServletRequest request, String requestUri) {
    ...
    if ("".equals(this.pathInfo)) {
      ...
      String extraPath =
          requestUri.substring(this.contextPath.length() + this.servletPath.length());
      this.pathInfo =
          (StringUtils.hasText(extraPath)
              ? urlPathHelper.decodeRequestString(request, extraPath)
              : null);
    }
  ...
  private FlashMapManager getFlashMapManager(MockHttpServletRequest request) {
    ...
    try {
      ...
      flashMapManager =
          wac.getBean(DispatcherServlet.FLASH_MAP_MANAGER_BEAN_NAME, FlashMapManager.class);
  ...
  private static <T> void addToMultiValueMap(
      MultiValueMap<String, T> map, String name, T[] values) {
    Assert.hasLength(name, "'name' must not be empty");
