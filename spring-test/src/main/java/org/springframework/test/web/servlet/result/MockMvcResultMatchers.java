...
public abstract class MockMvcResultMatchers {
   ...
   * Asserts the request was forwarded to the given URL.
   *
   * <p>This method accepts {@link org.springframework.util.AntPathMatcher} patterns.
   *
  ...
  public static ResultMatcher forwardedUrlPattern(String urlPattern) {
    return result -> {
      assertTrue(
          "'" + urlPattern + "' is not an Ant-style path pattern",
          pathMatcher.isPattern(urlPattern));
      String url = result.getResponse().getForwardedUrl();
      assertTrue(
          "Forwarded URL '"
              + url
              + "' does not match the expected URL pattern '"
              + urlPattern
              + "'",
   ...
   * href="https://github.com/jayway/JsonPath">JsonPath</a> expression to inspect a specific subset
   * of the body.
   *
   * <p>The JSON path expression can be a parameterized string using formatting specifiers as
   * defined in {@link String#format(String, Object...)}.
   *
  ...
  /**
   * Evaluate the given <a href="https://github.com/jayway/JsonPath">JsonPath</a> expression against
   * the response body and assert the resulting value with the given Hamcrest {@link Matcher}.
   *
   ...
   * Evaluate the given <a href="https://github.com/jayway/JsonPath">JsonPath</a> expression against
   * the response body and assert the resulting value with the given Hamcrest {@link Matcher},
   * coercing the resulting value into the given target type before applying the matcher.
   *
   ...
   * @see #jsonPath(String, Matcher)
   */
  public static <T> ResultMatcher jsonPath(
      String expression, Matcher<T> matcher, Class<T> targetType) {
   ...
   * the body.
   *
   * <p>The XPath expression can be a parameterized string using formatting specifiers as defined in
   ...
   * @param args arguments to parameterize the XPath expression with
   */
  public static XpathResultMatchers xpath(
      String expression, Map<String, String> namespaces, Object... args)
      throws XPathExpressionException {
