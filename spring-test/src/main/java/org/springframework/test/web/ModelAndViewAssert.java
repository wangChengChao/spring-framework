 ...
 *
 * <p>Intended for use with JUnit 4 and TestNG. All {@code assert*()} methods throw {@link
 * AssertionError AssertionErrors}.
 *
 * @author Sam Brannen
...
public abstract class ModelAndViewAssert {
  ...
  /**
   * Checks whether the model value under the given {@code modelName} exists and checks it type,
   * based on the {@code expectedType}. If the model entry exists and the type matches, the model
   * value is returned.
   *
  ...
  public static <T> T assertAndReturnModelAttributeOfType(
      ModelAndView mav, String modelName, Class<T> expectedType) {
    ...
    assertTrue(
        "Model attribute is not of expected type '"
            + expectedType.getName()
            + "' but rather of type '"
            + obj.getClass().getName()
            + "'",
        expectedType.isAssignableFrom(obj.getClass()));
    return (T) obj;
  }
  ...
  public static void assertCompareListModelAttribute(
      ModelAndView mav, String modelName, List expectedList) {
    List modelList = assertAndReturnModelAttributeOfType(mav, modelName, List.class);
    assertTrue(
        "Size of model list is '"
            + modelList.size()
            + "' while size of expected list is '"
            + expectedList.size()
            + "'",
        expectedList.size() == modelList.size());
    assertTrue(
        "List in model under name '" + modelName + "' is not equal to the expected list.",
  ...
  public static void assertModelAttributeAvailable(ModelAndView mav, String modelName) {
    Map<String, Object> model = mav.getModel();
    assertTrue(
        "Model attribute with name '" + modelName + "' is not available",
        model.containsKey(modelName));
  }

  /**
   * Compare a given {@code expectedValue} to the value from the model bound under the given {@code
   * modelName}.
   *
   * @param mav the ModelAndView to test against (never {@code null})
   * @param modelName name of the object to add to the model (never {@code null})
   ...
   */
  public static void assertModelAttributeValue(
      ModelAndView mav, String modelName, Object expectedValue) {
    Object modelValue = assertAndReturnModelAttributeOfType(mav, modelName, Object.class);
    assertTrue(
        "Model value with name '"
            + modelName
            + "' is not the same as the expected value which was '"
            + expectedValue
            + "'",
        modelValue.equals(expectedValue));
  }
   ...
   */
  public static void assertModelAttributeValues(
      ModelAndView mav, Map<String, Object> expectedModel) {
    Map<String, Object> model = mav.getModel();
    ...
    model.forEach(
        (modelName, mavValue) -> {
          ...
          if (!assertionValue.equals(mavValue)) {
            sb.append("Value under name '")
                .append(modelName)
                .append("' differs, should have been '")
  ...
  /**
   * Compare each individual entry in a list after having sorted both lists (optionally using a
   * comparator).
   *
   * @param mav the ModelAndView to test against (never {@code null})
   * @param modelName name of the object to add to the model (never {@code null})
   * @param expectedList the expected list
   * @param comparator the comparator to use (may be {@code null}). If not specifying the
   *     comparator, both lists will be sorted not using any comparator.
   */
