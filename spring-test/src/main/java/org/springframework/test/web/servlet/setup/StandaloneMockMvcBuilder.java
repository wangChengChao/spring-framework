...
/**
 * A {@code MockMvcBuilder} that accepts {@code @Controller} registrations thus allowing full
 * control over the instantiation and initialization of controllers and their dependencies similar
 * to plain unit tests, and also making it possible to test one controller at a time.
 *
 * <p>This builder creates the minimum infrastructure required by the {@link DispatcherServlet} to
 * serve requests with annotated controllers and also provides methods for customization. The
 * resulting configuration and customization options are equivalent to using MVC Java config except
 * using builder style methods.
 *
 * <p>To configure view resolution, either select a "fixed" view to use for every request performed
 * (see {@link #setSingleView(View)}) or provide a list of {@code ViewResolver}s (see {@link
...
public class StandaloneMockMvcBuilder extends AbstractMockMvcBuilder<StandaloneMockMvcBuilder> {
  ...
  @Nullable private FormattingConversionService conversionService;
  ...
  @Nullable private Boolean removeSemicolonContent;

  private Map<String, String> placeholderValues = new HashMap<>();

  private Supplier<RequestMappingHandlerMapping> handlerMappingFactory =
  ...
  /**
   * Register one or more {@link org.springframework.web.bind.annotation.ControllerAdvice} instances
   * to be used in tests (specified {@code Class} will be turned into instance).
   *
   * <p>Normally {@code @ControllerAdvice} are auto-detected as long as they're declared as Spring
   * beans. However since the standalone setup does not load any Spring config, they need to be
   * registered explicitly here instead much like controllers.
   *
   * @since 4.2
   */
  ...
  /**
   * Set the message converters to use in argument resolvers and in return value handlers, which
   * support reading and/or writing to the body of the request and response. If no message
   * converters are added to the list, a default list of converters is added instead.
   */
  public StandaloneMockMvcBuilder setMessageConverters(
  ...
  /**
   * Provide a custom {@link Validator} instead of the one created by default. The default
   * implementation used, assuming JSR-303 is on the classpath, is {@link
  ...
  /**
   * Provide a conversion service with custom formatters and converters. If not set, a {@link
   * DefaultFormattingConversionService} is used by default.
   */
  public StandaloneMockMvcBuilder setConversionService(
  ...
  /**
   * Specify the timeout value for async execution. In Spring MVC Test, this value is used to
   * determine how to long to wait for async execution to complete so that a test can verify the
  ...
  public StandaloneMockMvcBuilder setCustomArgumentResolvers(
  ...
  public StandaloneMockMvcBuilder setHandlerExceptionResolvers(
      HandlerExceptionResolver... exceptionResolvers) {
    this.handlerExceptionResolvers = Arrays.asList(exceptionResolvers);
    return this;
  }

  /**
   * Set up view resolution with the given {@link ViewResolver ViewResolvers}. If not set, an {@link
   * InternalResourceViewResolver} is used by default.
   */
  ...
  /**
   * Sets up a single {@link ViewResolver} that always returns the provided view instance. This is a
   * convenient shortcut if you need to use one View instance only -- e.g. rendering generated
   * content (JSON, XML, Atom).
   */
  ...
  /**
   * Provide a LocaleResolver instance. If not provided, the default one used is {@link
  ...
  /**
   * Provide a custom FlashMapManager instance. If not provided, {@code SessionFlashMapManager} is
   * used by default.
   */
  ...
  /**
   * Whether to match to URLs irrespective of the presence of a trailing slash. If enabled a method
   * mapped to "/users" also matches to "/users/".
   *
   * <p>The default value is {@code true}.
   */
  public StandaloneMockMvcBuilder setUseTrailingSlashPatternMatch(
      boolean useTrailingSlashPatternMatch) {
    this.useTrailingSlashPatternMatch = useTrailingSlashPatternMatch;
    return this;
  }

  /**
   * Set if ";" (semicolon) content should be stripped from the request URI. The value, if provided,
   * is in turn set on {@link AbstractHandlerMapping#setRemoveSemicolonContent(boolean)}.
   */
   ...
   * This method allows manually provided placeholder values so they can be resolved. Alternatively
   * consider creating a test that initializes a {@link WebApplicationContext}.
   *
   * @since 4.2.8
   */
   ...
   * @since 5.0
   */
  public StandaloneMockMvcBuilder setCustomHandlerMapping(
      Supplier<RequestMappingHandlerMapping> factory) {
  ...
  private void registerMvcSingletons(StubWebApplicationContext wac) {
    ...
    RequestMappingHandlerMapping hm =
        config.getHandlerMapping(mvcConversionService, resourceUrlProvider);
    if (sc != null) {
    ...
    wac.addBean(
        "handlerExceptionResolver", config.handlerExceptionResolver(mvcContentNegotiationManager));
  ...
  /**
   * This method could be used from a sub-class to register additional Spring MVC infrastructure
   * such as additional {@code HandlerMapping}, {@code HandlerAdapter}, and others.
   *
  ...
  private class StandaloneConfiguration extends WebMvcConfigurationSupport {
    ...
    public ContentNegotiationManager mvcContentNegotiationManager() {
      return (contentNegotiationManager != null)
