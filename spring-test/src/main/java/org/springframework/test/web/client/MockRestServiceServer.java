...
/**
 * <strong>Main entry point for client-side REST testing</strong>. Used for tests that involve
 * direct or indirect use of the {@link RestTemplate}. Provides a way to set up expected requests
 * that will be performed through the {@code RestTemplate} as well as mock responses to send back
 * thus removing the need for an actual server.
 *
 * <p>Below is an example that assumes static imports from {@code MockRestRequestMatchers}, {@code
 * MockRestResponseCreators}, and {@code ExpectedCount}:
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * <p>Note that as an alternative to the above you can also set the {@link
 * MockMvcClientHttpRequestFactory} on a {@code RestTemplate} which allows executing requests
 * against an instance of {@link org.springframework.test.web.servlet.MockMvc MockMvc}.
 *
 * @author Craig Walls
...
public final class MockRestServiceServer {
  ...
  /**
   * Private constructor with {@code RequestExpectationManager}. See static builder methods and
   * {@code createServer} shortcut methods.
   */
  ...
  /**
   * Set up an expectation for a single HTTP request. The returned {@link ResponseActions} can be
   * used to set up further expectations as well as to define the response.
   *
   * <p>This method may be invoked any number times before starting to make request through the
   * underlying {@code RestTemplate} in order to set up all expected requests.
   *
  ...
  /**
   * An alternative to {@link #expect(RequestMatcher)} that also indicates how many times the
   * request is expected to be executed.
   *
   * <p>When request expectations have an expected count greater than one, only the first execution
   * is expected to match the order of declaration. Subsequent request executions may be inserted
   * anywhere thereafter.
   *
  ...
  /**
   * Verify that all expected requests set up via {@link #expect(RequestMatcher)} were indeed
   * performed.
   *
  ...
  /**
   * Return a builder for a {@code MockRestServiceServer} that should be used to reply to the given
   * {@code RestTemplate}.
   *
   * @since 4.3
   */
   ...
   *
   * @since 4.3
   * @deprecated see deprecation notice on {@link org.springframework.web.client.AsyncRestTemplate}
   *     itself
   */
  @Deprecated
  public static MockRestServiceServerBuilder bindTo(
   ...
   */
  @Deprecated
  public static MockRestServiceServer createServer(
      org.springframework.web.client.AsyncRestTemplate asyncRestTemplate) {
  ...
  public interface MockRestServiceServerBuilder {
    ...
    /**
     * Whether to allow expected requests to be executed in any order not necessarily matching the
    ...
    /**
     * Use the {@link BufferingClientHttpRequestFactory} wrapper to buffer the input and output
     * streams, and for example, allow multiple reads of the response body.
     *
     * @since 5.0.5
     */
    ...
    /**
     * Build the {@code MockRestServiceServer} and set up the underlying {@code RestTemplate} or
     * {@code AsyncRestTemplate} with a {@link ClientHttpRequestFactory} that creates mock requests.
     */
    MockRestServiceServer build();

    /** An overloaded build alternative that accepts a custom {@link RequestExpectationManager}. */
  ...
  private static class DefaultBuilder implements MockRestServiceServerBuilder {
    ...
    @Nullable private final org.springframework.web.client.AsyncRestTemplate asyncRestTemplate;
  ...
  /**
   * Mock ClientHttpRequestFactory that creates requests by iterating over the list of expected
   * {@link DefaultRequestExpectation}'s.
   */
