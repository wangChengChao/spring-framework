 ...
 *
 * <p>An instance of this class is typically accessed via {@link MockMvcResultMatchers#xpath}.
 *
...
public class XpathResultMatchers {
  ...
  /**
   * Protected constructor, not for direct instantiation. Use {@link
   * MockMvcResultMatchers#xpath(String, Object...)} or {@link MockMvcResultMatchers#xpath(String,
   * Map, Object...)}.
   *
  ...
  /**
   * Evaluate the XPath and assert the {@link Node} content found with the given Hamcrest {@link
   * Matcher}.
   */
  public ResultMatcher node(Matcher<? super Node> matcher) {
    return result -> {
      MockHttpServletResponse response = result.getResponse();
      this.xpathHelper.assertNode(
  ...
  /**
   * Evaluate the XPath and assert the number of nodes found with the given Hamcrest {@link
   * Matcher}.
   */
  public ResultMatcher nodeCount(Matcher<Integer> matcher) {
    return result -> {
      MockHttpServletResponse response = result.getResponse();
      this.xpathHelper.assertNodeCount(
          response.getContentAsByteArray(), getDefinedEncoding(response), matcher);
    };
  }
  ...
  /**
   * Apply the XPath and assert the {@link String} value found with the given Hamcrest {@link
   * Matcher}.
   */
  public ResultMatcher string(Matcher<? super String> matcher) {
    return result -> {
      MockHttpServletResponse response = result.getResponse();
      this.xpathHelper.assertString(
          response.getContentAsByteArray(), getDefinedEncoding(response), matcher);
    };
  }
  ...
  public ResultMatcher string(String expectedValue) {
    return result -> {
      MockHttpServletResponse response = result.getResponse();
      this.xpathHelper.assertString(
          response.getContentAsByteArray(), getDefinedEncoding(response), expectedValue);
    };
  }

  /**
   * Evaluate the XPath and assert the {@link Double} value found with the given Hamcrest {@link
   * Matcher}.
   */
  public ResultMatcher number(Matcher<? super Double> matcher) {
    return result -> {
      MockHttpServletResponse response = result.getResponse();
      this.xpathHelper.assertNumber(
          response.getContentAsByteArray(), getDefinedEncoding(response), matcher);
    };
  }
  ...
  public ResultMatcher booleanValue(Boolean value) {
    return result -> {
      MockHttpServletResponse response = result.getResponse();
      this.xpathHelper.assertBoolean(
          response.getContentAsByteArray(), getDefinedEncoding(response), value);
    };
  }
}
