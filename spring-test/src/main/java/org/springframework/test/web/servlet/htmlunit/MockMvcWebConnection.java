...
public final class MockMvcWebConnection implements WebConnection {
  ...
  /**
   * Create a new instance that assumes the context path of the application is {@code ""} (i.e., the
   * root context).
   *
   * <p>For example, the URL {@code http://localhost/test/this} would use {@code ""} as the context
   * path.
   *
   ...
   *
   * <p>The path may be {@code null} in which case the first path segment of the URL is turned into
   * the contextPath. Otherwise it must conform to {@link
