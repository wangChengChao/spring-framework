...
/**
 * {@code MockMvcWebClientBuilder} simplifies the creation of an HtmlUnit {@link WebClient} that
 * delegates to a {@link MockMvc} instance.
 *
 * <p>The {@code MockMvc} instance used by the builder may be {@linkplain #mockMvcSetup supplied
 * directly} or created transparently from a {@link #webAppContextSetup WebApplicationContext}.
 *
 * @author Rob Winch
 * @author Sam Brannen
 * @since 4.2
...
public class MockMvcWebClientBuilder
  ...
  @Nullable private WebClient webClient;
  ...
  /**
   * Create a new {@code MockMvcWebClientBuilder} based on the supplied {@link
  ...
  /**
   * Supply the {@code WebClient} that the client {@linkplain #build built} by this builder should
   * delegate to when processing non-{@linkplain WebRequestMatcher matching} requests.
   *
   * @param webClient the {@code WebClient} to delegate to for requests that do not match; never
   *     {@code null}
   ...
   *
   * <p>The returned client will use the configured {@link MockMvc} instance for processing any
   * {@linkplain WebRequestMatcher matching} requests and a delegate {@code WebClient} for all other
   * requests.
   *
   * <p>If a {@linkplain #withDelegate delegate} has been explicitly configured, it will be used;
   * otherwise, a default {@code WebClient} will be configured as the delegate.
   *
