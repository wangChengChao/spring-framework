...
public class XpathAssertions {
  ...
  XpathAssertions(
      ...
      String expression,
      @Nullable Map<String, String> namespaces,
      Object... args) {
  ...
  public WebTestClient.BodyContentSpec isEqualTo(String expectedValue) {
    return assertWith(
        () -> this.xpathHelper.assertString(getContent(), getCharset(), expectedValue));
  }
  ...
  public WebTestClient.BodyContentSpec isEqualTo(Double expectedValue) {
    return assertWith(
        () -> this.xpathHelper.assertNumber(getContent(), getCharset(), expectedValue));
  }
  ...
  public WebTestClient.BodyContentSpec isEqualTo(boolean expectedValue) {
    return assertWith(
        () -> this.xpathHelper.assertBoolean(getContent(), getCharset(), expectedValue));
  }
  ...
  public WebTestClient.BodyContentSpec nodeCount(int expectedCount) {
    return assertWith(
        () -> this.xpathHelper.assertNodeCount(getContent(), getCharset(), expectedCount));
  }
  ...
  /**
   * Lets us be able to use lambda expressions that could throw checked exceptions, since {@link
   * XpathExpectationsHelper} throws {@link Exception} on its methods.
   */
