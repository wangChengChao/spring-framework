...
/**
 * Encapsulates the behavior required to implement {@link MockRestServiceServer} including its
 * public API (create expectations + verify/reset) along with an extra method for verifying actual
 * requests.
 *
 * <p>This contract is not used directly in applications but a custom implementation can be {@link
 * org.springframework.test.web.client.MockRestServiceServer.MockRestServiceServerBuilder#build(RequestExpectationManager)
...
public interface RequestExpectationManager {
  ...
  /**
   * Set up a new request expectation. The returned {@link ResponseActions} is used to add more
   * expectations and define a response.
   *
   * <p>This is a delegate for {@link MockRestServiceServer#expect(ExpectedCount, RequestMatcher)}.
   *
  ...
  /**
   * Validate the given actual request against the declared expectations. Is successful return the
   * mock response to use or raise an error.
   *
