...
/**
 * Client for testing web servers that uses {@link WebClient} internally to perform requests while
 * also providing a fluent API to verify responses. This client can connect to any server over HTTP,
 * or to a WebFlux application via mock request and response objects.
 *
 ...
 * </ul>
 *
 * <p><strong>Warning</strong>: {@code WebTestClient} is not usable yet in Kotlin due to a <a
 * href="https://youtrack.jetbrains.com/issue/KT-5464">type inference issue</a> which is expected to
 * be fixed as of Kotlin 1.3. You can watch <a
 * href="https://github.com/spring-projects/spring-framework/issues/20606">gh-20606</a> for
 * up-to-date information. Meanwhile, the proposed alternative is to use directly {@link WebClient}
 * with its Reactor and Spring Kotlin extensions to perform integration tests on an embedded WebFlux
 * server.
 *
...
public interface WebTestClient {
   ...
   * {@code WebTestClient}. This can be useful for storing contextual information at all phases of
   * request processing (e.g. from a server-side component) under that id and later to look up that
   * information once an {@link ExchangeResult} is available.
   */
  ...
  /**
   * Mutate the {@link WebTestClient}, apply the given configurer, and build a new instance.
  ...
  /**
   * Use this server setup to test one `@Controller` at a time. This option loads the default
   * configuration of {@link org.springframework.web.reactive.config.EnableWebFlux @EnableWebFlux}.
   * There are builder methods to customize the Java config. The resulting WebFlux application will
   * be tested without an HTTP server using a mock request and response.
   *
   * @param controllers one or more controller instances to tests (specified {@code Class} will be
   *     turned into instance)
   * @return chained API to customize server and client config; use {@link
   *     MockServerSpec#configureClient()} to transition to client config
   */
  ...
  /**
   * Use this option to set up a server from a {@link RouterFunction}. Internally the provided
   * configuration is passed to {@code RouterFunctions#toWebHandler}. The resulting WebFlux
   * application will be tested without an HTTP server using a mock request and response.
   *
  ...
  /**
   * Use this option to setup a server from the Spring configuration of your application, or some
   * subset of it. Internally the provided configuration is passed to {@code WebHttpHandlerBuilder}
   * to set up the request processing chain. The resulting WebFlux application will be tested
   * without an HTTP server using a mock request and response.
   *
   * <p>Consider using the TestContext framework and {@link
   * org.springframework.test.context.ContextConfiguration @ContextConfiguration} in order to
   * efficiently load and inject the Spring configuration into the test class.
   *
  ...
  interface MockServerSpec<B extends MockServerSpec<B>> {
     ...
     *
     * <p>By default an instance of {@link
     * org.springframework.web.server.session.DefaultWebSessionManager DefaultWebSessionManager} is
     * used.
     *
  ...
  /**
   * Specification for customizing controller configuration equivalent to, and internally delegating
   * to, a {@link WebFluxConfigurer}.
   */
  ...
  /**
   * Steps for customizing the {@link WebClient} used to test with, internally delegating to a
   * {@link org.springframework.web.reactive.function.client.WebClient.Builder WebClient.Builder}.
   */
  interface Builder {

    /**
     * Configure a base URI as described in {@link
     * org.springframework.web.reactive.function.client.WebClient#create(String)
     * WebClient.create(String)}.
     */
    Builder baseUrl(String baseUrl);

    /**
     * Provide a pre-configured {@link UriBuilderFactory} instance as an alternative to and
     * effectively overriding {@link #baseUrl(String)}.
     */
    ...
    /**
     * Manipulate the default headers with the given consumer. The headers provided to the consumer
     * are "live", so that the consumer can be used to {@linkplain HttpHeaders#set(String, String)
     * overwrite} existing header values, {@linkplain HttpHeaders#remove(Object) remove} values, or
     * use any of the other {@link HttpHeaders} methods.
     *
    ...
    /**
     * Manipulate the default cookies with the given consumer. The map provided to the consumer is
     * "live", so that the consumer can be used to {@linkplain MultiValueMap#set(Object, Object)
     * overwrite} existing header values, {@linkplain MultiValueMap#remove(Object) remove} values,
     * or use any of the other {@link MultiValueMap} methods.
     *
    ...
    /**
     * Manipulate the filters with the given consumer. The list provided to the consumer is "live",
     * so that the consumer can be used to remove filters, change ordering, etc.
     *
  ...
  interface UriSpec<S extends RequestHeadersSpec<?>> {
    ...
    /**
     * Specify the URI for the request using a URI template and URI variables. If a {@link
     * UriBuilderFactory} was configured for the client (e.g. with a base URI) it will be used to
    ...
    /**
     * Build the URI for the request with a {@link UriBuilder} obtained through the {@link
     * UriBuilderFactory} configured for this client.
     *
     * @return spec to add headers or perform the exchange
     */
  ...
  interface RequestHeadersSpec<S extends RequestHeadersSpec<S>> {
    ...
    /**
     * Manipulate this request's cookies with the given consumer. The map provided to the consumer
     * is "live", so that the consumer can be used to {@linkplain MultiValueMap#set(Object, Object)
     * overwrite} existing header values, {@linkplain MultiValueMap#remove(Object) remove} values,
     * or use any of the other {@link MultiValueMap} methods.
     *
     * @param cookiesConsumer a function that consumes the cookies map
    ...
    /**
     * Manipulate the request's headers with the given consumer. The headers provided to the
     * consumer are "live", so that the consumer can be used to {@linkplain HttpHeaders#set(String,
     * String) overwrite} existing header values, {@linkplain HttpHeaders#remove(Object) remove}
    ...
    /**
     * Manipulate the request attributes with the given consumer. The attributes provided to the
     * consumer are "live", so that the consumer can be used to inspect attributes, remove
     * attributes, or use any of the other map-provided methods.
     *
  ...
  interface RequestBodySpec extends RequestHeadersSpec<RequestBodySpec> {
    ...
    /**
     * Set the body to the given {@code Object} value. This method invokes the {@link
     * WebClient.RequestBodySpec#bodyValue(Object) bodyValue} method on the underlying {@code
     * WebClient}.
     *
    ...
    /**
     * Set the body from the given {@code Publisher}. Shortcut for {@link #body(BodyInserter)} with
     * a {@linkplain BodyInserters#fromPublisher Publisher inserter}.
     *
    ...
    /**
     * Set the body from the given producer. This method invokes the {@link
     * WebClient.RequestBodySpec#body(Object, Class)} method on the underlying {@code WebClient}.
     *
     * @param producer the producer to write to the request. This must be a {@link Publisher} or
     *     another producer adaptable to a {@code Publisher} via {@link ReactiveAdapterRegistry}
    ...
    /**
     * Set the body from the given producer. This method invokes the {@link
     * WebClient.RequestBodySpec#body(Object, ParameterizedTypeReference)} method on the underlying
     * {@code WebClient}.
     *
     * @param producer the producer to write to the request. This must be a {@link Publisher} or
     *     another producer adaptable to a {@code Publisher} via {@link ReactiveAdapterRegistry}
    ...
    /**
     * Set the body of the request to the given {@code BodyInserter}. This method invokes the {@link
     * WebClient.RequestBodySpec#body(BodyInserter)} method on the underlying {@code WebClient}.
     *
    ...
    /**
     * Shortcut for {@link #body(BodyInserter)} with a {@linkplain BodyInserters#fromValue value
     * inserter}. As of 5.2 this method delegates to {@link #bodyValue(Object)}.
     *
  ...
  interface ResponseSpec {
    ...
    /**
     * Consume and decode the response body to a single object of type {@code <B>} and then apply
     * assertions.
     *
    ...
    /**
     * Alternative to {@link #expectBody(Class)} that accepts information about a target type with
     * generics.
     */
    <B> BodySpec<B, ?> expectBody(ParameterizedTypeReference<B> bodyType);

    /**
     * Consume and decode the response body to {@code List<E>} and then apply List-specific
     * assertions.
     *
    ...
    /**
     * Alternative to {@link #expectBodyList(Class)} that accepts information about a target type
     * with generics.
     */
    <E> ListBodySpec<E> expectBodyList(ParameterizedTypeReference<E> elementType);

    /**
     * Consume and decode the response body to {@code byte[]} and then apply assertions on the raw
     * content (e.g. isEmpty, JSONPath, etc.)
     */
    BodyContentSpec expectBody();

    /**
     * Exit the chained API and consume the response body externally. This is useful for testing
     * infinite streams (e.g. SSE) where you need to to assert decoded objects as they come and then
     * cancel at some point when test objectives are met. Consider using {@code StepVerifier} from
     * {@literal "reactor-test"} to assert the {@code Flux<T>} stream of decoded objects.
     *
     * <p><strong>Note:</strong> Do not use this option for cases where there is no content (e.g.
     * 204, 4xx) or you're not interested in the content. For such cases you can use {@code
     * expectBody().isEmpty()} or {@code expectBody(Void.class)} which ensures that resources are
     * released regardless of whether the response has content or not.
     */
    <T> FluxExchangeResult<T> returnResult(Class<T> elementClass);

    /**
     * Alternative to {@link #returnResult(Class)} that accepts information about a target type with
     * generics.
     */
  ...
  interface BodySpec<B, S extends BodySpec<B, S>> {
    ...
    /**
     * Transform the extracted the body with a function, e.g. extracting a property, and assert the
     * mapped value with a {@link Matcher}.
     *
     * @since 5.1
     */
    ...
    /**
     * Exit the chained API and return an {@code ExchangeResult} with the decoded response content.
     */
  ...
  interface BodyContentSpec {
    ...
    /**
     * Parse the expected and actual response content as JSON and perform a "lenient" comparison
     * verifying the same attribute-value pairs.
     *
     * <p>Use of this option requires the <a
     * href="https://jsonassert.skyscreamer.org/">JSONassert</a> library on to be on the classpath.
     *
     ...
     *
     * <p>Use of this method requires the <a href="https://github.com/xmlunit/xmlunit">XMLUnit</a>
     ...
     * href="https://github.com/jayway/JsonPath">JsonPath</a> expression to inspect a specific
     * subset of the body.
     *
     * <p>The JSON path expression can be a parameterized string using formatting specifiers as
     * defined in {@link String#format}.
     *
    ...
    /**
     * Access to response body assertions using an XPath expression to inspect a specific subset of
     * the body.
     *
     * <p>The XPath expression can be a parameterized string using formatting specifiers as defined
    ...
    /**
     * Access to response body assertions with specific namespaces using an XPath expression to
     ...
     *
     * @param consumer the consumer for the response body; the input {@code byte[]} may be {@code
     *     null} if there was no response body.
     */
    BodyContentSpec consumeWith(Consumer<EntityExchangeResult<byte[]>> consumer);

    /** Exit the chained API and return an {@code ExchangeResult} with the raw response content. */
