 ...
 *
 * <p>An instance of this class is typically accessed via {@link MockMvcResultMatchers#request}.
 *
...
public class RequestResultMatchers {
  ...
  /**
   * Assert whether asynchronous processing started, usually as a result of a controller method
   * returning {@link Callable} or {@link DeferredResult}.
   *
   * <p>The test will await the completion of a {@code Callable} so that {@link
   * #asyncResult(Matcher)} can be used to assert the resulting value. Neither a {@code Callable}
   * nor a {@code DeferredResult} will complete processing all the way since a {@link
   * MockHttpServletRequest} does not perform asynchronous dispatches.
   */
   ...
   *
   * <p>This method can be used when a controller method returns {@link Callable} or {@link
   * WebAsyncTask}.
   */
   ...
   *
   * <p>This method can be used when a controller method returns {@link Callable} or {@link
   * WebAsyncTask}. The value matched is the value returned from the {@code Callable} or the
   * exception raised.
   */
  ...
  public <T> ResultMatcher attribute(String name, Object expectedValue) {
    return result ->
        assertEquals(
            ...
            expectedValue,
            result.getRequest().getAttribute(name));
  }
