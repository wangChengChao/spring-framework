...
public abstract class JdbcTestUtils {
   ...
   *
   * <p>If the provided {@code WHERE} clause contains text, it will be prefixed with {@code " WHERE
   * "} and then appended to the generated {@code DELETE} statement. For example, if the provided
   * table name is {@code "person"} and the provided where clause is {@code "name = 'Bob' and age >
   * 25"}, the resulting SQL statement to execute will be {@code "DELETE FROM person WHERE name =
   * 'Bob' and age > 25"}.
   *
   * <p>As an alternative to hard-coded values, the {@code "?"} placeholder can be used within the
   * {@code WHERE} clause, binding to the given arguments.
   *
   * @param jdbcTemplate the JdbcTemplate with which to perform JDBC operations
