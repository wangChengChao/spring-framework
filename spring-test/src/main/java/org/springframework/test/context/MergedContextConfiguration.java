...
/**
 * {@code MergedContextConfiguration} encapsulates the <em>merged</em> context configuration
 * declared on a test class and all of its superclasses via {@link
 * ContextConfiguration @ContextConfiguration}, {@link ActiveProfiles @ActiveProfiles}, and {@link
 * TestPropertySource @TestPropertySource}.
 *
 * <p>Merged context resource locations, annotated classes, active profiles, property resource
 * locations, and in-lined properties represent all declared values in the test class hierarchy
 * taking into consideration the semantics of the {@link ContextConfiguration#inheritLocations},
 * {@link ActiveProfiles#inheritProfiles}, {@link TestPropertySource#inheritLocations}, and {@link
 * TestPropertySource#inheritProperties} flags.
 *
 * <p>A {@link SmartContextLoader} uses {@code MergedContextConfiguration} to load an {@link
 * org.springframework.context.ApplicationContext ApplicationContext}.
 *
 * <p>{@code MergedContextConfiguration} is also used by the {@link
 * org.springframework.test.context.cache.ContextCache ContextCache} as the key for caching an
 * {@link org.springframework.context.ApplicationContext ApplicationContext} that was loaded using
 * properties of this {@code MergedContextConfiguration}.
 *
 * @author Sam Brannen
 * @author Phillip Webb
 * @since 3.1
...
public class MergedContextConfiguration implements Serializable {
  ...
  private static final Set<ContextCustomizer> EMPTY_CONTEXT_CUSTOMIZERS =
      Collections.<ContextCustomizer>emptySet();
   ...
   * @param activeProfiles the merged active bean definition profiles
   * @param contextLoader the resolved {@code ContextLoader}
   * @param cacheAwareContextLoaderDelegate a cache-aware context loader delegate with which to
  ...
  /**
   * Create a new {@code MergedContextConfiguration} instance by copying all fields from the
  ...
  public MergedContextConfiguration(MergedContextConfiguration mergedConfig) {
    this(
        mergedConfig.testClass,
        mergedConfig.locations,
        mergedConfig.classes,
        mergedConfig.contextInitializerClasses,
        mergedConfig.activeProfiles,
        mergedConfig.propertySourceLocations,
        mergedConfig.propertySourceProperties,
        mergedConfig.contextCustomizers,
        mergedConfig.contextLoader,
        mergedConfig.cacheAwareContextLoaderDelegate,
        mergedConfig.parent);
  }
   ...
   * Create a new {@code MergedContextConfiguration} instance for the supplied parameters.
   *
   * <p>If a {@code null} value is supplied for {@code locations}, {@code classes}, {@code
   * activeProfiles}, {@code propertySourceLocations}, or {@code propertySourceProperties} an empty
   * array will be stored instead. If a {@code null} value is supplied for the {@code
   * contextInitializerClasses} an empty set will be stored instead. Furthermore, active profiles
   * will be sorted, and duplicate profiles will be removed.
   *
   * @param testClass the test class for which the configuration was merged
   * @param locations the merged context resource locations
   * @param classes the merged annotated classes
  ...
  public MergedContextConfiguration(
      ...
      @Nullable String[] propertySourceLocations,
      @Nullable String[] propertySourceProperties,
      ContextLoader contextLoader,
      @Nullable CacheAwareContextLoaderDelegate cacheAwareContextLoaderDelegate,
      @Nullable MergedContextConfiguration parent) {
   ...
   * <p>If a {@code null} value is supplied for {@code locations}, {@code classes}, {@code
   * activeProfiles}, {@code propertySourceLocations}, or {@code propertySourceProperties} an empty
   * array will be stored instead. If a {@code null} value is supplied for {@code
   * contextInitializerClasses} or {@code contextCustomizers}, an empty set will be stored instead.
   * Furthermore, active profiles will be sorted, and duplicate profiles will be removed.
   *
   * @param testClass the test class for which the configuration was merged
   * @param locations the merged context resource locations
   * @param classes the merged annotated classes
  ...
  /**
   * Get the merged resource locations for {@code ApplicationContext} configuration files for the
   * {@linkplain #getTestClass() test class}.
   *
   * <p>Context resource locations typically represent XML configuration files or Groovy scripts.
   */
  ...
  /**
   * Determine if this {@code MergedContextConfiguration} instance has path-based context resource
   * locations.
   *
  ...
  /**
   * Determine if this {@code MergedContextConfiguration} instance has either path-based context
   * resource locations or class-based resources.
   *
   * @return {@code true} if either the {@link #getLocations() locations} or the {@link
   *     #getClasses() classes} array is not empty
   * @since 4.0.4
   * @see #hasLocations()
   * @see #hasClasses()
   */
  ...
  /**
   * Get the merged {@code ApplicationContextInitializer} classes for the {@linkplain
   * #getTestClass() test class}.
   */
  ...
  /**
   * Get the merged resource locations for test {@code PropertySources} for the {@linkplain
   * #getTestClass() test class}.
   *
  ...
  /**
   * Get the merged test {@code PropertySource} properties for the {@linkplain #getTestClass() test
   * class}.
   *
   * <p>Properties will be loaded into the {@code Environment}'s set of {@code PropertySources}.
   *
  ...
  /**
   * Get the merged {@link ContextCustomizer ContextCustomizers} that will be applied when the
  ...
  /**
   * Get the {@link MergedContextConfiguration} for the parent application context in a context
   * hierarchy.
   *
  ...
  /**
   * Get the parent {@link ApplicationContext} for the context defined by this {@code
   * MergedContextConfiguration} from the context cache.
   *
   * <p>If the parent context has not yet been loaded, it will be loaded, stored in the cache, and
   * then returned.
   *
  ...
  public ApplicationContext getParentApplicationContext() {
    ...
    Assert.state(
        this.cacheAwareContextLoaderDelegate != null,
  ...
  /**
   * Determine if the supplied object is equal to this {@code MergedContextConfiguration} instance
   * by comparing both object's {@linkplain #getLocations() locations}, {@linkplain #getClasses()
   * annotated classes}, {@linkplain #getContextInitializerClasses() context initializer classes},
   * {@linkplain #getActiveProfiles() active profiles}, {@linkplain #getPropertySourceLocations()
   * property source locations}, {@linkplain #getPropertySourceProperties() property source
   * properties}, {@linkplain #getParent() parents}, and the fully qualified names of their {@link
   * #getContextLoader() ContextLoaders}.
   */
  @Override
  public boolean equals(@Nullable Object other) {
    ...
    if (!nullSafeClassName(this.contextLoader)
  ...
  /**
   * Generate a unique hash code for all properties of this {@code MergedContextConfiguration}
   * excluding the {@linkplain #getTestClass() test class}.
   */
  @Override
  public int hashCode() {
  ...
  private static Set<Class<? extends ApplicationContextInitializer<?>>>
  ...
  private static Set<ContextCustomizer> processContextCustomizers(
    ...
    return (contextCustomizers != null
        ? Collections.unmodifiableSet(contextCustomizers)
        : EMPTY_CONTEXT_CUSTOMIZERS);
  }
  ...
  /**
   * Generate a null-safe {@link String} representation of the supplied {@link ContextLoader} based
   * solely on the fully qualified name of the loader or &quot;null&quot; if the supplied loader is
   * {@code null}.
   */
