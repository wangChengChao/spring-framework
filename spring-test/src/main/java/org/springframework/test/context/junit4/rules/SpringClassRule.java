...
/**
 * {@code SpringClassRule} is a custom JUnit {@link TestRule} that supports <em>class-level</em>
 * features of the <em>Spring TestContext Framework</em> in standard JUnit tests by means of the
 ...
 * {@code SpringClassRule} must be combined with a {@link SpringMethodRule}, since {@code
 * SpringClassRule} only supports the class-level features of the {@code SpringJUnit4ClassRunner}.
 *
 * <h3>Example Usage</h3>
 *
 ...
 * {@code SpringClassRule}. <em>(Note that additional annotations may be supported by various {@link
 * org.springframework.test.context.TestExecutionListener TestExecutionListener} or {@link
...
public class SpringClassRule implements TestRule {
  ...
  /**
   * Apply <em>class-level</em> features of the <em>Spring TestContext Framework</em> to the
   * supplied {@code base} statement.
   *
   * <p>Specifically, this method retrieves the {@link TestContextManager} used by this rule and its
   * associated {@link SpringMethodRule} and invokes the {@link TestContextManager#beforeTestClass()
   * beforeTestClass()} and {@link TestContextManager#afterTestClass() afterTestClass()} methods on
   * the {@code TestContextManager}.
   *
   * <p>In addition, this method checks whether the test is enabled in the current execution
   * environment. This prevents classes with a non-matching {@code @IfProfileValue} annotation from
   * running altogether, even skipping the execution of {@code beforeTestClass()} methods in {@code
   ...
   * @return a statement that wraps the supplied {@code base} with class-level features of the
   ...
   */
  private Statement withBeforeTestClassCallbacks(
      Statement next, TestContextManager testContextManager) {
