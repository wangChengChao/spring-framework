...
public abstract class TestConstructorUtils {
   ...
   *
   * <p>This method delegates to {@link #isAutowirableConstructor(Constructor, Class)} if the
   ...
   *
   * <p>A constructor is considered to be autowirable if one of the following conditions is {@code
   * true}.
   *
   * <ol>
   *   <li>The constructor is annotated with {@link Autowired @Autowired}.
   *   <li>{@link TestConstructor @TestConstructor} is <em>present</em> or <em>meta-present</em> on
   *       the test class with {@link TestConstructor#autowireMode() autowireMode} set to {@link
   *       AutowireMode#ALL ALL}.
   *   <li>The default <em>test constructor autowire mode</em> has been changed to {@code ALL} (see
  ...
  public static boolean isAutowirableConstructor(Constructor<?> constructor, Class<?> testClass) {
    ...
    TestConstructor testConstructor =
        AnnotatedElementUtils.findMergedAnnotation(testClass, TestConstructor.class);
      ...
      if (value != null) {
        try {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug(
                String.format(
                    "Failed to parse autowire mode '%s' for property '%s': %s",
                    value,
                    TestConstructor.TEST_CONSTRUCTOR_AUTOWIRE_MODE_PROPERTY_NAME,
                    ex.getMessage()));
          }
