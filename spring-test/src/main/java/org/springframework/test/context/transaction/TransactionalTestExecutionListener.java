...
/**
 * {@code TestExecutionListener} that provides support for executing tests within <em>test-managed
 * transactions</em> by honoring Spring's {@link Transactional @Transactional} annotation.
 *
 * <h3>Test-managed Transactions</h3>
 *
 * <p><em>Test-managed transactions</em> are transactions that are managed declaratively via this
 * listener or programmatically via {@link TestTransaction}. Such transactions should not be
 * confused with <em>Spring-managed transactions</em> (i.e., those managed directly by Spring within
 * the {@code ApplicationContext} loaded for tests) or <em>application-managed transactions</em>
 * (i.e., those managed programmatically within application code that is invoked via tests).
 * Spring-managed and application-managed transactions will typically participate in test-managed
 * transactions; however, caution should be taken if Spring-managed or application-managed
 * transactions are configured with any propagation type other than {@link
 * org.springframework.transaction.annotation.Propagation#REQUIRED REQUIRED} or {@link
 * org.springframework.transaction.annotation.Propagation#SUPPORTS SUPPORTS}.
 *
 * <h3>Enabling and Disabling Transactions</h3>
 *
 * <p>Annotating a test method with {@code @Transactional} causes the test to be run within a
 * transaction that will, by default, be automatically <em>rolled back</em> after completion of the
 * test. If a test class is annotated with {@code @Transactional}, each test method within that
 * class hierarchy will be run within a transaction. Test methods that are <em>not</em> annotated
 * with {@code @Transactional} (at the class or method level) will not be run within a transaction.
 * Furthermore, tests that <em>are</em> annotated with {@code @Transactional} but have the {@link
 * Transactional#propagation propagation} type set to {@link
 * org.springframework.transaction.annotation.Propagation#NOT_SUPPORTED NOT_SUPPORTED} will not be
 * run within a transaction.
 *
 * <h3>Declarative Rollback and Commit Behavior</h3>
 *
 * <p>By default, test transactions will be automatically <em>rolled back</em> after completion of
 * the test; however, transactional commit and rollback behavior can be configured declaratively via
 * the {@link Commit @Commit} and {@link Rollback @Rollback} annotations at the class level and at
 * the method level.
 *
 ...
 * programmatically via the static methods in {@link TestTransaction}. {@code TestTransaction} may
 * be used within <em>test</em> methods, <em>before</em> methods, and <em>after</em> methods.
 *
 ...
 * BeforeTransaction @BeforeTransaction} or {@link AfterTransaction @AfterTransaction}. As of Spring
 * Framework 4.3, {@code @BeforeTransaction} and {@code @AfterTransaction} may also be declared on
 * Java 8 based interface default methods.
 *
 * <h3>Configuring a Transaction Manager</h3>
 *
 * <p>{@code TransactionalTestExecutionListener} expects a {@link PlatformTransactionManager} bean
 * to be defined in the Spring {@code ApplicationContext} for the test. In case there are multiple
 * instances of {@code PlatformTransactionManager} within the test's {@code ApplicationContext}, a
 * <em>qualifier</em> may be declared via {@link Transactional @Transactional} (e.g.,
 * {@code @Transactional("myTxMgr")} or {@code @Transactional(transactionManger = "myTxMgr")}, or
 * {@link org.springframework.transaction.annotation.TransactionManagementConfigurer
 * TransactionManagementConfigurer} can be implemented by an {@link
 * org.springframework.context.annotation.Configuration @Configuration} class. See {@link
 * TestContextTransactionUtils#retrieveTransactionManager} for details on the algorithm used to look
 * up a transaction manager in the test's {@code ApplicationContext}.
 *
...
public class TransactionalTestExecutionListener extends AbstractTestExecutionListener {
  ...
  /**
   * If the test method of the supplied {@linkplain TestContext test context} is configured to run
   * within a transaction, this method will run {@link BeforeTransaction @BeforeTransaction} methods
  ...
  public void beforeTestMethod(final TestContext testContext) throws Exception {
    ...
    Assert.state(
        txContext == null, "Cannot start new transaction without ending existing transaction");

    PlatformTransactionManager tm = null;
    TransactionAttribute transactionAttribute =
        this.attributeSource.getTransactionAttribute(testMethod, testClass);

    if (transactionAttribute != null) {
      transactionAttribute =
          TestContextTransactionUtils.createDelegatingTransactionAttribute(
              testContext, transactionAttribute);
    ...
    if (tm != null) {
      txContext =
          new TransactionContext(testContext, tm, transactionAttribute, isRollback(testContext));
  ...
  /**
   * If a transaction is currently active for the supplied {@linkplain TestContext test context},
   * this method will end the transaction and run {@link AfterTransaction @AfterTransaction}
   * methods.
   *
  ...
  /**
   * Run all {@link BeforeTransaction @BeforeTransaction} methods for the specified {@linkplain
  ...
  /**
   * Run all {@link AfterTransaction @AfterTransaction} methods for the specified {@linkplain
  ...
  /**
   * Get the {@linkplain PlatformTransactionManager transaction manager} to use for the supplied
   * {@linkplain TestContext test context} and {@code qualifier}.
   *
  ...
  protected PlatformTransactionManager getTransactionManager(
      TestContext testContext, @Nullable String qualifier) {
  ...
  /**
   * Get the {@linkplain PlatformTransactionManager transaction manager} to use for the supplied
   * {@linkplain TestContext test context}.
   *
   ...
   * @param testContext the test context for which the transaction manager should be retrieved
   * @return the transaction manager to use, or {@code null} if not found
   * @throws BeansException if an error occurs while retrieving an explicitly named transaction
   *     manager
   * @throws IllegalStateException if more than one TransactionManagementConfigurer exists in the
   *     ApplicationContext
   * @see #getTransactionManager(TestContext, String)
   */
  @Nullable
  ...
  /**
   * Determine whether or not to rollback transactions by default for the supplied {@linkplain
   * TestContext test context}.
   *
   * <p>Supports {@link Rollback @Rollback} or {@link Commit @Commit} at the class-level.
   *
  ...
  protected final boolean isDefaultRollback(TestContext testContext) throws Exception {
    ...
    if (rollbackPresent) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            String.format(
                "Retrieved default @Rollback(%s) for test class [%s].",
  ...
  /**
   * Determine whether or not to rollback transactions for the supplied {@linkplain TestContext test
   * context} by taking into consideration the {@linkplain #isDefaultRollback(TestContext) default
   * rollback} flag and a possible method-level override via the {@link Rollback @Rollback}
   * annotation.
   *
  ...
  /**
   * Get all methods in the supplied {@link Class class} and its superclasses which are annotated
   ...
   * @return all annotated methods in the supplied class and its superclasses as well as annotated
