...
/**
 * Abstract {@linkplain Transactional transactional} extension of {@link
 * AbstractJUnit4SpringContextTests} which adds convenience functionality for JDBC access. Expects a
 * {@link DataSource} bean and a {@link PlatformTransactionManager} bean to be defined in the Spring
 * {@linkplain ApplicationContext application context}.
 *
 * <p>This class exposes a {@link JdbcTemplate} and provides an easy way to {@linkplain
 * #countRowsInTable count the number of rows in a table} (potentially {@linkplain
 * #countRowsInTableWhere with a WHERE clause}), {@linkplain #deleteFromTables delete from tables},
 * {@linkplain #dropTables drop tables}, and {@linkplain #executeSqlScript execute SQL scripts}
 * within a transaction.
 *
 * <p>Concrete subclasses must fulfill the same requirements outlined in {@link
 ...
 *       specify your runner of choice via {@link org.junit.runner.RunWith @RunWith(...)}.
 * </ul>
 *
...
public abstract class AbstractTransactionalJUnit4SpringContextTests
  ...
  /**
   * Convenience method for counting the rows in the given table, using the provided {@code WHERE}
   * clause.
   *
  ...
  /**
   * Convenience method for deleting all rows from the given table, using the provided {@code WHERE}
   * clause.
   *
   * <p>Use with caution outside of a transaction!
   *
   ...
   * @param whereClause the {@code WHERE} clause to append to the query
   * @param args arguments to bind to the query (leaving it to the {@code PreparedStatement} to
   *     guess the corresponding SQL type); may also contain {@link
   *     org.springframework.jdbc.core.SqlParameterValue SqlParameterValue} objects which indicate
  ...
  protected void executeSqlScript(String sqlResourcePath, boolean continueOnError)
    ...
    new ResourceDatabasePopulator(continueOnError, false, this.sqlScriptEncoding, resource)
        .execute(ds);
  }
}
