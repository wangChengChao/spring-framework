...
class TransactionContext {
   ...
   *
   * <p>In other words, should we roll back or commit the current transaction upon completion of the
   * current test?
   */
  ...
  /**
   * Immediately force a <em>commit</em> or <em>rollback</em> of the transaction for the configured
   * test context, according to the {@linkplain #isFlaggedForRollback rollback flag}.
   */
  void endTransaction() {
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
          (this.flaggedForRollback ? "Rolled back" : "Committed")
