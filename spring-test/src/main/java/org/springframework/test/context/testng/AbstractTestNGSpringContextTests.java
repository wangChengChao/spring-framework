 ...
 * {@link ApplicationContext} testing support in a <strong>TestNG</strong> environment.
 *
 * <p>Concrete subclasses:
 *
 * <ul>
 *   <li>Typically declare a class-level {@link ContextConfiguration &#064;ContextConfiguration}
 *       annotation to configure the {@linkplain ApplicationContext application context} {@linkplain
 *       ContextConfiguration#locations() resource locations} or {@linkplain
 *       ContextConfiguration#classes() annotated classes}. <em>If your test does not need to load
 *       an application context, you may choose to omit the {@code @ContextConfiguration}
 *       declaration and to configure the appropriate {@link
 *       org.springframework.test.context.TestExecutionListener TestExecutionListeners}
 *       manually.</em>
 *   <li>Must have constructors which either implicitly or explicitly delegate to {@code super();}.
 * </ul>
 *
...
public abstract class AbstractTestNGSpringContextTests
  ...
  @Nullable private Throwable testException;

  /**
   * Construct a new AbstractTestNGSpringContextTests instance and initialize the internal {@link
   * TestContextManager} for the current test class.
   */
  ...
  /**
   * Delegates to the configured {@link TestContextManager} to call {@linkplain
   * TestContextManager#beforeTestClass() 'before test class'} callbacks.
   *
  ...
  /**
   * Delegates to the configured {@link TestContextManager} to {@linkplain
   * TestContextManager#prepareTestInstance(Object) prepare} this test instance prior to execution
   * of any individual tests, for example for injecting dependencies, etc.
   *
   * @throws Exception if a registered TestExecutionListener throws an exception
   */
  ...
  /**
   * Delegates to the configured {@link TestContextManager} to {@linkplain
   * TestContextManager#beforeTestMethod(Object,Method) pre-process} the test method before the
  ...
  /**
   * Delegates to the {@linkplain IHookCallBack#runTestMethod(ITestResult) test method} in the
   * supplied {@code callback} to execute the actual test and then tracks the exception thrown
   * during test execution, if any.
   *
  ...
  /**
   * Delegates to the configured {@link TestContextManager} to {@linkplain
   * TestContextManager#afterTestMethod(Object, Method, Throwable) post-process} the test method
   * after the actual test has executed.
   *
  ...
  /**
   * Delegates to the configured {@link TestContextManager} to call {@linkplain
   * TestContextManager#afterTestClass() 'after test class'} callbacks.
   *
   * @throws Exception if a registered TestExecutionListener throws an exception
   */
