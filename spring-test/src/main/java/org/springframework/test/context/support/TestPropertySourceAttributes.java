...
/**
 * {@code TestPropertySourceAttributes} encapsulates attributes declared via {@link
 * TestPropertySource @TestPropertySource} annotations.
 *
 * <p>In addition to encapsulating declared attributes, {@code TestPropertySourceAttributes} also
...
class TestPropertySourceAttributes {
  ...
  /**
   * Determine if the annotation represented by this {@code TestPropertySourceAttributes} instance
   * can be merged with the supplied {@code annotation}.
   *
   * <p>This method effectively checks that two annotations are declared at the same level in the
   * type hierarchy (i.e., have the same {@linkplain MergedAnnotation#getAggregateIndex() aggregate
   * index}).
   *
   * @since 5.2
  ...
  /**
   * Merge this {@code TestPropertySourceAttributes} instance with the supplied {@code annotation},
   * asserting that the two sets of test property source attributes have identical values for the
   * {@link TestPropertySource#inheritLocations} and {@link TestPropertySource#inheritProperties}
   * flags and that the two underlying annotations were declared on the same class.
   *
   * <p>This method should only be invoked if {@link #canMergeWith(MergedAnnotation)} returns {@code
   * true}.
   *
   * @since 5.2
  ...
  void mergeWith(MergedAnnotation<TestPropertySource> annotation) {
    ...
    logger.trace(
        LogMessage.format(
            "Retrieved %s for declaring class [%s].", annotation, this.declaringClass.getName()));
  ...
  private void assertSameBooleanAttribute(
      boolean expected, MergedAnnotation<TestPropertySource> annotation, String attribute) {

    Assert.isTrue(
        expected == annotation.getBoolean(attribute),
        () ->
            String.format(
                ...
                this.rootAnnotation.getType().getSimpleName(),
                this.declaringClass.getSimpleName(),
                annotation.getRoot().getType().getSimpleName(),
                declaringClass(annotation).getSimpleName(),
                attribute));
  }
  ...
  /**
   * Add all of the supplied elements to the provided list, honoring the {@code prepend} flag.
   *
   * <p>If the {@code prepend} flag is {@code false}, the elements will appended to the list.
   *
  ...
  private String detectDefaultPropertiesFile(MergedAnnotation<TestPropertySource> annotation) {
    ...
    String resourcePath =
        ClassUtils.convertClassNameToResourcePath(testClass.getName()) + ".properties";
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
          String.format(
              "Detected default properties file \"%s\" for test class [%s]",
   ...
   *
   * <p>Note: The returned value may represent a <em>detected default</em> or merged locations that
   * do not match the original value declared via a single {@code @TestPropertySource} annotation.
   *
   ...
   *
   * <p>Note: The returned value may represent merged properties that do not match the original
   * value declared via a single {@code @TestPropertySource} annotation.
   *
  ...
  /**
   * Provide a String representation of the {@code @TestPropertySource} attributes and declaring
   * class.
   */
  @Override
