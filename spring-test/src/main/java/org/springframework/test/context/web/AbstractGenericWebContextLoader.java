 ...
 *
 * <p>If instances of concrete subclasses are invoked via the {@link
 * org.springframework.test.context.SmartContextLoader SmartContextLoader} SPI, the context will be
 * loaded from the {@link MergedContextConfiguration} provided to {@link
 * #loadContext(MergedContextConfiguration)}. In such cases, a {@code SmartContextLoader} will
 * decide whether to load the context from <em>locations</em> or <em>annotated classes</em>. Note
 * that {@code AbstractGenericWebContextLoader} does not support the {@code loadContext(String...
 * locations)} method from the legacy {@link org.springframework.test.context.ContextLoader
 * ContextLoader} SPI.
 *
...
public abstract class AbstractGenericWebContextLoader extends AbstractContextLoader {
   ...
   *
   * <ul>
   *   <li>Calls {@link #validateMergedContextConfiguration(WebMergedContextConfiguration)} to allow
   ...
   *       retrieved and {@linkplain GenericWebApplicationContext#setParent(ApplicationContext) set
   *       as the parent} for the context created by this method.
  ...
  public final ConfigurableApplicationContext loadContext(MergedContextConfiguration mergedConfig)
      throws Exception {
    Assert.isTrue(
        mergedConfig instanceof WebMergedContextConfiguration,
        () ->
            String.format(
  ...
  /**
   * Validate the supplied {@link WebMergedContextConfiguration} with respect to what this context
   * loader supports.
   *
   ...
   *
   * <p>If the supplied WAC has no parent or its parent is not a WAC, the supplied WAC will be
   * configured as the Root WAC (see "<em>Root WAC Configuration</em>" below).
   *
   * <p>Otherwise the context hierarchy of the supplied WAC will be traversed to find the top-most
   * WAC (i.e., the root); and the {@link ServletContext} of the Root WAC will be set as the {@code
   * ServletContext} for the supplied WAC.
   *
  ...
  /**
   * Customize the internal bean factory of the {@code WebApplicationContext} created by this
   * context loader.
   *
  ...
  /**
   * Load bean definitions into the supplied {@link GenericWebApplicationContext context} from the
   * locations or classes in the supplied {@code WebMergedContextConfiguration}.
   *
  ...
  /**
   * Customize the {@link GenericWebApplicationContext} created by this context loader <i>after</i>
   * bean definitions have been loaded into the context but <i>before</i> the context is refreshed.
   *
   * <p>The default implementation simply delegates to {@link
   * AbstractContextLoader#customizeContext(ConfigurableApplicationContext,
   * MergedContextConfiguration)}.
   *
  ...
  /**
   * {@code AbstractGenericWebContextLoader} should be used as a {@link
   * org.springframework.test.context.SmartContextLoader SmartContextLoader}, not as a legacy {@link
   * org.springframework.test.context.ContextLoader ContextLoader}. Consequently, this method is not
   * supported.
   *
