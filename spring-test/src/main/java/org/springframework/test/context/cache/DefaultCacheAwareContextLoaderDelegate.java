 ...
 *
 * <p>To use a static {@link DefaultContextCache}, invoke the {@link
 * #DefaultCacheAwareContextLoaderDelegate()} constructor; otherwise, invoke the {@link
 * #DefaultCacheAwareContextLoaderDelegate(ContextCache)} and provide a custom {@link ContextCache}
 * implementation.
 *
 * @author Sam Brannen
 * @since 4.1
...
public class DefaultCacheAwareContextLoaderDelegate implements CacheAwareContextLoaderDelegate {
  ...
  /**
   * Construct a new {@code DefaultCacheAwareContextLoaderDelegate} using a static {@link
   * DefaultContextCache}.
   *
   * <p>This default cache is static so that each context can be cached and reused for all
   * subsequent tests that declare the same unique context configuration within the same JVM
   * process.
   *
  ...
  /**
   * Construct a new {@code DefaultCacheAwareContextLoaderDelegate} using the supplied {@link
   * ContextCache}.
   *
  ...
  protected ApplicationContext loadContextInternal(
      MergedContextConfiguration mergedContextConfiguration) throws Exception {
      ...
      if (context == null) {
        try {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug(
                String.format(
                    "Storing ApplicationContext [%s] in cache under key [%s]",
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              String.format(
                  "Retrieved ApplicationContext [%s] from cache with key [%s]",
                  System.identityHashCode(context), mergedContextConfiguration));
        }
      }
  ...
  public void closeContext(
      ...
      @Nullable HierarchyMode hierarchyMode) {
    synchronized (this.contextCache) {
