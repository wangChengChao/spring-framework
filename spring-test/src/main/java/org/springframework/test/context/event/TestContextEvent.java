...
public abstract class TestContextEvent extends ApplicationEvent {
   ...
   *
   * @param source the {@code TestContext} associated with this event (must not be {@code null})
   */
   ...
   *
   * <p>This method may be favored over {@code getSource()} &mdash; for example, to improve
   * readability in SpEL expressions for event processing {@linkplain
   * org.springframework.context.event.EventListener#condition conditions}.
   *
   * @return the {@code TestContext} associated with this event (never {@code null})
   * @see #getSource()
   */
