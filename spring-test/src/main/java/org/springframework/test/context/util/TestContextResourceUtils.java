...
/**
 * Utility methods for working with resources within the <em>Spring TestContext Framework</em>.
...
public abstract class TestContextResourceUtils {
   ...
   *
   * <p>Delegates to {@link #convertToClasspathResourcePaths(Class, boolean, String...)} with {@code
   * false} supplied for the {@code preservePlaceholders} flag.
   *
   ...
   *
   * <ul>
   *   <li>A plain path &mdash; for example, {@code "context.xml"} &mdash; will be treated as a
   *       classpath resource that is relative to the package in which the specified class is
   *       defined. Such a path will be prepended with the {@code classpath:} prefix and the path to
   *       the package for the class.
   *   <li>A path starting with a slash will be treated as an absolute path within the classpath,
   *       for example: {@code "/org/example/schema.sql"}. Such a path will be prepended with the
   *       {@code classpath:} prefix.
   *   <li>A path which is already prefixed with a URL protocol (e.g., {@code classpath:}, {@code
   *       file:}, {@code http:}, etc.) will not have its protocol modified.
   * </ul>
   *
   * <p>Each path will then be {@linkplain StringUtils#cleanPath cleaned}, unless the {@code
   * preservePlaceholders} flag is {@code true} and the path contains one or more placeholders in
   * the form <code>${placeholder.name}</code>.
   *
   * @param clazz the class with which the paths are associated
  ...
  public static String[] convertToClasspathResourcePaths(
      Class<?> clazz, boolean preservePlaceholders, String... paths) {
  ...
  /**
   * Convert the supplied paths to an array of {@link Resource} handles using the given {@link
   * ResourceLoader}.
   *
  ...
  /**
   * Convert the supplied paths to a list of {@link Resource} handles using the given {@link
   * ResourceLoader}.
   *
   * @param resourceLoader the {@code ResourceLoader} to use to convert the paths
   * @param paths the paths to be converted
  ...
  public static List<Resource> convertToResourceList(
      ResourceLoader resourceLoader, String... paths) {
