...
/**
 * Abstract base class for implementations of {@link ExecutionCondition} that evaluate expressions
 * configured via annotations to determine if a container or test is enabled.
 *
...
abstract class AbstractExpressionEvaluatingCondition implements ExecutionCondition {
  ...
  /**
   * Evaluate the expression configured via the supplied annotation type on the {@link
   * AnnotatedElement} for the supplied {@link ExtensionContext}.
   *
   * @param annotationType the type of annotation to process
   * @param expressionExtractor a function that extracts the expression from the annotation
   * @param reasonExtractor a function that extracts the reason from the annotation
   * @param loadContextExtractor a function that extracts the {@code loadContext} flag from the
   *     annotation
   * @param enabledOnTrue indicates whether the returned {@code ConditionEvaluationResult} should be
   *     {@link ConditionEvaluationResult#enabled enabled} if the expression evaluates to {@code
   *     true}
   * @param context the {@code ExtensionContext}
   * @return {@link ConditionEvaluationResult#enabled enabled} if the container or test should be
   *     enabled; otherwise {@link ConditionEvaluationResult#disabled disabled}
   */
  protected <A extends Annotation> ConditionEvaluationResult evaluateAnnotation(
    ...
    if (!annotation.isPresent()) {
      String reason =
          String.format(
              "%s is enabled since @%s is not present", element, annotationType.getSimpleName());
            ...
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        String.format(
                            "The expression in @%s on [%s] must not be blank",
    ...
    if (evaluatedToTrue) {
      ...
      String reason =
          annotation
              ...
              .orElseGet(
                  () ->
                      String.format(
                          "%s is %s because @%s(\"%s\") evaluated to true",
      ...
      String reason =
          String.format(
              "%s is %s because @%s(\"%s\") did not evaluate to true",
              element, adjective, annotationType.getSimpleName(), expression);
      if (logger.isDebugEnabled()) {
        logger.debug(reason);
      }
  ...
  private <A extends Annotation> boolean evaluateExpression(
    ...
    if (result instanceof Boolean) {
      ...
      Assert.state(
          ...
          () ->
              String.format(
                  "@%s(\"%s\") on %s must evaluate to \"true\" or \"false\", not \"%s\"",
                  annotationType.getSimpleName(), expression, element, result));
      return false;
    } else {
      String message =
          String.format(
              "@%s(\"%s\") on %s must evaluate to a String or a Boolean, not %s",
