...
/**
 * Utility methods for {@link SmartContextLoader SmartContextLoaders} that deal with annotated
...
public abstract class AnnotationConfigContextLoaderUtils {
   ...
   *
   * <p>The returned class array will contain all static nested classes of the supplied class that
   * meet the requirements for {@code @Configuration} class implementations as specified in the
   * documentation for {@link Configuration @Configuration}.
   *
   * <p>The implementation of this method adheres to the contract defined in the {@link
   * org.springframework.test.context.SmartContextLoader SmartContextLoader} SPI. Specifically, this
   * method uses introspection to detect default configuration classes that comply with the
   * constraints required of {@code @Configuration} class implementations. If a potential candidate
   * configuration class does not meet these requirements, this method will log a debug message, and
   * the potential candidate class will be ignored.
   *
  ...
  /**
   * Determine if the supplied {@link Class} meets the criteria for being considered a <em>default
   * configuration class</em> candidate.
   *
   * <p>Specifically, such candidates:
   *
   * <ul>
   *   <li>must not be {@code null}
   *   <li>must not be {@code private}
   *   <li>must not be {@code final}
   *   <li>must be {@code static}
   *   <li>must be annotated or meta-annotated with {@code @Configuration}
   * </ul>
   *
  ...
  private static boolean isDefaultConfigurationClassCandidate(@Nullable Class<?> clazz) {
    return (clazz != null
        && isStaticNonPrivateAndNonFinal(clazz)
