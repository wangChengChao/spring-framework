...
/**
 * {@code TestExecutionListener} that provides support for executing SQL {@link Sql#scripts scripts}
 * and inlined {@link Sql#statements statements} configured via the {@link Sql @Sql} annotation.
 *
 * <p>Scripts and inlined statements will be executed {@linkplain #beforeTestMethod(TestContext)
 * before} or {@linkplain #afterTestMethod(TestContext) after} execution of the corresponding
 * {@linkplain java.lang.reflect.Method test method}, depending on the configured value of the
 * {@link Sql#executionPhase executionPhase} flag.
 *
 * <p>Scripts and inlined statements will be executed without a transaction, within an existing
 * Spring-managed transaction, or within an isolated transaction, depending on the configured value
 * of {@link SqlConfig#transactionMode} and the presence of a transaction manager.
 *
 * <h3>Script Resources</h3>
 *
 * <p>For details on default script detection and how script resource locations are interpreted, see
 * {@link Sql#scripts}.
 *
 * <h3>Required Spring Beans</h3>
 *
 * <p>A {@link PlatformTransactionManager} <em>and</em> a {@link DataSource}, just a {@link
 * PlatformTransactionManager}, or just a {@link DataSource} must be defined as beans in the Spring
 * {@link ApplicationContext} for the corresponding test. Consult the javadocs for {@link
 * SqlConfig#transactionMode}, {@link SqlConfig#transactionManager}, {@link SqlConfig#dataSource},
 * {@link TestContextTransactionUtils#retrieveDataSource}, and {@link
 * TestContextTransactionUtils#retrieveTransactionManager} for details on permissible configuration
 * constellations and on the algorithms used to locate these beans.
 *
 * @author Sam Brannen
...
public class SqlScriptsTestExecutionListener extends AbstractTestExecutionListener {
  ...
  /**
   * Execute SQL scripts configured via {@link Sql @Sql} for the supplied {@link TestContext}
   * <em>before</em> the current test method.
   */
  @Override
  ...
  /**
   * Execute SQL scripts configured via {@link Sql @Sql} for the supplied {@link TestContext}
   * <em>after</em> the current test method.
   */
  @Override
  ...
  /**
   * Execute SQL scripts configured via {@link Sql @Sql} for the supplied {@link TestContext} and
   * {@link ExecutionPhase}.
   */
  ...
  /**
   * Determine if method-level {@code @Sql} annotations should be merged with class-level
   * {@code @Sql} annotations.
   */
  ...
  /**
   * Execute the SQL scripts configured via the supplied {@link Sql @Sql} annotation for the given
   * {@link ExecutionPhase} and {@link TestContext}.
   *
   * <p>Special care must be taken in order to properly support the configured {@link
  ...
  private void executeSqlScripts(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          String.format(
              "Processing %s for execution phase [%s] and test context %s.",
    ...
    scripts =
        TestContextResourceUtils.convertToClasspathResourcePaths(
            testContext.getTestClass(), scripts);
    ...
    if (txMgr == null) {
      ...
      if (dataSource != null
          && dataSourceFromTxMgr != null
          && !dataSource.equals(dataSourceFromTxMgr)) {
      ...
      if (dataSource == null) {
        ...
        Assert.state(
            ...
            () ->
                String.format(
                    "Failed to execute SQL scripts for "
  ...
  private String detectDefaultScript(TestContext testContext, boolean classLevel) {
    ...
    if (classPathResource.exists()) {
      if (logger.isInfoEnabled()) {
        logger.info(
            String.format(
                "Detected default SQL script \"%s\" for test %s [%s]",
      ...
      String msg =
          String.format(
              "Could not detect default SQL script for test %s [%s]: "
                  + "%s does not exist. Either declare statements or scripts via @Sql or make the "
                  + "default SQL script available.",
              elementType, elementName, classPathResource);
      logger.error(msg);
