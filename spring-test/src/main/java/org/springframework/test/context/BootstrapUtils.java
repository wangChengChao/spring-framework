...
/**
 * {@code BootstrapUtils} is a collection of utility methods to assist with bootstrapping the
...
abstract class BootstrapUtils {
   ...
   *
   * <p>Uses reflection to create a {@link
   * org.springframework.test.context.support.DefaultBootstrapContext} that uses a {@link
   * org.springframework.test.context.cache.DefaultCacheAwareContextLoaderDelegate}.
   *
  ...
  static BootstrapContext createBootstrapContext(Class<?> testClass) {
    ...
    try {
      clazz =
          (Class<? extends BootstrapContext>)
              ClassUtils.forName(
  ...
  private static CacheAwareContextLoaderDelegate createCacheAwareContextLoaderDelegate() {
    ...
    try {
      clazz =
          (Class<? extends CacheAwareContextLoaderDelegate>)
              ClassUtils.forName(
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            String.format(
                "Instantiating CacheAwareContextLoaderDelegate from class [%s]", clazz.getName()));
      }
      return BeanUtils.instantiateClass(clazz, CacheAwareContextLoaderDelegate.class);
    } catch (Throwable ex) {
      throw new IllegalStateException(
          "Could not load CacheAwareContextLoaderDelegate [" + clazz + "]", ex);
    }
  }

  /**
   * Resolve the {@link TestContextBootstrapper} type for the test class in the supplied {@link
   * BootstrapContext}, instantiate it, and provide it a reference to the {@link BootstrapContext}.
   *
   * <p>If the {@link BootstrapWith @BootstrapWith} annotation is present on the test class, either
   * directly or as a meta-annotation, then its {@link BootstrapWith#value value} will be used as
   * the bootstrapper type. Otherwise, either the {@link
   * org.springframework.test.context.support.DefaultTestContextBootstrapper
   * DefaultTestContextBootstrapper} or the {@link
   * org.springframework.test.context.web.WebTestContextBootstrapper WebTestContextBootstrapper}
   * will be used, depending on the presence of {@link
  ...
  static TestContextBootstrapper resolveTestContextBootstrapper(BootstrapContext bootstrapContext) {
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            String.format(
                "Instantiating TestContextBootstrapper for test class [%s] from class [%s]",
  ...
  private static Class<?> resolveExplicitTestContextBootstrapper(Class<?> testClass) {
    Set<BootstrapWith> annotations =
        AnnotatedElementUtils.findAllMergedAnnotations(testClass, BootstrapWith.class);
  ...
  private static Class<?> resolveDefaultTestContextBootstrapper(Class<?> testClass)
    ...
    AnnotationAttributes attributes =
        AnnotatedElementUtils.findMergedAnnotationAttributes(
            testClass, WEB_APP_CONFIGURATION_ANNOTATION_CLASS_NAME, false, false);
    if (attributes != null) {
