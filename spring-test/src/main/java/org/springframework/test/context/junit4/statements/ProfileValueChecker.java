...
/**
 * {@code ProfileValueChecker} is a custom JUnit {@link Statement} that checks whether a test class
 * or test method is enabled in the current environment via Spring's {@link
 * IfProfileValue @IfProfileValue} annotation.
 *
 * @author Sam Brannen
...
public class ProfileValueChecker extends Statement {
   ...
   * @param testMethod the test method to check; may be {@code null} if this {@code
   *     ProfileValueChecker} is being applied at the class level
   */
  ...
  /**
   * Determine if the test specified by arguments to the {@linkplain #ProfileValueChecker
   * constructor} is <em>enabled</em> in the current environment, as configured via the {@link
   * IfProfileValue &#064;IfProfileValue} annotation.
   *
   * <p>If the test is not annotated with {@code @IfProfileValue} it is considered enabled.
   *
   * <p>If a test is not enabled, this method will abort further evaluation of the execution chain
   * with a failed assumption; otherwise, this method will simply evaluate the next {@link
  ...
  public void evaluate() throws Throwable {
    if (this.testMethod == null) {
      if (!ProfileValueUtils.isTestEnabledInThisEnvironment(this.testClass)) {
        Annotation ann =
            AnnotatedElementUtils.findMergedAnnotation(this.testClass, IfProfileValue.class);
