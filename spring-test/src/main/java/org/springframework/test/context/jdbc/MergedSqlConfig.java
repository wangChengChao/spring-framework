...
/**
 * {@code MergedSqlConfig} encapsulates the <em>merged</em> {@link SqlConfig @SqlConfig} attributes
 * declared locally via {@link Sql#config} and globally as a class-level annotation.
 *
...
class MergedSqlConfig {
  ...
  /**
   * Construct a {@code MergedSqlConfig} instance by merging the configuration from the supplied
   * local (potentially method-level) {@code @SqlConfig} annotation with class-level configuration
   * discovered on the supplied {@code testClass}.
   *
   * <p>Local configuration overrides class-level configuration.
   *
   * <p>If the test class is not annotated with {@code @SqlConfig}, no merging takes place and the
   * local configuration is used "as is".
   */
  MergedSqlConfig(SqlConfig localSqlConfig, Class<?> testClass) {
    ...
    this.transactionMode =
        getEnum(
            mergedAttributes, "transactionMode", TransactionMode.DEFAULT, TransactionMode.INFERRED);
  ...
  private static <E extends Enum<?>> E getEnum(
  ...
  private static String getString(
      AnnotationAttributes attributes, String attributeName, String defaultValue) {
  ...
  private static void enforceCommentPrefixAliases(AnnotationAttributes attributes) {
    ...
    Assert.isTrue(
        !(explicitCommentPrefix && explicitCommentPrefixes),
  ...
  private static String[] getCommentPrefixes(AnnotationAttributes attributes) {
    ...
    Assert.state(
        Arrays.equals(commentPrefix, commentPrefixes),
