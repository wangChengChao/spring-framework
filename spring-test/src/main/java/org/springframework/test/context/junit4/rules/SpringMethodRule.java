...
/**
 * {@code SpringMethodRule} is a custom JUnit 4 {@link MethodRule} that supports instance-level and
 * method-level features of the <em>Spring TestContext Framework</em> in standard JUnit tests by
 * means of the {@link TestContextManager} and associated support classes and annotations.
 *
 * <p>In contrast to the {@link org.springframework.test.context.junit4.SpringJUnit4ClassRunner
 * SpringJUnit4ClassRunner}, Spring's rule-based JUnit support has the advantage that it is
 * independent of any {@link org.junit.runner.Runner Runner} and can therefore be combined with
 * existing alternative runners like JUnit's {@code Parameterized} or third-party runners such as
 * the {@code MockitoJUnitRunner}.
 *
 * <p>In order to achieve the same functionality as the {@code SpringJUnit4ClassRunner}, however, a
 * {@code SpringMethodRule} must be combined with a {@link SpringClassRule}, since {@code
 * SpringMethodRule} only supports the instance-level and method-level features of the {@code
 ...
 * {@code SpringMethodRule}. <em>(Note that additional annotations may be supported by various
 * {@link org.springframework.test.context.TestExecutionListener TestExecutionListener} or {@link
 * org.springframework.test.context.TestContextBootstrapper TestContextBootstrapper}
 * implementations.)</em>
 *
 * <ul>
 ...
 *
 * <p><strong>WARNING:</strong> Due to the shortcomings of JUnit rules, the {@code SpringMethodRule}
 * does <strong>not</strong> support the {@code beforeTestExecution()} and {@code
 * afterTestExecution()} callbacks of the {@link
 * org.springframework.test.context.TestExecutionListener TestExecutionListener} API.
 *
 * @author Sam Brannen
...
public class SpringMethodRule implements MethodRule {
  ...
  /**
   * Apply <em>instance-level</em> and <em>method-level</em> features of the <em>Spring TestContext
   * Framework</em> to the supplied {@code base} statement.
   *
   * <p>Specifically, this method invokes the {@link TestContextManager#prepareTestInstance
   * prepareTestInstance()}, {@link TestContextManager#beforeTestMethod beforeTestMethod()}, and
   * {@link TestContextManager#afterTestMethod afterTestMethod()} methods on the {@code
   * TestContextManager}, potentially with Spring timeouts and repetitions.
   *
   * <p>In addition, this method checks whether the test is enabled in the current execution
   * environment. This prevents methods with a non-matching {@code @IfProfileValue} annotation from
   * running altogether, even skipping the execution of {@code prepareTestInstance()} methods in
   ...
   * @return a statement that wraps the supplied {@code base} with instance-level and method-level
  ...
  public Statement apply(Statement base, FrameworkMethod frameworkMethod, Object testInstance) {
    ...
    statement =
        withBeforeTestMethodCallbacks(statement, testMethod, testInstance, testContextManager);
    statement =
        withAfterTestMethodCallbacks(statement, testMethod, testInstance, testContextManager);
   ...
   */
  private Statement withBeforeTestMethodCallbacks(
      Statement next,
      Method testMethod,
      Object testInstance,
   ...
   */
  private Statement withAfterTestMethodCallbacks(
      Statement next,
      Method testMethod,
      Object testInstance,
      TestContextManager testContextManager) {
   ...
   *
   * <p>Supports Spring's {@link org.springframework.test.annotation.Timed @Timed} annotation.
   *
