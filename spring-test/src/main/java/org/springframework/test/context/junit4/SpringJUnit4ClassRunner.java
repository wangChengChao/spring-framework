...
/**
 * {@code SpringJUnit4ClassRunner} is a custom extension of JUnit's {@link BlockJUnit4ClassRunner}
 * which provides functionality of the <em>Spring TestContext Framework</em> to standard JUnit tests
 * by means of the {@link TestContextManager} and associated support classes and annotations.
 *
 ...
 *
 * <p>The following list constitutes all annotations currently supported directly or indirectly by
 * {@code SpringJUnit4ClassRunner}. <em>(Note that additional annotations may be supported by
 * various {@link org.springframework.test.context.TestExecutionListener TestExecutionListener} or
 * {@link org.springframework.test.context.TestContextBootstrapper TestContextBootstrapper}
 * implementations.)</em>
 *
 * <ul>
 *   <li>{@link Test#expected() @Test(expected=...)}
 *   <li>{@link Test#timeout() @Test(timeout=...)}
 *   <li>{@link org.springframework.test.annotation.Timed @Timed}
 *   <li>{@link org.springframework.test.annotation.Repeat @Repeat}
 *   <li>{@link Ignore @Ignore}
 *   <li>{@link
 *       org.springframework.test.annotation.ProfileValueSourceConfiguration @ProfileValueSourceConfiguration}
 *   <li>{@link org.springframework.test.annotation.IfProfileValue @IfProfileValue}
 * </ul>
 *
 * <p>If you would like to use the Spring TestContext Framework with a runner other than this one,
 * use {@link SpringClassRule} and {@link SpringMethodRule}.
 *
...
public class SpringJUnit4ClassRunner extends BlockJUnit4ClassRunner {
  ...
  static {
    Assert.state(
        ClassUtils.isPresent(
            "org.junit.internal.Throwables", SpringJUnit4ClassRunner.class.getClassLoader()),
  ...
  private static void ensureSpringRulesAreNotPresent(Class<?> testClass) {
    for (Field field : testClass.getFields()) {
      Assert.state(
          !SpringClassRule.class.isAssignableFrom(field.getType()),
          () ->
              String.format(
   ...
   * {@code @IfProfileValue} at the class-level, and otherwise delegate to the parent
   * implementation.
   *
   ...
   *
   * <p>This prevents classes with a non-matching {@code @IfProfileValue} annotation from running
   * altogether, even skipping the execution of {@code prepareTestInstance()} methods in {@code
  ...
  /**
   * Wrap the {@link Statement} returned by the parent implementation with a {@code
   * RunBeforeTestClassCallbacks} statement, thus preserving the default JUnit functionality while
   * adding support for the Spring TestContext Framework.
   *
  ...
  /**
   * Wrap the {@link Statement} returned by the parent implementation with a {@code
   * RunAfterTestClassCallbacks} statement, thus preserving the default JUnit functionality while
   * adding support for the Spring TestContext Framework.
   *
  ...
  /**
   * Perform the same logic as {@link BlockJUnit4ClassRunner#runChild(FrameworkMethod,
   * RunNotifier)}, except that tests are determined to be <em>ignored</em> by {@link
  ...
  /**
   * Augment the default JUnit behavior {@linkplain #withPotentialRepeat with potential repeats} of
   * the entire execution chain.
   *
   * <p>Furthermore, support for timeouts has been moved down the execution chain in order to
   * include execution of {@link org.junit.Before @Before} and {@link org.junit.After @After}
   * methods within the timed execution. Note that this differs from the default JUnit behavior of
   * executing {@code @Before} and {@code @After} methods in the main thread while executing the
   * actual test method in a separate thread. Thus, the net effect is that {@code @Before} and
   * {@code @After} methods will be executed in the same thread as the test method. As a
   * consequence, JUnit-specified timeouts will work fine in combination with Spring transactions.
   * However, JUnit-specific timeouts still differ from Spring-specific timeouts in that the former
   * execute in a separate thread while the latter simply execute in the main thread (like regular
   * tests).
   *
  ...
  private Statement withRulesReflectively(
      FrameworkMethod frameworkMethod, Object testInstance, Statement statement) {
    Object result =
        ReflectionUtils.invokeMethod(
            withRulesMethod, this, frameworkMethod, testInstance, statement);
    Assert.state(result instanceof Statement, "withRules mismatch");
    return (Statement) result;
  }

  /**
   * Return {@code true} if {@link Ignore @Ignore} is present for the supplied {@linkplain
   * FrameworkMethod test method} or if the test method is disabled via {@code @IfProfileValue}.
   *
  ...
  protected boolean isTestMethodIgnored(FrameworkMethod frameworkMethod) {
    ...
    return (method.isAnnotationPresent(Ignore.class)
        || !ProfileValueUtils.isTestEnabledInThisEnvironment(
            method, getTestClass().getJavaClass()));
  }

  /**
   * Perform the same logic as {@link
   * BlockJUnit4ClassRunner#possiblyExpectingExceptions(FrameworkMethod, Object, Statement)} except
   * that the <em>expected exception</em> is retrieved using {@link
  ...
  /**
   * Get the {@code exception} that the supplied {@linkplain FrameworkMethod test method} is
   * expected to throw.
   *
  ...
  /**
   * Perform the same logic as {@link BlockJUnit4ClassRunner#withPotentialTimeout(FrameworkMethod,
   * Object, Statement)} but with additional support for Spring's {@code @Timed} annotation.
   *
   * <p>Supports both Spring's {@link org.springframework.test.annotation.Timed @Timed} and JUnit's
   * {@link Test#timeout() @Test(timeout=...)} annotations, but not both simultaneously.
   *
   * @return either a {@link SpringFailOnTimeout}, a {@link FailOnTimeout}, or the supplied {@link
  ...
  protected Statement withPotentialTimeout(
      FrameworkMethod frameworkMethod, Object testInstance, Statement next) {
    ...
    if (springTimeout > 0 && junitTimeout > 0) {
      ...
      statement =
          FailOnTimeout.builder().withTimeout(junitTimeout, TimeUnit.MILLISECONDS).build(next);
    } else {
      statement = next;
    }
  ...
  /**
   * Retrieve the configured JUnit {@code timeout} from the {@link Test @Test} annotation on the
   * supplied {@linkplain FrameworkMethod test method}.
   *
  ...
  /**
   * Retrieve the configured Spring-specific {@code timeout} from the {@link
   * org.springframework.test.annotation.Timed @Timed} annotation on the supplied {@linkplain
   ...
   * thus preserving the default functionality while adding support for the Spring TestContext
   * Framework.
   *
   * @see RunBeforeTestExecutionCallbacks
   */
  protected Statement withBeforeTestExecutionCallbacks(
      FrameworkMethod frameworkMethod, Object testInstance, Statement statement) {
    return new RunBeforeTestExecutionCallbacks(
        statement, testInstance, frameworkMethod.getMethod(), getTestContextManager());
  }

  /**
   * Wrap the supplied {@link Statement} with a {@code RunAfterTestExecutionCallbacks} statement,
   * thus preserving the default functionality while adding support for the Spring TestContext
   * Framework.
   *
   * @see RunAfterTestExecutionCallbacks
   */
  protected Statement withAfterTestExecutionCallbacks(
      FrameworkMethod frameworkMethod, Object testInstance, Statement statement) {
  ...
  /**
   * Wrap the {@link Statement} returned by the parent implementation with a {@code
   * RunBeforeTestMethodCallbacks} statement, thus preserving the default functionality while adding
   * support for the Spring TestContext Framework.
   *
  ...
  /**
   * Wrap the {@link Statement} returned by the parent implementation with a {@code
   * RunAfterTestMethodCallbacks} statement, thus preserving the default functionality while adding
   * support for the Spring TestContext Framework.
   *
  ...
  protected Statement withAfters(
    ...
    return new RunAfterTestMethodCallbacks(
        junitAfters, testInstance, frameworkMethod.getMethod(), getTestContextManager());
  }

  /**
   * Wrap the supplied {@link Statement} with a {@code SpringRepeat} statement.
   *
   * <p>Supports Spring's {@link org.springframework.test.annotation.Repeat @Repeat} annotation.
   *
   * @see TestAnnotationUtils#getRepeatCount(Method)
   * @see SpringRepeat
   */
  protected Statement withPotentialRepeat(
      FrameworkMethod frameworkMethod, Object testInstance, Statement next) {
