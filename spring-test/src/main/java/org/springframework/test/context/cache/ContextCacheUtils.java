...
public abstract class ContextCacheUtils {
   ...
   *
   * <p>Uses {@link SpringProperties} to retrieve a system property or Spring property named {@code
   * spring.test.context.cache.maxSize}.
   *
   * <p>Falls back to the value of the {@link ContextCache#DEFAULT_MAX_CONTEXT_CACHE_SIZE} if no
   * such property has been set or if the property is not an integer.
   *
  ...
  public static int retrieveMaxCacheSize() {
    try {
      String maxSize =
          SpringProperties.getProperty(ContextCache.MAX_CONTEXT_CACHE_SIZE_PROPERTY_NAME);
