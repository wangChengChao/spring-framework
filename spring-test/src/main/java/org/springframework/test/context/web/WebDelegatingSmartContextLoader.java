...
/**
 * {@code WebDelegatingSmartContextLoader} is a concrete implementation of {@link
 * AbstractDelegatingSmartContextLoader} that delegates to a {@link GenericXmlWebContextLoader} (or
 * a {@link GenericGroovyXmlWebContextLoader} if Groovy is present on the classpath) and an {@link
...
public class WebDelegatingSmartContextLoader extends AbstractDelegatingSmartContextLoader {
  ...
  private static final String GROOVY_XML_WEB_CONTEXT_LOADER_CLASS_NAME =
      "org.springframework.test.context.web.GenericGroovyXmlWebContextLoader";

  private static final boolean groovyPresent =
      ClassUtils.isPresent(
              "groovy.lang.Closure", WebDelegatingSmartContextLoader.class.getClassLoader())
          && ClassUtils.isPresent(
    ...
    if (groovyPresent) {
      try {
        ...
        throw new IllegalStateException(
            "Failed to enable support for Groovy scripts; "
