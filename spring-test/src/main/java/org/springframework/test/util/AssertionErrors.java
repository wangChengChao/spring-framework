...
public abstract class AssertionErrors {
  ...
  /**
   * Fail a test with the given message passing along expected and actual values to be appended to
   * the message.
   *
   * <p>For example given:
   *
  ...
  /**
   * Assert the given condition is {@code true} and raise an {@link AssertionError} otherwise.
   *
   * @param message a message that describes the reason for the failure
  ...
  /**
   * Assert that the given object is not {@code null} and raise an {@link AssertionError} otherwise.
   *
   * @param message a message that describes the reason for the failure
  ...
  public static void assertNotEquals(
    ...
    if (ObjectUtils.nullSafeEquals(expected, actual)) {
      throw new AssertionError(
          message + " was not expected to be:" + "<" + ObjectUtils.nullSafeToString(actual) + ">");
    }
  }
}
