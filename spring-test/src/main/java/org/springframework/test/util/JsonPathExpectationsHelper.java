 ...
 *
 * <p>Based on the <a href="https://github.com/jayway/JsonPath">JsonPath</a> project: requiring
 * version 0.9+, with 1.1+ strongly recommended.
 *
...
public class JsonPathExpectationsHelper {
   ...
   * @param args arguments to parameterize the {@code JsonPath} expression with, using formatting
  ...
  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert the resulting
   * value with the given {@code Matcher}.
   *
  ...
  /**
   * An overloaded variant of {@link #assertValue(String, Matcher)} that also accepts a target type
   * for the resulting value. This can be useful for matching numbers reliably for example coercing
  ...
  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that the
   * result is equal to the expected value.
   *
   * @param content the JSON content
  ...
  public void assertValue(String content, @Nullable Object expectedValue) {
    ...
    AssertionErrors.assertEquals(
        "JSON path \"" + this.expression + "\"", expectedValue, actualValue);
  }

  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that the
   * resulting value is a {@link String}.
   *
   * @param content the JSON content
   * @since 4.2.1
   */
  public void assertValueIsString(String content) {
    ...
    MatcherAssert.assertThat(
        failureReason("a string", value), value, CoreMatchers.instanceOf(String.class));
  }

  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that the
   * resulting value is a {@link Boolean}.
   *
   * @param content the JSON content
   * @since 4.2.1
   */
  public void assertValueIsBoolean(String content) {
    Object value = assertExistsAndReturn(content);
    MatcherAssert.assertThat(
        failureReason("a boolean", value), value, CoreMatchers.instanceOf(Boolean.class));
  }

  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that the
   * resulting value is a {@link Number}.
   *
   * @param content the JSON content
   * @since 4.2.1
   */
  public void assertValueIsNumber(String content) {
    Object value = assertExistsAndReturn(content);
    MatcherAssert.assertThat(
        failureReason("a number", value), value, CoreMatchers.instanceOf(Number.class));
  }
  ...
  public void assertValueIsArray(String content) {
    Object value = assertExistsAndReturn(content);
    MatcherAssert.assertThat(
        failureReason("an array", value), value, CoreMatchers.instanceOf(List.class));
  }

  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that the
   * resulting value is a {@link Map}.
   *
   * @param content the JSON content
   * @since 4.2.1
   */
  public void assertValueIsMap(String content) {
    Object value = assertExistsAndReturn(content);
    MatcherAssert.assertThat(
        failureReason("a map", value), value, CoreMatchers.instanceOf(Map.class));
  }

  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that a
   * non-null value, possibly an empty array or map, exists at the given path.
   *
   * <p>Note that if the JSON path expression is not {@linkplain JsonPath#isDefinite() definite},
   * this method asserts that the list of values at the given path is not <em>empty</em>.
   *
   * @param content the JSON content
   */
  ...
  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that a
   * non-null value does not exist at the given path.
   *
   * <p>Note that if the JSON path expression is not {@linkplain JsonPath#isDefinite() definite},
   * this method asserts that the list of values at the given path is <em>empty</em>.
   *
   * @param content the JSON content
   */
  ...
  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that an empty
   * value exists at the given path.
   *
   * <p>For the semantics of <em>empty</em>, consult the Javadoc for {@link
  ...
  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that a
   * non-empty value exists at the given path.
   *
   * <p>For the semantics of <em>empty</em>, consult the Javadoc for {@link
   * ObjectUtils#isEmpty(Object)}.
   *
  ...
  public void assertValueIsNotEmpty(String content) {
    ...
    AssertionErrors.assertTrue(
        failureReason("a non-empty value", value), !ObjectUtils.isEmpty(value));
  }

  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that a value,
   * possibly {@code null}, exists.
   *
   * <p>If the JSON path expression is not {@linkplain JsonPath#isDefinite() definite}, this method
   * asserts that the list of values at the given path is not <em>empty</em>.
   *
   * @param content the JSON content
   * @since 5.0.3
   */
  ...
  /**
   * Evaluate the JSON path expression against the supplied {@code content} and assert that a value,
   * including {@code null} values, does not exist at the given path.
   *
   * <p>If the JSON path expression is not {@linkplain JsonPath#isDefinite() definite}, this method
   * asserts that the list of values at the given path is <em>empty</em>.
   *
   * @param content the JSON content
   * @since 5.0.3
   */
  ...
  private String failureReason(String expectedDescription, @Nullable Object value) {
    return String.format(
        "Expected %s at JSON path \"%s\" but found: %s",
        expectedDescription,
        this.expression,
  ...
  /**
   * Variant of {@link #evaluateJsonPath(String)} with a target type. This can be useful for
