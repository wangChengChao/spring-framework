...
public class XpathExpectationsHelper {
   ...
   * @param args arguments to parameterize the XPath expression with using the formatting specifiers
   *     defined in {@link String#format(String, Object...)}
   * @throws XPathExpressionException if expression compilation failed
   */
  public XpathExpectationsHelper(
  ...
  private static XPathExpression compileXpathExpression(
      String expression, @Nullable Map<String, String> namespaces) throws XPathExpressionException {
  ...
  /**
   * Parse the content, evaluate the XPath expression as a {@link Node}, and assert it with the
   * given {@code Matcher<Node>}.
   */
  public void assertNode(
      byte[] content, @Nullable String encoding, final Matcher<? super Node> matcher)
      throws Exception {
   ...
   * @throws Exception if content parsing or expression evaluation fails
   */
  public void assertNodeCount(byte[] content, @Nullable String encoding, int expectedCount)
      throws Exception {
    NodeList nodeList = evaluateXpath(content, encoding, NodeList.class);
    AssertionErrors.assertEquals(
        "nodeCount for XPath " + this.expression,
        expectedCount,
  ...
  public void assertString(
      byte[] content, @Nullable String encoding, Matcher<? super String> matcher) throws Exception {
   ...
   * @throws Exception if content parsing or expression evaluation fails
   */
  public void assertString(byte[] content, @Nullable String encoding, String expectedValue)
      throws Exception {
    String actual = evaluateXpath(content, encoding, String.class);
  ...
  public void assertNumber(
      byte[] content, @Nullable String encoding, Matcher<? super Double> matcher) throws Exception {
   ...
   * @throws Exception if content parsing or expression evaluation fails
   */
  public void assertNumber(byte[] content, @Nullable String encoding, Double expectedValue)
      throws Exception {
    Double actual = evaluateXpath(content, encoding, Double.class);
    AssertionErrors.assertEquals("XPath " + this.expression, expectedValue, actual);
  }
   ...
   * @throws Exception if content parsing or expression evaluation fails
   */
  public void assertBoolean(byte[] content, @Nullable String encoding, boolean expectedValue)
      throws Exception {
    String actual = evaluateXpath(content, encoding, String.class);
    AssertionErrors.assertEquals(
        "XPath " + this.expression, expectedValue, Boolean.parseBoolean(actual));
  }
   ...
   * @param targetClass the target class, one of Number, String, Boolean, org.w3c.Node, or NodeList
   * @throws Exception if content parsing or expression evaluation fails
   * @since 5.1
   */
  @Nullable
  public <T> T evaluateXpath(byte[] content, @Nullable String encoding, Class<T> targetClass)
      throws Exception {
