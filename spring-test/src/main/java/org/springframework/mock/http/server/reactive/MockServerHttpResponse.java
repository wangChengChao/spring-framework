 ...
 *
 * <p>By default response content is consumed in full upon writing and cached for subsequent access,
 * however it is also possible to set a custom {@link #setWriteHandler(Function) writeHandler}.
 *
...
public class MockServerHttpResponse extends AbstractServerHttpResponse {
  ...
  public MockServerHttpResponse(DataBufferFactory dataBufferFactory) {
    ...
    this.writeHandler =
        body -> {
          ...
          this.body =
              body.doOnComplete(completion::onComplete).doOnError(completion::onError).cache();
          this.body.subscribe();
          return completion;
        };
  }
   ...
   *
   * <p>By default, response body content is consumed in full and cached for subsequent access in
   * tests. Use this option to take control over how the response body is consumed.
   *
  ...
  /**
   * Aggregate response data and convert to a String using the "Content-Type" charset or "UTF-8" by
   * default.
   */
