...
/**
 * Mock extension of {@link AbstractServerHttpRequest} for use in tests without an actual server.
 * Use the static methods to obtain a builder.
 *
...
public final class MockServerHttpRequest extends AbstractServerHttpRequest {
  ...
  @Nullable private final SslInfo sslInfo;
  ...
  /**
   * Alternative to {@link #method(HttpMethod, URI)} that accepts a URI template. The given URI may
   * contain query parameters, or those may be added later via {@link BaseBuilder#queryParam
   * queryParam} builder methods.
   *
   * @param method the HTTP method (GET, POST, etc)
  ...
  /**
   * Create an HTTP GET builder with the given URI template. The given URI may contain query
   * parameters, or those may be added later via {@link BaseBuilder#queryParam queryParam} builder
   * methods.
   *
  ...
  /**
   * HTTP PUT variant. See {@link #get(String, Object...)} for general info. {@link
   * BaseBuilder#queryParam queryParam} builder methods.
   *
   * @param urlTemplate a URL template; the resulting URL will be encoded
   * @param uriVars zero or more URI variables
   * @return the created builder
  ...
  public interface BaseBuilder<B extends BaseBuilder<B>> {
    ...
    /**
     * Append the given query parameter to the existing query parameters. If no values are given,
     * the resulting URI will contain the query parameter name only (i.e. {@code ?foo} instead of
     * {@code ?foo=bar}).
     *
    ...
    /**
     * Add the given query parameters and values. The provided query name and corresponding values
     * will be encoded.
     *
    ...
    /**
     * Set the list of acceptable {@linkplain Locale locales}, as specified by the {@code
  ...
  public interface BodyBuilder extends BaseBuilder<BodyBuilder> {
     ...
     * Set the body of the request and build it.
     *
     * <p>The String is assumed to be UTF-8 encoded unless the request has a "content-type" header
