...
/**
 * Mock implementation of the {@link javax.servlet.jsp.PageContext} interface. Only necessary for
 * testing applications when testing custom JSP tags.
 *
 * <p>Note: Expects initialization via the constructor rather than via the {@code
 * PageContext.initialize} method. Does not support writing to a JspWriter, request dispatching, or
 * {@code handlePageException} calls.
 *
...
public class MockPageContext extends PageContext {
  ...
  @Nullable private JspWriter out;

  /**
   * Create new MockPageContext with a default {@link MockServletContext}, {@link
   * MockHttpServletRequest}, {@link MockHttpServletResponse}, {@link MockServletConfig}.
   */
  ...
  /**
   * Create new MockPageContext with a default {@link MockHttpServletRequest}, {@link
   * MockHttpServletResponse}, {@link MockServletConfig}.
   *
   * @param servletContext the ServletContext that the JSP page runs in (only necessary when
   *     actually accessing the ServletContext)
   */
   ...
   * @param request the current HttpServletRequest (only necessary when actually accessing the
   *     request)
   */
  public MockPageContext(
      @Nullable ServletContext servletContext, @Nullable HttpServletRequest request) {
   ...
   * @param response the current HttpServletResponse (only necessary when actually writing to the
   *     response)
   */
  public MockPageContext(
  ...
  public MockPageContext(
    ...
    this.servletConfig =
        (servletConfig != null ? servletConfig : new MockServletConfig(servletContext));
  }
  ...
  public byte[] getContentAsByteArray() {
    Assert.state(
        this.response instanceof MockHttpServletResponse, "MockHttpServletResponse required");
