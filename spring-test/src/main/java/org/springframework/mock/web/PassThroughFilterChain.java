...
/**
 * Implementation of the {@link javax.servlet.FilterConfig} interface which simply passes the call
 * through to a given Filter/FilterChain combination (indicating the next Filter in the chain along
 * with the FilterChain that it is supposed to work on) or to a given Servlet (indicating the end of
 * the chain).
 *
...
public class PassThroughFilterChain implements FilterChain {
  ...
  @Nullable private FilterChain nextFilterChain;

  @Nullable private Servlet servlet;

  /**
   * Create a new PassThroughFilterChain that delegates to the given Filter, calling it with the
   * given FilterChain.
   *
