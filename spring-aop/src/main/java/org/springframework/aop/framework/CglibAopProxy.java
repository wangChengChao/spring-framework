 ...
 *
 * <p>Objects of this type should be obtained through proxy factories, configured by an {@link
 * AdvisedSupport} object. This class is internal to Spring's AOP framework and need not be used
 * directly by client code.
 *
 * <p>{@link DefaultAopProxyFactory} will automatically create CGLIB-based proxies if necessary, for
 * example in case of proxying a target class (see the {@link DefaultAopProxyFactory attendant
 * javadoc} for details).
 *
 * <p>Proxies created using this class are thread-safe if the underlying (target) class is
 * thread-safe.
 *
 * @author Rod Johnson
 * @author Rob Harrop
...
class CglibAopProxy implements AopProxy, Serializable {
  ...
  public void setConstructorArguments(
      @Nullable Object[] constructorArgs, @Nullable Class<?>[] constructorArgTypes) {
   ...
   * not.
   */
  private void validateClassIfNecessary(
      Class<?> proxySuperClass, @Nullable ClassLoader proxyClassLoader) {
  ...
  private void doValidateClass(
      Class<?> proxySuperClass, @Nullable ClassLoader proxyClassLoader, Set<Class<?>> ifcs) {
  ...
  private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
    ...
    if (isStatic && isFrozen) {
      ...
      for (int x = 0; x < methods.length; x++) {
        ...
        List<Object> chain =
            this.advised.getInterceptorsAndDynamicInterceptionAdvice(method, rootClass);
        fixedCallbacks[x] =
            new FixedChainStaticTargetInterceptor(
  ...
  private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
    ...
    public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy)
      ...
      try {
        ...
        // Get as late as possible to minimize the time we "own" the target, in case it comes from a
        // pool...
  ...
  private static class CglibMethodInvocation extends ReflectiveMethodInvocation {
    ...
    @Nullable private final MethodProxy methodProxy;
  ...
  private static class ProxyCallbackFilter implements CallbackFilter {
    ...
    public ProxyCallbackFilter(
        ...
        Map<Method, Integer> fixedInterceptorMap,
     ...
     *       chain. Otherwise a DynamicAdvisedInterceptor is used.
    ...
    public int accept(Method method) {
      ...
      if (haveAdvice || !isFrozen) {
        ...
        if (targetClass != null && returnType.isAssignableFrom(targetClass)) {
          if (logger.isTraceEnabled()) {
            logger.trace(
                "Method return type is assignable from target type and "
                    + "may therefore return 'this' - using INVOKE_TARGET: "
                    + method);
          }
          return INVOKE_TARGET;
        } else {
          if (logger.isTraceEnabled()) {
            logger.trace(
                "Method return type ensures 'this' cannot be returned - "
      ...
      return (!(a instanceof PointcutAdvisor)
          || (b instanceof PointcutAdvisor
              && ObjectUtils.nullSafeEquals(
                  ((PointcutAdvisor) a).getPointcut(), ((PointcutAdvisor) b).getPointcut())));
    }
