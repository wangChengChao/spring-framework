...
/**
 * Convenient superclass for {@link org.springframework.aop.framework.autoproxy.TargetSourceCreator}
 * implementations that require creating multiple instances of a prototype bean.
 *
 * <p>Uses an internal BeanFactory to manage the target instances, copying the original bean
 * definition to this internal factory. This is necessary because the original BeanFactory will just
 * contain the proxy instance created through auto-proxying.
 *
 * <p>Requires running in an {@link org.springframework.beans.factory.support.AbstractBeanFactory}.
 *
 * @author Rod Johnson
...
public abstract class AbstractBeanFactoryBasedTargetSourceCreator
  ...
  public final void setBeanFactory(BeanFactory beanFactory) {
    if (!(beanFactory instanceof ConfigurableBeanFactory)) {
      throw new IllegalStateException(
          "Cannot do auto-TargetSource creation with a BeanFactory "
              + "that doesn't implement ConfigurableBeanFactory: "
  ...
  protected DefaultListableBeanFactory buildInternalBeanFactory(
      ConfigurableBeanFactory containingFactory) {
  ...
  /**
   * Return whether this TargetSourceCreator is prototype-based. The scope of the target bean
   * definition will be set accordingly.
   *
   * <p>Default is "true".
   *
  ...
  /**
   * Subclasses must implement this method to return a new AbstractPrototypeBasedTargetSource if
   * they wish to create a custom TargetSource for this bean, or {@code null} if they are not
   * interested it in, in which case no special target source will be created. Subclasses should not
   * call {@code setTargetBeanName} or {@code setBeanFactory} on the
   * AbstractPrototypeBasedTargetSource: This class' implementation of {@code getTargetSource()}
   * will do that.
   *
