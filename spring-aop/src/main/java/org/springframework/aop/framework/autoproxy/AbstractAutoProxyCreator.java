 ...
 * itself.
 *
 * <p>This class distinguishes between "common" interceptors: shared for all proxies it creates, and
 * "specific" interceptors: unique per bean instance. There need not be any common interceptors. If
 * there are, they are set using the interceptorNames property. As with {@link
 ...
 *
 * <p>Such auto-proxying is particularly useful if there's a large number of beans that need to be
 * wrapped with similar proxies, i.e. delegating to the same interceptors. Instead of x repetitive
 * proxy definitions for x target beans, you can register one single such post processor with the
 * bean factory to achieve the same effect.
 *
 * <p>Subclasses can apply any strategy to decide if a bean is to be proxied, e.g. by type, by name,
 * by definition details, etc. They can also return additional interceptors that should just be
 * applied to the specific bean instance. A simple concrete implementation is {@link
 * BeanNameAutoProxyCreator}, identifying the beans to be proxied via given names.
 *
 * <p>Any number of {@link TargetSourceCreator} implementations can be used to create a custom
 * target source: for example, to pool prototype objects. Auto-proxying will occur even if there is
 * no advice, as long as a TargetSourceCreator specifies a custom {@link
...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
   ...
   * @see #getAdvicesAndAdvisorsForBean
   */
  @Nullable protected static final Object[] DO_NOT_PROXY = null;
  ...
  @Nullable private TargetSourceCreator[] customTargetSourceCreators;
   ...
   * is created.
   *
   * <p>Overridden from the super class to prevent the proxy configuration from being frozen before
   ...
   *
   * <p>Note that TargetSourceCreators will kick in even for target beans where no advices or
   * advisors have been found. If a {@code TargetSourceCreator} returns a {@link TargetSource} for a
   * specific bean, that bean will be proxied in any case.
   *
   * <p>{@code TargetSourceCreators} can only be invoked if this post processor is used in a {@link
   * BeanFactory} and its {@link BeanFactoryAware} callback is triggered.
   *
   * @param targetSourceCreators the list of {@code TargetSourceCreators}. Ordering is significant:
   *     The {@code TargetSource} returned from the first matching {@code TargetSourceCreator} (that
   *     is, the first that returns non-null) will be used.
   */
   ...
   *
   * <p>If this property isn't set, there will be zero common interceptors. This is perfectly valid,
   ...
   *
   * <p>Note: As of 4.2.3, this implementation does not return a concatenated class/name String
   * anymore but rather the most efficient cache key possible: a plain bean name, prepended with
   * {@link BeanFactory#FACTORY_BEAN_PREFIX} in case of a {@code FactoryBean}; or if no bean name
   * specified, then the given bean {@code Class} as-is.
   *
   ...
   * proxied.
   *
   * <p>The default implementation considers Advices, Advisors and AopInfrastructureBeans as
   ...
   *
   * <p>Sometimes we need to be able to avoid this happening, e.g. if it will lead to a circular
   * reference or if the existing target instance needs to be preserved. This implementation returns
   * {@code false} unless the bean name indicates an "original instance" according to {@code
   * AutowireCapableBeanFactory} conventions.
   *
   * @param beanClass the class of the bean
   * @param beanName the name of the bean
   * @return whether to skip the given bean
   * @see
   *     org.springframework.beans.factory.config.AutowireCapableBeanFactory#ORIGINAL_INSTANCE_SUFFIX
   */
   ...
   *
   * <p>This implementation uses the "customTargetSourceCreators" property. Subclasses can override
   * this method to use a different mechanism.
   *
   ...
   * attribute} of the corresponding bean definition.
   *
   * @param beanClass the class of the bean
   * @param beanName the name of the bean
   ...
   *
   * <p>Default is {@code false}. Subclasses may override this if they will always return
   * pre-filtered Advisors.
   *
   ...
   */
  protected Advisor[] buildAdvisors(
      @Nullable String beanName, @Nullable Object[] specificInterceptors) {
    ...
    if (logger.isTraceEnabled()) {
      ...
      int nrOfSpecificInterceptors =
          (specificInterceptors != null ? specificInterceptors.length : 0);
      logger.trace(
   ...
   *
   * @param proxyFactory a ProxyFactory that is already configured with TargetSource and interfaces
   ...
   *     additional interceptors but just the common ones; or {@code null} if no proxy at all, not
   *     even with the common interceptors. See constants DO_NOT_PROXY and
  ...
  protected abstract Object[] getAdvicesAndAdvisorsForBean(
      Class<?> beanClass, String beanName, @Nullable TargetSource customTargetSource)
