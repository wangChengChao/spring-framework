...
public class ProxyProcessorSupport extends ProxyConfig
  ...
  /**
   * This should run after all other processors, so that it can just add an advisor to existing
   * proxies rather than double-proxy.
   */
  private int order = Ordered.LOWEST_PRECEDENCE;

  @Nullable private ClassLoader proxyClassLoader = ClassUtils.getDefaultClassLoader();

  private boolean classLoaderConfigured = false;

  /**
   * Set the ordering which will apply to this processor's implementation of {@link Ordered}, used
   * when applying multiple processors.
   *
   ...
   *
   * <p>Default is the bean ClassLoader, i.e. the ClassLoader used by the containing {@link
   * org.springframework.beans.factory.BeanFactory} for loading all bean classes. This can be
   ...
   * appropriate.
   *
   * <p>Calls {@link #isConfigurationCallbackInterface} and {@link #isInternalLanguageInterface} to
   * filter for reasonable proxy interfaces, falling back to a target-class proxy otherwise.
   *
   ...
   * full target class, assuming that as the user's intention.
   *
  ...
  /**
   * Determine whether the given interface is a well-known internal language interface and therefore
