...
/**
 * Spring {@link org.springframework.aop.Pointcut} implementation that uses the AspectJ weaver to
 * evaluate a pointcut expression.
 *
 * <p>The pointcut expression value is an AspectJ expression. This can reference other pointcuts and
 * use composition and other operations.
 *
 * <p>Naturally, as this is to be processed by Spring AOP's proxy-based model, only method execution
 * pointcuts are supported.
 *
 * @author Rob Harrop
...
public class AspectJExpressionPointcut extends AbstractExpressionPointcut
  ...
  @Nullable private transient PointcutExpression pointcutExpression;
   ...
   */
  public AspectJExpressionPointcut(
      Class<?> declarationScope, String[] paramNames, Class<?>[] paramTypes) {
  ...
  /**
   * Check whether this pointcut is ready to match, lazily building the underlying AspectJ pointcut
   * expression.
   */
    ...
    try {
      ...
      if (pmi != null && thisObject != null) { // there is a current invocation
        RuntimeTestWalker originalMethodResidueTest =
            getRuntimeTestWalker(getShadowMatch(method, method));
  ...
  /**
   * Handler for the Spring-specific {@code bean()} pointcut designator extension to AspectJ.
   *
   * <p>This handler must be added to each pointcut object that needs to handle the {@code bean()}
   * PCD. Matching context is obtained automatically by examining a thread local variable and
   * therefore a matching context need not be set on the pointcut.
   */
   ...
   *
   * <p>Dynamic match tests for this matcher always return true, since the matching decision is made
   * at the proxy creation time. For static match tests, this matcher abstains to allow the overall
    ...
    private FuzzyBoolean contextMatch(@Nullable Class<?> targetType) {
      ...
      if (targetType != null) {
        ...
        return FuzzyBoolean.fromBoolean(
            matchesBean(
                isFactory ? BeanFactory.FACTORY_BEAN_PREFIX + advisedBeanName : advisedBeanName));
      } else {
        return FuzzyBoolean.fromBoolean(
            matchesBean(advisedBeanName)
