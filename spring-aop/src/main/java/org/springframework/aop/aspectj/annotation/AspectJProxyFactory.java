...
/**
 * AspectJ-based proxy factory, allowing for programmatic building of proxies which include AspectJ
 * aspects (code style as well Java 5 annotation style).
 *
 * @author Rob Harrop
...
public class AspectJProxyFactory extends ProxyCreatorSupport {
  ...
  /**
   * Create a new {@code AspectJProxyFactory}. No target, only interfaces. Must add interceptors.
   */
  ...
  /**
   * Add the supplied aspect instance to the chain. The type of the aspect instance supplied must be
   * a singleton aspect. True singleton lifecycle is not honoured when using this method - the
   * caller is responsible for managing the lifecycle of any aspects added in this way.
   *
   ...
   * type has no per clause, then a {@link SingletonMetadataAwareAspectInstanceFactory} is returned,
   * otherwise a {@link PrototypeAspectInstanceFactory} is returned.
   */
  ...
  /**
   * Get the singleton aspect instance for the supplied aspect type. An instance is created if one
   * cannot be found in the instance cache.
   */
   ...
   *
   * <p>Can be called repeatedly. Effect will vary if we've added or removed interfaces. Can add and
   * remove interceptors.
   *
   * <p>Uses a default class loader: Usually, the thread context class loader (if necessary for
   * proxy creation).
   *
