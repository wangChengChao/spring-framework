...
/**
 * An implementation of the AspectJ {@link ProceedingJoinPoint} interface wrapping an AOP Alliance
 * {@link org.aopalliance.intercept.MethodInvocation}.
 *
 * <p><b>Note</b>: The {@code getThis()} method returns the current Spring AOP proxy. The {@code
 * getTarget()} method returns the current Spring AOP target (which may be {@code null} if there is
 * no target instance) as a plain POJO without any advice. <b>If you want to call the object and
 * have the advice take effect, use {@code getThis()}.</b> A common example is casting the object to
 * an introduced interface in the implementation of an introduction. There is no such distinction
 * between target and proxy in AspectJ itself.
 *
 * @author Rod Johnson
  ...
  public Object proceed(Object[] arguments) throws Throwable {
    ...
    if (arguments.length != this.methodInvocation.getArguments().length) {
      throw new IllegalArgumentException(
          "Expecting "
              + this.methodInvocation.getArguments().length
