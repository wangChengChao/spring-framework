 ...
 *
 * <ul>
 *   <li>if {@code a} and {@code b} are defined in different aspects, then the advice in the aspect
 *       with the lowest order value has the highest precedence
 *   <li>if {@code a} and {@code b} are defined in the same aspect, then if one of {@code a} or
 *       {@code b} is a form of after advice, then the advice declared last in the aspect has the
 *       highest precedence. If neither {@code a} nor {@code b} is a form of after advice, then the
 *       advice declared first in the aspect has the highest precedence.
 * </ul>
 *
 * <p>Important: Note that unlike a normal comparator a return of 0 means we don't care about the
 * ordering, not that the two elements must be sorted identically. Used with AspectJ PartialOrder
 * class.
 *
...
class AspectJPrecedenceComparator implements Comparator<Advisor> {
  ...
  /**
   * Create a AspectJPrecedenceComparator, using the given Comparator for comparing {@link
   * org.springframework.aop.Advisor} instances.
   *
  ...
  private boolean declaredInSameAspect(Advisor advisor1, Advisor advisor2) {
    return (hasAspectName(advisor1)
        && hasAspectName(advisor2)
