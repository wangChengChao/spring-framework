...
/**
 * This class encapsulates some AspectJ internal knowledge that should be pushed back into the
 * AspectJ project in a future release.
 *
 * <p>It relies on implementation specific knowledge in AspectJ to break encapsulation and do
 * something AspectJ was not designed to do: query the types of runtime tests that will be
 * performed. The code here should migrate to {@code
 * ShadowMatch.getVariablesInvolvedInRuntimeTest()} or some similar operation.
 *
...
class RuntimeTestWalker {
  ...
  static {
    try {
      ...
      throw new IllegalStateException(
          "The version of aspectjtools.jar / aspectjweaver.jar "
              + "on the classpath is incompatible with this version of Spring: "
              + ex);
    }
  }

  @Nullable private final Test runtimeTest;
  ...
  /**
   * If the test uses any of the this, target, at_this, at_target, and at_annotation vars, then it
   * tests subtype sensitive vars.
   */
  ...
  public boolean testTargetInstanceOfResidue(Class<?> targetClass) {
    return (this.runtimeTest != null
        && new TargetInstanceOfResidueTestVisitor(targetClass)
            .targetInstanceOfMatches(this.runtimeTest));
  }
  ...
  private abstract static class InstanceOfResidueTestVisitor extends TestVisitorAdapter {
    ...
    public InstanceOfResidueTestVisitor(
        Class<?> matchClass, boolean defaultMatches, int matchVarType) {
