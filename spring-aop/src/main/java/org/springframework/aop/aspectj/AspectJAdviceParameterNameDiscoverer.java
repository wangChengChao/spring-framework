...
/**
 * {@link ParameterNameDiscoverer} implementation that tries to deduce parameter names for an advice
 * method from the pointcut expression, returning, and throwing clauses. If an unambiguous
 * interpretation is not available, it returns {@code null}.
 *
 * <p>This class interprets arguments in the following way:
 *
 * <ol>
 *   <li>If the first parameter of the method is of type {@link JoinPoint} or {@link
 *       ProceedingJoinPoint}, it is assumed to be for passing {@code thisJoinPoint} to the advice,
 *       and the parameter name will be assigned the value {@code "thisJoinPoint"}.
 *   <li>If the first parameter of the method is of type {@code JoinPoint.StaticPart}, it is assumed
 *       to be for passing {@code "thisJoinPointStaticPart"} to the advice, and the parameter name
 *       will be assigned the value {@code "thisJoinPointStaticPart"}.
 *   <li>If a {@link #setThrowingName(String) throwingName} has been set, and there are no unbound
 *       arguments of type {@code Throwable+}, then an {@link IllegalArgumentException} is raised.
 *       If there is more than one unbound argument of type {@code Throwable+}, then an {@link
 *       AmbiguousBindingException} is raised. If there is exactly one unbound argument of type
 *       {@code Throwable+}, then the corresponding parameter name is assigned the value
 *       &lt;throwingName&gt;.
 *   <li>If there remain unbound arguments, then the pointcut expression is examined. Let {@code a}
 *       be the number of annotation-based pointcut expressions (&#64;annotation, &#64;this,
 *       &#64;target, &#64;args, &#64;within, &#64;withincode) that are used in binding form. Usage
 *       in binding form has itself to be deduced: if the expression inside the pointcut is a single
 *       string literal that meets Java variable name conventions it is assumed to be a variable
 *       name. If {@code a} is zero we proceed to the next stage. If {@code a} &gt; 1 then an {@code
 *       AmbiguousBindingException} is raised. If {@code a} == 1, and there are no unbound arguments
 *       of type {@code Annotation+}, then an {@code IllegalArgumentException} is raised. if there
 *       is exactly one such argument, then the corresponding parameter name is assigned the value
 *       from the pointcut expression.
 *   <li>If a returningName has been set, and there are no unbound arguments then an {@code
 *       IllegalArgumentException} is raised. If there is more than one unbound argument then an
 *       {@code AmbiguousBindingException} is raised. If there is exactly one unbound argument then
 *       the corresponding parameter name is assigned the value &lt;returningName&gt;.
 *   <li>If there remain unbound arguments, then the pointcut expression is examined once more for
 *       {@code this}, {@code target}, and {@code args} pointcut expressions used in the binding
 *       form (binding forms are deduced as described for the annotation based pointcuts). If there
 *       remains more than one unbound argument of a primitive type (which can only be bound in
 *       {@code args}) then an {@code AmbiguousBindingException} is raised. If there is exactly one
 *       argument of a primitive type, then if exactly one {@code args} bound variable was found, we
 *       assign the corresponding parameter name the variable name. If there were no {@code args}
 *       bound variables found an {@code IllegalStateException} is raised. If there are multiple
 *       {@code args} bound variables, an {@code AmbiguousBindingException} is raised. At this
 *       point, if there remains more than one unbound argument we raise an {@code
 *       AmbiguousBindingException}. If there are no unbound arguments remaining, we are done. If
 *       there is exactly one unbound argument remaining, and only one candidate variable name
 *       unbound from {@code this}, {@code target}, or {@code args}, it is assigned as the
 *       corresponding parameter name. If there are multiple possibilities, an {@code
 *       AmbiguousBindingException} is raised.
 * </ol>
 *
 * <p>The behavior on raising an {@code IllegalArgumentException} or {@code
 * AmbiguousBindingException} is configurable to allow this discoverer to be used as part of a
 * chain-of-responsibility. By default the condition will be logged and the {@code
 * getParameterNames(..)} method will simply return {@code null}. If the {@link
 * #setRaiseExceptions(boolean) raiseExceptions} property is set to {@code true}, the conditions
 * will be thrown as {@code IllegalArgumentException} and {@code AmbiguousBindingException},
 * respectively.
 *
 * <p>Was that perfectly clear? ;)
 *
 * <p>Short version: If an unambiguous binding can be deduced, then it is. If the advice
 * requirements cannot possibly be satisfied, then {@code null} is returned. By setting the {@link
 * #setRaiseExceptions(boolean) raiseExceptions} property to {@code true}, descriptive exceptions
 * will be thrown instead of returning {@code null} in the case that the parameter names cannot be
 * discovered.
 *
...
public class AspectJAdviceParameterNameDiscoverer implements ParameterNameDiscoverer {
  ...
  @Nullable private String pointcutExpression;
  ...
  /**
   * Create a new discoverer that attempts to discover parameter names. from the given pointcut
   * expression.
   */
  ...
  /**
   * Indicate whether {@link IllegalArgumentException} and {@link AmbiguousBindingException} must be
   * thrown as appropriate in the case of failing to deduce advice parameter names.
   *
  ...
  /**
   * If {@code afterReturning} advice binds the return value, the returning variable name must be
   * specified.
   *
  ...
  /**
   * If {@code afterThrowing} advice binds the thrown value, the throwing variable name must be
   * specified.
   *
   ...
   *
   * <p>See the {@link AspectJAdviceParameterNameDiscoverer class level javadoc} for this class for
  ...
  public String[] getParameterNames(Method method) {
    ...
    if (this.numberOfRemainingUnboundArguments == 0) {
      ...
      if (this.raiseExceptions) {
        throw new IllegalStateException(
            "Failed to bind all argument names: "
                + this.numberOfRemainingUnboundArguments
                + " argument(s) could not be bound");
      } else {
        // convention for failing is to return null, allowing participation in a chain of
        // responsibility
        return null;
      }
    }
   ...
   *
   * @return {@code null}
   * @throws UnsupportedOperationException if {@link #setRaiseExceptions(boolean) raiseExceptions}
  ...
  private boolean maybeBindThisJoinPoint() {
    if ((this.argumentTypes[0] == JoinPoint.class)
        || (this.argumentTypes[0] == ProceedingJoinPoint.class)) {
  ...
  /**
   * If a throwing name was specified and there is exactly one choice remaining (argument that is a
   * subtype of Throwable) then bind it.
   */
      ...
      if (isUnbound(i) && isSubtypeOf(Throwable.class, i)) {
        if (throwableIndex == -1) {
          ...
          throw new AmbiguousBindingException(
              "Binding of throwing parameter '"
                  + this.throwingName
                  + "' is ambiguous: could be bound to argument "
                  + throwableIndex
                  + " or argument "
                  + i);
        }
  ...
  private void maybeBindReturningVariable() {
    ...
    if (this.returningName != null) {
      if (this.numberOfRemainingUnboundArguments > 1) {
        throw new AmbiguousBindingException(
            "Binding of returning parameter '"
                + this.returningName
                + "' is ambiguous, there are "
                + this.numberOfRemainingUnboundArguments
                + " candidates.");
      }
  ...
  /**
   * Parse the string pointcut expression looking for: &#64;this, &#64;target, &#64;args,
   * &#64;within, &#64;withincode, &#64;annotation. If we find one of these pointcut expressions,
   * try and extract a candidate variable name (or variable names, in the case of args).
   *
  ...
  /**
   * Given an args pointcut body (could be {@code args} or {@code at_args}), add any candidate
   * variable names to the given list.
   */
  ...
  /**
   * Parse the string pointcut expression looking for this(), target() and args() expressions. If we
   * find one, try and extract a candidate variable name and bind it.
   */
  ...
  private void findAndBind(Class<?> argumentType, String varName) {
    ...
    throw new IllegalStateException(
        "Expected to find an unbound argument of type '" + argumentType.getName() + "'");
  }

  /**
   * Simple struct to hold the extracted text from a pointcut body, together with the number of
   * tokens consumed in extracting it.
   */
  ...
  /**
   * Thrown in response to an ambiguous binding being detected when trying to resolve a method's
   * parameter names.
   */
