 ...
 * to ensure only one new aspect comes back.
 *
 * @author Rod Johnson
...
public class BeanFactoryAspectInstanceFactory
  ...
  /**
   * Create a BeanFactoryAspectInstanceFactory. AspectJ will be called to introspect to create
   * AJType metadata using the type returned for the given bean name from the BeanFactory.
   *
  ...
  /**
   * Create a BeanFactoryAspectInstanceFactory, providing a type that AspectJ should introspect to
   * create AJType metadata. Use if the BeanFactory may consider the type to be a subclass (as when
   * using CGLIB), and the information should relate to a superclass.
   *
   * @param beanFactory the BeanFactory to obtain instance(s) from
   * @param name the name of the bean
   * @param type the type that should be introspected by AspectJ ({@code null} indicates resolution
   *     through {@link BeanFactory#getType} via the bean name)
   */
  public BeanFactoryAspectInstanceFactory(
      BeanFactory beanFactory, String name, @Nullable Class<?> type) {
