 ...
 *
 * <p>Can be subclassed to override {@code requiresRefresh()} to suppress unnecessary refreshes. By
 * default, a refresh will be performed every time the "refreshCheckDelay" has elapsed.
 *
 * @author Rob Harrop
 * @author Rod Johnson
...
public class BeanFactoryRefreshableTargetSource extends AbstractRefreshableTargetSource {
   ...
   *
   * <p>Note that the passed-in BeanFactory should have an appropriate bean definition set up for
   * the given bean name.
   *
  ...
  /**
   * A template method that subclasses may override to provide a fresh target object for the given
   * bean factory and bean name.
   *
   * <p>This default implementation fetches a new target bean instance from the bean factory.
   *
