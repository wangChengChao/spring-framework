 ...
 * See {@link LazyInitTargetSource} and {@link AbstractPrototypeBasedTargetSource}'s subclasses for
 * concrete strategies.
 *
 * <p>BeanFactory-based TargetSources are serializable. This involves disconnecting the current
 * target and turning into a {@link SingletonTargetSource}.
 *
...
public abstract class AbstractBeanFactoryBasedTargetSource
  ...
  /**
   * BeanFactory that owns this TargetSource. We need to hold onto this reference so that we can
   * create new prototype instances as necessary.
   */
   ...
   *
   * <p>The target bean should not be a singleton, else the same instance will always be obtained
   * from the factory, resulting in the same behavior as provided by {@link SingletonTargetSource}.
   *
  ...
  /**
   * Specify the target class explicitly, to avoid any kind of access to the target bean (for
   * example, to avoid initialization of a FactoryBean instance).
   *
   * <p>Default is to detect the type automatically, through a {@code getType} call on the
   * BeanFactory (or even a full {@code getBean} call as fallback).
   */
  ...
  /**
   * Set the owning BeanFactory. We need to save a reference so that we can use the {@code getBean}
  ...
  public Class<?> getTargetClass() {
    ...
    synchronized (this) {
      // Full check within synchronization, entering the BeanFactory interaction algorithm only
      // once...
  ...
  /**
   * Copy configuration from the other AbstractBeanFactoryBasedTargetSource object. Subclasses
   * should override this if they wish to expose it.
   *
  ...
  public boolean equals(Object other) {
    ...
    AbstractBeanFactoryBasedTargetSource otherTargetSource =
        (AbstractBeanFactoryBasedTargetSource) other;
