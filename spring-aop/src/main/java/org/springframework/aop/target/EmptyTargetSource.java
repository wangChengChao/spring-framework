...
/**
 * Canonical {@code TargetSource} when there is no target (or just the target class known), and
 * behavior is supplied by interfaces and advisors only.
 *
 * @author Rod Johnson
...
public final class EmptyTargetSource implements TargetSource, Serializable {
   ...
   *
   * <p>This constructor is {@code private} to enforce the Singleton pattern / factory method
   * pattern.
   *
  ...
  /**
   * Returns the canonical instance on deserialization in case of no target class, thus protecting
  ...
  public boolean equals(Object other) {
    ...
    return (ObjectUtils.nullSafeEquals(this.targetClass, otherTs.targetClass)
        && this.isStatic == otherTs.isStatic);
  }
  ...
  public String toString() {
    return "EmptyTargetSource: "
        + (this.targetClass != null
            ? "target class [" + this.targetClass.getName() + "]"
            : "no target class")
        + ", "
