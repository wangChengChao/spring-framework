...
/**
 * Base class for dynamic {@link org.springframework.aop.TargetSource} implementations that create
 * new prototype bean instances to support a pooling or new-instance-per-invocation strategy.
 *
 * <p>Such TargetSources must run in a {@link BeanFactory}, as it needs to call the {@code getBean}
 * method to create a new prototype instance. Therefore, this base class extends {@link
...
public abstract class AbstractPrototypeBasedTargetSource
  ...
  protected void destroyPrototypeInstance(Object target) {
    ...
    if (getBeanFactory() instanceof ConfigurableBeanFactory) {
      ...
      try {
        ...
        logger.warn(
            "Destroy method on bean with name '" + getTargetBeanName() + "' threw an exception",
            ex);
      }
    }
  }
  ...
  /**
   * Replaces this object with a SingletonTargetSource on serialization. Protected as otherwise it
   * won't be invoked for subclasses. (The {@code writeReplace()} method must be visible to the
   * class being serialized.)
   *
   * <p>With this implementation of this method, there is no need to mark non-serializable fields in
   * this class or subclasses as transient.
   */
