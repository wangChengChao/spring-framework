 ...
 *
 * <p>By default, an instance of {@code GenericObjectPool} is created. Subclasses may change the
 * type of {@code ObjectPool} used by overriding the {@code createObjectPool()} method.
 *
 * <p>Provides many configuration properties mirroring those of the Commons Pool {@code
 * GenericObjectPool} class; these properties are passed to the {@code GenericObjectPool} during
 * construction. If creating a subclass of this class to change the {@code ObjectPool}
 * implementation type, pass in the values of configuration properties that are relevant to your
 * chosen implementation.
 *
 * <p>The {@code testOnBorrow}, {@code testOnReturn} and {@code testWhileIdle} properties are
 * explicitly not mirrored because the implementation of {@code PoolableObjectFactory} used by this
 * class does not implement meaningful validation. All exposed Commons Pool properties use the
...
public class CommonsPool2TargetSource extends AbstractPoolingTargetSource
  ...
  /**
   * Set the maximum waiting time for fetching an object from the pool. Default is -1, waiting
   * forever.
   *
  ...
  /**
   * Set the time between eviction runs that check idle objects whether they have been idle for too
   * long or have become invalid. Default is -1, not performing any eviction.
   *
  ...
  /**
   * Set the minimum time that an idle object can sit in the pool before it becomes subject to
   * eviction. Default is 1800000 (30 minutes).
   *
   * <p>Note that eviction runs need to be performed to take this setting into effect.
   *
   * @see #setTimeBetweenEvictionRunsMillis
