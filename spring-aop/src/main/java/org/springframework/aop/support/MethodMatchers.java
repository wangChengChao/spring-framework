 ...
 *
 * <p>A MethodMatcher may be evaluated statically (based on method and target class) or need further
...
public abstract class MethodMatchers {
  ...
  public static MethodMatcher union(MethodMatcher mm1, MethodMatcher mm2) {
    return (mm1 instanceof IntroductionAwareMethodMatcher
            || mm2 instanceof IntroductionAwareMethodMatcher
   ...
   * @param mm1 the first MethodMatcher
   * @param mm2 the second MethodMatcher
   * @return a distinct MethodMatcher that matches all methods that both of the given MethodMatchers
   *     match
   */
  public static MethodMatcher intersection(MethodMatcher mm1, MethodMatcher mm2) {
    return (mm1 instanceof IntroductionAwareMethodMatcher
            || mm2 instanceof IntroductionAwareMethodMatcher
        ? new IntersectionIntroductionAwareMethodMatcher(mm1, mm2)
        : new IntersectionMethodMatcher(mm1, mm2));
  }

  /**
   * Apply the given MethodMatcher to the given Method, supporting an {@link
   ...
   * @param hasIntroductions {@code true} if the object on whose behalf we are asking is the subject
   *     on one or more introductions; {@code false} otherwise
   * @return whether or not this method matches statically
   */
  public static boolean matches(
      MethodMatcher mm, Method method, Class<?> targetClass, boolean hasIntroductions) {
  ...
  private static class UnionMethodMatcher implements MethodMatcher, Serializable {
    ...
    public boolean matches(Method method, Class<?> targetClass, Object... args) {
      return this.mm1.matches(method, targetClass, args)
          || this.mm2.matches(method, targetClass, args);
    }
  ...
  /**
   * MethodMatcher implementation for a union of two given MethodMatchers of which at least one is
   * an IntroductionAwareMethodMatcher.
   *
   * @since 5.1
   */
  @SuppressWarnings("serial")
  ...
  /**
   * MethodMatcher implementation for a union of two given MethodMatchers, supporting an associated
  ...
  private static class ClassFilterAwareUnionMethodMatcher extends UnionMethodMatcher {
    ...
    public ClassFilterAwareUnionMethodMatcher(
        MethodMatcher mm1, ClassFilter cf1, MethodMatcher mm2, ClassFilter cf2) {
      super(mm1, mm2);
      this.cf1 = cf1;
      this.cf2 = cf2;
  ...
  /**
   * MethodMatcher implementation for a union of two given MethodMatchers of which at least one is
   * an IntroductionAwareMethodMatcher, supporting an associated ClassFilter per MethodMatcher.
   *
   * @since 5.1
   */
  @SuppressWarnings("serial")
  ...
  private static class IntersectionMethodMatcher implements MethodMatcher, Serializable {
    ...
    public boolean matches(Method method, Class<?> targetClass, Object... args) {
      ...
      boolean aMatches =
          (this.mm1.isRuntime()
              ? this.mm1.matches(method, targetClass, args)
  ...
  /**
   * MethodMatcher implementation for an intersection of two given MethodMatchers of which at least
   * one is an IntroductionAwareMethodMatcher.
   *
   * @since 5.1
   */
  @SuppressWarnings("serial")
