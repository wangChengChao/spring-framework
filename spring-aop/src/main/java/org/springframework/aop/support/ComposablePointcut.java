...
/**
 * Convenient class for building up pointcuts. All methods return ComposablePointcut, so we can use
 * a concise idiom like:
 *
 * <p>{@code Pointcut pc = new
 * ComposablePointcut().union(classFilter).intersection(methodMatcher).intersection(pointcut); }
 *
 * @author Rod Johnson
...
public class ComposablePointcut implements Pointcut, Serializable {
  ...
  /**
   * Create a default ComposablePointcut, with {@code ClassFilter.TRUE} and {@code
   * MethodMatcher.TRUE}.
   */
   ...
   *
   * <p>Note that for a Pointcut union, methods will only match if their original ClassFilter (from
   * the originating Pointcut) matches as well. MethodMatchers and ClassFilters from different
   * Pointcuts will never get interleaved with each other.
   *
