 ...
 *
 * <ul>
 *   <li>pattern: regular expression for the fully-qualified method names to match. The exact regexp
 *       syntax will depend on the subclass (e.g. Perl5 regular expressions)
 *   <li>patterns: alternative property taking a String array of patterns. The result will be the
 *       union of these patterns.
 * </ul>
 *
 * <p>Note: the regular expressions must be a match. For example, {@code .*get.*} will match
 * com.mycom.Foo.getBar(). {@code get.*} will not.
 *
 * <p>This base class is serializable. Subclasses should declare all fields transient; the {@link
 * #initPatternRepresentation} method will be invoked again on deserialization.
 *
 * @author Rod Johnson
...
public abstract class AbstractRegexpMethodPointcut extends StaticMethodMatcherPointcut
  ...
  /**
   * Convenience method when we have only a single pattern. Use either this method or {@link
   * #setPatterns}, not both.
   *
   * @see #setPatterns
   */
   ...
   * {@link #setExcludedPatterns}, not both.
   *
  ...
  /**
   * Try to match the regular expression against the fully qualified name of the target class as
   * well as against the method's declaring class, plus the name of the method.
   */
  @Override
   ...
   *
   * <p>This method will be invoked from the {@link #setPatterns} method, and also on
   * deserialization.
   *
   ...
   * Subclasses must implement this to initialize regexp pointcuts. Can be invoked multiple times.
   *
   * <p>This method will be invoked from the {@link #setExcludedPatterns} method, and also on
   * deserialization.
   *
   * @param patterns the patterns to initialize
   * @throws IllegalArgumentException in case of an invalid pattern
