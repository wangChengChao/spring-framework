...
public abstract class ScopedProxyUtils {
  ...
  /**
   * Generate a scoped proxy for the supplied target bean, registering the target bean with an
   * internal name and setting 'targetBeanName' on the scoped proxy.
   *
  ...
  public static BeanDefinitionHolder createScopedProxy(
    ...
    if (proxyTargetClass) {
      ...
      // ScopedProxyFactoryBean's "proxyTargetClass" default is TRUE, so we don't need to set it
      // explicitly here.
    } else {
  ...
  /**
   * Get the original bean name for the provided {@linkplain #getTargetBeanName target bean name}.
   *
   * @param targetBeanName the target bean name for the scoped proxy
   * @return the original bean name
   * @throws IllegalArgumentException if the supplied bean name does not refer to the target of a
   *     scoped proxy
   * @since 5.1.10
   * @see #getTargetBeanName(String)
  ...
  /**
   * Specify if the {@code beanName} is the name of a bean that references the target bean within a
   * scoped proxy.
   *
   * @since 4.1.4
   */
