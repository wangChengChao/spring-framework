...
/**
 * {@code MethodInterceptor} implementation that allows for highly customizable method-level
 * tracing, using placeholders.
 *
 * <p>Trace messages are written on method entry, and if the method invocation succeeds on method
 * exit. If an invocation results in an exception, then an exception message is written. The
 * contents of these trace messages is fully customizable and special placeholders are available to
 * allow you to include runtime information in your log messages. The placeholders available are:
 *
 * <p>
 *
 * <ul>
 *   <li>{@code $[methodName]} - replaced with the name of the method being invoked
 *   <li>{@code $[targetClassName]} - replaced with the name of the class that is the target of the
 *       invocation
 *   <li>{@code $[targetClassShortName]} - replaced with the short name of the class that is the
 *       target of the invocation
 *   <li>{@code $[returnValue]} - replaced with the value returned by the invocation
 *   <li>{@code $[argumentTypes]} - replaced with a comma-separated list of the short class names of
 *       the method arguments
 *   <li>{@code $[arguments]} - replaced with a comma-separated list of the {@code String}
 *       representation of the method arguments
 *   <li>{@code $[exception]} - replaced with the {@code String} representation of any {@code
 *       Throwable} raised during the invocation
 *   <li>{@code $[invocationTime]} - replaced with the time, in milliseconds, taken by the method
 *       invocation
 * </ul>
 *
 * <p>There are restrictions on which placeholders can be used in which messages: see the individual
 * message properties for details on the valid placeholders.
 *
 * @author Rob Harrop
...
public class CustomizableTraceInterceptor extends AbstractTraceInterceptor {
  ...
  /**
   * The {@code $[targetClassName]} placeholder. Replaced with the fully-qualified name of the
   * {@code Class} of the method invocation target.
   */
  public static final String PLACEHOLDER_TARGET_CLASS_NAME = "$[targetClassName]";

  /**
   * The {@code $[targetClassShortName]} placeholder. Replaced with the short name of the {@code
   * Class} of the method invocation target.
   */
  public static final String PLACEHOLDER_TARGET_CLASS_SHORT_NAME = "$[targetClassShortName]";

  /**
   * The {@code $[returnValue]} placeholder. Replaced with the {@code String} representation of the
   * value returned by the method invocation.
   */
  public static final String PLACEHOLDER_RETURN_VALUE = "$[returnValue]";

  /**
   * The {@code $[argumentTypes]} placeholder. Replaced with a comma-separated list of the argument
   * types for the method invocation. Argument types are written as short class names.
   */
  public static final String PLACEHOLDER_ARGUMENT_TYPES = "$[argumentTypes]";

  /**
   * The {@code $[arguments]} placeholder. Replaced with a comma separated list of the argument
   * values for the method invocation. Relies on the {@code toString()} method of each argument
   * type.
   */
  public static final String PLACEHOLDER_ARGUMENTS = "$[arguments]";

  /**
   * The {@code $[exception]} placeholder. Replaced with the {@code String} representation of any
   * {@code Throwable} raised during method invocation.
   */
  public static final String PLACEHOLDER_EXCEPTION = "$[exception]";

  /**
   * The {@code $[invocationTime]} placeholder. Replaced with the time taken by the invocation (in
   * milliseconds).
   */
  ...
  /**
   * Set the template used for method entry log messages. This template can contain any of the
   * following placeholders:
   *
   * <ul>
   *   <li>{@code $[targetClassName]}
   *   <li>{@code $[targetClassShortName]}
   *   <li>{@code $[argumentTypes]}
   *   <li>{@code $[arguments]}
   * </ul>
   */
  ...
  /**
   * Set the template used for method exit log messages. This template can contain any of the
   * following placeholders:
   *
   * <ul>
   *   <li>{@code $[targetClassName]}
  ...
  /**
   * Set the template used for method exception log messages. This template can contain any of the
   * following placeholders:
   *
   * <ul>
   *   <li>{@code $[targetClassName]}
  ...
  /**
   * Writes a log message before the invocation based on the value of {@code enterMessage}. If the
   * invocation succeeds, then a log message is written on exit based on the value {@code
   * exitMessage}. If an exception occurs during invocation, then a message is written based on the
   * value of {@code exceptionMessage}.
   *
   * @see #setEnterMessage
   * @see #setExitMessage
   * @see #setExceptionMessage
  ...
  protected Object invokeUnderTrace(MethodInvocation invocation, Log logger) throws Throwable {
    ...
    try {
      ...
      writeToLog(
          logger,
          replacePlaceholders(
              this.exceptionMessage, invocation, null, ex, stopWatch.getTotalTimeMillis()),
          ex);
      throw ex;
    } finally {
  ...
  /**
   * Replace the placeholders in the given message with the supplied values, or values derived from
   * those supplied.
   *
   * @param message the message template containing the placeholders to be replaced
   * @param methodInvocation the {@code MethodInvocation} being logged. Used to derive values for
   *     all placeholders except {@code $[exception]} and {@code $[returnValue]}.
   * @param returnValue any value returned by the invocation. Used to replace the {@code
   *     $[returnValue]} placeholder. May be {@code null}.
   * @param throwable any {@code Throwable} raised during the invocation. The value of {@code
   *     Throwable.toString()} is replaced for the {@code $[exception]} placeholder. May be {@code
   *     null}.
   * @param invocationTime the value to write in place of the {@code $[invocationTime]} placeholder
    ...
    while (matcher.find()) {
      ...
      if (PLACEHOLDER_METHOD_NAME.equals(match)) {
        matcher.appendReplacement(
            output, Matcher.quoteReplacement(methodInvocation.getMethod().getName()));
  ...
  /**
   * Adds the {@code String} representation of the method return value to the supplied {@code
   * StringBuffer}. Correctly handles {@code null} and {@code void} results.
   *
  ...
  /**
   * Adds a comma-separated list of the short {@code Class} names of the method argument types to
   * the output. For example, if a method has signature {@code put(java.lang.String,
   * java.lang.Object)} then the value returned will be {@code String, Object}.
   *
   * @param methodInvocation the {@code MethodInvocation} being logged. Arguments will be retrieved
   *     from the corresponding {@code Method}.
  ...
  private void appendArgumentTypes(
      MethodInvocation methodInvocation, Matcher matcher, StringBuffer output) {
  ...
  /**
   * Checks to see if the supplied {@code String} has any placeholders that are not specified as
   * constants on this class and throws an {@code IllegalArgumentException} if so.
   */
