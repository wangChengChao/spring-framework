...
/**
 * Performance monitor interceptor that uses <b>JAMon</b> library to perform the performance
 * measurement on the intercepted method and output the stats. In addition, it tracks/counts
 * exceptions thrown by the intercepted method. The stack traces can be viewed in the JAMon web
 * application.
 *
...
public class JamonPerformanceMonitorInterceptor extends AbstractMonitoringInterceptor {
  ...
  /**
   * Create a new JamonPerformanceMonitorInterceptor with a dynamic or static logger, according to
   * the given flag.
   *
   ...
   *
   * @param useDynamicLogger whether to use a dynamic logger or a static logger
   * @param trackAllInvocations whether to track all invocations that go through this interceptor,
  ...
  /**
   * Set whether to track all invocations that go through this interceptor, or just invocations with
   * trace logging enabled.
   *
   * <p>Default is "false": Only invocations with trace logging enabled will be monitored. Specify
   * "true" to let JAMon track all invocations, gathering statistics even when trace logging is
   * disabled.
   */
  ...
  /**
   * Always applies the interceptor if the "trackAllInvocations" flag has been set; else just kicks
   * in if the log is enabled.
   *
  ...
  /**
   * Wraps the invocation with a JAMon Monitor and writes the current performance statistics to the
   * log (if enabled).
   *
  ...
  /**
   * Count the thrown exception and put the stack trace in the details portion of the key. This will
   * allow the stack trace to be viewed in the JAMon web application.
   */
