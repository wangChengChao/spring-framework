...
/**
 * Convenient methods for creating advisors that may be used when autoproxying beans created with
 * the Spring IoC container, binding the bean name to the current invocation. May support a {@code
...
public abstract class ExposeBeanNameAdvisors {
  ...
  /**
   * Binding for the bean name of the bean which is currently being invoked in the
   * ReflectiveMethodInvocation userAttributes Map.
   */
  private static final String BEAN_NAME_ATTRIBUTE =
      ExposeBeanNameAdvisors.class.getName() + ".BEAN_NAME";

  /**
   * Find the bean name for the current invocation. Assumes that an ExposeBeanNameAdvisor has been
   * included in the interceptor chain, and that the invocation is exposed with
  ...
  /**
   * Find the bean name for the given invocation. Assumes that an ExposeBeanNameAdvisor has been
   * included in the interceptor chain.
   *
  ...
  /**
   * Create a new advisor that will expose the given bean name, introducing the NamedBean interface
   * to make the bean name accessible without forcing the target object to be aware of this Spring
   * IoC concept.
   *
