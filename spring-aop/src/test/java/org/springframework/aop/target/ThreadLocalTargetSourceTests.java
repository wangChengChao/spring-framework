...
public class ThreadLocalTargetSourceTests {
  ...
  public void setup() {
    ...
    new XmlBeanDefinitionReader(this.beanFactory)
        .loadBeanDefinitions(qualifiedResource(ThreadLocalTargetSourceTests.class, "context.xml"));
  }
  ...
  /**
   * Check we can use two different ThreadLocalTargetSources managing objects of different types
  ...
  public void testCanGetStatsViaMixin() {
    ThreadLocalTargetSourceStats stats =
        (ThreadLocalTargetSourceStats) beanFactory.getBean("apartment");
  ...
  public void testReuseDestroyedTarget() {
    ThreadLocalTargetSource source =
        (ThreadLocalTargetSource) this.beanFactory.getBean("threadLocalTs");
