...
public class ControlFlowPointcutTests {
  ...
  /**
   * Check that we can use a cflow pointcut only in conjunction with a static pointcut: e.g. all
   * setter methods that are invoked under a particular class. This greatly reduces the number of
   * calls to the cflow pointcut, meaning that it's not so prohibitively expensive.
   */
  @Test
  ...
  public void testEqualsAndHashCode() throws Exception {
    ...
    assertThat(
            new ControlFlowPointcut(One.class, "getAge").hashCode()
                == new ControlFlowPointcut(One.class).hashCode())
        .isFalse();
  }
