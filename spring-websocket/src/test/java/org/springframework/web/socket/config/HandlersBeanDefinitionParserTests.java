...
public class HandlersBeanDefinitionParserTests {
    ...
    for (HandlerMapping hm : handlersMap.values()) {
      ...
      if (shm.getUrlMap().keySet().contains("/foo")) {
        ...
        WebSocketHttpRequestHandler handler =
            (WebSocketHttpRequestHandler) shm.getUrlMap().get("/foo");
        assertThat(handler).isNotNull();
        unwrapAndCheckDecoratedHandlerType(
            handler.getWebSocketHandler(), FooWebSocketHandler.class);
        ...
        boolean condition =
            handler.getHandshakeInterceptors().get(0) instanceof OriginHandshakeInterceptor;
        assertThat(condition).isTrue();
      } else {
        assertThat(shm.getUrlMap()).containsOnlyKeys("/test");
        WebSocketHttpRequestHandler handler =
            (WebSocketHttpRequestHandler) shm.getUrlMap().get("/test");
        assertThat(handler).isNotNull();
        unwrapAndCheckDecoratedHandlerType(
            handler.getWebSocketHandler(), TestWebSocketHandler.class);
        HandshakeHandler handshakeHandler = handler.getHandshakeHandler();
        assertThat(handshakeHandler).isNotNull();
        boolean condition1 = handshakeHandler instanceof DefaultHandshakeHandler;
        assertThat(condition1).isTrue();
  ...
  public void sockJs() {
    ...
    unwrapAndCheckDecoratedHandlerType(
        testHandler.getWebSocketHandler(), TestWebSocketHandler.class);
    ...
    WebSocketTransportHandler handler =
        (WebSocketTransportHandler) handlerMap.get(TransportType.WEBSOCKET);
  ...
  public void sockJsAttributes() {
    ...
    TransportHandlingSockJsService transportService =
    ...
    assertThat(transportService.getTransportHandlers().values())
        .extracting("class")
        .containsExactlyInAnyOrder(
            XhrPollingTransportHandler.class, XhrStreamingTransportHandler.class);
  ...
  private void loadBeanDefinitions(String fileName) {
    ...
    ClassPathResource resource =
        new ClassPathResource(fileName, HandlersBeanDefinitionParserTests.class);
    reader.loadBeanDefinitions(resource);
    this.appContext.refresh();
  }

  private static void unwrapAndCheckDecoratedHandlerType(
      WebSocketHandler handler, Class<?> handlerClass) {
