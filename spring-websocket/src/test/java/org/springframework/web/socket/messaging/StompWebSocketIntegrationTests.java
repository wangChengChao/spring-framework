...
class StompWebSocketIntegrationTests extends AbstractWebSocketIntegrationTests {
  ...
  void sendMessageToBrokerAndReceiveReplyViaTopic(
      WebSocketTestServer server, WebSocketClient webSocketClient, TestInfo testInfo)
      throws Exception {
    super.setup(server, webSocketClient, testInfo);
    ...
    TextMessage m0 = create(StompCommand.CONNECT).headers("accept-version:1.1").build();
    TextMessage m1 =
        create(StompCommand.SUBSCRIBE).headers("id:subs1", "destination:/topic/foo").build();
    ...
    try {
      ...
      assertThat(payload.startsWith("MESSAGE\n"))
          .as("Expected STOMP MESSAGE, got " + payload)
          .isTrue();
    } finally {
      session.close();
    }
  ...
  void sendSubscribeToControllerAndReceiveReply(
    ...
    try {
      assertThat(clientHandler.latch.await(TIMEOUT, TimeUnit.SECONDS)).isTrue();
      String payload = clientHandler.actual.get(1).getPayload();
      assertThat(payload.contains(destHeader))
          .as("Expected STOMP destination=/app/number, got " + payload)
          .isTrue();
  ...
  void webSocketScope(
    ...
    TextMessage m1 =
        create(StompCommand.SUBSCRIBE)
            .headers("id:subs1", "destination:/topic/scopedBeanValue")
            .build();
  ...
  @Configuration
  static class TestMessageBrokerConfiguration
