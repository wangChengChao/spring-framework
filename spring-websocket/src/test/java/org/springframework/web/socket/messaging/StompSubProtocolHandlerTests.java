...
public class StompSubProtocolHandlerTests {
  ...
  public void handleMessageToClientWithConnectedFrame() {
    ...
    Message<byte[]> message =
        MessageBuilder.createMessage(EMPTY_PAYLOAD, headers.getMessageHeaders());
  ...
  public void handleMessageToClientWithSimpConnectAck() {
    ...
    SimpMessageHeaderAccessor ackAccessor =
    ...
    Message<byte[]> ackMessage =
        MessageBuilder.createMessage(EMPTY_PAYLOAD, ackAccessor.getMessageHeaders());
  ...
  public void handleMessageToClientWithUserDestination() {
    ...
    assertThat(((String) textMessage.getPayload()).contains("destination:/user/queue/foo\n"))
        .isTrue();
    assertThat(
            ((String) textMessage.getPayload())
                .contains(SimpMessageHeaderAccessor.ORIGINAL_DESTINATION))
        .isFalse();
  }
  ...
  public void handleMessageFromClient() {
    ...
    TextMessage textMessage =
        StompTextMessageBuilder.create(StompCommand.STOMP)
            .headers(
    ...
    assertThat(SimpMessageHeaderAccessor.getHeartbeat(actual.getHeaders()))
    ...
    assertThat(stompAccessor.getAcceptVersion())
        .isEqualTo(new HashSet<>(Arrays.asList("1.1", "1.0")));
  ...
  public void webSocketScope() {
    ...
    SimpAttributes simpAttributes =
        new SimpAttributes(this.session.getId(), this.session.getAttributes());
    ...
    assertThat(this.session.getSentMessages())
        .isEqualTo(Collections.<WebSocketMessage<?>>emptyList());
