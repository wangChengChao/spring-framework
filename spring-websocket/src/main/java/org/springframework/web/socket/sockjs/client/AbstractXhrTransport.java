...
public abstract class AbstractXhrTransport implements XhrTransport {
   ...
   *
   * <p>Typically an {@code XhrTransport} is used as "XHR streaming" first and then, if that fails,
   * as "XHR". In some cases however it may be helpful to suppress XHR streaming so that only XHR is
   * attempted.
   *
   * <p>By default this property is set to {@code false} which means both "XHR streaming" and "XHR"
   * apply.
   */
