...
/**
 * A SockJS implementation of {@link org.springframework.web.socket.client.WebSocketClient
 * WebSocketClient} with fallback alternatives that simulate a WebSocket interaction through plain
 * HTTP streaming and long polling techniques..
 *
 * <p>Implements {@link Lifecycle} in order to propagate lifecycle events to the transports it is
 * configured with.
 *
 * @author Rossen Stoyanchev
 * @since 4.1
 * @see <a
 *     href="https://github.com/sockjs/sockjs-client">https://github.com/sockjs/sockjs-client</a>
...
public class SockJsClient implements WebSocketClient, Lifecycle {
  ...
  @Nullable private String[] httpHeaderNames;

  private InfoReceiver infoReceiver;

  @Nullable private SockJsMessageCodec messageCodec;

  @Nullable private TaskScheduler connectTimeoutScheduler;
   ...
   *
   * <p>If the list includes an {@link XhrTransport} (or more specifically an implementation of
   * {@link InfoReceiver}) the instance is used to initialize the {@link
   * #setInfoReceiver(InfoReceiver) infoReceiver} property, or otherwise is defaulted to {@link
   ...
   * {@link SockJsClient#doHandshake(WebSocketHandler, WebSocketHttpHeaders, URI)} and also used
   * with other HTTP requests issued as part of that SockJS connection, e.g. the initial info
   * request, XHR send or receive requests.
   *
   * <p>By default if this property is not set, all handshake headers are also used for other HTTP
   * requests. Set it if you want only a subset of handshake headers (e.g. auth headers) to be used
  ...
  /**
   * The configured HTTP header names to be copied from the handshake headers and also included in
   * other HTTP requests.
   */
  @Nullable
  ...
  /**
   * Configure the {@code InfoReceiver} to use to perform the SockJS "Info" request before the
   * SockJS session starts.
   *
   * <p>If the list of transports provided to the constructor contained an {@link XhrTransport} or
   * an implementation of {@link InfoReceiver} that instance would have been used to initialize this
   * property, or otherwise it defaults to {@link RestTemplateXhrTransport}.
   *
  ...
  /**
   * Configure a {@code TaskScheduler} for scheduling a connect timeout task where the timeout value
   * is calculated based on the duration of the initial SockJS "Info" request. The connect timeout
   * task ensures a more timely fallback but is otherwise entirely optional.
   *
  ...
  public ListenableFuture<WebSocketSession> doHandshake(
    ...
    URI uri =
        UriComponentsBuilder.fromUriString(uriTemplate).buildAndExpand(uriVars).encode().toUri();
    ...
    for (Transport transport : this.transports) {
      for (TransportType type : transport.getTransportTypes()) {
        if (serverInfo.isWebSocketEnabled() || !TransportType.WEBSOCKET.equals(type)) {
          requests.add(
              new DefaultTransportRequest(
                  urlInfo,
                  headers,
                  getHttpRequestHeaders(headers),
                  transport,
                  type,
                  getMessageCodec()));
        }
  ...
  /**
   * Return the user to associate with the SockJS session and make available via {@link
   * org.springframework.web.socket.WebSocketSession#getPrincipal()}.
   *
  ...
  /**
   * By default the result of a SockJS "Info" request, including whether the server has WebSocket
   * disabled and how long the request took (used for calculating transport timeout time) is cached.
   * This method can be used to clear that cache hence causing it to re-populate.
   */
