...
/**
 * WebSocket-based {@link TransportHandler}. Uses {@link SockJsWebSocketHandler} and {@link
 * WebSocketServerSockJsSession} to add SockJS processing.
 *
 * <p>Also implements {@link HandshakeHandler} to support raw WebSocket communication at SockJS URL
 * "/websocket".
 *
...
public class WebSocketTransportHandler extends AbstractTransportHandler
  ...
  public void handleRequest(
    ...
    try {
      ...
      this.handshakeHandler.doHandshake(
          request, response, wsHandler, sockJsSession.getAttributes());
    } catch (Throwable ex) {
      sockJsSession.tryCloseWithSockJsTransportError(ex, CloseStatus.SERVER_ERROR);
      throw new SockJsTransportFailureException(
          "WebSocket handshake failure", wsSession.getId(), ex);
    }
  }
