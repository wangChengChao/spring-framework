...
/**
 * A helper class for configuring SockJS fallback options for use with an {@link
 * org.springframework.web.socket.config.annotation.EnableWebSocket} and {@link WebSocketConfigurer}
 * setup.
 *
...
public class SockJsServiceRegistration {
  ...
  @Nullable private String clientLibraryUrl;

  @Nullable private Integer streamBytesLimit;

  @Nullable private Boolean sessionCookieNeeded;

  @Nullable private Long heartbeatTime;

  @Nullable private Long disconnectDelay;

  @Nullable private Integer httpMessageCacheSize;

  @Nullable private Boolean webSocketEnabled;
  ...
  @Nullable private Boolean suppressCors;

  @Nullable private SockJsMessageCodec messageCodec;
   ...
   *
   * <p>By default this is set to point to "https://cdn.jsdelivr.net/sockjs/0.3.4/sockjs.min.js".
  ...
  /**
   * The SockJS protocol requires a server to respond to the initial "/info" request from clients
  ...
  /**
   * The amount of time in milliseconds when the server has not sent any messages and after which
   * the server should send a heartbeat frame to the client in order to keep the connection from
   * breaking.
   *
  ...
  /**
   * Some load balancers don't support WebSocket. This option can be used to disable the WebSocket
   ...
   *
   * <p>By default {@code Jackson2SockJsMessageCodec} is used requiring the Jackson library to be
  ...
  private TransportHandlingSockJsService createSockJsService() {
    ...
    Assert.state(
        this.transportHandlers.isEmpty() || this.transportHandlerOverrides.isEmpty(),
