...
/**
 * Initiates WebSocket requests to a WebSocket server programmatically through the Jetty WebSocket
 * API.
 *
 * <p>As of 4.1 this class implements {@link Lifecycle} rather than {@link
 * org.springframework.context.SmartLifecycle}. Use {@link
 * org.springframework.web.socket.client.WebSocketConnectionManager WebSocketConnectionManager}
...
public class JettyWebSocketClient extends AbstractWebSocketClient implements Lifecycle {
  ...
  @Nullable private AsyncListenableTaskExecutor taskExecutor = new SimpleAsyncTaskExecutor();

  /**
   * Default constructor that creates an instance of {@link
  ...
  /**
   * Constructor that accepts an existing {@link org.eclipse.jetty.websocket.client.WebSocketClient}
   * instance.
   */
  ...
  /**
   * Set an {@link AsyncListenableTaskExecutor} to use when opening connections. If this property is
   * set to {@code null}, calls to any of the {@code doHandshake} methods will block until the
  ...
  public ListenableFuture<WebSocketSession> doHandshake(
    ...
    UriComponents uriComponents =
        UriComponentsBuilder.fromUriString(uriTemplate).buildAndExpand(uriVars).encode();
  ...
  public ListenableFuture<WebSocketSession> doHandshakeInternal(
    ...
    final JettyWebSocketHandlerAdapter listener =
        new JettyWebSocketHandlerAdapter(wsHandler, wsSession);
  ...
  /**
   * Return the user to make available through {@link WebSocketSession#getPrincipal()}. By default
