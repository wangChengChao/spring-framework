...
/**
 * A {@link FactoryBean} for configuring {@link javax.websocket.server.ServerContainer}. Since there
 * is usually only one {@code ServerContainer} instance accessible under a well-known {@code
 * javax.servlet.ServletContext} attribute, simply declaring this FactoryBean and using its setters
 * allows for configuring the {@code ServerContainer} through Spring configuration.
 *
 * <p>This is useful even if the {@code ServerContainer} is not injected into any other bean within
 * the Spring application context. For example, an application can configure a {@link
 * org.springframework.web.socket.server.support.DefaultHandshakeHandler}, a {@link
 * org.springframework.web.socket.sockjs.SockJsService}, or {@link ServerEndpointExporter}, and
 * separately declare this FactoryBean in order to customize the properties of the (one and only)
...
public class ServletServerContainerFactoryBean
  ...
  @Nullable private Long asyncSendTimeout;

  @Nullable private Long maxSessionIdleTimeout;

  @Nullable private Integer maxTextMessageBufferSize;

  @Nullable private Integer maxBinaryMessageBufferSize;
  ...
  public void afterPropertiesSet() {
    ...
    this.serverContainer =
        (ServerContainer)
            this.servletContext.getAttribute("javax.websocket.server.ServerContainer");
    Assert.state(
