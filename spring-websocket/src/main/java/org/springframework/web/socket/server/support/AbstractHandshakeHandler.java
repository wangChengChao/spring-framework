 ...
 *
 * <p>Performs initial validation of the WebSocket handshake request - possibly rejecting it through
 * the appropriate HTTP status code - while also allowing its subclasses to override various parts
 * of the negotiation process (e.g. origin validation, sub-protocol negotiation, extensions
 * negotiation, etc).
 *
 * <p>If the negotiation succeeds, the actual upgrade is delegated to a server-specific {@link
 * org.springframework.web.socket.server.RequestUpgradeStrategy}, which will update the response as
 * necessary and initialize the WebSocket. Currently supported servers are Jetty 9.0-9.3, Tomcat
 * 7.0.47+ and 8.x, Undertow 1.0-1.3, GlassFish 4.1+, WebLogic 12.1.3+.
 *
...
public abstract class AbstractHandshakeHandler implements HandshakeHandler, Lifecycle {
  ...
  /**
   * Default constructor that auto-detects and instantiates a {@link RequestUpgradeStrategy}
  ...
  private static RequestUpgradeStrategy initRequestUpgradeStrategy() {
    ...
    try {
      Class<?> clazz =
          ClassUtils.forName(className, AbstractHandshakeHandler.class.getClassLoader());
   ...
   * the response will not contain a {@literal Sec-WebSocket-Protocol} header.
   *
   * <p>Note that if the WebSocketHandler passed in at runtime is an instance of {@link
   * SubProtocolCapable} then there is not need to explicitly configure this property. That is
   * certainly the case with the built-in STOMP over WebSocket support. Therefore this property
   * should be configured explicitly only if the WebSocketHandler does not implement {@code
   * SubProtocolCapable}.
   */
  ...
  public final boolean doHandshake(
    ...
    this.requestUpgradeStrategy.upgrade(
        request, response, subProtocol, extensions, user, wsHandler, attributes);
    return true;
  }
  ...
  protected void handleWebSocketVersionNotSupported(
    ...
    if (logger.isErrorEnabled()) {
      ...
      logger.error(
          "Handshake failed due to unsupported WebSocket version: "
              + version
              + ". Supported versions: "
   ...
   * as considered as valid. Consider using an {@link OriginHandshakeInterceptor} for filtering
   * origins if needed.
   */
   ...
   * list of supported sub-protocols, this method first checks if the target WebSocketHandler is a
   * {@link SubProtocolCapable} and then also checks if any sub-protocols have been explicitly
   * configured with {@link #setSupportedProtocols(String...)}.
   *
  ...
  protected String selectProtocol(
      List<String> requestedProtocols, WebSocketHandler webSocketHandler) {
   ...
   *
   * <p>As of 4.1, the default implementation of this method filters the list to leave only
   * extensions that are both requested and supported.
   *
  ...
  /**
   * A method that can be used to associate a user with the WebSocket session in the process of
   * being established. The default implementation calls {@link ServerHttpRequest#getPrincipal()}
   *
   * <p>Subclasses can provide custom logic for associating a user with a session, for example for
   * assigning a name to anonymous users (i.e. not fully authenticated).
   *
