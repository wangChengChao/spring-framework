...
/**
 * Parses the configuration for the {@code <websocket:handlers/>} namespace element. Registers a
 * Spring MVC {@code SimpleUrlHandlerMapping} to map HTTP WebSocket handshake (or SockJS) requests
 * to {@link org.springframework.web.socket.WebSocketHandler WebSocketHandlers}.
 *
 * @author Brian Clozel
...
class HandlersBeanDefinitionParser implements BeanDefinitionParser {
  ...
  public BeanDefinition parse(Element element, ParserContext context) {
    ...
    RuntimeBeanReference sockJsService =
        WebSocketNamespaceUtils.registerSockJsService(
    ...
    if (sockJsService != null) {
      ...
      RuntimeBeanReference handler =
          WebSocketNamespaceUtils.registerHandshakeHandler(element, context, source);
      Element interceptElem = DomUtils.getChildElementByTagName(element, "handshake-interceptors");
      ManagedList<Object> interceptors =
          WebSocketNamespaceUtils.parseBeanSubElements(interceptElem, context);
  ...
  private static class WebSocketHandlerMappingStrategy implements HandlerMappingStrategy {
    ...
    public WebSocketHandlerMappingStrategy(
        RuntimeBeanReference handshakeHandler, ManagedList<?> interceptors) {
    ...
    public void addMapping(
        Element element, ManagedMap<String, Object> urlMap, ParserContext context) {
      String pathAttribute = element.getAttribute("path");
      String[] mappings = StringUtils.tokenizeToStringArray(pathAttribute, ",");
      RuntimeBeanReference handlerReference =
          new RuntimeBeanReference(element.getAttribute("handler"));
      ...
      RootBeanDefinition requestHandlerDef =
          new RootBeanDefinition(WebSocketHttpRequestHandler.class, cargs, null);
      ...
      String requestHandlerName =
          context.getReaderContext().registerWithGeneratedName(requestHandlerDef);
  ...
  private static class SockJsHandlerMappingStrategy implements HandlerMappingStrategy {
    ...
    public void addMapping(
      ...
      RootBeanDefinition requestHandlerDef =
          new RootBeanDefinition(SockJsHttpRequestHandler.class, cargs, null);
      requestHandlerDef.setSource(context.extractSource(element));
      requestHandlerDef.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
      String requestHandlerName =
          context.getReaderContext().registerWithGeneratedName(requestHandlerDef);
