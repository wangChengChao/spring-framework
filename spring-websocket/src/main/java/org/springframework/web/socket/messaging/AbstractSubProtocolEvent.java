...
/**
 * A base class for events for a message received from a WebSocket client and parsed into a
 * higher-level sub-protocol (e.g. STOMP).
 *
...
public abstract class AbstractSubProtocolEvent extends ApplicationEvent {
   ...
   * @param message the incoming message (never {@code null})
   */
  protected AbstractSubProtocolEvent(
      Object source, Message<byte[]> message, @Nullable Principal user) {
    super(source);
  ...
  /**
   * Return the Message associated with the event. Here is an example of obtaining information about
   * the session id or any headers in the message:
   *
   * <pre class="code">
