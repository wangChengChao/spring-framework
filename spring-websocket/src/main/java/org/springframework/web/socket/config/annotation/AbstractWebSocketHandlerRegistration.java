...
/**
 * Base class for {@link WebSocketHandlerRegistration WebSocketHandlerRegistrations} that gathers
 * all the configuration options but allows sub-classes to put together the actual HTTP request
 * mappings.
 *
...
public abstract class AbstractWebSocketHandlerRegistration<M>
  ...
  @Nullable private HandshakeHandler handshakeHandler;

  private final List<HandshakeInterceptor> interceptors = new ArrayList<>();

  private final List<String> allowedOrigins = new ArrayList<>();

  @Nullable private SockJsServiceRegistration sockJsServiceRegistration;
  ...
  public WebSocketHandlerRegistration setHandshakeHandler(
      @Nullable HandshakeHandler handshakeHandler) {
  ...
  public SockJsServiceRegistration withSockJS() {
    ...
    if (!this.allowedOrigins.isEmpty()) {
      this.sockJsServiceRegistration.setAllowedOrigins(
          StringUtils.toStringArray(this.allowedOrigins));
    }
  ...
  /**
   * Expose the {@code SockJsServiceRegistration} -- if SockJS is enabled or {@code null} otherwise
   * -- so that it can be configured with a TaskScheduler if the application did not provide one.
   * This should be done prior to calling {@link #getMappings()}.
   */
  @Nullable
  ...
  protected abstract void addSockJsServiceMapping(
      M mappings, SockJsService sockJsService, WebSocketHandler handler, String pathPattern);

  protected abstract void addWebSocketHandlerMapping(
      M mappings,
