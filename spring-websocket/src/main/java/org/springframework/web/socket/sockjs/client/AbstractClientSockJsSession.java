...
/**
 * Base class for SockJS client implementations of {@link WebSocketSession}. Provides processing of
 * incoming SockJS message frames and delegates lifecycle events and messages to the (application)
 * {@link WebSocketHandler}. Sub-classes implement actual send as well as disconnect logic.
 *
...
public abstract class AbstractClientSockJsSession implements WebSocketSession {
  ...
  @Nullable private volatile State state = State.NEW;

  @Nullable private volatile CloseStatus closeStatus;

  protected AbstractClientSockJsSession(
  ...
  /**
   * Return a timeout cleanup task to invoke if the SockJS sessions is not fully established within
   * the retransmission timeout period calculated in {@code SockJsRequest} based on the duration of
   * the initial SockJS "Info" request.
   */
  ...
  private void handleOpenFrame() {
    ...
    if (this.state == State.NEW) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Open frame received in "
                + getId()
                + " but we're not connecting (current state "
                + this.state
                + "). The server might have been restarted and lost track of the session.");
      }
