...
public abstract class AbstractSockJsSession implements SockJsSession {
   ...
   * Therefore network IO failures may occur simply because a client has gone away, and that can
   * fill the logs with unnecessary stack traces.
   *
   * <p>We make a best effort to identify such network failures, on a per-server basis, and log them
   * under a separate log category. A simple one-line message is logged at DEBUG level, while a full
  ...
  /**
   * Tomcat: ClientAbortException or EOFException Jetty: EofException WildFly, GlassFish:
   * java.io.IOException "Broken pipe" (already covered)
   *
   * <p>TODO: This definition is currently duplicated between HttpWebHandlerAdapter and
  ...
  protected static final Log disconnectedClientLogger =
      LogFactory.getLog(DISCONNECTED_CLIENT_LOG_CATEGORY);
  ...
  @Nullable private HeartbeatTask heartbeatTask;
  ...
  /**
   * Polling and Streaming sessions periodically close the current HTTP request and wait for the
   * next request to come through. During this "downtime" the session is still open but inactive and
   * unable to send messages and therefore has to buffer them temporarily. A WebSocket session by
   * contrast is stateful and remain active until closed.
   */
  public abstract boolean isActive();

  /**
   * Actually close the underlying WebSocket session or in the case of HTTP transports complete the
   * underlying request.
   */
