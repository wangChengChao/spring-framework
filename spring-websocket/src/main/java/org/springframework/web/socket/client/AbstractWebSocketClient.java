...
public abstract class AbstractWebSocketClient implements WebSocketClient {
  ...
  public final ListenableFuture<WebSocketSession> doHandshake(
    ...
    return doHandshakeInternal(
        webSocketHandler, headersToUse, uri, subProtocols, extensions, Collections.emptyMap());
  }
   ...
   * @param headers the HTTP headers to use for the handshake, with unwanted (forbidden) headers
   *     filtered out (never {@code null})
   ...
   * @param attributes attributes to associate with the WebSocketSession, i.e. via {@link
