...
public class StandardWebSocketClient extends AbstractWebSocketClient {
  ...
  /**
   * Default constructor that calls {@code ContainerProvider.getWebSocketContainer()} to obtain a
   * (new) {@link WebSocketContainer} instance. Also see constructor accepting existing {@code
   ...
   *
   * <p>For XML configuration, see {@link WebSocketContainerFactoryBean}. For Java configuration,
   * use {@code ContainerProvider.getWebSocketContainer()} to obtain the {@code WebSocketContainer}
   * instance.
   */
  ...
  /**
   * The standard Java WebSocket API allows passing "user properties" to the server via {@link
   * ClientEndpointConfig#getUserProperties() userProperties}. Use this property to configure one or
   * more properties to be passed on every handshake.
   */
  ...
  protected ListenableFuture<WebSocketSession> doHandshakeInternal(
    ...
    final StandardWebSocketSession session =
        new StandardWebSocketSession(headers, attributes, localAddress, remoteAddress);

    final ClientEndpointConfig endpointConfig =
