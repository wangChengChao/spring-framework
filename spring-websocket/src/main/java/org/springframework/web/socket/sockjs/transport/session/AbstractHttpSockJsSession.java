...
public abstract class AbstractHttpSockJsSession extends AbstractSockJsSession {
  ...
  @Nullable private volatile URI uri;

  @Nullable private volatile HttpHeaders handshakeHeaders;

  @Nullable private volatile Principal principal;

  @Nullable private volatile InetSocketAddress localAddress;

  @Nullable private volatile InetSocketAddress remoteAddress;

  @Nullable private volatile String acceptedProtocol;

  @Nullable private volatile ServerHttpResponse response;

  @Nullable private volatile SockJsFrameFormat frameFormat;

  @Nullable private volatile ServerHttpAsyncRequestControl asyncRequestControl;

  private boolean readyToSend;

  public AbstractHttpSockJsSession(
      String id,
  ...
  /**
   * Unlike WebSocket where sub-protocol negotiation is part of the initial handshake, in HTTP
   * transports the same negotiation must be emulated and the selected protocol set through this
   * setter.
   *
  ...
  /**
   * Return the SockJS buffer for messages stored transparently between polling requests. If the
   * polling request takes longer than 5 seconds, the session is closed.
   *
  ...
  /**
   * Called when the connection is active and ready to write to the response. Subclasses should only
   * call this method from a method where the "responseLock" is acquired.
   */
