...
/**
 * An interceptor to copy information from the HTTP session to the "handshake attributes" map to
 * made available via{@link WebSocketSession#getAttributes()}.
 *
 * <p>Copies a subset or all HTTP session attributes and/or the HTTP session id under the key {@link
...
public class HttpSessionHandshakeInterceptor implements HandshakeInterceptor {
  ...
  /**
   * The name of the attribute under which the HTTP session id is exposed when {@link
   * #setCopyHttpSessionId(boolean) copyHttpSessionId} is "true".
   */
  ...
  /**
   * Whether to copy all attributes from the HTTP session. If set to "true", any explicitly
   * configured attribute names are ignored.
   *
   * <p>By default this is set to either "true" or "false" depending on which constructor was used
   * (default or with attribute names respectively).
   *
  ...
  /**
   * Whether the HTTP session id should be copied to the handshake attributes under the key {@link
   * #HTTP_SESSION_ID_ATTR_NAME}.
   *
