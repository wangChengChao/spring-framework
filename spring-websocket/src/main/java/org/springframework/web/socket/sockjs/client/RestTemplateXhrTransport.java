...
/**
 * An {@code XhrTransport} implementation that uses a {@link
...
public class RestTemplateXhrTransport extends AbstractXhrTransport {
   ...
   * SimpleAsyncTaskExecutor} is configured which creates a new thread every time the transports
   * connects.
   */
    ...
    getTaskExecutor()
        .execute(
            () -> {
              ...
              XhrRequestCallback requestCallbackAfterHandshake =
                  new XhrRequestCallback(httpHeaders);
              ...
              while (true) {
                ...
                try {
                  ...
                  getRestTemplate()
                      .execute(receiveUrl, HttpMethod.POST, requestCallback, responseExtractor);
  ...
  protected ResponseEntity<String> executeInfoRequestInternal(URI infoUrl, HttpHeaders headers) {
    ...
    return nonNull(
        this.restTemplate.execute(infoUrl, HttpMethod.GET, requestCallback, textResponseExtractor));
  }
      ...
      if (this.body != null) {
        if (request instanceof StreamingHttpOutputMessage) {
          ((StreamingHttpOutputMessage) request)
              .setBody(
                  outputStream -> StreamUtils.copy(this.body, SockJsFrame.CHARSET, outputStream));
        } else {
  ...
  /**
   * Splits the body of an HTTP response into SockJS frames and delegates those to an {@link
