 ...
 *
 * <p>The typical usage will be to set either the "contextPath" or the "classesToBeBound" property
 * on this bean, possibly customize the marshaller and unmarshaller by setting properties, schemas,
 * adapters, and listeners, and to refer to it.
 *
...
public class Jaxb2Marshaller
  ...
  @Nullable private Class<?>[] classesToBeBound;

  @Nullable private String[] packagesToScan;

  @Nullable private Map<String, ?> jaxbContextProperties;

  @Nullable private Map<String, ?> marshallerProperties;

  @Nullable private Map<String, ?> unmarshallerProperties;

  @Nullable private Marshaller.Listener marshallerListener;

  @Nullable private Unmarshaller.Listener unmarshallerListener;

  @Nullable private ValidationEventHandler validationEventHandler;

  @Nullable private XmlAdapter<?, ?>[] adapters;

  @Nullable private Resource[] schemaResources;
  ...
  /**
   * Set multiple JAXB context paths. The given array of context paths gets converted to a
   ...
   *
   * <p>Setting either this property, {@link #setClassesToBeBound "classesToBeBound"} or {@link
   * #setPackagesToScan "packagesToScan"} is required.
   */
   ...
   *
   * <p>Setting either this property, {@link #setContextPath "contextPath"} or {@link
   * #setPackagesToScan "packagesToScan"} is required.
   */
  ...
  /**
   * Set the packages to search for classes with JAXB2 annotations in the classpath. This is using a
   * Spring-bases search and therefore analogous to Spring's component-scan feature ({@link
   * org.springframework.context.annotation.ClassPathBeanDefinitionScanner}).
   *
   * <p>Setting either this property, {@link #setContextPath "contextPath"} or {@link
   * #setClassesToBeBound "classesToBeBound"} is required.
   */
  ...
  /**
   * Set the {@code JAXBContext} properties. These implementation-specific properties will be set on
   * the underlying {@code JAXBContext}.
   */
   ...
   *
   * <p>These properties will be set on the underlying JAXB {@code Marshaller}, and allow for
  ...
  /**
   * Set the JAXB validation event handler. This event handler will be called by JAXB if any
   * validation errors are encountered during calls to any of the marshal APIs.
   */
  ...
  /**
   * Specify the {@code XmlAdapter}s to be registered with the JAXB {@code Marshaller} and {@code
   * Unmarshaller}.
   */
  ...
  /**
   * Set the schema language. Default is the W3C XML Schema: {@code
  ...
  /**
   * Set whether to lazily initialize the {@link JAXBContext} for this marshaller. Default is {@code
   * false} to initialize on startup; can be switched to {@code true}.
   *
  ...
  /**
   * Specify whether MTOM support should be enabled or not. Default is {@code false}: marshalling
   * using XOP/MTOM not being enabled.
   */
  ...
  /**
   * Specify whether the {@link #supports(Class)} returns {@code true} for the {@link JAXBElement}
   * class.
   *
   * <p>Default is {@code false}, meaning that {@code supports(Class)} always returns {@code false}
   * for {@code JAXBElement} classes (though {@link #supports(Type)} can return {@code true}, since
   * it can obtain the type parameters of {@code JAXBElement}).
   *
   * <p>This property is typically enabled in combination with usage of classes like {@link
   * org.springframework.web.servlet.view.xml.MarshallingView MarshallingView}, since the {@code
   * ModelAndView} does not offer type parameter information at runtime.
   *
   * @see #supports(Class)
   * @see #supports(Type)
   */
  ...
  /**
   * Specify whether the {@link #supports(Class)} should check for {@link
   * XmlRootElement @XmlRootElement} annotations.
   *
   * <p>Default is {@code true}, meaning that {@code supports(Class)} will check for this
   * annotation. However, some JAXB implementations (i.e. EclipseLink MOXy) allow for defining the
   * bindings in an external definition file, thus keeping the classes annotations free. Setting
   * this property to {@code false} supports these JAXB implementations.
   *
   * @see #supports(Class)
   * @see #supports(Type)
   */
   ...
   *
   * <p>Default is {@code false}, meaning that external entities are not resolved. Note that
   * processing of external entities will only be enabled/disabled when the {@code Source} passed to
   * {@link #unmarshal(Source)} is a {@link SAXSource} or {@link StreamSource}. It has no effect for
   * {@link DOMSource} or {@link StAXSource} instances.
   *
   * <p><strong>Note:</strong> setting this option to {@code true} also automatically sets {@link
   * #setSupportDtd} to {@code true}.
   */
    ...
    if (this.jaxbContextProperties != null) {
      if (this.beanClassLoader != null) {
        return JAXBContext.newInstance(
            contextPath, this.beanClassLoader, this.jaxbContextProperties);
      } else {
  ...
  private JAXBContext createJaxbContextFromPackages(String... packagesToScan) throws JAXBException {
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Creating JAXBContext by scanning packages ["
              + StringUtils.arrayToCommaDelimitedString(packagesToScan)
              + "]");
    }
    ClassPathJaxb2TypeScanner scanner =
        new ClassPathJaxb2TypeScanner(this.beanClassLoader, packagesToScan);
  ...
  private Schema loadSchema(Resource[] resources, String schemaLanguage)
    ...
    for (int i = 0; i < resources.length; i++) {
      ...
      Assert.isTrue(
          resource != null && resource.exists(), () -> "Resource does not exist: " + resource);
  ...
  /**
   * Checks whether the given type is a primitive wrapper type. Compare section 8.5.1 of the JAXB2
   * spec.
   */
  ...
  private boolean isStandardClass(Class<?> clazz) {
    return (String.class == clazz
        ...
        ||
        // Source and subclasses should be supported according to the JAXB2 spec, but aren't in the
        // RI
   ...
   *
   * <p>Note: JAXB marshallers are not necessarily thread-safe. This method is public as of 5.2.
   *
   * @since 5.2
  ...
  /**
   * Template method that can be overridden by concrete JAXB marshallers for custom initialization
   * behavior. Gets called after creation of JAXB {@code Marshaller}, and after the respective
   * properties have been set.
   *
   * <p>The default implementation sets the {@link #setMarshallerProperties defined properties}, the
   * {@link #setValidationEventHandler validation event handler}, the {@link #setSchemas schemas},
   * {@link #setMarshallerListener listener}, and {@link #setAdapters adapters}.
   */
  ...
  @Override
  public Object unmarshal(Source source, @Nullable MimeContainer mimeContainer)
      throws XmlMappingException {
   ...
   *
   * <p>Note: JAXB unmarshallers are not necessarily thread-safe. This method is public as of 5.2.
   *
   * @since 5.2
  ...
  protected Object unmarshalStaxSource(Unmarshaller jaxbUnmarshaller, Source staxSource)
      throws JAXBException {
   ...
   * properties have been set.
   *
   * <p>The default implementation sets the {@link #setUnmarshallerProperties defined properties},
   * the {@link #setValidationEventHandler validation event handler}, the {@link #setSchemas
   * schemas}, {@link #setUnmarshallerListener listener}, and {@link #setAdapters adapters}.
   */
  ...
  /**
   * Convert the given {@code JAXBException} to an appropriate exception from the {@code
  ...
  private static class Jaxb2AttachmentMarshaller extends AttachmentMarshaller {
    ...
    @Override
    public String addMtomAttachment(
        DataHandler dataHandler, String elementNamespace, String elementLocalName) {
