 ...
 *
 * <p>By default, XStream does not require any further configuration and can (un)marshal any class
 * on the classpath. As such, it is <b>not recommended to use the {@code XStreamMarshaller} to
 * unmarshal XML from external sources</b> (i.e. the Web), as this can result in <b>security
 * vulnerabilities</b>. If you do use the {@code XStreamMarshaller} to unmarshal external XML, set
 * the {@link #setSupportedClasses(Class[]) supportedClasses} and {@link
 * #setConverters(ConverterMatcher[]) converters} properties (possibly using a {@link
 * CatchAllConverter}) or override the {@link #customizeXStream(XStream)} method to make sure it
 * only accepts the classes you want it to support.
 *
 * <p>Due to XStream's API, it is required to set the encoding used for writing to OutputStreams. It
 * defaults to {@code UTF-8}.
 *
 * <p><b>NOTE:</b> XStream is an XML serialization library, not a data binding library. Therefore,
 * it has limited namespace support. As such, it is rather unsuitable for usage within Web Services.
 *
 * <p>This marshaller requires XStream 1.4.5 or higher, as of Spring 4.3. Note that {@link XStream}
 * construction has been reworked in 4.0, with the stream driver and the class loader getting passed
 * into XStream itself now.
 *
 * @author Peter Meijer
...
public class XStreamMarshaller extends AbstractMarshaller
  ...
  @Nullable private ReflectionProvider reflectionProvider;

  @Nullable private HierarchicalStreamDriver streamDriver;

  @Nullable private HierarchicalStreamDriver defaultDriver;

  @Nullable private Mapper mapper;

  @Nullable private Class<? extends MapperWrapper>[] mapperWrappers;
  ...
  @Nullable private ConverterMatcher[] converters;

  @Nullable private MarshallingStrategy marshallingStrategy;

  @Nullable private Integer mode;

  @Nullable private Map<String, ?> aliases;

  @Nullable private Map<String, ?> aliasesByType;

  @Nullable private Map<String, String> fieldAliases;

  @Nullable private Class<?>[] useAttributeForTypes;

  @Nullable private Map<?, ?> useAttributeFor;

  @Nullable private Map<Class<?>, String> implicitCollections;

  @Nullable private Map<Class<?>, String> omittedFields;
  ...
  @Nullable private Class<?>[] supportedClasses;
   ...
   *
   * <p>As of Spring 4.0, this stream driver will also be passed to the {@link XStream} constructor
  ...
  /**
   * Set one or more custom XStream {@link MapperWrapper} classes. Each of those classes needs to
   * have a constructor with a single argument of type {@link Mapper} or {@link MapperWrapper}.
   *
   * @since 4.0
   */
  ...
  /**
   * Set a custom XStream {@link ConverterLookup} to use. Also used as {@link ConverterRegistry} if
   * the given reference implements it as well.
   *
   * @since 4.0
  ...
  /**
   * Set the {@code Converters} or {@code SingleValueConverters} to be registered with the {@code
   * XStream} instance.
   *
   * @see Converter
   ...
   *
   * <p>Any class that is assignable to this type will be aliased to the same name. Keys are
   * aliases; values are either {@code Class} instances, or String class names.
   *
  ...
  /**
   * Set the types to use XML attributes for. The given map can contain either {@code <String,
   * Class>} pairs, in which case {@link XStream#useAttributeFor(String, Class)} is called.
   * Alternatively, the map can contain {@code <Class, String>} or {@code <Class, List<String>>}
   * pairs, which results in {@link XStream#useAttributeFor(Class, String)} calls.
   */
   ...
   *
   * <p><b>Note</b>: Autodetection implies that the XStream instance is being configured while it is
   * processing the XML streams, and thus introduces a potential concurrency problem.
   *
  ...
  /**
   * Build the native XStream delegate to be used by this marshaller, delegating to {@link
   * #constructXStream()}, {@link #configureXStream} and {@link #customizeXStream}.
   */
  ...
  /**
   * Construct an XStream instance, either using one of the standard constructors or creating a
   * custom subclass.
   *
  ...
  protected XStream constructXStream() {
    return new XStream(
        ...
        getDefaultDriver(),
        new ClassLoaderReference(this.beanClassLoader),
        this.mapper,
        ...
        if (mapperWrappers != null) {
          for (Class<? extends MapperWrapper> mapperWrapper : mapperWrappers) {
            ...
            try {
              ...
              throw new IllegalStateException(
                  "Failed to construct MapperWrapper: " + mapperWrapper);
            }
          }
        }
        return mapperToWrap;
  ...
  protected void configureXStream(XStream xstream) {
    ...
    if (this.implicitCollections != null) {
      this.implicitCollections.forEach(
          (key, fields) -> {
  ...
  private Map<String, Class<?>> toClassMap(Map<String, ?> map) throws ClassNotFoundException {
    ...
    for (Map.Entry<String, ?> entry : map.entrySet()) {
      ...
      if (value instanceof Class) {
        ...
        throw new IllegalArgumentException(
            "Unknown value [" + value + "] - expected String or Class");
      }
      result.put(key, type);
    }
    return result;
   ...
   *
   * <p><b>NOTE: This method has been marked as final as of Spring 4.0.</b> It can be used to access
   * the fully configured XStream for marshalling but not configuration purposes anymore.
   */
  ...
  @Override
  public void marshalOutputStream(Object graph, OutputStream outputStream)
      throws XmlMappingException, IOException {
    marshalOutputStream(graph, outputStream, null);
  }

  public void marshalOutputStream(
      Object graph, OutputStream outputStream, @Nullable DataHolder dataHolder)
      throws XmlMappingException, IOException {
  ...
  /**
   * Marshals the given graph to the given XStream HierarchicalStreamWriter. Converts exceptions
   * using {@link #convertXStreamException}.
   */
  private void doMarshal(
      Object graph, HierarchicalStreamWriter streamWriter, @Nullable DataHolder dataHolder) {
    try {
  ...
  /**
   * Unmarshals the given graph to the given XStream HierarchicalStreamWriter. Converts exceptions
   * using {@link #convertXStreamException}.
   */
  private Object doUnmarshal(
      HierarchicalStreamReader streamReader, @Nullable DataHolder dataHolder) {
    try {
  ...
  /**
   * Convert the given XStream exception to an appropriate exception from the {@code
   ...
   * @param marshalling indicates whether the exception occurs during marshalling ({@code true}), or
   *     unmarshalling ({@code false})
