...
/**
 * {@link Source} implementation that uses a {@link Marshaller}.Can be constructed with a {@code
 ...
 * {@code SAXSource} is <strong>not supported</strong>. In general, the only supported operation on
 * this class is to use the {@code XMLReader} obtained via {@link #getXMLReader()} to parse the
 * input source obtained via {@link #getInputSource()}. Calling {@link #setXMLReader(XMLReader)} or
 * {@link #setInputSource(InputSource)} will result in {@code UnsupportedOperationException}s.
 *
    ...
    private void parse() throws SAXException {
      ...
      try {
        ...
        SAXParseException saxException =
            new SAXParseException(ex.getMessage(), null, null, -1, -1, ex);
