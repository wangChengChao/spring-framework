...
public abstract class AbstractUnmarshallerTests<U extends Unmarshaller> {
  ...
  public void unmarshalDomSource() throws Exception {
    ...
    Element flightsElement =
        document.createElementNS("http://samples.springframework.org/flight", "tns:flights");
    document.appendChild(flightsElement);
    Element flightElement =
        document.createElementNS("http://samples.springframework.org/flight", "tns:flight");
    flightsElement.appendChild(flightElement);
    Element numberElement =
        document.createElementNS("http://samples.springframework.org/flight", "tns:number");
  ...
  public void unmarshalStreamSourceInputStream() throws Exception {
    StreamSource source =
        new StreamSource(new ByteArrayInputStream(INPUT_STRING.getBytes("UTF-8")));
    Object flights = unmarshaller.unmarshal(source);
    testFlights(flights);
  }
  ...
  public void unmarshalPartialStaxSourceXmlStreamReader() throws Exception {
    ...
    assertThat(streamReader.getName())
        .as("Invalid element")
