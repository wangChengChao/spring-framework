...
public class XStreamMarshallerTests {
  ...
  public void marshalSaxResult() throws Exception {
    ...
    ordered
        ...
        .startElement(eq(""), eq("flight"), eq("flight"), isA(Attributes.class));
    ordered
        .verify(contentHandler)
        .startElement(eq(""), eq("flightNumber"), eq("flightNumber"), isA(Attributes.class));
  ...
  public void fieldAliases() throws Exception {
    marshaller.setFieldAliases(
        Collections.singletonMap(
            "org.springframework.oxm.xstream.Flight.flightNumber", "flightNo"));
    Writer writer = new StringWriter();
    marshaller.marshal(flight, new StreamResult(writer));
  ...
  public void jettisonDriver() throws Exception {
    ...
    assertThat(writer.toString())
        .as("Invalid result")
        .isEqualTo("{\"flight\":{\"flightNumber\":42}}");
  ...
  private static void assertXpathExists(String xPathExpression, String inXMLString) {
    ...
    assertThat(count(nodes) > 0)
        .as("Expecting to find matches for Xpath " + xPathExpression)
        .isTrue();
  }
