...
/**
 * Mock object based abstract class for RowMapper tests. Initializes mock objects and verifies
 * results.
 *
...
public abstract class AbstractRowMapperTests {
  ...
  protected void verifyPerson(Person bean) throws Exception {
    ...
    assertThat(bean.getBirth_date())
        .usingComparator(Date::compareTo)
        .isEqualTo(new java.util.Date(1221222L));
  ...
  protected static class Mock {
    ...
    public Mock(MockType type) throws Exception {
      ...
      given(resultSet.getObject(anyInt(), any(Class.class)))
      ...
      given(resultSetMetaData.getColumnLabel(1))
          .willReturn(type == MockType.THREE ? "Last Name" : "name");
