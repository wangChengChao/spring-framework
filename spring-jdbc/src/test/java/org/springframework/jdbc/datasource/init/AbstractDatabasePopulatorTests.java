...
/**
 * Abstract base class for integration tests for {@link ResourceDatabasePopulator} and {@link
...
abstract class AbstractDatabasePopulatorTests extends AbstractDatabaseInitializationTests {
  ...
  void constructorWithMultipleScriptResources() throws Exception {
    final ResourceDatabasePopulator populator =
        new ResourceDatabasePopulator(usersSchema(), resource("users-data-with-comments.sql"));
  ...
  private void assertTestDatabaseCreated(String name) {
    assertThat(jdbcTemplate.queryForObject("select NAME from T_TEST", String.class))
        .isEqualTo(name);
  }
}
