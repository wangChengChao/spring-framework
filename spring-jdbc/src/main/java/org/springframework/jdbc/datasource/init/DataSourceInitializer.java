...
/**
 * Used to {@linkplain #setDatabasePopulator set up} a database during initialization and {@link
 * #setDatabaseCleaner clean up} a database during destruction.
 *
 * @author Dave Syer
 * @author Sam Brannen
 * @since 3.0
...
public class DataSourceInitializer implements InitializingBean, DisposableBean {
  ...
  @Nullable private DatabasePopulator databaseCleaner;

  private boolean enabled = true;

  /**
   * The {@link DataSource} for the database to populate when this component is initialized and to
   * clean up when this component is shut down.
   *
  ...
  /**
   * Set the {@link DatabasePopulator} to execute during the bean destruction phase, cleaning up the
  ...
  /**
   * Flag to explicitly enable or disable the {@linkplain #setDatabasePopulator database populator}
   * and {@linkplain #setDatabaseCleaner database cleaner}.
   *
   * @param enabled {@code true} if the database populator and database cleaner should be called on
