...
/**
 * Maps well-known {@linkplain EmbeddedDatabaseType embedded database types} to {@link
 * EmbeddedDatabaseConfigurer} strategies.
 *
 * @author Keith Donald
      ...
      switch (type) {
        ...
        default:
          throw new UnsupportedOperationException(
              "Embedded database type [" + type + "] is not supported");
      }
    } catch (ClassNotFoundException | NoClassDefFoundError ex) {
      throw new IllegalStateException(
          "Driver for test database type [" + type + "] is not available", ex);
    }
  }
}
