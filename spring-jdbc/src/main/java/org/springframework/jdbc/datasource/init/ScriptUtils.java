...
public abstract class ScriptUtils {
   ...
   *
   * <p>Used if neither a custom separator nor the {@link #DEFAULT_STATEMENT_SEPARATOR} is present
   * in a given script.
   */
   ...
   *
   * <p>This value may be supplied as the {@code separator} to {@link #executeSqlScript(Connection,
   * EncodedResource, boolean, boolean, String, String, String, String)} to denote that an SQL
   * script contains a single statement (potentially spanning multiple lines) with no explicit
   * statement separator. Note that such a script should not actually contain this value; it is
   * merely a <em>virtual</em> statement separator.
   */
  ...
  /**
   * Split an SQL script into separate statements delimited by the provided separator string. Each
   ...
   * individual statement will be added to the provided {@code List}.
   *
   * <p>Within the script, the provided {@code commentPrefix} will be honored: any text beginning
   * with the comment prefix and extending to the end of the line will be omitted from the output.
   * Similarly, the provided {@code blockCommentStartDelimiter} and {@code blockCommentEndDelimiter}
   * delimiters will be honored: any text enclosed in a block comment will be omitted from the
   * output. In addition, multiple adjacent whitespace characters will be collapsed into a single
   * space.
   *
   ...
   * individual statement will be added to the provided {@code List}.
   *
   * <p>Within the script, the provided {@code commentPrefixes} will be honored: any text beginning
   * with one of the comment prefixes and extending to the end of the line will be omitted from the
   * output. Similarly, the provided {@code blockCommentStartDelimiter} and {@code
   * blockCommentEndDelimiter} delimiters will be honored: any text enclosed in a block comment will
   * be omitted from the output. In addition, multiple adjacent whitespace characters will be
  ...
  public static void splitSqlScript(
    ...
    for (int i = 0; i < commentPrefixes.length; i++) {
      Assert.hasText(
          commentPrefixes[i], "'commentPrefixes' must not contain null or empty elements");
    }
    Assert.hasText(
        blockCommentStartDelimiter, "'blockCommentStartDelimiter' must not be null or empty");
    Assert.hasText(
        blockCommentEndDelimiter, "'blockCommentEndDelimiter' must not be null or empty");
  ...
  /**
   * Read a script from the given resource, using "{@code --}" as the comment prefix and "{@code ;}"
  ...
  /**
   * Read a script from the provided resource, using the supplied comment prefixes and statement
   * separator, and build a {@code String} containing the lines.
   *
   * <p>Lines <em>beginning</em> with one of the comment prefixes are excluded from the results;
   * however, line comments anywhere else &mdash; for example, within a statement &mdash; will be
  ...
  public static String readScript(
      ...
      @Nullable String lineCommentPrefix,
      @Nullable String separator,
      @Nullable String blockCommentEndDelimiter)
      throws IOException {

    String[] lineCommentPrefixes =
        (lineCommentPrefix != null) ? new String[] {lineCommentPrefix} : null;
    return readScript(lineNumberReader, lineCommentPrefixes, separator, blockCommentEndDelimiter);
  }

  /**
   * Read a script from the provided {@code LineNumberReader}, using the supplied comment prefixes
   ...
   *
   * @param lineNumberReader the {@code LineNumberReader} containing the script to be processed
   * @param lineCommentPrefixes the prefixes that identify comments in the SQL script (typically
   *     "--")
   * @param separator the statement separator in the SQL script (typically ";")
   * @param blockCommentEndDelimiter the <em>end</em> block comment delimiter
   * @return a {@code String} containing the script lines
  ...
  public static String readScript(
      LineNumberReader lineNumberReader,
      @Nullable String[] lineCommentPrefixes,
      @Nullable String separator,
      @Nullable String blockCommentEndDelimiter)
      throws IOException {
    ...
    while (currentStatement != null) {
      if ((blockCommentEndDelimiter != null && currentStatement.contains(blockCommentEndDelimiter))
          || (lineCommentPrefixes != null
              && !startsWithAny(currentStatement, lineCommentPrefixes, 0))) {
  ...
  private static void appendSeparatorToScriptIfNecessary(
      StringBuilder scriptBuilder, @Nullable String separator) {
    if (separator == null) {
      return;
    }
  ...
  /**
   * Execute the given SQL script using default settings for statement separators, comment
   ...
   *
   * <p><strong>Warning</strong>: this method does <em>not</em> release the provided {@link
   * Connection}.
   *
   ...
   *     {@code DROP} statement
   * @param commentPrefix the prefix that identifies single-line comments in the SQL script
   *     (typically "--")
   * @param separator the script statement separator; defaults to {@value
   *     #DEFAULT_STATEMENT_SEPARATOR} if not specified and falls back to {@value
   *     #FALLBACK_STATEMENT_SEPARATOR} as a last resort; may be set to {@value
   *     #EOF_STATEMENT_SEPARATOR} to signal that the script contains a single statement without a
   *     separator
   ...
   * @param ignoreFailedDrops whether or not to continue in the event of specifically an error on a
   *     {@code DROP} statement
   * @param commentPrefixes the prefixes that identify single-line comments in the SQL script
   *     (typically "--")
   * @param separator the script statement separator; defaults to {@value
   *     #DEFAULT_STATEMENT_SEPARATOR} if not specified and falls back to {@value
   *     #FALLBACK_STATEMENT_SEPARATOR} as a last resort; may be set to {@value
  ...
  public static void executeSqlScript(
    ...
    try {
      ...
      if (!EOF_STATEMENT_SEPARATOR.equals(separator)
          && !containsSqlScriptDelimiters(script, separator)) {
