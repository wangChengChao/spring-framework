 ...
 *   <li>Call {@link #populate} or {@link #execute} to initialize or clean up the database using the
 *       configured scripts.
 * </ul>
 *
 * @author Keith Donald
...
public class ResourceDatabasePopulator implements DatabasePopulator {
  ...
  /**
   * Construct a new {@code ResourceDatabasePopulator} with default settings for the supplied
   * scripts.
   *
  ...
  public ResourceDatabasePopulator(
      ...
      @Nullable String sqlScriptEncoding,
      Resource... scripts) {
   ...
   * <p>Defaults to {@code "/*"}.
   *
   * @param blockCommentStartDelimiter the start delimiter for block comments (never {@code null} or
   *     empty)
   * @since 4.0.3
   ...
   *
   * @param blockCommentEndDelimiter the end delimiter for block comments (never {@code null} or
   *     empty)
   * @since 4.0.3
   ...
   *
   * <p>The default is {@code false} so that if the populator runs accidentally, it will fail fast
   * if a script starts with a {@code DROP} statement.
   *
