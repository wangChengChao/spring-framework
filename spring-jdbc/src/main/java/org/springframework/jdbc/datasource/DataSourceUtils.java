...
/**
 * Helper class that provides static methods for obtaining JDBC Connections from a {@link
 * javax.sql.DataSource}. Includes special support for Spring-managed transactional Connections,
 * e.g. managed by {@link DataSourceTransactionManager} or {@link
 * org.springframework.transaction.jta.JtaTransactionManager}.
 *
 * <p>Used internally by Spring's {@link org.springframework.jdbc.core.JdbcTemplate}, Spring's JDBC
 * operation objects and the JDBC {@link DataSourceTransactionManager}. Can also be used directly in
 * application code.
 *
 * @author Rod Johnson
...
public abstract class DataSourceUtils {
  ...
  /**
   * Obtain a Connection from the given DataSource. Translates SQLExceptions into the Spring
   * hierarchy of unchecked generic data access exceptions, simplifying calling code and making any
   ...
   * synchronization is active, e.g. when running within a {@link
   * org.springframework.transaction.jta.JtaTransactionManager JTA} transaction).
   *
  ...
  /**
   * Actually fetch a {@link Connection} from the given {@link DataSource}, defensively turning an
   * unexpected {@code null} return value from {@link DataSource#getConnection()} into an {@link
  ...
  public static void resetConnectionAfterTransaction(
      Connection con, @Nullable Integer previousIsolationLevel) {
    Assert.notNull(con, "No Connection specified");
    try {
      ...
      if (previousIsolationLevel != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Resetting isolation level of JDBC Connection ["
                  + con
                  + "] to "
   ...
   */
  public static void doCloseConnection(Connection con, @Nullable DataSource dataSource)
      throws SQLException {
  ...
  /**
   * Return the innermost target Connection of the given Connection. If the given Connection is a
   * proxy, it will be unwrapped until a non-proxy Connection is found. Otherwise, the passed-in
   * Connection will be returned as-is.
   *
  ...
  /**
   * Determine the connection synchronization order to use for the given DataSource. Decreased for
   * every level of nesting that a DataSource has, checked through the level of DelegatingDataSource
   * nesting.
   *
  ...
  /**
   * Callback for resource cleanup at the end of a non-native JDBC transaction (e.g. when
