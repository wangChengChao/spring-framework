 ...
 *
 * <p>Callers are guaranteed that the returned database has been fully initialized and populated.
 *
 * <p>The factory can be configured as follows:
 *
 * <ul>
 *   <li>Call {@link #generateUniqueDatabaseName} to set a unique, random name for the database.
 *   <li>Call {@link #setDatabaseName} to set an explicit name for the database.
 *   <li>Call {@link #setDatabaseType} to set the database type if you wish to use one of the
 *       supported types.
 *   <li>Call {@link #setDatabaseConfigurer} to configure support for a custom embedded database
 *       type.
 *   <li>Call {@link #setDatabasePopulator} to change the algorithm used to populate the database.
 *   <li>Call {@link #setDataSourceFactory} to change the type of {@link DataSource} used to connect
 *       to the database.
 * </ul>
 *
 * <p>After configuring the factory, call {@link #getDatabase()} to obtain a reference to the {@link
...
public class EmbeddedDatabaseFactory {
  ...
  @Nullable private EmbeddedDatabaseConfigurer databaseConfigurer;

  @Nullable private DatabasePopulator databasePopulator;

  @Nullable private DataSource dataSource;

  /**
   * Set the {@code generateUniqueDatabaseName} flag to enable or disable generation of a
   * pseudo-random unique ID to be used as the database name.
   *
   * <p>Setting this flag to {@code true} overrides any explicit name set via {@link
   * #setDatabaseName}.
   *
   * @since 4.2
   * @see #setDatabaseName
   ...
   *
   * <p>Will be overridden if the {@code generateUniqueDatabaseName} flag has been set to {@code
   * true}.
   *
  ...
  /**
   * Set the factory to use to create the {@link DataSource} instance that connects to the embedded
   * database.
   *
  ...
  /**
   * Factory method that returns the {@linkplain EmbeddedDatabase embedded database} instance, which
   * is also a {@link DataSource}.
   */
   ...
   *
   * <p>If the {@code generateUniqueDatabaseName} flag has been set to {@code true}, the current
   * value of the {@linkplain #setDatabaseName database name} will be overridden with an
   * auto-generated name.
   *
   * <p>Subclasses may call this method to force initialization; however, this method should only be
   * invoked once.
   *
   * <p>After calling this method, {@link #getDataSource()} returns the {@link DataSource} providing
  ...
  protected void initDatabase() {
    ...
    if (logger.isInfoEnabled()) {
      if (this.dataSource instanceof SimpleDriverDataSource) {
        ...
        logger.info(
            String.format(
                "Starting embedded database: url='%s', username='%s'",
    ...
    if (this.dataSource != null) {
      if (logger.isInfoEnabled()) {
        if (this.dataSource instanceof SimpleDriverDataSource) {
          logger.info(
              String.format(
                  "Shutting down embedded database: url='%s'",
  ...
  /**
   * Hook that gets the {@link DataSource} that provides the connectivity to the embedded database.
   *
   * <p>Returns {@code null} if the {@code DataSource} has not been initialized or if the database
   * has been shut down. Subclasses may call this method to access the {@code DataSource} instance
   * directly.
   */
  @Nullable
