...
/**
 * Thrown by {@link ScriptUtils} if a statement in an SQL script failed when executing it against
 * the target database.
 *
...
public class ScriptStatementFailedException extends ScriptException {
   ...
   * @param stmtNumber the statement number in the SQL script (i.e., the n<sup>th</sup> statement
   *     present in the resource)
   * @param encodedResource the resource from which the SQL statement was read
   * @param cause the underlying cause of the failure
   */
  public ScriptStatementFailedException(
      String stmt, int stmtNumber, EncodedResource encodedResource, Throwable cause) {
  ...
  public static String buildErrorMessage(
      String stmt, int stmtNumber, EncodedResource encodedResource) {
    return String.format(
        "Failed to execute SQL script statement #%s of %s: %s", stmtNumber, encodedResource, stmt);
  }
}
