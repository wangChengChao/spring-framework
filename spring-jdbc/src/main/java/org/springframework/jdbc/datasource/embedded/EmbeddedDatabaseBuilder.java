...
public class EmbeddedDatabaseBuilder {
   ...
   *
   * <p>If the configuration for this builder is reused across multiple application contexts within
   * a single JVM, this flag should be <em>enabled</em> (i.e., set to {@code true}) in order to
   * ensure that each application context gets its own embedded database.
   *
   ...
   *
   * <p>Defaults to {@link EmbeddedDatabaseFactory#DEFAULT_DATABASE_NAME} if not called.
   *
   * <p>Will be overridden if the {@code generateUniqueName} flag has been set to {@code true}.
   *
   ...
   * database.
   *
   * <p>Defaults to {@link SimpleDriverDataSourceFactory} but can be overridden, for example to
   ...
   *
   * <p>The default scripts are {@code "schema.sql"} to create the database schema and {@code
   * "data.sql"} to populate the database with data.
   *
   * @return {@code this}, to facilitate method chaining
   */
  ...
  /**
   * Specify the character encoding used in all SQL scripts, if different from the platform
   * encoding.
   *
   ...
   * may be set to {@link ScriptUtils#EOF_STATEMENT_SEPARATOR} to signal that each script contains a
  ...
  /**
   * Specify that all failures which occur while executing SQL scripts should be logged but should
   * not cause a failure.
   *
  ...
  /**
   * Specify that a failed SQL {@code DROP} statement within an executed script can be ignored.
   *
   * <p>This is useful for a database whose SQL dialect does not support an {@code IF EXISTS} clause
   * in a {@code DROP} statement.
   *
   * <p>The default is {@code false} so that {@link #build building} will fail fast if a script
   * starts with a {@code DROP} statement.
   *
