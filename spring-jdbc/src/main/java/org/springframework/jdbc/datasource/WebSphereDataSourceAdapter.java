...
/**
 * {@link DataSource} implementation that delegates all calls to a WebSphere target {@link
 * DataSource}, typically obtained from JNDI, applying a current isolation level and/or current user
 * credentials to every Connection obtained from it.
 *
 * <p>Uses IBM-specific API to get a JDBC Connection with a specific isolation level (and read-only
 * flag) from a WebSphere DataSource (<a
 * href="https://publib.boulder.ibm.com/infocenter/wasinfo/v5r1//topic/com.ibm.websphere.base.doc/info/aes/ae/rdat_extiapi.html">IBM
 * code example</a>). Supports the transaction-specific isolation level exposed by {@link
 * org.springframework.transaction.support.TransactionSynchronizationManager#getCurrentTransactionIsolationLevel()}.
 * It's also possible to specify a default isolation level, to be applied when the current
 * Spring-managed transaction does not define a specific isolation level.
 *
 * <p>Usage example, defining the target DataSource as an inner-bean JNDI lookup (of course, you can
 * link to any WebSphere DataSource through a bean reference):
 *
 * <pre class="code">
...
public class WebSphereDataSourceAdapter extends IsolationLevelDataSourceAdapter {
  ...
  /**
   * This constructor retrieves the WebSphere JDBC connection spec API, so we can get obtain
   * specific WebSphere Connections using reflection.
   */
  public WebSphereDataSourceAdapter() {
    try {
      this.wsDataSourceClass =
          getClass().getClassLoader().loadClass("com.ibm.websphere.rsadapter.WSDataSource");
      Class<?> jdbcConnSpecClass =
          getClass().getClassLoader().loadClass("com.ibm.websphere.rsadapter.JDBCConnectionSpec");
      Class<?> wsrraFactoryClass =
          getClass().getClassLoader().loadClass("com.ibm.websphere.rsadapter.WSRRAFactory");
      ...
      throw new IllegalStateException(
          "Could not initialize WebSphereDataSourceAdapter because WebSphere API classes are not available: "
              + ex);
    }
  }
  ...
  /**
   * Builds a WebSphere JDBCConnectionSpec object for the current settings and calls {@code
  ...
  protected Connection doGetConnection(@Nullable String username, @Nullable String password)
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Obtaining JDBC Connection from WebSphere DataSource ["
              + getTargetDataSource()
   ...
   *
   * <p>The default implementation uses reflection to apply the given settings. Can be overridden in
   * subclasses to customize the JDBCConnectionSpec object (<a
   * href="https://publib.boulder.ibm.com/infocenter/wasinfo/v6r0/topic/com.ibm.websphere.javadoc.doc/public_html/api/com/ibm/websphere/rsadapter/JDBCConnectionSpec.html">JDBCConnectionSpec
   * javadoc</a>; <a
   * href="https://www.ibm.com/developerworks/websphere/library/techarticles/0404_tang/0404_tang.html">IBM
  ...
  @Nullable
  private static Object invokeJdbcMethod(
      Method method, @Nullable Object target, @Nullable Object... args) throws SQLException {
    try {
