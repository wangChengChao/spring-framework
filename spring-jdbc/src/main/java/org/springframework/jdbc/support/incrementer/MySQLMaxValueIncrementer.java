 ...
 * with the equivalent of an auto-increment column. Note: If you use this class, your MySQL key
 ...
 * auto-generated key. The storage engine used by the sequence table can be MYISAM or INNODB since
 * the sequences are allocated using a separate connection without being affected by any other
...
public class MySQLMaxValueIncrementer extends AbstractColumnMaxValueIncrementer {
   ...
   *
   * <p>{@code true} is necessary to support transactional storage engines, using an isolated
   * separate transaction for the increment operation. {@code false} is sufficient if the storage
   * engine of the sequence table is non-transactional (like MYISAM), avoiding the effort of
   * acquiring an extra {@code Connection} for the increment operation.
   *
