...
/**
 * Base implementation of {@link DataFieldMaxValueIncrementer} that delegates to a single {@link
 * #getNextKey} template method that returns a {@code long}. Uses longs for String values, padding
 * with zeroes if required.
 *
...
public abstract class AbstractDataFieldMaxValueIncrementer
  ...
  /**
   * Set the padding length, i.e. the length to which a string result should be pre-pended with
   * zeroes.
   */
   ...
   *
   * @return the key to use as a long. It will eventually be converted later in another format by
   *     the public concrete methods of this class.
   */
