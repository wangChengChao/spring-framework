...
/**
 * Generic utility methods for working with JDBC. Mainly for internal use within the framework, but
 * also useful for custom JDBC access code.
 *
...
public abstract class JdbcUtils {
   ...
   *
   * <p>Uses the specifically typed ResultSet accessor methods, falling back to {@link
   * #getResultSetValue(java.sql.ResultSet, int)} for unknown types.
   *
   * <p>Note that the returned value may not be assignable to the specified required type, in case
   * of an unknown type. Calling code needs to deal with this case appropriately, e.g. throwing a
   ...
   * Since this method is called before the exception translation feature is configured for a
   * datasource, this method can not rely on the SQLException translation functionality.
   *
  ...
  /**
   * Call the specified method on DatabaseMetaData for the given DataSource, and extract the
   * invocation result.
   *
   * @param dataSource the DataSource to extract meta-data for
   ...
   * @throws MetaDataAccessException if we couldn't access the DatabaseMetaData or failed to invoke
   *     the specified method
   * @see java.sql.DatabaseMetaData
   */
  @SuppressWarnings("unchecked")
  public static <T> T extractDatabaseMetaData(
      DataSource dataSource, final String metaDataMethodName) throws MetaDataAccessException {
            ...
            dbmd -> {
              try {
                ...
                throw new MetaDataAccessException(
                    "No method named '"
                        + metaDataMethodName
                        + "' found on DatabaseMetaData instance ["
                        + dbmd
                        + "]",
                    ex);
                ...
                throw new MetaDataAccessException(
                    "Invocation of DatabaseMetaData method '" + metaDataMethodName + "' failed",
                    ex);
              }
            });
  }
   ...
   *
   * <p>Typically invoked right before execution of a given set of statements: to decide whether the
   * set of SQL statements should be executed through the JDBC 2.0 batch mechanism or simply in a
   * traditional one-by-one fashion.
   *
   * <p>Logs a warning if the "supportsBatchUpdates" methods throws an exception and simply returns
  ...
  /**
   * Extract a common name for the target database in use even if various drivers/platforms provide
   * varying names at runtime.
   *
   ...
   * @return the corresponding constant name in {@link java.sql.Types} (e.g. "VARCHAR"/"NUMERIC"),
   *     or {@code null} if not resolvable
   * @since 5.2
   */
  @Nullable
   ...
   * ResultSetMetaData.
   *
   * <p>This method implementation takes into account recent clarifications expressed in the JDBC
   * 4.0 specification:
   *
   * <p><i>columnLabel - the label for the column specified with the SQL AS clause. If the SQL AS
  ...
  /**
   * Convert a column name with underscores to the corresponding property name using "camel case". A
   * name like "customer_number" would match a "customerNumber" property name.
   *
