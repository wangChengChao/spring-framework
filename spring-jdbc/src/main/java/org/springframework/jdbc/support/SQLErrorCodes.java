...
/**
 * JavaBean for holding JDBC error codes for a particular database. Instances of this class are
 * normally loaded through a bean factory.
 *
 * <p>Used by Spring's {@link SQLErrorCodeSQLExceptionTranslator}. The file "sql-error-codes.xml" in
 * this package contains default {@code SQLErrorCodes} instances for various databases.
 *
...
public class SQLErrorCodes {
  ...
  @Nullable private String[] databaseProductNames;
  ...
  @Nullable private CustomSQLErrorCodesTranslation[] customTranslations;

  @Nullable private SQLExceptionTranslator customSqlExceptionTranslator;

  /**
   * Set this property if the database name contains spaces, in which case we can not use the bean
   * name for lookup.
   */
  ...
  public String getDatabaseProductName() {
    return (this.databaseProductNames != null && this.databaseProductNames.length > 0
        ? this.databaseProductNames[0]
        : null);
  }

  /**
   * Set this property to specify multiple database names that contains spaces, in which case we can
   * not use bean names for lookup.
   */
  ...
  /**
   * Set this property to true for databases that do not provide an error code but that do provide
   * SQL State (this includes PostgreSQL).
   */
  ...
  public void setDataAccessResourceFailureCodes(String... dataAccessResourceFailureCodes) {
    this.dataAccessResourceFailureCodes =
        StringUtils.sortStringArray(dataAccessResourceFailureCodes);
  }
  ...
  public void setTransientDataAccessResourceCodes(String... transientDataAccessResourceCodes) {
    this.transientDataAccessResourceCodes =
        StringUtils.sortStringArray(transientDataAccessResourceCodes);
  }
  ...
  public void setCannotSerializeTransactionCodes(String... cannotSerializeTransactionCodes) {
    this.cannotSerializeTransactionCodes =
        StringUtils.sortStringArray(cannotSerializeTransactionCodes);
  }
  ...
  public void setCustomSqlExceptionTranslator(
      @Nullable SQLExceptionTranslator customSqlExceptionTranslator) {
