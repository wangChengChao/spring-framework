...
/**
 * Bean that checks if a database has already started up. To be referenced via "depends-on" from
 * beans that depend on database startup, like a Hibernate SessionFactory or custom data access
 * objects that access a DataSource directly.
 *
 * <p>Useful to defer application initialization until a database has started up. Particularly
 * appropriate for waiting on a slowly starting Oracle database.
 *
...
public class DatabaseStartupValidator implements InitializingBean {
  ...
  @Nullable private String validationQuery;
  ...
  /**
   * Set the timeout (in seconds) after which a fatal exception will be thrown. Default is {@value
   * #DEFAULT_TIMEOUT}.
   */
  ...
  /**
   * Check whether the validation query can be executed on a Connection from the specified
   * DataSource, with the specified interval between checks, until the specified timeout.
   */
  @Override
        ...
        try {
          ...
          if (con == null) {
            throw new CannotGetJdbcConnectionException(
                "Failed to execute validation query: "
          ...
          if (logger.isInfoEnabled()) {
            ...
            if (rest > this.interval) {
              logger.info(
                  "Database has not started up yet - retrying in "
                      + this.interval
                      + " seconds (timeout in "
                      + rest
                      + " seconds)");
