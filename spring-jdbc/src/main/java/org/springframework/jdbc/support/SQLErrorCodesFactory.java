...
/**
 * Factory for creating {@link SQLErrorCodes} based on the "databaseProductName" taken from the
 * {@link java.sql.DatabaseMetaData}.
 *
 * <p>Returns {@code SQLErrorCodes} populated with vendor codes defined in a configuration file
 * named "sql-error-codes.xml". Reads the default file in this package if not overridden by a file
 * in the root of the class path (for example in the "/WEB-INF/classes" directory).
 *
...
public class SQLErrorCodesFactory {
  ...
  /**
   * The name of custom SQL error codes file, loading from the root of the class path (e.g. from the
  ...
  /**
   * Map to hold error codes for all databases defined in the config file. Key is the database
   * product name, value is the SQLErrorCodes instance.
   */
   ...
   *
   * <p>Not public to enforce Singleton design pattern. Would be private except to allow testing via
   * overriding the {@link #loadResource(String)} method.
   *
   ...
   *
   * <p><b>Not to be overridden by application developers, who should obtain instances of this class
   * from the static {@link #getInstance()} method.</b>
   *
   * <p>Protected for testability.
   *
   * @param path resource path; either a custom path or one of either {@link
   *     #SQL_ERROR_CODE_DEFAULT_PATH} or {@link #SQL_ERROR_CODE_OVERRIDE_PATH}.
  ...
  /**
   * Return {@link SQLErrorCodes} for the given {@link DataSource}, evaluating "databaseProductName"
   * from the {@link java.sql.DatabaseMetaData}, or an empty error codes instance if no {@code
   * SQLErrorCodes} were found.
   *
   ...
   *
   * @param dataSource the {@code DataSource} identifying the database
   * @param databaseName the corresponding database name as stated in the error codes definition
  ...
  public SQLErrorCodes registerDatabase(DataSource dataSource, String databaseName) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Caching SQL error codes for DataSource ["
              + identify(dataSource)
   ...
   *
   * @param dataSource the {@code DataSource} identifying the database
   * @return the corresponding {@code SQLErrorCodes} object that got removed, or {@code null} if not
   *     registered
   * @since 4.3.5
  ...
  /**
   * Build an identification String for the given {@link DataSource}, primarily for logging
   * purposes.
   *
      ...
      if (errorCodes.getCustomSqlExceptionTranslator() != null && logger.isDebugEnabled()) {
        logger.debug(
            "Overriding already defined custom translator '"
                ...
                + "' found in the CustomSQLExceptionTranslatorRegistry for database '"
                + databaseName
                + "'");
