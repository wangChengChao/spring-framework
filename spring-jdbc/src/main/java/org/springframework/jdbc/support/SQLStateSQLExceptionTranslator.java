...
/**
 * {@link SQLExceptionTranslator} implementation that analyzes the SQL state in the {@link
 * SQLException} based on the first two digits (the SQL state "class"). Detects standard SQL state
 * values and well-known vendor-specific SQL states.
 *
 * <p>Not able to diagnose all problems, but is portable between databases and does not require
 * special initialization (no database vendor detection, etc.). For more precise translation,
 * consider {@link SQLErrorCodeSQLExceptionTranslator}.
 *
 * @author Rod Johnson
...
public class SQLStateSQLExceptionTranslator extends AbstractFallbackSQLExceptionTranslator {
  ...
  static {
    ...
    DATA_ACCESS_RESOURCE_FAILURE_CODES.add(
