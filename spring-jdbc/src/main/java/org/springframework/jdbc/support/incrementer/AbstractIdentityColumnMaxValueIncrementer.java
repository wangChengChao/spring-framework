...
/**
 * Abstract base class for {@link DataFieldMaxValueIncrementer} implementations which are based on
 * identity columns in a sequence-like table.
 *
...
public abstract class AbstractIdentityColumnMaxValueIncrementer
  ...
  /**
   * Specify whether to delete the entire range below the current maximum key value ({@code false} -
   * the default), or the specifically generated values ({@code true}). The former mode will use a
   * where range clause whereas the latter will use an in clause starting with the lowest value
   * minus 1, just preserving the maximum value.
   */
  ...
  /**
   * Return whether to delete the entire range below the current maximum key value ({@code false} -
   * the default), or the specifically generated values ({@code true}).
   */
          ...
          try {
            if (!rs.next()) {
              throw new DataAccessResourceFailureException(
                  "Identity statement failed after inserting");
            }
   ...
   *
   * <p>The default implementation either deletes the entire range below the current maximum value,
   * or the specifically generated values (starting with the lowest minus 1, just preserving the
   * maximum value) - according to the {@link #isDeleteSpecificValues()} setting.
   *
   * @param values the currently generated key values (the number of values corresponds to {@link
   *     #getCacheSize()})
   * @return the SQL statement to use
   */
