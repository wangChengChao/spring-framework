...
/**
 * Implementation of {@link SQLExceptionTranslator} that analyzes vendor-specific error codes. More
 * precise than an implementation based on SQL state, but heavily vendor-specific.
 *
 * <p>This class applies the following matching rules:
 *
 * <ul>
 *   <li>Try custom translation implemented by any subclass. Note that this class is concrete and is
 *       typically used itself, in which case this rule doesn't apply.
 *   <li>Apply error code matching. Error codes are obtained from the SQLErrorCodesFactory by
 *       default. This factory loads a "sql-error-codes.xml" file from the class path, defining
 *       error code mappings for database names from database meta-data.
 *   <li>Fallback to a fallback translator. {@link SQLStateSQLExceptionTranslator} is the default
 *       fallback translator, analyzing the exception's SQL state only. On Java 6 which introduces
 *       its own {@code SQLException} subclass hierarchy, we will use {@link
 *       SQLExceptionSubclassTranslator} by default, which in turns falls back to Spring's own SQL
 *       state translation when not encountering specific subclasses.
 * </ul>
 *
 * <p>The configuration file named "sql-error-codes.xml" is by default read from this package. It
 * can be overridden through a file of the same name in the root of the class path (e.g. in the
 * "/WEB-INF/classes" directory), as long as the Spring JDBC package is loaded from the same
 * ClassLoader.
 *
 * @author Rod Johnson
...
public class SQLErrorCodeSQLExceptionTranslator extends AbstractFallbackSQLExceptionTranslator {
  ...
  /**
   * Create an SQL error code translator for the given DataSource. Invoking this constructor will
   * cause a Connection to be obtained from the DataSource to get the meta-data.
   *
   * @param dataSource the DataSource to use to find meta-data and establish which error codes are
   *     usable
   * @see SQLErrorCodesFactory
   */
  ...
  /**
   * Create an SQL error code translator for the given database product name. Invoking this
   * constructor will avoid obtaining a Connection from the DataSource to get the meta-data.
   *
  ...
  /**
   * Create an SQLErrorCode translator given these error codes. Does not require a database
   * meta-data lookup to be performed using a connection.
   *
  ...
  protected DataAccessException doTranslate(String task, @Nullable String sql, SQLException ex) {
      ...
      if (errorCode != null) {
          ...
          for (CustomSQLErrorCodesTranslation customTranslation : customTranslations) {
            if (Arrays.binarySearch(customTranslation.getErrorCodes(), errorCode) >= 0
              ...
              DataAccessException customException =
                  createCustomException(task, sql, sqlEx, customTranslation.getExceptionClass());
        ...
        if (Arrays.binarySearch(this.sqlErrorCodes.getBadSqlGrammarCodes(), errorCode) >= 0) {
          ...
          return new BadSqlGrammarException(task, (sql != null ? sql : ""), sqlEx);
                ...
                this.sqlErrorCodes.getInvalidResultSetAccessCodes(), errorCode)
            >= 0) {
          logTranslation(task, sql, sqlEx, false);
          ...
          return new DuplicateKeyException(buildMessage(task, sql, sqlEx), sqlEx);
                ...
                this.sqlErrorCodes.getDataIntegrityViolationCodes(), errorCode)
            >= 0) {
          logTranslation(task, sql, sqlEx, false);
          return new DataIntegrityViolationException(buildMessage(task, sql, sqlEx), sqlEx);
        } else if (Arrays.binarySearch(this.sqlErrorCodes.getPermissionDeniedCodes(), errorCode)
            >= 0) {
          logTranslation(task, sql, sqlEx, false);
          return new PermissionDeniedDataAccessException(buildMessage(task, sql, sqlEx), sqlEx);
        } else if (Arrays.binarySearch(
                this.sqlErrorCodes.getDataAccessResourceFailureCodes(), errorCode)
            >= 0) {
          logTranslation(task, sql, sqlEx, false);
          return new DataAccessResourceFailureException(buildMessage(task, sql, sqlEx), sqlEx);
        } else if (Arrays.binarySearch(
                this.sqlErrorCodes.getTransientDataAccessResourceCodes(), errorCode)
            >= 0) {
          logTranslation(task, sql, sqlEx, false);
          return new TransientDataAccessResourceException(buildMessage(task, sql, sqlEx), sqlEx);
        } else if (Arrays.binarySearch(this.sqlErrorCodes.getCannotAcquireLockCodes(), errorCode)
            >= 0) {
          logTranslation(task, sql, sqlEx, false);
          return new CannotAcquireLockException(buildMessage(task, sql, sqlEx), sqlEx);
        } else if (Arrays.binarySearch(this.sqlErrorCodes.getDeadlockLoserCodes(), errorCode)
            >= 0) {
          logTranslation(task, sql, sqlEx, false);
          return new DeadlockLoserDataAccessException(buildMessage(task, sql, sqlEx), sqlEx);
        } else if (Arrays.binarySearch(
                this.sqlErrorCodes.getCannotSerializeTransactionCodes(), errorCode)
            >= 0) {
          logTranslation(task, sql, sqlEx, false);
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
          "Unable to translate SQLException with "
              + codes
   ...
   *     resulting from custom translation. This exception should include the {@code sqlEx}
   *     parameter as a nested root cause. This implementation always returns {@code null}, meaning
   *     that the translator always falls back to the default error codes.
   */
  @Nullable
  ...
  /**
   * Create a custom {@link DataAccessException}, based on a given exception class from a {@link
   * CustomSQLErrorCodesTranslation} definition.
   *
   * @param task readable text describing the task being attempted
   * @param sql the SQL query or update that caused the problem (may be {@code null})
   * @param sqlEx the offending SQLException
   * @param exceptionClass the exception class to use, as defined in the {@link
   *     CustomSQLErrorCodesTranslation} definition
   * @return {@code null} if the custom exception could not be created, otherwise the resulting
   *     {@link DataAccessException}. This exception should include the {@code sqlEx} parameter as a
   *     nested root cause.
    ...
    try {
      ...
      switch (constructorType) {
        case MESSAGE_SQL_SQLEX_CONSTRUCTOR:
          Class<?>[] messageAndSqlAndSqlExArgsClass =
              new Class<?>[] {String.class, String.class, SQLException.class};
        ...
        case MESSAGE_SQL_THROWABLE_CONSTRUCTOR:
          Class<?>[] messageAndSqlAndThrowableArgsClass =
          ...
          return (DataAccessException)
              exceptionConstructor.newInstance(messageAndSqlAndThrowableArgs);
  ...
  private void logTranslation(
      String task, @Nullable String sql, SQLException sqlEx, boolean custom) {
    if (logger.isDebugEnabled()) {
      String intro = custom ? "Custom translation of" : "Translating";
      logger.debug(
          intro
              + " SQLException with SQL state '"
              + sqlEx.getSQLState()
              + "', error code '"
              + sqlEx.getErrorCode()
              + "', message ["
