...
/**
 * {@link LobCreator} implementation based on temporary LOBs, using JDBC 4.0's {@link
 * java.sql.Connection#createBlob()} / {@link java.sql.Connection#createClob()} mechanism.
 *
 * <p>Used by DefaultLobHandler's {@link DefaultLobHandler#setCreateTemporaryLob} mode. Can also be
 * used directly to reuse the tracking and freeing of temporary LOBs.
 *
...
public class TemporaryLobCreator implements LobCreator {
  ...
  public void setBlobAsBytes(PreparedStatement ps, int paramIndex, @Nullable byte[] content)
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          content != null
              ? "Copied bytes into temporary BLOB with length " + content.length
              : "Set BLOB to null");
    }
  }
  ...
  public void setClobAsString(PreparedStatement ps, int paramIndex, @Nullable String content)
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          content != null
              ? "Copied string into temporary CLOB with length " + content.length()
              : "Set CLOB to null");
    }
  }
