...
/**
 * Mirror interface for {@link javax.sql.RowSet}, representing a disconnected variant of {@link
 * java.sql.ResultSet} data.
 *
 * <p>The main difference to the standard JDBC RowSet is that a {@link java.sql.SQLException} is
 * never thrown here. This allows an SqlRowSet to be used without having to deal with checked
 * exceptions. An SqlRowSet will throw Spring's {@link InvalidResultSetAccessException} instead
 * (when appropriate).
 *
...
public interface SqlRowSet extends Serializable {
  ...
  /**
   * Retrieve the value of the indicated column in the current row as a String (for NCHAR, NVARCHAR,
   ...
   *
   * <p>Note that you must first call one of the getter methods and then call the {@code wasNull()}
   * method.
   *
   * @return {@code true} if the most recent column retrieved was SQL {@code NULL}, {@code false}
   *     otherwise
