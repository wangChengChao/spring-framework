 ...
 * updated, and adds them as a single batch once {@code flush} is called or the given batch size has
 * been met.
 *
 * <p>Note that this class is a <b>non-thread-safe object</b>, in contrast to all other JDBC
 * operations objects in this package. You need to create a new instance of it for each use, or call
 * {@code reset} before reuse within the same thread.
 *
 * @author Keith Donald
...
public class BatchSqlUpdate extends SqlUpdate {
   ...
   * @param sql the SQL statement to execute
   * @param types the SQL types of the parameters, as defined in the {@code java.sql.Types} class
   * @param batchSize the number of statements that will trigger an automatic intermediate flush
   * @see java.sql.Types
   */
  ...
  /**
   * Set the number of statements that will trigger an automatic intermediate flush. {@code update}
   * calls or the given statement parameters will be queued until the batch size is met, at which
   * point it will empty the queue and execute the batch.
   *
   * <p>You can also flush already queued statements with an explicit {@code flush} call. Note that
   * you need to this after queueing all parameters to guarantee that all statements have been
   * flushed.
   */
   ...
   *
   * <p>Default is "true". Turn this off to save the memory needed for the list of row counts.
   *
   * @see #getRowsAffected()
   */
  ...
  /**
   * Overridden version of {@code update} that adds the given statement parameters to the queue
   * rather than executing them immediately. All other {@code update} methods of the SqlUpdate base
   * class go through this method and will thus behave similarly.
   *
   * <p>You need to call {@code flush} to actually execute the batch. If the specified batch size is
   * reached, an implicit flush will happen; you still need to finally call {@code flush} to flush
   * all statements.
   *
   * @param params array of parameter objects
   * @return the number of rows affected by the update (always -1, meaning "not applicable", as the
   ...
   * {@code flush}'s return values until {@code reset} is invoked.
   *
   * @return an array of the number of rows affected by each statement
   * @see #reset
   */
