...
/**
 * RdbmsOperation using a JdbcTemplate and representing an SQL-based call such as a stored procedure
...
public abstract class SqlCall extends RdbmsOperation {
  ...
  /**
   * Flag used to indicate that the sql for this call should be used exactly as it is defined. No
   * need to add the escape syntax and parameter place holders.
   */
  private boolean sqlReadyForUse = false;

  /**
   * Call string as defined in java.sql.CallableStatement. String of form {call add_invoice(?, ?,
   * ?)} or {? = call get_invoice_count(?)} if isFunction is set to true. Updated after each
   * parameter is added.
   */
  @Nullable private String callString;

  /**
   * Object enabling us to create CallableStatementCreators efficiently, based on this class's
   * declared parameters.
   */
  ...
  /**
   * Create a new SqlCall object with SQL, but without parameters. Must add parameters or settle
   * with none.
   *
  ...
  protected final void compileInternal() {
    ...
    this.callableStatementFactory =
        new CallableStatementCreatorFactory(this.callString, getDeclaredParameters());
   ...
   */
  protected CallableStatementCreator newCallableStatementCreator(
      @Nullable Map<String, ?> inParams) {
  ...
  /**
   * Return a CallableStatementCreator to perform an operation with the parameters returned from
