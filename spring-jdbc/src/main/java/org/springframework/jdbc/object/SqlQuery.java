 ...
 *
 * <p>Subclasses must implement the {@link #newRowMapper} method to provide an object that can
 * extract the results of iterating over the {@code ResultSet} created during the execution of the
 * query.
 *
 * <p>This class provides a number of public {@code execute} methods that are analogous to the
 * different convenient JDO query execute methods. Subclasses can either rely on one of these
 * inherited methods, or can add their own custom execution methods, with meaningful names and typed
 * parameters (definitely a best practice). Each custom query method will invoke one of this class's
 * untyped query methods.
 *
 * <p>Like all {@code RdbmsOperation} classes that ship with the Spring Framework, {@code SqlQuery}
 * instances are thread-safe after their initialization is complete. That is, after they are
 * constructed and configured via their setter methods, they can be used safely from multiple
 * threads.
 *
 * @author Rod Johnson
...
public abstract class SqlQuery<T> extends SqlOperation {
   ...
   *
   * <p>The {@code DataSource} and SQL must be supplied before compilation and use.
   */
  public SqlQuery() {}
   ...
   * @param sql the SQL to execute; SQL can also be supplied at runtime by overriding the {@link
   *     #getSql()} method.
   */
   ...
   *
   * <p>This can be used to ensure efficient storage of results. The default behavior is not to
   ...
   *
   * @param params parameters, similar to JDO query parameters. Primitive parameters must be
   *     represented by their Object wrapper type. The ordering of parameters is significant.
   * @param context contextual information passed to the {@code mapRow} callback method. The JDBC
   *     operation itself doesn't rely on this parameter, but it can be useful for creating the
   *     objects of the result list.
   * @return a List of objects, one per row of the ResultSet. Normally all these will be of the same
   *     class, although it is possible to use different types.
   */
  public List<T> execute(@Nullable Object[] params, @Nullable Map<?, ?> context)
   ...
   *
   * @param params parameters for the query. Primitive parameters must be represented by their
   *     Object wrapper type. The ordering of parameters is significant.
   */
   ...
   *
   * @param paramMap parameters associated with the name specified while declaring the
   *     SqlParameters. Primitive parameters must be represented by their Object wrapper type. The
   *     ordering of parameters is not significant since they are supplied in a SqlParameterMap
   *     which is an implementation of the Map interface.
   * @param context contextual information passed to the {@code mapRow} callback method. The JDBC
   *     operation itself doesn't rely on this parameter, but it can be useful for creating the
   *     objects of the result list.
   * @return a List of objects, one per row of the ResultSet. Normally all these will be of the same
  ...
  public List<T> executeByNamedParam(Map<String, ?> paramMap, @Nullable Map<?, ?> context)
    ...
    Object[] params =
        NamedParameterUtils.buildValueArray(parsedSql, paramSource, getDeclaredParameters());
    RowMapper<T> rowMapper = newRowMapper(params, context);
  ...
  /**
   * Generic object finder method, used by all other {@code findObject} methods. Object finder
   * methods are like EJB entity bean finders, in that it is considered an error if they return more
   * than one result.
   *
   * @return the result object, or {@code null} if not found. Subclasses may choose to treat this as
   *     an error and throw an exception.
   * @see org.springframework.dao.support.DataAccessUtils#singleResult
   */
  @Nullable
  public T findObject(@Nullable Object[] params, @Nullable Map<?, ?> context)
      throws DataAccessException {
   ...
   *
   * @param paramMap a Map of parameter name to parameter object, matching named parameters
   *     specified in the SQL statement. Ordering is not significant.
   * @param context contextual information passed to the {@code mapRow} callback method. The JDBC
   *     operation itself doesn't rely on this parameter, but it can be useful for creating the
   *     objects of the result list.
   * @return a List of objects, one per row of the ResultSet. Normally all these will be of the same
   ...
   * {@code execute} method as an aggregated {@link List}.
   *
   * @param parameters the parameters to the {@code execute()} method, in case subclass is
   *     interested; may be {@code null} if there were no parameters.
   * @param context contextual information passed to the {@code mapRow} callback method. The JDBC
   *     operation itself doesn't rely on this parameter, but it can be useful for creating the
   *     objects of the result list.
   * @see #execute
  ...
  protected abstract RowMapper<T> newRowMapper(
      @Nullable Object[] parameters, @Nullable Map<?, ?> context);
}
