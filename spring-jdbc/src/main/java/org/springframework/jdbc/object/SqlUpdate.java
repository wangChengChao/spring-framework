 ...
 *
 * <p>This class provides a number of {@code update} methods, analogous to the {@code execute}
 * methods of query objects.
 *
 * <p>This class is concrete. Although it can be subclassed (for example to add a custom update
 * method) it can easily be parameterized by setting SQL and declaring parameters.
 *
...
public class SqlUpdate extends SqlOperation {
  ...
  /**
   * Maximum number of rows the update may affect. If more are affected, an exception will be
   * thrown. Ignored if 0.
   */
   ...
   * @param ds the DataSource to use to obtain connections
   * @param sql the SQL statement to execute
   * @param types the SQL types of the parameters, as defined in the {@code java.sql.Types} class
   * @see java.sql.Types
   */
  ...
  /**
   * Construct an update object with a given DataSource, SQL, anonymous parameters and specifying
   * the maximum number of rows that may be affected.
   *
   * @param ds the DataSource to use to obtain connections
   * @param sql the SQL statement to execute
   * @param types the SQL types of the parameters, as defined in the {@code java.sql.Types} class
   * @param maxRowsAffected the maximum number of rows that may be affected by the update
   * @see java.sql.Types
   */
  ...
  /**
   * Set the maximum number of rows that may be affected by this update. The default value is 0,
   * which does not limit the number of rows affected.
   *
   * @param maxRowsAffected the maximum number of rows that can be affected by this update without
   *     this class's update method considering it an error
   */
  ...
  /**
   * Set the <i>exact</i> number of rows that must be affected by this update. The default value is
   * 0, which allows any number of rows to be affected.
   *
   * <p>This is an alternative to setting the <i>maximum</i> number of rows that may be affected.
   *
   * @param requiredRowsAffected the exact number of rows that must be affected by this update
  ...
  /**
   * Check the given number of affected rows against the specified maximum number or required
   * number.
   *
   * @param rowsAffected the number of affected rows
   * @throws JdbcUpdateAffectedIncorrectNumberOfRowsException if the actually affected rows are out
   *     of bounds
  ...
  protected void checkRowsAffected(int rowsAffected)
    ...
    if (this.maxRowsAffected > 0 && rowsAffected > this.maxRowsAffected) {
      throw new JdbcUpdateAffectedIncorrectNumberOfRowsException(
          resolveSql(), this.maxRowsAffected, rowsAffected);
    }
    if (this.requiredRowsAffected > 0 && rowsAffected != this.requiredRowsAffected) {
      throw new JdbcUpdateAffectedIncorrectNumberOfRowsException(
          resolveSql(), this.requiredRowsAffected, rowsAffected);
    }
  }
  ...
  public int update(Object[] params, KeyHolder generatedKeyHolder) throws DataAccessException {
    ...
    validateParameters(params);
    int rowsAffected =
        getJdbcTemplate().update(newPreparedStatementCreator(params), generatedKeyHolder);
    checkRowsAffected(rowsAffected);
    return rowsAffected;
  }
  ...
  public int updateByNamedParam(Map<String, ?> paramMap, KeyHolder generatedKeyHolder)
    ...
    Object[] params =
        NamedParameterUtils.buildValueArray(parsedSql, paramSource, getDeclaredParameters());
    int rowsAffected =
        getJdbcTemplate().update(newPreparedStatementCreator(sqlToUse, params), generatedKeyHolder);
    checkRowsAffected(rowsAffected);
    return rowsAffected;
  }
}
