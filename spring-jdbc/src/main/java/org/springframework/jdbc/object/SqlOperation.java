...
/**
 * Operation object representing an SQL-based operation such as a query or update, as opposed to a
 * stored procedure.
 *
 * <p>Configures a {@link org.springframework.jdbc.core.PreparedStatementCreatorFactory} based on
...
public abstract class SqlOperation extends RdbmsOperation {
  ...
  /**
   * Object enabling us to create PreparedStatementCreators efficiently, based on this class's
   * declared parameters.
   */
  @Nullable private PreparedStatementCreatorFactory preparedStatementFactory;

  /** Parsed representation of the SQL statement. */
  @Nullable private ParsedSql cachedSql;
  ...
  /**
   * Overridden method to configure the PreparedStatementCreatorFactory based on our declared
   * parameters.
   */
  @Override
  protected final void compileInternal() {
    this.preparedStatementFactory =
        new PreparedStatementCreatorFactory(resolveSql(), getDeclaredParameters());
  ...
  /**
   * Hook method that subclasses may override to post-process compilation. This implementation does
   * nothing.
   *
   * @see #compileInternal
   */
   ...
   * Return a PreparedStatementCreator to perform an operation with the given parameters.
   *
   * @param sqlToUse the actual SQL statement to use (if different from the factory's, for example
   *     because of named parameter expanding)
   * @param params the parameter array (may be {@code null})
   */
  protected final PreparedStatementCreator newPreparedStatementCreator(
      String sqlToUse, @Nullable Object[] params) {
    Assert.state(this.preparedStatementFactory != null, "No PreparedStatementFactory available");
