...
/**
 * Exception thrown when a JDBC update affects an unexpected number of rows. Typically we expect an
 * update to affect a single row, meaning it's an error if it affects multiple rows.
 *
 * @author Rod Johnson
...
public class JdbcUpdateAffectedIncorrectNumberOfRowsException
  ...
  public JdbcUpdateAffectedIncorrectNumberOfRowsException(String sql, int expected, int actual) {
    super(
        "SQL update '" + sql + "' affected " + actual + " rows, not " + expected + " as expected");
