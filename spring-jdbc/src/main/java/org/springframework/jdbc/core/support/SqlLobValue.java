...
/**
 * Object to represent an SQL BLOB/CLOB value parameter. BLOBs can either be an InputStream or a
 * byte array. CLOBs can be in the form of a Reader, InputStream or String. Each CLOB/BLOB value
 * will be stored together with its length. The type is based on which constructor is used. Objects
 * of this class are immutable except for the LobCreator reference. Use them and discard them.
 *
 * <p>This class holds a reference to a LocCreator that must be closed after the update has
 * completed. This is done via a call to the closeLobCreator method. All handling of the LobCreator
 * is done by the framework classes that use it - no need to set or close the LobCreator for end
 * users of this class.
 *
 * <p>A usage example:
 *
    ...
    if (sqlType == Types.BLOB) {
      if (this.content instanceof byte[] || this.content == null) {
        ...
        this.lobCreator.setBlobAsBinaryStream(
            ps, paramIndex, (InputStream) this.content, this.length);
      } else {
      ...
      if (this.content instanceof String || this.content == null) {
        ...
        this.lobCreator.setClobAsAsciiStream(
            ps, paramIndex, (InputStream) this.content, this.length);
      } else if (this.content instanceof Reader) {
        this.lobCreator.setClobAsCharacterStream(
            ps, paramIndex, (Reader) this.content, this.length);
      } else {
        throw new IllegalArgumentException(
            "Content type ["
                + this.content.getClass().getName()
