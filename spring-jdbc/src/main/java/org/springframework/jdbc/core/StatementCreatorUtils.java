...
/**
 * Utility methods for PreparedStatementSetter/Creator and CallableStatementCreator implementations,
 * providing sophisticated parameter management (including support for LOB values).
 *
 * <p>Used by PreparedStatementCreatorFactory and CallableStatementCreatorFactory, but also
 * available for direct use in custom setter/creator implementations.
 *
...
public abstract class StatementCreatorUtils {
  ...
  /**
   * System property that instructs Spring to ignore {@link
   * java.sql.ParameterMetaData#getParameterType} completely, i.e. to never even attempt to retrieve
   * {@link PreparedStatement#getParameterMetaData()} for {@link StatementCreatorUtils#setNull}
   * calls.
   *
   ...
   * well-known behavior of common databases. Spring records JDBC drivers with non-working {@code
   * getParameterType} implementations and won't attempt to call that method for that driver again,
   * always falling back.
   *
   * <p>Consider switching this flag to "true" if you experience misbehavior at runtime, e.g. with a
   * connection pool setting back the {@link PreparedStatement} instance in case of an exception
  ...
  static boolean shouldIgnoreGetParameterType =
      SpringProperties.getFlag(IGNORE_GETPARAMETERTYPE_PROPERTY_NAME);
  ...
  /**
   * Set the value for a parameter. The method used is based on the SQL type of the parameter and we
   * can handle complex types like arrays and LOBs.
   *
  ...
  public static void setParameterValue(
      PreparedStatement ps, int paramIndex, SqlParameter param, @Nullable Object inValue)
      throws SQLException {

    setParameterValueInternal(
        ps, paramIndex, param.getSqlType(), param.getTypeName(), param.getScale(), inValue);
  }
   ...
   * @param paramIndex index of the parameter we are setting
   * @param sqlType the SQL type of the parameter
   * @param typeName the type name of the parameter (optional, only used for SQL NULL and
   *     SqlTypeValue)
   * @param inValue the value to set (plain value or an SqlTypeValue)
   * @throws SQLException if thrown by PreparedStatement methods
   * @see SqlTypeValue
   */
  public static void setParameterValue(
      PreparedStatement ps, int paramIndex, int sqlType, String typeName, @Nullable Object inValue)
      throws SQLException {
  ...
  private static void setParameterValueInternal(
    ...
    if (logger.isTraceEnabled()) {
      logger.trace(
          "Setting SQL statement parameter value: column index "
              + paramIndex
              + ", parameter value ["
              + inValueToUse
              + "], value class ["
              ...
              + "], SQL type "
              + (sqlTypeToUse == SqlTypeValue.TYPE_UNKNOWN
                  ? "unknown"
                  : Integer.toString(sqlTypeToUse)));
    }
  ...
  /**
   * Set the specified PreparedStatement parameter to null, respecting database-specific
   * peculiarities.
   */
  private static void setNull(
      PreparedStatement ps, int paramIndex, int sqlType, @Nullable String typeName)
      throws SQLException {
    ...
    if (sqlType == SqlTypeValue.TYPE_UNKNOWN || (sqlType == Types.OTHER && typeName == null)) {
      ...
      if (sqlTypeToUse == null) {
        ...
        if (databaseProductName.startsWith("Informix")
          ...
          // "Microsoft SQL Server JDBC Driver 3.0" versus "Microsoft JDBC Driver 4.0 for SQL
          // Server"
          useSetObject = true;
  ...
  /**
   * Clean up all resources held by parameter values which were passed to an execute method. This is
   * for example important for closing LOB values.
   *
