...
/**
 * Implementation of RowCallbackHandler. Convenient superclass for callback handlers. An instance
 * can only be used once.
 *
 * <p>We can either use this on its own (for example, in a test case, to ensure that our result sets
 * have valid dimensions), or use it as a superclass for callback handlers that actually do
 * something, and will benefit from the dimension information it provides.
 *
...
public class RowCountCallbackHandler implements RowCallbackHandler {
  ...
  /**
   * Indexed from 0. Type (as in java.sql.Types) for the columns as returned by ResultSetMetaData
   * object.
   */
   ...
   *
   * <p>Subclasses can perform custom extraction or processing by overriding the {@code
   * processRow(ResultSet, int)} method.
   *
  ...
  /**
   * Subclasses may override this to perform custom extraction or processing. This class's
  ...
  /**
   * Return the types of the columns as java.sql.Types constants Valid after processRow is invoked
   * the first time.
   *
   * @return the types of the columns as java.sql.Types constants. <b>Indexed from 0 to n-1.</b>
   */
  @Nullable
  ...
  /**
   * Return the number of columns in this result set. Valid once we've seen the first row, so
   * subclasses can use it during processing
   *
