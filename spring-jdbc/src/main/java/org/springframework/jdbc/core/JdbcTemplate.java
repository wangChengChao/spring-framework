...
/**
 * <b>This is the central class in the JDBC core package.</b> It simplifies the use of JDBC and
 * helps to avoid common errors. It executes core JDBC workflow, leaving application code to provide
 * SQL and extract results. This class executes SQL queries or updates, initiating iteration over
 * ResultSets and catching JDBC exceptions and translating them to the generic, more informative
 * exception hierarchy defined in the {@code org.springframework.dao} package.
 *
 * <p>Code using this class need only implement callback interfaces, giving them a clearly defined
 * contract. The {@link PreparedStatementCreator} callback interface creates a prepared statement
 * given a Connection, providing SQL and any necessary parameters. The {@link ResultSetExtractor}
 * interface extracts values from a ResultSet. See also {@link PreparedStatementSetter} and {@link
 * RowMapper} for two popular alternative callback interfaces.
 *
 * <p>Can be used within a service implementation via direct instantiation with a DataSource
 * reference, or get prepared in an application context and given to services as bean reference.
 * Note: The DataSource should always be configured as a bean in the application context, in the
 ...
 * org.springframework.jdbc.support.SQLExceptionTranslator} interface, there should be no need to
 * subclass it.
 *
 * <p>All SQL operations performed by this class are logged at debug level, using
...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
  ...
  /**
   * If this variable is set to a non-negative value, it will be used for setting the fetchSize
   * property on statements used for query processing.
   */
  private int fetchSize = -1;

  /**
   * If this variable is set to a non-negative value, it will be used for setting the maxRows
   * property on statements used for query processing.
   */
  private int maxRows = -1;

  /**
   * If this variable is set to a non-negative value, it will be used for setting the queryTimeout
   * property on statements used for query processing.
   */
  private int queryTimeout = -1;

  /**
   * If this variable is set to true, then all results checking will be bypassed for any callable
   * statement processing. This can be used to avoid a bug in some older Oracle JDBC drivers like
   * 10.1.0.2.
   */
   ...
   * be take place unless the variable {@code skipResultsProcessing} is set to {@code true}.
   */
  private boolean skipUndeclaredResults = false;

  /**
   * If this variable is set to true then execution of a CallableStatement will return the results
   ...
   * Construct a new JdbcTemplate, given a DataSource to obtain connections from.
   *
   * <p>Note: Depending on the "lazyInit" flag, initialization of the exception translator will be
   * triggered.
   *
   * @param dataSource the JDBC DataSource to obtain connections from
   ...
   *
   * <p>Default is "true", swallowing and logging all warnings. Switch this flag to "false" to make
   * the JdbcTemplate throw an SQLWarningException instead.
   *
  ...
  /**
   * Set the fetch size for this JdbcTemplate. This is important for processing large result sets:
   ...
   * application.
   *
   * <p>Default is -1, indicating to use the JDBC driver's default configuration (i.e. to not pass a
   * specific fetch size setting on to the driver).
   *
   * <p>Note: As of 4.3, negative values other than -1 will get passed on to the driver, since e.g.
   * MySQL supports special behavior for {@code Integer.MIN_VALUE}.
   *
  ...
  /**
   * Set the maximum number of rows for this JdbcTemplate. This is important for processing subsets
   ...
   *
   * <p>Default is -1, indicating to use the JDBC driver's default configuration (i.e. to not pass a
   * specific max rows setting on to the driver).
   *
   * <p>Note: As of 4.3, negative values other than -1 will get passed on to the driver, in sync
   * with {@link #setFetchSize}'s support for special MySQL values.
   *
   ...
   *
   * <p>Default is -1, indicating to use the JDBC driver's default (i.e. to not pass a specific
   * query timeout setting on the driver).
   *
   * <p>Note: Any timeout specified here will be overridden by the remaining transaction timeout
   * when executing within a transaction that has a timeout specified at the transaction level.
   *
  ...
  /**
   * Set whether results processing should be skipped. Can be used to optimize callable statement
   * processing when we know that no results are being passed back - the processing of out parameter
   * will still take place. This can be used to avoid a bug in some older Oracle JDBC drivers like
   * 10.1.0.2.
   */
  ...
  /**
   * Set whether execution of a CallableStatement will return the results in a Map that uses case
   * insensitive names for the parameters.
   */
  ...
  /**
   * Return whether execution of a CallableStatement will return the results in a Map that uses case
   * insensitive names for the parameters.
   */
  ...
  /**
   * Create a close-suppressing proxy for the given JDBC Connection. Called by the {@code execute}
   * method.
   *
   * <p>The proxy also prepares returned JDBC Statements, applying statement settings such as fetch
  ...
  /**
   * Query using a prepared statement, allowing for a PreparedStatementCreator and a
   * PreparedStatementSetter. Most other query methods use this method, but application code will
   * always work with either a creator or a setter.
   *
   * @param psc a callback that creates a PreparedStatement given a Connection
   * @param pss a callback that knows how to set values on the prepared statement. If this is {@code
   *     null}, the SQL will be assumed to contain no bind parameters.
  ...
  protected int update(
      final PreparedStatementCreator psc, @Nullable final PreparedStatementSetter pss)
      throws DataAccessException {
            ...
            for (Object value : values) {
              ...
              if (value instanceof SqlParameterValue) {
                ...
                StatementCreatorUtils.setParameterValue(
                    ps, colIndex, paramValue, paramValue.getValue());
              } else {
                int colType;
                    ...
                    for (T obj : batchArgs) {
                      ...
                      if (batchSupported) {
                        ...
                        if (n % batchSize == 0 || n == batchArgs.size()) {
                          if (logger.isTraceEnabled()) {
                            int batchIdx =
                                (n % batchSize == 0) ? n / batchSize : (n / batchSize) + 1;
                            int items =
                                n
                                    - ((n % batchSize == 0) ? n / batchSize - 1 : (n / batchSize))
                                        * batchSize;
                            logger.trace(
                                "Sending SQL batch update #"
                                    + batchIdx
                                    + " with "
                                    + items
                                    + " items");
              ...
              if (retVal || updateCount != -1) {
                resultsMap.putAll(
                    extractReturnedResults(
                        cs, updateCountParameters, resultSetParameters, updateCount));
              }
   ...
   * @param updateCountParameters the parameter list of declared update count parameters for the
   *     stored procedure
   * @param resultSetParameters the parameter list of declared resultSet parameters for the stored
   *     procedure
      ...
      do {
        if (updateCount == -1) {
          if (resultSetParameters != null && resultSetParameters.size() > rsIndex) {
            SqlReturnResultSet declaredRsParam =
                (SqlReturnResultSet) resultSetParameters.get(rsIndex);
            results.putAll(processResultSet(cs.getResultSet(), declaredRsParam));
            rsIndex++;
          } else {
            if (!this.skipUndeclaredResults) {
              String rsName = RETURN_RESULT_SET_PREFIX + (rsIndex + 1);
              SqlReturnResultSet undeclaredRsParam =
                  new SqlReturnResultSet(rsName, getColumnMapRowMapper());
              if (logger.isTraceEnabled()) {
          ...
          if (updateCountParameters != null && updateCountParameters.size() > updateIndex) {
            SqlReturnUpdateCount ucParam =
                (SqlReturnUpdateCount) updateCountParameters.get(updateIndex);
   ...
   * @return a Map that contains returned results
   */
  protected Map<String, Object> extractOutputParameters(
      CallableStatement cs, List<SqlParameter> parameters) throws SQLException {
      ...
      try {
        if (param.getRowMapper() != null) {
          ...
          return Collections.singletonMap(
              param.getName(), "ResultSet returned from stored procedure was processed");
   ...
   *
   * <p>If {@link #resultsMapCaseInsensitive} has been set to true, a {@link
   * LinkedCaseInsensitiveMap} will be created; otherwise, a {@link LinkedHashMap} will be created.
   *
  ...
  /**
   * Prepare the given JDBC Statement (or PreparedStatement or CallableStatement), applying
   ...
   *
   * <p>By default, we'll create an {@link ArgumentPreparedStatementSetter}. This method allows for
   * the creation to be overridden by subclasses.
   *
   ...
   *
   * <p>By default, we'll create an {@link ArgumentTypePreparedStatementSetter}. This method allows
   * for the creation to be overridden by subclasses.
   *
   * @param args object array with arguments
  ...
  /**
   * Throw an SQLWarningException if we're not ignoring warnings, otherwise log the warnings at
   * debug level.
   *
    ...
    if (isIgnoreWarnings()) {
      if (logger.isDebugEnabled()) {
        ...
        while (warningToLog != null) {
          logger.debug(
              "SQLWarning ignored: SQL state '"
                  + warningToLog.getSQLState()
                  + "', error code '"
   ...
   *
   * @param warning the warnings object from the current statement. May be {@code null}, in which
   ...
   */
  protected DataAccessException translateException(
      String task, @Nullable String sql, SQLException ex) {
  ...
  /**
   * Invocation handler that suppresses close calls on JDBC Connections. Also prepares returned
   * Statement (Prepared/CallbackStatement) objects.
   *
   * @see java.sql.Connection#close()
   */
   ...
   *
   * <p>Uses a regular ResultSet, so we have to be careful when using it: We don't use it for
   * navigating since this could lead to unpredictable consequences.
   */
