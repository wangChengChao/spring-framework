...
/**
 * Interface specifying the API for a Simple JDBC Call implemented by {@link SimpleJdbcCall}. This
 * interface is not often used directly, but provides the option to enhance testability, as it can
...
public interface SimpleJdbcCallOperations {
  ...
  /**
   * Specify the procedure name to be used - this implies that we will be calling a stored
   * procedure.
   *
   ...
   *
   * <p>To provide consistency with the Oracle DatabaseMetaData, this is used to specify the package
   * name if the procedure is declared as part of a package.
   *
   ...
   * which - for backwards compatibility reasons - allows input values to be provided for parameters
   * declared as {@code SqlOutParameter}.
   *
  ...
  /**
   * Used to specify when a ResultSet is returned by the stored procedure and you want it mapped by
   * a {@link RowMapper}. The results will be returned using the parameter name specified. Multiple
   * ResultSets must be declared in the correct order.
   *
   * <p>If the database you are using uses ref cursors then the name specified must match the name
   * of the parameter declared for the procedure in the database.
   *
   * @param parameterName the name of the returned results and/or the name of the ref cursor
   *     parameter
  ...
  /**
   * Execute the stored function and return the results obtained as an Object of the specified
   * return type.
   *
   * @param returnType the type of the value to return
   * @param args optional array containing the in parameter values to be used in the call. Parameter
   *     values must be provided in the same order as the parameters are defined for the stored
   *     procedure.
   */
  ...
  /**
   * Execute the stored procedure and return the single out parameter as an Object of the specified
   * return type. In the case where there are multiple out parameters, the first one is returned and
   * additional out parameters are ignored.
   *
   * @param returnType the type of the value to return
   * @param args optional array containing the in parameter values to be used in the call. Parameter
   *     values must be provided in the same order as the parameters are defined for the stored
  ...
  /**
   * Execute the stored procedure and return a map of output params, keyed by name as in parameter
   * declarations.
   *
   * @param args optional array containing the in parameter values to be used in the call. Parameter
   *     values must be provided in the same order as the parameters are defined for the stored
