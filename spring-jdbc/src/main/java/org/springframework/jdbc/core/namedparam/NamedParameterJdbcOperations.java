...
/**
 * Interface specifying a basic set of JDBC operations allowing the use of named parameters rather
 * than the traditional '?' placeholders.
 *
 * <p>This is an alternative to the classic {@link org.springframework.jdbc.core.JdbcOperations}
 * interface, implemented by {@link NamedParameterJdbcTemplate}. This interface is not often used
 * directly, but provides a useful option to enhance testability, as it can easily be mocked or
 * stubbed.
 *
...
public interface NamedParameterJdbcOperations {
   ...
   *
   * @param sql the SQL to execute
   * @param paramMap map of parameters to bind to the query (leaving it to the PreparedStatement to
   *     guess the corresponding SQL type)
   * @param action callback object that specifies the action
   * @return a result object returned by the action, or {@code null}
   * @throws DataAccessException if there is any problem
   */
  ...
  /**
   * Query given SQL to create a prepared statement from SQL, reading the ResultSet with a
   * ResultSetExtractor.
   *
   * <p>Note: In contrast to the JdbcOperations method with the same signature, this query variant
   * always uses a PreparedStatement. It is effectively equivalent to a query call with an empty
   * parameter Map.
   *
   * @param sql the SQL query to execute
   * @param rse object that will extract results
  ...
  /**
   * Query given SQL to create a prepared statement from SQL, reading the ResultSet on a per-row
  ...
  /**
   * Query given SQL to create a prepared statement from SQL, mapping each row to a Java object via
   * a RowMapper.
   *
   * <p>Note: In contrast to the JdbcOperations method with the same signature, this query variant
   * always uses a PreparedStatement. It is effectively equivalent to a query call with an empty
   ...
   *     return exactly one row, or does not return exactly one column in that row
   * @throws org.springframework.dao.DataAccessException if the query fails
   */
  @Nullable
  ...
  /**
   * Query given SQL to create a prepared statement from SQL and a list of arguments to bind to the
   * query, expecting a result Map. The queryForMap() methods defined by this interface are
   * appropriate when you don't have a domain model. Otherwise, consider using one of the
  ...
  /**
   * Issue an update via a prepared statement, binding the given arguments, returning generated
   * keys.
   *
   * @param sql the SQL containing named parameters
   * @param paramSource container of arguments and SQL types to bind to the query
   ...
   *
   * @param sql the SQL statement to execute
   * @param batchArgs the array of {@link SqlParameterSource} containing the batch of arguments for
   *     the query
   * @return an array containing the numbers of rows affected by each update in the batch
   */
