 ...
 *
 * <p>Expects columns for bean name, property name and value as String. Formats for each are
 * identical to the properties format recognized by PropertiesBeanDefinitionReader.
 *
 * <p><b>NOTE:</b> This is mainly intended as an example for a custom JDBC-based bean definition
 * reader. It does not aim to offer comprehensive functionality.
 *
 * @author Rod Johnson
...
public class JdbcBeanDefinitionReader {
  ...
  /**
   * Set the DataSource to use to obtain database connections. Will implicitly create a new
   * JdbcTemplate with the given DataSource.
   */
  ...
  /**
   * Set the JdbcTemplate to be used by this bean factory. Contains settings for DataSource,
   * SQLExceptionTranslator, etc.
   */
   ...
   *
   * @param sql the SQL query to use for loading bean definitions. The first three columns must be
   *     bean name, property name and value. Any join and any other columns are permitted: e.g.
   *     {@code SELECT BEAN_NAME, PROPERTY, VALUE FROM CONFIG WHERE CONFIG.APP_ID = 1} It's also
   *     possible to perform a join. Column names are not significant -- only the ordering of these
   *     first three columns.
   */
