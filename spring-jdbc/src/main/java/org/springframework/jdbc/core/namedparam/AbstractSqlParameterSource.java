...
/**
 * Abstract base class for {@link SqlParameterSource} implementations. Provides registration of SQL
 * types per parameter and a friendly {@link #toString() toString} representation enumerating all
 * parameters for a {@code SqlParameterSource} implementing {@link #getParameterNames()}. Concrete
 * subclasses must implement {@link #hasValue} and {@link #getValue}.
 *
...
public abstract class AbstractSqlParameterSource implements SqlParameterSource {
  ...
  /**
   * Enumerate the parameter names and values with their corresponding SQL type if available, or
   * just return the simple {@code SqlParameterSource} implementation class name otherwise.
   *
   * @since 5.2
   * @see #getParameterNames()
   */
