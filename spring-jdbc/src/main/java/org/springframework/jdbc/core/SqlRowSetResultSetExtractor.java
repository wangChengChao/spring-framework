...
/**
 * {@link ResultSetExtractor} implementation that returns a Spring {@link SqlRowSet} representation
 * for each given {@link ResultSet}.
 *
...
public class SqlRowSetResultSetExtractor implements ResultSetExtractor<SqlRowSet> {
  ...
  /**
   * Create a {@link SqlRowSet} that wraps the given {@link ResultSet}, representing its data in a
   * disconnected fashion.
   *
   * <p>This implementation creates a Spring {@link ResultSetWrappingSqlRowSet} instance that wraps
   * a standard JDBC {@link CachedRowSet} instance. Can be overridden to use a different
   * implementation.
   *
  ...
  /**
   * Create a new {@link CachedRowSet} instance, to be populated by the {@code createSqlRowSet}
   * implementation.
   *
