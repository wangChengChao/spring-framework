...
/**
 * Helper class that efficiently creates multiple {@link CallableStatementCreator} objects with
 * different parameters based on an SQL statement and a single set of parameter declarations.
 *
 * @author Rod Johnson
...
public class CallableStatementCreatorFactory {
  ...
  /**
   * Create a new factory. Will need to add parameters via the {@link #addParameter} method or have
   * no parameters.
   *
  ...
  /**
   * Set whether to use prepared statements that return a specific type of ResultSet. specific type
   * of ResultSet.
   *
  ...
  private class CallableStatementCreatorImpl
    ...
    @Nullable private ParameterMapper inParameterMapper;

    @Nullable private Map<String, ?> inParameters;
            ...
            if (declaredParam instanceof SqlOutParameter) {
              if (declaredParam.getTypeName() != null) {
                cs.registerOutParameter(
                    sqlColIndx, declaredParam.getSqlType(), declaredParam.getTypeName());
              } else {
                if (declaredParam.getScale() != null) {
                  cs.registerOutParameter(
                      sqlColIndx, declaredParam.getSqlType(), declaredParam.getScale());
                } else {
