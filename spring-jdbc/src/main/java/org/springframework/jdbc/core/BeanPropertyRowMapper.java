...
/**
 * {@link RowMapper} implementation that converts a row into a new instance of the specified mapped
 ...
 * directly or by transforming a name separating the parts with underscores to the same name using
 * "camel" case.
 *
 * <p>Mapping is provided for fields in the target class for many common types, e.g.: String,
 * boolean, Boolean, byte, Byte, short, Short, int, Integer, long, Long, float, Float, double,
 * Double, BigDecimal, {@code java.util.Date}, etc.
 *
 * <p>To facilitate mapping between columns and fields that don't have matching names, try using
 * column aliases in the SQL statement like "select fname as first_name from customer".
 *
 * <p>For 'null' values read from the database, we will attempt to call the setter, but in the case
 * of Java primitives, this causes a TypeMismatchException. This class can be configured (using the
 * primitivesDefaultedForNullValue property) to trap this exception and use the primitives default
 * value. Be aware that if you use the values from the generated bean to update the database the
 * primitive value will have been set to the primitive's default value instead of null.
 *
...
public class BeanPropertyRowMapper<T> implements RowMapper<T> {
  ...
  @Nullable private Map<String, PropertyDescriptor> mappedFields;

  /** Set of bean properties we provide mapping for. */
  @Nullable private Set<String> mappedProperties;
  ...
  /**
   * Static factory method to create a new {@code BeanPropertyRowMapper} (with the mapped class
   * specified only once).
   *
   * @param mappedClass the class that each row should be mapped to
   */
