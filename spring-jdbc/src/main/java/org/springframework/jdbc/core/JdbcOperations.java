...
/**
 * Interface specifying a basic set of JDBC operations. Implemented by {@link JdbcTemplate}. Not
 * often used directly, but a useful option to enhance testability, as it can easily be mocked or
 * stubbed.
 *
 * <p>Alternatively, the standard JDBC infrastructure can be mocked. However, mocking this interface
 * constitutes significantly less work. As an alternative to a mock objects approach to testing data
 * access code, consider the powerful integration testing support provided in the {@code
 * org.springframework.test} package, shipped in {@code spring-test.jar}.
 *
 * @author Rod Johnson
...
public interface JdbcOperations {
  ...
  /**
   * Execute a JDBC data access operation, implemented as callback action working on a JDBC
   * Connection. This allows for implementing arbitrary data access operations, within Spring's
   * managed JDBC environment: that is, participating in Spring-managed transactions and converting
   * JDBC SQLExceptions into Spring's DataAccessException hierarchy.
   *
   * <p>The callback action can return a result object, for example a domain object or a collection
   * of domain objects.
   *
  ...
  /**
   * Execute a JDBC data access operation, implemented as callback action working on a JDBC
   * Statement. This allows for implementing arbitrary data access operations on a single Statement,
   * within Spring's managed JDBC environment: that is, participating in Spring-managed transactions
   ...
   *
   * <p>Uses a JDBC Statement, not a PreparedStatement. If you want to execute a static query with a
   * PreparedStatement, use the overloaded {@code query} method with {@code null} as argument array.
   *
  ...
  /**
   * Execute a query given static SQL, mapping a single result row to a result object via a
   * RowMapper.
   *
   * <p>Uses a JDBC Statement, not a PreparedStatement. If you want to execute a static query with a
   * PreparedStatement, use the overloaded {@link #queryForObject(String, RowMapper, Object...)}
   * method with {@code null} as argument array.
   *
   * @param sql the SQL query to execute
   * @param rowMapper a callback that will map one object per row
   * @return the single mapped object (may be {@code null} if the given {@link RowMapper} returned
   *     {@code} null)
   ...
   *
   * <p>Uses a JDBC Statement, not a PreparedStatement. If you want to execute a static query with a
   * PreparedStatement, use the overloaded {@link #queryForObject(String, Class, Object...)} method
   * with {@code null} as argument array.
   *
   * <p>This method is useful for running static SQL with a known outcome. The query is expected to
   * be a single row/single column query; the returned result will be directly mapped to the
   * corresponding object type.
   *
   * @param sql the SQL query to execute
   * @param requiredType the type that the result object is expected to match
   * @return the result object of the required type, or {@code null} in case of SQL NULL
   * @throws IncorrectResultSizeDataAccessException if the query does not return exactly one row, or
   *     does not return exactly one column in that row
   * @throws DataAccessException if there is any problem executing the query
   ...
   *
   * <p>Uses a JDBC Statement, not a PreparedStatement. If you want to execute a static query with a
   * PreparedStatement, use the overloaded {@link #queryForMap(String, Object...)} method with
   * {@code null} as argument array.
   *
   * <p>The query is expected to be a single row query; the result row will be mapped to a Map (one
   ...
   *
   * <p>Uses a JDBC Statement, not a PreparedStatement. If you want to execute a static query with a
   * PreparedStatement, use the overloaded {@code queryForList} method with {@code null} as argument
   * array.
   *
   * <p>The results will be mapped to a List (one entry for each row) of result objects, each of
   * them matching the specified element type.
   *
   * @param sql the SQL query to execute
   * @param elementType the required type of element in the result list (for example, {@code
   *     Integer.class})
   ...
   * array.
   *
   * <p>The results will be mapped to a List (one entry for each row) of Maps (one entry for each
   * column using the column name as the key). Each element in the list will be of the form returned
   * by this interface's {@code queryForMap} methods.
   *
   * @param sql the SQL query to execute
   ...
   *
   * <p>Uses a JDBC Statement, not a PreparedStatement. If you want to execute a static query with a
   * PreparedStatement, use the overloaded {@code queryForRowSet} method with {@code null} as
   * argument array.
   *
   * <p>The results will be mapped to an SqlRowSet which holds the data in a disconnected fashion.
   * This wrapper will translate any SQLExceptions thrown.
   *
   * <p>Note that, for the default implementation, JDBC RowSet support needs to be available at
   * runtime: by default, Sun's {@code com.sun.rowset.CachedRowSetImpl} class is used, which is part
   * of JDK 1.5+ and also available separately as part of Sun's JDBC RowSet Implementations download
   * (rowset.jar).
   *
   * @param sql the SQL query to execute
   * @return an SqlRowSet representation (possibly a wrapper around a {@code
   ...
   *
   * <p>Will fall back to separate updates on a single Statement if the JDBC driver does not support
   * batch updates.
   *
  ...
  /**
   * Execute a JDBC data access operation, implemented as callback action working on a JDBC
   * PreparedStatement. This allows for implementing arbitrary data access operations on a single
   * Statement, within Spring's managed JDBC environment: that is, participating in Spring-managed
   * transactions and converting JDBC SQLExceptions into Spring's DataAccessException hierarchy.
   *
   * <p>The callback action can return a result object, for example a domain object or a collection
   * of domain objects.
   *
   ...
   *
   * <p>A PreparedStatementCreator can either be implemented directly or configured through a
   ...
   *
   * @param sql the SQL query to execute
   * @param pss a callback that knows how to set values on the prepared statement. If this is {@code
   *     null}, the SQL will be assumed to contain no bind parameters. Even if there are no bind
   *     parameters, this callback may be used to set the fetch size and other performance options.
   * @param rse a callback that will extract results
   * @return an arbitrary result object, as returned by the ResultSetExtractor
   * @throws DataAccessException if there is any problem
   */
   ...
   * query, reading the ResultSet with a ResultSetExtractor.
   *
   * @param sql the SQL query to execute
   * @param args arguments to bind to the query
   * @param argTypes the SQL types of the arguments (constants from {@code java.sql.Types})
   * @param rse a callback that will extract results
   * @return an arbitrary result object, as returned by the ResultSetExtractor
   ...
   *     corresponding SQL type); may also contain {@link SqlParameterValue} objects which indicate
   *     not only the argument value but also the SQL type and optionally the scale
   * @param rse a callback that will extract results
   * @return an arbitrary result object, as returned by the ResultSetExtractor
   * @throws DataAccessException if the query fails
   */
  ...
  /**
   * Query using a prepared statement, reading the ResultSet on a per-row basis with a
   * RowCallbackHandler.
   *
   * <p>A PreparedStatementCreator can either be implemented directly or configured through a
   * PreparedStatementCreatorFactory.
  ...
  /**
   * Query given SQL to create a prepared statement from SQL and a PreparedStatementSetter
   * implementation that knows how to bind values to the query, reading the ResultSet on a per-row
  ...
  /**
   * Query given SQL to create a prepared statement from SQL and a PreparedStatementSetter
   * implementation that knows how to bind values to the query, mapping each row to a result object
   * via a RowMapper.
   *
   * @param sql the SQL query to execute
   * @param pss a callback that knows how to set values on the prepared statement. If this is {@code
  ...
  /**
   * Query given SQL to create a prepared statement from SQL and a list of arguments to bind to the
   * query, mapping each row to a result object via a RowMapper.
   *
   * @param sql the SQL query to execute
   * @param args arguments to bind to the query
   * @param argTypes the SQL types of the arguments (constants from {@code java.sql.Types})
  ...
  /**
   * Query given SQL to create a prepared statement from SQL and a list of arguments to bind to the
   * query, mapping a single result row to a result object via a RowMapper.
   *
   * @param sql the SQL query to execute
   * @param args arguments to bind to the query (leaving it to the PreparedStatement to guess the
  ...
  /**
   * Query given SQL to create a prepared statement from SQL and a list of arguments to bind to the
   * query, expecting a result object.
   *
   * <p>The query is expected to be a single row/single column query; the returned result will be
   * directly mapped to the corresponding object type.
   *
   * @param sql the SQL query to execute
   * @param args arguments to bind to the query
   * @param argTypes the SQL types of the arguments (constants from {@code java.sql.Types})
   ...
   * query, expecting a result map.
   *
   * <p>The {@code queryForMap} methods defined by this interface are appropriate when you don't
   * have a domain model. Otherwise, consider using one of the {@code queryForObject} methods.
   *
   * <p>The query is expected to be a single row query; the result row will be mapped to a Map (one
   * entry for each column, using the column name as the key).
   *
   ...
   *
   * <p>The results will be mapped to a List (one entry for each row) of Maps (one entry for each
   * column, using the column name as the key). Each element in the list will be of the form
   * returned by this interface's {@code queryForMap} methods.
   *
   * @param sql the SQL query to execute
   * @param args arguments to bind to the query
   * @param argTypes the SQL types of the arguments (constants from {@code java.sql.Types})
  ...
  /**
   * Issue an update statement using a PreparedStatementCreator to provide SQL and any required
   * parameters. Generated keys will be put into the given KeyHolder.
   *
   * <p>Note that the given PreparedStatementCreator has to create a statement with activated
   * extraction of generated keys (a JDBC 3.0 feature). This can either be done directly or through
   * using a PreparedStatementCreatorFactory.
   *
   * @param psc a callback that provides SQL and any necessary parameters
  ...
  /**
   * Issue an update statement using a PreparedStatementSetter to set bind parameters, with given
   * SQL. Simpler than using a PreparedStatementCreator as this method will create the
   * PreparedStatement: The PreparedStatementSetter just needs to set parameters.
   *
   * @param sql the SQL containing bind parameters
   * @param pss helper that sets bind parameters. If this is {@code null} we run an update with
   *     static SQL.
   * @return the number of rows affected
   * @throws DataAccessException if there is any problem issuing the update
   */
  ...
  /**
   * Issue multiple update statements on a single PreparedStatement, using batch updates and a
   * BatchPreparedStatementSetter to set values.
   *
   * <p>Will fall back to separate updates on a single PreparedStatement if the JDBC driver does not
   * support batch updates.
   *
   * @param sql defining PreparedStatement that will be reused. All statements in the batch will use
   *     the same SQL.
   * @param pss object to set parameters on the PreparedStatement created by this method
   * @return an array of the number of rows affected by each statement
   * @throws DataAccessException if there is any problem issuing the update
   */
   ...
   * @return an array containing for each batch another array containing the numbers of rows
  ...
  /**
   * Execute a JDBC data access operation, implemented as callback action working on a JDBC
   * CallableStatement. This allows for implementing arbitrary data access operations on a single
   * Statement, within Spring's managed JDBC environment: that is, participating in Spring-managed
   * transactions and converting JDBC SQLExceptions into Spring's DataAccessException hierarchy.
   *
   * <p>The callback action can return a result object, for example a domain object or a collection
