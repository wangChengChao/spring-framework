...
/**
 * Abstract implementation of the SqlTypeValue interface, for convenient creation of type values
 * that are supposed to be passed into the {@code PreparedStatement.setObject} method. The {@code
 * createTypeValue} callback method has access to the underlying Connection, if that should be
 * needed to create any database-specific objects.
 *
 * <p>A usage example from a StoredProcedure (compare this to the plain SqlTypeValue version in the
 * superclass javadoc):
 *
...
public abstract class AbstractSqlTypeValue implements SqlTypeValue {
  ...
  @Override
  public final void setTypeValue(
