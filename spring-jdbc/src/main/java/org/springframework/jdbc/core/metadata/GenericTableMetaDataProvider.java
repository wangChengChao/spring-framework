...
/**
 * A generic implementation of the {@link TableMetaDataProvider} interface which should provide
...
public class GenericTableMetaDataProvider implements TableMetaDataProvider {
  ...
  @Nullable private String databaseVersion;
  ...
  public void setGeneratedKeysColumnNameArraySupported(
      boolean generatedKeysColumnNameArraySupported) {
  ...
  private void locateTableAndProcessMetaData(
    ...
    if (tableMeta.isEmpty()) {
      if (logger.isInfoEnabled()) {
        logger.info(
            "Unable to locate table meta-data for '"
                + tableName
  ...
  private TableMetaData findTableMetaData(
      @Nullable String schemaName,
      @Nullable String tableName,
