...
/**
 * Generates classes to handle multi-valued keys, for use in things such as Maps and Sets. Code for
 * <code>equals</code> and <code>hashCode</code> methods follow the the rules laid out in
 * <i>Effective Java</i> by Joshua Bloch.
 *
 * <p>To generate a <code>KeyFactory</code>, you need to supply an interface which describes the
 * structure of the key. The interface should have a single method named <code>newInstance</code>,
 * which returns an <code>Object</code>. The arguments array can be <i>anything</i>--Objects,
 * primitive values, or single or multi-dimension arrays of either. For example:
 *
 * <p>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Once you have made a <code>KeyFactory</code>, you generate a new key by calling the <code>
 * newInstance</code> method defined by your interface.
 *
 * <p>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p><b>Note:</b> <code>hashCode</code> equality between two keys <code>key1</code> and <code>key2
 * </code> is only guaranteed if <code>key1.equals(key2)</code> <i>and</i> the keys were produced by
 * the same factory.
 *
...
public abstract class KeyFactory {
  ...
  /**
   * {@link Type#hashCode()} is very expensive as it traverses full descriptor to calculate hash
   * code. This customizer uses {@link Type#getSort()} as a hash code.
   */
  public static final HashCodeCustomizer HASH_ASM_TYPE =
      new HashCodeCustomizer() {
  ...
  /**
   * @deprecated this customizer might result in unexpected class leak since key object still holds
   *     a strong reference to the Object and class. It is recommended to have pre-processing method
   *     that would strip Objects and represent Classes as Strings
   */
  @Deprecated
  ...
  public static KeyFactory create(
  ...
  public static class Generator extends AbstractClassGenerator {
    ...
    private static final Class[] KNOWN_CUSTOMIZER_TYPES =
        new Class[] {Customizer.class, FieldTypeCustomizer.class};
    ...
    public void generateClass(ClassVisitor v) {
      ...
      for (int i = 0; i < parameterTypes.length; i++) {
        ...
        ce.declare_field(
            Constants.ACC_PRIVATE | Constants.ACC_FINAL, getFieldName(i), fieldType, null);
        e.dup();
        e.load_arg(i);
        for (FieldTypeCustomizer customizer : fieldTypeCustomizers) {
