...
/**
 * Spring-specific variant of {@link ObjenesisStd} / {@link ObjenesisBase}, providing a cache based
 * on {@code Class} keys instead of class names, and allowing for selective use of the cache.
 *
...
public class SpringObjenesis implements Objenesis {
   ...
   * Setting this flag to "true" is equivalent to letting Spring find out that Objenesis isn't
   * working at runtime, triggering the fallback code path immediately: Most importantly, this means
   * that all CGLIB AOP proxies will be created through regular instantiation via a default
   * constructor.
   */
  ...
  /**
   * Return whether this Objenesis instance is worth trying for instance creation, i.e. whether it
   * hasn't been used yet or is known to work.
   *
   * <p>If the configured Objenesis instantiator strategy has been identified to not work on the
   * current JVM at all or if the "spring.objenesis.ignore" property has been set to "true", this
   ...
   * @param useCache whether to use the instantiator cache (typically {@code true} but can be set to
   *     {@code false} e.g. for reloadable classes)
