...
public class OverridingClassLoader extends DecoratingClassLoader {
  ...
  @Nullable private final ClassLoader overrideDelegate;
   ...
   * @since 4.3
   */
  public OverridingClassLoader(
      @Nullable ClassLoader parent, @Nullable ClassLoader overrideDelegate) {
    super(parent);
   ...
   *
   * <p>The default implementation delegates to {@link #findLoadedClass}, {@link #loadBytesForClass}
   * and {@link #defineClass}.
   *
   ...
   *
   * @param name the name of the class
   * @return the byte content (with transformers already applied), or {@code null} if no class
   ...
   * @return the transformed bytes (never {@code null}; same as the input bytes if the
   *     transformation produced no changes)
   */
