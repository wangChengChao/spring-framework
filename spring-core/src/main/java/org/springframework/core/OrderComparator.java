 ...
 *
 * <p>{@link PriorityOrdered} objects will be sorted with higher priority than <em>plain</em> {@code
 * Ordered} objects.
 *
 * <h3>Same Order Objects</h3>
 *
 * <p>Objects that have the same order value will be sorted with arbitrary ordering with respect to
 * other objects with the same order value.
 *
 * <h3>Non-ordered Objects</h3>
 *
 * <p>Any object that does not provide its own order value is implicitly assigned a value of {@link
 * Ordered#LOWEST_PRECEDENCE}, thus ending up at the end of a sorted collection in arbitrary order
...
public class OrderComparator implements Comparator<Object> {
   ...
   *
   * <p>The default implementation checks against the given {@link OrderSourceProvider} using {@link
   * #findOrder} and falls back to a regular {@link #getOrder(Object)} call.
   *
   ...
   * Determine the order value for the given object.
   *
   * <p>The default implementation checks against the {@link Ordered} interface through delegating
   * to {@link #findOrder}. Can be overridden in subclasses.
   *
   * @param obj the object to check
   * @return the order value, or {@code Ordered.LOWEST_PRECEDENCE} as fallback
   */
   ...
   *
   * <p>The default implementation checks against the {@link Ordered} interface. Can be overridden
   * in subclasses.
   *
   * @param obj the object to check
   ...
   *
   * <p>Optimized to skip sorting for lists with size 0 or 1, in order to avoid unnecessary array
   * extraction.
   *
  ...
  /**
   * Sort the given array or List with a default OrderComparator, if necessary. Simply skips sorting
   * when given any other value.
   *
   * <p>Optimized to skip sorting for lists with size 0 or 1, in order to avoid unnecessary array
   * extraction.
   *
  ...
  public interface OrderSourceProvider {
     ...
     *
     * <p>If the returned object does not indicate any order, the comparator will fall back to
