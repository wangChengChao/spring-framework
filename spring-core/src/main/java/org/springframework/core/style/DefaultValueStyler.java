...
/**
 * Converts objects to String form, generally for debugging purposes, using Spring's {@code
 * toString} styling conventions.
 *
 * <p>Uses the reflective visitor pattern underneath the hood to nicely encapsulate styling
...
public class DefaultValueStyler implements ValueStyler {
  ...
  private String styleArray(Object[] array) {
    if (array.length == 0) {
      return ARRAY
          + '<'
          + ClassUtils.getShortName(array.getClass().getComponentType())
          + '>'
          + EMPTY;
    }
