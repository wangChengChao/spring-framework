...
/**
 * A simple log message type for use with Commons Logging, allowing for convenient lazy resolution
 * of a given {@link Supplier} instance (typically bound to a Java 8 lambda expression) or a
 * printf-style format string ({@link String#format}) in its {@link #toString()}.
 *
...
public abstract class LogMessage implements CharSequence {
  ...
  /**
   * This will be called by the logging provider, potentially once per log target (therefore locally
   ...
   * @see String#format(String, Object...)
   */
  public static LogMessage format(
