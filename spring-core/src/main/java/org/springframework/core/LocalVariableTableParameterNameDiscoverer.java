...
/**
 * Implementation of {@link ParameterNameDiscoverer} that uses the LocalVariableTable information in
 * the method attributes to discover parameter names. Returns {@code null} if the class file was
 * compiled without debug information.
 *
 * <p>Uses ObjectWeb's ASM library for analyzing class files. Each discoverer instance caches the
 * ASM discovered information for each introspected Class, in a thread-safe manner. It is
 * recommended to reuse ParameterNameDiscoverer instances as far as possible.
 *
...
public class LocalVariableTableParameterNameDiscoverer implements ParameterNameDiscoverer {
  ...
  private static final Log logger =
      LogFactory.getLog(LocalVariableTableParameterNameDiscoverer.class);
  ...
  // size
  private final Map<Class<?>, Map<Executable, String[]>> parameterNamesCache =
   ...
   *
   * <p>Exceptions will be logged, and a marker map returned to indicate the lack of debug
   * information.
   */
  ...
  /**
   * Helper class that inspects all methods and constructors and then attempts to find the parameter
   * names for the given {@link Executable}.
   */
  private static class ParameterNameDiscoveringVisitor extends ClassVisitor {
    ...
    public ParameterNameDiscoveringVisitor(
        Class<?> clazz, Map<Executable, String[]> executableMap) {
    ...
    public MethodVisitor visitMethod(
      ...
      if (!isSyntheticOrBridged(access) && !STATIC_CLASS_INIT.equals(name)) {
        return new LocalVariableTableVisitor(
            this.clazz, this.executableMap, name, desc, isStatic(access));
      }
      return null;
    }
