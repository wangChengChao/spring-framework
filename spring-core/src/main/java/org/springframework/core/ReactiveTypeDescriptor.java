...
/**
 * Describes the semantics of a reactive type including boolean checks for {@link #isMultiValue()},
 * {@link #isNoValue()}, and {@link #supportsEmpty()}.
 *
...
public final class ReactiveTypeDescriptor {
  ...
  @Nullable private final Supplier<?> emptyValueSupplier;
  ...
  /**
   * Return {@code true} if the reactive type can produce more than 1 value can be produced and is
   * therefore a good fit to adapt to {@code Flux}. A {@code false} return value implies the
   * reactive type can produce 1 value at most and is therefore a good fit to adapt to {@code Mono}.
   */
  ...
  /**
   * Return {@code true} if the reactive type does not produce any values and only provides
  ...
  /**
   * Return an empty-value instance for the underlying reactive or async type. Use of this type
   * implies {@link #supportsEmpty()} is true.
   */
   ...
   * @param emptySupplier a supplier of an empty-value instance of the reactive type
   */
  public static ReactiveTypeDescriptor singleOptionalValue(
      Class<?> type, Supplier<?> emptySupplier) {
