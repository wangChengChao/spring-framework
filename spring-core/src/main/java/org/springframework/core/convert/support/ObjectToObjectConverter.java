...
/**
 * Generic converter that uses conventions to convert a source object to a {@code targetType} by
 * delegating to a method on the source object or to a static factory method or constructor on the
 * {@code targetType}.
 *
 * <h3>Conversion Algorithm</h3>
 *
 * <ol>
 *   <li>Invoke a non-static {@code to[targetType.simpleName]()} method on the source object that
 *       has a return type equal to {@code targetType}, if such a method exists. For example, {@code
 *       org.example.Bar Foo#toBar()} is a method that follows this convention.
 *   <li>Otherwise invoke a <em>static</em> {@code valueOf(sourceType)} or Java 8 style
 *       <em>static</em> {@code of(sourceType)} or {@code from(sourceType)} method on the {@code
 *       targetType}, if such a method exists.
 *   <li>Otherwise invoke a constructor on the {@code targetType} that accepts a single {@code
 *       sourceType} argument, if such a constructor exists.
 *   <li>Otherwise throw a {@link ConversionFailedException}.
 * </ol>
 *
 * <p><strong>Warning</strong>: this converter does <em>not</em> support the {@link
 * Object#toString()} method for converting from a {@code sourceType} to {@code java.lang.String}.
 * For {@code toString()} support, use {@link FallbackObjectToStringConverter} instead.
 *
 * @author Keith Donald
...
final class ObjectToObjectConverter implements ConditionalGenericConverter {
  ...
  private static Method determineToMethod(Class<?> targetClass, Class<?> sourceClass) {
    ...
    Method method =
        ClassUtils.getMethodIfAvailable(sourceClass, "to" + targetClass.getSimpleName());
    return (method != null
  ...
  @Nullable
  private static Constructor<?> determineFactoryConstructor(
