...
/**
 * A {@link Comparator} that converts values before they are compared. The specified {@link
 * Converter} will be used to convert each value before it passed to the underlying {@code
 * Comparator}.
 *
 * @author Phillip Webb
 * @since 3.2
...
public class ConvertingComparator<S, T> implements Comparator<S> {
  ...
  /**
   * Create a new {@link ConvertingComparator} that compares {@link java.util.Map.Entry map *
   * entries} based on their {@link java.util.Map.Entry#getKey() keys}.
   *
  ...
  /**
   * Create a new {@link ConvertingComparator} that compares {@link java.util.Map.Entry map entries}
   * based on their {@link java.util.Map.Entry#getValue() values}.
   *
  ...
  private static class ConversionServiceConverter<S, T> implements Converter<S, T> {
    ...
    public ConversionServiceConverter(
