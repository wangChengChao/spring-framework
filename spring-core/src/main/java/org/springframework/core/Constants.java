...
/**
 * This class can be used to parse other classes containing constant definitions in public static
 * final members. The {@code asXXXX} methods of this class allow these constant values to be
 * accessed via their string names.
 *
 * <p>Consider class Foo containing {@code public final static int CONSTANT1 = 66;} An instance of
 * this class wrapping {@code Foo.class} will return the constant value of 66 from its {@code
 * asNumber} method given the argument {@code "CONSTANT1"}.
 *
 * <p>This class is ideal for use in PropertyEditors, enabling them to recognize the same names as
 * the constants themselves, and freeing them from maintaining their own mapping.
 *
 * @author Rod Johnson
...
public class Constants {
   ...
   * @throws ConstantException if the field name wasn't found or if the type wasn't compatible with
   *     Number
   * @see #asObject
   */
  ...
  /**
   * Parse the given String (upper or lower case accepted) and return the appropriate value if it's
   * the name of a constant field in the class that we're analysing.
   *
   * @param code the name of the field (never {@code null})
   ...
   *
   * <p>Note that this method assumes that constants are named in accordance with the standard Java
   * convention for constant values (i.e. all uppercase). The supplied {@code namePrefix} will be
   * uppercased (in a locale-insensitive fashion) prior to the main logic of this method kicking in.
   *
   ...
   *
   * <p>Note that this method assumes that constants are named in accordance with the standard Java
   * convention for constant values (i.e. all uppercase). The supplied {@code nameSuffix} will be
   * uppercased (in a locale-insensitive fashion) prior to the main logic of this method kicking in.
   *
  ...
  /**
   * Look up the given value within the group of constants for the given bean property name. Will
   ...
   * @throws ConstantException if the value wasn't found
   */
  public String toCodeForSuffix(Object value, @Nullable String nameSuffix)
   ...
   *
   * <p>Uses a common naming idiom: turning all lower case characters to upper case, and prepending
