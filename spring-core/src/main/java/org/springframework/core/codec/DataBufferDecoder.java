 ...
 *
 * <p><strong>Note:</strong> The data buffers should be released via {@link
 * org.springframework.core.io.buffer.DataBufferUtils#release(DataBuffer)} after they have been
 * consumed. In addition, if using {@code Flux} or {@code Mono} operators such as flatMap, reduce,
 * and others that prefetch, cache, and skip or filter out data items internally, please add {@code
 * doOnDiscard(PooledDataBuffer.class, DataBufferUtils::release)} to the composition chain to ensure
