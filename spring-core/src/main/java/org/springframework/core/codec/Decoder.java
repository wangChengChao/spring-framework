...
/**
 * Strategy for decoding a {@link DataBuffer} input stream into an output stream of elements of type
 * {@code <T>}.
 *
...
public interface Decoder<T> {
  ...
  /**
   * Whether the decoder supports the given target element type and the MIME type of the source
   * stream.
   *
   * @param elementType the target element type for the output stream
   * @param mimeType the mime type associated with the stream to decode (can be {@code null} if not
   *     specified)
   ...
   * @param elementType the expected type of elements in the output stream; this type must have been
   *     previously passed to the {@link #canDecode} method and it must have returned {@code true}.
  ...
  /**
   * Decode a data buffer to an Object of type T. This is useful when the input stream consists of
   * discrete messages (or events) and the content for each can be decoded on its own.
   *
  ...
  default T decode(
    ...
    if (ex != null) {
      throw (ex instanceof CodecException
          ? (CodecException) ex
