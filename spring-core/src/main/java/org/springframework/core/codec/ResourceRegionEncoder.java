...
public class ResourceRegionEncoder extends AbstractEncoder<ResourceRegion> {
  ...
  public static final String BOUNDARY_STRING_HINT =
      ResourceRegionEncoder.class.getName() + ".boundaryString";
  ...
  public Flux<DataBuffer> encode(
      Publisher<? extends ResourceRegion> input,
    ...
    if (input instanceof Mono) {
      ...
      byte[] contentType =
          mimeType != null ? toAsciiBytes("Content-Type: " + mimeType + "\r\n") : new byte[0];
      ...
      return Flux.from(input)
          .concatMap(
              region -> {
                ...
                Flux<DataBuffer> prefix =
                    Flux.just(
                        bufferFactory.wrap(startBoundary),
                        bufferFactory.wrap(contentType),
                        bufferFactory.wrap(
                            getContentRangeHeader(region))); // only wrapping, no allocation
