...
public abstract class Hints {
  ...
  /**
   * Name of boolean hint whether to avoid logging data either because it's potentially sensitive,
   * or because it has been logged by a composite encoder, e.g. for multipart requests.
   */
  ...
  /**
   * Merge two maps of hints, creating and copying into a new map if both have values, or returning
   * the non-empty map, or an empty map if both are empty.
   *
  ...
  /**
   * Merge a single hint into a map of hints, possibly creating and copying all hints into a new
   * map, or otherwise if the map of hints is empty, creating a new single entry map.
   *
  ...
  public static Map<String, Object> merge(
      Map<String, Object> hints, String hintName, Object hintValue) {
    if (hints.isEmpty()) {
