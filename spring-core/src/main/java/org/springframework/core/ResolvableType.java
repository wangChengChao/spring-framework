 ...
 * parameters} along with the ability to ultimately {@link #resolve() resolve} to a {@link
 * java.lang.Class}.
 *
 * <p>{@code ResolvableTypes} may be obtained from {@link #forField(Field) fields}, {@link
 * #forMethodParameter(Method, int) method parameters}, {@link #forMethodReturnType(Method) method
 * returns} or {@link #forClass(Class) classes}. Most methods on this class will themselves return
...
public class ResolvableType implements Serializable {
  ...
  @Nullable private final ResolvableType componentType;

  @Nullable private final Integer hash;

  @Nullable private Class<?> resolved;

  @Nullable private volatile ResolvableType superType;

  @Nullable private volatile ResolvableType[] interfaces;

  @Nullable private volatile ResolvableType[] generics;
  ...
  /**
   * Return the underlying source of the resolvable type. Will return a {@link Field}, {@link
   * MethodParameter} or {@link Type} depending on how the {@link ResolvableType} was constructed.
   * With the exception of the {@link #NONE} constant, this method will never return {@code null}.
   ...
   * Determine whether this {@code ResolvableType} is assignable from the specified other type.
   *
   * <p>Attempts to follow the same rules as the Java compiler, considering whether both the {@link
   * #resolve() resolved} {@code Class} is {@link Class#isAssignableFrom(Class) assignable from} the
   * given type as well as whether all {@link #getGenerics() generics} are assignable.
   *
   * @param other the type to be checked against (as a {@code ResolvableType})
   * @return {@code true} if the specified other type can be assigned to this {@code
   *     ResolvableType}; {@code false} otherwise
   */
  ...
  public ResolvableType getComponentType() {
    ...
    if (this.type instanceof GenericArrayType) {
      return forType(
          ((GenericArrayType) this.type).getGenericComponentType(), this.variableResolver);
    }
    return resolveType().getComponentType();
  }

  /**
   * Convenience method to return this type as a resolvable {@link Collection} type. Returns {@link
   * #NONE} if this type does not implement or extend {@link Collection}.
   *
   * @see #as(Class)
   * @see #asMap()
   */
  ...
  /**
   * Convenience method to return this type as a resolvable {@link Map} type. Returns {@link #NONE}
   * if this type does not implement or extend {@link Map}.
   *
   * @see #as(Class)
   * @see #asCollection()
   */
  ...
  /**
   * Return this type as a {@link ResolvableType} of the specified class. Searches {@link
   * #getSuperType() supertype} and {@link #getInterfaces() interface} hierarchies to find a match,
   ...
   * @return a {@link ResolvableType} representing this object as the specified type, or {@link
   *     #NONE} if not resolvable as that type
   * @see #asCollection()
   * @see #asMap()
   * @see #getSuperType()
   * @see #getInterfaces()
  ...
  /**
   * Return a {@link ResolvableType} for the specified nesting level. See {@link #getNested(int,
   * Map)} for details.
   *
   ...
   *
   * <p>The nesting level refers to the specific generic parameter that should be returned. A
   * nesting level of 1 indicates this type; 2 indicates the first nested generic; 3 the second; and
   * so on. For example, given {@code List<Set<Integer>>} level 1 refers to the {@code List}, level
   * 2 the {@code Set}, and level 3 the {@code Integer}.
   *
   * <p>The {@code typeIndexesPerLevel} map can be used to reference a specific generic for the
   * given level. For example, an index of 0 would refer to a {@code Map} key; whereas, 1 would
   * refer to the value. If the map does not contain a value for a specific level the last generic
   * will be used (e.g. a {@code Map} value).
   *
   * <p>Nesting levels may also apply to array types; for example given {@code String[]}, a nesting
   * level of 2 refers to {@code String}.
   *
   * <p>If a type does not {@link #hasGenerics() contain} generics the {@link #getSuperType()
   ...
   * Indexes are zero based; for example given the type {@code Map<Integer, List<String>>}, {@code
   * getGeneric(0)} will access the {@code Integer}. Nested generics can be accessed by specifying
   * multiple indexes; for example {@code getGeneric(1, 0)} will access the {@code String} from the
   * nested {@code List}. For convenience, if no indexes are specified the first generic is
   * returned.
   *
   ...
   * of this type. If no generics are available an empty array is returned. If you need to access a
   * specific generic consider using the {@link #getGeneric(int...)} method as it allows access to
   * nested generics and protects against {@code IndexOutOfBoundsExceptions}.
   *
   * @return an array of {@link ResolvableType ResolvableTypes} representing the generic parameters
   *     (never {@code null})
   * @see #hasGenerics()
   ...
   * parameters.
   *
   * @return an array of resolved generic parameters (the resulting array will never be {@code
   *     null}, but it may contain {@code null} elements})
   * @see #getGenerics()
   * @see #resolve()
   */
   ...
   * resolved. This method will consider bounds of {@link TypeVariable TypeVariables} and {@link
   * WildcardType WildcardTypes} if direct resolution fails; however, bounds of {@code Object.class}
   * will be ignored.
   *
   * <p>If this method returns a non-null {@code Class} and {@link #hasGenerics()} returns {@code
   * false}, the given type effectively wraps a plain {@code Class}, allowing for plain {@code
   * Class} processing if desirable.
   *
   ...
   * and {@link WildcardType WildcardTypes} if direct resolution fails; however, bounds of {@code
   * Object.class} will be ignored.
   *
   * @param fallback the fallback class to use if resolution fails
   ...
   *
   * <p>Note: The returned {@link ResolvableType} should only be used as an intermediary as it
  ...
  public boolean equals(@Nullable Object other) {
    ...
    if (this.typeProvider != otherType.typeProvider
        && (this.typeProvider == null
            ...
            || !ObjectUtils.nullSafeEquals(
                this.typeProvider.getType(), otherType.typeProvider.getType()))) {
      return false;
    }
    if (this.variableResolver != otherType.variableResolver
        && (this.variableResolver == null
            ...
            || !ObjectUtils.nullSafeEquals(
                this.variableResolver.getSource(), otherType.variableResolver.getSource()))) {
      return false;
    }
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link Class}, using the full generic type
   * information for assignability checks. For example: {@code
   * ResolvableType.forClass(MyArrayList.class)}.
   *
   * @param clazz the class to introspect ({@code null} is semantically equivalent to {@code
   *     Object.class} for typical use cases here}
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link Class}, doing assignability checks
   * against the raw class only (analogous to {@link Class#isAssignableFrom}, which this serves as a
   * wrapper for. For example: {@code ResolvableType.forRawClass(List.class)}.
   *
   * @param clazz the class to introspect ({@code null} is semantically equivalent to {@code
   *     Object.class} for typical use cases here)
   * @return a {@link ResolvableType} for the specified class
  ...
  /**
   * Return a {@link ResolvableType} for the specified base type (interface or base class) with a
   * given implementation class. For example: {@code ResolvableType.forClass(List.class,
   * MyArrayList.class)}.
   *
   * @param baseType the base type (must not be {@code null})
   * @param implementationClass the implementation class
   * @return a {@link ResolvableType} for the specified base type backed by the given implementation
   *     class
   * @see #forClass(Class)
   * @see #forClassWithGenerics(Class, Class...)
   */
  ...
  public static ResolvableType forClassWithGenerics(Class<?> clazz, ResolvableType... generics) {
    ...
    for (int i = 0; i < generics.length; i++) {
      ...
      arguments[i] =
          (argument != null && !(argument instanceof TypeVariable) ? argument : variables[i]);
    }
  ...
  /**
   * Return a {@link ResolvableType} for the specified instance. The instance does not convey
   * generic information but if it implements {@link ResolvableTypeProvider} a more precise {@link
   * ResolvableType} can be used than the simple one based on the {@link #forClass(Class) Class
   * instance}.
   *
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link Field} with a given implementation.
   *
   * <p>Use this variant when the class that declares the field includes generic parameter variables
   * that are satisfied by the implementation class.
   *
   * @param field the source field
   * @param implementationClass the implementation class
   * @return a {@link ResolvableType} for the specified field
   ...
   *
   * <p>Use this variant when the class that declares the field includes generic parameter variables
   * that are satisfied by the implementation type.
   *
   * @param field the source field
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link Field} with the given nesting level.
   *
   * @param field the source field
   * @param nestingLevel the nesting level (1 for the outer level; 2 for a nested generic type; etc)
   * @see #forField(Field)
   */
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link Field} with a given implementation and
  ...
  public static ResolvableType forField(
      Field field, int nestingLevel, @Nullable Class<?> implementationClass) {
    Assert.notNull(field, "Field must not be null");
    ResolvableType owner = forType(implementationClass).as(field.getDeclaringClass());
    return forType(null, new FieldTypeProvider(field), owner.asVariableResolver())
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link Constructor} parameter with a given
   * implementation. Use this variant when the class that declares the constructor includes generic
   ...
   */
  public static ResolvableType forConstructorParameter(
      Constructor<?> constructor, int parameterIndex, Class<?> implementationClass) {

    Assert.notNull(constructor, "Constructor must not be null");
    MethodParameter methodParameter =
        new MethodParameter(constructor, parameterIndex, implementationClass);
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link Method} return type. Use this variant
   * when the class that declares the method includes generic parameter variables that are satisfied
  ...
  public static ResolvableType forMethodParameter(
    ...
    Assert.notNull(method, "Method must not be null");
    MethodParameter methodParameter =
        new MethodParameter(method, parameterIndex, implementationClass);
    return forMethodParameter(methodParameter);
  }
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link MethodParameter} with a given
   * implementation type. Use this variant when the class that declares the method includes generic
   ...
   */
  public static ResolvableType forMethodParameter(
      MethodParameter methodParameter, @Nullable ResolvableType implementationType) {
    ...
    return forType(
            null, new MethodParameterTypeProvider(methodParameter), owner.asVariableResolver())
   ...
   */
  public static ResolvableType forMethodParameter(
      MethodParameter methodParameter, @Nullable Type targetType) {
    Assert.notNull(methodParameter, "MethodParameter must not be null");
    return forMethodParameter(methodParameter, targetType, methodParameter.getNestingLevel());
  }

  /**
   * Return a {@link ResolvableType} for the specified {@link MethodParameter} at a specific nesting
   * level, overriding the target type to resolve with a specific given type.
   *
   * @param methodParameter the source method parameter (must not be {@code null})
   * @param targetType the type to resolve (a part of the method parameter's type)
  ...
  /**
   * Return a {@link ResolvableType} for the specified {@link Type} backed by a given {@link
   * VariableResolver}.
   *
   * @param type the source type or {@code null}
