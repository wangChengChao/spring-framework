...
/**
 * Provides a quick way to access the attribute methods of an {@link Annotation} with consistent
 * ordering as well as a few useful utility methods.
 *
 * @author Phillip Webb
 * @since 5.2
 */
final class AttributeMethods {
  ...
  private AttributeMethods(
      @Nullable Class<? extends Annotation> annotationType, Method[] attributeMethods) {
  ...
  /**
   * Determine if values from the given annotation can be safely accessed without causing any {@link
   * TypeNotPresentException TypeNotPresentExceptions}.
   *
  ...
  /**
   * Check if values from the given annotation can be safely accessed without causing any {@link
   * TypeNotPresentException TypeNotPresentExceptions}. In particular, this method is designed to
   * cover Google App Engine's late arrival of such exceptions for {@code Class} values (instead of
   * the more typical early {@code Class.getAnnotations() failure}.
   *
  ...
  /**
   * Get the attribute with the specified name or {@code null} if no matching attribute exists.
   *
  ...
  /**
   * Determine if the attribute at the specified index could throw a {@link TypeNotPresentException}
   * when accessed.
   *
  ...
  /**
   * Get the index of the attribute with the specified name, or {@code -1} if there is no attribute
   * with the name.
   *
  ...
  /**
   * Get the index of the specified attribute, or {@code -1} if the attribute is not in this
   * collection.
   *
   ...
   *
   * @return {@code true} if there is at least one attribute method with a nested annotation type
   */
  ...
  /**
   * Create a description for the given attribute method suitable to use in exception messages and
   * logs.
   *
