...
abstract class AbstractMergedAnnotation<A extends Annotation> implements MergedAnnotation<A> {
  ...
  @Nullable private volatile A synthesizedAnnotation;
  ...
  public AnnotationAttributes asAnnotationAttributes(Adapt... adaptations) {
    return asMap(
        mergedAnnotation -> new AnnotationAttributes(mergedAnnotation.getType()), adaptations);
  }
   ...
   * @param type the type to return (see {@link MergedAnnotation} class documentation for details)
   * @return the attribute value or {@code null} if the value is not found and is not required
