...
/**
 * Strategy used to determine annotations that act as containers for other annotations. The {@link
 * #standardRepeatables()} method provides a default strategy that respects Java's {@link
 * Repeatable @Repeatable} support and should be suitable for most situations.
 *
 * <p>The {@link #of} method can be used to register relationships for annotations that do not wish
 * to use {@link Repeatable @Repeatable}.
 *
...
public abstract class RepeatableContainers {
  ...
  /**
   * Create a {@link RepeatableContainers} instance that searches using Java's {@link
  ...
  /**
   * Create a {@link RepeatableContainers} instance that uses a defined container and repeatable
   * type.
   *
   * @param repeatable the contained repeatable annotation
   * @param container the container annotation or {@code null}. If specified, this annotation must
   *     declare a {@code value} attribute returning an array of repeatable annotations. If not
   *     specified, the container will be deduced by inspecting the {@code @Repeatable} annotation
   *     on {@code repeatable}.
   * @return a {@link RepeatableContainers} instance
   */
  ...
  /**
   * Create a {@link RepeatableContainers} instance that does not expand any repeatable annotations.
   *
   * @return a {@link RepeatableContainers} instance
   */
  ...
  /**
   * Standard {@link RepeatableContainers} implementation that searches using Java's {@link
   * Repeatable @Repeatable} annotation.
   */
    ...
    private static Method getRepeatedAnnotationsMethod(Class<? extends Annotation> annotationType) {
      Object result =
          cache.computeIfAbsent(
              annotationType, StandardRepeatableContainers::computeRepeatedAnnotationsMethod);
  ...
  private static class ExplicitRepeatableContainer extends RepeatableContainers {
    ...
    ExplicitRepeatableContainer(
        @Nullable RepeatableContainers parent,
      ...
      Method valueMethod =
          AttributeMethods.forAnnotationType(container).get(MergedAnnotation.VALUE);
      try {
        if (valueMethod == null) {
    ...
    public boolean equals(@Nullable Object other) {
      ...
      return (this.container.equals(otherErc.container)
          && this.repeatable.equals(otherErc.repeatable));
    }
