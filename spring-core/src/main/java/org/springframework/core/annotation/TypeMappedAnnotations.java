...
/**
 * {@link MergedAnnotations} implementation that searches for and adapts annotations and
 * meta-annotations using {@link AnnotationTypeMappings}.
 *
 * @author Phillip Webb
 * @since 5.2
 */
final class TypeMappedAnnotations implements MergedAnnotations {
  ...
  static final MergedAnnotations NONE =
      new TypeMappedAnnotations(
          null, new Annotation[0], RepeatableContainers.none(), AnnotationFilter.ALL);

  @Nullable private final Object source;

  @Nullable private final AnnotatedElement element;

  @Nullable private final SearchStrategy searchStrategy;
  ...
  @Nullable private volatile List<Aggregate> aggregates;
  ...
  public <A extends Annotation> boolean isDirectlyPresent(Class<A> annotationType) {
    ...
    return Boolean.TRUE.equals(
        scan(
            annotationType, IsPresent.get(this.repeatableContainers, this.annotationFilter, true)));
  }
  ...
  public <A extends Annotation> MergedAnnotation<A> get(
    ...
    MergedAnnotation<A> result =
        scan(annotationType, new MergedAnnotationFinder<>(annotationType, predicate, selector));
  ...
  static MergedAnnotations from(
    ...
    return new TypeMappedAnnotations(
  ...
  /**
   * {@link AnnotationsProcessor} used to detect if an annotation is directly present or
   * meta-present.
   */
  private static final class IsPresent implements AnnotationsProcessor<Object, Boolean> {
    ...
    static {
      ...
      SHARED[2] =
          new IsPresent(RepeatableContainers.standardRepeatables(), AnnotationFilter.PLAIN, true);
      SHARED[3] =
          new IsPresent(RepeatableContainers.standardRepeatables(), AnnotationFilter.PLAIN, false);
    }
        ...
        if (annotation != null) {
          ...
          if (type != null && !this.annotationFilter.matches(type)) {
            ...
            if (repeatedAnnotations != null) {
              Boolean result =
                  doWithAnnotations(requiredType, aggregateIndex, source, repeatedAnnotations);
              if (result != null) {
                return result;
              }
            }
  ...
  private class MergedAnnotationFinder<A extends Annotation>
    ...
    @Nullable private final Predicate<? super MergedAnnotation<A>> predicate;

    private final MergedAnnotationSelector<A> selector;

    @Nullable private MergedAnnotation<A> result;
    ...
    private MergedAnnotation<A> process(
      ...
      AnnotationTypeMappings mappings =
          AnnotationTypeMappings.forAnnotationType(annotation.annotationType(), annotationFilter);
      for (int i = 0; i < mappings.size(); i++) {
        AnnotationTypeMapping mapping = mappings.get(i);
  ...
  private class AggregatesCollector implements AnnotationsProcessor<Object, List<Aggregate>> {
    ...
    private void addAggregateAnnotations(
        List<Annotation> aggregateAnnotations, Annotation[] annotations) {
      for (Annotation annotation : annotations) {
        if (annotation != null && !annotationFilter.matches(annotation)) {
          Annotation[] repeatedAnnotations =
  ...
  private static class Aggregate {
    ...
    Aggregate(int aggregateIndex, @Nullable Object source, List<Annotation> annotations) {
      ...
      for (int i = 0; i < annotations.size(); i++) {
        this.mappings[i] =
            AnnotationTypeMappings.forAnnotationType(annotations.get(i).annotationType());
      }
    }
  ...
  /**
   * {@link Spliterator} used to consume merged annotations from the aggregates in distance fist
   * order.
   */
  private class AggregatesSpliterator<A extends Annotation>
      implements Spliterator<MergedAnnotation<A>> {

    @Nullable private final Object requiredType;

    private final List<Aggregate> aggregates;

    private int aggregateCursor;

    @Nullable private int[] mappingCursors;
    ...
    private boolean tryAdvance(Aggregate aggregate, Consumer<? super MergedAnnotation<A>> action) {
      ...
      if (annotationResult != -1) {
        MergedAnnotation<A> mergedAnnotation =
            aggregate.createMergedAnnotationIfPossible(
                annotationResult,
    ...
    public long estimateSize() {
      ...
      for (int aggregateIndex = this.aggregateCursor;
          aggregateIndex < this.aggregates.size();
          aggregateIndex++) {
