...
/**
 * Collector implementations that provide various reduction operations for {@link MergedAnnotation}
 * instances.
 *
 * @author Phillip Webb
 * @since 5.2
...
public abstract class MergedAnnotationCollectors {
  ...
  /**
   * Create a new {@link Collector} that accumulates merged annotations to a {@link LinkedHashSet}
   * containing {@linkplain MergedAnnotation#synthesize() synthesized} versions.
   *
   * @param <A> the annotation type
   * @return a {@link Collector} which collects and synthesizes the annotations into a {@link Set}
   */
  ...
  /**
   * Create a new {@link Collector} that accumulates merged annotations to an {@link Annotation}
   * array containing {@linkplain MergedAnnotation#synthesize() synthesized} versions.
   *
   * @param <A> the annotation type
   * @return a {@link Collector} which collects and synthesizes the annotations into an {@code
   *     Annotation[]}
   ...
   *     length
   * @return a {@link Collector} which collects and synthesizes the annotations into an annotation
   *     array
   * @see #toAnnotationArray
   */
  public static <R extends Annotation, A extends R>
  ...
  /**
   * Create a new {@link Collector} that accumulates merged annotations to an {@link MultiValueMap}
   * with items {@linkplain MultiValueMap#add(Object, Object) added} from each merged annotation
   * {@link MergedAnnotation#asMap(Adapt...) as a map}.
   *
   * @param <A> the annotation type
   * @param adaptations adaptations that should be applied to the annotation values
   * @return a {@link Collector} which collects and synthesizes the annotations into a {@link
   *     LinkedMultiValueMap}
  ...
  private static <K, V> MultiValueMap<K, V> merge(
      MultiValueMap<K, V> map, MultiValueMap<K, V> additions) {
