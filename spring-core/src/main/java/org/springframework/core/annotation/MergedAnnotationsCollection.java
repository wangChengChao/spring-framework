...
/**
 * {@link MergedAnnotations} implementation backed by a {@link Collection} {@link MergedAnnotation}
 * instances that represent direct annotations.
 *
 * @author Phillip Webb
 * @since 5.2
...
final class MergedAnnotationsCollection implements MergedAnnotations {
  ...
  private MergedAnnotationsCollection(Collection<MergedAnnotation<?>> annotations) {
    ...
    for (int i = 0; i < this.annotations.length; i++) {
      ...
      Assert.isTrue(
          annotation.getAggregateIndex() == 0, "Annotation must have aggregate index of zero");
        ...
        MergedAnnotation<A> candidate =
            (mappingIndex == 0
                ...
                : TypeMappedAnnotation.createIfPossible(
                    mapping, root, IntrospectionFailureLogger.INFO));
  ...
  private static boolean isMappingForType(
      AnnotationTypeMapping mapping, @Nullable Object requiredType) {
  ...
  private class AnnotationsSpliterator<A extends Annotation>
    ...
    @Nullable private Object requiredType;
