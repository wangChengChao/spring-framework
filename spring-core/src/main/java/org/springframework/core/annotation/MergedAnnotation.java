...
/**
 * A single merged annotation returned from a {@link MergedAnnotations} collection. Presents a view
 * onto an annotation where attribute values may have been "merged" from different source values.
 *
 * <p>Attribute values may be accessed using the various {@code get} methods. For example, to access
 * an {@code int} attribute the {@link #getInt(String)} method would be used.
 *
 * <p>Note that attribute values are <b>not</b> converted when accessed. For example, it is not
 * possible to call {@link #getString(String)} if the underlying attribute is an {@code int}. The
 * only exception to this rule is {@code Class} and {@code Class[]} values which may be accessed as
 * {@code String} and {@code String[]} respectively to prevent potential early class initialization.
 *
 * <p>If necessary, a {@code MergedAnnotation} can be {@linkplain #synthesize() synthesized} back
 * into an actual {@link java.lang.annotation.Annotation}.
 *
 * @author Phillip Webb
...
public interface MergedAnnotation<A extends Annotation> {
  ...
  /**
   * Determine if the annotation is present on the source. Considers {@linkplain
   * #isDirectlyPresent() directly present} and {@linkplain #isMetaPresent() meta-present}
   * annotations within the context of the {@link SearchStrategy} used.
   *
  ...
  /**
   * Determine if the annotation is directly present on the source. A directly present annotation is
   * one that the user has explicitly declared and not one that is {@linkplain #isMetaPresent()
   * meta-present} or {@link Inherited @Inherited}.
   *
  ...
  /**
   * Determine if the annotation is meta-present on the source. A meta-present annotation is an
   * annotation that the user hasn't explicitly declared, but has been used as a meta-annotation
   * somewhere in the annotation hierarchy.
   *
  ...
  /**
   * Get the distance of this annotation related to its use as a meta-annotation. A directly
   * declared annotation has a distance of {@code 0}, a meta-annotation has a distance of {@code 1},
   * a meta-annotation on a meta-annotation has a distance of {@code 2}, etc. A {@linkplain
   * #missing() missing} annotation will always return a distance of {@code -1}.
   *
  ...
  /**
   * Get the index of the aggregate collection containing this annotation. Can be used to reorder a
   * stream of annotations, for example, to give a higher priority to annotations declared on a
   * superclass or interface. A {@linkplain #missing() missing} annotation will always return an
   * aggregate index of {@code -1}.
   *
   * @return the aggregate index (starting at {@code 0}) or {@code -1} if the annotation is missing
   */
  int getAggregateIndex();

  /**
   * Get the source that ultimately declared the root annotation, or {@code null} if the source is
   * not known. If this merged annotation was created {@link
   * MergedAnnotations#from(java.lang.reflect.AnnotatedElement) from} an {@link AnnotatedElement}
   * then this source will be an element of the same type. If the annotation was loaded without
   * using reflection, the source can be of any type, but should have a sensible {@code toString()}.
   * Meta-annotations will always return the same source as the {@link #getRoot() root}.
   *
  ...
  /**
   * Get the source of the meta-annotation, or {@code null} if the annotation is not {@linkplain
   * #isMetaPresent() meta-present}. The meta-source is the annotation that was meta-annotated with
   * this annotation.
   *
  ...
  /**
   * Get the root annotation, i.e. the {@link #getDistance() distance} {@code 0} annotation as
   * directly declared on the source.
   *
  ...
  /**
   * Return the complete list of annotation types from this annotation to the {@link #getRoot()
   * root}. Provides a useful way to uniquely identify a merged annotation instance.
   *
  ...
  /**
   * Determine if the specified attribute name has a non-default value when compared to the
   * annotation declaration.
   *
   * @param attributeName the attribute name
   * @return {@code true} if the attribute value is different from the default value
   */
  boolean hasNonDefaultValue(String attributeName);

  /**
   * Determine if the specified attribute name has a default value when compared to the annotation
   * declaration.
   *
   * @param attributeName the attribute name
   * @return {@code true} if the attribute value is the same as the default value
   */
   ...
   * @throws NoSuchElementException if there is no matching attribute
   */
  <T extends Annotation> MergedAnnotation<T>[] getAnnotationArray(
   ...
   *
   * @param attributeName the attribute name
   * @return an optional value or {@link Optional#empty()} if there is no matching attribute
   */
   ...
   *
   * @param attributeName the attribute name
   * @param type the attribute type. Must be compatible with the underlying attribute type or {@code
   *     Object.class}.
   * @return an optional value or {@link Optional#empty()} if there is no matching attribute
   */
   ...
   *
   * @param attributeName the attribute name
   * @return an optional of the default value or {@link Optional#empty()} if there is no matching
   *     attribute or no defined default
   */
   ...
   *
   * @return a filtered view of the annotation without any attributes that have a default value
   ...
   *
   * <p>Methods from this view will return attribute values with only alias mirroring rules applied.
   * Aliases to {@link #getMetaSource() meta-source} attributes will not be applied.
   *
  ...
  /**
   * Create a new mutable {@link AnnotationAttributes} instance from this merged annotation.
   *
  ...
  <T extends Map<String, Object>> T asMap(
      Function<MergedAnnotation<?>, T> factory, Adapt... adaptations);

  /**
   * Create a type-safe synthesized version of this annotation that can be used directly in code.
   *
   * <p>The result is synthesized using a JDK {@link Proxy} and as a result may incur a
   * computational cost when first invoked.
   *
  ...
  /**
   * Optionally create a type-safe synthesized version of this annotation based on a condition
   * predicate.
   *
   * <p>The result is synthesized using a JDK {@link Proxy} and as a result may incur a
   * computational cost when first invoked.
   ...
   * @return a optional containing the synthesized version of the annotation or an empty optional if
  ...
  /**
   * Create a {@link MergedAnnotation} that represents a missing annotation (i.e. one that is not
   * present).
   *
  ...
  /**
   * Create a new {@link MergedAnnotation} instance from the specified annotation.
   *
   ...
   * Create a new {@link MergedAnnotation} instance from the specified annotation.
   *
   * @param source the source for the annotation. This source is used only for information and
  ...
  /**
   * Create a new {@link MergedAnnotation} instance of the specified annotation type. The resulting
   * annotation will not have any attribute values but may still be used to query default values.
   *
  ...
  /**
   * Create a new {@link MergedAnnotation} instance of the specified annotation type with attributes
   * values supplied by a map.
   *
   * @param annotationType the annotation type
   * @param attributes the annotation attributes or {@code null} if just default values should be
   *     used
  ...
  /**
   * Adaptations that can be applied to attributes values when creating {@linkplain
   * MergedAnnotation#asMap(Adapt...) Maps} or {@link
   * MergedAnnotation#asAnnotationAttributes(Adapt...) AnnotationAttributes}.
   */
  enum Adapt {
