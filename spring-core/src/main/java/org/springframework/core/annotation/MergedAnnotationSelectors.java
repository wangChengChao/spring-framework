...
/**
 * {@link MergedAnnotationSelector} implementations that provide various options for {@link
...
public abstract class MergedAnnotationSelectors {
  ...
  private static final MergedAnnotationSelector<?> FIRST_DIRECTLY_DECLARED =
      new FirstDirectlyDeclared();
  ...
  /**
   * Select the first directly declared annotation when possible. If no direct annotations are
   * declared then the nearest annotation is selected.
   *
  ...
  /** {@link MergedAnnotationSelector} to select the first directly declared annotation. */
