...
interface AnnotationsProcessor<C, R> {
  ...
  /**
   * Called when an aggregate is about to be processed. This method may return a {@code non-null}
   * result to short-circuit any further processing.
   *
  ...
  /**
   * Called when an array of annotations can be processed. This method may return a {@code non-null}
   * result to short-circuit any further processing.
   *
   * @param context context information relevant to the processor
  ...
  @Nullable
  R doWithAnnotations(
      C context, int aggregateIndex, @Nullable Object source, Annotation[] annotations);
