...
/**
 * Predicate implementations that provide various test operations for {@link MergedAnnotation
 * MergedAnnotations}.
 *
 * @author Phillip Webb
 * @since 5.2
...
public abstract class MergedAnnotationPredicates {
  ...
  /**
   * Create a new {@link Predicate} that evaluates to {@code true} if the {@linkplain
   * MergedAnnotation#getType() merged annotation type} is contained in the specified array.
   *
  ...
  /**
   * Create a new {@link Predicate} that evaluates to {@code true} if the {@linkplain
   * MergedAnnotation#getType() merged annotation type} is contained in the collection.
   *
   * @param <A> the annotation type
  ...
  /**
   * Create a new stateful, single use {@link Predicate} that matches only the first run of an
   * extracted value. For example, {@code
   * MergedAnnotationPredicates.firstRunOf(MergedAnnotation::distance)} will match the first
   * annotation, and any subsequent run that have the same distance.
   *
   * <p>NOTE: This predicate only matches the first run. Once the extracted value changes, the
   * predicate always returns {@code false}. I.e. if you have a set of annotations with distances
   * {@code [1, 1, 2, 1]} then only the first two will match.
   *
  ...
  /**
   * Create a new stateful, single use {@link Predicate} that matches annotations that are unique
   * based on the extracted key. For example {@code
   * MergedAnnotationPredicates.unique(MergedAnnotation::getType)} will match the first time a
   * unique type is encountered.
   *
   * @param keyExtractor function used to extract the key used to test for uniqueness
   * @return a {@link Predicate} that matches a unique annotation based on the extracted key
   */
  ...
  /**
   * {@link Predicate} implementation used for {@link
  ...
  /**
   * {@link Predicate} implementation used for {@link MergedAnnotationPredicates#unique(Function)}.
   */
