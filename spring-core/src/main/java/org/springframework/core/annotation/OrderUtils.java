...
/**
 * General utility for determining the order of an object based on its type declaration. Handles
 * Spring's {@link Order} annotation as well as {@link javax.annotation.Priority}.
 *
...
public abstract class OrderUtils {
  ...
  private static final Map<AnnotatedElement, Object> orderCache =
      new ConcurrentReferenceHashMap<>(64);

  /**
   * Return the order on the specified {@code type}, or the specified default value if none can be
   * found.
   *
  ...
  public static Integer getOrder(Class<?> type) {
    return getOrderFromAnnotations(
        type, MergedAnnotations.from(type, SearchStrategy.TYPE_HIERARCHY));
  }
  ...
  /**
   * Return the value of the {@code javax.annotation.Priority} annotation declared on the specified
