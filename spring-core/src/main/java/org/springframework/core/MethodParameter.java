...
public class MethodParameter {
  ...
  @Nullable private volatile Parameter parameter;
  ...
  @Nullable private volatile Class<?> containingClass;

  @Nullable private volatile Class<?> parameterType;

  @Nullable private volatile Type genericParameterType;

  @Nullable private volatile Annotation[] parameterAnnotations;

  @Nullable private volatile ParameterNameDiscoverer parameterNameDiscoverer;

  @Nullable private volatile String parameterName;

  @Nullable private volatile MethodParameter nestedMethodParameter;
   ...
   *     of Lists, 1 would indicate the nested List, whereas 2 would indicate the element of the
   *     nested List)
   */
  ...
  /**
   * Internal constructor used to create a {@link MethodParameter} with a containing class already
   * set.
   *
   ...
   *
   * <p>Note: This method exposes the annotations declared on the method/constructor itself (i.e. at
   ...
   *
   * @see #getNestingLevel()
   * @deprecated since 5.2 in favor of retaining the original MethodParameter and using {@link
  ...
  /**
   * Return a variant of this {@code MethodParameter} with the type for the current level set to the
   * specified value.
   *
  ...
  /**
   * Return a variant of this {@code MethodParameter} which points to the same parameter but one
   ...
   * annotation, e.g. {@code javax.annotation.Nullable} or {@code
  ...
  /**
   * Return a variant of this {@code MethodParameter} which points to the same parameter but one
   * nesting level deeper in case of a {@link java.util.Optional} declaration.
   *
   * @since 4.3
   * @see #isOptional()
   * @see #nested()
   ...
   *
   * @param containingClass a specific containing class (potentially a subclass of the declaring
   *     class, e.g. substituting a type variable)
   * @since 5.2
   ...
   *
   * @return a specific containing class (potentially a subclass of the declaring class), or
   *     otherwise simply the declaring class itself
  ...
  public Annotation[] getParameterAnnotations() {
    ...
    if (paramAnns == null) {
      ...
      paramAnns =
          (index >= 0 && index < annotationArray.length
   ...
   *
   * <p>This method does not actually try to retrieve the parameter name at this point; it just
   * allows discovery to happen when the application calls {@link #getParameterName()} (if ever).
   */
   ...
   *     the class file or no {@link #initParameterNameDiscovery ParameterNameDiscoverer} has been
   *     set to begin with)
   */
  @Nullable
   ...
   *
   * <p>This is a convenience factory method for scenarios where a Method or Constructor reference
   * is treated in a generic fashion.
   *
   ...
   *
   * <p>This is a convenience factory method for scenarios where a Java 8 {@link Parameter}
  ...
  private static class KotlinDelegate {
      ...
      if (method != null && index == -1) {
        ...
        if (function != null) {
          ...
          KParameter parameter =
              parameters.stream().filter(predicate).collect(Collectors.toList()).get(index);
    ...
    /**
     * Return the generic return type of the method, with support of suspending functions via Kotlin
     * reflection.
     */
    ...
    /**
     * Return the return type of the method, with support of suspending functions via Kotlin
     * reflection.
     */
