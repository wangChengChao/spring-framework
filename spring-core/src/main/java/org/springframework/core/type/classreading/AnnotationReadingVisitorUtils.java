...
abstract class AnnotationReadingVisitorUtils {
      ...
      try {
        ...
        if (value instanceof AnnotationAttributes) {
          ...
          for (int i = 0; i < values.length; i++) {
            values[i] =
                convertClassValues(annotatedElement, classLoader, values[i], classValuesAsString);
          }
          value = values;
  ...
  /**
   * Retrieve the merged attributes of the annotation of the given type, if any, from the supplied
   * {@code attributesMap}.
   *
   * <p>Annotation attribute values appearing <em>lower</em> in the annotation hierarchy (i.e.,
   * closer to the declaring class) will override those defined <em>higher</em> in the annotation
   * hierarchy.
   *
   * @param attributesMap the map of annotation attribute lists, keyed by annotation type name
   * @param metaAnnotationMap the map of meta annotation relationships, keyed by annotation type
   *     name
   * @param annotationName the fully qualified class name of the annotation type to look for
   * @return the merged annotation attributes, or {@code null} if no matching annotation is present
   *     in the {@code attributesMap}
   * @since 4.0.3
   */
  @Nullable
