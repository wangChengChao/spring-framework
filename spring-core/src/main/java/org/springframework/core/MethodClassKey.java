...
/**
 * A common key class for a method against a specific target class, including {@link #toString()}
 * representation and {@link Comparable} support (as suggested for custom {@code HashMap} keys as of
 * Java 8).
 *
...
public final class MethodClassKey implements Comparable<MethodClassKey> {
   ...
   * @param targetClass the target class that the method will be invoked on (may be {@code null} if
   *     identical to the declaring class)
   */
  ...
  public int hashCode() {
    return this.method.hashCode()
        + (this.targetClass != null ? this.targetClass.hashCode() * 29 : 0);
  }
