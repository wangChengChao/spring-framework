 ...
 *
 * <p>Inspired by Netty's {@code ByteBuf}. Introduced so that non-Netty runtimes (i.e. Servlet) do
 * not require Netty on the classpath.
 *
...
public class DefaultDataBuffer implements DataBuffer {
  ...
  static DefaultDataBuffer fromFilledByteBuffer(
      DefaultDataBufferFactory dataBufferFactory, ByteBuffer byteBuffer) {
  ...
  static DefaultDataBuffer fromEmptyByteBuffer(
      DefaultDataBufferFactory dataBufferFactory, ByteBuffer byteBuffer) {
    return new DefaultDataBuffer(dataBufferFactory, byteBuffer);
  }

  /**
   * Directly exposes the native {@code ByteBuffer} that this buffer is based on also updating the
   * {@code ByteBuffer's} position and limit to match the current {@link #readPosition()} and {@link
   * #readableByteCount()}.
   *
  ...
  public byte getByte(int index) {
    ...
    assertIndex(
        index <= this.writePosition - 1, "index %d must be <= %d", index, this.writePosition - 1);
  ...
  public DefaultDataBuffer read(byte[] destination, int offset, int length) {
    Assert.notNull(destination, "Byte array must not be null");
    assertIndex(
        this.readPosition <= this.writePosition - length,
