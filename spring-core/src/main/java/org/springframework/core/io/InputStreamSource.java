...
public interface InputStreamSource {
   ...
   *
   * <p>This requirement is particularly important when you consider an API such as JavaMail, which
   * needs to be able to read the stream multiple times when creating mail attachments. For such a
   * use case, it is <i>required</i> that each {@code getInputStream()} call returns a fresh stream.
   *
