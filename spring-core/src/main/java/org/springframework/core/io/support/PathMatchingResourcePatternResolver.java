 ...
 * <p><b>No Wildcards:</b>
 *
 * <p>In the simple case, if the specified location path does not start with the {@code
 * "classpath*:}" prefix, and does not contain a PathMatcher pattern, this resolver will simply
 * return a single resource via a {@code getResource()} call on the underlying {@code
 ...
 *
 * <p>If the specified path is already a file URL (either explicitly, or implicitly because the base
 * {@code ResourceLoader} is a filesystem one, then wildcarding is guaranteed to work in a
 * completely portable fashion.
 *
 * <p>If the specified path is a classpath location, then the resolver must obtain the last
 * non-wildcard path segment URL via a {@code Classloader.getResource()} call. Since this is just a
 * node of the path (not the file at the end) it is actually undefined (in the ClassLoader Javadocs)
 * exactly what sort of a URL is returned in this case. In practice, it is usually a {@code
 * java.io.File} representing the directory, where the classpath resource resolves to a filesystem
 * location, or a jar URL of some sort, where the classpath resource resolves to a jar location.
 * Still, there is a portability concern on this operation.
 *
 * <p>If a jar URL is obtained for the last non-wildcard segment, the resolver must be able to get a
 * {@code java.net.JarURLConnection} from it, or manually parse the jar URL, to be able to walk the
 * contents of the jar, and resolve the wildcard. This will work in most environments, but will fail
 * in others, and it is strongly recommended that the wildcard resolution of resources coming from
 * jars be thoroughly tested in your specific environment before you rely on it.
 *
 * <p><b>{@code classpath*:} Prefix:</b>
 *
 * <p>There is special support for retrieving multiple class path resources with the same name, via
 * the "{@code classpath*:}" prefix. For example, "{@code classpath*:META-INF/beans.xml}" will find
 * all "beans.xml" files in the class path, be it in "classes" directories or in JAR files. This is
 * particularly useful for autodetecting config files of the same name at the same location within
 * each jar file. Internally, this happens via a {@code ClassLoader.getResources()} call, and is
 * completely portable.
 *
 * <p>The "classpath*:" prefix can also be combined with a PathMatcher pattern in the rest of the
 * location path, for example "classpath*:META-INF/*-beans.xml". In this case, the resolution
 * strategy is fairly simple: a {@code ClassLoader.getResources()} call is used on the last
 * non-wildcard path segment to get all the matching resources in the class loader hierarchy, and
 * then off each resource the same PathMatcher resolution strategy described above is used for the
 * wildcard subpath.
 *
 * <p><b>Other notes:</b>
 *
 * <p><b>WARNING:</b> Note that "{@code classpath*:}" when combined with Ant-style patterns will
 * only work reliably with at least one root directory before the pattern starts, unless the actual
 * target files reside in the file system. This means that a pattern like "{@code classpath*:*.xml}"
 * will <i>not</i> retrieve files from the root of jar files but rather only from the root of
 * expanded directories. This originates from a limitation in the JDK's {@code
 ...
 *
 * <p><b>WARNING:</b> Ant-style patterns with "classpath:" resources are not guaranteed to find
 * matching resources if the root package to search is available in multiple class path locations.
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
   ...
   * @since 4.1.1
   */
  protected void addAllClassLoaderJarRoots(
   ...
   *
   * <p>Used for determining the starting point for file matching, resolving the root directory
   * location to a {@code java.io.File} and passing it into {@code retrieveMatchingFiles}, with the
   * remainder of the location as pattern.
   *
   * <p>Will return "/WEB-INF/" for the pattern "/WEB-INF/*.xml", for example.
   *
   ...
   *
   * <p>By default, Equinox OSGi "bundleresource:" / "bundleentry:" URL will be resolved into a
   * standard jar file URL that be traversed using Spring's standard jar file traversal algorithm.
   * For any preceding custom resolution, override this method and replace the resource handle
   * accordingly.
   *
   ...
   *
   * <p>By default, the URL protocols "jar", "zip", "vfszip and "wsjar" will be treated as jar
   * resources. This template method allows for detecting further kinds of jar-like resources, e.g.
   * through {@code instanceof} checks on the resource handle type.
   *
  ...
  protected Set<File> retrieveMatchingFiles(File rootDir, String pattern) throws IOException {
    ...
    if (!rootDir.canRead()) {
      if (logger.isInfoEnabled()) {
        logger.info(
            "Skipping search for matching files underneath directory ["
