...
public class ClassPathResource extends AbstractFileResolvingResource {
  ...
  /**
   * Create a new {@code ClassPathResource} with optional {@code ClassLoader} and {@code Class}.
   * Only for internal usage.
   *
   * @param path relative or absolute path within the classpath
   * @param classLoader the class loader to load the resource with, if any
   * @param clazz the class to load resources with, if any
   * @deprecated as of 4.3.13, in favor of selective use of {@link #ClassPathResource(String,
   *     ClassLoader)} vs {@link #ClassPathResource(String, Class)}
   */
  @Deprecated
  protected ClassPathResource(
      String path, @Nullable ClassLoader classLoader, @Nullable Class<?> clazz) {
    this.path = StringUtils.cleanPath(path);
