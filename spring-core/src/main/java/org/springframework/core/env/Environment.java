...
public interface Environment extends PropertyResolver {
   ...
   * creating logical groupings of bean definitions to be registered conditionally, for example
   * based on deployment environment. Profiles can be activated by setting {@linkplain
   * AbstractEnvironment#ACTIVE_PROFILES_PROPERTY_NAME "spring.profiles.active"} as a system
   * property or by calling {@link ConfigurableEnvironment#setActiveProfiles(String...)}.
   *
  ...
  /**
   * Return the set of profiles to be active by default when no active profiles have been set
   * explicitly.
   *
  ...
  /**
   * Return whether one or more of the given profiles is active or, in the case of no explicit
   * active profiles, whether one or more of the given profiles is included in the set of default
   * profiles. If a profile begins with '!' the logic is inverted, i.e. the method will return
   * {@code true} if the given profile is <em>not</em> active. For example, {@code
   * env.acceptsProfiles("p1", "!p2")} will return {@code true} if profile 'p1' is active or 'p2' is
   * not active.
   *
   * @throws IllegalArgumentException if called with zero arguments or if any profile is {@code
  ...
  /**
   * Return whether the {@linkplain #getActiveProfiles() active profiles} match the given {@link
   * Profiles} predicate.
   */
