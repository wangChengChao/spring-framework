...
public final class GenericTypeResolver {
   ...
   * @deprecated since 5.2 in favor of {@code
   *     methodParameter.withContainingClass(implementationClass).getParameterType()}
   */
  @Deprecated
  public static Class<?> resolveParameterType(
      MethodParameter methodParameter, Class<?> implementationClass) {
  ...
  /**
   * Determine the target type for the generic return type of the given method, where formal type
   * variables are declared on the given class.
   *
  ...
  /**
   * Resolve the single type argument of the given generic interface against the given target method
   * which is assumed to return the given interface or an implementation of it.
   *
   * @param method the target method to check the return type of
   * @param genericIfc the generic interface or superclass to resolve the type argument from
   * @return the resolved parameter type of the method return type, or {@code null} if not
   *     resolvable or if the single argument is of type {@link WildcardType}.
   */
  @Nullable
    ...
    Assert.isTrue(
        ...
        () ->
            "Expected 1 type argument on generic interface ["
                + resolvableType
                + "] but found "
                + resolvableType.getGenerics().length);
   ...
   *     type arguments, or {@code null} if not resolvable
   */
  @Nullable
  ...
  /**
   * Resolve the given generic type against the given context class, substituting type variables as
   * far as possible.
   *
    ...
    if (contextClass != null) {
      if (genericType instanceof TypeVariable) {
        ResolvableType resolvedTypeVariable =
            resolveVariable((TypeVariable<?>) genericType, ResolvableType.forClass(contextClass));
  ...
  private static ResolvableType resolveVariable(
      TypeVariable<?> typeVariable, ResolvableType contextType) {
  ...
  /**
   * Build a mapping of {@link TypeVariable#getName TypeVariable names} to {@link Class concrete
   * classes} for the specified {@link Class}. Searches all super types, enclosing types and
   * interfaces.
   *
  ...
  @SuppressWarnings("rawtypes")
  private static void buildTypeVariableMap(
      ResolvableType type, Map<TypeVariable, Type> typeVariableMap) {
    if (type != ResolvableType.NONE) {
      ...
      if (resolved != null && resolved.isMemberClass()) {
        buildTypeVariableMap(
            ResolvableType.forClass(resolved.getEnclosingClass()), typeVariableMap);
      }
    }
  }
