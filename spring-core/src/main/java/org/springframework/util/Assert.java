 ...
 *
 * <p>For example, if the contract of a public method states it does not allow {@code null}
 * arguments, {@code Assert} can be used to validate that contract. Doing this clearly indicates a
...
public abstract class Assert {
   ...
   *
   * <p>Call {@link #isTrue} if you wish to throw an {@code IllegalArgumentException} on an
   * assertion failure.
   *
  ...
  public static void isInstanceOf(
      Class<?> type, @Nullable Object obj, Supplier<String> messageSupplier) {
    notNull(type, "Type to check against must not be null");
    if (!type.isInstance(obj)) {
  ...
  public static void isAssignable(
      Class<?> superType, @Nullable Class<?> subType, Supplier<String> messageSupplier) {
    notNull(superType, "Super type to check against must not be null");
    if (subType == null || !superType.isAssignableFrom(subType)) {
  ...
  private static void instanceCheckFailed(
      Class<?> type, @Nullable Object obj, @Nullable String msg) {
  ...
  private static void assignableCheckFailed(
      Class<?> superType, @Nullable Class<?> subType, @Nullable String msg) {
    String result = "";
    boolean defaultMessage = true;
    if (StringUtils.hasLength(msg)) {
      if (endsWithSeparator(msg)) {
