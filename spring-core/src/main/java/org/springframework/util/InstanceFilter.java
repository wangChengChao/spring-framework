...
/**
 * A simple instance filter that checks if a given instance match based on a collection of includes
 * and excludes element.
 *
 * <p>Subclasses may want to override {@link #match(Object, Object)} to provide a custom matching
 * algorithm.
 *
...
public class InstanceFilter<T> {
   ...
   *
   * <p>A particular element will match if it "matches" the one of the element in the includes list
   * and does not match one of the element in the excludes list.
   *
   * <p>Subclasses may redefine what matching means. By default, an element match with another if it
   * is equals according to {@link Object#equals(Object)}
   *
   * <p>If both collections are empty, {@code matchIfEmpty} defines if an element matches or not.
   *
   * @param includes the collection of includes
   * @param excludes the collection of excludes
   * @param matchIfEmpty the matching result if both the includes and the excludes collections are
   *     empty
   */
  public InstanceFilter(
      @Nullable Collection<? extends T> includes,
