...
/**
 * Implementation of {@link BackOff} that increases the back off period for each retry attempt. When
 * the interval has reached the {@link #setMaxInterval(long) max interval}, it is no longer
 * increased. Stops retrying once the {@link #setMaxElapsedTime(long) max elapsed time} has been
 * reached.
 *
 * <p>Example: The default interval is {@value #DEFAULT_INITIAL_INTERVAL} ms, the default multiplier
 * is {@value #DEFAULT_MULTIPLIER}, and the default max interval is {@value #DEFAULT_MAX_INTERVAL}.
 ...
 * </pre>
 *
 * <p>Note that the default max elapsed time is {@link Long#MAX_VALUE}. Use {@link
 * #setMaxElapsedTime(long)} to limit the maximum length of time that an instance should accumulate
 * before returning {@link BackOffExecution#STOP}.
 *
...
public class ExponentialBackOff implements BackOff {
  ...
  /**
   * The maximum elapsed time in milliseconds after which a call to {@link
   * BackOffExecution#nextBackOff()} returns {@link BackOffExecution#STOP}.
   */
  ...
  /**
   * Return the maximum elapsed time in milliseconds after which a call to {@link
   * BackOffExecution#nextBackOff()} returns {@link BackOffExecution#STOP}.
   */
  ...
  private class ExponentialBackOffExecution implements BackOffExecution {
    ...
    public String toString() {
      ...
      sb.append("currentInterval=")
          .append(this.currentInterval < 0 ? "n/a" : this.currentInterval + "ms");
