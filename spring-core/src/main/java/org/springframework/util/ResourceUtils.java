...
/**
 * Utility methods for resolving resource locations to files in the file system. Mainly for internal
 * use within the framework.
 *
 * <p>Consider using Spring's Resource abstraction in the core package for handling all kinds of
 * file resources in a uniform manner. {@link org.springframework.core.io.ResourceLoader}'s {@code
 * getResource()} method can resolve any location to a {@link org.springframework.core.io.Resource}
 * object, which in turn allows one to obtain a {@code java.io.File} in the file system through its
 * {@code getFile()} method.
 *
...
public abstract class ResourceUtils {
   ...
   * system.
   *
   * <p>Does not check whether the file actually exists; simply returns the File that the given
   * location would correspond to.
   *
   * @param resourceLocation the resource location to resolve: either a "classpath:" pseudo URL, a
   *     "file:" URL, or a plain file path
  ...
  /**
   * Determine whether the given URL points to a resource in a jar file. i.e. has protocol "jar",
   * "war, ""zip", "vfszip" or "wsjar".
   *
   * @param url the URL to check
  ...
  /**
   * Determine whether the given URL points to a jar file itself, that is, has protocol "file" and
   * ends with the ".jar" extension.
   *
   * @param url the URL to check
