...
/**
 * Abstract class that adapts a {@link ListenableFuture} parameterized over S into a {@code
 * ListenableFuture} parameterized over T. All methods are delegated to the adaptee, where {@link
 * #get()}, {@link #get(long, java.util.concurrent.TimeUnit)}, and {@link
 * ListenableFutureCallback#onSuccess(Object)} call {@link #adapt(Object)} on the adaptee's result.
 *
...
public abstract class ListenableFutureAdapter<T, S> extends FutureAdapter<T, S>
  ...
  public void addCallback(
      final SuccessCallback<? super T> successCallback, final FailureCallback failureCallback) {
    ListenableFuture<S> listenableAdaptee = (ListenableFuture<S>) getAdaptee();
    listenableAdaptee.addCallback(
        new ListenableFutureCallback<S>() {
          @Override
