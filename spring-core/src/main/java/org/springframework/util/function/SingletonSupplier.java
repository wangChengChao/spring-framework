...
/**
 * A {@link java.util.function.Supplier} decorator that caches a singleton result and makes it
 * available from {@link #get()} (nullable) and {@link #obtain()} (null-safe).
 *
 * <p>A {@code SingletonSupplier} can be constructed via {@code of} factory methods or via
 * constructors that provide a default supplier as a fallback. This is particularly useful for
 * method reference suppliers, falling back to a default supplier for a method that returned {@code
 * null} and caching the result.
 *
...
public class SingletonSupplier<T> implements Supplier<T> {
  ...
  /**
   * Build a {@code SingletonSupplier} with the given singleton instance and a default supplier for
   * the case when the instance is {@code null}.
   *
  ...
  /**
   * Build a {@code SingletonSupplier} with the given instance supplier and a default supplier for
   * the case when the instance is {@code null}.
   *
   * @param instanceSupplier the immediate instance supplier
   * @param defaultSupplier the default supplier as a fallback
   */
  public SingletonSupplier(
      @Nullable Supplier<? extends T> instanceSupplier, Supplier<? extends T> defaultSupplier) {
