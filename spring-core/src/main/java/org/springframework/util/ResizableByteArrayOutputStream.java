 ...
 *       {@link org.springframework.util.ResizableByteArrayOutputStream#resize(int)} methods to get
 *       more control over the size of the internal buffer
 *   <li>has a higher initial capacity (256) by default
 * </ul>
 *
 * <p>As of 4.2, this class has been superseded by {@link FastByteArrayOutputStream} for Spring's
 * internal use where no assignability to {@link ByteArrayOutputStream} is needed (since {@link
 * FastByteArrayOutputStream} is more efficient with buffer resize management but doesn't extend the
...
public class ResizableByteArrayOutputStream extends ByteArrayOutputStream {
  ...
  /**
   * Create a new <code>ResizableByteArrayOutputStream</code> with the default initial capacity of
   * 256 bytes.
   */
  ...
  public synchronized void resize(int targetCapacity) {
    Assert.isTrue(
        targetCapacity >= this.count, "New capacity must not be smaller than current size");
