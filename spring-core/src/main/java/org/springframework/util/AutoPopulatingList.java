...
/**
 * Simple {@link List} wrapper class that allows for elements to be automatically populated as they
 * are requested. This is particularly useful for data binding to {@link List Lists}, allowing for
 * elements to be created and added to the {@link List} in a "just in time" fashion.
 *
 * <p>Note: This class is not thread-safe. To create a thread-safe version, use the {@link
...
public class AutoPopulatingList<E> implements List<E>, Serializable {
  ...
  /**
   * Creates a new {@code AutoPopulatingList} that is backed by a standard {@link ArrayList} and
   * adds new instances of the supplied {@link Class element Class} to the backing {@link List} on
   * demand.
   */
  ...
  /**
   * Creates a new {@code AutoPopulatingList} that is backed by the supplied {@link List} and adds
   * new instances of the supplied {@link Class element Class} to the backing {@link List} on
   * demand.
   */
  ...
  /**
   * Creates a new {@code AutoPopulatingList} that is backed by a standard {@link ArrayList} and
   * creates new elements on demand using the supplied {@link ElementFactory}.
   */
  ...
  /**
   * Creates a new {@code AutoPopulatingList} that is backed by the supplied {@link List} and
   * creates new elements on demand using the supplied {@link ElementFactory}.
   */
  ...
  public interface ElementFactory<E> {
     ...
     * @throws ElementInstantiationException if the instantiation process failed (any exception
     *     thrown by a target constructor should be propagated as-is)
     */
  ...
  /**
   * Reflective implementation of the ElementFactory interface, using {@code
   * Class.getDeclaredConstructor().newInstance()} on a given element class.
   */
