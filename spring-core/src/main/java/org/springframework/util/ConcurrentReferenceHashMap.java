...
/**
 * A {@link ConcurrentHashMap} that uses {@link ReferenceType#SOFT soft} or {@linkplain
 * ReferenceType#WEAK weak} references for both {@code keys} and {@code values}.
 *
 * <p>This class can be used as an alternative to {@code Collections.synchronizedMap(new
 * WeakHashMap<K, Reference<V>>())} in order to support better performance when accessed
 * concurrently. This implementation follows the same design constraints as {@link
 * ConcurrentHashMap} with the exception that {@code null} values and {@code null} keys are
 * supported.
 *
 * <p><b>NOTE:</b> The use of references means that there is no guarantee that items placed into the
 * map will be subsequently available. The garbage collector may discard references at any time, so
 * it may appear that an unknown thread is silently removing entries.
 *
 * <p>If not explicitly specified, this implementation will use {@linkplain SoftReference soft entry
 * references}.
 *
 * @author Phillip Webb
...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V>
  ...
  @Nullable private volatile Set<Map.Entry<K, V>> entrySet;
   ...
   *
   * @param initialCapacity the initial capacity of the map
   * @param loadFactor the load factor. When the average number of references per table exceeds this
   *     value resize will be attempted
   */
  ...
  /**
   * Factory method that returns the {@link ReferenceManager}. This method will be called once for
   * each {@link Segment}.
   *
  ...
  /**
   * Get the hash for a given object, apply an additional hash function to reduce collisions. This
   * implementation uses the same Wang/Jenkins algorithm as {@link ConcurrentHashMap}. Subclasses
   * can override to provide alternative hashing.
   *
  ...
  /**
   * Return a {@link Reference} to the {@link Entry} for the specified {@code key}, or {@code null}
   * if not found.
   *
  ...
  private V put(@Nullable final K key, @Nullable final V value, final boolean overwriteExisting) {
    return doTask(
        key,
        new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.RESIZE) {
          @Override
          @Nullable
          protected V execute(
              @Nullable Reference<K, V> ref,
              @Nullable Entry<K, V> entry,
              @Nullable Entries<V> entries) {
            if (entry != null) {
  ...
  public V remove(Object key) {
    return doTask(
        key,
        new Task<V>(TaskOption.RESTRUCTURE_AFTER, TaskOption.SKIP_IF_EMPTY) {
          @Override
          @Nullable
  ...
  public boolean remove(Object key, final Object value) {
    Boolean result =
        doTask(
            key,
            new Task<Boolean>(TaskOption.RESTRUCTURE_AFTER, TaskOption.SKIP_IF_EMPTY) {
              @Override
              protected Boolean execute(
                  @Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry) {
  ...
  public boolean replace(K key, final V oldValue, final V newValue) {
    Boolean result =
        doTask(
            key,
            new Task<Boolean>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.SKIP_IF_EMPTY) {
              @Override
              protected Boolean execute(
                  @Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry) {
  ...
  public V replace(K key, final V value) {
    return doTask(
        key,
        new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.SKIP_IF_EMPTY) {
          @Override
          @Nullable
          protected V execute(@Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry) {
            if (entry != null) {
  ...
  /**
   * Remove any entries that have been garbage collected and are no longer referenced. Under normal
   * circumstances garbage collected entries are automatically purged as items are added or removed
   * from the Map. This method can be used to force a purge, and is useful when the Map is read
  ...
  /**
   * Calculate a shift value that can be used to create a power-of-two value between the specified
  ...
  protected final class Segment extends ReentrantLock {
    ...
    /**
     * Array of references indexed using the low order bits from the hash. This property should only
     * be set along with {@code resizeThreshold}.
     */
    private volatile Reference<K, V>[] references;

    /**
     * The total number of references contained in this segment. This includes chained references
     * and references that have been garbage collected but not purged.
     */
    private final AtomicInteger count = new AtomicInteger(0);

    /**
     * The threshold when resizing of the references should occur. When {@code count} exceeds this
     * value references will be resized.
     */
    ...
    /**
     * Restructure the underlying data structure when it becomes necessary. This method can increase
     * the size of the references table as well as purge any references that have been garbage
     * collected.
     *
  ...
  /**
   * A reference to an {@link Entry} contained in the map. Implementations are usually wrappers
   * around specific Java reference implementations (e.g., {@link SoftReference}).
   *
   * @param <K> the key type
   * @param <V> the value type
   */
  protected interface Reference<K, V> {
    ...
    /**
     * Release this entry and ensure that it will be returned from {@code
  ...
  private abstract class Task<T> {
    ...
    @Nullable
    protected T execute(
        @Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry, @Nullable Entries<V> entries) {
  ...
  private class EntryIterator implements Iterator<Map.Entry<K, V>> {
    ...
    @Nullable private Reference<K, V>[] references;

    @Nullable private Reference<K, V> reference;

    @Nullable private Entry<K, V> next;

    @Nullable private Entry<K, V> last;
  ...
  protected class ReferenceManager {
    ...
    /**
     * Return any reference that has been garbage collected and can be purged from the underlying
     * structure or {@code null} if no references need purging. This method must be thread safe and
     * ideally should not block when returning {@code null}. References should be returned once and
     * only once.
     *
  ...
  private static final class SoftEntryReference<K, V> extends SoftReference<Entry<K, V>>
      implements Reference<K, V> {

    private final int hash;

    @Nullable private final Reference<K, V> nextReference;
  ...
  private static final class WeakEntryReference<K, V> extends WeakReference<Entry<K, V>>
      implements Reference<K, V> {
