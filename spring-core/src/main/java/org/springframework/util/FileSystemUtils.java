...
public abstract class FileSystemUtils {
   ...
   * or files as well.
   *
   * <p>Note: Like {@link File#delete()}, this method does not throw any exception but rather
   * silently returns {@code false} in case of I/O errors. Consider using {@link
   * #deleteRecursively(Path)} for NIO-style handling of I/O errors, clearly differentiating between
   * non-existence and failure to delete an existing file.
   *
   ...
   *
   * @param root the root {@code File} to delete
   * @return {@code true} if the {@code File} existed and was deleted, or {@code false} it it did
   *     not exist
      ...
      Files.walkFileTree(
          ...
          new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
                throws IOException {
              Files.copy(
                  file, dest.resolve(src.relativize(file)), StandardCopyOption.REPLACE_EXISTING);
              return FileVisitResult.CONTINUE;
            }
          });
