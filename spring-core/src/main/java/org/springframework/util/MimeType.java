...
/**
 * Represents a MIME Type, as originally defined in RFC 2046 and subsequently used in other Internet
 * protocols including HTTP.
 *
 * <p>This class, however, does not contain support for the q-parameters used in HTTP content
 * negotiation. Those can be found in the subclass {@code org.springframework.http.MediaType} in the
 * {@code spring-web} module.
 *
 * <p>Consists of a {@linkplain #getType() type} and a {@linkplain #getSubtype() subtype}. Also has
 * functionality to parse MIME Type values from a {@code String} using {@link #valueOf(String)}. For
 * more parsing options see {@link MimeTypeUtils}.
 *
...
public class MimeType implements Comparable<MimeType>, Serializable {
   ...
   *
   * <p>The {@linkplain #getSubtype() subtype} is set to <code>"&#42;"</code>, and the parameters
   * are empty.
   *
  ...
  /**
   * Copy-constructor that copies the type, subtype, parameters of the given {@code MimeType}, and
   * allows to set the specified character set.
   *
  ...
  /**
   * Copy-constructor that copies the type and subtype of the given {@code MimeType}, and allows for
   * different parameter.
   *
   * @param other the other MimeType
    ...
    for (int i = 0; i < token.length(); i++) {
      ...
      if (!TOKEN.get(ch)) {
        throw new IllegalArgumentException(
            "Invalid token character '" + ch + "' in token \"" + token + "\"");
      }
    }
  }
  ...
  /**
   * Indicates whether the {@linkplain #getType() type} is the wildcard character <code>&#42;</code>
   * or not.
   */
   ...
   * </code> or the wildcard character followed by a suffix (e.g. <code>&#42;+xml</code>).
   *
  ...
  /**
   * Indicates whether this MIME Type is concrete, i.e. whether neither the type nor the subtype is
   ...
   *
   * <p>For instance, {@code text/*} includes {@code text/plain} and {@code text/html}, and {@code
   * application/*+xml} includes {@code application/soap+xml}, etc. This method is <b>not</b>
   * symmetric.
   *
   * @param other the reference MIME Type with which to compare
   * @return {@code true} if this MIME Type includes the given MIME Type; {@code false} otherwise
   */
   ...
   * vice versa. In effect, this method is similar to {@link #includes}, except that it <b>is</b>
   * symmetric.
   *
   * @param other the reference MIME Type with which to compare
   * @return {@code true} if this MIME Type is compatible with the given MIME Type; {@code false}
   *     otherwise
   */
  ...
  /**
   * Similar to {@link #equals(Object)} but based on the type and subtype only, i.e. ignoring
   * parameters.
   *
  ...
  /**
   * Unlike {@link Collection#contains(Object)} which relies on {@link MimeType#equals(Object)},
   * this method only checks the type and the subtype, but otherwise ignores parameters.
   *
  ...
  /**
   * Determine if the parameters in this {@code MimeType} and the supplied {@code MimeType} are
   * equal, performing case-insensitive comparisons for {@link Charset Charsets}.
   *
   * @since 4.2
   */
  ...
  /**
   * Parse the given String value into a {@code MimeType} object, with this method name following
   * the 'valueOf' naming convention (as supported by {@link
  ...
  private static Map<String, String> addCharsetParameter(
      Charset charset, Map<String, String> parameters) {
  ...
  public static class SpecificityComparator<T extends MimeType> implements Comparator<T> {
    ...
    public int compare(T mimeType1, T mimeType2) {
      if (mimeType1.isWildcardType() && !mimeType2.isWildcardType()) { // */* < audio/*
        ...
        if (mimeType1.isWildcardSubtype()
            && !mimeType2.isWildcardSubtype()) { // audio/* < audio/basic
          return 1;
        } else if (mimeType2.isWildcardSubtype()
            && !mimeType1.isWildcardSubtype()) { // audio/basic > audio/*
          return -1;
        } else if (!mimeType1
            .getSubtype()
            .equals(mimeType2.getSubtype())) { // audio/basic == audio/wave
          return 0;
