...
/**
 * A decorator for a comparator, with an "ascending" flag denoting whether comparison results should
 * be treated in forward (standard ascending) order or flipped for reverse (descending) order.
 *
 * @author Keith Donald
...
public class InvertibleComparator<T> implements Comparator<T>, Serializable {
  ...
  /**
   * Create an InvertibleComparator that sorts ascending by default. For the actual comparison, the
   * specified Comparator will be used.
   *
  ...
  /**
   * Create an InvertibleComparator that sorts based on the provided order. For the actual
