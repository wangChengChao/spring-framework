...
/**
 * A speedy alternative to {@link java.io.ByteArrayOutputStream}. Note that this variant does
 * <i>not</i> extend {@code ByteArrayOutputStream}, unlike its sibling {@link
 * ResizableByteArrayOutputStream}.
 *
 * <p>Unlike {@link java.io.ByteArrayOutputStream}, this implementation is backed by a {@link
 * java.util.LinkedList} of {@code byte[]} instead of 1 constantly resizing {@code byte[]}. It does
 * not copy buffers when it gets expanded.
 *
 * <p>The initial buffer is only created when the stream is first written. There is also no copying
 * of the internal buffer if its contents is extracted with the {@link #writeTo(OutputStream)}
 * method.
 *
...
public class FastByteArrayOutputStream extends OutputStream {
  ...
  /**
   * Create a new <code>FastByteArrayOutputStream</code> with the default initial capacity of 256
   * bytes.
   */
  ...
  /**
   * Convert the buffer's contents into a string decoding bytes using the platform's default
   ...
   * default replacement string for the platform's default character set. The {@linkplain
   ...
   *
   * <p>Also replaces the internal structures with the byte array to conserve memory: if the byte
   * array is being made anyways, mind as well as use it. This approach also means that if this
   * method is called twice without any writes in between, the second call is a no-op.
   *
   * <p>This method is "unsafe" as it returns the internal buffer. Callers should not modify the
   * returned buffer.
   *
   ...
   *
   * <p>Its size is the current size of this output stream and the valid contents of the buffer have
   * been copied into it.
   *
   * @return the current contents of this output stream, as a byte array.
   * @see #size()
   ...
   *
   * <p>All currently accumulated output in the output stream is discarded. The output stream can be
   * used again.
   */
  public void reset() {
    this.buffers.clear();
   ...
   *
   * <p>Note that if any methods are called on the OutputStream (including, but not limited to, any
   * of the write methods, {@link #reset()}, {@link #toByteArray()}, and {@link
   * #toByteArrayUnsafe()}) then the {@link java.io.InputStream}'s behavior is undefined.
   *
   ...
   * @throws IllegalArgumentException if the given capacity is smaller than the actual size of the
   *     content stored in the buffer already
  ...
  /**
   * An implementation of {@link java.io.InputStream} that reads from a given <code>
   * FastByteArrayOutputStream</code>.
   */
  private static final class FastByteArrayInputStream extends UpdateMessageDigestInputStream {
    ...
    /**
     * Create a new <code>FastByteArrayOutputStreamInputStream</code> backed by the given <code>
     * FastByteArrayOutputStream</code>.
     */
    ...
    public int read(byte[] b, int off, int len) {
      if (off < 0 || len < 0 || len > b.length - off) {
        ...
        if (this.currentBuffer == null) {
          ...
          if (this.nextIndexInCurrentBuffer < this.currentBufferLength) {
            int bytesToCopy =
                Math.min(len, this.currentBufferLength - this.nextIndexInCurrentBuffer);
            System.arraycopy(
                this.currentBuffer, this.nextIndexInCurrentBuffer, b, off, bytesToCopy);
    ...
    /**
     * Update the message digest with the next len bytes in this stream. Avoids creating new byte
     * arrays and use internal buffers for performance.
     *
     * @param messageDigest the message digest to update
