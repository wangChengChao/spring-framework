 ...
 *
 * <p>In particular, methods for using StAX ({@code javax.xml.stream}) in combination with the TrAX
 * API ({@code javax.xml.transform}), and converting StAX readers/writers into SAX readers/handlers
 * and vice-versa.
 *
...
public abstract class StaxUtils {
  ...
  /**
   * Create an {@link XMLInputFactory} with Spring's defensive setup, i.e. no support for the
   * resolution of DTDs and external entities.
   *
   ...
   * @since 5.0.12
   */
  public static <T extends XMLInputFactory> T createDefensiveInputFactory(
   ...
   *
   * @return {@code true} if {@code source} is a JAXP 1.4 {@link StAXSource} or custom StAX Source;
   ...
   * @throws IllegalArgumentException if {@code source} isn't a JAXP 1.4 {@link StAXSource} or
   *     custom StAX Source
   */
  @Nullable
  public static XMLStreamReader getXMLStreamReader(Source source) {
    if (source instanceof StAXSource) {
      ...
      throw new IllegalArgumentException(
          "Source '" + source + "' is neither StaxSource nor StAXSource");
    }
  }
   ...
   *
   * @return {@code true} if {@code result} is a JAXP 1.4 {@link StAXResult} or custom StAX Result;
   *     {@code false} otherwise
   */
   ...
   * @return the {@link XMLStreamReader}
   * @throws IllegalArgumentException if {@code source} isn't a JAXP 1.4 {@link StAXResult} or
   *     custom StAX Result
   */
  @Nullable
  public static XMLStreamWriter getXMLStreamWriter(Result result) {
    if (result instanceof StAXResult) {
      ...
      throw new IllegalArgumentException(
          "Result '" + result + "' is neither StaxResult nor StAXResult");
    }
  }
  ...
  /**
   * Create a SAX {@link ContentHandler} that writes events to the given StAX {@link
   * XMLEventWriter}.
   *
   * @param eventWriter the StAX event writer
  ...
  /**
   * Return a {@link XMLStreamReader} that reads from a {@link XMLEventReader}. Useful because the
   * StAX {@code XMLInputFactory} allows one to create an event reader from a stream reader, but not
   * vice-versa.
   *
   ...
   * @since 3.0.5
   */
  public static XMLStreamWriter createEventStreamWriter(
