...
/**
 * Simple customizable helper class for creating new {@link Thread} instances. Provides various bean
 * properties: thread name prefix, thread priority, etc.
 *
 * <p>Serves as base class for thread factories such as {@link
...
public class CustomizableThreadCreator implements Serializable {
  ...
  @Nullable private ThreadGroup threadGroup;
  ...
  public CustomizableThreadCreator(@Nullable String threadNamePrefix) {
    this.threadNamePrefix =
        (threadNamePrefix != null ? threadNamePrefix : getDefaultThreadNamePrefix());
  }
   ...
   *
   * <p>Default is "false": Concrete factories usually support explicit cancelling. Hence, if the
   * application shuts down, Runnables will by default finish their execution.
   *
   * <p>Specify "true" for eager shutdown of threads which still actively execute a {@link Runnable}
  ...
  /**
   * Return the thread group that threads should be created in (or {@code null} for the default
   * group).
   */
  @Nullable
   ...
   *
   * <p>The default implementation creates a new Thread for the given {@link Runnable}, applying an
   ...
   *
   * <p>The default implementation returns the specified thread name prefix with an increasing
   * thread count appended: e.g. "SimpleAsyncTaskExecutor-0".
   *
