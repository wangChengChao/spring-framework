...
/**
 * {@link LinkedHashMap} variant that stores String keys in a case-insensitive manner, for example
 * for key-based access in a results table.
 *
 * <p>Preserves the original order as well as the original casing of keys, while allowing for
 * contains, get and remove calls with any case of key.
 *
...
public class LinkedCaseInsensitiveMap<V> implements Map<String, V>, Serializable, Cloneable {
  ...
  @Nullable private transient volatile Set<String> keySet;

  @Nullable private transient volatile Collection<V> values;

  @Nullable private transient volatile Set<Entry<String, V>> entrySet;

  /**
   * Create a new LinkedCaseInsensitiveMap that stores case-insensitive keys according to the
  ...
  /**
   * Create a new LinkedCaseInsensitiveMap that stores case-insensitive keys according to the given
   * Locale (by default in lower case).
   *
  ...
  /**
   * Create a new LinkedCaseInsensitiveMap that wraps a {@link LinkedHashMap} with the given initial
   * capacity and stores case-insensitive keys according to the default Locale (by default in lower
   * case).
   *
  ...
  /**
   * Create a new LinkedCaseInsensitiveMap that wraps a {@link LinkedHashMap} with the given initial
   * capacity and stores case-insensitive keys according to the given Locale (by default in lower
   * case).
   *
   * @param initialCapacity the initial capacity
   * @param locale the Locale to use for case-insensitive key conversion
  ...
  public LinkedCaseInsensitiveMap(int initialCapacity, @Nullable Locale locale) {
    this.targetMap =
        new LinkedHashMap<String, V>(initialCapacity) {
          @Override
  ...
  /**
   * Return the locale used by this {@code LinkedCaseInsensitiveMap}. Used for case-insensitive key
   * conversion.
   *
   * @since 4.3.10
  ...
  private abstract class EntryIterator<T> implements Iterator<T> {
    ...
    @Nullable private Entry<String, V> last;
