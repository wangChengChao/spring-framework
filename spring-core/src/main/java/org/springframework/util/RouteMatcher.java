 ...
 *
 * <p>Equivalent to {@link PathMatcher}, but enables use of parsed representations of routes and
 * patterns for efficiency reasons in scenarios where routes from incoming messages are continuously
 * matched against a large number of message handler patterns.
 *
...
public interface RouteMatcher {
  ...
  /**
   * Whether the given {@code route} contains pattern syntax which requires the {@link
   * #match(String, Route)} method, or if it is a regular String that could be compared directly to
   * others.
   *
  ...
  /**
   * Given a route, return a {@link Comparator} suitable for sorting patterns in order of
   * explicitness for that route, so that more specific patterns come before more generic ones.
   *
