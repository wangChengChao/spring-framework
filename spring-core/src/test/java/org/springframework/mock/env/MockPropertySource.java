...
/**
 * Simple {@link PropertySource} implementation for use in testing. Accepts a user-provided {@link
 * Properties} object, or if omitted during construction, the implementation will initialize its
 * own.
 *
 * <p>The {@link #setProperty} and {@link #withProperty} methods are exposed for convenience, for
 * example:
 *
 * <pre>{@code
...
public class MockPropertySource extends PropertiesPropertySource {
  ...
  /**
   * {@value} is the default name for {@link MockPropertySource} instances not otherwise given an
   * explicit name.
   *
  ...
  /**
   * Create a new {@code MockPropertySource} with the given name that will maintain its own internal
  ...
  /**
   * Create a new {@code MockPropertySource} with the given name and backed by the given {@link
   * Properties} object
   *
   * @param name the {@linkplain #getName() name} of the property source
   * @param properties the properties to use
