...
public enum TestGroup {
  ...
  /**
   * Tests that take a considerable amount of time to run. Any test lasting longer than 500ms should
   * be considered a candidate in order to avoid making the overall test suite too slow to run
   * during the normal development cycle.
   */
  LONG_RUNNING,

  /**
   * Performance-related tests that may fail unpredictably based on CPU profile and load. Any test
   * using {@link Thread#sleep}, {@link Object#wait}, Spring's {@code StopWatch}, etc. should be
   * considered a candidate as their successful execution is likely to be based on events occurring
  ...
  /**
   * Load test groups dynamically instead of during static initialization in order to avoid a {@link
   * NoClassDefFoundError} being thrown while attempting to load collaborator classes.
   */
   ...
   * @throws IllegalArgumentException if any specified group name is not a valid {@link TestGroup}
   */
  ...
  private static Set<TestGroup> parseGroups(String originalValue, String value)
