...
/**
 * Provides utility methods that allow JUnit tests to assume certain conditions hold {@code true}.
 * If the assumption fails, it means the test should be aborted.
 *
 * <p>Tests can be categorized into {@link TestGroup}s. Active groups are enabled using the
 * 'testGroups' system property, usually activated from the gradle command line:
 *
 * <pre class="code">
 * gradle test -PtestGroups="performance"
 * </pre>
 *
 * <p>Groups can be activated as a comma separated list of values, or using the pseudo group 'all'.
 * See {@link TestGroup} for a list of valid groups.
 *
 * @author Rob Winch
 * @author Phillip Webb
 * @author Sam Brannen
