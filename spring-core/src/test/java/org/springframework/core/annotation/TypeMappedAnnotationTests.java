...
/**
 * Tests for {@link TypeMappedAnnotation}. See also {@link MergedAnnotationsTests} for a much more
 * extensive collection of tests.
 *
 * @author Phillip Webb
 */
class TypeMappedAnnotationTests {
  ...
  private void testExplicitMirror(Class<?> annotatedClass) {
    TypeMappedAnnotation<ExplicitMirror> annotation =
        getTypeMappedAnnotation(annotatedClass, ExplicitMirror.class);
  ...
  void adaptFromEmptyArrayToAnyComponentType() {
    ...
    MergedAnnotation<ArrayTypes> annotation =
        TypeMappedAnnotation.of(null, null, ArrayTypes.class, attributes);
  ...
  void adaptFromNestedMergedAnnotation() {
    ...
    MergedAnnotation<?> annotation =
        TypeMappedAnnotation.of(
            null, null, NestedContainer.class, Collections.singletonMap("value", nested));
    ...
    MergedAnnotation<?> annotation =
        TypeMappedAnnotation.of(
            ...
            Collections.singletonMap(
                "classArrayValue", new String[] {InputStream.class.getName()}));
  ...
  private <A extends Annotation> TypeMappedAnnotation<A> getTypeMappedAnnotation(
      Class<?> source, Class<? extends Annotation> rootAnnotationType, Class<A> annotationType) {
    ...
    return TypeMappedAnnotation.createIfPossible(
        mapping, source, rootAnnotation, 0, IntrospectionFailureLogger.INFO);
  }

  private AnnotationTypeMapping getMapping(
      Annotation annotation, Class<? extends Annotation> mappedAnnotationType) {
