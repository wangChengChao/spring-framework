...
class ResourcePropertySourceTests {
  ...
  private static final String PROPERTIES_RESOURCE_DESCRIPTION =
      "class path resource [" + PROPERTIES_PATH + "]";
  ...
  private static final String XML_PROPERTIES_RESOURCE_DESCRIPTION =
      "class path resource [" + XML_PROPERTIES_PATH + "]";
  ...
  void withLocationAndExplicitNameAndExplicitClassLoader() throws IOException {
    PropertySource<?> ps =
        new ResourcePropertySource("ps1", PROPERTIES_LOCATION, getClass().getClassLoader());
    assertThat(ps.getProperty("foo")).isEqualTo("bar");
    assertThat(ps.getName()).isEqualTo("ps1");
  }
  ...
  void withLocationAndGeneratedNameAndExplicitClassLoader() throws IOException {
    PropertySource<?> ps =
        new ResourcePropertySource(PROPERTIES_LOCATION, getClass().getClassLoader());
    assertThat(ps.getProperty("foo")).isEqualTo("bar");
    assertThat(ps.getName()).isEqualTo(PROPERTIES_RESOURCE_DESCRIPTION);
  }
  ...
  void withResourceAndExplicitName() throws IOException {
    PropertySource<?> ps =
        new ResourcePropertySource("ps1", new ClassPathResource(PROPERTIES_PATH));
    assertThat(ps.getProperty("foo")).isEqualTo("bar");
    assertThat(ps.getName()).isEqualTo("ps1");
  }
  ...
  void withResourceHavingNoDescription() throws IOException {
    PropertySource<?> ps =
        new ResourcePropertySource(new ByteArrayResource("foo=bar".getBytes(), ""));
    assertThat(ps.getProperty("foo")).isEqualTo("bar");
