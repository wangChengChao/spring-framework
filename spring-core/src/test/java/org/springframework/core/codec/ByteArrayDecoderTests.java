...
class ByteArrayDecoderTests extends AbstractDecoderTestCase<ByteArrayDecoder> {
  ...
  public void canDecode() {
    assertThat(
            this.decoder.canDecode(ResolvableType.forClass(byte[].class), MimeTypeUtils.TEXT_PLAIN))
        .isTrue();
    assertThat(
    ...
    assertThat(
            this.decoder.canDecode(
                ResolvableType.forClass(byte[].class), MimeTypeUtils.APPLICATION_JSON))
        .isTrue();
  }
  ...
  public void decodeToMono() {
    ...
    testDecodeToMonoAll(
        input, byte[].class, step -> step.consumeNextWith(expectBytes(expected)).verifyComplete());
  }
