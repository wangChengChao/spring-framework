...
class MergedAnnotationMetadataVisitorTests {
  ...
  void visitWhenHasEnumAttributesCreatesAnnotation() {
    ...
    assertThat(this.annotation.getValue("enumArrayValue"))
        .contains(new ExampleEnum[] {ExampleEnum.ONE, ExampleEnum.TWO});
  }
  ...
  void visitWhenHasAnnotationAttributesCreatesAnnotation() {
    ...
    MergedAnnotation<NestedAnnotation> value =
        this.annotation.getAnnotation("annotationValue", NestedAnnotation.class);
    assertThat(value.isPresent()).isTrue();
    assertThat(value.getString(MergedAnnotation.VALUE)).isEqualTo("a");
    MergedAnnotation<NestedAnnotation>[] arrayValue =
        this.annotation.getAnnotationArray("annotationArrayValue", NestedAnnotation.class);
  ...
  void visitWhenHasClassAttributesCreatesAnnotation() {
    ...
    assertThat(this.annotation.getStringArray("classArrayValue"))
        .containsExactly(OutputStream.class.getName());
    assertThat(this.annotation.getValue("classArrayValue"))
        .contains(new Class<?>[] {OutputStream.class});
  }

  private void loadFrom(Class<?> type) {
    ClassVisitor visitor =
        new ClassVisitor(SpringAsmInfo.ASM_VERSION) {
    ...
    try {
      new ClassReader(type.getName())
          .accept(
              visitor, ClassReader.SKIP_DEBUG | ClassReader.SKIP_CODE | ClassReader.SKIP_FRAMES);
    } catch (IOException ex) {
      throw new IllegalStateException(ex);
    }
  }

  @SimpleTypesAnnotation(
      stringValue = "string",
      byteValue = 1,
      shortValue = 2,
      intValue = 3,
  ...
  @EnumAnnotation(
      ...
      enumArrayValue = {ExampleEnum.ONE, ExampleEnum.TWO})
  ...
  @AnnotationAnnotation(
      ...
      annotationArrayValue = {@NestedAnnotation("b"), @NestedAnnotation("c")})
