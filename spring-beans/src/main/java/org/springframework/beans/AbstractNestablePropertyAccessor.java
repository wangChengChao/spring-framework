...
/**
 * A basic {@link ConfigurablePropertyAccessor} that provides the necessary infrastructure for all
 * typical use cases.
 *
 * <p>This accessor will convert collection and array values to the corresponding target collections
 * or arrays, if necessary. Custom property editors that deal with collections or arrays can either
 * be written via PropertyEditor's {@code setValue}, or against a comma-delimited String via {@code
 * setAsText}, as String arrays are converted in such a format if the array itself is not
 * assignable.
 *
...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
  ...
  @Nullable private Map<String, AbstractNestablePropertyAccessor> nestedPropertyAccessors;

  /**
   * Create a new empty accessor. Wrapped instance needs to be set afterwards. Registers default
   * editors.
   *
   ...
   *
   * @param registerDefaultEditors whether to register default editors (can be suppressed if the
   *     accessor won't need any type conversion)
   * @see #setWrappedInstance
   */
   ...
   */
  protected AbstractNestablePropertyAccessor(
      Object object, String nestedPath, AbstractNestablePropertyAccessor parent) {
  ...
  /**
   * Switch the target object, replacing the cached introspection results only if the class of the
   * new object is different to that of the replaced object.
   *
   ...
   * @param rootObject the root object at the top of the path
   */
  public void setWrappedInstance(
      Object object, @Nullable String nestedPath, @Nullable Object rootObject) {
    ...
    try {
      ...
      if (ex.getTargetException() instanceof ClassCastException) {
        throw new TypeMismatchException(
            propertyChangeEvent, ph.getPropertyType(), ex.getTargetException());
      } else {
  ...
  private Object convertIfNecessary(
    ...
    try {
      ...
      PropertyChangeEvent pce =
          new PropertyChangeEvent(
              getRootInstance(), this.nestedPath + propertyName, oldValue, newValue);
  ...
  /**
   * Return the {@link PropertyHandler} for the specified {@code propertyName}, navigating if
   * necessary. Return {@code null} if not found rather than throwing an exception.
   *
  ...
  /**
   * Return a {@link PropertyHandler} for the specified local {@code propertyName}. Only used to
   * reach a property available in the current context.
   *
  ...
  /**
   * Create a new nested property accessor instance. Can be overridden in subclasses to create a
   * PropertyAccessor subclass.
   *
   * @param object object wrapped by this PropertyAccessor
   * @param nestedPath the nested path of the object
   * @return the nested PropertyAccessor instance
   */
  protected abstract AbstractNestablePropertyAccessor newNestedPropertyAccessor(
      Object object, String nestedPath);

  /** Create a {@link NotWritablePropertyException} for the specified property. */
  protected abstract NotWritablePropertyException createNotWritablePropertyException(
      String propertyName);
  ...
  protected AbstractNestablePropertyAccessor getPropertyAccessorForPropertyPath(
      String propertyPath) {
  ...
  /**
   * Retrieve a Property accessor for the given nested property. Create a new one if not found in
   * the cache.
   *
   * <p>Note: Caching nested PropertyAccessors is necessary now, to keep registered custom editors
    ...
    if (nestedPa == null || nestedPa.getWrappedInstance() != ObjectUtils.unwrapOptional(value)) {
      ...
      nestedPa =
          newNestedPropertyAccessor(
              value, this.nestedPath + canonicalName + NESTED_PROPERTY_SEPARATOR);
    ...
    try {
      if (type.isArray()) {
        ...
        return CollectionFactory.createCollection(
            type, (elementDesc != null ? elementDesc.getType() : null), 16);
  ...
  private PropertyTokenHolder getPropertyNameTokens(String propertyName) {
    ...
    PropertyTokenHolder tokens =
        new PropertyTokenHolder(actualName != null ? actualName : propertyName);
    if (!keys.isEmpty()) {
      tokens.canonicalName +=
          PROPERTY_KEY_PREFIX
  ...
  public String toString() {
    ...
    return className
        + ": wrapping object ["
        + ObjectUtils.identityToString(this.wrappedObject)
        + ']';
  }
