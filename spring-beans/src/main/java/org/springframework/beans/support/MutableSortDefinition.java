...
/**
 * Mutable implementation of the {@link SortDefinition} interface. Supports toggling the ascending
...
public class MutableSortDefinition implements SortDefinition, Serializable {
   ...
   *
   * @param toggleAscendingOnSameProperty whether to toggle the ascending flag if the same property
   *     gets set again (that is, {@code setProperty} gets called with already set property name
   *     again).
   */
  ...
  /**
   * Set whether to toggle the ascending flag if the same property gets set again (that is, {@link
   * #setProperty} gets called with already set property name again).
   *
   * <p>This is particularly useful for parameter binding through a web request, where clicking on
   * the field header again might be supposed to trigger a resort for the same field but opposite
   * order.
   */
   ...
   * {@code setProperty} gets called with already set property name again).
   */
