...
/**
 * PagedListHolder is a simple state holder for handling lists of objects, separating them into
 * pages. Page numbering starts with 0.
 *
 * <p>This is mainly targeted at usage in web UIs. Typically, an instance will be instantiated with
 * a list of beans, put into the session, and exported as model. The properties can all be set/get
 * programmatically, but the most common way will be data binding, i.e. populating the bean from
 * request parameters. The getters will mainly be used by the view.
 *
 * <p>Supports sorting the underlying list via a {@link SortDefinition} implementation, available as
 * property "sort". By default, a {@link MutableSortDefinition} instance will be used, toggling the
 * ascending value on setting the same property again.
 *
 * <p>The data binding names have to be called "pageSize" and "sort.ascending", as expected by
 * BeanWrapper. Note that the names and the nesting syntax match the respective JSTL EL expressions,
 * like "myModelAttr.pageSize" and "myModelAttr.sort.ascending".
 *
...
public class PagedListHolder<E> implements Serializable {
  ...
  @Nullable private Date refreshDate;

  @Nullable private SortDefinition sort;

  @Nullable private SortDefinition sortUsed;
  ...
  /**
   * Create a new holder instance with the given source list, starting with a default sort
   * definition (with "toggleAscendingOnProperty" activated).
   *
  ...
  /**
   * Return the element index of the first element on the current page. Element numbering starts
   * with 0.
   */
  ...
  /**
   * Return the element index of the last element on the current page. Element numbering starts with
   * 0.
   */
  ...
  /**
   * Resort the list if necessary, i.e. if the current {@code sort} instance isn't equal to the
   * backed-up {@code sortUsed} instance.
   *
  ...
  /**
   * Create a deep copy of the given sort definition, for use as state holder to compare a modified
   * sort definition against.
   *
   * <p>Default implementation creates a MutableSortDefinition instance. Can be overridden in
   * subclasses, in particular in case of custom extensions to the SortDefinition interface. Is
   * allowed to return null, which means that no sort state will be held, triggering actual sorting
   * for each {@code resort} call.
   *
   ...
   *
   * <p>The default implementation uses Spring's PropertyComparator. Can be overridden in
   * subclasses.
   *
