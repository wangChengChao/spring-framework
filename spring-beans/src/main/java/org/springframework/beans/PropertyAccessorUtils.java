...
/**
 * Utility methods for classes that perform bean property access according to the {@link
...
public abstract class PropertyAccessorUtils {
   ...
   *
   * @param propertyPath the property path to determine the property name for (can include property
   *     keys, for example for specifying a map entry)
  ...
  /**
   * Determine whether the given registered path matches the given property path, either indicating
   * the property itself or an indexed element of the property.
   *
  ...
  public static boolean matchesProperty(String registeredPath, String propertyPath) {
    ...
    return (registeredPath.indexOf(
            PropertyAccessor.PROPERTY_KEY_SUFFIX_CHAR, propertyPath.length() + 1)
        == registeredPath.length() - 1);
  }

  /**
   * Determine the canonical name for the given property path. Removes surrounding quotes from map
   * keys:<br>
  ...
  public static String canonicalPropertyName(@Nullable String propertyName) {
    ...
    while (searchIndex != -1) {
      ...
      if (keyStart != -1) {
        ...
        if (keyEnd != -1) {
          String key =
              sb.substring(keyStart + PropertyAccessor.PROPERTY_KEY_PREFIX.length(), keyEnd);
