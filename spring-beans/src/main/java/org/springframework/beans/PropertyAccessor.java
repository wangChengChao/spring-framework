...
public interface PropertyAccessor {
  ...
  /**
   * Path separator for nested properties. Follows normal Java conventions: getFoo().getBar() would
   * be "foo.bar".
   */
  ...
  /**
   * Determine the property type for the specified property, either checking the property descriptor
  ...
  /**
   * Return a type descriptor for the specified property: preferably from the read method, falling
   * back to the write method.
   *
   * @param propertyName the property to check (may be a nested path and/or an indexed/mapped
   *     property)
   * @return the property type for the particular property, or {@code null} if not determinable
   ...
   * Behavior will be identical to that of the {@link #setPropertyValues(PropertyValues)} method.
   *
   * @param map a Map to take properties from. Contains property value objects, keyed by property
   *     name
   * @throws InvalidPropertyException if there is no such property or if the property isn't writable
   * @throws PropertyBatchUpdateException if one or more PropertyAccessExceptions occurred for
   *     specific properties during the batch update. This exception bundles all individual
   *     PropertyAccessExceptions. All other properties will have been successfully updated.
   */
   ...
   *
   * <p>Note that performing a batch update differs from performing a single update, in that an
   * implementation of this class will continue to update properties if a <b>recoverable</b> error
   * (such as a type mismatch, but <b>not</b> an invalid field name or the like) is encountered,
   * throwing a {@link PropertyBatchUpdateException} containing all the individual errors. This
   * exception can be examined later to see all binding errors. Properties that were successfully
   * updated remain changed.
   *
