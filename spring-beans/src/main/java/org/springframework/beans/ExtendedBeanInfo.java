...
/**
 * Decorator for a standard {@link BeanInfo} object, e.g. as created by {@link
 * Introspector#getBeanInfo(Class)}, designed to discover and register static and/or non-void
 ...
 * }</pre>
 *
 * The standard JavaBeans {@code Introspector} will discover the {@code getFoo} read method, but
 * will bypass the {@code #setFoo(Foo)} write method, because its non-void returning signature does
 * not comply with the JavaBeans specification. {@code ExtendedBeanInfo}, on the other hand, will
 * recognize and include it. This is designed to allow APIs with "builder" or method-chaining style
 * setter signatures to be used within Spring {@code <beans>} XML. {@link #getPropertyDescriptors()}
 * returns all existing property descriptors from the wrapped {@code BeanInfo} as well any added for
 * non-void returning setters. Both standard ("non-indexed") and <a
 * href="https://docs.oracle.com/javase/tutorial/javabeans/writing/properties.html">indexed
 * properties</a> are fully supported.
 *
 * @author Chris Beams
...
class ExtendedBeanInfo implements BeanInfo {
  ...
  /**
   * Return the set of {@link PropertyDescriptor PropertyDescriptors} from the wrapped {@link
   * BeanInfo} object as well as {@code PropertyDescriptors} for each non-void returning setter
  ...
  static class SimplePropertyDescriptor extends PropertyDescriptor {
    ...
    @Nullable private Class<?> propertyType;

    @Nullable private Class<?> propertyEditorClass;
    ...
    public SimplePropertyDescriptor(
        String propertyName, @Nullable Method readMethod, Method writeMethod)
  ...
  static class SimpleIndexedPropertyDescriptor extends IndexedPropertyDescriptor {
    ...
    @Nullable private Method indexedReadMethod;

    @Nullable private Method indexedWriteMethod;

    @Nullable private Class<?> indexedPropertyType;
    ...
    public SimpleIndexedPropertyDescriptor(
        ...
        @Nullable Method indexedReadMethod,
    ...
    @Override
    public void setIndexedReadMethod(@Nullable Method indexedReadMethod)
        throws IntrospectionException {
      this.indexedReadMethod = indexedReadMethod;
    }
  ...
  /**
   * Sorts PropertyDescriptor instances alpha-numerically to emulate the behavior of {@link
