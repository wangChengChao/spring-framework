 ...
 *
 * <p>Also supports registration of {@link org.springframework.beans.factory.DisposableBean}
 * instances, (which might or might not correspond to registered singletons), to be destroyed on
 * shutdown of the registry. Dependencies between beans can be registered to enforce an appropriate
 * shutdown order.
 *
 * <p>This class mainly serves as base class for {@link
 * org.springframework.beans.factory.BeanFactory} implementations, factoring out the common
 * management of singleton bean instances. Note that the {@link
 * org.springframework.beans.factory.config.ConfigurableBeanFactory} interface extends the {@link
 * SingletonBeanRegistry} interface.
 *
 * <p>Note that this class assumes neither a bean definition concept nor a specific creation process
 * for bean instances, in contrast to {@link AbstractBeanFactory} and {@link
 * DefaultListableBeanFactory} (which inherit from it). Can alternatively also be used as a nested
...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry
   ...
   *
   * <p>To be called for eager registration of singletons, e.g. to be able to resolve circular
   * references.
   *
   * @param beanName the name of the bean
   ...
   * clean up eager registration of a singleton if creation failed.
   *
   * @param beanName the name of the bean
   ...
   *
   * <p>Disposable beans usually correspond to registered singletons, matching the bean name but
   * potentially being a different instance (for example, a DisposableBean adapter for a singleton
   * that does not naturally implement Spring's DisposableBean interface).
   *
   * @param beanName the name of the bean
   ...
   *
   * <p>Also registers the containing bean as dependent on the contained bean in terms of
   * destruction order.
   *
   ...
   *
   * <p>Subclasses should synchronize on the given Object if they perform any sort of extended
   * singleton creation phase. In particular, subclasses should <i>not</i> have their own mutexes
   * involved in singleton creation, to avoid the potential for deadlocks in lazy-init situations.
   */
  @Override
