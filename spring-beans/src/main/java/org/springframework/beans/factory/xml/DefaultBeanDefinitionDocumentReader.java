...
/**
 * Default implementation of the {@link BeanDefinitionDocumentReader} interface that reads bean
 * definitions according to the "spring-beans" DTD and XSD format (Spring's default XML bean
 * definition format).
 *
 * <p>The structure, elements, and attribute names of the required XML document are hard-coded in
 * this class. (Of course a transform could be run if necessary to produce this format). {@code
 * <beans>} does not need to be the root element of the XML document: this class will parse all bean
 * definition elements in the XML file, regardless of the actual root element.
 *
 * @author Rod Johnson
...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
   ...
   * historically).
   *
   * <p>Opens a DOM Document; then initializes the default settings specified at the {@code
   * <beans/>} level; then parses the contained bean definitions.
   */
  @Override
  ...
  protected void importBeanDefinitionResource(Element ele) {
    ...
    if (absoluteLocation) {
      ...
      try {
        ...
        getReaderContext()
            .error(
                "Failed to import bean definitions from relative location [" + location + "]",
                ele,
                ex);
      }
    }
  ...
  protected void processAliasRegistration(Element ele) {
    ...
    if (valid) {
      try {
        ...
        getReaderContext()
            .error(
                "Failed to register alias '" + alias + "' for bean with name '" + name + "'",
                ele,
                ex);
      }
  ...
  protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
    ...
    if (bdHolder != null) {
      ...
      try {
        ...
        getReaderContext()
            .error(
                "Failed to register bean definition with name '" + bdHolder.getBeanName() + "'",
                ele,
                ex);
      }
   ...
   *
   * <p>The default implementation is empty. Subclasses can override this method to convert custom
   * elements into standard Spring bean definitions, for example. Implementors have access to the
   * parser's bean definition reader and the underlying XML resource, through the corresponding
   * accessors.
   *
