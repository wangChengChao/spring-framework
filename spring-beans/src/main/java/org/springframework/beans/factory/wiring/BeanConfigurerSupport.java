...
/**
 * Convenient base class for bean configurers that can perform Dependency Injection on objects
 * (however they may be created). Typically subclassed by AspectJ aspects.
 *
 * <p>Subclasses may also need a custom metadata resolution strategy, in the {@link
 * BeanWiringInfoResolver} interface. The default implementation looks for a bean with the same name
 * as the fully-qualified class name. (This is the default name of the bean in a Spring XML file if
 * the '{@code id}' attribute is not used.)
 *
 * @author Rob Harrop
 * @author Rod Johnson
...
public class BeanConfigurerSupport implements BeanFactoryAware, InitializingBean, DisposableBean {
  ...
  @Nullable private volatile BeanWiringInfoResolver beanWiringInfoResolver;
   ...
   *
   * <p>The default behavior is to look for a bean with the same name as the class. As an
   * alternative, consider using annotation-driven bean wiring.
   *
   * @see ClassNameBeanWiringInfoResolver
  ...
  public void setBeanFactory(BeanFactory beanFactory) {
    if (!(beanFactory instanceof ConfigurableListableBeanFactory)) {
      throw new IllegalArgumentException(
          "Bean configurer aspect needs to run in a ConfigurableListableBeanFactory: "
              + beanFactory);
    }
  ...
  /**
   * Release references to the {@link BeanFactory} and {@link BeanWiringInfoResolver} when the
   ...
   *
   * <p>Subclasses can override this to provide custom configuration logic. Typically called by an
   * aspect, for all bean instances matched by a pointcut.
   *
    ...
    try {
      ...
      if (bwi.indicatesAutowiring()
        ...
        beanFactory.autowireBeanProperties(
            beanInstance, bwi.getAutowireMode(), bwi.getDependencyCheck());
      ...
      if (rootCause instanceof BeanCurrentlyInCreationException) {
        ...
        if (bceBeanName != null && beanFactory.isCurrentlyInCreation(bceBeanName)) {
          if (logger.isDebugEnabled()) {
            logger.debug(
                "Failed to create target bean '"
                    + bce.getBeanName()
                    + "' while configuring object of type ["
                    + beanInstance.getClass().getName()
                    + "] - probably due to a circular reference. This is a common startup situation "
                    + "and usually not fatal. Proceeding without injection. Original exception: "
                    + ex);
          }
          return;
        }
