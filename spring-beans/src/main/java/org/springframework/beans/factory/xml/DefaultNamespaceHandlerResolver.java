...
/**
 * Default implementation of the {@link NamespaceHandlerResolver} interface. Resolves namespace URIs
 * to implementation classes based on the mappings contained in mapping file.
 *
 * <p>By default, this implementation looks for the mapping file at {@code
 * META-INF/spring.handlers}, but this can be changed using the {@link
...
public class DefaultNamespaceHandlerResolver implements NamespaceHandlerResolver {
   ...
   * Create a new {@code DefaultNamespaceHandlerResolver} using the default mapping file location.
   *
   * @param classLoader the {@link ClassLoader} instance used to load mapping resources (may be
   *     {@code null}, in which case the thread context ClassLoader will be used)
   * @see #DEFAULT_HANDLER_MAPPINGS_LOCATION
   */
  ...
  /**
   * Locate the {@link NamespaceHandler} for the supplied namespace URI from the configured
   * mappings.
   *
      ...
      synchronized (this) {
        ...
        if (handlerMappings == null) {
          if (logger.isTraceEnabled()) {
            logger.trace(
                "Loading NamespaceHandler mappings from [" + this.handlerMappingsLocation + "]");
          }
          try {
            Properties mappings =
