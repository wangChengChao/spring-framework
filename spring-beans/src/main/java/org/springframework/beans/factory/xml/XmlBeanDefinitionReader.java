 ...
 *
 * <p>This class loads a DOM document and applies the BeanDefinitionDocumentReader to it. The
 * document reader will register each bean definition with the given bean factory, talking to the
 * latter's implementation of the {@link
 * org.springframework.beans.factory.support.BeanDefinitionRegistry} interface.
 *
...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
  ...
  @Nullable private NamespaceHandlerResolver namespaceHandlerResolver;
   ...
   *
   * <p>This method switches namespace awareness on if validation is turned off, in order to still
   * process schema namespaces properly in such a scenario.
   *
   ...
   *
   * <p>Note that this only activates or deactivates validation itself. If you are switching
   * validation off for schema files, you might need to activate schema namespace support
   * explicitly: see {@link #setNamespaceAware}.
   */
   ...
   *
   * <p>This is typically not needed when schema validation is active. However, without validation,
   * this has to be switched to "true" in order to properly process schema namespaces.
   */
   ...
   *
   * <p>The default implementation is {@link
   * org.springframework.beans.factory.parsing.FailFastProblemReporter} which exhibits fail fast
   * behaviour. External tools can provide an alternative implementation that collates errors and
   * warnings for display in the tool UI.
   */
  public void setProblemReporter(@Nullable ProblemReporter problemReporter) {
    this.problemReporter =
        (problemReporter != null ? problemReporter : new FailFastProblemReporter());
  }
   ...
   * notification. External tools can provide an alternative implementation to monitor the
   * components being registered in the BeanFactory.
   */
   ...
   *
   * <p>If none is specified, a default instance will be created through {@link
   * #createDefaultNamespaceHandlerResolver()}.
   */
  public void setNamespaceHandlerResolver(
      @Nullable NamespaceHandlerResolver namespaceHandlerResolver) {
   ...
   *
   * <p>The default implementation is {@link DefaultDocumentLoader} which loads {@link Document}
   ...
   *
   * <p>By default, {@link ResourceEntityResolver} will be used. Can be overridden for custom entity
   ...
   *
   * <p>If not set, a default SimpleSaxErrorHandler is used that simply logs warnings using the
   * logger instance of the view class, and rethrows errors to discontinue the XML transformation.
   *
   ...
   */
  public void setDocumentReaderClass(
  ...
  protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
    ...
    try {
      ...
      throw new XmlBeanDefinitionStoreException(
          ...
          "Line " + ex.getLineNumber() + " in XML document from " + resource + " is invalid",
          ex);
    } catch (SAXException ex) {
      throw new XmlBeanDefinitionStoreException(
          resource.getDescription(), "XML document from " + resource + " is invalid", ex);
      ...
      throw new BeanDefinitionStoreException(
          resource.getDescription(), "IOException parsing XML document from " + resource, ex);
    } catch (Throwable ex) {
      throw new BeanDefinitionStoreException(
          resource.getDescription(),
  ...
  /**
   * Determine the validation mode for the specified {@link Resource}. If no explicit validation
   * mode has been configured, then the validation mode gets {@link #detectValidationMode detected}
   * from the given resource.
   *
   * <p>Override this method if you would like full control over the validation mode, even when
   * something other than {@link #VALIDATION_AUTO} was set.
   *
   ...
   * loadBeanDefinitions}.
   *
   * <p>Creates a new instance of the parser class and invokes {@code registerBeanDefinitions} on
   * it.
   *
