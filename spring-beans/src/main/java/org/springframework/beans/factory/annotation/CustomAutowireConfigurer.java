...
/**
 * A {@link org.springframework.beans.factory.config.BeanFactoryPostProcessor} implementation that
 * allows for convenient registration of custom autowire qualifier types.
 *
 * <pre class="code">
...
public class CustomAutowireConfigurer
  ...
  @Nullable private Set<?> customQualifierTypes;
  ...
  /**
   * Register custom qualifier annotation types to be considered when autowiring beans. Each element
   * of the provided set may be either a Class instance or a String representation of the
   * fully-qualified class name of the custom annotation.
   *
   * <p>Note that any annotation that is itself annotated with Spring's {@link
   * org.springframework.beans.factory.annotation.Qualifier} does not require explicit registration.
   *
  ...
  public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory)
    ...
    if (this.customQualifierTypes != null) {
      ...
      if (!(dlbf.getAutowireCandidateResolver()
          instanceof QualifierAnnotationAutowireCandidateResolver)) {
      ...
      for (Object value : this.customQualifierTypes) {
        ...
        if (value instanceof Class) {
          ...
          throw new IllegalArgumentException(
              "Invalid value ["
                  + value
                  + "] for custom qualifier type: needs to be Class or String.");
        }
