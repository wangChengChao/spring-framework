 ...
 *
 * <p>Provides bean definition registration methods for Map/Properties and ResourceBundle. Typically
 * applied to a DefaultListableBeanFactory.
 *
 * <p><b>Example:</b>
 *
 * <pre class="code">
...
public class PropertiesBeanDefinitionReader extends AbstractBeanDefinitionReader {
  ...
  /**
   * Property suffix for references to other beans in the current BeanFactory: e.g. {@code
   * owner.dog(ref)=fido}. Whether this is a reference to a singleton or a prototype will depend on
   * the definition of the target bean.
   */
  ...
  @Nullable private String defaultParentBean;
  ...
  /**
   * Set the default parent bean for this bean factory. If a child bean definition handled by this
   * factory provides neither a parent nor a class attribute, this default value gets used.
   *
   * <p>Can be used e.g. for view definition files, to define a parent with a default view class and
   * common attributes for all views. View definitions that define their own parent or carry their
   * own class can still override this.
   *
   * <p>Strictly speaking, the rule that a default parent setting does not apply to a bean
  ...
  /**
   * Load bean definitions from the specified properties file, using all property keys (i.e. not
   * filtering by prefix).
   *
   ...
   * @throws BeanDefinitionStoreException in case of loading or parsing errors
   */
  public int loadBeanDefinitions(Resource resource, @Nullable String prefix)
  ...
  public int loadBeanDefinitions(EncodedResource encodedResource, @Nullable String prefix)
    ...
    try {
      ...
      int count =
          registerBeanDefinitions(props, prefix, encodedResource.getResource().getDescription());
  ...
  /**
   * Register bean definitions contained in a resource bundle, using all property keys (i.e. not
   * filtering by prefix).
   *
   ...
   *
   * <p>Similar syntax as for a Map. This method is useful to enable standard Java
   ...
   * @throws BeanDefinitionStoreException in case of loading or parsing errors
   */
  public int registerBeanDefinitions(ResourceBundle rb, @Nullable String prefix)
      throws BeanDefinitionStoreException {
  ...
  /**
   * Register bean definitions contained in a Map, using all property keys (i.e. not filtering by
   * prefix).
   *
   * @param map a map of {@code name} to {@code property} (String or Object). Property values will
   *     be strings if coming from a Properties file etc. Property names (keys) <b>must</b> be
   *     Strings. Class keys must be Strings.
   * @return the number of bean definitions found
   ...
   */
  public int registerBeanDefinitions(
      Map<?, ?> map, @Nullable String prefix, String resourceDescription) throws BeansException {
    ...
    for (Map.Entry<?, ?> entry : map.entrySet()) {
      ...
      if (key.startsWith(prefix + SEPARATOR)) {
        ...
        if (CLASS_KEY.equals(property)) {
          ...
          String val = StringUtils.trimWhitespace((String) entry.getValue());
          scope =
              ("".equals(val) || TRUE_VALUE.equals(val)
  ...
  /**
   * Reads the value of the entry. Correctly interprets bean references for values that are prefixed
   * with an asterisk.
   */
