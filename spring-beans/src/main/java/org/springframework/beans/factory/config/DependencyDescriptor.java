...
public class DependencyDescriptor extends InjectionPoint implements Serializable {
  ...
  @Nullable private Class<?> containingClass;

  @Nullable private transient volatile ResolvableType resolvableType;

  @Nullable private transient volatile TypeDescriptor typeDescriptor;
   ...
   *
   * <p>Optional semantics are derived from Java 8's {@link java.util.Optional}, any variant of a
  ...
  /**
   * Resolve the specified not-unique scenario: by default, throwing a {@link
   * NoUniqueBeanDefinitionException}.
   *
   * <p>Subclasses may override this to select one of the instances or to opt out with no result at
   * all through returning {@code null}.
   *
   * @param type the requested bean type
   * @param matchingBeans a map of bean names and corresponding bean instances which have been
   *     pre-selected for the given type (qualifiers etc already applied)
   ...
   *
   * <p>The resolution algorithm will first attempt to resolve a shortcut through this method before
   * going into the regular type matching algorithm across all beans. Subclasses may override this
   * method to improve resolution performance based on pre-cached information while still receiving
   * {@link InjectionPoint} exposure etc.
   *
   ...
   *
   * <p>The default implementation calls {@link BeanFactory#getBean(String)}. Subclasses may provide
  ...
  /**
   * Optionally set the concrete class that contains this dependency. This may differ from the class
   * that declares the parameter/field in that it may be a subclass thereof, potentially
   ...
   *
   * <p>This is {@code false} by default but may be overridden to return {@code true} in order to
   * suggest to an {@link org.springframework.beans.factory.support.AutowireCandidateResolver} that
   * a fallback match is acceptable as well.
   *
   * @since 4.0
   */
   ...
   * allows discovery to happen when the application calls {@link #getDependencyName()} (if ever).
   */
  ...
  public boolean equals(@Nullable Object other) {
    ...
    return (this.required == otherDesc.required
        && this.eager == otherDesc.eager
