...
/**
 * Holder for bean wiring metadata information about a particular class. Used in conjunction with
 * the {@link org.springframework.beans.factory.annotation.Configurable} annotation and the AspectJ
 * {@code AnnotationBeanConfigurerAspect}.
 *
 * @author Rod Johnson
...
public class BeanWiringInfo {
   ...
   * @throws IllegalArgumentException if the supplied beanName is {@code null}, is empty, or
   ...
   *
   * @param beanName the name of the bean definition to take the property values from
   * @param isDefaultBeanName whether the given bean name is a suggested default bean name, not
   *     necessarily matching an actual bean definition
   * @throws IllegalArgumentException if the supplied beanName is {@code null}, is empty, or
   *     consists wholly of whitespace
   */
   ...
   * @throws IllegalArgumentException if the supplied {@code autowireMode} is not one of the allowed
   *     values
  ...
  public BeanWiringInfo(int autowireMode, boolean dependencyCheck) {
    if (autowireMode != AUTOWIRE_BY_NAME && autowireMode != AUTOWIRE_BY_TYPE) {
      throw new IllegalArgumentException(
          "Only constants AUTOWIRE_BY_NAME and AUTOWIRE_BY_TYPE supported");
    }
  ...
  /**
   * Return whether the specific bean name is a suggested default bean name, not necessarily
   * matching an actual bean definition in the factory.
   */
  ...
  /**
   * Return one of the constants {@link #AUTOWIRE_BY_NAME} / {@link #AUTOWIRE_BY_TYPE}, if
   * autowiring is indicated.
   */
  ...
  /**
   * Return whether to perform a dependency check for object references in the bean instance (after
   * autowiring).
   */
