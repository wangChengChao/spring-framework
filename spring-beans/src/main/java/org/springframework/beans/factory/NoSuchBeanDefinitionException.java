...
/**
 * Exception thrown when a {@code BeanFactory} is asked for a bean instance for which it cannot find
 * a definition. This may point to a non-existing bean, a non-unique bean, or a manually registered
 * singleton instance without an associated bean definition.
 *
 * @author Rod Johnson
...
public class NoSuchBeanDefinitionException extends BeansException {
  ...
  @Nullable private final ResolvableType resolvableType;
  ...
  /**
   * Return the required {@link ResolvableType} of the missing bean, if it was a lookup <em>by
   * type</em> that failed.
   *
   * @since 4.3.4
   */
  ...
  /**
   * Return the number of beans found when only one matching bean was expected. For a regular
   * NoSuchBeanDefinitionException, this will always be 0.
   *
   * @see NoUniqueBeanDefinitionException
   */
