...
/**
 * Convenience methods performing bean lookups related to Spring-specific annotations, for example
...
public abstract class BeanFactoryAnnotationUtils {
  ...
  /**
   * Retrieve all bean of type {@code T} from the given {@code BeanFactory} declaring a qualifier
   * (e.g. via {@code <qualifier>} or {@code @Qualifier}) matching the given qualifier, or having a
   * bean name matching the given qualifier.
   *
  ...
  public static <T> Map<String, T> qualifiedBeansOfType(
    ...
    String[] candidateBeans =
        BeanFactoryUtils.beanNamesForTypeIncludingAncestors(beanFactory, beanType);
  ...
  /**
   * Obtain a bean of type {@code T} from the given {@code BeanFactory} declaring a qualifier (e.g.
   * via {@code <qualifier>} or {@code @Qualifier}) matching the given qualifier, or having a bean
  ...
  /**
   * Obtain a bean of type {@code T} from the given {@code BeanFactory} declaring a qualifier (e.g.
   * {@code <qualifier>} or {@code @Qualifier}) matching the given qualifier).
   *
   ...
   * @return the matching bean of type {@code T} (never {@code null})
   */
  private static <T> T qualifiedBeanOfType(
      ListableBeanFactory bf, Class<T> beanType, String qualifier) {
    ...
    if (matchingBean != null) {
      ...
      // Fallback: target bean at least found by bean name - probably a manually registered
      // singleton.
   ...
   * @return {@code true} if either the bean definition (in the XML case) or the bean's factory
   *     method (in the {@code @Bean} case) defines a matching qualifier value (through {@code
   *     <qualifier>} or {@code @Qualifier})
   * @since 5.0
   */
    ...
    if (beanFactory != null) {
      ...
      try {
        ...
        if (beanFactory instanceof ConfigurableBeanFactory) {
          BeanDefinition bd =
              ((ConfigurableBeanFactory) beanFactory).getMergedBeanDefinition(beanName);
          ...
          if (bd instanceof RootBeanDefinition) {
            ...
            if (factoryMethod != null) {
              Qualifier targetAnnotation =
                  AnnotationUtils.getAnnotation(factoryMethod, Qualifier.class);
