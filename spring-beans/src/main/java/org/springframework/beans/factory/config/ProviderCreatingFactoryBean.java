...
public class ProviderCreatingFactoryBean extends AbstractFactoryBean<Provider<Object>> {
   ...
   *
   * <p>The target does not <i>have</i> to be a non-singleton bean, but realistically always will be
   * (because if the target bean were a singleton, then said singleton bean could simply be injected
   * straight into the dependent object, thus obviating the need for the extra level of indirection
   * afforded by this factory approach).
   */
