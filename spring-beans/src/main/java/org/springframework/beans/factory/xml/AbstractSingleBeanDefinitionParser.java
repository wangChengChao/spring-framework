 ...
 *
 * <p>Extend this parser class when you want to create a single bean definition from an arbitrarily
 * complex XML element. You may wish to consider extending the {@link
 ...
 *
 * <p>The resulting {@code BeanDefinition} will be automatically registered with the {@link
 * org.springframework.beans.factory.support.BeanDefinitionRegistry}. Your job simply is to {@link
 * #doParse parse} the custom XML {@link Element} into a single {@code BeanDefinition}.
 *
 * @author Rob Harrop
...
public abstract class AbstractSingleBeanDefinitionParser extends AbstractBeanDefinitionParser {
   ...
   */
  @Override
  protected final AbstractBeanDefinition parseInternal(
   ...
   *
   * <p>The default implementation returns {@code null}, indicating a root bean definition.
   *
   ...
   *
   * <p>Note that, for application classes, it is generally preferable to override {@link
   * #getBeanClassName} instead, in order to avoid a direct dependence on the bean implementation
   * class. The BeanDefinitionParser and its NamespaceHandler can be used within an IDE plugin then,
   * even if the application classes are not available on the plugin's classpath.
   *
   * @param element the {@code Element} that is being parsed
   ...
   * required.
   *
   * <p>The default implementation delegates to the {@code doParse} version without ParserContext
   * argument.
   *
