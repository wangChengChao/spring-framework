 ...
 * </pre>
 *
 * <p>If you are using Spring 2.0 and XML Schema support in your configuration file(s), you can also
 * use the following style of configuration for property path access. (See also the appendix
 * entitled 'XML Schema-based configuration' in the Spring reference manual for more examples.)
 *
 * <pre class="code">
 *  &lt;!-- will result in 10, which is the value of property 'age' of bean 'tb' --&gt;
...
public class PropertyPathFactoryBean
  ...
  @Nullable private BeanWrapper targetBeanWrapper;

  @Nullable private String targetBeanName;

  @Nullable private String propertyPath;

  @Nullable private Class<?> resultType;
  ...
  /**
   * Specify a target object to apply the property path to. Alternatively, specify a target bean
   * name.
   *
   * @param targetObject a target object, for example a bean reference or an inner bean
   * @see #setTargetBeanName
   */
  ...
  /**
   * Specify the name of a target bean to apply the property path to. Alternatively, specify a
   * target object directly.
   *
   * @param targetBeanName the bean name to be looked up in the containing bean factory (e.g.
   *     "testBean")
   * @see #setTargetObject
   */
   ...
   *
   * <p>Note: This is not necessary for directly specified target objects or singleton target beans,
   * where the type can be determined through introspection. Just specify this in case of a
   * prototype target, provided that you need matching by type (for example, for autowiring).
   *
  ...
  /**
   * The bean name of this PropertyPathFactoryBean will be interpreted as "beanName.property"
   * pattern, if neither "targetObject" nor "targetBeanName" nor "propertyPath" have been specified.
      ...
      if (dotIndex == -1) {
        throw new IllegalArgumentException(
            "Neither 'targetObject' nor 'targetBeanName' specified, and PropertyPathFactoryBean "
                + "bean name '"
                + this.beanName
                + "' does not follow 'beanName.property' syntax");
      }
    ...
    if (target != null) {
      if (logger.isWarnEnabled()
          ...
          && ((ConfigurableBeanFactory) this.beanFactory)
              .isCurrentlyInCreation(this.targetBeanName)) {
        logger.warn(
            "Target bean '"
                + this.targetBeanName
                + "' is still in creation due to a circular "
                + "reference - obtained value for property '"
                + this.propertyPath
                + "' may be outdated!");
      }
    } else {
  ...
  /**
   * While this FactoryBean will often be used for singleton targets, the invoked getters for the
   * property path might return a new object for each call, so we have to assume that we're not
   * returning the same object for each {@link #getObject()} call.
   */
  @Override
