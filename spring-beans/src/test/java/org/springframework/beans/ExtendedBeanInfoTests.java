...
public class ExtendedBeanInfoTests {
  ...
  /** {@link ExtendedBeanInfo} should behave exactly like {@link BeanInfo} in strange edge cases. */
  @Test
  public void readMethodReturnsSupertypeOfWriteMethodParameter() throws IntrospectionException {
    ...
    assertThat(hasReadMethodForProperty(bi, "foo")).isTrue();
    assertThat(hasReadMethodForProperty(ebi, "foo")).isTrue();
    assertThat(hasWriteMethodForProperty(ebi, "foo"))
        .isEqualTo(hasWriteMethodForProperty(bi, "foo"));
  }

  @Test
  public void indexedReadMethodReturnsSupertypeOfIndexedWriteMethodParameter()
    ...
    assertThat(hasIndexedWriteMethodForProperty(ebi, "foos"))
        .isEqualTo(hasIndexedWriteMethodForProperty(bi, "foos"));
  }
  ...
  @Test
  public void indexedReadMethodReturnsSubtypeOfIndexedWriteMethodParameter()
      throws IntrospectionException {
    @SuppressWarnings("unused")
    class C {
  ...
  public void indexedReadAndNonStandardIndexedWrite() throws IntrospectionException {
    ...
    assertThat(hasIndexedReadMethodForProperty(bi, "foos")).isTrue();
    // interesting! standard Inspector picks up non-void return types on indexed write methods by
    // default
    assertThat(hasIndexedWriteMethodForProperty(bi, "foos")).isFalse();
  ...
  @Test
  public void indexedReadAndNonStandardWriteAndNonStandardIndexedWrite()
      throws IntrospectionException {
    @SuppressWarnings("unused")
    class C {
    ...
    assertThat(hasIndexedReadMethodForProperty(bi, "foos")).isTrue();
    assertThat(hasWriteMethodForProperty(bi, "foos")).isFalse();
    // again as above, standard Inspector picks up non-void return types on indexed write methods by
    // default
    assertThat(hasIndexedWriteMethodForProperty(bi, "foos")).isFalse();
   ...
   * IntrospectionException regarding a "type mismatch between indexed and non-indexed methods"
   * intermittently (approximately one out of every four times) under JDK 7 due to non-deterministic
   * results from {@link Class#getDeclaredMethods()}. See
   * https://bugs.java.com/view_bug.do?bug_id=7023180
   *
   * @see #cornerSpr9702()
   */
  @Test
  ...
  /**
   * Ensures that an empty string is not passed into a PropertyDescriptor constructor. This could
   * occur when handling ArrayList.set(int,Object)
   */
  @Test
  ...
  @Test
  public void overloadedNonStandardWriteMethodsOnly_orderA()
  ...
  /**
   * Corners the bug revealed by SPR-8522, in which an (apparently) indexed write method without a
   * corresponding indexed read method would fail to be processed correctly by ExtendedBeanInfo. The
   * local class C below represents the relevant methods from Google's GsonBuilder class.
   * Interestingly, the setDateFormat(int, int) method was not actually intended to serve as an
   * indexed write method; it just appears that way.
   */
  @Test
  ...
  public void propertyCountsWithNonStandardWriteMethod() throws IntrospectionException {
    ...
    assertThat(ebi.getPropertyDescriptors().length)
        .isEqualTo(bi.getPropertyDescriptors().length + 1);
  }

  /**
   * {@link BeanInfo#getPropertyDescriptors()} returns alphanumerically sorted. Test that {@link
   * ExtendedBeanInfo#getPropertyDescriptors()} does the same.
   */
  @Test
  public void propertyDescriptorOrderIsEqual() throws IntrospectionException {
    ...
    for (int i = 0; i < bi.getPropertyDescriptors().length; i++) {
      assertThat(ebi.getPropertyDescriptors()[i].getName())
          .isEqualTo(bi.getPropertyDescriptors()[i].getName());
    }
  }

  @Test
  public void propertyDescriptorComparator() throws IntrospectionException {
    ExtendedBeanInfo.PropertyDescriptorComparator c =
        new ExtendedBeanInfo.PropertyDescriptorComparator();

    assertThat(
            c.compare(
                new PropertyDescriptor("a", null, null), new PropertyDescriptor("a", null, null)))
        .isEqualTo(0);
    assertThat(
            c.compare(
                new PropertyDescriptor("abc", null, null),
                new PropertyDescriptor("abc", null, null)))
        .isEqualTo(0);
    assertThat(
            c.compare(
                new PropertyDescriptor("a", null, null), new PropertyDescriptor("b", null, null)))
        .isLessThan(0);
    assertThat(
            c.compare(
                new PropertyDescriptor("b", null, null), new PropertyDescriptor("a", null, null)))
        .isGreaterThan(0);
    assertThat(
            c.compare(
                new PropertyDescriptor("abc", null, null),
                new PropertyDescriptor("abd", null, null)))
        .isLessThan(0);
    assertThat(
            c.compare(
                new PropertyDescriptor("xyz", null, null),
                new PropertyDescriptor("123", null, null)))
        .isGreaterThan(0);
    assertThat(
            c.compare(
                new PropertyDescriptor("a", null, null), new PropertyDescriptor("abc", null, null)))
        .isLessThan(0);
    assertThat(
            c.compare(
                new PropertyDescriptor("abc", null, null), new PropertyDescriptor("a", null, null)))
        .isGreaterThan(0);
    assertThat(
            c.compare(
                new PropertyDescriptor("abc", null, null), new PropertyDescriptor("b", null, null)))
        .isLessThan(0);

    assertThat(
            c.compare(
                new PropertyDescriptor(" ", null, null), new PropertyDescriptor("a", null, null)))
        .isLessThan(0);
    assertThat(
            c.compare(
                new PropertyDescriptor("1", null, null), new PropertyDescriptor("a", null, null)))
        .isLessThan(0);
    assertThat(
            c.compare(
                new PropertyDescriptor("a", null, null), new PropertyDescriptor("A", null, null)))
  ...
  @Test // SPR-12434
  public void shouldDetectValidPropertiesAndIgnoreInvalidProperties()
      throws IntrospectionException {
