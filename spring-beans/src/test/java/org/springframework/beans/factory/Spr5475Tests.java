...
/**
 * SPR-5475 exposed the fact that the error message displayed when incorrectly invoking a factory
 * method is not instructive to the user and rather misleading.
 *
 * @author Chris Beams
 */
public class Spr5475Tests {
  ...
  public void noArgFactoryMethodInvokedWithOneArg() {
    assertExceptionMessageForMisconfiguredFactoryMethod(
        rootBeanDefinition(Foo.class)
            ...
            .addConstructorArgValue("bogusArg")
            .getBeanDefinition(),
  ...
  public void noArgFactoryMethodInvokedWithTwoArgs() {
    assertExceptionMessageForMisconfiguredFactoryMethod(
        rootBeanDefinition(Foo.class)
            ...
            .addConstructorArgValue("bogusArg2".getBytes())
            .getBeanDefinition(),
  ...
  private void assertExceptionMessageForMisconfiguredFactoryMethod(
      BeanDefinition bd, String expectedMessage) {
    DefaultListableBeanFactory factory = new DefaultListableBeanFactory();
    factory.registerBeanDefinition("foo", bd);
    assertThatExceptionOfType(BeanCreationException.class)
        .isThrownBy(factory::preInstantiateSingletons)
        .withMessageContaining(expectedMessage);
  }

  @Test
  public void singleArgFactoryMethodInvokedWithNoArgs() {
    // calling a factory method that accepts arguments without any arguments emits an exception
    // unlike cases
    // where a no-arg factory method is called with arguments. Adding this test just to document the
    // difference
    assertExceptionMessageForMisconfiguredFactoryMethod(
        rootBeanDefinition(Foo.class).setFactoryMethod("singleArgFactory").getBeanDefinition(),
