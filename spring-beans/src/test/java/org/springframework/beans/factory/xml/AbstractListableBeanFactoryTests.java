...
public abstract class AbstractListableBeanFactoryTests extends AbstractBeanFactoryTests {
  ...
  protected final void assertCount(int count) {
    ...
    assertThat(defnames.length == count)
        .as("We should have " + count + " beans, not " + defnames.length)
        .isTrue();
  }
  ...
  protected void assertTestBeanCount(int count) {
    ...
    assertThat(defNames.length == count)
        .as(
            "We should have "
                + count
                + " beans for class org.springframework.tests.sample.beans.TestBean, not "
                + defNames.length)
        .isTrue();

    int countIncludingFactoryBeans = count + 2;
    String[] names = getListableBeanFactory().getBeanNamesForType(TestBean.class, true, true);
    assertThat(names.length == countIncludingFactoryBeans)
        .as(
            "We should have "
                + countIncludingFactoryBeans
                + " beans for class org.springframework.tests.sample.beans.TestBean, not "
                + names.length)
        .isTrue();
  }
  ...
  /**
   * Check that count refers to factory class, not bean class. (We don't know what type factories
   * may return, and it may even change over time.)
   */
  @Test
  public void getCountForFactoryClass() {
    assertThat(getListableBeanFactory().getBeanNamesForType(FactoryBean.class).length == 2)
        .as(
            "Should have 2 factories, not "
