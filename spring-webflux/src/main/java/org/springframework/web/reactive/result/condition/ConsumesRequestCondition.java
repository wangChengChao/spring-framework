...
/**
 * A logical disjunction (' || ') request condition to match a request's 'Content-Type' header to a
 * list of media type expressions. Two kinds of media type expressions are supported, which are
 * described in {@link RequestMapping#consumes()} and {@link RequestMapping#headers()} where the
 * header name is 'Content-Type'. Regardless of which syntax is used, the semantics are the same.
 *
...
public final class ConsumesRequestCondition
   ...
   *
   * @param consumes expressions with the syntax described in {@link RequestMapping#consumes()}; if
   *     0 expressions are provided, the condition will match to every request
   */
  ...
  /**
   * Creates a new instance with "consumes" and "header" expressions. "Header" expressions where the
   * header name is not 'Content-Type' or have no header value defined are ignored. If 0 expressions
   * are provided in total, the condition will match to every request
   *
  ...
  /**
   * Private constructor for internal when creating matching conditions. Note the expressions List
  ...
  private static Set<ConsumeMediaTypeExpression> parseExpressions(
   ...
   *
   * <p>By default this is set to {@code true} in which case it is assumed a request body is
   * required and this condition matches to the "Content-Type" header or falls back on
   * "Content-Type: application/octet-stream".
   *
   * <p>If set to {@code false}, and the request does not have a body, then this condition matches
   * automatically, i.e. without checking expressions.
   *
   ...
   * Practically that means a method-level "consumes" overrides a type-level "consumes" condition.
   */
  @Override
   ...
   * match is performed via {@link MediaType#includes(MediaType)}.
   *
   * @param exchange the current exchange
   * @return the same instance if the condition contains no expressions; or a new condition with
   *     matching expressions only; or {@code null} if no expressions match.
   */
  @Override
   ...
   * #getMatchingCondition(ServerWebExchange)} and each instance contains the matching consumable
