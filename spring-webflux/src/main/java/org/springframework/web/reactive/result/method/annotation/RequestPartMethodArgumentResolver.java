...
/**
 * Resolver for {@code @RequestPart} arguments where the named part is decoded much like an
 * {@code @RequestBody} argument but based on the content of an individual part instead. The
 * arguments may be wrapped with a reactive type for a single value (e.g. Reactor {@code Mono},
 * RxJava {@code Single}).
 *
 * <p>This resolver also supports arguments of type {@link Part} which may be wrapped with are
 * reactive type for a single or multiple values.
 *
...
public class RequestPartMethodArgumentResolver extends AbstractMessageReaderArgumentResolver {
  ...
  public Mono<Object> resolveArgument(
    ...
    if (adapter != null) {
      MethodParameter elementType = parameter.nested();
      return Mono.just(
          adapter.fromPublisher(
              Part.class.isAssignableFrom(elementType.getNestedParameterType())
                  ? parts
                  : decodePartValues(parts, elementType, bindingContext, exchange, isRequired)));
    }
