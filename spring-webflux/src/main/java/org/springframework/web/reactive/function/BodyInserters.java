...
public abstract class BodyInserters {
   ...
   *
   * <p>Alternatively, consider using the {@code bodyValue(Object)} shortcuts on {@link
   * org.springframework.web.reactive.function.client.WebClient WebClient} and {@link
   * org.springframework.web.reactive.function.server.ServerResponse ServerResponse}.
   *
   * @param body the value to write
   * @param <T> the type of the body
   * @return the inserter to write a single value
   * @throws IllegalArgumentException if {@code body} is a {@link Publisher} or an instance of a
   *     type supported by {@link ReactiveAdapterRegistry#getSharedInstance()}, for which {@link
   *     #fromPublisher(Publisher, Class)} or {@link #fromProducer(Object, Class)} should be used.
  ...
  public static <T> BodyInserter<T, ReactiveHttpOutputMessage> fromValue(T body) {
    ...
    Assert.isNull(
        registry.getAdapter(body.getClass()),
    ...
    return (message, context) ->
        writeWithMessageWriters(
            message, context, Mono.just(body), ResolvableType.forInstance(body), null);
  }
  ...
  /**
   * Inserter to write the given producer of value(s) which must be a {@link Publisher} or another
   * producer adaptable to a {@code Publisher} via {@link ReactiveAdapterRegistry}.
   *
   * <p>Alternatively, consider using the {@code body} shortcuts on {@link
   * org.springframework.web.reactive.function.client.WebClient WebClient} and {@link
   * org.springframework.web.reactive.function.server.ServerResponse ServerResponse}.
   *
   * @param <T> the type of the body
  ...
  public static <T> BodyInserter<T, ReactiveHttpOutputMessage> fromProducer(
      T producer, Class<?> elementClass) {
    Assert.notNull(producer, "'producer' must not be null");
    Assert.notNull(elementClass, "'elementClass' must not be null");
    ReactiveAdapter adapter =
        ReactiveAdapterRegistry.getSharedInstance().getAdapter(producer.getClass());
    Assert.notNull(adapter, "'producer' type is unknown to ReactiveAdapterRegistry");
    return (message, context) ->
        writeWithMessageWriters(
            message, context, producer, ResolvableType.forClass(elementClass), adapter);
  }
  ...
  public static <T> BodyInserter<T, ReactiveHttpOutputMessage> fromProducer(
    ...
    return (message, context) ->
        writeWithMessageWriters(
            message, context, producer, ResolvableType.forType(elementTypeRef), adapter);
  }
  ...
  public static <T, P extends Publisher<T>>
      BodyInserter<P, ReactiveHttpOutputMessage> fromPublisher(P publisher, Class<T> elementClass) {

    Assert.notNull(publisher, "'publisher' must not be null");
    Assert.notNull(elementClass, "'elementClass' must not be null");
    return (message, context) ->
        writeWithMessageWriters(
            message, context, publisher, ResolvableType.forClass(elementClass), null);
  }
  ...
  public static <T, P extends Publisher<T>>
    ...
    return (message, context) ->
        writeWithMessageWriters(
            message, context, publisher, ResolvableType.forType(elementTypeRef.getType()), null);
  }

  /**
   * Inserter to write the given {@code Resource}.
   *
   * <p>If the resource can be resolved to a {@linkplain Resource#getFile() file}, it will be copied
   * using <a href="https://en.wikipedia.org/wiki/Zero-copy">zero-copy</a>.
   *
   * @param resource the resource to write to the output message
   * @param <T> the type of the {@code Resource}
   * @return the inserter to write a {@code Publisher}
   */
  public static <T extends Resource> BodyInserter<T, ReactiveHttpOutputMessage> fromResource(
      T resource) {
   ...
   *
   * <p>Alternatively, you can provide event data objects via {@link #fromPublisher(Publisher,
   * Class)} or {@link #fromProducer(Object, Class)}, and set the "Content-Type" to {@link
  ...
  /**
   * Return a {@link FormInserter} to write the given {@code MultiValueMap} as URL-encoded form
   * data. The returned inserter allows for additional entries to be added via {@link
   * FormInserter#with(String, Object)}.
   *
   * <p>Note that you can also use the {@code bodyValue(Object)} method in the request builders of
   * both the {@code WebClient} and {@code WebTestClient}. In that case the setting of the request
   * content type is also not required, just be sure the map contains String values only or
   * otherwise it would be interpreted as a multipart request.
   *
  ...
  /**
   * Return a {@link FormInserter} to write the given key-value pair as URL-encoded form data. The
   * returned inserter allows for additional entries to be added via {@link
   * FormInserter#with(String, Object)}.
   *
  ...
  /**
   * Return a {@link MultipartInserter} to write the given {@code MultiValueMap} as multipart data.
   * Values in the map can be an Object or an {@link HttpEntity}.
   *
   * <p>Note that you can also build the multipart data externally with {@link
   * MultipartBodyBuilder}, and pass the resulting map directly to the {@code bodyValue(Object)}
   * shortcut method in {@code WebClient}.
   *
  ...
  /**
   * Return a {@link MultipartInserter} to write the given parts, as multipart data. Values in the
   * map can be an Object or an {@link HttpEntity}.
   *
   * <p>Note that you can also build the multipart data externally with {@link
   * MultipartBodyBuilder}, and pass the resulting map directly to the {@code bodyValue(Object)}
   * shortcut method in {@code WebClient}.
  ...
  /**
   * Return a {@link MultipartInserter} to write the given asynchronous parts, as multipart data.
   *
   * <p>Note that you can also build the multipart data externally with {@link
   * MultipartBodyBuilder}, and pass the resulting map directly to the {@code bodyValue(Object)}
   * shortcut method in {@code WebClient}.
  ...
  /**
   * Variant of {@link #fromMultipartAsyncData(String, Publisher, Class)} that accepts a {@link
   * ParameterizedTypeReference} for the element type, which allows specifying generic type
   * information.
   *
   * <p>Note that you can also build the multipart data externally with {@link
   * MultipartBodyBuilder}, and pass the resulting map directly to the {@code bodyValue(Object)}
  ...
  private static <M extends ReactiveHttpOutputMessage> Mono<Void> writeWithMessageWriters(
      ...
      @Nullable ReactiveAdapter adapter) {
  ...
  /**
   * Extension of {@link BodyInserter} that allows for adding form data or multipart form data.
   *
   * @param <T> the value type
   */
  public interface FormInserter<T>
  ...
  public interface MultipartInserter extends FormInserter<Object> {
     ...
     * @return this inserter for adding more parts
     */
    <T, P extends Publisher<T>> MultipartInserter withPublisher(
        String name, P publisher, Class<T> elementClass);

    /**
     * Variant of {@link #withPublisher(String, Publisher, Class)} that accepts a {@link
     * ParameterizedTypeReference} for the element type, which allows specifying generic type
     * information.
     *
