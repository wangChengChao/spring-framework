 ...
 *       9de0f09ed7caf84e885f1f0f11c7e326"), thus changing the content of the manifest in order to
 *       trigger an appcache reload in the browser.
 * </ul>
 *
 * <p>All files with an ".appcache" file extension (or the extension given to the constructor) will
 * be transformed by this class. The hash is computed using the content of the appcache manifest so
 * that changes in the manifest should invalidate the browser cache. This should also work with
 * changes in referenced resources whose links are also versioned.
 *
 * @author Rossen Stoyanchev
 * @author Brian Clozel
 * @since 5.0
 * @see <a href="https://html.spec.whatwg.org/multipage/browsers.html#offline">HTML5 offline
 *     applications spec</a>
 */
public class AppCacheManifestTransformer extends ResourceTransformerSupport {
  ...
  /**
   * Create an AppCacheResourceTransformer that transforms files with the extension given as a
   * parameter.
   */
  ...
  public Mono<Resource> transform(
      ServerWebExchange exchange, Resource inputResource, ResourceTransformerChain chain) {
  ...
  private Mono<? extends Resource> transform(
      String content,
      Resource resource,
  ...
  private Mono<String> processLine(
      LineInfo info,
      ServerWebExchange exchange,
      Resource resource,
      ResourceTransformerChain chain) {
  ...
  private static class LineInfoGenerator implements Consumer<SynchronousSink<LineInfo>> {
    ...
    @Nullable private LineInfo previous;
