 ...
 *
 * <p>See {@link RequestedContentTypeResolverBuilder} to create a sequence of strategies.
 *
...
public interface RequestedContentTypeResolver {
  ...
  /**
   * A singleton list with {@link MediaType#ALL} that is returned from {@link #resolveMediaTypes}
  ...
  /**
   * Resolve the given request to a list of requested media types. The returned list is ordered by
   * specificity first and by quality parameter second.
   *
   * @param exchange the current exchange
   * @return the requested media types, or {@link #MEDIA_TYPE_ALL_LIST} if none were requested.
