 ...
 *
 * <p>See static factory methods in {@link WebSocketSession} for creating messages with the {@link
 * org.springframework.core.io.buffer.DataBufferFactory DataBufferFactory} for the session.
 *
...
public class WebSocketMessage {
   ...
   *
   * <p>See static factory methods in {@link WebSocketSession} or alternatively use {@link
   * WebSocketSession#bufferFactory()} to create the payload and then invoke this constructor.
   */
  ...
  /**
   * A variant of {@link #getPayloadAsText(Charset)} that uses {@code UTF-8} for decoding the raw
   * content to text.
   */
   ...
   * encoding. This is useful for text WebSocket messages, or otherwise when the payload is expected
   * to contain text.
   *
  ...
  /**
   * Retain the data buffer for the message payload, which is useful on runtimes (e.g. Netty) with
   * pooled buffers. A shortcut for:
   *
   * <pre>
  ...
  /**
   * Release the payload {@code DataBuffer} which is useful on runtimes (e.g. Netty) with pooled
   * buffers such as Netty. A shortcut for:
   *
   * <pre>
   * DataBuffer payload = message.getPayload();
  ...
  public String toString() {
    return "WebSocket "
        + this.type.name()
        + " message ("
        + this.payload.readableByteCount()
        + " bytes)";
  }
