 ...
 * functionality, such as to {@linkplain #route() create} a {@code RouterFunction} using a
 * discoverable builder-style API, to {@linkplain #route(RequestPredicate, HandlerFunction) create}
 * a {@code RouterFunction} given a {@code RequestPredicate} and {@code HandlerFunction}, and to do
 * further {@linkplain #nest(RequestPredicate, RouterFunction) subrouting} on an existing routing
 * function.
 *
 * <p>Additionally, this class can {@linkplain #toHttpHandler(RouterFunction) transform} a {@code
 * RouterFunction} into an {@code HttpHandler}, which can be run in Servlet 3.1+, Reactor, or
 * Undertow.
 *
...
public abstract class RouterFunctions {
  ...
  /**
   * Name of the {@link ServerWebExchange#getAttributes() attribute} that contains the matching
   * pattern, as a {@link org.springframework.web.util.pattern.PathPattern}.
   */
   ...
   *
   * <p>For instance, the following example routes GET requests for "/user" to the {@code listUsers}
   ...
   * @return a router function that routes to {@code handlerFunction} if {@code predicate} evaluates
   *     to {@code true}
   ...
   *
   * <p>For instance, the following example first creates a composed route that resolves to {@code
   * listUsers} for a GET, and {@code createUser} for a POST. This composed route then gets nested
   * with a "/user" path predicate, so that GET requests for "/user" will list users, and POST
   * request for "/user" will create a new user.
   *
   * <pre class="code">
   ...
   * @param <T> the type of response returned by the handler function
   * @return a router function that routes to {@code routerFunction} if {@code predicate} evaluates
   *     to {@code true}
   * @see RequestPredicates
   */
  ...
  /**
   * Returns the resource lookup function used by {@link #resources(String, Resource)}. The returned
   * function can be {@linkplain Function#andThen(Function) composed} on, for instance to return a
   * default resource when the lookup function does not match:
   *
   * <pre class="code">
   ...
   */
  public static Function<ServerRequest, Mono<Resource>> resourceLookupFunction(
      String pattern, Resource location) {
    return new PathResourceLookupFunction(pattern, location);
  }

  /**
   * Route to resources using the provided lookup function. If the lookup function provides a {@link
   * Resource} for the given request, it will be it will be exposed using a {@link HandlerFunction}
   * that handles GET, HEAD, and OPTIONS requests.
   *
   * @param lookupFunction the function to provide a {@link Resource} given the {@link
   *     ServerRequest}
   * @return a router function that routes to resources
   */
  ...
  /**
   * Convert the given {@linkplain RouterFunction router function} into a {@link HttpHandler}. This
   ...
   * </ul>
   *
   * <p>Note that {@code HttpWebHandlerAdapter} also implements {@link WebHandler}, allowing for
   * additional filter and exception handler registration through {@link WebHttpHandlerBuilder}.
   *
  ...
  /**
   * Convert the given {@linkplain RouterFunction router function} into a {@link HttpHandler}, using
  ...
  /**
   * Convert the given {@linkplain RouterFunction router function} into a {@link WebHandler}. This
   * conversion uses {@linkplain HandlerStrategies#builder() default strategies}.
   *
   * @param routerFunction the router function to convert
  ...
  /**
   * Convert the given {@linkplain RouterFunction router function} into a {@link WebHandler}, using
   * the given strategies.
   *
   * @param routerFunction the router function to convert
   * @param strategies the strategies to use
  ...
  /**
   * Represents a discoverable builder for router functions. Obtained via {@link
  ...
  public interface Builder {
    ...
    /**
     * Adds a route to the given handler function that handles all HTTP {@code GET} requests that
     * match the given pattern.
     *
     * @param pattern the pattern to match to
     * @param handlerFunction the handler function to handle all {@code GET} requests that match
     *     {@code pattern}
     * @return this builder
     */
    ...
    /**
     * Adds a route to the given handler function that handles all HTTP {@code GET} requests that
     * match the given pattern and predicate.
     *
     * <p>For instance, the following example routes GET requests for "/user" that accept JSON to
     * the {@code listUsers} method in {@code userController}:
     *
     * <pre class="code">
     * RouterFunction&lt;ServerResponse&gt; route =
     *   RouterFunctions.route()
    ...
    /**
     * Adds a route to the given handler function that handles all HTTP {@code HEAD} requests that
     * match the given pattern.
     *
     * @param pattern the pattern to match to
     * @param handlerFunction the handler function to handle all {@code HEAD} requests that match
     *     {@code pattern}
     * @return this builder
     */
    ...
    /**
     * Adds a route to the given handler function that handles all HTTP {@code POST} requests that
     * match the given pattern.
     *
     * @param pattern the pattern to match to
     * @param handlerFunction the handler function to handle all {@code POST} requests that match
     *     {@code pattern}
     * @return this builder
     */
     ...
     * match the given pattern and predicate.
     *
     * <p>For instance, the following example routes POST requests for "/user" that contain JSON to
     * the {@code addUser} method in {@code userController}:
     *
     * <pre class="code">
     * RouterFunction&lt;ServerResponse&gt; route =
     *   RouterFunctions.route()
    ...
    /**
     * Adds a route to the given handler function that handles all HTTP {@code PUT} requests that
     * match the given pattern.
     *
     * @param pattern the pattern to match to
     * @param handlerFunction the handler function to handle all {@code PUT} requests that match
     *     {@code pattern}
     * @return this builder
     */
     ...
     * match the given pattern and predicate.
     *
     * <p>For instance, the following example routes PUT requests for "/user" that contain JSON to
     * the {@code editUser} method in {@code userController}:
     *
     * <pre class="code">
     * RouterFunction&lt;ServerResponse&gt; route =
     *   RouterFunctions.route()
    ...
    /**
     * Adds a route to the given handler function that handles all HTTP {@code PATCH} requests that
     * match the given pattern.
     *
     * @param pattern the pattern to match to
     * @param handlerFunction the handler function to handle all {@code PATCH} requests that match
     *     {@code pattern}
     * @return this builder
     */
     ...
     * match the given pattern and predicate.
     *
     * <p>For instance, the following example routes PATCH requests for "/user" that contain JSON to
     * the {@code editUser} method in {@code userController}:
     *
     * <pre class="code">
     * RouterFunction&lt;ServerResponse&gt; route =
    ...
    /**
     * Adds a route to the given handler function that handles all HTTP {@code DELETE} requests that
     * match the given pattern.
     *
     * @param pattern the pattern to match to
     * @param handlerFunction the handler function to handle all {@code DELETE} requests that match
     *     {@code pattern}
     * @return this builder
     */
     ...
     *
     * @param pattern the pattern to match to
     * @param handlerFunction the handler function to handle all {@code OPTIONS} requests that match
     *     {@code pattern}
     * @return this builder
     */
    ...
    /**
     * Adds a route to the given handler function that handles all requests that match the given
     * predicate.
     *
     ...
     * functions into this builder, or can be combined with {@link
     * RouterFunctions#route(RequestPredicate, HandlerFunction)} to allow for more flexible
     * predicate matching.
     *
     * <p>For instance, the following example adds the router function returned from {@code
     * OrderController.routerFunction()}. to the {@code changeUser} method in {@code
     * userController}:
     *
     * <pre class="code">
     * RouterFunction&lt;ServerResponse&gt; route =
     ...
     * {@link Resource} for the given request, it will be it will be exposed using a {@link
    ...
    /**
     * Route to the supplied router function if the given request predicate applies. This method can
     * be used to create <strong>nested routes</strong>, where a group of routes share a common path
     * (prefix), header, or other request predicate.
     *
     * <p>For instance, the following example creates a nested route with a "/user" path predicate,
     * so that GET requests for "/user" will list users, and POST request for "/user" will create a
     * new user.
     *
     * <pre class="code">
     * RouterFunction&lt;ServerResponse&gt; nestedRoute =
     ...
     *
     * @param predicate the predicate to test
     * @param routerFunctionSupplier supplier for the nested router function to delegate to if the
     *     predicate applies
     * @return this builder
     * @see RequestPredicates
     */
    ...
    /**
     * Route to a built router function if the given request predicate applies. This method can be
     * used to create <strong>nested routes</strong>, where a group of routes share a common path
     * (prefix), header, or other request predicate.
     *
     * <p>For instance, the following example creates a nested route with a "/user" path predicate,
     ...
     * can be used to create <strong>nested routes</strong>, where a group of routes share a common
     * path prefix. Specifically, this method can be used to merge externally defined router
     * functions under a path prefix.
     *
     * <p>For instance, the following example creates a nested route with a "/user" path predicate
     * that delegates to the router function defined in {@code userController}, and with a "/order"
     * path that delegates to {@code orderController}.
     *
     * <pre class="code">
     * RouterFunction&lt;ServerResponse&gt; nestedRoute =
     *   RouterFunctions.route()
    ...
    /**
     * Route to a built router function if the given path prefix pattern applies. This method can be
     * used to create <strong>nested routes</strong>, where a group of routes share a common path
     * prefix.
     *
     * <p>For instance, the following example creates a nested route with a "/user" path predicate,
    ...
    /**
     * Filters all routes created by this builder with the given filter function. Filter functions
     * are typically used to address cross-cutting concerns, such as logging, security, etc.
     *
     ...
     * processing function. Filters are typically used to address cross-cutting concerns, such as
     * logging, security, etc.
     *
     * <p>For instance, the following example creates a filter that logs the request before the
     ...
     * logging, security, etc.
     *
     * <p>For instance, the following example creates a filter that logs the response after the
     * handler function executes.
     *
     * <pre class="code">
     * RouterFunction&lt;ServerResponse&gt; filteredRoute =
     ...
     * function.
     *
     * <p>For instance, the following example creates a filter that returns a 500 response status
     * when an {@code IllegalStateException} occurs.
     *
     * <pre class="code">
     * RouterFunction&lt;ServerResponse&gt; filteredRoute =
     *   RouterFunctions.route()
     ...
     * @return this builder
     */
    <T extends Throwable> Builder onError(
        Class<T> exceptionType,
        BiFunction<? super T, ServerRequest, Mono<ServerResponse>> responseProvider);

    /**
     * Builds the {@code RouterFunction}. All created routes are {@linkplain
     * RouterFunction#and(RouterFunction) composed} with one another, and filters (if any) are
  ...
  private abstract static class AbstractRouterFunction<T extends ServerResponse>
  ...
  /**
   * A composed routing function that first invokes one function, and then invokes the another
   * function (of the same response type {@code T}) if this route had {@linkplain Mono#empty() no
   * result}.
   *
  ...
  /**
   * A composed routing function that first invokes one function, and then invokes another function
   * (of a different response type) if this route had {@linkplain Mono#empty() no result}.
   */
  ...
  /**
   * Filter the specified {@linkplain HandlerFunction handler functions} with the given {@linkplain
   * HandlerFilterFunction filter function}.
   *
  ...
  private static final class DefaultNestedRouterFunction<T extends ServerResponse>
    ...
    public DefaultNestedRouterFunction(
        RequestPredicate predicate, RouterFunction<T> routerFunction) {
      Assert.notNull(predicate, "Predicate must not be null");
      Assert.notNull(routerFunction, "RouterFunction must not be null");
      this.predicate = predicate;
  ...
  private static class RouterFunctionWebHandler implements WebHandler {
    ...
    public RouterFunctionWebHandler(
        HandlerStrategies strategies, RouterFunction<?> routerFunction) {
      this.strategies = strategies;
      this.routerFunction = routerFunction;
    }
      ...
      return Mono.defer(
          () -> {
            ServerRequest request =
                new DefaultServerRequest(exchange, this.strategies.messageReaders());
