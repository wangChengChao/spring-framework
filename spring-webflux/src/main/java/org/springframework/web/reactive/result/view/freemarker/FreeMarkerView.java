 ...
 * accessible in the application context. Alternatively the FreeMarker {@link Configuration} can be
 * set directly on this class via {@link #setConfiguration}.
 *
 * <p>The {@link #setUrl(String) url} property is the location of the FreeMarker template relative
 * to the FreeMarkerConfigurer's {@link FreeMarkerConfigurer#setTemplateLoaderPath
 * templateLoaderPath}.
 *
...
public class FreeMarkerView extends AbstractUrlBasedView {
  ...
  /**
   * Attribute name of the {@link RequestContext} instance in the template model, available to
   * Spring's macros &mdash; for example, for creating {@link
   * org.springframework.web.reactive.result.view.BindStatus BindStatus} objects.
   *
   * @since 5.2
   ...
   *
   * <p>Typically this property is not set directly. Instead a single {@link FreeMarkerConfig} is
   * expected in the Spring application context which is used to obtain the FreeMarker
   * configuration.
   */
   ...
   *
   * <p>By default {@link FreeMarkerConfigurer} sets the default encoding in the FreeMarker
   * configuration to "UTF-8". It's recommended to specify the encoding in the FreeMarker {@link
   * Configuration} rather than per template if all your templates share a common encoding.
   */
  ...
  /**
   * Set whether to expose a {@link RequestContext} for use by Spring's macro library, under the
   * name {@value #SPRING_MACRO_REQUEST_CONTEXT_ATTRIBUTE}.
   *
   * <p>Default is {@code true}.
   *
   * <p>Needed for Spring's FreeMarker default macros. Note that this is <i>not</i> required for
   * templates that use HTML forms <i>unless</i> you wish to take advantage of the Spring helper
   * macros.
   *
   * @since 5.2
  ...
  /**
   * Prepare the model to use for rendering by potentially exposing a {@link RequestContext} for use
   * in Spring FreeMarker macros and then delegating to the inherited implementation of this method.
   *
   * @since 5.2
   * @see #setExposeSpringMacroHelpers(boolean)
   * @see org.springframework.web.reactive.result.view.AbstractView#getModelAttributes(Map,
   *     ServerWebExchange)
   */
  @Override
            ...
            Mono.fromCallable(
                    () -> {
                      ...
                      try {
                        ...
                        Writer writer =
                            new OutputStreamWriter(dataBuffer.asOutputStream(), charset);
  ...
  /**
   * Get the configured FreeMarker {@link ObjectWrapper}, or the {@linkplain
