...
/**
 * Represents a typed server-side HTTP response, as returned by a {@linkplain HandlerFunction
 * handler function} or {@linkplain HandlerFilterFunction filter function}.
 *
...
public interface ServerResponse {
  ...
  /**
   * Create a new builder with a {@linkplain HttpStatus#CREATED 201 Created} status and a location
  ...
  /**
   * Create a builder with a {@linkplain HttpStatus#SEE_OTHER 303 See Other} status and a location
   * header set to the given URI.
   *
   * @param location the location URI
   * @return the created builder
  ...
  /**
   * Create a builder with an {@linkplain HttpStatus#UNPROCESSABLE_ENTITY 422 Unprocessable Entity}
   * status.
   *
   * @return the created builder
   */
  ...
  interface HeadersBuilder<B extends HeadersBuilder<B>> {
    ...
    /**
     * Manipulate this response's headers with the given consumer. The headers provided to the
    ...
    /**
     * Build the response entity with no body. The response will be committed when the given {@code
     * voidPublisher} completes.
     *
  ...
  interface BodyBuilder extends HeadersBuilder<BodyBuilder> {
    ...
    /**
     * Add a serialization hint like {@link Jackson2CodecSupport#JSON_VIEW_HINT} to customize how
     * the body will be serialized.
     *
    ...
    /**
     * Set the body of the response to the given {@code Object} and return it. This is a shortcut
     * for using a {@link #body(BodyInserter)} with a {@linkplain BodyInserters#fromValue value
     * inserter}.
     *
     ...
     * @return the built response
     */
    <T, P extends Publisher<T>> Mono<ServerResponse> body(
    ...
    /**
     * Set the response body to the given {@code Object} and return it. As of 5.2 this method
    ...
    /**
     * Render the template with the given {@code name} using the given {@code modelAttributes}. The
     * model attributes are mapped under a {@linkplain
     * org.springframework.core.Conventions#getVariableName generated name}.
     *
     * <p><em>Note: Empty {@link Collection Collections} are not added to the model when using this
     * method because we cannot correctly determine the true convention name.</em>
     *
  ...
  interface Context {
    ...
    /**
     * Return the {@link HttpMessageWriter HttpMessageWriters} to be used for response body
     * conversion.
     *
