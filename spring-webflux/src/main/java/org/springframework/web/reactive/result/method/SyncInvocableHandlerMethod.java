...
/**
 * Extension of {@link HandlerMethod} that invokes the underlying method via {@link
 * InvocableHandlerMethod} but uses sync argument resolvers only and thus can return directly a
 * {@link HandlerResult} with no async wrappers.
 *
...
public class SyncInvocableHandlerMethod extends HandlerMethod {
   ...
   */
  @Nullable
  public HandlerResult invokeForHandlerResult(
    ...
    if (processor.isTerminated()) {
      ...
      if (ex != null) {
        throw (ex instanceof ServerErrorException
            ? (ServerErrorException) ex
            : new ServerErrorException(
                "Failed to invoke: " + getShortLogMessage(), getMethod(), ex));
      }
