...
/**
 * Represents an HTTP response, as returned by {@link WebClient} and also {@link ExchangeFunction}.
 * Provides access to the response status and headers, and also methods to consume the response
 * body.
 *
 * <p><strong>NOTE:</strong> When using a {@link ClientResponse} through the {@code WebClient}
 * {@link WebClient.RequestHeadersSpec#exchange() exchange()} method, you have to make sure that the
 * body is consumed or released by using one of the following methods:
 *
 * <ul>
 *   <li>{@link #body(BodyExtractor)}
 *   <li>{@link #bodyToMono(Class)} or {@link #bodyToMono(ParameterizedTypeReference)}
 *   <li>{@link #bodyToFlux(Class)} or {@link #bodyToFlux(ParameterizedTypeReference)}
 *   <li>{@link #toEntity(Class)} or {@link #toEntity(ParameterizedTypeReference)}
 *   <li>{@link #toEntityList(Class)} or {@link #toEntityList(ParameterizedTypeReference)}
 *   <li>{@link #toBodilessEntity()}
 *   <li>{@link #releaseBody()}
 * </ul>
 *
 * You can use {@code bodyToMono(Void.class)} if no response content is expected. However keep in
 * mind that if the response does have content, the connection will be closed and will not be placed
 * back in the pool.
 *
 * @author Brian Clozel
...
public interface ClientResponse {
  ...
  /**
   * Return this response as a delayed {@code ResponseEntity} containing status and headers, but no
   * body. Calling this method will {@linkplain #releaseBody() release} the body of the response.
   *
  ...
  /**
   * Creates a {@link WebClientResponseException} based on the status code, headers, and body of
   * this response as well as the corresponding request.
   *
  ...
  interface Builder {
    ...
    /**
     * Manipulate this response's cookies with the given consumer. The map provided to the consumer
    ...
    /**
     * Set the body of the response. Calling this methods will {@linkplain
    ...
    /**
     * Set the body of the response to the UTF-8 encoded bytes of the given string. Calling this
     * methods will {@linkplain
     * org.springframework.core.io.buffer.DataBufferUtils#release(DataBuffer) release} the existing
     * body of the builder.
     *
     * @param body the new body.
