...
/**
 * Undertow {@link WebSocketConnectionCallback} implementation that adapts and delegates to a Spring
...
public class UndertowWebSocketHandlerAdapter extends AbstractReceiveListener {
  ...
  protected void onFullBinaryMessage(WebSocketChannel channel, BufferedBinaryMessage message) {
    this.session.handleMessage(
        Type.BINARY, toMessage(Type.BINARY, message.getData().getResource()));
  ...
  private <T> WebSocketMessage toMessage(Type type, T message) {
    if (Type.TEXT.equals(type)) {
      ...
      DataBuffer buffer =
          this.session.bufferFactory().allocateBuffer().write((ByteBuffer[]) message);
