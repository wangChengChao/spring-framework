...
public abstract class BodyExtractors {
  ...
  private static <T> BodyExtractor<Flux<T>, ReactiveHttpInputMessage> toFlux(
      ResolvableType elementType) {
    return (inputMessage, context) ->
        readWithMessageReaders(
            inputMessage,
   ...
   *
   * <p>As of 5.1 this method can also be used on the client side to read form data from a server
   * response (e.g. OAuth).
   *
   * @return {@code BodyExtractor} for form data
   */
  public static BodyExtractor<Mono<MultiValueMap<String, String>>, ReactiveHttpInputMessage>
      toFormData() {
    return (message, context) -> {
      ...
      HttpMessageReader<MultiValueMap<String, String>> reader =
          findReader(elementType, mediaType, context);
  ...
  public static BodyExtractor<Mono<MultiValueMap<String, Part>>, ServerHttpRequest>
      toMultipartData() {
   ...
   *
   * <p><strong>Note:</strong> the data buffers should be {@link
   * org.springframework.core.io.buffer.DataBufferUtils#release(DataBuffer) released} after being
   * used.
   *
  ...
  private static <T, S extends Publisher<T>> S readWithMessageReaders(
    ...
    MediaType contentType =
        Optional.ofNullable(message.getHeaders().getContentType())
  ...
  private static <T> Mono<T> readToMono(
      ReactiveHttpInputMessage message,
      BodyExtractor.Context context,
      ResolvableType type,
  ...
  private static <T> Flux<T> readToFlux(
      ReactiveHttpInputMessage message,
      BodyExtractor.Context context,
      ResolvableType type,
      HttpMessageReader<T> reader) {
    ...
    return context
        ...
        .map(
            response ->
                reader.read(type, type, (ServerHttpRequest) message, response, context.hints()))
    ...
    if (message.getHeaders().getContentType() == null) {
      ...
      result =
          message instanceof ClientHttpResponse
              ? consumeAndCancel(message).thenMany(Flux.error(ex))
              : Flux.error(ex);
    }
    return result;
  }
  ...
  private static <T> Supplier<Flux<T>> skipBodyAsFlux(ReactiveHttpInputMessage message) {
    return message instanceof ClientHttpResponse
        ? () -> consumeAndCancel(message).thenMany(Mono.empty())
        : Flux::empty;
  }
  ...
  private static <T> Supplier<Mono<T>> skipBodyAsMono(ReactiveHttpInputMessage message) {
    return message instanceof ClientHttpResponse
        ? () -> consumeAndCancel(message).then(Mono.empty())
        : Mono::empty;
  }
