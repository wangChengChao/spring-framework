...
/**
 * A logical conjunction (' && ') request condition that matches a request against a set of header
 * expressions with syntax defined in {@link RequestMapping#headers()}.
 *
 * <p>Expressions passed to the constructor with header names 'Accept' or 'Content-Type' are
 * ignored. See {@link ConsumesRequestCondition} and {@link ProducesRequestCondition} for those.
 *
...
public final class HeadersRequestCondition
  ...
  /**
   * Create a new instance from the given header expressions. Expressions with header names 'Accept'
   * or 'Content-Type' are ignored. See {@link ConsumesRequestCondition} and {@link
  ...
  /**
   * Returns a new instance with the union of the header expressions from "this" and the "other"
   * instance.
   */
  @Override
  ...
  /**
   * Compare to another condition based on header expressions. A condition is considered to be a
   * more specific match, if it has:
   *
   * <ol>
   ...
   * #getMatchingCondition(ServerWebExchange)} and each instance contains the matching header
  ...
  static class HeaderExpression extends AbstractNameValueExpression<String> {
    ...
    protected boolean matchValue(ServerWebExchange exchange) {
      return (this.value != null
          && this.value.equals(exchange.getRequest().getHeaders().getFirst(this.name)));
    }
  }
}
