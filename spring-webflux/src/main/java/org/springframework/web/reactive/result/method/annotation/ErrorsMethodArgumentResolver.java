...
/**
 * Resolve {@link Errors} or {@link BindingResult} method arguments. An {@code Errors} argument is
 * expected to appear immediately after the model attribute in the method signature.
 *
...
public class ErrorsMethodArgumentResolver extends HandlerMethodArgumentResolverSupport {
  ...
  private Object getErrors(MethodParameter parameter, BindingContext context) {
    ...
    MethodParameter attributeParam =
        MethodParameter.forExecutable(parameter.getExecutable(), index);
    ...
    String name =
        (ann != null && StringUtils.hasText(ann.value())
            ? ann.value()
