 ...
 * stream, and {@link WebSocketSession#send(Publisher) session.send(publisher)} to provide the
...
public interface WebSocketSession {
   ...
   *
   * <p>This stream receives a completion or error signal when the connection is closed. In a
   * typical {@link WebSocketHandler} implementation this stream is composed into the overall
   * processing flow, so that when the connection is closed, handling will end.
   *
   * <p>See the class-level doc of {@link WebSocketHandler} and the reference for more details and
   * examples of how to handle the session.
   */
  Flux<WebSocketMessage> receive();

  /**
   * Give a source of outgoing messages, write the messages and return a {@code Mono<Void>} that
  ...
  /**
   * Factory method to create a text {@link WebSocketMessage} using the {@link #bufferFactory()} for
   * the session.
   */
  WebSocketMessage textMessage(String payload);

  /**
   * Factory method to create a binary WebSocketMessage using the {@link #bufferFactory()} for the
   * session.
   */
  WebSocketMessage binaryMessage(Function<DataBufferFactory, DataBuffer> payloadFactory);

  /**
   * Factory method to create a ping WebSocketMessage using the {@link #bufferFactory()} for the
   * session.
   */
  WebSocketMessage pingMessage(Function<DataBufferFactory, DataBuffer> payloadFactory);

  /**
   * Factory method to create a pong WebSocketMessage using the {@link #bufferFactory()} for the
   * session.
   */
