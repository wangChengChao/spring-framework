 ...
 * given request.
 *
 * <p>When {@code CompositeRequestCondition} instances are combined or compared they are expected to
 * (a) contain the same number of conditions and (b) that conditions in the respective index are of
 * the same type. It is acceptable to provide {@code null} conditions or no conditions at all to the
 * constructor.
 *
...
public class CompositeRequestCondition extends AbstractRequestCondition<CompositeRequestCondition> {
  ...
  /**
   * Create an instance with 0 or more {@code RequestCondition} types. It is important to create
   * {@code CompositeRequestCondition} instances with the same number of conditions so they may be
   * compared and combined. It is acceptable to provide {@code null} conditions.
   */
  ...
  /**
   * If one instance is empty, return the other. If both instances have conditions, combine the
   * individual conditions after ensuring they are of the same type and number.
   */
  @Override
  ...
  private void assertNumberOfConditions(CompositeRequestCondition other) {
    Assert.isTrue(
        ...
        "Cannot combine CompositeRequestConditions with a different number of conditions. "
            + ObjectUtils.nullSafeToString(this.requestConditions)
            + " and  "
            + ObjectUtils.nullSafeToString(other.requestConditions));
  }

  /**
   * Delegate to <em>all</em> contained conditions to match the request and return the resulting
   * "matching" condition instances.
   *
  ...
  /**
   * If one instance is empty, the other "wins". If both instances have conditions, compare them in
