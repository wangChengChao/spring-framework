...
/**
 * Resolver for method arguments annotated with @{@link RequestParam} from URI query string
 * parameters.
 *
 * <p>This resolver can also be created in default resolution mode in which simple types (int, long,
 * etc.) not annotated with @{@link RequestParam} are also treated as request parameters with the
 ...
 * conversion assuming a suitable {@link Converter} has been registered. Or if a request parameter
 * name is not specified the {@link RequestParamMapMethodArgumentResolver} is used instead to
 * provide access to all request parameters in the form of a map.
 *
...
public class RequestParamMethodArgumentResolver extends AbstractNamedValueSyncArgumentResolver {
   ...
   *
   * @param factory a bean factory used for resolving ${...} placeholder and #{...} SpEL expressions
   *     in default values, or {@code null} if default values are not expected to contain
   *     expressions
