...
/**
 * Resolves request paths containing a version string that can be used as part of an HTTP caching
 * strategy in which a resource is cached with a date in the distant future (e.g. 1 year) and cached
 * until the version, and therefore the URL, is changed.
 *
 * <p>Different versioning strategies exist, and this resolver must be configured with one or more
 * such strategies along with path mappings to indicate which strategy applies to which resources.
 *
 * <p>{@code ContentVersionStrategy} is a good default choice except in cases where it cannot be
 * used. Most notably the {@code ContentVersionStrategy} cannot be combined with JavaScript module
 * loaders. For such cases the {@code FixedVersionStrategy} is a better choice.
 *
 * <p>Note that using this resolver to serve CSS files means that the {@link
 * CssLinkResourceTransformer} should also be used in order to modify links within CSS files to also
 * contain the appropriate versions generated by this resolver.
 *
...
public class VersionResourceResolver extends AbstractResourceResolver {
  ...
  /**
   * Insert a content-based version in resource URLs that match the given path patterns. The version
   * is computed from the content of the file, e.g. {@code
   * "css/main-e36d2e05253c6c7085a91522ce43a0b4.css"}. This is a good default strategy to use except
   * when it cannot be, for example when using JavaScript module loaders, use {@link
   * #addFixedVersionStrategy} instead for serving JavaScript files.
   *
   * @param pathPatterns one or more resource URL path patterns, relative to the pattern configured
   *     with the resource handler
  ...
  /**
   * Insert a fixed, prefix-based version in resource URLs that match the given path patterns, for
   * example: <code>"{version}/js/main.js"</code>. This is useful (vs. content-based versions) when
   * using JavaScript module loaders.
   *
   * <p>The version may be a random number, the current date, or a value fetched from a git commit
   * sha, a property file, or environment variable and set with SpEL expressions in the
   * configuration (e.g. see {@code @Value} in Java config).
   *
   * <p>If not done already, variants of the given {@code pathPatterns}, prefixed with the {@code
   * version} will be also configured. For example, adding a {@code "/js/**"} path pattern will also
   * cofigure automatically a {@code "/v1.0.0/js/**"} with {@code "v1.0.0"} the {@code version}
  ...
  public VersionResourceResolver addFixedVersionStrategy(String version, String... pathPatterns) {
    ...
    return addVersionStrategy(
        new FixedVersionStrategy(version), StringUtils.toStringArray(prefixedPatterns));
  }
   ...
   * @see VersionStrategy
   */
  public VersionResourceResolver addVersionStrategy(
      VersionStrategy strategy, String... pathPatterns) {
  ...
  protected Mono<Resource> resolveResourceInternal(
    ...
    return chain
        ...
        .switchIfEmpty(
            Mono.defer(() -> resolveVersionedResource(exchange, requestPath, locations, chain)));
  }

  private Mono<Resource> resolveVersionedResource(
      @Nullable ServerWebExchange exchange,
      String requestPath,
      List<? extends Resource> locations,
      ResourceResolverChain chain) {
    ...
    return chain
        .resolveResource(exchange, simplePath, locations)
        .filterWhen(
            resource ->
                versionStrategy
        ...
        .flatMap(
            baseUrl -> {
              if (StringUtils.hasText(baseUrl)) {
                ...
                return chain
                    .resolveResource(null, baseUrl, locations)
                    .flatMap(
                        resource ->
                            strategy
                                .getResourceVersion(resource)
