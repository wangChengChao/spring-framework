...
/**
 * {@link WebSocketSession} implementation for use with the Reactor Netty's {@link NettyInbound} and
 * {@link NettyOutbound}.
 *
...
public class ReactorNettyWebSocketSession
  ...
  public ReactorNettyWebSocketSession(
  ...
  public Mono<Void> send(Publisher<WebSocketMessage> messages) {
    Flux<WebSocketFrame> frames =
        Flux.from(messages)
            ...
            .map(this::toFrame);
    return getDelegate().getOutbound().sendObject(frames).then();
  }
