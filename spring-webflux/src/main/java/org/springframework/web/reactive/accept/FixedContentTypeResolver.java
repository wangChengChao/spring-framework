...
/**
 * Resolver that always resolves to a fixed list of media types. This can be used as the "last in
 * line" strategy providing a fallback for when the client has not requested any media types.
 *
...
public class FixedContentTypeResolver implements RequestedContentTypeResolver {
  ...
  /**
   * Constructor with an ordered List of default {@code MediaType}'s to return for use in
   * applications that support a variety of content types.
   *
   * <p>Consider appending {@link MediaType#ALL} at the end if destinations are present which do not
   * support any of the other default media types.
   */
