 ...
 *
 * <p>Applies validation if the method argument is annotated with {@code @javax.validation.Valid} or
 * {@link org.springframework.validation.annotation.Validated}. Validation failure results in an
...
public abstract class AbstractMessageReaderArgumentResolver
   ...
   * @param actualParam the actual method argument type; possibly different from {@code bodyParam},
   *     e.g. for an {@code HttpEntity} argument
   * @param isBodyRequired true if the body is required
   * @param bindingContext the binding context to use
   * @param exchange the current exchange
   ...
   */
  protected Mono<Object> readBody(
      MethodParameter bodyParam,
      @Nullable MethodParameter actualParam,
      boolean isBodyRequired,
      BindingContext bindingContext,
      ServerWebExchange exchange) {
    ...
    ResolvableType actualType =
        (actualParam != null ? ResolvableType.forMethodParameter(actualParam) : bodyType);
      ...
      if (reader.canRead(elementType, mediaType)) {
        ...
        if (adapter != null && adapter.isMultiValue()) {
          ...
          if (hints != null) {
            flux =
                flux.doOnNext(
                    target -> validate(target, hints, bodyParam, bindingContext, exchange));
          }
              ...
              .doOnNext(
                  buffer -> {
                    ...
                    throw new UnsupportedMediaTypeStatusException(
                        mediaType, this.supportedMediaTypes, elementType);
                  });
      if (isBodyRequired) {
  ...
  /**
   * Check if the given MethodParameter requires validation and if so return a (possibly empty)
   * Object[] with validation hints. A return value of {@code null} indicates that validation is not
   * required.
   */
  @Nullable
