...
/**
 * Static factory methods providing access to built-in implementations of {@link
...
public abstract class ExchangeFilterFunctions {
   ...
   *
   * @deprecated as of Spring 5.1 in favor of using {@link HttpHeaders#setBasicAuth(String, String)}
  ...
  /**
   * Return a filter that generates an error signal when the given {@link HttpStatus} predicate
   * matches.
   *
  ...
  public static ExchangeFilterFunction statusError(
    ...
    return ExchangeFilterFunction.ofResponseProcessor(
        response ->
            (statusPredicate.test(response.statusCode())
                ? Mono.error(exceptionFunction.apply(response))
                : Mono.just(response)));
  }

  /**
   * Return a filter that applies HTTP Basic Authentication to the request headers via {@link
   * HttpHeaders#setBasicAuth(String)} and {@link HttpHeaders#encodeBasicAuth(String, String,
   * Charset)}.
   *
  ...
  /**
   * Variant of {@link #basicAuthentication(String, String)} that looks up the {@link Credentials
   * Credentials} in a {@link #BASIC_AUTHENTICATION_CREDENTIALS_ATTRIBUTE request attribute}.
   *
  ...
  public static final class Credentials {
    ...
    /**
     * Return a {@literal Consumer} that stores the given username and password as a request
     * attribute of type {@code Credentials} that is in turn used by {@link
     * ExchangeFilterFunctions#basicAuthentication()}.
     *
     * @param username the username
     * @param password the password
     * @return a consumer that can be passed into {@linkplain
     *     ClientRequest.Builder#attributes(java.util.function.Consumer)}
     * @see ClientRequest.Builder#attributes(java.util.function.Consumer)
     * @see #BASIC_AUTHENTICATION_CREDENTIALS_ATTRIBUTE
     */
    public static Consumer<Map<String, Object>> basicAuthenticationCredentials(
