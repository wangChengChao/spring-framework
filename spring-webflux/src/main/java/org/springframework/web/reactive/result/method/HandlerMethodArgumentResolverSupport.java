...
/**
 * Base class for {@link HandlerMethodArgumentResolver} implementations with access to a {@code
...
public abstract class HandlerMethodArgumentResolverSupport
  ...
  /**
   * Evaluate the {@code Predicate} on the method parameter type or on the generic type within a
  ...
  /**
   * Evaluate the {@code Predicate} on the method parameter type but raise an {@code
   * IllegalStateException} if the same matches the generic type within a reactive type wrapper.
   */
  protected boolean checkParameterTypeNoReactiveWrapper(
  ...
  /**
   * Evaluate the {@code Predicate} on the method parameter type if it has the given annotation,
   * nesting within {@link java.util.Optional} if necessary, but raise an {@code
   * IllegalStateException} if the same matches the generic type within a reactive type wrapper.
   */
