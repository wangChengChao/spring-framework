...
public abstract class AbstractListenerWebSocketSession<T> extends AbstractWebSocketSession<T>
  ...
  /**
   * Whether the underlying WebSocket API has flow control and can suspend and resume the receiving
   * of messages.
   *
  ...
  protected final class WebSocketSendProcessor
    ...
    /**
     * Sub-classes can invoke this before sending a message (false) and after receiving the async
     * send callback (true) effective translating async completion callback into simple flow
     * control.
     */
