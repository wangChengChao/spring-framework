...
/**
 * Simple container of information related to the handshake request that started the {@link
...
public class HandshakeInfo {
  ...
  public HandshakeInfo(
      URI uri, HttpHeaders headers, Mono<Principal> principal, @Nullable String protocol) {
    this(uri, headers, principal, protocol, null, Collections.emptyMap(), null);
  }

  /**
   * Constructor targetting server-side use with extra information about the handshake, the remote
   * address, and a pre-existing log prefix for correlation.
   *
   * @param uri the endpoint URL
   * @param headers request headers for server or response headers or client
   * @param principal the principal for the session
   ...
   * @param logPrefix log prefix used during the handshake for correlating log messages, if any.
   * @since 5.1
   */
  public HandshakeInfo(
      URI uri,
  ...
  /**
   * Return the handshake HTTP headers. Those are the request headers for a server session and the
   * response headers for a client session.
   */
