...
public final class PatternsRequestCondition
   ...
   *       in "other" using {@link PathPattern#combine(PathPattern)}.
   ...
   * guaranteed to contain matching patterns, sorted.
   *
   ...
   * equivalent to those of calling {@link #getMatchingCondition(ServerWebExchange)}. This method is
   * provided as an alternative to be used if no request is available (e.g. introspection, tooling,
   * etc).
   *
   * @param exchange the current exchange
   ...
   * a time, from top to bottom. If all compared patterns match equally, but one instance has more
   * patterns, it is considered a closer match.
   *
   * <p>It is assumed that both instances have been obtained via {@link
   * #getMatchingCondition(ServerWebExchange)} to ensure they contain only patterns that match the
  ...
  public int compareTo(PatternsRequestCondition other, ServerWebExchange exchange) {
    ...
    while (iterator.hasNext() && iteratorOther.hasNext()) {
      int result =
          PathPattern.SPECIFICITY_COMPARATOR.compare(iterator.next(), iteratorOther.next());
      if (result != 0) {
        return result;
      }
    }
