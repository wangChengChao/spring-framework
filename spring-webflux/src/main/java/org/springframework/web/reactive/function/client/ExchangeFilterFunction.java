 ...
 *
 * <p>The filter is executed when a {@code Subscriber} subscribes to the {@code Publisher} returned
 * by the {@code WebClient}.
 *
...
public interface ExchangeFilterFunction {
   ...
   *
   * <p>The given {@linkplain ExchangeFunction} represents the next entity in the chain, to be
   * invoked via {@linkplain ExchangeFunction#exchange(ClientRequest) invoked} in order to proceed
   * with the exchange, or not invoked to shortcut the chain.
   *
  ...
  /**
   * Apply this filter to the given {@linkplain ExchangeFunction}, resulting in a filtered exchange
   * function.
   *
