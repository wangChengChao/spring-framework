...
public class CachingResourceResolverTests {
  ...
  public void resolveResourceInternal() {
    ...
    Resource actual =
        this.chain.resolveResource(exchange, "bar.css", this.locations).block(TIMEOUT);
  ...
  public void resolverUrlPathFromCache() {
    ...
    this.cache.put(
        CachingResourceResolver.RESOLVED_URL_PATH_CACHE_KEY_PREFIX + "imaginary.css", expected);
  ...
  public void resolveResourceAcceptEncodingInCacheKey() throws IOException {
    ...
    exchange =
        MockServerWebExchange.from(
            get(file).header("Accept-Encoding", "gzip ; a=b  , deflate ,  br  ; c=d "));
  ...
  public void resolveResourceMatchingEncoding() {
    ...
    String file = "bar.css";
    MockServerWebExchange exchange = MockServerWebExchange.from(get(file));
    assertThat(this.chain.resolveResource(exchange, file, this.locations).block(TIMEOUT))
        .isSameAs(resource);
