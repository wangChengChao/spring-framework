...
/**
 * Unit tests for {@link ResponseEntityResultHandler}. When adding a test also consider whether the
...
public class ResponseEntityResultHandlerTests {
  ...
  public void supports() throws Exception {
    ...
    returnType =
        on(TestController.class).resolveReturnType(CompletableFuture.class, entity(String.class));
    assertThat(this.resultHandler.supports(handlerResult(value, returnType))).isTrue();
  ...
  public void httpHeaders() throws Exception {
    ...
    headers.setAllow(
        new LinkedHashSet<>(Arrays.asList(HttpMethod.GET, HttpMethod.POST, HttpMethod.OPTIONS)));
    MethodParameter returnType = on(TestController.class).resolveReturnType(entity(Void.class));
  ...
  public void handleResponseEntityWithNullBody() throws Exception {
    ...
    MethodParameter type =
        on(TestController.class).resolveReturnType(Mono.class, entity(String.class));
  ...
  public void handleReturnValueLastModified() throws Exception {
    ...
    MockServerWebExchange exchange =
        MockServerWebExchange.from(get("/path").ifModifiedSince(timestamp));
  ...
  public void handleReturnValueEtag() throws Exception {
    ...
    MockServerWebExchange exchange =
        MockServerWebExchange.from(get("/path").ifNoneMatch(etagValue));
  ...
  public void handleReturnValueEtagInvalidIfNoneMatch() throws Exception {
    MockServerWebExchange exchange =
        MockServerWebExchange.from(get("/path").ifNoneMatch("unquoted"));
  ...
  public void handleReturnValueETagAndLastModified() throws Exception {
    ...
    MockServerWebExchange exchange =
        MockServerWebExchange.from(
            get("/path").ifNoneMatch(eTag).ifModifiedSince(currentTime.toEpochMilli()));

    ResponseEntity<String> entity =
        ok().eTag(eTag).lastModified(oneMinAgo.toEpochMilli()).body("body");
    MethodParameter returnType = on(TestController.class).resolveReturnType(entity(String.class));
    HandlerResult result = handlerResult(entity, returnType);
    this.resultHandler.handleResult(exchange, result).block(Duration.ofSeconds(5));
  ...
  public void handleReturnValueChangedETagAndLastModified() throws Exception {
    ...
    ResponseEntity<String> entity =
        ok().eTag(newEtag).lastModified(oneMinAgo.toEpochMilli()).body("body");
    MethodParameter returnType = on(TestController.class).resolveReturnType(entity(String.class));
    HandlerResult result = handlerResult(entity, returnType);
    this.resultHandler.handleResult(exchange, result).block(Duration.ofSeconds(5));
  ...
  public void handleMonoWithWildcardBodyType() throws Exception {
    ...
    MethodParameter type =
        on(TestController.class).resolveReturnType(Mono.class, ResponseEntity.class);
    HandlerResult result =
        new HandlerResult(new TestController(), Mono.just(ok().body("body")), type);
  ...
  public void handleMonoWithWildcardBodyTypeAndNullBody() throws Exception {
    ...
    MethodParameter returnType =
        on(TestController.class).resolveReturnType(Mono.class, ResponseEntity.class);
    HandlerResult result =
        new HandlerResult(new TestController(), Mono.just(notFound().build()), returnType);
  ...
  public void handleWithPresetContentTypeShouldFailWithServerError() {
    ...
    MockServerWebExchange exchange = MockServerWebExchange.from(get("/path"));
    ResponseEntityResultHandler resultHandler =
        new ResponseEntityResultHandler(
  ...
  private void assertConditionalResponse(
    ...
    if (lastModified.isAfter(Instant.EPOCH)) {
      assertThat(exchange.getResponse().getHeaders().get(HttpHeaders.LAST_MODIFIED).size())
          .isEqualTo(1);
      assertThat(exchange.getResponse().getHeaders().getLastModified())
