...
/**
 * Integration tests with requests mapped via {@link SimpleUrlHandlerMapping} to plain {@link
 * WebHandler}s.
 *
  ...
  static class WebConfig {
    ...
    public SimpleUrlHandlerMapping handlerMapping() {
      ...
      map.put(
          "/foo",
          (WebHandler)
              exchange -> exchange.getResponse().writeWith(Flux.just(asDataBuffer("foo"))));
      map.put(
          "/bar",
          (WebHandler)
              exchange -> exchange.getResponse().writeWith(Flux.just(asDataBuffer("bar"))));
      map.put(
          "/header",
          (WebHandler)
              exchange -> {
