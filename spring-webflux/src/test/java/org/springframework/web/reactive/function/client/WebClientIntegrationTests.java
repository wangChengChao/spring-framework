...
class WebClientIntegrationTests {
  ...
  void shouldReceiveJsonAsString(ClientHttpConnector connector) {
    ...
    prepareResponse(
        response -> response.setHeader("Content-Type", "application/json").setBody(content));
  ...
  void shouldReceiveJsonAsTypeReferenceString(ClientHttpConnector connector) {
    ...
    Mono<ValueContainer<Foo>> result =
        this.webClient
            .get()
            .uri("/json")
            .accept(MediaType.APPLICATION_JSON)
  ...
  void exchangeShouldReceiveJsonAsResponseEntityList(ClientHttpConnector connector) {
    ...
    Mono<ResponseEntity<List<Pojo>>> result =
        this.webClient
            .get()
            .uri("/json")
            .accept(MediaType.APPLICATION_JSON)
  ...
  void retrieveEntityListWithServerError(ClientHttpConnector connector) {
    ...
    Mono<ResponseEntity<List<String>>> result =
        this.webClient
            .get()
            .uri("/")
            .accept(MediaType.APPLICATION_JSON)
    ...
    expectRequest(
        request -> {
          ...
          assertThat(request.getBody().readUtf8())
              .isEqualTo("{\"foo\":\"foofoo\",\"bar\":\"barbar\"}");
  ...
  void shouldSendCookies(ClientHttpConnector connector) {
    ...
    prepareResponse(response -> response.setHeader("Content-Type", "text/plain").setBody("test"));
  ...
  void shouldReceive404Response(ClientHttpConnector connector) {
    ...
    StepVerifier.create(result)
        .consumeNextWith(
            response -> assertThat(response.statusCode()).isEqualTo(HttpStatus.NOT_FOUND))
        .expectComplete()
        .verify(Duration.ofSeconds(3));
  ...
  void shouldGetErrorSignalOn404(ClientHttpConnector connector) {
    ...
    Mono<String> result =
        this.webClient.get().uri("/greeting?name=Spring").retrieve().bodyToMono(String.class);
  ...
  void shouldGetErrorSignalOnEmptyErrorResponse(ClientHttpConnector connector) {
    ...
    prepareResponse(
        response -> response.setResponseCode(404).setHeader("Content-Type", "text/plain"));

    Mono<String> result = this.webClient.get().uri("/greeting").retrieve().bodyToMono(String.class);
  ...
  void shouldGetInternalServerErrorSignal(ClientHttpConnector connector) {
    ...
    Mono<String> result = this.webClient.get().uri(path).retrieve().bodyToMono(String.class);
  ...
  void shouldGetErrorSignalWhenRetrievingUnknownStatusCode(ClientHttpConnector connector) {
    ...
    Mono<String> result =
        this.webClient.get().uri("/unknownPage").retrieve().bodyToMono(String.class);
  ...
  void shouldApplyCustomStatusHandler(ClientHttpConnector connector) {
    ...
    Mono<String> result =
        this.webClient
            ...
            .retrieve()
            .onStatus(
                HttpStatus::is5xxServerError, response -> Mono.just(new MyException("500 error!")))
            .bodyToMono(String.class);
  ...
  void shouldApplyCustomRawStatusHandler(ClientHttpConnector connector) {
    ...
    Mono<String> result =
        this.webClient
            ...
            .retrieve()
            .onRawStatus(
                value -> value >= 500 && value < 600,
                response -> Mono.just(new MyException("500 error!")))
            .bodyToMono(String.class);
  ...
  void emptyStatusHandlerShouldReturnBody(ClientHttpConnector connector) {
    ...
    StepVerifier.create(result).expectNext("Internal Server error").verifyComplete();
  ...
  void shouldApplyExchangeFilter(ClientHttpConnector connector) {
    ...
    Mono<String> result =
        filteredClient.get().uri("/greeting?name=Spring").retrieve().bodyToMono(String.class);
        ...
        ExchangeFilterFunction.ofResponseProcessor(
            clientResponse -> {
              ...
              return headerValues.isEmpty()
                  ? Mono.error(new MyException("Response does not contain Foo header"))
