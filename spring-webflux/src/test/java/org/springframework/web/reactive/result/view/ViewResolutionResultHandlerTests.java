...
/**
 * ViewResolutionResultHandler relying on a canned {@link TestViewResolver} or a (Mockito) "mock".
 *
...
public class ViewResolutionResultHandlerTests {
  ...
  public void supports() {
    testSupports(
        on(Handler.class).annotPresent(ModelAttribute.class).resolveReturnType(String.class));
    testSupports(
    ...
    testSupports(
        on(Handler.class).annotNotPresent(ModelAttribute.class).resolveReturnType(Map.class));

    testSupports(on(Handler.class).resolveReturnType(TestBean.class));

    testSupports(
        on(Handler.class).annotPresent(ModelAttribute.class).resolveReturnType(Long.class));
    testDoesNotSupport(
        on(Handler.class).annotNotPresent(ModelAttribute.class).resolveReturnType(Long.class));
  ...
  private void testSupports(MethodParameter returnType, boolean supports) {
    ...
    HandlerResult handlerResult =
        new HandlerResult(new Object(), null, returnType, this.bindingContext);
    if (supports) {
      assertThat(resultHandler.supports(handlerResult))
          .as("return type [" + returnType + "] should be supported")
          .isTrue();
    } else {
      assertThat(resultHandler.supports(handlerResult))
          .as("return type [" + returnType + "] should not be supported")
          .isFalse();
    }
  }
  ...
  public void handleReturnValueTypes() {
    ...
    returnValue = Collections.singletonMap("name", "Joe");
    testHandle(
        "/account", returnType, returnValue, "account: {id=123, myMap={name=Joe}}", resolver);
  ...
  public void handleWithMultipleResolvers() {
    testHandle(
        ...
        "profile",
        "profile: {id=123}",
        new TestViewResolver("account"),
        new TestViewResolver("profile"));
  }

  @Test
  public void defaultViewName() {
    testDefaultViewName(
        null, on(Handler.class).annotPresent(ModelAttribute.class).resolveReturnType(String.class));
    testDefaultViewName(
        Mono.empty(), on(Handler.class).resolveReturnType(Mono.class, String.class));
    testDefaultViewName(Mono.empty(), on(Handler.class).resolveReturnType(Mono.class, Void.class));
    testDefaultViewName(
        Completable.complete(), on(Handler.class).resolveReturnType(Completable.class));
  }
  ...
  private void testDefaultViewName(Object returnValue, MethodParameter returnType) {
    ...
    HandlerResult result =
        new HandlerResult(new Object(), returnValue, returnType, this.bindingContext);
  ...
  public void contentNegotiation() {
    ...
    HandlerResult handlerResult =
        new HandlerResult(new Object(), value, returnType, this.bindingContext);

    MockServerWebExchange exchange =
        MockServerWebExchange.from(get("/account").accept(APPLICATION_JSON));
  ...
  private ViewResolutionResultHandler resultHandler(
      List<View> defaultViews, ViewResolver... resolvers) {
    ...
    ViewResolutionResultHandler handler =
        new ViewResolutionResultHandler(resolverList, contentTypeResolver);
    handler.setDefaultViews(defaultViews);
    return handler;
  }

  private ServerWebExchange testHandle(
      String path,
      MethodParameter returnType,
      Object returnValue,
      String responseBody,
  ...
  private void assertResponseBody(MockServerWebExchange exchange, String responseBody) {
    StepVerifier.create(exchange.getResponse().getBody())
        .consumeNextWith(
            buf -> assertThat(DataBufferTestUtils.dumpString(buf, UTF_8)).isEqualTo(responseBody))
        .expectComplete()
        .verify();
  }
  ...
  private static final class TestView implements View {
    ...
    public Mono<Void> render(
        @Nullable Map<String, ?> model, @Nullable MediaType mediaType, ServerWebExchange exchange) {
