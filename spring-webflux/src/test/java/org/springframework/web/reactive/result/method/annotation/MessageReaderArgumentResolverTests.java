...
public class MessageReaderArgumentResolverTests {
  ...
  public void emptyBody() throws Exception {
    ...
    ResolvableType type = forClassWithGenerics(Mono.class, TestBean.class);
    MethodParameter param = this.testMethod.arg(type);
    Mono<TestBean> result =
        (Mono<TestBean>) this.resolver.readBody(param, true, this.bindingContext, exchange).block();
  ...
  public void rxJava2ObservableTestBean() throws Exception {
    ...
    assertThat(observable.toList().blockingGet())
        .isEqualTo(Arrays.asList(new TestBean("f1", "b1"), new TestBean("f2", "b2")));
  }
  ...
  public void flowableTestBean() throws Exception {
    ...
    assertThat(flowable.toList().blockingGet())
        .isEqualTo(Arrays.asList(new TestBean("f1", "b1"), new TestBean("f2", "b2")));
  }
  ...
  public void monoList() throws Exception {
    String body = "[{\"bar\":\"b1\",\"foo\":\"f1\"},{\"bar\":\"b2\",\"foo\":\"f2\"}]";
    ResolvableType type =
        forClassWithGenerics(Mono.class, forClassWithGenerics(List.class, TestBean.class));
    MethodParameter param = this.testMethod.arg(type);
  ...
  public void array() throws Exception {
    ...
    assertThat(value)
        .isEqualTo(new TestBean[] {new TestBean("f1", "b1"), new TestBean("f2", "b2")});
  }
  ...
  public void parameterizedMethodArgument() throws Exception {
    Method method =
        AbstractParameterizedController.class.getMethod("handleDto", Identifiable.class);
  ...
  private <T> T resolveValue(MethodParameter param, String body) {
    MockServerHttpRequest request =
        post("/path").contentType(MediaType.APPLICATION_JSON).body(body);
    ServerWebExchange exchange = MockServerWebExchange.from(request);
    Mono<Object> result = this.resolver.readBody(param, true, this.bindingContext, exchange);
    ...
    assertThat(param.getParameterType().isAssignableFrom(value.getClass()))
        .as("Unexpected return value type: " + value)
        .isTrue();
  ...
  private AbstractMessageReaderArgumentResolver resolver(Decoder<?>... decoders) {
    ...
    Arrays.asList(decoders)
        .forEach(decoder -> readers.add(new DecoderHttpMessageReader<>(decoder)));
    return new AbstractMessageReaderArgumentResolver(readers) {
      ...
      public Mono<Object> resolveArgument(
          MethodParameter p, BindingContext bc, ServerWebExchange e) {
        return null;
      }
    };
  }
  ...
  private static class ConcreteParameterizedController
      extends AbstractParameterizedController<SimpleBean> {}
