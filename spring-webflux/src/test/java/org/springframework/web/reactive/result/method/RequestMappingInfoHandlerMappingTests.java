...
public class RequestMappingInfoHandlerMappingTests {
  ...
  private static final HandlerMethod handlerMethod =
      new HandlerMethod(new TestController(), ClassUtils.getMethod(TestController.class, "dummy"));
  ...
  public void getHandlerGlobMatch() {
    Method expected =
        on(TestController.class).annot(requestMapping("/ba*").method(GET, HEAD)).resolveMethod();
  ...
  public void getHandlerBestMatch() {
    Method expected =
        on(TestController.class).annot(getMapping("/foo").params("p")).resolveMethod();
    ...
    assertError(
        ...
        ex ->
            assertThat(ex.getSupportedMethods())
                .isEqualTo(EnumSet.of(HttpMethod.GET, HttpMethod.HEAD)));
  }

  @Test // SPR-9603
  public void getHandlerRequestMethodMatchFalsePositive() {
    ServerWebExchange exchange =
        MockServerWebExchange.from(get("/users").accept(MediaType.APPLICATION_XML));
  ...
  public void getHandlerProducibleMediaTypesAttribute() {
    ServerWebExchange exchange =
        MockServerWebExchange.from(get("/content").accept(MediaType.APPLICATION_XML));
  ...
  public void handleMatchBestMatchingPatternAttribute() {
    ...
    PathPattern bestMatch =
        (PathPattern) exchange.getAttributes().get(BEST_MATCHING_PATTERN_ATTRIBUTE);
  ...
  public void handleMatchMatrixVariables() {
    ...
    ServerWebExchange exchange =
        MockServerWebExchange.from(get("/cars;colors=red,blue,green;year=2012"));
  ...
  private void testHttpOptions(String requestURI, Set<HttpMethod> allowedMethods) {
    ServerWebExchange exchange =
        MockServerWebExchange.from(MockServerHttpRequest.options(requestURI));
  ...
  private void testMediaTypeNotAcceptable(String url) {
    ServerWebExchange exchange =
        MockServerWebExchange.from(get(url).accept(MediaType.APPLICATION_JSON));
    Mono<Object> mono = this.handlerMapping.getHandler(exchange);
  ...
  @SuppressWarnings("unchecked")
  private MultiValueMap<String, String> getMatrixVariables(
      ServerWebExchange exchange, String uriVarName) {
    return ((Map<String, MultiValueMap<String, String>>)
            exchange.getAttributes().get(HandlerMapping.MATRIX_VARIABLES_ATTRIBUTE))
        .get(uriVarName);
  }
  ...
  private Map<String, String> getUriTemplateVariables(ServerWebExchange exchange) {
    return (Map<String, String>)
        exchange.getAttributes().get(HandlerMapping.URI_TEMPLATE_VARIABLES_ATTRIBUTE);
  }
  ...
  private static class TestRequestMappingInfoHandlerMapping
