...
/**
 * {@link ThemeSource} implementation that looks up an individual {@link java.util.ResourceBundle}
 * per theme. The theme name gets interpreted as ResourceBundle basename, supporting a common
...
public class ResourceBundleThemeSource implements HierarchicalThemeSource, BeanClassLoaderAware {
  ...
  @Nullable private Boolean fallbackToSystemLocale;
   ...
   *
   * <p>{@link ResourceBundleMessageSource}'s default is the {@code java.util.ResourceBundle}
   ...
   * MessageSource whose basename corresponds to the given theme name (prefixed by the configured
   * "basenamePrefix").
   *
   * <p>SimpleTheme instances are cached per theme name. Use a reloadable MessageSource if themes
   * should reflect changes to the underlying files.
   *
   * @see #setBasenamePrefix
  ...
  /**
   * Create a MessageSource for the given basename, to be used as MessageSource for the
   * corresponding theme.
   *
   * <p>Default implementation creates a ResourceBundleMessageSource. for the given basename. A
   * subclass could create a specifically configured ReloadableResourceBundleMessageSource, for
   * example.
   *
  ...
  /**
   * Initialize the MessageSource of the given theme with the one from the corresponding parent of
   * this ThemeSource.
   *
  ...
  protected void initParent(Theme theme) {
    if (theme.getMessageSource() instanceof HierarchicalMessageSource) {
      HierarchicalMessageSource messageSource =
          (HierarchicalMessageSource) theme.getMessageSource();
