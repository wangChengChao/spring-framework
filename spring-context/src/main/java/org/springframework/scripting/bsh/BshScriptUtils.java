...
public abstract class BshScriptUtils {
   ...
   *
   * <p>With this {@code createBshObject} variant, the script needs to declare a full class or
   * return an actual instance of the scripted object.
   *
  ...
  /**
   * Create a new BeanShell-scripted object from the given script source, using the default
   * ClassLoader.
   *
   * <p>The script may either be a simple script that needs a corresponding proxy generated
   * (implementing the specified interfaces), or declare a full class or return an actual instance
   * of the scripted object (in which case the specified interfaces, if any, need to be implemented
   * by that class/instance).
   *
   * @param scriptSource the script source text
   * @param scriptInterfaces the interfaces that the scripted Java object is supposed to implement
   *     (may be {@code null} or empty if the script itself declares a full class or returns an
   *     actual instance of the scripted object)
   * @return the scripted Java object
   * @throws EvalError in case of BeanShell parsing failure
   * @see #createBshObject(String, Class[], ClassLoader)
   */
  public static Object createBshObject(String scriptSource, @Nullable Class<?>... scriptInterfaces)
      throws EvalError {
   ...
   * @throws EvalError in case of BeanShell parsing failure
   */
  public static Object createBshObject(
    ...
    if (result instanceof Class) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "Could not instantiate script class: " + clazz.getName(), ex);
      }
    } else {
      return result;
    }
  ...
  /**
   * Evaluate the specified BeanShell script based on the given script source, returning the Class
   * defined by the script.
   *
   * <p>The script may either declare a full class or return an actual instance of the scripted
   * object (in which case the Class of the object will be returned). In any other case, the
   * returned Class will be {@code null}.
   *
   * @param scriptSource the script source text
   * @param classLoader the ClassLoader to use for evaluating the script
   ...
   */
  @Nullable
  static Class<?> determineBshObjectType(String scriptSource, @Nullable ClassLoader classLoader)
      throws EvalError {
  ...
  /**
   * Evaluate the specified BeanShell script based on the given script source, keeping a returned
   * script Class or script Object as-is.
   *
   * <p>The script may either be a simple script that needs a corresponding proxy generated
   * (implementing the specified interfaces), or declare a full class or return an actual instance
   * of the scripted object (in which case the specified interfaces, if any, need to be implemented
    ...
    if (result != null) {
      ...
      if (ObjectUtils.isEmpty(scriptInterfaces)) {
        throw new IllegalArgumentException(
            "Given script requires a script proxy: "
                + "At least one script interface is required.\nScript: "
                + scriptSource);
      }
      XThis xt = (XThis) interpreter.eval("return this");
      return Proxy.newProxyInstance(
          classLoader, scriptInterfaces, new BshObjectInvocationHandler(xt));
    }
  }
