 ...
 *
 * <p>Typically used in combination with a {@link
 * org.springframework.scripting.support.ScriptFactoryPostProcessor}; see the latter's javadoc for a
...
public class GroovyScriptFactory implements ScriptFactory, BeanFactoryAware, BeanClassLoaderAware {
  ...
  @Nullable private GroovyObjectCustomizer groovyObjectCustomizer;

  @Nullable private CompilerConfiguration compilerConfiguration;

  @Nullable private GroovyClassLoader groovyClassLoader;

  @Nullable private Class<?> scriptClass;

  @Nullable private Class<?> scriptResultClass;

  @Nullable private CachedResultHolder cachedResult;
   ...
   *
   * <p>We don't need to specify script interfaces here, since a Groovy script defines its Java
   * interfaces itself.
   *
   * @param scriptSourceLocator a locator that points to the source of the script. Interpreted by
   *     the post-processor that actually creates the script.
   */
  ...
  /**
   * Create a new GroovyScriptFactory for the given script source, specifying a strategy interface
   * that can create a custom MetaClass to supply missing methods and otherwise change the behavior
   * of the object.
   *
   * @param scriptSourceLocator a locator that points to the source of the script. Interpreted by
   *     the post-processor that actually creates the script.
   * @param groovyObjectCustomizer a customizer that can set a custom metaclass or make other
   *     changes to the GroovyObject created by this factory (may be {@code null})
   * @see GroovyObjectCustomizer#customize
   */
  public GroovyScriptFactory(
      String scriptSourceLocator, @Nullable GroovyObjectCustomizer groovyObjectCustomizer) {
   ...
   * @param scriptSourceLocator a locator that points to the source of the script. Interpreted by
   *     the post-processor that actually creates the script.
   * @param compilerConfiguration a custom compiler configuration to be applied to the
   *     GroovyClassLoader (may be {@code null})
   * @since 4.3.3
   * @see GroovyClassLoader#GroovyClassLoader(ClassLoader, CompilerConfiguration)
   */
  public GroovyScriptFactory(
      String scriptSourceLocator, @Nullable CompilerConfiguration compilerConfiguration) {
    this(scriptSourceLocator);
  ...
  /**
   * Create a new GroovyScriptFactory for the given script source, specifying a strategy interface
   * that can customize Groovy's compilation process within the underlying GroovyClassLoader.
   *
   * @param scriptSourceLocator a locator that points to the source of the script. Interpreted by
   *     the post-processor that actually creates the script.
   * @param compilationCustomizers one or more customizers to be applied to the GroovyClassLoader
   ...
   */
  public GroovyScriptFactory(
      String scriptSourceLocator, CompilationCustomizer... compilationCustomizers) {
    this(scriptSourceLocator);
  ...
  protected GroovyClassLoader buildGroovyClassLoader(@Nullable ClassLoader classLoader) {
    return (this.compilerConfiguration != null
        ? new GroovyClassLoader(classLoader, this.compilerConfiguration)
  ...
  /**
   * Groovy scripts determine their interfaces themselves, hence we don't need to explicitly expose
   * interfaces here.
   *
  ...
  /**
   * Groovy scripts do not need a config interface, since they expose their setters as public
   * methods.
   */
  @Override
        ...
        if (this.scriptClass == null || scriptSource.isModified()) {
          ...
          this.scriptClass =
              getGroovyClassLoader()
                  .parseClass(scriptSource.getScriptAsString(), scriptSource.suggestedClassName());
   ...
   * @return the result object (either an instance of the script class or the result of running the
   *     script instance)
  ...
  protected Object executeScript(ScriptSource scriptSource, Class<?> scriptClass)
    ...
    try {
      GroovyObject goo =
          (GroovyObject) ReflectionUtils.accessibleConstructor(scriptClass).newInstance();
      ...
      throw new ScriptCompilationException(
          "Failed to invoke Groovy script constructor: " + scriptClass.getName(),
