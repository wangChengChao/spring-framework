...
/**
 * {@link org.springframework.scripting.ScriptSource} implementation based on Spring's {@link
 * org.springframework.core.io.Resource} abstraction. Loads the script text from the underlying
 * Resource's {@link org.springframework.core.io.Resource#getFile() File} or {@link
 * org.springframework.core.io.Resource#getInputStream() InputStream}, and tracks the last-modified
 * timestamp of the file (if possible).
 *
 * @author Rob Harrop
...
public class ResourceScriptSource implements ScriptSource {
   ...
   *
   * <p>The default value for regular Resources is "UTF-8". A {@code null} value implies the
   * platform default.
   */
  ...
  protected long retrieveLastModifiedTime() {
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            getResource()
                + " could not be resolved in the file system - "
