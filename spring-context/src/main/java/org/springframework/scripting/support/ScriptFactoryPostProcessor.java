...
/**
 * {@link org.springframework.beans.factory.config.BeanPostProcessor} that handles {@link
 * org.springframework.scripting.ScriptFactory} definitions, replacing each factory with the actual
 * scripted Java object generated by it.
 *
 * <p>This is similar to the {@link org.springframework.beans.factory.FactoryBean} mechanism, but is
 * specifically tailored for scripts and not built into Spring's core container itself but rather
 * implemented as an extension.
 *
 * <p><b>NOTE:</b> The most important characteristic of this post-processor is that constructor
 * arguments are applied to the {@link org.springframework.scripting.ScriptFactory} instance while
 * bean property values are applied to the generated scripted object. Typically, constructor
 * arguments include a script source locator and potentially script interfaces, while bean property
 * values include references and config values to inject into the scripted object itself.
 *
 * <p>The following {@link ScriptFactoryPostProcessor} will automatically be applied to the two
 * {@link org.springframework.scripting.ScriptFactory} definitions below. At runtime, the actual
 * scripted objects will be exposed for "bshMessenger" and "groovyMessenger", rather than the {@link
 * org.springframework.scripting.ScriptFactory} instances. Both of those are supposed to be castable
 * to the example's {@code Messenger} interfaces here.
 *
 * <pre class="code">
 ...
 * &lt;/bean&gt;</pre>
 *
 * <p><b>NOTE:</b> Please note that the above excerpt from a Spring XML bean definition file uses
 * just the &lt;bean/&gt;-style syntax (in an effort to illustrate using the {@link
 * ScriptFactoryPostProcessor} itself). In reality, you would never create a &lt;bean/&gt;
 * definition for a {@link ScriptFactoryPostProcessor} explicitly; rather you would import the tags
 * from the {@code 'lang'} namespace and simply create scripted beans using the tags in that
 * namespace... as part of doing so, a {@link ScriptFactoryPostProcessor} will implicitly be created
 * for you.
 *
 * <p>The Spring reference documentation contains numerous examples of using tags in the {@code
 * 'lang'} namespace; by way of an example, find below a Groovy-backed bean defined using the {@code
 * 'lang:groovy'} tag.
 *
 * <pre class="code">
...
public class ScriptFactoryPostProcessor extends InstantiationAwareBeanPostProcessorAdapter
  ...
  /**
   * The {@link org.springframework.core.io.Resource}-style prefix that denotes an inline script.
   *
  ...
  public static final String LANGUAGE_ATTRIBUTE =
      Conventions.getQualifiedAttributeName(ScriptFactoryPostProcessor.class, "language");
   ...
   * checks at all.
   *
   * <p>Note that an actual refresh will only happen when the {@link
   * org.springframework.scripting.ScriptSource} indicates that it has been modified.
   *
  ...
  /**
   * Flag to signal that refreshable proxies should be created to proxy the target class not its
   * interfaces.
   *
  ...
  public void setBeanFactory(BeanFactory beanFactory) {
    if (!(beanFactory instanceof ConfigurableBeanFactory)) {
      throw new IllegalStateException(
          "ScriptFactoryPostProcessor doesn't work with "
              + "non-ConfigurableBeanFactory: "
  ...
  public Class<?> predictBeanType(Class<?> beanClass, String beanName) {
    ...
    try {
      ...
      if (ex instanceof BeanCreationException
          && ((BeanCreationException) ex).getMostSpecificCause()
  ...
  /**
   * Prepare the script beans in the internal BeanFactory that this post-processor uses. Each
   * original bean definition will be split into a ScriptFactory definition and a scripted object
   * definition.
   *
  ...
  /**
   * Get the refresh check delay for the given {@link ScriptFactory} {@link BeanDefinition}. If the
   * {@link BeanDefinition} has a {@link org.springframework.core.AttributeAccessor metadata
   * attribute} under the key {@link #REFRESH_CHECK_DELAY_ATTRIBUTE} which is a valid {@link Number}
   * type, then this value is used. Otherwise, the {@link #defaultRefreshCheckDelay} value is used.
   *
  ...
  protected boolean resolveProxyTargetClass(BeanDefinition beanDefinition) {
    ...
    if (attributeValue instanceof Boolean) {
      ...
      throw new BeanDefinitionStoreException(
          "Invalid proxy target class attribute ["
  ...
  /**
   * Create a ScriptFactory bean definition based on the given script definition, extracting only
   * the definition data that is relevant for the ScriptFactory (that is, only bean class and
   ...
   *
   * <p>By default, supported locators are Spring resource locations (such as "file:C:/myScript.bsh"
   * or "classpath:myPackage/myScript.bsh") and inline scripts ("inline:myScriptText...").
   *
   * @param beanName the name of the scripted bean
  ...
  /**
   * Create a config interface for the given bean definition, defining setter methods for the
   * defined property values as well as an init method and a destroy method (if defined).
   *
   * <p>This implementation creates the interface via CGLIB's InterfaceMaker, determining the
   * property types from the given interfaces (as far as possible).
   *
   * @param bd the bean definition (property values etc) to create a config interface for
   * @param interfaces the interfaces to check against (might define getters corresponding to the
   *     setters we're supposed to generate)
  ...
  protected Class<?> createConfigInterface(BeanDefinition bd, @Nullable Class<?>[] interfaces) {
    ...
    for (PropertyValue pv : pvs) {
      ...
      Signature signature =
          new Signature(setterName, Type.VOID_TYPE, new Type[] {Type.getType(propertyType)});
  ...
  /**
   * Create a bean definition for the scripted object, based on the given script definition,
   * extracting the definition data that is relevant for the scripted object (that is, everything
   * but bean class and constructor arguments).
   *
   * @param bd the full script bean definition
   * @param scriptFactoryBeanName the name of the internal ScriptFactory bean
   ...
   * @param interfaces the proxy interfaces (may be {@code null} to indicate proxying of all
   *     interfaces implemented by the target class)
   * @return the generated proxy
   * @see RefreshableScriptTargetSource
   */
  protected Object createRefreshableProxy(
      TargetSource ts, @Nullable Class<?>[] interfaces, boolean proxyTargetClass) {
    ...
    if (interfaces != null) {
      ...
      if (targetClass != null) {
        proxyFactory.setInterfaces(
            ClassUtils.getAllInterfacesForClass(targetClass, this.beanClassLoader));
      }
    }
