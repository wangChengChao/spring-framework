...
/**
 * {@code javax.script} (JSR-223) based implementation of Spring's {@link ScriptEvaluator} strategy
 * interface.
 *
...
public class StandardScriptEvaluator implements ScriptEvaluator, BeanClassLoaderAware {
  ...
  @Nullable private volatile Bindings globalBindings;
  ...
  /**
   * Construct a new {@code StandardScriptEvaluator} for the given JSR-223 {@link
   ...
   *
   * <p>This is effectively an alias for {@link #setEngineName "engineName"}, potentially (but not
   * yet) providing common abbreviations for certain languages beyond what the JSR-223 script engine
   * factory exposes.
   *
   * @see #setEngineName
   */
  ...
  /**
   * Set the name of the script engine for evaluating the scripts (e.g. "Groovy"), as exposed by the
  ...
  /**
   * Set the globally scoped bindings on the underlying script engine manager, shared by all
   * scripts, as an alternative to script argument bindings.
   *
   * @since 4.2.2
  ...
  protected ScriptEngine getScriptEngine(ScriptSource script) {
    ...
    if (StringUtils.hasText(this.engineName)) {
      ...
      if (engine == null) {
        throw new IllegalStateException(
            "No matching engine found for file extension '" + extension + "'");
      }
      return engine;
    } else {
      throw new IllegalStateException(
