...
/**
 * {@link org.springframework.scripting.ScriptFactory} implementation based on the JSR-223 script
 * engine abstraction (as included in Java 6+). Supports JavaScript, Groovy, JRuby, and other
 * JSR-223 compliant engines.
 *
...
public class StandardScriptFactory implements ScriptFactory, BeanClassLoaderAware {
  ...
  @Nullable private final String scriptEngineName;

  private final String scriptSourceLocator;

  @Nullable private final Class<?>[] scriptInterfaces;

  @Nullable private ClassLoader beanClassLoader = ClassUtils.getDefaultClassLoader();

  @Nullable private volatile ScriptEngine scriptEngine;
   ...
   * @param scriptSourceLocator a locator that points to the source of the script. Interpreted by
   *     the post-processor that actually creates the script.
   * @param scriptInterfaces the Java interfaces that the scripted object is supposed to implement
   */
   ...
   * Create a new StandardScriptFactory for the given script source.
   *
   * @param scriptEngineName the name of the JSR-223 ScriptEngine to use (explicitly given instead
   *     of inferred from the script source)
   * @param scriptSourceLocator a locator that points to the source of the script. Interpreted by
   *     the post-processor that actually creates the script.
   */
  ...
  public StandardScriptFactory(
      ...
      @Nullable Class<?>... scriptInterfaces) {
  ...
  public Object getScriptedObject(ScriptSource scriptSource, @Nullable Class<?>... actualInterfaces)
    ...
    if (!ObjectUtils.isEmpty(actualInterfaces)) {
      ...
      for (Class<?> requestedIfc : actualInterfaces) {
        if (script instanceof Class
            ? !requestedIfc.isAssignableFrom((Class<?>) script)
    ...
    if (script instanceof Class) {
      ...
      try {
        ...
        throw new ScriptCompilationException(
            "Failed to invoke script constructor: " + scriptClass.getName(),
  ...
  protected Object adaptToInterfaces(
    ...
    if (adaptedIfc != null) {
      ...
      if (script == null) {
        ...
        if (script == null) {
          throw new ScriptCompilationException(
              scriptSource, "Could not adapt script to interface [" + adaptedIfc.getName() + "]");
        }
      }
    }
