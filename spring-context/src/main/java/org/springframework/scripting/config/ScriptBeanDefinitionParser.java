...
/**
 * BeanDefinitionParser implementation for the '{@code <lang:groovy/>}', '{@code <lang:std/>}' and
 * '{@code <lang:bsh/>}' tags. Allows for objects written using dynamic languages to be easily
 * exposed with the {@link org.springframework.beans.factory.BeanFactory}.
 *
 * <p>The script for each object can be specified either as a reference to the resource containing
 * it (using the '{@code script-source}' attribute) or inline in the XML configuration itself (using
 * the '{@code inline-script}' attribute.
 *
 * <p>By default, dynamic objects created with these tags are <strong>not</strong> refreshable. To
 * enable refreshing, specify the refresh check delay for each object (in milliseconds) using the
...
class ScriptBeanDefinitionParser extends AbstractBeanDefinitionParser {
  ...
  /**
   * The {@link org.springframework.scripting.ScriptFactory} class that this parser instance will
   * create bean definitions for.
   */
  private final String scriptFactoryClassName;

  /**
   * Create a new instance of this parser, creating bean definitions for the supplied {@link
  ...
  /**
   * Parses the dynamic object element and returns the resulting bean definition. Registers a {@link
   * ScriptFactoryPostProcessor} if needed.
   */
  @Override
  ...
  protected AbstractBeanDefinition parseInternal(Element element, ParserContext parserContext) {
    ...
    if (StringUtils.hasText(refreshCheckDelay)) {
      bd.setAttribute(
          ...
          Long.valueOf(refreshCheckDelay));
    }
    ...
    if (StringUtils.hasText(proxyTargetClass)) {
      bd.setAttribute(
          ScriptFactoryPostProcessor.PROXY_TARGET_CLASS_ATTRIBUTE,
    ...
    if (element.hasAttribute(SCRIPT_INTERFACES_ATTRIBUTE)) {
      cav.addIndexedArgumentValue(
          constructorArgNum++,
          element.getAttribute(SCRIPT_INTERFACES_ATTRIBUTE),
          "java.lang.Class[]");
    }
    ...
    if (element.hasAttribute(CUSTOMIZER_REF_ATTRIBUTE)) {
      ...
      if (!StringUtils.hasText(customizerBeanName)) {
        parserContext
            .getReaderContext()
            .error("Attribute 'customizer-ref' has empty value", element);
      } else {
        cav.addIndexedArgumentValue(
            constructorArgNum++, new RuntimeBeanReference(customizerBeanName));
      }
    }
  ...
  /**
   * Resolves the script source from either the '{@code script-source}' attribute or the '{@code
   * inline-script}' element. Logs and {@link XmlReaderContext#error} and returns {@code null} if
   * neither or both of these values are specified.
   */
  @Nullable
  private String resolveScriptSource(Element element, XmlReaderContext readerContext) {
    ...
    if (hasScriptSource && !elements.isEmpty()) {
      readerContext.error(
          "Only one of 'script-source' and 'inline-script' should be specified.", element);
      return null;
