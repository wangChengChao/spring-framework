...
/**
 * {@link org.springframework.beans.factory.FactoryBean} that looks up a JNDI object. Exposes the
 * object found in JNDI for bean references, e.g. for data access object's "dataSource" property in
 * case of a {@link javax.sql.DataSource}.
 *
 * <p>The typical usage will be to register this as singleton factory (e.g. for a certain JNDI-bound
 * DataSource) in an application context, and give bean references to application services that need
 * it.
 *
 * <p>The default behavior is to look up the JNDI object on startup and cache it. This can be
 * customized through the "lookupOnStartup" and "cache" properties, using a {@link
 * JndiObjectTargetSource} underneath. Note that you need to specify a "proxyInterface" in such a
 * scenario, since the actual JNDI object type is not known in advance.
 *
 * <p>Of course, bean classes in a Spring environment may lookup e.g. a DataSource from JNDI
 * themselves. This class simply enables central configuration of the JNDI name, and easy switching
 * to non-JNDI alternatives. The latter is particularly convenient for test setups, reuse in
 * standalone clients, etc.
 *
 * <p>Note that switching to e.g. DriverManagerDataSource is just a matter of configuration: Simply
 * replace the definition of this FactoryBean with a {@link
...
public class JndiObjectFactoryBean extends JndiObjectLocator
  ...
  @Nullable private Object jndiObject;

  /**
   * Specify the proxy interface to use for the JNDI object.
   *
   * <p>Typically used in conjunction with "lookupOnStartup"=false and/or "cache"=false. Needs to be
   * specified because the actual JNDI object type is not known in advance in case of a lazy lookup.
   *
   ...
   *
   * <p>Typically used in conjunction with "lookupOnStartup"=false and/or "cache"=false. Note that
   * proxy interfaces will be autodetected from a specified "expectedType", if necessary.
   *
   * @see #setExpectedType
   * @see #setLookupOnStartup
   * @see #setCache
   ...
   *
   * <p>Can be turned off to allow for late availability of the JNDI object. In this case, the JNDI
   * object will be fetched on first access.
   *
   ...
   *
   * <p>Can be turned off to allow for hot redeployment of JNDI objects. In this case, the JNDI
   * object will be fetched for each invocation.
   *
   ...
   * each method invocation, as needed by WebLogic for JNDI-obtained factories (e.g. JDBC
   ...
   *
   * <p>This can be an arbitrary bean reference or literal value. It is typically used for literal
   * values in scenarios where the JNDI environment might define specific config settings but those
   * are not required to be present.
   *
   * <p>Note: This is only supported for lookup on startup. If specified together with {@link
   * #setExpectedType}, the specified value needs to be either of that type or convertible to it.
   *
   * @see #setLookupOnStartup
  ...
  public void afterPropertiesSet() throws IllegalArgumentException, NamingException {
    ...
    if (this.proxyInterfaces != null
        || !this.lookupOnStartup
        || !this.cache
        || this.exposeAccessContext) {
      ...
      if (this.defaultObject != null
        ...
        TypeConverter converter =
            (this.beanFactory != null
                ? this.beanFactory.getTypeConverter()
        ...
        try {
          ...
          throw new IllegalArgumentException(
              "Default object ["
                  + this.defaultObject
                  + "] of type ["
                  + this.defaultObject.getClass().getName()
                  + "] is not of expected type ["
                  + getExpectedType().getName()
                  + "] and cannot be converted either",
              ex);
        }
      }
  ...
  protected Object lookupWithFallback() throws NamingException {
    ClassLoader originalClassLoader =
        ClassUtils.overrideThreadContextClassLoader(this.beanClassLoader);
    try {
      return lookup();
  ...
  /**
   * Interceptor that exposes the JNDI context for all method invocations, according to
   * JndiObjectFactoryBean's "exposeAccessContext" flag.
   */
