...
/**
 * Convenient superclass for classes that can locate any number of JNDI objects. Derives from
 * JndiAccessor to inherit the "jndiTemplate" and "jndiEnvironment" bean properties.
 *
 * <p>JNDI names may or may not include the "java:comp/env/" prefix expected by Java EE applications
 * when accessing a locally mapped (ENC - Environmental Naming Context) resource. If it doesn't, the
 * "java:comp/env/" prefix will be prepended if the "resourceRef" property is true (the default is
...
public abstract class JndiLocatorSupport extends JndiAccessor {
   ...
   * to be added if the JNDI name doesn't already contain it. Default is "false".
   *
   ...
   *
   * <p>If the name doesn't begin with "java:comp/env/", this prefix is added if "resourceRef" is
   * set to "true".
   *
      ...
      if (!convertedName.equals(jndiName)) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Converted JNDI name ["
                  + convertedName
                  + "] not found - trying original name ["
                  + jndiName
                  + "]. "
                  + ex);
        }
   ...
   *
   * <p>The default implementation applies the "java:comp/env/" prefix if "resourceRef" is "true"
   * and no other scheme (e.g. "java:") is given.
   *
