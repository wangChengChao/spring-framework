...
/**
 * ClassLoader decorator that shadows an enclosing ClassLoader, applying registered transformers to
...
public class ShadowingClassLoader extends DecoratingClassLoader {
  ...
  /**
   * Create a new ShadowingClassLoader, decorating the given ClassLoader, applying {@link
  ...
  /**
   * Add the given ClassFileTransformer to the list of transformers that this ClassLoader will
   * apply.
   *
  ...
  /**
   * Copy all ClassFileTransformers from the given ClassLoader to the list of transformers that this
  ...
  private boolean shouldShadow(String className) {
    return (!className.equals(getClass().getName())
        && !className.endsWith("ShadowingClassLoader")
