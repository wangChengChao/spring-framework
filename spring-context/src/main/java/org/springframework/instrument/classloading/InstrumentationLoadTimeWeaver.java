 ...
 *
 * <p>Start the JVM specifying the Java agent to be used &mdash; for example, as follows where
 * <code>spring-instrument-{version}.jar</code> is a JAR file containing the {@link
 * InstrumentationSavingAgent} class shipped with Spring and where <code>{version}</code> is the
 * release version of the Spring Framework (e.g., {@code 5.1.5.RELEASE}).
 *
 * <p><code>-javaagent:path/to/spring-instrument-{version}.jar</code>
 *
 * <p>In Eclipse, for example, add something similar to the following to the JVM arguments for the
...
public class InstrumentationLoadTimeWeaver implements LoadTimeWeaver {
  ...
  /**
   * We have the ability to weave the current class loader when starting the JVM in this way, so the
   * instrumentable class loader will always be the current loader.
   */
  @Override
  ...
  private static class FilteringClassFileTransformer implements ClassFileTransformer {
    ...
    @Nullable private final ClassLoader targetClassLoader;
