...
/**
 * Adapter that implements WebSphere 7.0 ClassPreProcessPlugin interface, delegating to a standard
 * JDK {@link ClassFileTransformer} underneath.
 *
 * <p>To avoid compile time checks again the vendor API, a dynamic proxy is being used.
 *
 * @author Costin Leau
 * @since 3.1
 */
class WebSphereClassPreDefinePlugin implements InvocationHandler {
  ...
  public WebSphereClassPreDefinePlugin(ClassFileTransformer transformer) {
    ...
    try {
      ...
      byte[] bytes =
          FileCopyUtils.copyToByteArray(classLoader.getResourceAsStream(dummyClass + ".class"));
  ...
  public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
    ...
    if ("equals".equals(name)) {
      ...
      return transform(
          (String) args[0], (byte[]) args[1], (CodeSource) args[2], (ClassLoader) args[3]);
    } else {
  ...
  protected byte[] transform(
      String className, byte[] classfileBuffer, CodeSource codeSource, ClassLoader classLoader)
      throws Exception {

    // NB: WebSphere passes className as "." without class while the transformer expects a VM "/"
    // format
    byte[] result =
        this.transformer.transform(
            classLoader, className.replace('.', '/'), null, null, classfileBuffer);
