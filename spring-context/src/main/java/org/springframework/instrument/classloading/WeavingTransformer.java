...
/**
 * ClassFileTransformer-based weaver, allowing for a list of transformers to be applied on a class
 * byte array. Normally used inside class loaders.
 *
 * <p>Note: This class is deliberately implemented for minimal external dependencies, since it is
 * included in weaver jars (to be deployed into application servers).
 *
 * @author Rod Johnson
 * @author Costin Leau
...
public class WeavingTransformer {
  ...
  /**
   * Apply transformation on a given class byte definition. The method will always return a non-null
   * byte array (if no transformation has taken place the array content will be identical to the
   * original one).
   *
   * @param className the full qualified name of the class in dot format (i.e.
   ...
   * @return (possibly transformed) class byte definition
   */
  public byte[] transformIfNecessary(
      String className, String internalName, byte[] bytes, @Nullable ProtectionDomain pd) {
    byte[] result = bytes;
