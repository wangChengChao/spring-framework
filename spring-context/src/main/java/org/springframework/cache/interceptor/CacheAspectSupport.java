...
/**
 * Base class for caching aspects, such as the {@link CacheInterceptor} or an AspectJ aspect.
 *
 * <p>This enables the underlying Spring caching infrastructure to be used easily to implement an
 * aspect for any aspect system.
 *
 * <p>Subclasses are responsible for calling relevant methods in the correct order.
 *
 * <p>Uses the <b>Strategy</b> design pattern. A {@link CacheOperationSource} is used for
 * determining caching operations, a {@link KeyGenerator} will build the cache keys, and a {@link
 * CacheResolver} will resolve the actual cache(s) to use.
 *
 * <p>Note: A cache aspect is serializable but does not perform any actual caching after
 * deserialization.
 *
 * @author Costin Leau
...
public abstract class CacheAspectSupport extends AbstractCacheInvoker
  ...
  private SingletonSupplier<KeyGenerator> keyGenerator =
      SingletonSupplier.of(SimpleKeyGenerator::new);

  @Nullable private SingletonSupplier<CacheResolver> cacheResolver;
   ...
   * than one source is provided, they will be aggregated using a {@link
  ...
  /**
   * Set the default {@link KeyGenerator} that this cache aspect should delegate to if no specific
   * key generator has been set for the operation.
   *
  ...
  /**
   * Set the default {@link CacheResolver} that this cache aspect should delegate to if no specific
   * cache resolver has been set for the operation.
   *
   * <p>The default resolver resolves the caches against their names and the default cache manager.
   *
   * @see #setCacheManager
  ...
  /**
   * Set the {@link CacheManager} to use to create a default {@link CacheResolver}. Replace the
   * current {@link CacheResolver}, if any.
   *
  ...
  /**
   * Set the containing {@link BeanFactory} for {@link CacheManager} and other service lookups.
   *
   * @since 4.3
   */
  @Override
  ...
  public void afterSingletonsInstantiated() {
    if (getCacheResolver() == null) {
      ...
      Assert.state(
          this.beanFactory != null, "CacheResolver or BeanFactory must be set on cache aspect");
      try {
   ...
   *
   * <p>Resolve the {@link CacheResolver} and the {@link KeyGenerator} to be used for the operation.
   *
  ...
  protected CacheOperationMetadata getCacheOperationMetadata(
    ...
    if (metadata == null) {
      ...
      metadata =
          new CacheOperationMetadata(
              operation, method, targetClass, operationKeyGenerator, operationCacheResolver);
  ...
  /**
   * Return a bean with the specified name and type. Used to resolve services that are referenced by
   * name in a {@link CacheOperation}.
   *
  ...
  protected <T> T getBean(String beanName, Class<T> expectedType) {
    if (this.beanFactory == null) {
      throw new IllegalStateException(
          "BeanFactory must be set on cache aspect for "
  ...
  protected Object execute(
      CacheOperationInvoker invoker, Object target, Method method, Object[] args) {
  ...
  /**
   * Execute the underlying operation (typically in case of cache miss) and return the result of the
   * invocation. If an exception occurs it will be wrapped in a {@link
   * CacheOperationInvoker.ThrowableWrapper}: the exception can be handled or modified but it
   * <em>must</em> be wrapped in a {@link CacheOperationInvoker.ThrowableWrapper} as well.
   *
  ...
  private Object execute(
      final CacheOperationInvoker invoker, Method method, CacheOperationContexts contexts) {
      ...
      if (isConditionPassing(context, CacheOperationExpressionEvaluator.NO_RESULT)) {
        ...
        try {
          return wrapCacheValue(
              method, cache.get(key, () -> unwrapReturnValue(invokeOperation(invoker))));
  ...
  private void processCacheEvicts(
      Collection<CacheOperationContext> contexts,
  ...
  private void logInvalidating(
      CacheOperationContext context, CacheEvictOperation operation, @Nullable Object key) {
    if (logger.isTraceEnabled()) {
      logger.trace(
          "Invalidating "
              + (key != null ? "cache key [" + key + "]" : "entire cache")
              + " for operation "
              + operation
              + " on method "
   ...
   * @return a {@link Cache.ValueWrapper} holding the cached item, or {@code null} if none is found
   */
  @Nullable
   ...
   */
  private void collectPutRequests(
      Collection<CacheOperationContext> contexts,
      @Nullable Object result,
  ...
  private class CacheOperationContexts {
    ...
    public CacheOperationContexts(
        Collection<? extends CacheOperation> operations,
        Method method,
        Object[] args,
        Object target,
        Class<?> targetClass) {
      ...
      for (CacheOperation op : operations) {
        this.contexts.add(
            op.getClass(), getOperationContext(op, method, args, target, targetClass));
      }
    ...
    private boolean determineSyncFlag(Method method) {
      List<CacheOperationContext> cacheOperationContexts =
          this.contexts.get(CacheableOperation.class);
  ...
  /**
   * Metadata of a cache operation that does not depend on a particular invocation which makes it a
  ...
  protected class CacheOperationContext implements CacheOperationInvocationContext<CacheOperation> {
    ...
    @Nullable private Boolean conditionPassing;
    ...
    protected boolean isConditionPassing(@Nullable Object result) {
      if (this.conditionPassing == null) {
        if (StringUtils.hasText(this.metadata.operation.getCondition())) {
          ...
          this.conditionPassing =
              evaluator.condition(
                  this.metadata.operation.getCondition(),
                  this.metadata.methodKey,
                  evaluationContext);
        } else {
    ...
    protected Object generateKey(@Nullable Object result) {
      if (StringUtils.hasText(this.metadata.operation.getKey())) {
        ...
        return evaluator.key(
            this.metadata.operation.getKey(), this.metadata.methodKey, evaluationContext);
      }
    ...
    private EvaluationContext createEvaluationContext(@Nullable Object result) {
      return evaluator.createEvaluationContext(
          ...
          this.args,
          this.target,
          this.metadata.targetClass,
          this.metadata.targetMethod,
          result,
          beanFactory);
    }
  ...
  private static final class CacheOperationCacheKey implements Comparable<CacheOperationCacheKey> {
    ...
    private CacheOperationCacheKey(
        CacheOperation cacheOperation, Method method, Class<?> targetClass) {
