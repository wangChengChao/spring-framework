...
/**
 * {@link CacheManager} implementation that lazily builds {@link ConcurrentMapCache} instances for
 * each {@link #getCache} request. Also supports a 'static' mode where the set of cache names is
 * pre-defined through {@link #setCacheNames}, with no dynamic creation of further cache regions at
 * runtime.
 *
 * <p>Note: This is by no means a sophisticated CacheManager; it comes with no cache configuration
 * options. However, it may be useful for testing or simple caching scenarios. For advanced local
 * caching needs, consider {@link org.springframework.cache.jcache.JCacheCacheManager}, {@link
...
public class ConcurrentMapCacheManager implements CacheManager, BeanClassLoaderAware {
  ...
  @Nullable private SerializationDelegate serialization;
  ...
  /**
   * Specify whether this cache manager stores a copy of each entry ({@code true} or the reference
   * ({@code false} for all of its caches.
   *
   * <p>Default is "false" so that the value itself is stored and no serializable contract is
   * required on cached values.
   *
   * <p>Note: A change of the store-by-value setting will reset all existing caches, if any, to
   * reconfigure them with the new store-by-value requirement.
   *
   * @since 4.3
   */
  ...
  /**
   * Return whether this cache manager stores a copy of each entry or a reference for all its
   * caches. If store by value is enabled, any cache entry must be serializable.
   *
   * @since 4.3
   */
  ...
  protected Cache createConcurrentMapCache(String name) {
    ...
    return new ConcurrentMapCache(
        name, new ConcurrentHashMap<>(256), isAllowNullValues(), actualSerialization);
  }
}
