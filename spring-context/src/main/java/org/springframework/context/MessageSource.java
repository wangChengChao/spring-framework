 ...
 *
 * <ul>
 *   <li>{@link org.springframework.context.support.ResourceBundleMessageSource}: built on top of
 *       the standard {@link java.util.ResourceBundle}, sharing its limitations.
 *   <li>{@link org.springframework.context.support.ReloadableResourceBundleMessageSource}: highly
 *       configurable, in particular with respect to reloading message definitions.
 * </ul>
 *
 * @author Rod Johnson
...
public interface MessageSource {
  ...
  /**
   * Try to resolve the message using all the attributes contained within the {@code
   * MessageSourceResolvable} argument that was passed in.
   *
   * <p>NOTE: We must throw a {@code NoSuchMessageException} on this method since at the time of
   * calling this method we aren't able to determine if the {@code defaultMessage} property of the
   * resolvable is {@code null} or not.
   *
   * @param resolvable the value object storing attributes required to resolve a message (may
   *     include a default message)
   * @param locale the locale in which to do the lookup
   * @return the resolved message (never {@code null} since even a {@code
   *     MessageSourceResolvable}-provided default message needs to be non-null)
   * @throws NoSuchMessageException if no corresponding message was found (and no default message
