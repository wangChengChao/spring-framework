 ...
 *
 * <p>Typically registered for the default bean name "{@code loadTimeWeaver}"; the most convenient
 * way to achieve this is Spring's {@code <context:load-time-weaver>} XML tag or
 * {@code @EnableLoadTimeWeaving} on a {@code @Configuration} class.
 *
 * <p>This class implements a runtime environment check for obtaining the appropriate weaver
 * implementation. As of Spring Framework 5.0, it detects Oracle WebLogic 10+, GlassFish 4+, Tomcat
 * 8+, WildFly 8+, IBM WebSphere 8.5+, {@link InstrumentationSavingAgent Spring's VM agent}, and any
 * {@link ClassLoader} supported by Spring's {@link ReflectiveLoadTimeWeaver} (such as Liberty's).
 *
...
public class DefaultContextLoadTimeWeaver
  ...
  public void setBeanClassLoader(ClassLoader classLoader) {
    ...
    if (serverSpecificLoadTimeWeaver != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Determined server-specific load-time weaver: "
      ...
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Using reflective load-time weaver for class loader: "
  ...
  public void destroy() {
    if (this.loadTimeWeaver instanceof InstrumentationLoadTimeWeaver) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Removing all registered transformers for class loader: "
                + this.loadTimeWeaver.getInstrumentableClassLoader().getClass().getName());
      }
