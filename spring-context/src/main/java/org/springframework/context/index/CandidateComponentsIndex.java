 ...
 *
 * <p>An arbitrary number of stereotypes can be registered (and queried) on the index: a typical
 * example is the fully qualified name of an annotation that flags the class for a certain use case.
 * The following call returns all the {@code @Component} <b>candidate</b> types for the {@code
 * com.example} package (and its sub-packages):
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * <p>The {@code type} is usually the fully qualified name of a class, though this is not a rule.
 * Similarly, the {@code stereotype} is usually the fully qualified name of a target type but it can
 * be any marker really.
 *
...
public class CandidateComponentsIndex {
   ...
   * @return the candidate types associated with the specified {@code stereotype} or an empty set if
   *     none has been found for the specified {@code basePackage}
   */
