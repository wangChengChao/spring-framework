...
public interface ApplicationContext
   ...
   *
   * <p>This is not typically used by application code, except for the purpose of initializing bean
   * instances that live outside of the application context, applying the Spring bean lifecycle
   * (fully or partly) to them.
   *
   * <p>Alternatively, the internal BeanFactory exposed by the {@link
   * ConfigurableApplicationContext} interface offers access to the {@link
   ...
   *
   * <p><b>NOTE: As of 4.2, this method will consistently throw IllegalStateException after the
   * application context has been closed.</b> In current Spring Framework versions, only refreshable
   * application contexts behave that way; as of 4.2, all application context implementations will
   * be required to comply.
   *
   * @return the AutowireCapableBeanFactory for this context
   * @throws IllegalStateException if the context does not support the {@link
   *     AutowireCapableBeanFactory} interface, or does not hold an autowire-capable bean factory
   *     yet (e.g. if {@code refresh()} has never been called), or if the context has been closed
   *     already
