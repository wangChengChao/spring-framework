...
/**
 * {@link GenericApplicationListener} adapter that delegates the processing of an event to an {@link
 * EventListener} annotated method.
 *
 * <p>Delegates to {@link #processEvent(ApplicationEvent)} to give subclasses a chance to deviate
 * from the default. Unwraps the content of a {@link PayloadApplicationEvent} if necessary to allow
 * a method declaration to define any arbitrary event type. If a condition is defined, it is
...
public class ApplicationListenerMethodAdapter implements GenericApplicationListener {
  ...
  @Nullable private EventExpressionEvaluator evaluator;
  ...
  public ApplicationListenerMethodAdapter(String beanName, Class<?> targetClass, Method method) {
    ...
    EventListener ann =
        AnnotatedElementUtils.findMergedAnnotation(this.targetMethod, EventListener.class);
  ...
  private static List<ResolvableType> resolveDeclaredEventTypes(
      Method method, @Nullable EventListener ann) {
  ...
  /**
   * Process the specified {@link ApplicationEvent}, checking if the condition matches and handling
   * a non-null result, if any.
   */
  ...
  protected Object doInvoke(Object... args) {
    ...
    try {
      ...
      if (targetException instanceof RuntimeException) {
        ...
        String msg =
            getInvocationErrorMessage(bean, "Failed to invoke event listener method", args);
   ...
   *
   * <p>Matches the {@code condition} attribute of the {@link EventListener} annotation or any
   * matching attribute on a composed annotation that is meta-annotated with {@code @EventListener}.
   */
  @Nullable
  ...
  /**
   * Assert that the target bean class is an instance of the class where the given method is
   * declared. In some cases the actual bean instance at event- processing time may be a JDK dynamic
   * proxy (lazy initialization, prototype beans, and others). Event listener beans that require
   * proxying should prefer class-based proxy mechanisms.
   */
  ...
  private class ReactiveResultHandler {
    ...
    public boolean subscribeToPublisher(Object result) {
      ReactiveAdapter adapter =
          ReactiveAdapterRegistry.getSharedInstance().getAdapter(result.getClass());
      if (adapter != null) {
