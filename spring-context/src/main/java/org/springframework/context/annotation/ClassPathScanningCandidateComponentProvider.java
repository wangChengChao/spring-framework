...
/**
 * A component provider that provides candidate components from a base package. Can use {@link
 * CandidateComponentsIndex the index} if it is available of scans the classpath otherwise.
 * Candidate components are identified by applying exclude and include filters. {@link
 * AnnotationTypeFilter}, {@link AssignableTypeFilter} include filters on an annotation/superclass
 * that are annotated with {@link Indexed} are supported: if any other include filter is specified,
 * the index is ignored and classpath scanning is used instead.
 *
 * <p>This implementation is based on Spring's {@link
 * org.springframework.core.type.classreading.MetadataReader MetadataReader} facility, backed by an
 * ASM {@link org.springframework.asm.ClassReader ClassReader}.
 *
 * @author Mark Fisher
...
public class ClassPathScanningCandidateComponentProvider
  ...
  @Nullable private ConditionEvaluator conditionEvaluator;

  @Nullable private ResourcePatternResolver resourcePatternResolver;

  @Nullable private MetadataReaderFactory metadataReaderFactory;
  ...
  /**
   * Set the resource pattern to use when scanning the classpath. This value will be appended to
  ...
  /**
   * Set the Environment to use when resolving placeholders and evaluating {@link
   ...
   *
   * <p>Default is a {@link CachingMetadataReaderFactory} for the specified {@linkplain
   * #setResourceLoader resource loader}.
   *
   * <p>Call this setter method <i>after</i> {@link #setResourceLoader} in order for the given
   * MetadataReaderFactory to override the default factory.
   */
   ...
   *
   * @return {@code true} if the index is available and the configuration of this instance is
   *     supported by it, {@code false} otherwise
   * @since 5.0
   */
    ...
    try {
      ...
      for (String type : types) {
        ...
        if (isCandidateComponent(metadataReader)) {
          AnnotatedGenericBeanDefinition sbd =
              new AnnotatedGenericBeanDefinition(metadataReader.getAnnotationMetadata());
  ...
  /**
   * Determine whether the given class is a candidate component based on any {@code @Conditional}
   * annotations.
   *
   * @param metadataReader the ASM ClassReader for the class
   * @return whether the class qualifies as a candidate component
   ...
   *
   * <p>The default implementation checks whether the class is not an interface and not dependent on
   * an enclosing class.
   *
