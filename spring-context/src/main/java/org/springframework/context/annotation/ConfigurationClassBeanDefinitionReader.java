...
/**
 * Reads a given fully-populated set of ConfigurationClass instances, registering bean definitions
 * with the given {@link BeanDefinitionRegistry} based on its contents.
 *
 * <p>This class was modeled after the {@link BeanDefinitionReader} hierarchy, but does not
 * implement/extend any of its artifacts as a set of configuration classes is not a {@link
 * Resource}.
 *
 * @author Chris Beams
...
class ConfigurationClassBeanDefinitionReader {
  ...
  /**
   * Create a new {@link ConfigurationClassBeanDefinitionReader} instance that will be used to
   * populate the given {@link BeanDefinitionRegistry}.
   */
  ...
  /**
   * Read {@code configurationModel}, registering bean definitions with the registry based on its
   * contents.
   */
   ...
   */
  @SuppressWarnings(
      "deprecation") // for RequiredAnnotationBeanPostProcessor.SKIP_REQUIRED_CHECK_ATTRIBUTE
    ...
    if (isOverriddenByExistingDefinition(beanMethod, beanName)) {
      if (beanName.equals(beanMethod.getConfigurationClass().getBeanName())) {
        throw new BeanDefinitionStoreException(
            ...
            beanName,
            "Bean name derived from @Bean method '"
  ...
  private void loadBeanDefinitionsFromImportedResources(
    ...
    importedResources.forEach(
        (resource, readerClass) -> {
  ...
  /**
   * {@link RootBeanDefinition} marker subclass used to signify that a bean definition was created
   * from a configuration class as opposed to any other configuration source. Used in bean
  ...
  /**
   * Evaluate {@code @Conditional} annotations, tracking results and taking into account 'imported
   * by'.
   */
