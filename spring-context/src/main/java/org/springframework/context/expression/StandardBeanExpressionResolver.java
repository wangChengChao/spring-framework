...
/**
 * Standard implementation of the {@link
 * org.springframework.beans.factory.config.BeanExpressionResolver} interface, parsing and
 * evaluating Spring EL using Spring's expression module.
 *
...
public class StandardBeanExpressionResolver implements BeanExpressionResolver {
  ...
  /**
   * Create a new {@code StandardBeanExpressionResolver} with the given bean class loader, using it
   * as the basis for expression compilation.
   *
   * @param beanClassLoader the factory's bean class loader
   */
  public StandardBeanExpressionResolver(@Nullable ClassLoader beanClassLoader) {
    this.expressionParser =
        new SpelExpressionParser(new SpelParserConfiguration(null, beanClassLoader));
  }
  ...
  public Object evaluate(@Nullable String value, BeanExpressionContext evalContext)
    ...
    try {
      ...
      if (sec == null) {
        ...
        sec.setTypeLocator(
            new StandardTypeLocator(evalContext.getBeanFactory().getBeanClassLoader()));
