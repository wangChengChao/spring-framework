...
/**
 * Standalone XML application context, taking the context definition files from the file system or
 * from URLs, interpreting plain paths as relative file system locations (e.g. "mydir/myfile.txt").
 * Useful for test harnesses as well as for standalone environments.
 *
 * <p><b>NOTE:</b> Plain paths will always be interpreted as relative to the current VM working
 * directory, even if they start with a slash. (This is consistent with the semantics in a Servlet
 * container.) <b>Use an explicit "file:" prefix to enforce an absolute file path.</b>
 *
...
public class FileSystemXmlApplicationContext extends AbstractXmlApplicationContext {
  ...
  /**
   * Create a new FileSystemXmlApplicationContext with the given parent, loading the definitions
   ...
   *
   * @param configLocations array of file paths
   * @param refresh whether to automatically refresh the context, loading all bean definitions and
   *     creating all singletons. Alternatively, call refresh manually after further configuring the
   *     context.
   * @throws BeansException if context creation failed
   * @see #refresh()
   */
   ...
   *
   * <p>Note: Even if a given path starts with a slash, it will get interpreted as relative to the
   * current VM working directory. This is consistent with the semantics in a Servlet container.
   *
