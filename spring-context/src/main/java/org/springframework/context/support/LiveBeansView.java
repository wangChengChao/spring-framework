...
/**
 * Adapter for live beans view exposure, building a snapshot of current beans and their dependencies
 * from either a local {@code ApplicationContext} (with a local {@code LiveBeansView} bean
 * definition) or all registered ApplicationContexts (driven by the {@value
 * #MBEAN_DOMAIN_PROPERTY_NAME} environment property).
 *
 * <p>Note: This feature is still in beta and primarily designed for use with Spring Tool Suite 3.1
 * and higher.
 *
...
public class LiveBeansView implements LiveBeansViewMBean, ApplicationContextAware {
  ...
  @Nullable private static String applicationName;

  static void registerApplicationContext(ConfigurableApplicationContext applicationContext) {
    String mbeanDomain =
        applicationContext.getEnvironment().getProperty(MBEAN_DOMAIN_PROPERTY_NAME);
    if (mbeanDomain != null) {
  ...
  /**
   * Generate a JSON snapshot of current beans and their dependencies, finding all active
   * ApplicationContexts through {@link #findApplicationContexts()}, then delegating to {@link
   * #generateJson(java.util.Set)}.
   */
  @Override
   ...
   *
   * <p>This implementation doesn't use any JSON parsing libraries in order to avoid third-party
   * library dependencies. It produces an array of context description objects, each containing a
   * context and parent attribute as well as a beans attribute with nested bean description objects.
   * Each bean object contains a bean, scope, type and resource attribute, as well as a dependencies
   * attribute with a nested array of bean names that the present bean depends on.
   *
    ...
    for (Iterator<ConfigurableApplicationContext> it = contexts.iterator(); it.hasNext(); ) {
      ...
      for (String beanName : beanNames) {
        ...
        if (isBeanEligible(beanName, bd, bf)) {
          ...
          result
              .append("\"resource\": \"")
              .append(getEscapedResourceDescription(bd))
              .append("\",\n");
  ...
  /**
   * Determine whether the specified bean is eligible for inclusion in the LiveBeansView JSON
   * snapshot.
   *
  ...
  /**
   * Determine a resource description for the given bean definition and apply basic JSON escaping
   * (backslashes, double quotes) to it.
   *
