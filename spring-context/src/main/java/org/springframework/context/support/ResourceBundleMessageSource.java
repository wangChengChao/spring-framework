 ...
 * using specified basenames. This class relies on the underlying JDK's {@link
...
public class ResourceBundleMessageSource extends AbstractResourceBasedMessageSource
  ...
  /**
   * Cache to hold loaded ResourceBundles. This Map is keyed with the bundle basename, which holds a
   * Map that is keyed with the Locale and in turn holds the ResourceBundle instances. This allows
   * for very efficient hash lookups, significantly faster than the ResourceBundle class's own
   * cache.
   */
  ...
  /**
   * Cache to hold already generated MessageFormats. This Map is keyed with the ResourceBundle,
   * which holds a Map that is keyed with the message code, which in turn holds a Map that is keyed
   * with the Locale and holds the MessageFormat values. This allows for very efficient hash lookups
  ...
  /**
   * Resolves the given message code as key in the registered resource bundles, returning the value
  ...
  /**
   * Resolves the given message code as key in the registered resource bundles, using a cached
   ...
   * ResourceBundle.Control}: When running on the JDK 9+ module path where such control handles are
   * not supported, any overrides in custom subclasses will effectively get ignored.
   *
   * <p>The default implementation returns a {@link PropertyResourceBundle}.
   *
