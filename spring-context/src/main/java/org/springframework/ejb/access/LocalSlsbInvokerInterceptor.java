...
/**
 * Invoker for a local Stateless Session Bean. Designed for EJB 2.x, but works for EJB 3 Session
 * Beans as well.
 *
 * <p>Caches the home object, since a local EJB home can never go stale. See {@link
...
public class LocalSlsbInvokerInterceptor extends AbstractSlsbInvokerInterceptor {
    ...
    try {
      ...
      if (targetEx instanceof CreateException) {
        throw new EjbAccessException(
            "Could not create local EJB [" + getJndiName() + "]", targetEx);
      } else {
        throw targetEx;
      }
  ...
  /**
   * Return an EJB instance to delegate the call to. Default implementation delegates to
