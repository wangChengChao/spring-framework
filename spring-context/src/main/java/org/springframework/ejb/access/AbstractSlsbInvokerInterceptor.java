...
/**
 * Base class for AOP interceptors invoking local or remote Stateless Session Beans. Designed for
 * EJB 2.x, but works for EJB 3 Session Beans as well.
 *
 * <p>Such an interceptor must be the last interceptor in the advice chain. In this case, there is
 * no direct target object: The call is handled in a special way, getting executed on an EJB
 * instance retrieved via an EJB home.
 *
 * @author Rod Johnson
...
public abstract class AbstractSlsbInvokerInterceptor extends JndiObjectLocator
  ...
  /**
   * The EJB's home object, potentially cached. The type must be Object as it could be either
   * EJBHome or EJBLocalHome.
   */
  @Nullable private Object cachedHome;
   ...
   *
   * <p>Can be turned off to allow for late start of the EJB server. In this case, the EJB home
   ...
   *
   * <p>Can be turned off to allow for hot restart of the EJB server. In this case, the EJB home
  ...
  /**
   * Set whether to expose the JNDI environment context for all access to the target EJB, i.e. for
   * all method invocations on the exposed object reference.
   *
   ...
   * each EJB invocation, as needed by WebLogic for EJBs with authorization requirements.
   */
  ...
  /**
   * Refresh the cached home object, if applicable. Also caches the create method on the home
   * object.
   *
   ...
   *
   * <p>Default implementation returns the home created on initialization, if any; else, it invokes
   * lookup to get a new proxy for each invocation.
   *
   * <p>Can be overridden in subclasses, for example to cache a home object for a given amount of
   * time before recreating it, or to test the home object whether it is still alive.
   *
  ...
  /** Prepares the thread context if necessary, and delegates to {@link #invokeInContext}. */
  @Override
  @Nullable
  ...
  /**
   * Perform the given invocation on the current EJB home, within the thread context being prepared
   * accordingly. Template method to be implemented by subclasses.
   *
