...
/**
 * Base class for interceptors proxying remote Stateless Session Beans. Designed for EJB 2.x, but
 * works for EJB 3 Session Beans as well.
 *
...
public abstract class AbstractRemoteSlsbInvokerInterceptor extends AbstractSlsbInvokerInterceptor {
   ...
   *
   * <p>Can be turned on to allow for hot restart of the EJB server. If a cached EJB home throws an
   * RMI exception that indicates a remote connect failure, a fresh home will be fetched and the
   ...
   *
   * <p>If configured to refresh on connect failure, it will call {@link #refreshAndRetry} on
  ...
  /**
   * Perform the given invocation on the current EJB home. Template method to be implemented by
   * subclasses.
   *
   * @param invocation the AOP method invocation
   * @return the invocation result, if any
  ...
  /**
   * Return a new instance of the stateless session bean. To be invoked by concrete remote SLSB
   * invoker subclasses.
   *
