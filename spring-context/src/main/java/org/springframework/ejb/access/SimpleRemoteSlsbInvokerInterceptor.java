...
/**
 * Basic invoker for a remote Stateless Session Bean. Designed for EJB 2.x, but works for EJB 3
 * Session Beans as well.
 *
 * <p>"Creates" a new EJB instance for each invocation, or caches the session bean instance for all
 * invocations (see {@link #setCacheSessionBean}). See {@link
 * org.springframework.jndi.JndiObjectLocator} for info on how to specify the JNDI location of the
 * target EJB.
 *
 ...
 * "lookupHomeOnStartup" property to "false", in which case the home will be fetched on first access
 * to the EJB. (This flag is only true by default for backwards compatibility reasons).
 *
 * <p>This invoker is typically used with an RMI business interface, which serves as super-interface
 * of the EJB component interface. Alternatively, this invoker can also proxy a remote SLSB with a
 * matching non-RMI business interface, i.e. an interface that mirrors the EJB business methods but
 * does not declare RemoteExceptions. In the latter case, RemoteExceptions thrown by the EJB stub
...
public class SimpleRemoteSlsbInvokerInterceptor extends AbstractRemoteSlsbInvokerInterceptor
  ...
  @Nullable private Object beanInstance;
   ...
   *
   * <p>Off by default for standard EJB compliance. Turn this flag on to optimize session bean
   * access for servers that are known to allow for caching the actual session bean object.
   *
   * @see #setCacheHome
   */
  ...
  /**
   * This implementation "creates" a new EJB instance for each invocation. Can be overridden for
   * custom invocation strategies.
   *
   * <p>Alternatively, override {@link #getSessionBeanInstance} and {@link
   * #releaseSessionBeanInstance} to change EJB instance creation, for example to hold a single
   * shared EJB component instance.
   */
  @Override
  @Nullable
