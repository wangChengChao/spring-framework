...
/**
 * Registrar object that associates a specific {@link javax.management.NotificationListener} with
 * one or more MBeans in an {@link javax.management.MBeanServer} (typically via a {@link
...
public class NotificationListenerRegistrar extends NotificationListenerHolder
  ...
  @Nullable private ObjectName[] actualObjectNames;
   ...
   *
   * <p>Ensures that an {@code MBeanServerConnection} is configured and attempts to detect a local
    ...
    try {
      ...
      if (this.actualObjectNames != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Registering NotificationListener for MBeans "
                  + Arrays.asList(this.actualObjectNames));
        }
