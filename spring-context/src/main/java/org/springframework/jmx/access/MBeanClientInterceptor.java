...
/**
 * {@link org.aopalliance.intercept.MethodInterceptor} that routes calls to an MBean running on the
 * supplied {@code MBeanServerConnection}. Works for both local and remote {@code
 * MBeanServerConnection}s.
 *
 * <p>By default, the {@code MBeanClientInterceptor} will connect to the {@code MBeanServer} and
 * cache MBean metadata at startup. This can be undesirable when running against a remote {@code
 * MBeanServer} that may not be running when the application starts. Through setting the {@link
 * #setConnectOnStartup(boolean) connectOnStartup} property to "false", you can defer this process
 * until the first invocation against the proxy.
 *
 * <p>This functionality is usually used through {@link MBeanProxyFactoryBean}. See the javadoc of
...
public class MBeanClientInterceptor
  ...
  @Nullable private MBeanServerConnection server;

  @Nullable private JMXServiceURL serviceUrl;

  @Nullable private Map<String, ?> environment;

  @Nullable private String agentId;
  ...
  @Nullable private Class<?> managementInterface;
  ...
  @Nullable private MBeanServerConnection serverToUse;

  @Nullable private MBeanServerInvocationHandler invocationHandler;
  ...
  /**
   * Set the {@code MBeanServerConnection} used to connect to the MBean which all invocations are
   * routed to.
   */
  ...
  /**
   * Allow Map access to the environment to be set for the connector, with the option to add or
   * override specific entries.
   *
   * <p>Useful for specifying entries directly, for example via "environment[myKey]". This is
   ...
   *
   * <p>Default is none. If specified, this will result in an attempt being made to locate the
   * attendant MBeanServer, unless the {@link #setServiceUrl "serviceUrl"} property has been set.
   *
  ...
  /**
   * Set whether or not the proxy should connect to the {@code MBeanServer} at creation time
   * ("true") or the first time it is invoked ("false"). Default is "true".
   */
   ...
   *
   * <p>Can be turned on to allow for hot restart of the JMX server, automatically reconnecting and
   * retrying in case of an IOException.
   */
  ...
  /**
   * Set the {@code ObjectName} of the MBean which calls are routed to, as {@code ObjectName}
   * instance or as {@code String}.
   */
  ...
  /**
   * Set the management interface of the target MBean, exposing bean property setters and getters
   * for MBean attributes and conventional Java methods for MBean operations.
   */
  ...
  /**
   * Prepares the {@code MBeanServerConnection} if the "connectOnStartup" is turned on (which it is
   * by default).
   */
  @Override
  public void afterPropertiesSet() {
    if (this.server != null && this.refreshOnConnectFailure) {
      throw new IllegalArgumentException(
          "'refreshOnConnectFailure' does not work when setting "
  ...
  /**
   * Ensures that an {@code MBeanServerConnection} is configured and attempts to detect a local
   * connection if one is not supplied.
   */
  public void prepare() {
    synchronized (this.preparationMonitor) {
      ...
      if (this.useStrictCasing) {
        ...
        this.invocationHandler =
            new MBeanServerInvocationHandler(
                this.serverToUse,
                this.objectName,
                (this.managementInterface != null
                    && JMX.isMXBeanInterface(this.managementInterface)));
      } else {
  ...
  private void retrieveMBeanInfo(MBeanServerConnection server) throws MBeanInfoRetrievalException {
    try {
      ...
      for (MBeanOperationInfo infoEle : operationInfo) {
        Class<?>[] paramTypes =
            JmxUtils.parameterInfoToTypes(infoEle.getSignature(), this.beanClassLoader);
      ...
      throw new MBeanInfoRetrievalException(
          "An IOException occurred when communicating with the "
  ...
  /**
   * Return whether this client interceptor has already been prepared, i.e. has already looked up
   * the server and cached all metadata.
   */
   ...
   *
   * <p>If not configured to refresh on connect failure, this method simply rethrows the original
   * exception.
   *
   ...
   * access to {@code MBeanServerConnection.get/setAttribute} and method invocation to {@code
  ...
  protected Object doInvoke(MethodInvocation invocation) throws Throwable {
    ...
    try {
      ...
      if (this.invocationHandler != null) {
        result =
            this.invocationHandler.invoke(invocation.getThis(), method, invocation.getArguments());
      } else {
  ...
  /**
   * Convert the given result object (from attribute access or operation invocation) to the
   * specified target class for returning from the proxy method.
   *
   ...
   * @return the converted result object, or the passed-in object if no conversion is necessary
   */
  @Nullable
  protected Object convertResultValueIfNecessary(
      @Nullable Object result, MethodParameter parameter) {
    ...
    try {
      ...
      throw new InvocationFailureException(
          "Could not obtain 'from(CompositeData)' / 'from(TabularData)' method on target type ["
              + targetClass.getName()
              + "] for conversion of MXBean data structure ["
              + result
              + "]");
    }
  }
  ...
  private Collection<?> convertDataArrayToTargetCollection(
      Object[] array, Class<?> collectionType, Class<?> elementType) throws NoSuchMethodException {

    Method fromMethod = elementType.getMethod("from", array.getClass().getComponentType());
    Collection<Object> resultColl =
        CollectionFactory.createCollection(collectionType, Array.getLength(array));
    for (int i = 0; i < array.length; i++) {
  ...
  /**
   * Simple wrapper class around a method name and its signature. Used as the key when caching
   * methods.
   */
  private static final class MethodCacheKey implements Comparable<MethodCacheKey> {
    ...
    /**
     * Create a new instance of {@code MethodCacheKey} with the supplied method name and parameter
     * list.
     *
    ...
    public boolean equals(@Nullable Object other) {
      ...
      return (this.name.equals(otherKey.name)
          && Arrays.equals(this.parameterTypes, otherKey.parameterTypes));
    }
