...
/**
 * Builds on the {@link AbstractMBeanInfoAssembler} superclass to add a basic algorithm for building
 * metadata based on the reflective metadata of the MBean class.
 *
 * <p>The logic for creating MBean metadata from the reflective metadata is contained in this class,
 * but this class makes no decisions as to which methods and properties are to be exposed. Instead
 * it gives subclasses a chance to 'vote' on each property or method through the {@code includeXXX}
 * methods.
 *
 * <p>Subclasses are also given the opportunity to populate attribute and operation metadata with
 * additional descriptors once the metadata is assembled through the {@code populateXXXDescriptor}
 * methods.
 *
 * @author Rob Harrop
...
public abstract class AbstractReflectiveMBeanInfoAssembler extends AbstractMBeanInfoAssembler {
  ...
  @Nullable private Integer defaultCurrencyTimeLimit;
  ...
  /**
   * Set the default for the JMX field "currencyTimeLimit". The default will usually indicate to
   * never cache attribute values.
   *
   * <p>Default is none, not explicitly setting that field, as recommended by the JMX 1.2
   * specification. This should result in "never cache" behavior, always reading attribute values
   * freshly (which corresponds to a "currencyTimeLimit" of {@code -1} in JMX 1.2).
   *
   * <p>However, some JMX implementations (that do not follow the JMX 1.2 spec in that respect)
   * might require an explicit value to be set here to get "never cache" behavior: for example,
   * JBoss 3.2.x.
   *
   * <p>Note that the "currencyTimeLimit" value can also be specified on a managed attribute or
   * operation. The default value will apply if not overridden with a "currencyTimeLimit" value
   * {@code >= 0} there: a metadata "currencyTimeLimit" value of {@code -1} indicates to use the
   * default; a value of {@code 0} indicates to "always cache" and will be translated to {@code
   * Integer.MAX_VALUE}; a positive value indicates the number of cache seconds.
   *
   ...
   *
   * <p>When using strict casing, a JavaBean property with a getter such as {@code getFoo()}
   * translates to an attribute called {@code Foo}. With strict casing disabled, {@code getFoo()}
  ...
  /**
   * Set whether to expose the JMX descriptor field "class" for managed operations. Default is
   * "false", letting the JMX implementation determine the actual class through reflection.
   *
   * <p>Set this property to {@code true} for JMX implementations that require the "class" field to
   * be specified, for example WebLogic's. In that case, Spring will expose the target class name
   * there, in case of a plain bean instance or a CGLIB proxy. When encountering a JDK dynamic
   * proxy, the <b>first</b> interface implemented by the proxy will be specified.
   *
   * <p><b>WARNING:</b> Review your proxy definitions when exposing a JDK dynamic proxy through JMX,
   * in particular with this property turned to {@code true}: the specified interface list should
   * start with your management interface in this case, with all other interfaces following. In
   * general, consider exposing your target bean directly or a CGLIB proxy for it instead.
   *
   ...
   * parameter names of MBean operation methods).
   *
  ...
  /**
   * Return the ParameterNameDiscoverer to use for resolving method parameter names if needed (may
   * be {@code null} in order to skip parameter detection).
   */
  @Nullable
  ...
  /**
   * Iterate through all properties on the MBean class and gives subclasses the chance to vote on
   * the inclusion of both the accessor and mutator. If a particular accessor or mutator is voted
   * for inclusion, the appropriate metadata is assembled and passed to the subclass for descriptor
   * population.
   *
   * @param managedBean the bean instance (might be an AOP proxy)
   * @param beanKey the key associated with the MBean in the beans map of the {@code MBeanExporter}
    ...
    for (PropertyDescriptor prop : props) {
      ...
      if (getter != null || setter != null) {
        ...
        ModelMBeanAttributeInfo info =
            new ModelMBeanAttributeInfo(attrName, description, getter, setter);
  ...
  /**
   * Iterate through all methods on the MBean class and gives subclasses the chance to vote on their
   * inclusion. If a particular method corresponds to the accessor or mutator of an attribute that
   * is included in the management interface, then the corresponding operation is exposed with the
   * &quot;role&quot; descriptor field set to the appropriate value.
   *
   * @param managedBean the bean instance (might be an AOP proxy)
   * @param beanKey the key associated with the MBean in the beans map of the {@code MBeanExporter}
  ...
  protected ModelMBeanOperationInfo[] getOperationInfo(Object managedBean, String beanKey) {
    ...
    for (Method method : methods) {
      ...
      if (pd != null
          && ((method.equals(pd.getReadMethod()) && includeReadAttribute(method, beanKey))
  ...
  /**
   * Creates an instance of {@code ModelMBeanOperationInfo} for the given method. Populates the
   * parameter info for the operation.
   *
   * @param method the {@code Method} to create a {@code ModelMBeanOperationInfo} for
   * @param name the logical name for the operation (method name or property name); not used by the
   *     default implementation but possibly by subclasses
   * @param beanKey the key associated with the MBean in the beans map of the {@code MBeanExporter}
   * @return the {@code ModelMBeanOperationInfo}
   */
  protected ModelMBeanOperationInfo createModelMBeanOperationInfo(
      Method method, String name, String beanKey) {
  ...
  /**
   * Return the class to be used for the JMX descriptor field "class". Only applied when the
   * "exposeClassDescriptor" property is "true".
   *
   * <p>The default implementation returns the first implemented interface for a JDK proxy, and the
   * target class else.
   *
   * @param managedBean the bean instance (might be an AOP proxy)
   ...
   *
   * <p>The default implementation returns a description for the operation that is the name of
  ...
  protected MBeanParameterInfo[] getOperationParameters(Method method, String beanKey) {
    ...
    String[] paramNames =
        (paramNameDiscoverer != null ? paramNameDiscoverer.getParameterNames(method) : null);
    if (paramNames == null) {
   ...
   *
   * <p>The default implementation sets the {@code currencyTimeLimit} field to the specified
  ...
  protected void populateMBeanDescriptor(
      Descriptor descriptor, Object managedBean, String beanKey) {
    applyDefaultCurrencyTimeLimit(descriptor);
  }

  /**
   * Allows subclasses to add extra fields to the {@code Descriptor} for a particular attribute.
   *
   * <p>The default implementation sets the {@code currencyTimeLimit} field to the specified
   * "defaultCurrencyTimeLimit", if any (by default none).
   *
  ...
  /**
   * Allows subclasses to add extra fields to the {@code Descriptor} for a particular operation.
   *
   * <p>The default implementation sets the {@code currencyTimeLimit} field to the specified
   * "defaultCurrencyTimeLimit", if any (by default none).
   *
  ...
  /**
   * Set the {@code currencyTimeLimit} field to the specified "defaultCurrencyTimeLimit", if any (by
   * default none).
   *
   ...
   *
   * <p>The default implementation sets a value {@code >0} as-is (as number of cache seconds), turns
   * a value of {@code 0} into {@code Integer.MAX_VALUE} ("always cache") and sets the
   * "defaultCurrencyTimeLimit" (if any, indicating "never cache") in case of a value {@code <0}.
