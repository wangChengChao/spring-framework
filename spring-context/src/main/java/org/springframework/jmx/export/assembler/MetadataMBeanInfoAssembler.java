...
/**
 * Implementation of the {@link MBeanInfoAssembler} interface that reads the management interface
 * information from source level metadata.
 *
 * <p>Uses the {@link JmxAttributeSource} strategy interface, so that metadata can be read using any
 * supported implementation. Out of the box, Spring provides an implementation based on annotations:
...
public class MetadataMBeanInfoAssembler extends AbstractReflectiveMBeanInfoAssembler
  ...
  @Nullable private JmxAttributeSource attributeSource;

  /**
   * Create a new {@code MetadataMBeanInfoAssembler} which needs to be configured through the {@link
   * #setAttributeSource} method.
   */
  ...
  /**
   * Set the {@code JmxAttributeSource} implementation to use for reading the metadata from the bean
   * class.
   *
   * @see org.springframework.jmx.export.annotation.AnnotationJmxAttributeSource
   */
  ...
  /**
   * Used for autodetection of beans. Checks to see if the bean's class has a {@code
  ...
  protected String getAttributeDescription(PropertyDescriptor propertyDescriptor, String beanKey) {
    ...
    ManagedMetric metric =
        (readMethod != null ? obtainAttributeSource().getManagedMetric(readMethod) : null);
  ...
  /**
   * Retrieves the description for the supplied {@code Method} from the metadata. Uses the method
   * name is no description is present in the metadata.
   */
  @Override
  ...
  /**
   * Reads {@code MBeanParameterInfo} from the {@code ManagedOperationParameter} attributes attached
   * to a method. Returns an empty array of {@code MBeanParameterInfo} if no attributes are found.
   */
  @Override
  protected MBeanParameterInfo[] getOperationParameters(Method method, String beanKey) {
    ManagedOperationParameter[] params =
        obtainAttributeSource().getManagedOperationParameters(method);
    ...
    for (int i = 0; i < params.length; i++) {
      ...
      parameterInfo[i] =
          new MBeanParameterInfo(
              param.getName(), methodParameters[i].getName(), param.getDescription());
    }
  ...
  /**
   * Adds descriptor fields from the {@code ManagedResource} attribute to the MBean descriptor.
   * Specifically, adds the {@code currencyTimeLimit}, {@code persistPolicy}, {@code persistPeriod},
   * {@code persistLocation} and {@code persistName} descriptor fields if they are present in the
   * metadata.
   */
  @Override
  ...
  /**
   * Adds descriptor fields from the {@code ManagedAttribute} attribute or the {@code ManagedMetric}
   * attribute to the attribute descriptor.
   */
  @Override
  protected void populateAttributeDescriptor(
    ...
    ManagedAttribute gma =
        (getter != null ? obtainAttributeSource().getManagedAttribute(getter) : null);
    ManagedAttribute sma =
        (setter != null ? obtainAttributeSource().getManagedAttribute(setter) : null);
  ...
  private void populateAttributeDescriptor(
      Descriptor desc, ManagedAttribute gma, ManagedAttribute sma) {
  ...
  /**
   * Adds descriptor fields from the {@code ManagedAttribute} attribute to the attribute descriptor.
   * Specifically, adds the {@code currencyTimeLimit} descriptor field if it is present in the
   * metadata.
   */
  @Override
  ...
  /**
   * Determines which of two {@code int} values should be used as the value for an attribute
   * descriptor. In general, only the getter or the setter will be have a non-negative value so we
   * use that value. In the event that both values are non-negative, we use the greater of the two.
   * This method can be used to resolve any {@code int} valued descriptor where there are two
   * possible values.
   *
  ...
  /**
   * Locates the value of a descriptor based on values attached to both the getter and setter
   * methods. If both have values supplied then the value attached to the getter is preferred.
   *
  ...
  /**
   * Locates the value of a descriptor based on values attached to both the getter and setter
   * methods. If both have values supplied then the value attached to the getter is preferred. The
   * supplied default value is used to check to see if the value associated with the getter has
