...
/**
 * {@link NotificationPublisher} implementation that uses the infrastructure provided by the {@link
 * ModelMBean} interface to track {@link javax.management.NotificationListener
 * javax.management.NotificationListeners} and send {@link Notification Notifications} to those
 * listeners.
 *
 * @author Rob Harrop
...
public class ModelMBeanNotificationPublisher implements NotificationPublisher {
  ...
  /**
   * The {@link ModelMBean} instance wrapping the managed resource into which this {@code
  ...
  /**
   * Create a new instance of the {@link ModelMBeanNotificationPublisher} class that will publish
   * all {@link javax.management.Notification Notifications} to the supplied {@link ModelMBean}.
   *
  ...
  /**
   * Send the supplied {@link Notification} using the wrapped {@link ModelMBean} instance.
   *
   ...
   * @throws UnableToSendNotificationException if the supplied {@code notification} could not be
   *     sent
   */
  @Override
  ...
  /**
   * Replaces the notification source if necessary to do so. From the {@link Notification javadoc}:
   * <i>"It is strongly recommended that notification senders use the object name rather than a
   * reference to the MBean object as the source."</i>
   *
   * @param notification the {@link Notification} whose {@link
   *     javax.management.Notification#getSource()} might need massaging
   */
