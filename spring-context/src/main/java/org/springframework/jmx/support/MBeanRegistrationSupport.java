...
/**
 * Provides supporting infrastructure for registering MBeans with an {@link
 * javax.management.MBeanServer}. The behavior when encountering an existing MBean at a given {@link
 * ObjectName} is fully configurable allowing for flexible registration settings.
 *
 * <p>All registered MBeans are tracked and can be unregistered by calling the #{@link
 * #unregisterBeans()} method.
 *
 * <p>Sub-classes can receive notifications when an MBean is registered or unregistered by
 * overriding the {@link #onRegister(ObjectName)} and {@link #onUnregister(ObjectName)} methods
 * respectively.
 *
 * <p>By default, the registration process will fail if attempting to register an MBean using a
 * {@link javax.management.ObjectName} that is already used.
 *
 * <p>By setting the {@link #setRegistrationPolicy(RegistrationPolicy) registrationPolicy} property
 * to {@link RegistrationPolicy#IGNORE_EXISTING} the registration process will simply ignore
 * existing MBeans leaving them registered. This is useful in settings where multiple applications
 * want to share a common MBean in a shared {@link MBeanServer}.
 *
 * <p>Setting {@link #setRegistrationPolicy(RegistrationPolicy) registrationPolicy} property to
 * {@link RegistrationPolicy#REPLACE_EXISTING} will cause existing MBeans to be replaced during
 * registration if necessary. This is useful in situations where you can't guarantee the state of
 * your {@link MBeanServer}.
 *
 * @author Rob Harrop
...
public class MBeanRegistrationSupport {
  ...
  /**
   * The policy used when registering an MBean and finding that it already exists. By default an
   * exception is raised.
   */
  private RegistrationPolicy registrationPolicy = RegistrationPolicy.FAIL_ON_EXISTING;

  /**
   * Specify the {@code MBeanServer} instance with which all beans should be registered. The {@code
   * MBeanExporter} will attempt to locate an existing {@code MBeanServer} if none is supplied.
   */
  ...
  /**
   * The policy to use when attempting to register an MBean under an {@link
   * javax.management.ObjectName} that already exists.
   *
  ...
  /**
   * Actually register the MBean with the server. The behavior when encountering an existing MBean
   * can be configured using {@link #setRegistrationPolicy}.
   *
  ...
  /**
   * Called when an MBean is registered under the given {@link ObjectName}. Allows subclasses to
   * perform additional processing when an MBean is registered.
   *
  ...
  /**
   * Called when an MBean is unregistered under the given {@link ObjectName}. Allows subclasses to
   * perform additional processing when an MBean is unregistered.
   *
   * <p>The default implementation is empty. Can be overridden in subclasses.
   *
