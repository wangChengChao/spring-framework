...
/**
 * Collection of generic utility methods to support Spring JMX. Includes a convenient method to
 * locate an MBeanServer.
 *
 * @author Rob Harrop
...
public abstract class JmxUtils {
  ...
  /**
   * The key used when extending an existing {@link ObjectName} with the identity hash code of its
   * corresponding managed resource.
   */
  ...
  /**
   * Attempt to find a locally running {@code MBeanServer}. Fails if no {@code MBeanServer} can be
   * found. Logs a warning if more than one {@code MBeanServer} found, returning the first one from
   * the list.
   *
   ...
   * the list.
   *
   * @param agentId the agent identifier of the MBeanServer to retrieve. If this parameter is {@code
   *     null}, all registered MBeanServers are considered. If the empty String is given, the
   *     platform MBeanServer will be returned.
   * @return the {@code MBeanServer} if found
   * @throws MBeanServerNotFoundException if no {@code MBeanServer} could be found
  ...
  /**
   * Convert an array of {@code MBeanParameterInfo} into an array of {@code Class} instances
  ...
  /**
   * Create a {@code String[]} representing the argument signature of a method. Each element in the
   * array is the fully qualified class name of the corresponding argument in the methods signature.
   *
   ...
   *
   * <p>When using strict casing, a JavaBean property with a getter method such as {@code getFoo()}
  ...
  /**
   * Append an additional key/value pair to an existing {@link ObjectName} with the key being the
   * static value {@code identity} and the value being the identity hash code of the managed
   * resource being exposed on the supplied {@link ObjectName}. This can be used to provide a unique
   * {@link ObjectName} for each distinct instance of a particular bean or class. Useful when
   * generating {@link ObjectName ObjectNames} at runtime for a set of managed resources based on
   * the template value supplied by a {@link
  ...
  /**
   * Return the class or interface to expose for the given bean. This is the class that will be
   * searched for attributes and operations (for example, checked for annotations).
   *
   * <p>This implementation returns the superclass for a CGLIB proxy and the class of the given bean
   * else (for a JDK proxy or a plain bean class).
   *
  ...
  /**
   * Return the class or interface to expose for the given bean class. This is the class that will
   * be searched for attributes and operations (for example, checked for annotations).
   *
   * <p>This implementation returns the superclass for a CGLIB proxy and the class of the given bean
   * else (for a JDK proxy or a plain bean class).
   *
   ...
   *
   * <p>This implementation checks for {@link javax.management.DynamicMBean} classes as well as
   * classes with corresponding "*MBean" interface (Standard MBeans) or corresponding "*MXBean"
   * interface (Java 6 MXBeans).
   *
  ...
  /**
   * Return the Standard MBean interface for the given class, if any (that is, an interface whose
   * name matches the class name of the given class but with suffix "MBean").
   *
  ...
  /**
   * Return the Java 6 MXBean interface exists for the given class, if any (that is, an interface
   * whose name ends with "MXBean" and/or carries an appropriate MXBean annotation).
   *
   * @param clazz the class to check
