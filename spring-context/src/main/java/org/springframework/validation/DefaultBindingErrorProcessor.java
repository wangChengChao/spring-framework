 ...
 *
 * <p>Uses the "required" error code and the field name to resolve message codes for a missing field
 * error.
 *
 * <p>Creates a {@code FieldError} for each {@code PropertyAccessException} given, using the {@code
 * PropertyAccessException}'s error code ("typeMismatch", "methodInvocation") for resolving message
 * codes.
 *
...
public class DefaultBindingErrorProcessor implements BindingErrorProcessor {
  ...
  /**
   * Error code that a missing field error (i.e. a required field not found in the list of property
   * values) will be registered with: "required".
   */
  ...
  /**
   * Return FieldError arguments for a binding error on the given field. Invoked for each missing
   * required field and each type mismatch.
   *
   * <p>The default implementation returns a single argument indicating the field name (of type
