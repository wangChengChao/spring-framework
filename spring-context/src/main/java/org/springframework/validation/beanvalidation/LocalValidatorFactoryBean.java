...
/**
 * This is the central class for {@code javax.validation} (JSR-303) setup in a Spring application
 * context: It bootstraps a {@code javax.validation.ValidationFactory} and exposes it through the
 * Spring {@link org.springframework.validation.Validator} interface as well as through the JSR-303
 * {@link javax.validation.Validator} interface and the {@link javax.validation.ValidatorFactory}
 * interface itself.
 *
 ...
 * convenient in that you don't have to perform yet another call on the factory, assuming that you
 * will almost always use the default Validator anyway. This can also be injected directly into any
 * target dependency of type {@link org.springframework.validation.Validator}!
 *
 * <p><b>As of Spring 5.0, this class requires Bean Validation 1.1+, with special support for
 * Hibernate Validator 5.x</b> (see {@link #setValidationMessageSource}). This class is also
 * runtime-compatible with Bean Validation 2.0 and Hibernate Validator 6.0, with one special note:
 * If you'd like to call BV 2.0's {@code getClockProvider()} method, obtain the native {@code
 * ValidatorFactory} through {@code #unwrap(ValidatorFactory.class)} and call the {@code
 * getClockProvider()} method on the returned native reference there.
 *
 * <p>This class is also being used by Spring's MVC configuration namespace, in case of the {@code
...
public class LocalValidatorFactoryBean extends SpringValidatorAdapter
  ...
  private void configureParameterNameProvider(
  ...
  /**
   * Post-process the given Bean Validation configuration, adding to or overriding any of its
   * settings.
   *
   * <p>Invoked right before building the {@link ValidatorFactory}.
   *
   * @param configuration the Configuration object, pre-populated with settings driven by
   *     LocalValidatorFactoryBean's properties
   */
