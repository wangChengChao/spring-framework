 ...
 *
 * <p>Will create two message codes for an object error, in the following order (when using the
 * {@link Format#PREFIX_ERROR_CODE prefixed} {@link #setMessageCodeFormatter(MessageCodeFormatter)
 * formatter}):
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>This resolution algorithm thus can be leveraged for example to show specific messages for
 * binding errors like "required" and "typeMismatch":
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>In case of array, {@link List} or {@link java.util.Map} properties, both codes for specific
 * elements and for the whole collection are generated. Assuming a field "name" of an array "groups"
 * in object "user":
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>By default the {@code errorCode}s will be placed at the beginning of constructed message
 * strings. The {@link #setMessageCodeFormatter(MessageCodeFormatter) messageCodeFormatter} property
 * can be used to specify an alternative concatenation {@link MessageCodeFormatter format}.
 *
 * <p>In order to group all codes into a specific category within your resource bundles, e.g.
 * "validation.typeMismatch.name" instead of the default "typeMismatch.name", consider specifying a
 * {@link #setPrefix prefix} to be applied.
 *
...
public class DefaultMessageCodesResolver implements MessageCodesResolver, Serializable {
   ...
   *
   * <p>Default is none. Specify, for example, "validation." to get error codes like
   * "validation.typeMismatch.name".
   */
  ...
  /**
   * Build the code list for the given code and field: an object/field-specific code, a
   * field-specific code, a plain error code.
   *
   * <p>Arrays, Lists and Maps are resolved both for specific elements and the whole collection.
   *
   * <p>See the {@link DefaultMessageCodesResolver class level javadoc} for details on the generated
   * codes.
   *
  ...
  public enum Format implements MessageCodeFormatter {
    ...
    /**
     * Prefix the error code at the beginning of the generated message code. e.g.: {@code errorCode
     * + "." + object name + "." + field}
     */
    PREFIX_ERROR_CODE {
      @Override
    ...
    /**
     * Postfix the error code at the end of the generated message code. e.g.: {@code object name +
     * "." + field + "." + errorCode}
     */
    POSTFIX_ERROR_CODE {
      @Override
    ...
    /**
     * Concatenate the given elements, delimiting each with {@link
     * DefaultMessageCodesResolver#CODE_SEPARATOR}, skipping zero-length or null elements
     * altogether.
     */
