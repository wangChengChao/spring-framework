 ...
 *
 * <p>Delegates to {@link java.text.NumberFormat#getInstance(Locale)}. Configures BigDecimal parsing
 * so there is no loss in precision. Allows configuration over the decimal number pattern. The
 * {@link #parse(String, Locale)} routine always returns a BigDecimal.
 *
 * @author Keith Donald
...
public class NumberStyleFormatter extends AbstractNumberFormatter {
  ...
  /**
   * Specify the pattern to use to format number values. If not specified, the default DecimalFormat
   * pattern is used.
   *
