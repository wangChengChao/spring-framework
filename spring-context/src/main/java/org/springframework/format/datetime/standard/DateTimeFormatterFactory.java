 ...
 *
 * <p>Formatters will be created using the defined {@link #setPattern pattern}, {@link #setIso ISO},
 * and <code>xxxStyle</code> methods (considered in that order).
 *
...
public class DateTimeFormatterFactory {
  ...
  @Nullable private ISO iso;

  @Nullable private FormatStyle dateStyle;

  @Nullable private FormatStyle timeStyle;
   ...
   *
   * <p>The first character is used for the date style; the second is for the time style. Supported
   * characters are:
   *
   * <ul>
   *   <li>'S' = Small
   ...
   * </ul>
   *
   * <p>This method mimics the styles supported by Joda-Time. Note that JSR-310 natively favors
   * {@link java.time.format.FormatStyle} as used for {@link #setDateStyle}, {@link #setTimeStyle}
   * and {@link #setDateTimeStyle}.
   *
   ...
   *
   * <p>If no specific pattern or style has been defined, {@link FormatStyle#MEDIUM medium date time
   * format} will be used.
   *
   ...
   * Create a new {@code DateTimeFormatter} using this factory.
   *
   * <p>If no specific pattern or style has been defined, the supplied {@code fallbackFormatter}
   * will be used.
   *
   * @param fallbackFormatter the fall-back formatter to use when no specific factory properties
   *     have been set
   * @return a new date time formatter
   */
  public DateTimeFormatter createDateTimeFormatter(DateTimeFormatter fallbackFormatter) {
    ...
    if (StringUtils.hasLength(this.pattern)) {
      ...
      dateTimeFormatter =
          DateTimeFormatter.ofPattern(patternToUse).withResolverStyle(ResolverStyle.STRICT);
