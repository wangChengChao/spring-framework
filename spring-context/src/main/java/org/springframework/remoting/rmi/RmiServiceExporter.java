...
/**
 * RMI exporter that exposes the specified service as RMI object with the specified name. Such
 * services can be accessed via plain RMI or via {@link RmiProxyFactoryBean}. Also supports exposing
 * any non-RMI service via RMI invokers, to be accessed via {@link RmiClientInterceptor} / {@link
 * RmiProxyFactoryBean}'s automatic detection of such invokers.
 *
 * <p>With an RMI invoker, RMI communication works on the {@link RmiInvocationHandler} level,
 * needing only one stub for any service. Service interfaces do not have to extend {@code
 * java.rmi.Remote} or throw {@code java.rmi.RemoteException} on all methods, but in and out
 * parameters have to be serializable.
 *
 * <p>The major advantage of RMI, compared to Hessian, is serialization. Effectively, any
 * serializable Java object can be transported without hassle. Hessian has its own
 * (de-)serialization mechanisms, but is HTTP-based and thus much easier to setup than RMI.
 * Alternatively, consider Spring's HTTP invoker to combine Java serialization with HTTP-based
 * transport.
 *
 * <p>Note: RMI makes a best-effort attempt to obtain the fully qualified host name. If one cannot
 * be determined, it will fall back and use the IP address. Depending on your network configuration,
 * in some cases it will resolve the IP to the loopback address. To ensure that RMI will use the
 * host name bound to the correct network interface, you should pass the {@code
 * java.rmi.server.hostname} property to the JVM that will export the registry and/or the service
 * using the "-D" JVM argument. For example: {@code -Djava.rmi.server.hostname=myserver.com}
 *
...
public class RmiServiceExporter extends RmiBasedExporter
   ...
   *
   * <p>If the given object also implements {@code java.rmi.server.RMIServerSocketFactory}, it will
   ...
   * @see UnicastRemoteObject#exportObject(Remote, int, RMIClientSocketFactory,
   ...
   *
   * <p>Only needs to be specified when the client socket factory does not implement {@code
  ...
  /**
   * Specify the RMI registry to register the exported service with. Typically used in combination
   * with RmiRegistryFactoryBean.
   *
   * <p>Alternatively, you can specify all registry properties locally. This exporter will then try
   * to locate the specified registry, automatically creating a new local one if appropriate.
   *
   * <p>Default is a local registry at the default port (1099), created on the fly if necessary.
   *
  ...
  /**
   * Set the host of the registry for the exported RMI service, i.e. {@code rmi://HOST:port/name}
   *
  ...
  /**
   * Set the port of the registry for the exported RMI service, i.e. {@code rmi://host:PORT/name}
   *
  ...
  /**
   * Set whether to always create the registry in-process, not attempting to locate an existing
   * registry at the specified port.
   *
   * <p>Default is "false". Switch this flag to "true" in order to avoid the overhead of locating an
   * existing registry when you always intend to create a new registry in any case.
   */
  ...
  /**
   * Set whether to replace an existing binding in the RMI registry, that is, whether to simply
   * override an existing binding with the specified service in case of a naming conflict in the
   * registry.
   *
   * <p>Default is "true", assuming that an existing binding for this exporter's service name is an
   * accidental leftover from a previous execution. Switch this to "false" to make the exporter fail
   * in such a scenario, indicating that there was already an RMI object bound.
   */
  ...
  protected Registry getRegistry(
      ...
      @Nullable RMIServerSocketFactory serverSocketFactory)
