 ...
 *
 * <p>Exports services via the {@link javax.rmi.PortableRemoteObject} class. You need to run "rmic"
 * with the "-iiop" option to generate corresponding stubs and skeletons for each exported service.
 *
 * <p>Also supports exposing any non-RMI service via RMI invokers, to be accessed via {@link
 * JndiRmiClientInterceptor} / {@link JndiRmiProxyFactoryBean}'s automatic detection of such
 * invokers.
 *
 ...
 *
 * <p>The JNDI environment can be specified as "jndiEnvironment" bean property, or be configured in
 * a {@code jndi.properties} file or as system properties. For example:
 *
...
public class JndiRmiServiceExporter extends RmiBasedExporter
  ...
  @Nullable private static Method exportObject;

  @Nullable private static Method unexportObject;
  ...
  /**
   * Set the JNDI template to use for JNDI lookups. You can also specify JNDI environment settings
   * via "jndiEnvironment".
   *
   * @see #setJndiEnvironment
   */
  ...
  /**
   * Rebind the specified service to JNDI, for recovering in case of the target registry having been
   * restarted.
   *
   * @throws NamingException if service binding failed
   */
