...
/**
 * Factored-out methods for performing invocations within an RMI client. Can handle both RMI and
 * non-RMI service interfaces working on an RMI stub.
 *
...
public abstract class RmiClientInterceptorUtils {
  ...
  /**
   * Perform a raw method invocation on the given RMI stub, letting reflection exceptions through
   * as-is.
   *
   ...
   * RemoteException or a Spring RemoteAccessException (if the method signature does not support
   * RemoteException).
   *
   * <p>Only call this for remote access exceptions, not for exceptions thrown by the target service
   * itself!
   *
   * @param method the invoked method
   * @param ex the exception that happened, to be used as cause for the RemoteAccessException or
   *     RemoteException
   * @param message the message for the RemoteAccessException respectively RemoteException
   ...
   * RemoteAccessException if the method signature does not support RemoteException. Else, return
  ...
  public static Exception convertRmiAccessException(
      Method method, RemoteException ex, String serviceName) {
   ...
   * @param method the invoked method
   * @param ex the RemoteException that happened
   * @param isConnectFailure whether the given exception should be considered a connect failure
   * @param serviceName the name of the service (for debugging purposes)
   * @return the exception to be thrown to the caller
   */
  public static Exception convertRmiAccessException(
    ...
    if (ReflectionUtils.declaresException(method, ex.getClass())) {
      ...
      if (isConnectFailure) {
        ...
        return new RemoteAccessException(
            "Could not access remote service [" + serviceName + "]", ex);
      }
    }
  }
