 ...
 *
 * <p>Note that the service interface being used will show some signs of remotability, like the
 * granularity of method calls that it offers. Furthermore, it has to have serializable arguments
 * etc.
 *
...
public abstract class RemoteExporter extends RemotingSupport {
  ...
  /**
   * Set the interface of the service to export. The interface must be suitable for the particular
   * service and remoting strategy.
   */
  ...
  /**
   * Set whether to register a RemoteInvocationTraceInterceptor for exported services. Only applied
   * when a subclass uses {@code getProxyForService} for creating the proxy to expose.
   *
   * <p>Default is "true". RemoteInvocationTraceInterceptor's most important value is that it logs
   * exception stacktraces on the server, before propagating an exception to the client. Note that
   * RemoteInvocationTraceInterceptor will <i>not</i> be registered by default if the "interceptors"
   ...
   *
   * <p>Used to export a proxy that does not expose any internals but just a specific interface
   * intended for remote access. Furthermore, a {@link RemoteInvocationTraceInterceptor} will be
   ...
   *
   * <p>Default is the unqualified class name (without package). Can be overridden in subclasses.
   *
   * @see #getProxyForService
   * @see RemoteInvocationTraceInterceptor
