...
/**
 * Adapter that takes a {@code java.util.concurrent.Executor} and exposes a Spring {@link
 * org.springframework.core.task.TaskExecutor} for it. Also detects an extended {@code
 * java.util.concurrent.ExecutorService}, adapting the {@link
 * org.springframework.core.task.AsyncTaskExecutor} interface accordingly.
 *
 * <p>Autodetects a JSR-236 {@link javax.enterprise.concurrent.ManagedExecutorService} in order to
 * expose {@link javax.enterprise.concurrent.ManagedTask} adapters for it, exposing a long-running
 * hint based on {@link SchedulingAwareRunnable} and an identity name based on the given
 * Runnable/Callable's {@code toString()}. For JSR-236 style lookup in a Java EE 7 environment,
 * consider using {@link DefaultManagedTaskExecutor}.
 *
 * <p>Note that there is a pre-built {@link ThreadPoolTaskExecutor} that allows for defining a
 * {@link java.util.concurrent.ThreadPoolExecutor} in bean style, exposing it as a Spring {@link
 * org.springframework.core.task.TaskExecutor} directly. This is a convenient alternative to a raw
 * ThreadPoolExecutor definition with a separate definition of the present adapter class.
 *
...
public class ConcurrentTaskExecutor implements AsyncListenableTaskExecutor, SchedulingTaskExecutor {
  ...
  @Nullable private static Class<?> managedExecutorServiceClass;
  ...
  private static TaskExecutorAdapter getAdaptedExecutor(Executor concurrentExecutor) {
    if (managedExecutorServiceClass != null
        && managedExecutorServiceClass.isInstance(concurrentExecutor)) {
  ...
  /**
   * TaskExecutorAdapter subclass that wraps all provided Runnables and Callables with a JSR-236
   * ManagedTask, exposing a long-running hint based on {@link SchedulingAwareRunnable} and an
   * identity name based on the task's {@code toString()} representation.
   */
  ...
  /**
   * Delegate that wraps a given Runnable/Callable with a JSR-236 ManagedTask, exposing a
   * long-running hint based on {@link SchedulingAwareRunnable} and a given identity name.
   */
