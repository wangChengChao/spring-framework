...
/**
 * {@link FactoryBean} for creating {@link ThreadPoolTaskExecutor} instances, primarily used behind
 * the XML task namespace.
 *
 * @author Mark Fisher
...
public class TaskExecutorFactoryBean
  ...
  @Nullable private String poolSize;

  @Nullable private Integer queueCapacity;

  @Nullable private RejectedExecutionHandler rejectedExecutionHandler;

  @Nullable private Integer keepAliveSeconds;

  @Nullable private String beanName;

  @Nullable private ThreadPoolTaskExecutor target;
    ...
    if (StringUtils.hasText(this.poolSize)) {
      try {
        ...
        if (separatorIndex != -1) {
          ...
          maxPoolSize =
              Integer.parseInt(this.poolSize.substring(separatorIndex + 1, this.poolSize.length()));
