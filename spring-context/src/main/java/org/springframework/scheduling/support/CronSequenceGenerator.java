...
/**
 * Date sequence generator for a <a href="https://www.manpagez.com/man/5/crontab/">Crontab
 * pattern</a>, allowing clients to specify a pattern that the sequence matches.
 *
 * <p>The pattern is a list of six single space-separated fields: representing second, minute, hour,
 * day, month, weekday. Month and weekday names can be given as the first three letters of the
 * English names.
 *
 * <p>Example patterns:
 *
...
public class CronSequenceGenerator {
  ...
  /**
   * Construct a {@link CronSequenceGenerator} from the pattern provided, using the default {@link
   * TimeZone}.
   *
  ...
  /**
   * Construct a {@link CronSequenceGenerator} from the pattern provided, using the specified {@link
   * TimeZone}.
   *
   * @param expression a space-separated list of time fields
  ...
  private void doNext(Calendar calendar, int dot) {
    ...
    int updateSecond =
        findNext(this.seconds, second, calendar, Calendar.SECOND, Calendar.MINUTE, emptyList);
    ...
    int updateMinute =
        findNext(this.minutes, minute, calendar, Calendar.MINUTE, Calendar.HOUR_OF_DAY, resets);
    ...
    int updateHour =
        findNext(this.hours, hour, calendar, Calendar.HOUR_OF_DAY, Calendar.DAY_OF_WEEK, resets);
    if (hour == updateHour) {
    ...
    int updateDayOfMonth =
        findNextDay(calendar, this.daysOfMonth, dayOfMonth, this.daysOfWeek, dayOfWeek, resets);
   ...
   * @param field the field to increment in the calendar (@see {@link Calendar} for the static
   *     constants defining valid fields)
   * @param lowerOrders the Calendar field ids that should be reset (i.e. the ones of lower
   *     significance than the field of interest)
