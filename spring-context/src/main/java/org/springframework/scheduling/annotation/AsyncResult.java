...
/**
 * A pass-through {@code Future} handle that can be used for method signatures which are declared
 * with a {@code Future} return type for asynchronous execution.
 *
 * <p>As of Spring 4.1, this class implements {@link ListenableFuture}, not just plain {@link
 * java.util.concurrent.Future}, along with the corresponding support in {@code @Async} processing.
 *
 * <p>As of Spring 4.2, this class also supports passing execution exceptions back to the caller.
 *
...
public class AsyncResult<V> implements ListenableFuture<V> {
  ...
  @Nullable private final Throwable executionException;
  ...
  public void addCallback(
      SuccessCallback<? super V> successCallback, FailureCallback failureCallback) {
    try {
      if (this.executionException != null) {
  ...
  /**
   * Create a new async result which exposes the given exception as an {@link ExecutionException}
   * from {@link Future#get()}.
   *
   * @param ex the exception to expose (either an pre-built {@link ExecutionException} or a cause to
  ...
  /**
   * Determine the exposed exception: either the cause of a given {@link ExecutionException}, or the
