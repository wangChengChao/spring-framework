...
/**
 * A test {@link ClassLoader} that can be used in testing context to control the {@code
 * spring.components} resource that should be loaded. Can also simulate a failure by throwing a
 * configurable {@link IOException}.
 *
...
public class CandidateComponentsTestClassLoader extends ClassLoader {
  ...
  /**
   * Create a test {@link ClassLoader} that disable the use of the index, even if resources are
  ...
  public static ClassLoader disableIndex(ClassLoader classLoader) {
    return new CandidateComponentsTestClassLoader(
        classLoader, Collections.enumeration(Collections.emptyList()));
  }

  /**
   * Create a test {@link ClassLoader} that creates an index with the specified {@link Resource}
   * instances
   *
   * @param classLoader the classloader to use for all other operations
   * @return a test {@link ClassLoader} with an index built based on the specified resources.
   */
  ...
  public CandidateComponentsTestClassLoader(
      ClassLoader classLoader, Enumeration<URL> resourceUrls) {
    super(classLoader);
