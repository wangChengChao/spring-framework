...
public class EventCollector {
  ...
  /**
   * Return the events that the specified listener has received. The list of events is ordered
   * according to their reception order.
   */
  ...
  /**
   * Assert that the listener identified by the specified id has received the specified events, in
   * that specific order.
   */
  ...
  /**
   * Assert the number of events received by this instance. Checks that unexpected events have not
   * been received. If an event is handled by several listeners, each instance will be registered.
   */
  public void assertTotalEventsCount(int number) {
    ...
    assertThat(actual)
        .as("Wrong number of total events (" + this.content.size() + ") registered listener(s)")
        .isEqualTo(number);
  }
