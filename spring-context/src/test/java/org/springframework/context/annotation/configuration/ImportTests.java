...
public class ImportTests {
  ...
  public void testProcessImportsWithAsm() {
    ...
    beanFactory.registerBeanDefinition(
        "config", new RootBeanDefinition(ConfigurationWithImportAnnotation.class.getName()));
    ConfigurationClassPostProcessor pp = new ConfigurationClassPostProcessor();
    pp.postProcessBeanFactory(beanFactory);
  ...
  public void testProcessImportsWithDoubleImports() {
    ...
    assertBeanDefinitionCount(
        (configClasses + beansInClasses),
        ConfigurationWithImportAnnotation.class,
        OtherConfigurationWithImportAnnotation.class);
  }
  ...
  public void testImportAnnotationWithMultipleArguments() {
    ...
    int beansInClasses = 3;
    assertBeanDefinitionCount(
        (configClasses + beansInClasses), WithMultipleArgumentsToImportAnnotation.class);
  }
  ...
  public void testImportAnnotationWithMultipleArgumentsResultingInOverriddenBeanDefinition() {
    DefaultListableBeanFactory beanFactory = new DefaultListableBeanFactory();
    beanFactory.registerBeanDefinition(
        "config", new RootBeanDefinition(WithMultipleArgumentsThatWillCauseDuplication.class));
    ConfigurationClassPostProcessor pp = new ConfigurationClassPostProcessor();
    pp.postProcessBeanFactory(beanFactory);
  ...
  /**
   * Test that values supplied to @Configuration(value="...") are propagated as the bean name for
   * the configuration class even in the case of inclusion via @Import or in the case of automatic
   * registration via nesting
   */
  @Test
  ...
  public void testProcessImports() {
    ...
    int beansInClasses = 2;
    assertBeanDefinitionCount(
        (configClasses + beansInClasses), ConfigurationWithImportAnnotation.class);
  }
}
