...
/**
 * System tests covering use of {@link Autowired} and {@link Value} within {@link Configuration}
 * classes.
 *
 * @author Chris Beams
...
public class AutowiredConfigurationTests {
  ...
  public void testAutowiredConfigurationDependencies() {
    ...
    assertThat(context.getBean("testBean", TestBean.class).getName())
        .isEqualTo(Colour.RED.toString());
  }
  ...
  public void testAutowiredSingleConstructorSupported() {
    ...
    ctx.refresh();
    assertThat(ctx.getBean(Colour.class))
        .isSameAs(ctx.getBean(AutowiredConstructorConfig.class).colour);
  }
  ...
  public void testObjectFactoryConstructorWithTypeVariable() {
    ...
    ctx.registerBeanDefinition(
        "config1", new RootBeanDefinition(ObjectFactoryConstructorConfig.class));
    ctx.registerBeanDefinition("config2", new RootBeanDefinition(ColorConfig.class));
    ctx.refresh();
    assertThat(ctx.getBean(Colour.class))
        .isSameAs(ctx.getBean(ObjectFactoryConstructorConfig.class).colour);
  }
  ...
  public void testAutowiredAnnotatedConstructorSupported() {
    ...
    ctx.refresh();
    assertThat(ctx.getBean(Colour.class))
        .isSameAs(ctx.getBean(MultipleConstructorConfig.class).colour);
  }
  ...
  static class AutowiredConfig {
    ...
    @Autowired private Colour colour;
