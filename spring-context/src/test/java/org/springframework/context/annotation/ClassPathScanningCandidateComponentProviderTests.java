...
public class ClassPathScanningCandidateComponentProviderTests {
  ...
  private static final ClassLoader TEST_BASE_CLASSLOADER =
  ...
  private void testAntStyle(
      ClassPathScanningCandidateComponentProvider provider,
      Class<? extends BeanDefinition> expectedBeanDefinitionType) {
    Set<BeanDefinition> candidates =
        provider.findCandidateComponents(TEST_BASE_PACKAGE + ".**.sub");
    assertThat(containsBeanClass(candidates, BarComponent.class)).isTrue();
  ...
  private void testCustomAnnotationTypeIncludeFilter(
      ClassPathScanningCandidateComponentProvider provider,
      Class<? extends BeanDefinition> expectedBeanDefinitionType) {
    provider.addIncludeFilter(new AnnotationTypeFilter(Component.class));
  ...
  private void testCustomSupportedIncludeAndExcludeFilter(
      ClassPathScanningCandidateComponentProvider provider,
      Class<? extends BeanDefinition> expectedBeanDefinitionType) {
    provider.addIncludeFilter(new AnnotationTypeFilter(Component.class));
  ...
  public void excludeFilterWithScan() {
    ...
    provider.addExcludeFilter(
        new RegexPatternTypeFilter(Pattern.compile(TEST_BASE_PACKAGE + ".*Named.*")));
  ...
  private void assertBeanDefinitionType(
      Set<BeanDefinition> candidates, Class<? extends BeanDefinition> expectedType) {
    candidates.forEach(c -> assertThat(c).isInstanceOf(expectedType));
  }
