...
public class BeanMethodPolymorphismTests {
  ...
  public void beanMethodOverridingWithNarrowedReturnTypeOnASM() {
    ...
    ctx.registerBeanDefinition(
        "config", new RootBeanDefinition(NarrowedOverridingConfig.class.getName()));
    ctx.setAllowBeanDefinitionOverriding(false);
    ctx.refresh();
    assertThat(ctx.getDefaultListableBeanFactory().containsSingleton("testBean")).isFalse();
    assertThat(ctx.getBean("testBean", TestBean.class).toString()).isEqualTo("overridden");
  ...
  /**
   * When inheritance is not involved, it is still possible to override a bean method from the
   * container's point of view. This is not strictly 'overloading' of a method per se, so it's
   * referred to here as 'shadowing' to distinguish the difference.
   */
  @Test
  public void beanMethodShadowing() {
    AnnotationConfigApplicationContext ctx =
        new AnnotationConfigApplicationContext(ShadowConfig.class);
