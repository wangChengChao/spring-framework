...
/**
 * System tests covering use of AspectJ {@link Aspect}s in conjunction with {@link Configuration}
 * classes. {@link Bean} methods may return aspects, or Configuration classes may themselves be
 * annotated with Aspect. In the latter case, advice methods are declared inline within the
 * Configuration class. This makes for a particularly convenient syntax requiring no extra artifact
 * for the aspect.
 *
 * <p>Currently it is assumed that the user is bootstrapping Configuration class processing via XML
 * (using annotation-config or component-scan), and thus will also use {@code
 * <aop:aspectj-autoproxy/>} to enable processing of the Aspect annotation.
 *
 * @author Chris Beams
...
public class ConfigurationClassAspectIntegrationTests {
    ...
    new XmlBeanDefinitionReader(factory)
        .loadBeanDefinitions(
            new ClassPathResource(
                "aspectj-autoproxy-config.xml", ConfigurationClassAspectIntegrationTests.class));
  ...
  public void withInnerClassAndLambdaExpression() {
    ApplicationContext ctx =
        new AnnotationConfigApplicationContext(Application.class, CountingAspect.class);
    ctx.getBeansOfType(Runnable.class).forEach((k, v) -> v.run());

    // TODO: returns just 1 as of AspectJ 1.9 beta 3, not detecting the applicable lambda expression
    // anymore
  ...
  static class AspectConfig {
    ...
    @Before(
        "execution(* org.springframework.tests.sample.beans.TestBean.absquatulate(..)) && target(testBean)")
