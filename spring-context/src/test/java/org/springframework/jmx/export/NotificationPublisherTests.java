...
public class NotificationPublisherTests extends AbstractMBeanServerTests {
  ...
  public void testSimpleBean() throws Exception {
    ...
    ConfigurableApplicationContext ctx =
        loadContext("org/springframework/jmx/export/notificationPublisherTests.xml");
    this.server.addNotificationListener(
        ObjectNameManager.getInstance("spring:type=Publisher"), listener, null, null);

    MyNotificationPublisher publisher = (MyNotificationPublisher) ctx.getBean("publisher");
    assertThat(publisher.getNotificationPublisher())
        .as("NotificationPublisher should not be null")
        .isNotNull();
  ...
  public void testSimpleBeanRegisteredManually() throws Exception {
    ...
    exporter.registerManagedResource(
        publisher, ObjectNameManager.getInstance("spring:type=Publisher2"));
    this.server.addNotificationListener(
        ObjectNameManager.getInstance("spring:type=Publisher2"), listener, null, null);
  ...
  public void testMBean() throws Exception {
    ...
    ConfigurableApplicationContext ctx =
        loadContext("org/springframework/jmx/export/notificationPublisherTests.xml");
    this.server.addNotificationListener(
        ObjectNameManager.getInstance("spring:type=PublisherMBean"), listener, null, null);

    MyNotificationPublisherMBean publisher =
        (MyNotificationPublisherMBean) ctx.getBean("publisherMBean");
    publisher.sendNotification();
    assertThat(listener.count).as("Notification not sent").isEqualTo(1);
  }
  ...
  */
  ...
  public void testLazyInit() throws Exception {
    // start the MBeanExporter
    ConfigurableApplicationContext ctx =
        loadContext("org/springframework/jmx/export/notificationPublisherLazyTests.xml");
    assertThat(ctx.getBeanFactory().containsSingleton("publisher"))
        .as("Should not have instantiated the bean yet")
        .isFalse();
  ...
  public static class MyNotificationPublisherMBean extends NotificationBroadcasterSupport
  ...
  public static class MyNotificationPublisherStandardMBean extends NotificationBroadcasterSupport
      implements MyMBean {
