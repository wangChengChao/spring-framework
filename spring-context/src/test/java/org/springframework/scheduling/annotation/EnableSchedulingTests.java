...
public class EnableSchedulingTests {
  ...
  public void withExplicitScheduler() throws InterruptedException {
    ...
    Thread.sleep(100);
    assertThat(ctx.getBean(AtomicInteger.class).get()).isGreaterThanOrEqualTo(10);
    assertThat(ctx.getBean(ExplicitSchedulerConfig.class).threadName)
        .startsWith("explicitScheduler-");
    assertThat(
            Arrays.asList(ctx.getDefaultListableBeanFactory().getDependentBeans("myTaskScheduler"))
                .contains(TaskManagementConfigUtils.SCHEDULED_ANNOTATION_PROCESSOR_BEAN_NAME))
        .isTrue();
  }
  ...
  public void withExplicitScheduledTaskRegistrar() throws InterruptedException {
    ...
    Thread.sleep(100);
    assertThat(ctx.getBean(AtomicInteger.class).get()).isGreaterThanOrEqualTo(10);
    assertThat(ctx.getBean(ExplicitScheduledTaskRegistrarConfig.class).threadName)
        .startsWith("explicitScheduler1");
  }
  ...
  @EnableScheduling
  static
  class SchedulingEnabled_withAmbiguousTaskSchedulers_andSingleTask_disambiguatedByScheduledTaskRegistrar
  ...
  @EnableScheduling
  static
  class SchedulingEnabled_withAmbiguousTaskSchedulers_andSingleTask_disambiguatedBySchedulerNameAttribute
      implements SchedulingConfigurer {
  ...
  static class TriggerTaskConfig {
    ...
    public TaskScheduler scheduler() {
      ...
      scheduler.schedule(
          () -> counter().incrementAndGet(), triggerContext -> new Date(new Date().getTime() + 10));
      return scheduler;
    }
  }
