...
public class AdvisorAutoProxyCreatorTests {
  ...
  /**
   * Check that we can provide a common interceptor that will appear in the chain before "specific"
   * interceptors, which are sourced from matching advisors
   */
  @Test
  public void testCommonInterceptorAndAdvisor() throws Exception {
    ...
    PackageVisibleMethod packageVisibleMethod =
        (PackageVisibleMethod) bf.getBean("packageVisibleMethod");
    assertThat(nop1.getCount()).isEqualTo(5);
    assertThat(nop2.getCount()).isEqualTo(0);
  ...
  public void testWithOptimizedProxy() throws Exception {
    ...
    CountingBeforeAdvice beforeAdvice =
        (CountingBeforeAdvice) beanFactory.getBean("countingAdvice");
