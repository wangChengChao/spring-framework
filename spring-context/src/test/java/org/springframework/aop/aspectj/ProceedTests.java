...
/**
 * Test for SPR-3522. Arguments changed on a call to proceed should be visible to advice further
...
public class ProceedTests {
  ...
  public void testProceedWithArgsInSameAspect() {
    ...
    assertThat(this.firstTestAspect.getLastBeforeFloatValue() > 1.9F)
        .as("changed value visible to next advice in chain")
        .isTrue();
  }
  ...
  public void testProceedWithArgsAcrossAspects() {
    ...
    assertThat(this.secondTestAspect.getLastBeforeStringValue())
        .as("changed value visible to next before advice in chain")
        .isEqualTo("MALE");
    assertThat(this.secondTestAspect.getLastAroundStringValue())
        .as("changed value visible to next around advice in chain")
        .isEqualTo("MALE");
  }
}
...
class ProceedTestingAspect implements Ordered {
  ...
  public Object captureStringArgumentInAround(ProceedingJoinPoint pjp, String arg)
      throws Throwable {
  ...
  public void captureFloatArgument(JoinPoint tjp, float arg) {
    ...
    if (Math.abs(tjpArg - arg) > 0.000001) {
      throw new IllegalStateException(
          "argument is '" + arg + "', " + "but args array has '" + tjpArg + "'");
    }
