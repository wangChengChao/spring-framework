...
public class AspectAndAdvicePrecedenceTests {
  ...
  private static class PrecedenceVerifyingCollaborator
      implements PrecedenceTestAspect.Collaborator {
    ...
    private void checkAdvice(String whatJustHappened) {
      // System.out.println("[" + adviceInvocationNumber + "] " + whatJustHappened + " ==> " +
      // EXPECTED[adviceInvocationNumber]);
      if (adviceInvocationNumber > (EXPECTED.length - 1)) {
        throw new AssertionError(
            "Too many advice invocations, expecting "
                + EXPECTED.length
                + " but had "
                + adviceInvocationNumber);
      }
