...
/**
 * Integration tests for AspectJ auto-proxying. Includes mixing with Spring AOP Advisors to
 * demonstrate that existing autoproxying contract is honoured.
 *
 * @author Rod Johnson
...
public class AspectJAutoProxyCreatorTests {
  ...
  public void testAspectsAndAdvisorAreAppliedEvenIfComingFromParentFactory() {
    ...
    childAc.registerBeanDefinition(
  ...
  protected void doTestAspectsAndAdvisorAreApplied(
      ApplicationContext ac, ITestBean shouldBeWeaved) {
    ...
    assertThat(AopUtils.isAopProxy(shouldBeWeaved))
        .as("Autoproxying must apply from @AspectJ aspect")
        .isTrue();
   ...
   * fileSuffix='foo.xml', this method will return 'AspectJAutoProxyCreatorTests-foo.xml'
   */
  ...
  private void assertStopWatchTimeLimit(final StopWatch sw, final long maxTimeMillis) {
    ...
    assertThat(totalTimeMillis < maxTimeMillis)
        .as(
            "'"
                + sw.getLastTaskName()
                + "' took too long: expected less than<"
                + maxTimeMillis
                + "> ms, actual<"
                + totalTimeMillis
                + "> ms.")
