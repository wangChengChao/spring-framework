...
public class JeeNamespaceHandlerTests {
  ...
  public void testWithReferencedEnvironment() throws Exception {
    BeanDefinition beanDefinition =
        this.beanFactory.getMergedBeanDefinition("withReferencedEnvironment");
    assertPropertyValue(
        beanDefinition, "jndiEnvironment", new RuntimeBeanReference("myEnvironment"));
  ...
  public void testSimpleLocalSlsb() throws Exception {
    ...
    assertThat(beanDefinition.getBeanClassName())
        .isEqualTo(LocalStatelessSessionProxyFactoryBean.class.getName());
  ...
  public void testComplexRemoteSlsb() throws Exception {
    ...
    assertPropertyValue(beanDefinition, "jndiEnvironment", "foo=bar");
    assertPropertyValue(
        beanDefinition, "homeInterface", "org.springframework.tests.sample.beans.ITestBean");
  ...
  private void assertPropertyValue(
      BeanDefinition beanDefinition, String propertyName, Object expectedValue) {
    assertThat(beanDefinition.getPropertyValues().getPropertyValue(propertyName).getValue())
        .as("Property '" + propertyName + "' incorrect")
