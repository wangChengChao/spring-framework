...
public class CacheTestUtils {
  ...
  public static void assertCacheMiss(Object key, Cache... caches) {
    for (Cache cache : caches) {
      assertThat(cache.get(key))
          .as("No entry in " + cache + " should have been found with key " + key)
          .isNull();
    }
  }
  ...
  public static void assertCacheHit(Object key, Object value, Cache... caches) {
    for (Cache cache : caches) {
      ...
      assertThat(wrapper)
          .as("An entry in " + cache + " should have been found with key " + key)
          .isNotNull();
      assertThat(wrapper.get())
          .as("Wrong value in " + cache + " for entry with key " + key)
          .isEqualTo(value);
    }
  }
}
