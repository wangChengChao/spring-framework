...
public class ResourceScriptSourceTests {
  ...
  @Test
  public void doesNotPropagateFatalExceptionOnResourceThatCannotBeResolvedToAFile()
      throws Exception {
  ...
  public void lastModifiedWorksWithResourceThatDoesNotSupportFileBasedReading() throws Exception {
    ...
    ResourceScriptSource scriptSource = new ResourceScriptSource(resource);
    assertThat(scriptSource.isModified())
        .as("ResourceScriptSource must start off in the 'isModified' state (it obviously isn't).")
        .isTrue();
    scriptSource.getScriptAsString();
    assertThat(scriptSource.isModified())
        .as(
            "ResourceScriptSource must not report back as being modified if the underlying File resource is not reporting a changed lastModified time.")
        .isFalse();
    // Must now report back as having been modified
    assertThat(scriptSource.isModified())
        .as(
            "ResourceScriptSource must report back as being modified if the underlying File resource is reporting a changed lastModified time.")
        .isTrue();
  }

  @Test
  public void lastModifiedWorksWithResourceThatDoesNotSupportFileBasedAccessAtAll()
      throws Exception {
    ...
    assertThat(scriptSource.isModified())
        .as(
            "ResourceScriptSource must not report back as being modified if the underlying File resource is not reporting a changed lastModified time.")
        .isFalse();
    // Must now continue to report back as not having been modified 'cos the Resource does not
    // support access as a File (and so the lastModified date cannot be determined).
    assertThat(scriptSource.isModified())
        .as(
            "ResourceScriptSource must not report back as being modified if the underlying File resource is not reporting a changed lastModified time.")
        .isFalse();
